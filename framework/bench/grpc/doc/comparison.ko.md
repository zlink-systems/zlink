...
- `zlink-c` 행과 언어 행이 **서로 다른 Core 바이너리**를 로드한다(`#308`). 실측으로
  **6% 차이**다 — `core/build` 600.2 KOPS 대 설치 패키지 636.6 KOPS(latency는 13.1 대
  8.1 ms). 언어 행은 모두 설치 패키지를 쓰는데 분모만 개발 빌드였으므로 이 표의
  formula 1 값은 **낙관적이다.** 통일 분모 기준으로는 C++ 약 0.607, .NET 약 0.284로
  통과선에서 더 멀어진다.
