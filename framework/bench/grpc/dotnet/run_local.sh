...
for impl in "${bench_implementations[@]}"; do
  ...
  for pattern in "${bench_patterns[@]}"; do
    for payload in "${bench_payloads[@]}"; do
      ...
      if [[ -n "${BENCH_DIAGNOSTIC_TARGET_DLL:-}" ]]; then
        [[ "${impl}" == zlink-dotnet && -f "${BENCH_DIAGNOSTIC_TARGET_DLL}" ]] || {
          echo 'Diagnostic baseline requires zlink-dotnet and an existing driver DLL.' >&2; exit 1;
        }
        DOTNET_gcServer=1 setsid dotnet "${BENCH_DIAGNOSTIC_TARGET_DLL}" \
          --role target --stage core --bench-baseline "${target_args[@]}" >"${target_log}" 2>&1 &
      else
      ...
      if [[ -n "${BENCH_DIAGNOSTIC_SOURCE_DLL:-}" ]]; then
        [[ "${impl}" == zlink-dotnet && -f "${BENCH_DIAGNOSTIC_SOURCE_DLL}" ]] || {
          echo 'Diagnostic source requires zlink-dotnet and an existing driver DLL.' >&2; exit 1;
        }
        setsid dotnet "${BENCH_DIAGNOSTIC_SOURCE_DLL}" "${source_args[@]}" >"${source_log}" 2>&1 &
      else
