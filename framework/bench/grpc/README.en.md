...
- gRPC and ZLink framework use the same protobuf DTO. The ZLink raw binding skips the framework but
  ...
  experiments. The reference implementation is `framework/bench/grpc/c`,
...
at a different scale, the shared aggregator (`framework/bench/grpc/tools/`) recovers that scale from
