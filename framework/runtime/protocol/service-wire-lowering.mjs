...
function readSchema(schemaPathOrObject) {
  if (typeof schemaPathOrObject === "string") {
    return JSON.parse(
      fs.readFileSync(path.resolve(schemaPathOrObject), "utf8"),
    );
  }
...
function lowerValue(value, model) {
  ...
  if (hasOwn(value, "$bound")) {
    ...
    if (resolved === null) {
      throw new LoweringCoverageError([
        `unresolved bound ${JSON.stringify(value.$bound)}`,
      ]);
    }
  ...
  if (hasOwn(value, "$ref") && resolveReference(value, model.types) === null) {
    throw new LoweringCoverageError([
      `unresolved type ${JSON.stringify(value.$ref)}`,
    ]);
  }
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [
      key,
      lowerValue(entry, model),
    ]),
  );
}
...
function fieldPresence(fieldName, fields, model) {
  ...
  const type =
    field === undefined
      ? null
      : resolveReference({ $ref: field.$ref }, model.types);
...
function fieldOperation(field, model, fields) {
  ...
  if (hasOwn(field, "constraints")) {
    operation.constraints = field.constraints.map((constraint) =>
      constraintOperation(
...
function vectorItemFieldType(owner, fieldName, model) {
  const itemReference =
    owner.item ?? owner.layout?.find((entry) => entry.kind === "repeat")?.item;
...
function comparisonPart(owner, fieldName, comparison, model) {
  const source =
    fieldName === null ? { kind: "item" } : fieldPathOperand(fieldName);
...
function comparisonKey(constraint, owner, model) {
  ...
  return {
    kind: "tuple",
    parts: vectorComparisonSources(constraint).map((fieldName) =>
      comparisonPart(owner, fieldName, constraint.comparison, model),
    ),
  };
}
...
function lowerConstraints(constraints, owner, model) {
  ...
  return constraints.map((constraint) => {
    ...
    if (constraint.comparison !== undefined)
      inheritedComparison = constraint.comparison;
    ...
    if (constraint.kind === "sorted" || constraint.kind === "unique") {
      ...
      node.key = comparisonKey(
        { ...constraint, comparison: effective },
        owner,
        model,
      );
    }
...
function lowerField(field, model, fields) {
  ...
  if (hasOwn(field, "constraints")) {
    node.constraints = field.constraints.map((constraint) =>
      lowerConstraint(constraint, { kind: "field" }, model),
    );
  }
  return node;
...
function typeOperations(type, node, model, runtimePredicates) {
  ...
  if (
    type.kind === "length-prefixed-bytes" ||
    type.kind === "length-prefixed-text"
  ) {
    const operations = [
      {
        op: "length-prefixed",
    ...
    if (hasOwn(node, "runtimeMaximumBytes")) {
      operations.push(
        negotiatedBoundOperation(node.runtimeMaximumBytes, "content-bytes"),
      );
    }
    if (type.kind === "length-prefixed-text") {
      operations.push({
    ...
    const operations = [
      {
        ...
        otherwise:
          type.otherwise === "protocol-error"
            ? { kind: "protocol-error" }
            : {
                kind: "fields",
                fields: fieldOperations(type.otherwise.fields, model),
              },
        encode: {
          selection: "variant",
  ...
  throw new LoweringCoverageError([
    `type:${type.name}: no operation lowering for ${type.kind}`,
  ]);
}
...
function lowerType(type, model, runtimePredicates) {
  ...
  const node = Object.fromEntries(
    keys
      .filter((key) => hasOwn(type, key))
      .map((key) => [key, lowerValue(type[key], model)]),
  );
  if (type.kind === "struct") {
    ...
    node.discriminators = type.discriminators.map((entry) =>
      lowerDiscriminator(entry, model),
    );
...
function logicalStreamDecode(format, loweredTypes) {
  ...
  const errors = [];
  if (
    format.chunkSplit !== "any-byte-boundary-including-within-frozen-record"
  ) {
    errors.push(
      "$.relocationLogicalStreamFormat.chunkSplit: unsupported incremental boundary",
    );
  }
  for (const [name, expected] of Object.entries(expectedReplay)) {
    if (format.replay?.[name] !== expected) {
      errors.push(
        `$.relocationLogicalStreamFormat.replay.${name}: expected ${JSON.stringify(expected)}`,
      );
    }
  }
  if (
    JSON.stringify(Object.keys(format.replay ?? {}).sort()) !==
    JSON.stringify(Object.keys(expectedReplay).sort())
  ) {
    errors.push(
      "$.relocationLogicalStreamFormat.replay: unsupported incremental contract key",
    );
  }
  if (errors.length > 0) throw new LoweringCoverageError(errors);

  const typesByName = new Map(loweredTypes.map((type) => [type.name, type]));
  ...
  const references = (value, result = new Set()) => {
    if (Array.isArray(value)) {
      value.forEach((entry) => references(entry, result));
    } else if (isObject(value)) {
      if (typeof value.$ref === "string") result.add(value.$ref);
      Object.values(value).forEach((entry) => references(entry, result));
    }
    return result;
  };
  const visit = (name) => {
    ...
    if (visiting.has(name)) {
      throw new LoweringCoverageError([
        `logical stream type graph is recursive at ${name}`,
      ]);
    }
    const owner = typesByName.get(name);
    if (!owner)
      throw new LoweringCoverageError([
        `logical stream type is unresolved: ${name}`,
      ]);
    visiting.add(name);
    reachableTypes.push(name);
    const rootOperation = owner.operations.find((operation) =>
      [
        "integer",
        "enum",
        "length-prefixed",
    ...
    if (!rootOperation) {
      throw new LoweringCoverageError([
        `logical stream type has no continuation frame: ${name}`,
      ]);
    }
    if (!frameKinds.includes(rootOperation.op))
      frameKinds.push(rootOperation.op);
    let depth = 1;
    for (const reference of references(rootOperation)) {
      depth = Math.max(depth, 1 + visit(reference));
    }
    visiting.delete(name);
    depths.set(name, depth);
    return depth;
  };
  const maximumDepth = visit(format.body.$ref);
  return {
    replay: lowerValue(format.replay),
    frames: {
      kinds: frameKinds,
      reachableTypes,
      maximumDepth,
...
function lowerLogicalStreamFormat(
  format,
  model,
  runtimePredicates,
  loweredTypes,
  ...
  node.operations = [
    {
      ...
      decode: logicalStreamDecode(format, loweredTypes),
    },
...
function assertCondition(condition, location, errors) {
  ...
  if (isObject(condition.fieldEquals)) {
    assertKeys(
      ...
      new Set(["name", "value"]),
      `${location}.fieldEquals`,
      errors,
    );
  }
  if (isObject(condition.contextEquals)) {
    assertKeys(
      condition.contextEquals,
      new Set(["name", "value"]),
      `${location}.contextEquals`,
      errors,
    );
  }
}
...
function assertBoundReferences(value, location, errors) {
  if (Array.isArray(value)) {
    value.forEach((entry, index) =>
      assertBoundReferences(entry, `${location}[${index}]`, errors),
    );
    return;
  }
  if (!isObject(value)) {
  ...
  if (type.kind === "struct") {
    ...
    type.discriminators.forEach((entry, discriminatorIndex) => {
      assertKeys(
        entry,
        DISCRIMINATOR_KEYS,
        `${location}.discriminators[${discriminatorIndex}]`,
        errors,
      );
    });
    ...
    if (isObject(type.otherwise)) {
      ...
      assertFields(
        type.otherwise.fields,
        `${location}.otherwise.fields`,
        errors,
      );
    }
  } else if (type.kind === "tlv32") {
...
function assertLoweringCoverage(schema, ir) {
  ...
  schema.types.forEach((type, index) =>
    assertTypeKeywords(type, index, errors),
  );
  schema.flags.forEach((flag, index) =>
    assertKeys(flag, FLAG_KEYS, `$.flags[${index}]`, errors),
  );
  ...
  schema.commands.forEach((command, index) => {
    ...
    assertConstraints(
      command.flagConstraints,
      FLAG_CONSTRAINT_KEYS,
      `${location}.flagConstraints`,
      errors,
    );
  });
  ...
  assertKeys(
    schema.relocationLogicalStreamFormat.generatedObjectTree,
    GENERATED_OBJECT_TREE_KEYS,
    "$.relocationLogicalStreamFormat.generatedObjectTree",
    errors,
  );
  ...
  const sourceCommands = schema.commands.map(
    (command) => `${command.id}:${command.name}`,
  );
  const loweredCommands = ir.commands.map(
    (command) => `${command.id}:${command.name}`,
  );
  if (JSON.stringify(loweredCommands) !== JSON.stringify(sourceCommands)) {
    errors.push(
      `$.commands: lowered command inventory differs from the schema`,
    );
  }
  ...
  if (ir.semanticConstraints.length !== schema.semanticConstraints.length) {
    errors.push(
      `$.semanticConstraints: lowered constraint inventory differs from the schema`,
    );
  }
  for (const [index, source] of schema.semanticConstraints.entries()) {
    ...
    if (JSON.stringify(lowered) !== JSON.stringify(source)) {
      errors.push(
        `$.semanticConstraints[${index}]: literal tuple did not reach the IR`,
      );
    }
  }
  if (
    JSON.stringify(ir.relocationStateMachine) !==
    JSON.stringify(schema.relocationStateMachine)
  ) {
  ...
  function inspectOperations(value, location) {
    if (Array.isArray(value)) {
      value.forEach((entry, index) =>
        inspectOperations(entry, `${location}[${index}]`),
      );
      return;
    }
    ...
    if (hasOwn(value, "op")) {
      if (!OPERATION_KIND_SET.has(value.op)) {
        errors.push(
          `${location}.op: unknown operation ${JSON.stringify(value.op)}`,
        );
      } else {
  ...
  for (const operation of OPERATION_KINDS) {
    if (!seenOperations.has(operation)) {
      errors.push(
        `$.operationVocabulary: operation ${operation} has no schema coverage`,
      );
    }
  }
  if (
    JSON.stringify(ir.operationVocabulary) !== JSON.stringify(OPERATION_KINDS)
  ) {
    errors.push(
      `$.operationVocabulary: closed vocabulary differs from the lowering owner`,
    );
  }
  const sourceUnions = schema.types.filter(
    (type) => type.kind === "conditional-union",
  );
  const loweredUnions = ir.types.filter(
    (type) => type.kind === "conditional-union",
  );
  ...
  for (const source of sourceUnions) {
    ...
    if (
      ...
      errors.push(
        `type:${source.name}: conditional-union cases or discriminators did not reach the IR`,
      );
      continue;
    }
    const unionOperation = lowered.operations.find(
    ...
    for (const sourceCase of source.cases) {
      ...
      const expectedConstraints = (
        lowered.cases[signature].constraints ?? []
      ).map((constraint) => ({
      ...
      const constraints = trailingOperations.filter(
      ...
      if (
        ...
        errors.push(
          `type:${source.name}: case ${signature} constraints did not reach ordered operations`,
        );
      }
    }
  }
  for (const source of schema.types.filter((type) =>
    hasOwn(type, "maximumEncodedBytes"),
  )) {
    const lowered = ir.types.find((type) => type.name === source.name);
    const limit = lowered.operations.find(
    ...
    if (
      ...
      errors.push(
        `type:${source.name}: complete encoded limit did not reach operations`,
      );
    }
  }
  for (const source of schema.types) {
    ...
    for (const [index, constraint] of (source.constraints ?? []).entries()) {
      ...
      if (["sorted", "unique"].includes(constraint.kind)) {
        ...
        if (
          candidate.comparison !== inheritedComparison ||
          candidate.key === undefined
        ) {
          errors.push(
            `type:${source.name}: ${constraint.kind} comparison key did not reach operations`,
          );
        }
      }
    }
    ...
    for (const field of fields.filter(
      (entry) => entry.when?.fieldPresent !== undefined,
    )) {
      const loweredField = (lowered.fields ?? lowered.body).find(
        (entry) => entry.name === field.name,
      );
      const atom = loweredField.when?.all.find(
        (entry) => entry.kind === "fieldPresent",
      );
      if (
        ...
        errors.push(
          `type:${source.name}.${field.name}: field presence meaning did not reach IR`,
        );
      }
    }
    if (source.kind === "length-prefixed-text") {
      const validation = lowered.operations.find(
      ...
      if (
        ...
        errors.push(
          `type:${source.name}: strict UTF-8 rules did not reach operations`,
        );
      }
    }
    if (
      source.kind === "length-prefixed-text" ||
      source.kind === "length-prefixed-bytes"
    ) {
      const lengthPrefixed = lowered.operations.find(
        (operation) => operation.op === "length-prefixed",
      );
      if (
        JSON.stringify(lengthPrefixed?.encodeCapacity) !==
        JSON.stringify(representationCapacity(lowered.maximumBytes))
      ) {
        errors.push(
          `type:${source.name}: representation capacity did not reach operations`,
        );
      }
    }
    if (source.kind === "conditional-union") {
      const union = lowered.operations.find(
        (operation) => operation.op === "conditional-union",
      );
      if (
        ...
        errors.push(
          `type:${source.name}: encode discriminator agreement did not reach operations`,
        );
      }
    }
  }
  for (const source of schema.types.filter(
    (type) =>
      hasOwn(type, "runtimeMaximumBytes") ||
      hasOwn(type, "runtimeMaximumEncodedBytes"),
  )) {
    const lowered = ir.types.find((type) => type.name === source.name);
    const runtimeMaximum =
      lowered.runtimeMaximumBytes ?? lowered.runtimeMaximumEncodedBytes;
    const negotiated = lowered.operations.find(
    ...
    if (
      JSON.stringify(negotiated?.context) !== JSON.stringify(expectedContext) ||
      JSON.stringify(negotiated?.applications) !==
        JSON.stringify(expectedApplications) ||
      negotiated.comparison !== "less-than-or-equal"
    ) {
      errors.push(
        `type:${source.name}: ClientServer negotiated bound did not reach operations`,
      );
    }
  }
  ...
  if (
    JSON.stringify(loweredDurableNames) !== JSON.stringify(sourceDurableNames)
  ) {
    errors.push(
      `$.durableFormats: lowered durable format inventory differs from the schema`,
    );
  }
  for (const source of schema.durableFormats) {
    const lowered = ir.durableFormats.find(
      (format) => format.name === source.name,
    );
    if (
      lowered === undefined ||
      ...
      errors.push(
        `durableFormat:${source.name}: envelope declaration did not reach the IR`,
      );
    }
    const checksumIndex = lowered?.operations.findIndex(
      (operation) => operation.op === "checksum",
    );
    ...
    if (
      checksumIndex < 0 ||
      bodyIndex <= checksumIndex ||
      lowered.operations[checksumIndex].verification !==
        "before-body-interpretation"
    ) {
      errors.push(
        `durableFormat:${source.name}: checksum must precede body interpretation`,
      );
    }
  }
  const sourceLogical = schema.relocationLogicalStreamFormat;
  const loweredLogical = ir.relocationLogicalStreamFormat;
  const logicalTreeMatches = Object.entries(
    sourceLogical.generatedObjectTree ?? {},
  ).every(
    ([name, reference]) =>
      loweredLogical?.generatedObjectTree?.[name]?.$ref === reference.$ref,
  );
  if (
    loweredLogical?.name !== sourceLogical.name ||
    ...
    errors.push(
      `$.relocationLogicalStreamFormat: logical stream declaration did not reach the IR`,
    );
  }
  if (errors.length > 0) {
    throw new LoweringCoverageError(errors);
...
function lowerSchema(schemaPathOrObject) {
  ...
  const loweredTypes = schema.types.map((type) =>
    lowerType(type, model, runtimePredicates),
  );
  const ir = {
      ...
      ...(constraint.kind === "terminal-failure-integrity"
        ? {
            runtimePredicate: {
              ...
              operation: runtimePredicateOperation(
                runtimePredicates[0].reference,
                constraint.fields,
              ),
            },
          }
    ...
    commands: schema.commands.map((command) =>
      lowerCommand(command, model, runtimePredicates),
    ),
    durableFormats: schema.durableFormats.map((format) =>
      lowerDurableFormat(format, model, runtimePredicates),
    ),
...
function runSelfTests(schemaPath) {
  ...
  const expected = [
    [
      "integer",
      "u8",
      (type) => [type.encoding, type.minimum, type.maximum],
      ["u8", 0, 255],
    ],
    [
      "enum",
    ...
    [
      "length-prefixed-bytes",
      "rid",
      (type) => [type.lengthType.$ref, type.minimumBytes, type.maximumBytes],
      ["u8", 1, 255],
    ],
    [
      "length-prefixed-text",
      ...
      ["u8", 255, "utf-8", "forbidden"],
    ],
    [
      "struct",
      "metadata-entry",
      (type) => type.fields.map((field) => field.$ref),
      ["text8", "metadata-value"],
    ],
    [
      "vector",
      "aggregate-participant-vector",
      (type) => [type.countType.$ref, type.maximumItems, type.item.$ref],
      ["u16", 1024, "maintenance-aggregate-participant-v1"],
    ],
    [
      "versioned-vector",
      "metadata-frame",
      (type) => [
        type.maximumEncodedBytes,
        type.layout[2].countFrom,
        type.layout[2].item.$ref,
      ],
      [1024, "count", "metadata-entry"],
    ],
    [
      "versioned-length-delimited",
    ...
    [
      "conditional-union",
      "actor-join-reply-tail",
      (type) => [
        type.discriminators[0].source.kind,
        Object.keys(type.cases),
        type.otherwise.kind,
      ],
      [
        "wire",
        ['{"joinResult":"accepted"}', '{"joinResult":"rejected"}'],
        "protocol-error",
      ],
    ],
    [
      ...
      (type) => [
        type.totalLengthType.$ref,
        type.fields[0].id,
        type.fields[0].required,
        type.maximumEncodedBytes,
      ],
      ["u32", 1, true, 1048576],
    ],
  ...
  const fieldPresent = optionalActor.fields.find(
    (field) => field.name === "generation",
  );
  ...
  const fieldEquals = creationTerminal.body.find(
  ...
  const commands = new Map(
    ir.commands.map((command) => [command.name, command]),
  );
  const actorSend = commands.get("actorSend");
  const allFlagsSet = actorSend.body.find(
    (field) => field.name === "boundSessionTail",
  );
  ...
  const anyFlagsCommand = anyFlagsSchema.commands.find(
    (command) => command.name === "actorSend",
  );
  anyFlagsCommand.body.at(-1).when = {
    anyFlagsSet: ["boundSession", "sourceSpotId"],
  };
  const anyFlagsIr = lowerSchema(anyFlagsSchema);
  const anyFlagsSet = anyFlagsIr.commands
    .find((command) => command.name === "actorSend")
    .body.at(-1);
  ...
  expectValidatorFailure(schema, (candidate) => {
    const type = candidate.types.find(
      (entry) => entry.name === "optional-actor-ref",
    );
    type.fields.find((field) => field.name === "generation").when.fieldPresent =
      "missing";
  });
  expectValidatorFailure(schema, (candidate) => {
    const type = candidate.types.find(
      (entry) => entry.name === "creation-operation-terminal-v1",
    );
    type.body.find(
      (field) => field.name === "creation",
    ).when.fieldEquals.value = "unknown";
  });
  expectValidatorFailure(schema, (candidate) => {
    const command = candidate.commands.find(
      (entry) => entry.name === "actorSend",
    );
  ...
  const schemaUnions = schema.types.filter(
    (type) => type.kind === "conditional-union",
  );
  ...
  for (const source of schemaUnions) {
    ...
    assert.deepEqual(
      Object.keys(lowered.cases),
      source.cases.map((entry) => conditionSignature(entry.when)),
    );
    assert.equal(
      lowered.bodyLengthType === null,
      !hasOwn(source, "bodyLengthType"),
    );
  ...
  assert.deepEqual(
    new Set(
      irUnions.flatMap((type) =>
        type.discriminators.map((entry) => entry.source.kind),
      ),
    ),
  ...
  assert(
    irUnions.some(
      (type) =>
        type.otherwise.kind === "fields" && type.otherwise.fields.length === 0,
    ),
  );
  const activationRecovery = types.get("authority-activation-recovery-state");
  const activationPresent =
    activationRecovery.cases['{"hasActivationRecovery":"true"}'];
  ...
  assert.deepEqual(
    activationRecovery.operations[0].cases[
  ...
  assert.deepEqual(
    types
      .get("application-payload-bytes")
      .operations.find((operation) => operation.op === "negotiated-bound"),
    {
      op: "negotiated-bound",
      topology: "clientServer",
      context: {
  ...
  assert.deepEqual(
    types
      .get("application-payload-envelope-v1")
      .operations.find((operation) => operation.op === "negotiated-bound"),
    {
      op: "negotiated-bound",
      topology: "clientServer",
  ...
  assert(
    ir.semanticContexts.every(
      (context) => context.parameter === "decoder-context",
    ),
  );
  ...
  requiredFlagSchema.commands
    .find((command) => command.name === "nodeSend")
    .requiredFlags.push("metadata");
  const requiredFlagIr = lowerSchema(requiredFlagSchema);
  assert.deepEqual(
    requiredFlagIr.commands.find((command) => command.name === "nodeSend")
      .requiredFlags,
    [flagOperand("metadata")],
  );
  const impliesSchema = structuredClone(schema);
  impliesSchema.commands
    .find((command) => command.name === "actorSend")
    .flagConstraints.push({
      kind: "implies",
      if: "boundSession",
      then: ["sourceSpotId"],
    });
  ...
  assert.deepEqual(
    ir.commands.find((command) => command.name === "instanceSpot")
      .semanticConstraints,
    schema.commands.find((command) => command.name === "instanceSpot")
      .semanticConstraints,
  );
  ...
  assert.deepEqual(
    types
      .get("aggregate-participant-vector")
  ...
  assert.deepEqual(
    types
      .get("aggregate-participant-vector")
      .constraints.map((entry) => entry.field),
    [fieldPathOperand("object"), fieldPathOperand("object")],
  );
  const authorityKeyConstraint = types.get("aggregate-participant-vector")
    .constraints[0];
  assert.equal(
    authorityKeyConstraint.key.kind,
  ...
  assert.deepEqual(types.get("sorted-text8-vector").constraints[0].key.parts, [
    {
      kind: "utf-8-bytes",
      source: { kind: "item" },
      lengthPrefix: "excluded",
  ...
  assert.deepEqual(
    types
      .get("stateful-capability-vector")
      .constraints[0].key.parts.map((part) => [
        part.kind,
        part.encoding ?? part.lengthPrefix,
      ]),
    [
      ["wire-value", "u8"],
      ["utf-8-bytes", "excluded"],
    ],
  );
  assert.deepEqual(
    types
      .get("saved-work-vector")
      .constraints[0].key.parts.map((part) => [
        part.kind,
        part.encoding,
        part.byteOrder,
  ...
  assert.deepEqual(
    types.get("metadata-frame").constraints[0].field,
    fieldPathOperand("key"),
  );
  assert.deepEqual(
    creationTerminal.constraints.map((entry) => entry.kind),
    [
  ...
  assert.deepEqual(
    descriptor.fields.find((field) => field.name === "protocolCapabilities")
      .constraints,
    [
      {
  ...
  const textValidation = types
    .get("text8")
    .operations.find((operation) => operation.op === "text-validation");
  ...
  comparisonSchema.types.find(
    (type) => type.name === "retired-bound-session-route-fence",
  ).constraints = [
    {
      kind: "field-less-than-or-equal",
  ...
  presenceSchema.types.find(
    (type) => type.name === "descriptor-extension",
  ).presenceRules = [
    {
      when: {
        contextEquals: { name: "durableRelocationPresent", value: true },
      },
  ...
  assert.deepEqual(
    presenceIr.types.find((type) => type.name === "descriptor-extension")
      .presenceRules,
    [
      {
        when: {
  ...
  assert.deepEqual(
    ir.durableFormats.map((format) => format.name),
    [
  ...
  const { operations: authorityOperations, ...authorityDeclaration } =
    ir.durableFormats[0];
  ...
  assert.deepEqual(
    authorityOperations.map((operation) => operation.op),
    ["durable-header", "bounded-reader", "encoded-limit", "checksum", "field"],
  );
  assert.equal(
    authorityOperations[3].verification,
    "before-body-interpretation",
  );
  assert.equal(
    authorityOperations[4].interpretation,
    "after-checksum-verification",
  );
  assert(
    ir.durableFormats.every(
      (format) =>
        format.checksum.algorithm === "crc32c-castagnoli" &&
        format.checksum.position === "trailing",
    ),
  );
  const { operations: logicalOperations, ...logicalDeclaration } =
  ...
  assert.deepEqual(
    logicalOperations.map((operation) => operation.op),
    ["logical-stream", "encoded-limit"],
  );
  assert.deepEqual(logicalOperations[0].decode, {
    replay: schema.relocationLogicalStreamFormat.replay,
    frames: {
      kinds: [
        "struct",
        "integer",
      ...
      reachableTypes: logicalOperations[0].decode.frames.reachableTypes,
      maximumDepth: 12,
    },
  });
  assert.equal(
    new Set(logicalOperations[0].decode.frames.reachableTypes).size,
    logicalOperations[0].decode.frames.reachableTypes.length,
  );
  for (const [name, value] of Object.entries({
    mode: "unknown-mode",
    wholeStreamInputAllocation: "allowed",
    completion: "success-before-final",
    incompleteFinalChunk: "accept",
    bytesAfterRoot: "ignore",
  })) {
    const invalidReplay = structuredClone(schema);
    invalidReplay.relocationLogicalStreamFormat.replay[name] = value;
    assert.throws(() => lowerSchema(invalidReplay));
  }
  expectValidatorFailure(schema, (candidate) => {
    candidate.durableFormats[0].checksum.algorithm = "unknown-checksum";
  });
  expectValidatorFailure(schema, (candidate) => {
    candidate.relocationLogicalStreamFormat.generatedObjectTree.savedWork.$ref =
      "undefined-type";
  });
  ...
  assert.throws(
    () => lowerSchema(unknownDurableKeyword),
  ...
  assert.equal(
    ir.semanticConstraints.filter((constraint) => constraint.runtimePredicate)
      .length,
    1,
  );
  assert.equal(
    ir.semanticConstraints.find((constraint) => constraint.runtimePredicate)
      .kind,
  ...
  assert.deepEqual(
    terminalConstraint.runtimePredicate.targets,
    schema.semanticConstraints
      .find((constraint) => constraint.kind === "terminal-failure-integrity")
      .fields.map(fieldPathOperand),
  );
  assert.deepEqual(terminalConstraint.runtimePredicate.operation.reference, {
    asset: "service-wire-constants",
    name: "valid-terminal-failure",
  });
...
if (process.argv[1] && scriptPath === path.resolve(process.argv[1])) {
  ...
  if ((!selfTest && !check) || argumentsWithoutNode.length > optionOffset + 1) {
    console.error(
      "usage: service-wire-lowering.mjs --check|--self-test [schema-path]",
    );
    process.exit(2);
  }
