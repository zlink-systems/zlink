    ...
    for (const operation of type.operations) {
      if (operation.op === "integer" || operation.op === "enum") {
        ...
        if (!new Set(["bytes", "text"]).has(operation.content)
            || operation.encodeCapacity.throughMaximumBytes !== operation.maximumBytes
            || operation.encodeCapacity.implementationLimitBelowMaximum !== "forbidden") fail(type.name, operation, "content or encode capacity");
