...
function predicates(owner,value){const a=[];for(const p of owner.operations.filter(x=>x.op==="runtime-predicate")){seen.add(p.op);if(p.reference.asset!=="service-wire-constants"||p.reference.name!=="valid-terminal-failure")throw Error(`predicate ${p.reference.name}`);if(main(owner).op==="conditional-union"){const i=cases(main(owner)).findIndex(([,c])=>caseFields(c).some(f=>f.name==="failureCode"));if(i<0)throw Error(`${owner.name}: predicate target`);a.push(`if (${value} is ${id(owner.name)}Case${i} terminal) ValidateTerminalFailure(terminal.TerminalResult, terminal.FailureCode);`);}else a.push(`ValidateTerminalFailure(${value}.TerminalResult, ${value}.FailureCode);`);}return a;}
      ...
      if (operation.op === "integer" || operation.op === "enum") {
        ...
        for (const selected of Object.values(operation.cases)) {
          for (const selectedOperation of selected.operations) {
            ...
            else if (selectedOperation.op !== "constraint") fail(type.name, selectedOperation, "union case operation");
          }
        }
...
if(o.op==="integer"){read=[`var raw = reader.${o.encoding.toUpperCase()}();`,`if (raw < ${lit(o.minimum,o.encoding)} || raw > ${lit(o.maximum,o.encoding)}) throw Error("${t.name}: range");`,`return new(raw);`];write=[`if (value.Value < ${lit(o.minimum,o.encoding)} || value.Value > ${lit(o.maximum,o.encoding)}) throw Error("${t.name}: range");`,`writer.${o.encoding.toUpperCase()}(value.Value);`];}
  ...
  cases(o).forEach(([sig,v],i)=>{const s=JSON.parse(sig),test=disc.map(d=>`${id(d.name)} == ${expected(d.type,s[d.name])}`).join(" && "),rf=readFields(caseFields(v),"selected"),cc=constraints(t,"caseValue",v.operations,sig),args=[...disc.filter(d=>d.source.kind==="wire").map(d=>id(d.name)),rf.args].filter(Boolean).join(", ");caseHelpers.push(...cc.helpers);read.push(`${i?"else ":""}if (${test})`,"{",...rf.lines.map(x=>`    ${x}`),...(o.reader?[`    selected.End("${t.name}");`]:[]),`    var caseValue = new ${n}Case${i}(${args});`,...cc.lines.map(x=>`    ${x}`),`    ${n} value = caseValue;`,...pred.map(x=>`    ${x}`),"    return value;","}");});
