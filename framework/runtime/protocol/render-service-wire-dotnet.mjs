...
const caseFields = selected => selected.operations.filter(operation => operation.op === "field");
const unionFields = (o, selected) => [...o.discriminators.filter(d=>d.source.kind==="wire").map(d=>({name:d.name,type:d.type})),...caseFields(selected)];
...
case"conditional-union":{const d=cases(o).map(([,v],i)=>`internal sealed record ${n}Case${i}(${fieldsDecl(unionFields(o,v))}) : ${n};`);if(o.otherwise.kind==="fields")d.push(`internal sealed record ${n}Otherwise(${fieldsDecl(o.otherwise.fields)}) : ${n};`);return`internal abstract record ${n};\n${d.join("\n")}`;}
...
function constraints(owner, value="value", selectedOperations=null, caseSignature=null) {
  const o=main(owner), operations=selectedOperations??(o.constraints??[]), fields=selectedOperations?caseFields({operations:selectedOperations}):(o.fields??[]), lines=[], helpers=[], ownerFields=new Map(fields.map(field=>[field.name,field]));
  const equals=(name,expectedValue)=>`${value}.${id(name)} == ${expected(ownerFields.get(name).type,expectedValue)}`;
  operations.filter(operation=>operation.op==="constraint").forEach((constraint,index)=>{
    seen.add(constraint.op);
    if(caseSignature!==null){const expectedPath=["types",owner.name,"cases",caseSignature];if(constraint.owner?.kind!=="conditional-union-case"||JSON.stringify(constraint.owner.path)!==JSON.stringify(expectedPath))throw Error(`${owner.name}: constraint owner`);}
    else if(constraint.owner!==undefined)throw Error(`${owner.name}: unexpected constraint owner`);
    if(constraint.kind==="not-both-zero") lines.push(`if (${constraint.fields.map(field=>`${value}.${id(field.name)}.Value == 0`).join(" && ")}) throw Error("not-both-zero");`);
      ...
      const property=o.op==="vector"?"Items":id(o.layout.find(x=>x.kind==="repeat").name), item=o.op==="vector"?o.item:o.layout.find(x=>x.kind==="repeat").item, key=`Key${id(owner.name)}${index}`, body=[];
      const resolve=(reference,expression,pathValue)=>{for(const part of pathValue?pathValue.split("."):[]){const field=main(types.get(reference.$ref)).fields.find(entry=>entry.name===part);reference=field.type;expression+=`.${id(part)}`;}return{reference,expression};};
      if(constraint.key.kind==="tuple"){
        body.push("var writer = new Writer();");
        for(const part of constraint.key.parts){const source=part.source,found=resolve(item,"item",source.kind==="item"?"":source.path);if(part.kind==="utf-8-bytes"){if(part.lengthPrefix!=="excluded")throw Error(`${owner.name}: utf8 key prefix`);body.push(`writer.Bytes(Utf8.GetBytes(${found.expression}.Value!));`);}else{const encoding=main(types.get(found.reference.$ref)).encoding;if(!new Set(["wire-value","unsigned-wire-value"]).has(part.kind)||part.encoding!==encoding||part.byteOrder!=="big-endian")throw Error(`${owner.name}: wire key`);body.push(`${writeCall(found.reference,"writer",found.expression,new Map())};`);}}
        body.push("return writer.ToArray();");
      }else if(constraint.key.kind==="canonical-authority-key-bytes"){
        const source=resolve(item,"item",constraint.key.source.path),union=types.get(source.reference.$ref),uo=main(union),format=constraint.key.format;if(format.encoding!=="canonical-ascii-utf8"||format.componentLayout!=="decimal-raw-byte-length-colon-percent-encoded-bytes"||format.escaping!=="rfc3986-unreserved-literal-otherwise-uppercase-percent-hex"||format.unicodeNormalization!=="none")throw Error(`${owner.name}: authority key format`);const arms=[];
        for(const [variantName,variant] of Object.entries(constraint.key.variants)){const caseIndex=cases(uo).findIndex(([signature])=>JSON.parse(signature)[uo.discriminators[0].name]===variantName);if(caseIndex<0)throw Error(`${owner.name}: authority variant ${variantName}`);const selected=cases(uo)[caseIndex][1],kind=format.kindDiscriminators.find(x=>x.objectKind===variant.objectKind),components=[];for(const component of variant.components){const parts=component.split(".");let reference=caseFields(selected).find(field=>field.name===parts[0]).type,expression=`selected.${id(parts[0])}`;for(const part of parts.slice(1)){const field=main(types.get(reference.$ref)).fields.find(x=>x.name===part);reference=field.type;expression+=`.${id(part)}`;}components.push(`AuthorityComponent(${expression}.Value!)`);}arms.push(`${id(union.name)}Case${caseIndex} selected => Utf8.GetBytes(${JSON.stringify(`${format.prefix}${format.separator}${kind.wire}${format.separator}`)} + ${components.join(` + ${JSON.stringify(format.separator)} + `)})`);}
        body.push(`return ${source.expression} switch { ${arms.join(", ")}, _ => throw Error("authority key variant") };`);
      }else throw Error(`${owner.name}: key ${constraint.key.kind}`);
      helpers.push(`    private static byte[] ${key}(${typeOf(item)} item, DecodeContext context)\n    {\n        ${body.join("\n        ")}\n    }`);lines.push(`var keys${index} = ${value}.${property}.Select(item => ${key}(item, context)).ToArray();`,constraint.kind==="sorted"?`if (keys${index}.Zip(keys${index}.Skip(1), (left, right) => CompareBytes(left, right) <= 0).Any(ok => !ok)) throw Error("unsorted vector");`:`if (keys${index}.Select(Convert.ToHexString).Distinct(StringComparer.Ordinal).Count() != keys${index}.Length) throw Error("duplicate vector item");`);
...
function predicates(owner,value,operations=owner.operations){const a=[];for(const p of operations.filter(x=>x.op==="runtime-predicate")){seen.add(p.op);if(p.reference.asset!=="service-wire-constants"||p.reference.name!=="valid-terminal-failure")throw Error(`predicate ${p.reference.name}`);if(main(owner).op==="conditional-union"&&operations===owner.operations){const i=cases(main(owner)).findIndex(([,c])=>caseFields(c).some(f=>f.name==="failureCode"));if(i<0)throw Error(`${owner.name}: predicate target`);a.push(`if (${value} is ${id(owner.name)}Case${i} terminal) ValidateTerminalFailure(terminal.TerminalResult, terminal.FailureCode);`);}else a.push(`ValidateTerminalFailure(${value}.TerminalResult, ${value}.FailureCode);`);}return a;}
...
function assertSyntaxCoverage() {
  ...
  for (const type of ir.types) {
    for (const operation of type.operations) {
      if (operation.op === "integer" || operation.op === "enum") {
        ...
        if (!new Set(["bytes", "text"]).has(operation.content)
            ...
            || capacity.aboveRepresentationCeiling !== "capacity-error") fail(type.name, operation, "content or encode capacity");
      } else if (operation.op === "text-validation") {
        if (operation.encoding !== "utf-8" || operation.malformed !== "protocol-error" || operation.nul !== "forbidden"
            || operation.decode.bom !== "preserve" || operation.decode.overlong !== "protocol-error"
            || operation.decode.surrogateCodePoint !== "protocol-error" || operation.encode.loneSurrogate !== "protocol-error") fail(type.name, operation, "text policy");
        ...
        if (operation.encode.selection !== "variant" || operation.encode.discriminatorAgreement !== "required" || operation.encode.mismatch !== "protocol-error") fail(type.name, operation, "encode agreement");
        for (const selected of Object.values(operation.cases)) {
          for (const selectedOperation of selected.operations) {
            if (selectedOperation.op === "field") inspectField(type.name, selectedOperation);
            else if (!new Set(["constraint", "runtime-predicate"]).has(selectedOperation.op)) fail(type.name, selectedOperation, "union case operation");
          }
        }
        ...
        if (operation.encodingOrder !== "ascending-field-id" || operation.duplicateField !== "protocol-error" || operation.unknownField !== "skip-after-length-validation"
            || JSON.stringify(operation.requiredFields) !== JSON.stringify(operation.fields.filter(field=>field.required).map(field=>field.name))) fail(type.name, operation, "TLV policy");
      } else if (operation.op === "encoded-limit") {
        if (operation.measured !== "complete-encoded-value" || operation.applications.join(",") !== "encode,decode" || operation.exceeded !== "protocol-error"
            || !operation.includes.includes("body") && !operation.includes.includes("fields")) fail(type.name, operation, "encoded limit policy");
        ...
        if (operation.topology !== "clientServer"
            ...
            || operation.applications.encode.context.name !== operation.context.name
            || operation.applications.decode.context.name !== operation.context.name
            || !new Set(["content-bytes", "encoded-bytes"]).has(operation.measured)
            || operation.comparison !== "less-than-or-equal") fail(type.name, operation, "negotiated bound policy");
  ...
  for (const format of ir.durableFormats) {
    ...
    if (checksum.algorithm !== "crc32c-castagnoli" || checksum.encoding !== "u32-big-endian" || checksum.coverage !== "magic-through-body" || checksum.position !== "trailing" || checksum.mismatch !== "protocol-error" || checksum.verification !== "before-body-interpretation") fail(format.name, checksum, "checksum policy");
    const body = op(format, "field");
    if (body.name !== "body" || body.interpretation !== "after-checksum-verification") fail(format.name, body, "body interpretation");
  }
  const logical = op(ir.relocationLogicalStreamFormat, "logical-stream"), logicalLimit = op(ir.relocationLogicalStreamFormat, "encoded-limit");
  ...
  if (logicalLimit.measured !== "complete-encoded-value" || logicalLimit.includes.join(",") !== "logical-stream-bytes"
      || logicalLimit.applications.join(",") !== "encode,decode" || logicalLimit.exceeded !== "protocol-error") fail(ir.relocationLogicalStreamFormat.name, logicalLimit, "logical encoded limit");
}
...
if(o.op==="integer"){read=[`var raw = reader.${o.encoding.toUpperCase()}();`,`if (raw < ${lit(o.minimum,o.encoding)} || raw > ${lit(o.maximum,o.encoding)}) throw Error("${t.name}: range");`,`return new(raw);`];write=[`if (value.Value < ${lit(o.minimum,o.encoding)} || value.Value > ${lit(o.maximum,o.encoding)}) throw Error("${t.name}: range");`,`writer.${o.encoding.toUpperCase()}(value.Value);`];}
  ...
  const text=t.operations.find(x=>x.op==="text-validation");if(text)seen.add(text.op);const lr=o.lengthType,capacity=o.encodeCapacity;
  read=[`var encodedLength = (ulong)Read${rn(lr)}(reader, context).Value;`,`if (encodedLength > ${lit(capacity.representationCeiling)}) throw Capacity("${t.name}: capacity");`,`if (encodedLength < ${lit(o.minimumBytes)} || encodedLength > ${lit(capacity.declaredMaximumBytes)}) throw Error("${t.name}: length");`,`var length = checked((int)encodedLength);`,`var bytes = reader.Bytes(length);`,o.zeroLengthMeaning?`if (length == 0) return new(null);`:"",o.content==="text"?`if (bytes.Contains((byte)0)) throw Error("${t.name}: NUL");\n        return new(Utf8.GetString(bytes));`:`return new(bytes);`].filter(Boolean);
  write=[`var bytes = ${o.content==="text"?"value.Value is null ? Array.Empty<byte>() : Utf8.GetBytes(value.Value)":"value.Value ?? Array.Empty<byte>()"};`,o.content==="text"?`if (bytes.Contains((byte)0)) throw Error("${t.name}: NUL");`:"",`if ((ulong)bytes.LongLength > ${lit(capacity.representationCeiling)}) throw Capacity("${t.name}: capacity");`,`if ((ulong)bytes.LongLength < ${lit(o.minimumBytes)} || (ulong)bytes.LongLength > ${lit(capacity.declaredMaximumBytes)}) throw Error("${t.name}: length");`,`Write${rn(lr)}(writer, new ${rn(lr)}(checked((${intType(main(types.get(lr.$ref)).encoding)})bytes.Length)), context);`,`writer.Bytes(bytes);`].filter(Boolean);
}
  ...
  const disc=o.discriminators, caseHelpers=[];
  ...
  cases(o).forEach(([sig,v],i)=>{const s=JSON.parse(sig),test=disc.map(d=>`${id(d.name)} == ${expected(d.type,s[d.name])}`).join(" && "),rf=readFields(caseFields(v),"selected"),cc=constraints(t,"caseValue",v.operations,sig),cp=predicates(t,"caseValue",v.operations),args=[...disc.filter(d=>d.source.kind==="wire").map(d=>id(d.name)),rf.args].filter(Boolean).join(", ");caseHelpers.push(...cc.helpers);read.push(`${i?"else ":""}if (${test})`,"{",...rf.lines.map(x=>`    ${x}`),...(o.reader?[`    selected.End("${t.name}");`]:[]),`    var caseValue = new ${n}Case${i}(${args});`,...cc.lines.map(x=>`    ${x}`),...cp.map(x=>`    ${x}`),`    ${n} value = caseValue;`,...pred.map(x=>`    ${x}`),"    return value;","}");});
  ...
  cases(o).forEach(([sig,v],i)=>{const s=JSON.parse(sig),cc=constraints(t,"item",v.operations,sig),cp=predicates(t,"item",v.operations);write.push(`case ${n}Case${i} item:`,"{",...cc.lines.map(x=>`    ${x}`),...cp.map(x=>`    ${x}`));for(const d of disc){if(d.source.kind==="wire"){write.push(`    if (item.${id(d.name)} != ${expected(d.type,s[d.name])}) throw Error("${t.name}: discriminator");`,`    ${writeCall(d.type,"writer",`item.${id(d.name)}`,new Map())};`);}else{const actual=d.source.kind==="context"?`(context.${id(d.source.name)} ?? throw Error("missing context ${d.source.name}"))`:`enclosing${id(d.source.name)}`;write.push(`    if (${actual} != ${expected(d.type,s[d.name])}) throw Error("${t.name}: discriminator");`);}}const wf=writeFields(caseFields(v),o.reader?"body":"writer","item");if(o.reader)write.push("    var body = new Writer();",...wf.lines.map(x=>`    ${x}`),`    Write${rn(o.bodyLengthType)}(writer, new ${rn(o.bodyLengthType)}(checked((${intType(main(types.get(o.bodyLengthType.$ref)).encoding)})body.Length)), context);`,"    writer.Bytes(body.ToArray());");else write.push(...wf.lines.map(x=>`    ${x}`));write.push("    break;","}");});
  ...
  write.push(`default: throw Error("${t.name}: variant");`,"}",...pred);c.helpers.push(...caseHelpers);
}
else if(o.op==="tlv32") {
  ...
  const ordered = [...o.fields].sort((left, right) => left.id - right.id), required=new Set(o.requiredFields);
  ...
  for (const name of o.requiredFields) read.push(`if (${id(name)} is null) throw Error("${name}: required");`);
  read.push(...tlvPresence(o, readEnvironment), `var value = new ${n}(${o.fields.map(f=>`${id(f.name)}${required.has(f.name)?"!":""}`).join(", ")});`, "return value;");
  ...
  for (const f of ordered) {
    ...
    if(required.has(f.name)) write.push(`if (${x} is null) throw Error("${f.name}: required");`);
...
const encodedLimit=t.operations.find(operation=>operation.op==="encoded-limit");if(encodedLimit)seen.add(encodedLimit.op);
const negotiated=t.operations.find(operation=>operation.op==="negotiated-bound");if(negotiated)seen.add(negotiated.op);
...
let readBody=read;
if(negotiated){const name=negotiated.context.name,maximum=`context.${id(name)} ?? throw Error("missing context ${name}")`,measured=negotiated.measured==="content-bytes"?"bytes.LongLength":"negotiatedStart - reader.Remaining",check=`if (negotiatedMaximum < 0 || negotiatedMaximum > ${lit(negotiated.context.absoluteMaximum,"i64")} || (ulong)(${measured}) > (ulong)negotiatedMaximum) throw Error("${t.name}: negotiated bound");`;readBody=[`var negotiatedMaximum = ${maximum};`,...(negotiated.measured==="encoded-bytes"?["var negotiatedStart = reader.Remaining;"]:[]),...readBody.map(line=>line.replace(/return /g,`${check} return `))];}
if(max!==undefined)readBody=["var encodedStart = reader.Remaining;",...readBody.map(line=>line.replace(/return /g,`${limitCheck} return `))];
let writeBody=write;
if(negotiated){const name=negotiated.context.name,maximum=`context.${id(name)} ?? throw Error("missing context ${name}")`,measured=negotiated.measured==="content-bytes"?"bytes.LongLength":"writer.Length - negotiatedStart",check=`if (negotiatedMaximum < 0 || negotiatedMaximum > ${lit(negotiated.context.absoluteMaximum,"i64")} || (ulong)(${measured}) > (ulong)negotiatedMaximum) throw Error("${t.name}: negotiated bound");`;writeBody=[`var negotiatedMaximum = ${maximum};`,...(negotiated.measured==="encoded-bytes"?["var negotiatedStart = writer.Length;"]:[]),...writeBody,check];}
if(max!==undefined)writeBody=["var encodedStart = writer.Length;",...writeBody,`if ((ulong)(writer.Length - encodedStart) > ${lit(max)}) throw Error("${t.name}: encoded limit");`];
