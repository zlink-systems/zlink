...
const negotiatedContexts = [...new Set(ir.types.flatMap(type => type.operations
  ...
  .map(operation => operation.maximum.name)))];
const seen = new Set();
  ...
  for (const type of ir.types) {
    for (const operation of type.operations) {
      if (operation.op === "integer" || operation.op === "enum") {
        ...
        if (operation.topology !== "clientServer" || operation.maximum.kind !== "decoder-context"
            || !new Set(["content-bytes", "encoded-bytes"]).has(operation.measured)
            || operation.comparison !== "less-than-or-equal" || operation.direction !== "decode") fail(type.name, operation, "negotiated bound policy");
...
let readBody=read;
if(negotiated){const maximum=`context.${id(negotiated.maximum.name)} ?? throw Error("missing context ${negotiated.maximum.name}")`,measured=negotiated.measured==="content-bytes"?"bytes.LongLength":"negotiatedStart - reader.Remaining",check=`if (negotiatedMaximum > ${lit(negotiated.absoluteMaximum)} || (ulong)(${measured}) > negotiatedMaximum) throw Error("${t.name}: negotiated bound");`;readBody=[`var negotiatedMaximum = ${maximum};`,...(negotiated.measured==="encoded-bytes"?["var negotiatedStart = reader.Remaining;"]:[]),...readBody.map(line=>line.replace(/return /g,`${check} return `))];}
