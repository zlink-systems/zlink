...
case"conditional-union":{const d=cases(o).map(([,v],i)=>`internal sealed record ${n}Case${i}(${fieldsDecl(caseFields(v))}) : ${n};`);if(o.otherwise.kind==="fields")d.push(`internal sealed record ${n}Otherwise(${fieldsDecl(o.otherwise.fields)}) : ${n};`);return`internal abstract record ${n};\n${d.join("\n")}`;}
  ...
  for (const type of ir.types) {
    for (const operation of type.operations) {
      if (operation.op === "integer" || operation.op === "enum") {
        ...
        if (operation.boundary !== "complete-value" || operation.trailingBytes !== "forbidden") fail(type.name, operation, "encoded limit policy");
...
if(o.op==="integer"){read=[`var raw = reader.${o.encoding.toUpperCase()}();`,`if (raw < ${lit(o.minimum,o.encoding)} || raw > ${lit(o.maximum,o.encoding)}) throw Error("${t.name}: range");`,`return new(raw);`];write=[`if (value.Value < ${lit(o.minimum,o.encoding)} || value.Value > ${lit(o.maximum,o.encoding)}) throw Error("${t.name}: range");`,`writer.${o.encoding.toUpperCase()}(value.Value);`];}
  ...
  cases(o).forEach(([sig,v],i)=>{const s=JSON.parse(sig),test=disc.map(d=>`${id(d.name)} == ${expected(d.type,s[d.name])}`).join(" && "),rf=readFields(caseFields(v),"selected"),cc=constraints(t,"caseValue",v.operations,sig);caseHelpers.push(...cc.helpers);read.push(`${i?"else ":""}if (${test})`,"{",...rf.lines.map(x=>`    ${x}`),...(o.reader?[`    selected.End("${t.name}");`]:[]),`    var caseValue = new ${n}Case${i}(${rf.args});`,...cc.lines.map(x=>`    ${x}`),`    ${n} value = caseValue;`,...pred.map(x=>`    ${x}`),"    return value;","}");});
