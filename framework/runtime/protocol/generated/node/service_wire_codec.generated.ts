...
export type ServiceWireDecoderContext = Readonly<{ originalOperationKind?: MeshOperationKind; durableRelocationPresent?: boolean; applicationSnapshotPresent?: boolean; readonly effectiveCompleteMessageBytesMinusActualEnvelopeOverhead?: number | bigint; readonly effectiveCompleteMessageBytes?: number | bigint; }>;
...
return value; }
export function encodeReplyCommand(value: ReplyCommand, context: ServiceWireDecoderContext): readonly Uint8Array[] { if ((value.flags & ~0) !== 0 || (value.flags & 0) !== 0) fail("reply flags"); if (!(validTerminalFailure(enumWireRequestTerminalResult(value["terminalResult"]), enumWireFrameworkErrorCode(value["failureCode"])))) fail("reply runtime predicate"); const writer = new Writer(); writer.put([90,77, 1, 20, value.flags]);
  if (value["correlation"] === undefined) fail("correlation required");
...
return value; }
export function encodeReplyRelayCommand(value: ReplyRelayCommand, context: ServiceWireDecoderContext): readonly Uint8Array[] { if ((value.flags & ~0) !== 0 || (value.flags & 0) !== 0) fail("replyRelay flags"); if (!(validTerminalFailure(enumWireRequestTerminalResult(value["terminalResult"]), enumWireFrameworkErrorCode(value["failureCode"])))) fail("replyRelay runtime predicate"); const writer = new Writer(); writer.put([90,77, 1, 33, value.flags]);
  if (value["operation"] === undefined) fail("operation required");
