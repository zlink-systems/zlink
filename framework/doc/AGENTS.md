...
- server 공개 계약: `framework/common/spec/server/`
- server runtime 공통 내부 설계: `framework/common/spec/server/40-internal-*.md`부터
...
- 특정 언어에만 해당하는 내부 구현 설명: `framework/<lang>/internals/`. 여러 언어가 공유하는 내부
  결정은 `framework/common/spec/server/40-internal-*.md`부터 `52-internal-*.md`까지가 소유한다.
