...
|  |  | Message Follow local native route가 committed source로 남은 동안에도 coordinator relay를 선택하도록 `spot_runtime`을 보강했고, relay evidence에 correlation을 기록하도록 했다. ST-F4 실행에서 G1 `message_follow_relay`와 source route removal은 관측됐지만, G2 old-ref assertion은 현재 public `actor_client_t` surface로 표현할 수 없어 종결하지 않는다. |
