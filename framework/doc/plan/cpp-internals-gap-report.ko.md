...
| CPP-CONTRACT-ROLE-001 | GAP·중 | Server role만 등록한 `ChannelName`의 send/request는 `NotConfigured`여야 하지만 client runtime record가 없으면 `select_ready`가 `NotFound`를 반환한다. Target이 없을 때의 `NotFound`와 local Client role 미등록을 분리하지 못한다. 증거: `common/spec/09-client-server-channel.ko.md:57-66`, `cpp/src/runtime/client_server/client_server_location_runtime.cpp:646-670, 1385-1419` |
| CPP-CONTRACT-HTTP-001 | GAP·중 | exact interface는 snapshot 생성과 validation을 host startup 내부 책임으로 두지만 `http_options_builder_t::snapshot()`과 `validate()`가 public member로 export된다. 기능 삭제가 아니라 host가 같은 검증을 내부 경로로 호출하도록 가시성 경계를 옮겨야 한다. 증거: `common/spec/server/languages/cpp/interfaces/02-configuration-host.ko.md:629-630`, `cpp/include/zlink/framework/contracts/http/http.hpp:245-347` |
...
| ID | 분류 | 요약 |
|---|---|---|
| CPP-COMP-001 | GAP·상 | §6 "문자열로 실패를 분류하지 말라" 위반: `is_moving_stale`이 `what()`에서 `"transfer is in progress"` 부분 문자열을 검색해 재시도를 결정(코드 주석이 "retriable 플래그가 reply를 못 살아남는다"고 자인), `map_native_exception`은 `"not connected"`, `"stale"`, `"errno=113"` 등 문자열 매칭으로 오류 종류를 결정. 메시지 문구 하나 바뀌면 relocation 재시도 루프가 조용히 깨짐. Node에서는 유사 은닉 재시도가 `NODE-ROUTE-001`로 **제거**되어 종결됐는데 C++은 아직 문자열 키 재시도를 유지 — 크로스 언어 일관성 문제로 기록 필요. 증거: `cpp/src/runtime/actors/actor_client.cpp:513-521, 881-905`, 문자열 생산처 `cpp/src/runtime/spots/spot_runtime.cpp:5396-5397` |
...
| ID | 분류 | 요약 |
|---|---|---|
| CPP-FOLLOW-001 | GAP·중 | §3 스팬 ④ 오류 종류 표 위반: Message Follow 획득 실패(라우트 만료·generation 불일치·hop≥8·16 MiB 초과·물량 한도)가 전부 단일 `nullopt` → `unavailable` 하나로 축약. 스테일 fence도 `invalid_operation`이 아닌 `unavailable`. 루프 명시 탐지 없음(8-hop 캡으로만 절단). 호출자가 "다른 곳 재시도/참조 갱신/백오프"를 구분할 수 없음. 증거: `cpp/src/runtime/spots/actor_transfer_coordinator.cpp:240-268`, `cpp/src/runtime/mesh/mesh_node_runtime.cpp:2233-2243`, `cpp/src/runtime/spots/spot_runtime.cpp:7396-7408` |
...
| CPP-FOLLOW-003 | PARTIAL·중 | §4 "같은 restore 요청 2회 수신 = 1회와 동일 결과" 위반: finalize는 재실행 가능하나 **prepare/admission 반복 수신은 `protocol_error`로 거부** — prepare 응답 유실 후 소스 재전송이 첫 전달과 다른 결과를 관찰. 증거: `cpp/src/runtime/spots/actor_transfer_coordinator.cpp:397-403`, `cpp/src/runtime/spots/spot_runtime.cpp:4390-4400, 4880-4888` |
...
| ID | 분류 | 요약 |
|---|---|---|
| CPP-OBS-001 | PARTIAL·중 | §5 "계측 off 시 비용 0" 위반 지점: Instance-Spot 활성화 경로가 gate 확인 **전에** 완전한 `message_flow_event_t`(문자열 다수 + 상관 id 연결)를 eager 구축 — tracing off에도 해당 경로의 모든 메시지가 이벤트 구축 비용을 지불. 증거: `cpp/src/runtime/host/app.cpp:672-705, 1795-1796` |
...
| CPP-OWN-003 | GAP·중 | §2 이중 보관: 동일 페이로드가 (1) 원시 transport frame, (2) 디코딩된 vector, (3) canonical frozen 인코딩으로 동시 보관 + `try_claim`이 값 복사 1회 추가. 증거: `cpp/src/runtime/stateful/raw_stateful_dispatch.cpp:464-476, 517` |
| CPP-OWN-004 | GAP·중 | §2 경계 왕복: 기본 JSON serializer가 송수신 양방향에서 `message → byte array → message` 왕복 수행(중간 `encoded_payload_t` 표현이 순수 낭비). 증거: `cpp/include/zlink/framework/contracts/codecs/serializer.hpp:102-108, 224-229`, `cpp/src/runtime/messaging/envelope_codec.cpp:64-65` |
...
| CPP-OWN-006 | PARTIAL·중 | 접근자 복사: `to_encoded()`/`to_bytes()`/`decode<T>()`가 호출마다 전체 버퍼 복사 — 두 번 읽으면 복사 2배. no-copy `bytes()` span이 있으나 런타임 경로가 복사 접근자를 사용. 증거: `cpp/include/zlink/framework/contracts/messaging/message.hpp:130-150` |
...
| CPP-OWN-008 | PARTIAL·하 | §7 메시지당 `content_type(type)` 맵 조회 + 새 `std::string` 반환; erased 경로도 메시지당 `std::map` 조회. 증거: `cpp/src/runtime/codecs/serializer.cpp:117, 141, 168-178` |
| CPP-OWN-009 | PARTIAL·중 | Spot→Actor 내부 라우트 패킷이 페이로드 바이트를 nlohmann JSON 숫자 배열로 직렬화(약 4배 팽창) — 프레임워크 자체 생성 변환 복사. 증거: `cpp/src/runtime/spots/spot_route_packets.cpp:300-345` (CPP-WIRE-004와 동일 코드) |
...
| CPP-WIRE-006 | PARTIAL·하 | §1 생성 경계: 생성 헤더를 소비하면서도 1024/4096 바운드 리터럴과 terminal-failure 코드 매핑(101–113)을 소스에 재정의. 증거: `cpp/src/runtime/protocol/service_wire_codec.cpp:248, 263, 1269-1313, 2564, 2878` |
| CPP-WIRE-007 | PARTIAL·하 | §2 multipart 프로파일의 HWM byte 계상이 앱 페이로드 part 크기가 아닌 canonical envelope 전체를 계상. 증거: `cpp/src/runtime/stateful/stateful_object_runtime.cpp:58-65` |
