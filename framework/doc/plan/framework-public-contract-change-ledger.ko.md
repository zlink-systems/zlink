...
| CT-04 | IN-PROGRESS | Node의 `ZLinkSpotHandlerRegistry`가 `ZLinkActorHandlerRegistry`를 상속해 `addHandler(...)`로 Actor send/request handler를 직접 등록할 수 있게 된다. Nest builder에는 module 단위 `disableImplicitHandlerAutoRegistration()`을 추가한다. | 공통 Spot registry 계약에는 Actor handler 등록이 포함되지만 Node exact interface가 이를 숨겼고 runtime의 같은 이름 method도 dispatcher에 등록하지 않았다. 또한 수동 handler 등록을 보여 주는 TicTacToe가 Nest provider scan을 끄면서도 public builder·registry와 DI를 유지할 표면이 필요했다. | 두 변경 모두 additive다. 기존 application은 자동 handler 등록이 기본값이므로 동작이 바뀌지 않는다. Opt-out을 호출한 module은 모든 channel·session·Spot·timer handler를 public builder나 registry에 직접 등록해야 한다. | Node Spot·Nest exact interface와 declaration snapshot을 갱신한다. Spot registry runtime projection test, Nest implicit-off/explicit-handler test와 TicTacToe static sample gate로 검증한다. 다른 Node sample의 자동 등록 gate도 유지한다. | 대기 |
