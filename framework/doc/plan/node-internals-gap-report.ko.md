...
| `NODE-CONTRACT-DIAG-002` | PARTIAL·상 | Public observer, runtime error sink, raw event DTO와 trace file·label option을 제거하고 message-flow·dispatch-error record를 application이 설정한 OpenTelemetry logger·trace provider로 게시한다. Provider failure는 message operation과 분리한다. ObservabilityOps 대표 process는 통과했지만 제거 API를 사용하는 나머지 Node E2E consumer 이관, packed package·clean consumer와 다섯 언어 provider process gate가 남아 있다. |
...
| `NODE-CONTRACT-DIAG-001`, `NODE-CONTRACT-DIAG-002` | `514f0e5e2e` | `npm run build`, `npm run typecheck`, diagnostics·public surface·startup·metrics contract 94/94 PASS. Exact 네 level과 startup validation을 적용하고 legacy public API·동기 file 출력·observer queue를 제거했다. OpenTelemetry logger·trace provider가 구조화된 record를 받으며 provider failure는 fallback error sink로 격리된다. ObservabilityOps OBS-A1/A2/A3/A4와 OBS-B4 process PASS. OBS-B4 첫 반복에서 별도 Actor join result 101이 발생했고 재실행은 통과했다. | 제거 API를 사용하는 나머지 Node E2E consumer를 표준 provider로 이관하고 모두 build·process 검증. Packed declaration·clean consumer, provider 미설정 no-op·실패 격리 process와 다섯 언어 logger provider aggregate 확인 |
...
| NODE-CONTRACT-DIAG-002 | Source public surface와 ObservabilityOps는 표준 OpenTelemetry provider로 전환했다. **나머지 Node E2E consumer·packed package·다섯 언어 provider process 증거가 남음** | DEC-01 |
