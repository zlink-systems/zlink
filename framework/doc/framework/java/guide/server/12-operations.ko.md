...
[가이드 홈](README.ko.md) | [이전: Session 묶음의 동작 원리](39-session-binding.ko.md) | [다음: 옵션과 기본값](16-options.ko.md)
<!-- framework-adapter-nav:end -->

<!-- language-switch:start -->
다른 언어로 보기 — [C++](../../../cpp/guide/server/12-operations.ko.md) · [C#/.NET](../../../dotnet/guide/server/12-operations.ko.md) · **Java** · [Kotlin](../../../kotlin/guide/server/12-operations.ko.md) · [Node/TypeScript](../../../node/guide/server/12-operations.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->

!!! info "이 장을 읽고 나면"

    runtime metric을 수집하고 readiness·drain을 배포 절차에 연결할 수 있다.
    각 언어 code tab은 운영에 사용하는 public runtime surface를 보여 준다.
...
계기 카탈로그는 다음 표가 MeshNode, object·STREAM, location·fanout의 측정값과 단위를 정리한다.
세부 라벨·단위·종류는 [Runtime Metrics](../../../common/spec/server/06-observability/02-runtime-metrics.ko.md)와
[Host relocation 전체 흐름](../../../common/spec/server/05-location-relocation/05-host-relocation-flow.ko.md#16-관측-정보)을 참고한다.
...
넘어간다([운영 호출과 readiness 연결](#5-운영-호출과-readiness-연결)의 예제). 가능하면 `safeToShutdown` 게시까지 확인한다([SafeToShutdown](#33-safetoshutdown--종료해도-안전한-시점)).
...
`safeToShutdown` 게시([SafeToShutdown](#33-safetoshutdown--종료해도-안전한-시점))까지 관찰하고 종료하면 이전 route를 cache한 호출자의 실패를
피할 수 있다.
...
// status()에 전이가, loss()에 놓친 개수가 담긴다 — 공통 규칙은 모니터링 장이 다룬다.
