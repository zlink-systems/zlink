...
```java
client.get("/search").query("q", "ranked match").query("limit", "20").submitRaw();
...
```java
client.get("/players/7281").header("x-trace-id", "abc-123").submitRaw();
```
...
```java
client.get("/slow").timeout(Duration.ofSeconds(10)).submitRaw();
```
