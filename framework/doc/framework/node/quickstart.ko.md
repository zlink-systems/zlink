...
[**zlink-tutorial-node.zip 내려받기**](https://github.com/zlink-systems/zlink/releases/latest/download/zlink-tutorial-node.zip)

주소는 플랫폼과 무관하다. Windows에서도 WSL에서도 같은 파일을 받는다. 압축을 풀면
`zlink-tutorial-node/` 아래에 project가 그대로 나오고, 그 안의 패키지 버전은 받은 릴리즈에
맞춰져 있다.
