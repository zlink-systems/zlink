...
다른 언어로 보기 — [C++](../../../cpp/guide/server/README.ko.md) · [C#/.NET](../../../dotnet/guide/server/README.ko.md) · [Java](../../../java/guide/server/README.ko.md) · [Kotlin](../../../kotlin/guide/server/README.ko.md) · **Node/TypeScript**
{ .zlink-langswitch }
<!-- language-switch:end -->

**실시간 메시징이 중요한 서버 시스템**을 여러 프로세스로 나눠 만드는 Node.js
애플리케이션 프레임워크다. NestJS 위에 그대로 들어가므로 별도 런타임으로 옮겨갈
필요가 없다.
...
DI container, configuration, HTTP endpoint, logging은 NestJS의 것을 그대로 사용한다.
