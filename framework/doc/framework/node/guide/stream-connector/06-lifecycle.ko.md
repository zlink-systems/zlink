...
다른 언어로 보기 — [C++](../../../cpp/guide/stream-connector/06-lifecycle.ko.md) · [C#/.NET](../../../dotnet/guide/stream-connector/06-lifecycle.ko.md) · [Java](../../../java/guide/stream-connector/06-lifecycle.ko.md) · [Kotlin](../../../kotlin/guide/stream-connector/06-lifecycle.ko.md) · **Node/TypeScript**
{ .zlink-langswitch }
<!-- language-switch:end -->
...
쓰는 중인 frame을 기다리지 않은 채 transport를 닫으며, 두 frame의 Send·Request와 응답을 기다리던 request를 `disconnected`로 실패시킨다.
...
```typescript
// 이 런타임의 끊김 handler는 사유를 인자로 받지 않으므로 읽기 속성에서 읽는다.
...
```typescript
const reason = connector.closeReason;   // 끊긴 적이 없으면 undefined다
```
...
처리다. 아직 쓰지 않은 frame은 쓰지 않고, 쓰는 중인 frame의 쓰기가 끝나기를 기다리지 않으며, 상대가
읽거나 응답하기를 기다리지 않는다. handler 밖에서
...
dispatch mode를 따른다. `Immediate`에서는 다른 `Immediate` callback과 같은 경로로 실행하고, `Manual`에서는 종료 뒤의 다음
