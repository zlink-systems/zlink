...
`ApplicationSignaled` coordination mode를 선택한 `SpotWide` Spot에서, relocation 경계를 다음
...
optional `onRelocationReadyCompleted(...)`로 받는다(callback이 없으면 no-op으로 완료). `FrameworkManaged`
