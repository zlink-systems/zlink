...
| Modifier | 기본값 | 의미 |
| --- | --- | --- |
| topic 인자 생략 | event의 packet name을 topic으로 사용 | topic이 Channel messaging §7에서 금지한 값이면 호출 시 `ZLinkConfigurationException`으로 거부한다 |
...
**선택 기준.** JSON이 아닌 content type(MessagePack, Protobuf 등)을 사용할 때 사용한다. 공식 codec
