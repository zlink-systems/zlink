...
| --- | --- | --- |
| Admission | `OnActorJoinAsync` on the User Spot being entered | Whether to accept, and what answer to return with it |
| Commit | The Framework | Changes the location record and the membership together |
| Notice | `OnJoinedActorAsync` on the receiving side, `OnLeaveActorAsync` on the departing side | Runs after the commit |
...
The result arrives on the Actor's join-completion callback (`OnJoinCompletedAsync`). The
per-language spelling is what the code in each tab shows.

```csharp
--8<-- "framework/languages/dotnet/samples/TicTacToe/Server/Play/Infrastructure/ZLink/Actors/PlayActor.cs:doc-join-completed"
```
