...
capability on its adapter. The base snapshot (`CaptureBaseAsync`/`RestoreBaseAsync`) is moved to the
target ahead of time while the Actor keeps processing messages, and during the stop window
only the changes since then (`CaptureDeltaAsync`/`ApplyDeltaAsync`) are transferred — so the stop time
