...
<iframe class="zlink-diagram" src="/common/diagrams/16-option-scope.html"
        title="같은 설정도 어디에 지정하느냐에 따라 범위가 달라진다" loading="lazy" style="width:100%;border:0"></iframe>
<p><a href="/common/diagrams/16-option-scope.html" target="_blank">↗ 크게 보기</a></p>
...
| `Codecs.Use(...)` | payload를 bytes로 바꾸는 방식 | 내장 기본 codec | Protobuf·MessagePack으로 고정하거나 자체 serializer를 사용할 때 |
...
- codec 선택과 자체 serializer 등록: [Handler와 메시지 처리](31-handler-dispatch.ko.md#3-codec--payload를-바이트로-바꾼다)
- handler 발견과 노출의 차이: [Channel 메시징](20-channel-messaging.ko.md#3-routemesh)
- filter가 적용되는 범위: [Handler와 메시지 처리](31-handler-dispatch.ko.md#2-filter--공통-처리를-한곳에-모은다)
- worker 호출: [Timer와 worker](36-timer-worker.ko.md#2-worker--줄-밖에서-실행하기)
- 버전·점검 그룹을 사용하는 배포 흐름: [12-operations](12-operations.ko.md)

`AddLocationStore(...)`와 `AddRelocationStore(...)`도 루트에서 등록한다. 논리 이름으로
상대를 찾는 자동 연결은 [Location](25-location.ko.md)이, 상태를 다른 node로 옮길 때
사용하는 저장소는 [Relocation](37-relocation.ko.md)이 다룬다.
...
| `SetPlacementWeight(int)` | 새 Spot·actor를 이 node에 배치할 비율 | 100 | 사양이 다른 노드를 섞어 사용하거나 새 배치를 멈출 때 |
...
| `PeerConnections.Connect(endpoint)` | 수동으로 연결할 상대 endpoint | 없음 | 자동 연결을 사용하지 않는 구성일 때 |

`Objects().Server()`와 `Channel(name).Server()`에서 하는 등록(stable type, 이전 정책,
channel weight)은 [06-spot](21-spot.ko.md)과
[Channel 메시징](20-channel-messaging.ko.md)이 다룬다. 수동 연결은
[Channel 동작 원리](30-channel-patterns.ko.md#6-연결과-discovery)이 다룬다.
...
[33-backpressure](33-backpressure.ko.md)가 다룬다. 이 절과 다음 절은 그 동작에서 값을 정하는
...
`ConfigureRouterSocket()`은 이 node가 사용하는 소켓의 한도를, `ConfigureSpotPublisher()`는
...
별도로 계산하지 않는다([33-backpressure §4.1](33-backpressure.ko.md#41-core-hwm--core가-소유하는-byte-budget)).
...
여기서 남긴 기록을 읽는 방법은 [모니터링](26-monitoring.ko.md)이 다룬다.
...
| `EnableActorDispatch()` | 들어온 packet을 bind된 actor로 넘긴다 | 꺼져 있음 | 연결을 actor에 묶어 사용하는 구성일 때([Session과 Actor 연결](24-actor-session.ko.md)) |
...
[Channel 동작 원리](30-channel-patterns.ko.md#43-실행-중에-새-요청만-멈추기)과
...
| 자동 연결을 사용할 때 `AddLocationStore(...)`, 안 사용할 때 `PeerConnections.Connect(...)` | 연결할 상대를 찾지 못한다 |
...
- **컨테이너에서 다른 node가 접속하지 못한다** → bind 주소를 광고 주소로 그대로 사용하고
...
- 실행 중 관측과 운영: [모니터링](26-monitoring.ko.md) · [12-operations](12-operations.ko.md)
