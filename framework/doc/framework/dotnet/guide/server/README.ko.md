...
다른 언어로 보기 — [C++](../../../cpp/guide/server/README.ko.md) · **C#/.NET** · [Java](../../../java/guide/server/README.ko.md) · [Kotlin](../../../kotlin/guide/server/README.ko.md) · [Node/TypeScript](../../../node/guide/server/README.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->

**실시간 메시징이 중요한 서버 시스템**을 여러 프로세스로 나눠 만드는 `.NET`
애플리케이션 프레임워크다. `ASP.NET Core` 위에 그대로 들어가므로 별도 런타임으로 옮겨갈
필요가 없다.
...
### `ASP.NET Core`가 제공하는 것

DI container, configuration, HTTP endpoint, logging은 `ASP.NET Core`의 것을 그대로 사용한다.
...
01 · 11 · 13 · 16장은 설치 방법과 표면 이름이 언어마다 달라 `.NET` 전용으로 따로 사용한다.
