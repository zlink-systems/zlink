---
title: "Activation and Lifetime · C#/.NET"
---
...
View in another language — [C++](../../../cpp/guide/server/34-activation-lifetime.en.md) · **C#/.NET** · [Java](../../../java/guide/server/34-activation-lifetime.en.md) · [Kotlin](../../../kotlin/guide/server/34-activation-lifetime.en.md) · [Node/TypeScript](../../../node/guide/server/34-activation-lifetime.en.md)
{ .zlink-langswitch }
<!-- language-switch:end -->
...
!!! info "What you get from this chapter"
    ...
    The code in this chapter comes from the [TicTacToe sample in the per-language example repositories](https://github.com/zlink-systems/zlink-dotnet-examples/tree/main/samples/TicTacToe).
...
| `OnCreateAsync` | X | O | X | Examines a request to create a new User Spot and decides whether to accept it. Not called when an existing Spot was found |
| `OnInitializeAsync` | O | O | O | Initialization of the created instance. An Instance Spot receives only this, with no `OnCreateAsync` |
| `OnClosingAsync` | O | O | O | Before a still-valid local instance is torn down |
| `OnActorJoinAsync` | X | O※ | X | Accepts or refuses an existing Actor that wants to enter this User Spot |
| `OnCreateActorAsync` | O※ | X | X | Accepts or refuses a new Actor's first Entry Spot membership |
| `OnJoinedActorAsync` | O※ | O※ | X | Tells the **receiving** Spot that the join commit finished |
| `OnLeaveActorAsync` | O※ | O※ | X | Tells the **departing** Spot after the commit. It does not mean the Actor is gone |
| `OnDisconnectActorAsync` | O※ | O※ | X | When the connection of an Actor in that Spot drops |
...
Actor in a User Spot returns to its Entry Spot, **the Entry Spot's `OnCreateActorAsync` and
`OnActorJoinAsync` are not called** — returning to the Entry Spot is the default membership and has no
admission step. In both directions only the receiving side's `OnJoinedActorAsync` and the departing
side's `OnLeaveActorAsync` run after the commit.
