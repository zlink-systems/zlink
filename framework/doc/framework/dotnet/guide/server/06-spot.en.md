...
| `OnCreateAsync` | X | O | X | Confirms a new User Spot creation request and decides whether to accept it. Not called when an existing Spot is found |
...
| `OnActorJoinAsync` | X | O※ | X | Approves or rejects an existing Actor's attempt to join this User Spot |
