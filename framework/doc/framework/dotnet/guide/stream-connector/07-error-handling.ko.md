...
[목차](README.ko.md) | [이전: 연결 생명주기](06-lifecycle.ko.md) | [다음: Unity (네이티브 빌드)](08-unity.ko.md)
<!-- framework-adapter-nav:end -->

<!-- language-switch:start -->
다른 언어로 보기 — [C++](../../../cpp/guide/stream-connector/07-error-handling.ko.md) · **C#/.NET** · [Java](../../../java/guide/stream-connector/07-error-handling.ko.md) · [Kotlin](../../../kotlin/guide/stream-connector/07-error-handling.ko.md) · [Node/TypeScript](../../../node/guide/stream-connector/07-error-handling.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->
...
try
{
    ...
    when (failure.Error.Code == ZlinkStreamErrorCode.RequestTimeout)
{
    RetryLogin();
}
```
...
```csharp
using var errors = connector.OnErrorReceived((error, cancellationToken) =>
{
...
| `ConfigurationError` | 구성이 잘못됐다. endpoint scheme과 transport 충돌, 환경이 지원하지 않는 transport 등 |
| `ValidationFailed` | 전송 전 검증, option 값 범위 검증, 대기 표면의 관측 조건이 어긋났다 |
| `RequestTimeout` | 응답을 기다리다 시간이 초과됐다 |
