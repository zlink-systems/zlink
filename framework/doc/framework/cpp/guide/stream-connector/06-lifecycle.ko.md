...
다른 언어로 보기 — **C++** · [C#/.NET](../../../dotnet/guide/stream-connector/06-lifecycle.ko.md) · [Java](../../../java/guide/stream-connector/06-lifecycle.ko.md) · [Kotlin](../../../kotlin/guide/stream-connector/06-lifecycle.ko.md) · [Node/TypeScript](../../../node/guide/stream-connector/06-lifecycle.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->
...
if (!connected) {
    return;   // 실패 코드는 connected.error_code ()로 읽는다
}
...
```cpp
auto reason = connector.close_reason ();   // 끊긴 적이 없으면 빈 값이다
```
