...
[가이드 홈](../../../index.ko.md) | [이전: 3. 핵심 개념](03-concepts.ko.md) | [다음: 18. DI 컨테이너](40-di-container.ko.md)
<!-- framework-adapter-nav:end -->

# 21. 실행·구성 모델

> **이 장의 계약 소유 문서** — [C++ 공통 runtime 공개 계약](../../../common/spec/server/languages/cpp/interfaces/01-common-runtime.ko.md)과
> [비동기 실행 정책](../../../common/spec/server/01-execution/README.ko.md)이 다룬다. 이
...
보장은 [Spot](21-spot.ko.md), 채널 핸들러 노출은 [Channel 메시징](20-channel-messaging.ko.md).
...
몇 개로 수많은 동시 요청을 처리한다. 같은 코드를 `.result()` 로 사용하면 스레드 하나가
...
- **DI 컨테이너** — `options.services()` 에 `add_singleton/scoped/transient` 로
  ...
  [18장 DI 컨테이너](40-di-container.ko.md).
- **구성 표면 지도** — `app_t` 진입점이 역할별로 나뉜다:

  | 진입점 | 역할 | 다루는 장 |
  |--------|------|-----------|
  | `app.config()` / `app.logging()` | 설정·로그 | [19장](41-configuration.ko.md) · 12장 |
...
[다음: DI 컨테이너 →](40-di-container.ko.md)

## 5. 관련 문서

- 정식 계약: [C++ 공통 runtime 공개 계약](../../../common/spec/server/languages/cpp/interfaces/01-common-runtime.ko.md)
- 비동기 terminal 규약: [비동기 실행 정책](../../../common/spec/server/01-execution/README.ko.md)
- DI 주입 규칙: [18. DI 컨테이너](40-di-container.ko.md)
