...
다른 언어로 보기 — **C++** · [C#/.NET](../../../dotnet/guide/server/50-bingo.ko.md) · [Java](../../../java/guide/server/50-bingo.ko.md) · [Kotlin](../../../kotlin/guide/server/50-bingo.ko.md) · [Node/TypeScript](../../../node/guide/server/50-bingo.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->
...
!!! info "이 장을 읽고 나면"
    ...
    코드를 지나는지 따라갈 수 있다. 이 장의 코드는 [언어별 예제 저장소의 Bingo 샘플](https://github.com/zlink-systems/zlink-cpp-examples/tree/main/samples/Bingo)에서 가져온다.
...
기다리며, room의 `get_or_create`도 같은 규칙을 따른다 — API는 어느 Play node에 room을 만들지
