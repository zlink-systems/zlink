...
Turning on the publisher setting `no_drop` changes only that cap behavior. While even one
...
- Restrict the subscribed topics with the subscriber builder's `Subscribe(topic)`, and change HWM
  admission with the publisher builder's `set_no_drop()`. Omitted, they mean every topic is
