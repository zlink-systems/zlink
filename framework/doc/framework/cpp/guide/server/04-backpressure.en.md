...
count down. Tune these boundaries with `set_application_job_queue_pause_threshold_percent` (`1..100`)
and `set_application_job_queue_resume_threshold_percent` (`0..99`); resume must be below pause.
...
limit. For a workload that retains large payloads for longer, lower `max_queued_application_jobs`
