...
다른 언어로 보기 — **C++** · [C#/.NET](../../../dotnet/guide/server/52-supportchat.ko.md) · [Java](../../../java/guide/server/52-supportchat.ko.md) · [Kotlin](../../../kotlin/guide/server/52-supportchat.ko.md) · [Node/TypeScript](../../../node/guide/server/52-supportchat.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->
...
!!! info "이 장을 읽고 나면"
    ...
    메시지가 어느 서버의 어느 코드를 지나는지 따라갈 수 있다. 이 장의 코드는 [언어별 예제 저장소의 SupportChat 샘플](https://github.com/zlink-systems/zlink-cpp-examples/tree/main/samples/SupportChat)에서 가져온다.
...
```cpp
--8<-- "framework/languages/cpp/samples/SupportChat/Server/Session/main.cpp:doc-sc-metadata-relay"
```
...
```cpp
// C++ 샘플의 Entry Spot에는 disconnect callback이 없다. 끊겨도 availability는 내려가지 않는다.
```
