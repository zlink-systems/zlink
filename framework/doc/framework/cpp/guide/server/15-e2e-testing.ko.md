...
다른 언어로 보기 — **C++** · [C#/.NET](../../../dotnet/guide/server/15-e2e-testing.ko.md) · [Java](../../../java/guide/server/15-e2e-testing.ko.md) · [Kotlin](../../../kotlin/guide/server/15-e2e-testing.ko.md) · [Node/TypeScript](../../../node/guide/server/15-e2e-testing.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->

!!! info "이 장을 읽고 나면"

    실제 client로 등록, 연결, push, 응답 순서를 E2E 시나리오에서 검증할 수 있다.
    이 장의 검증 예제는 언어별 예제 저장소의 sample scenario를 기준으로 한다.
...
**terminal 표기는 언어를 따른다** — `.NET`은 `Async`, C++은 `async`, Java · Node는
`submit`, Kotlin은 `await`다([비동기 실행 정책](../../../common/spec/server/01-execution/README.ko.md)).
...
- **client는 통과했는데 서버 로그에 오류가 남는다** → 서버 로그 오류 검사를 script에
  추가하지 않은 경우다([실행 스크립트와 성공 판정](#7-실행-스크립트와-성공-판정)).
