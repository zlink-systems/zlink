...
- The `listen` host is where the current process binds its socket, and `0.0.0.0` accepts on every
  ...
  per-Pod DNS in Kubernetes. When omitted, `advertise_host` advertises a non-wildcard bind host or
...
- **The CPU worker pool has no queue limit.** [Core HWM and Application Job Queue Limits](#3-core-hwm-and-application-job-queue-limits)
  limits intake. `default_request_timeout` rejects values of `0` or below.
