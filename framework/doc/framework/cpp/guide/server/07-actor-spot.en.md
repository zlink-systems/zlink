...
capability on its adapter. The base snapshot (`capture_base`/`restore_base`) is moved to the
target ahead of time while the Actor keeps processing messages, and during the stop window
only the changes since then (`capture_delta`/`apply_delta`) are transferred — so the stop time
