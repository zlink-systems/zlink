...
View in another language — **C++** · [C#/.NET](../../../dotnet/guide/server/14-samples.en.md) · [Java](../../../java/guide/server/14-samples.en.md) · [Kotlin](../../../kotlin/guide/server/14-samples.en.md) · [Node/TypeScript](../../../node/guide/server/14-samples.en.md)
{ .zlink-langswitch }
<!-- language-switch:end -->
...
| --- | --- | --- |
| A real-time head-to-head game server | [TicTacToe](#3-tictactoe--building-a-real-time-head-to-head-game-server) | The smallest configuration, with auto-connect and auto-registration stripped away |
...
<iframe class="zlink-diagram" src="/common/diagrams/14-tictactoe-en.html" title="TicTacToe sample topology" style="width:100%;border:0"></iframe>
...
<iframe class="zlink-diagram" src="/common/diagrams/14-bingo-en.html" title="Bingo sample topology" style="width:100%;border:0"></iframe>
...
This sample uniquely shows how the Matchmaker Instance Spot atomically decides waiting-room
reservations against Redis and how, even when the player actor and the room Spot **are on
different Play servers**, the framework finds the current owner and executes a remote Spot
join. After that, the room Spot draws a number on a timer, pushes it to the
...
and binds both the roster actor and each per-conversation actor to that session.

<iframe class="zlink-diagram" src="/common/diagrams/14-supportchat-en.html" title="SupportChat sample topology" style="width:100%;border:0"></iframe>
...
actor, allowing the conversation state to continue unchanged. If no message arrives for a
while, a Spot timer starts the conversation-closing flow.
...
business rules — it shows **how the common requirements to create a request, pick a
fulfiller, deliver to a specific user's connection, and retry on no response map to
framework features.** Ride-hailing, field dispatch, and on-site service requests use the
same architecture.

<iframe class="zlink-diagram" src="/common/diagrams/14-delivery-en.html" title="DeliveryDispatch sample topology" style="width:100%;border:0"></iframe>
<p><a href="/common/diagrams/14-delivery-en.html" target="_blank">↗ View larger</a></p>

The external boundary still uses standard web technology. The customer creates a delivery
...
<iframe class="zlink-diagram" src="/common/diagrams/14-shoppingmall-en.html" title="ShoppingMall sample topology" style="width:100%;border:0"></iframe>
...
event stream. An idempotency key handles duplicate clicks, an expected version handles
...
<iframe class="zlink-diagram" src="/common/diagrams/14-gamequest-en.html" title="GameQuest sample topology" style="width:100%;border:0"></iframe>
...
resynchronization as a safety valve, so **it accepts possible data loss in exchange for
real-time responsiveness.** That's why it doesn't have zero-loss mechanisms like blocking the previous
...
> ZoneWorld is a common sample implemented in every supported framework language. Each implementation
> follows this chapter and the common scenario for topology, actor relocation, operations
> fanout, and browser verification.

The world is divided into zones managed across several `ZoneNode`s. The Location Store and
...
<iframe class="zlink-diagram" src="/common/diagrams/14-zoneworld-en.html" title="ZoneWorld sample topology" style="width:100%;border:0"></iframe>
...
- The server and runner are provided in every supported language and share the business behavior and
...
# published packages and configures the samples into build/ (inside the repository:
...
Samples run one at a time. Checking every sample means one invocation for each. ZoneWorld and its
