---
title: "Backpressure — When Arrival Outpaces Processing · C++"
---

<!-- generated:start -->
<!-- This file is generated from `common/guide/server/33-backpressure.en.md`. Do not edit directly.
...
[Guide Home](README.en.md) | [Previous: The Execution Model](32-execution-model.en.md) | [Next: Activation and Lifetime](34-activation-lifetime.en.md)
<!-- framework-adapter-nav:end -->

<!-- language-switch:start -->
View in another language — **C++** · [C#/.NET](../../../dotnet/guide/server/33-backpressure.en.md) · [Java](../../../java/guide/server/33-backpressure.en.md) · [Kotlin](../../../kotlin/guide/server/33-backpressure.en.md) · [Node/TypeScript](../../../node/guide/server/33-backpressure.en.md)
{ .zlink-langswitch }
<!-- language-switch:end -->

This chapter quotes no tutorial code; it explains the limits and outcomes observable when load reaches tutorial `Server` and `Client` processes that have been bootstrapped and built using the [README Run section](https://github.com/zlink-systems/zlink-cpp-examples/blob/main/tutorial/README.md#run).
...
Even when retrying, sending again immediately returns the request to a queue that
...
| `SendHighWaterMark` | Bytes that can be held **to send**, per peer. `0` means unlimited | `configure_router_socket()` |
| `ReceiveHighWaterMark` | Bytes that can be held **after receiving**, per peer. `0` means unlimited | `configure_router_socket()` |
...
| `core_hwm_memory_limit_bytes` · `core_hwm_budget_bytes` · `CoreHwmProfile` | The Core context's ordinary-queue byte budget | root inbound-dispatch configuration |
| `ApplicationJobQueueProfile` · `max_queued_application_jobs` · pause/resume thresholds | The host instance's queued-application-job limit and flow-transition boundaries | root inbound-dispatch configuration |
...
blocked ratio directly from the Core snapshot. `core_hwm_budget_bytes` is not a hard process
...
bytes like `core_hwm_budget_bytes`, but have a different owner and scope. A manual socket HWM
...
| Release | When the Core queue releases frame ownership | Immediately before the user callback's actual first instruction |
| Saturation result | The sender for that origin waits | The ordinary ingress source waits for a permit |
| Settings | `core_hwm_memory_limit_bytes` · `core_hwm_budget_bytes` · `CoreHwmProfile` | `ApplicationJobQueueProfile` · `max_queued_application_jobs` · `set_application_job_queue_pause_threshold_percent` · `set_application_job_queue_resume_threshold_percent` |

Manual `max_queued_application_jobs` is an exact limit in `1..2,147,483,647`. `0` is not
...
resume permit count down. Tune these boundaries with `set_application_job_queue_pause_threshold_percent`
(`1..100`) and `set_application_job_queue_resume_threshold_percent` (`0..99`); resume must be below
pause.
...
For a workload that retains large payloads for longer, lower `max_queued_application_jobs` to
...
`max_message_size` remains an independent single-message cap.
...
target is causing the delay (the `11. Monitoring` chapter ·
