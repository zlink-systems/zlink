...
`trust_certificate_file` adds a PEM certificate to the system trust store. It validates a private CA or self-signed server without replacing system roots. No option disables hostname validation.

## 3. An mTLS Client Certificate

`client_certificate_file` configures a PEM client certificate and a PKCS#8 key. It applies to an HTTPS endpoint that requires client authentication.
