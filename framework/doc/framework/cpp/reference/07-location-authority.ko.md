...
분산 discovery, Instance Spot cold activation 또는 Actor·Spot relocation을 사용하는 host가 Store
구현을 root에 등록한다.
...
| `redis_location_options_t::key_prefix` / `redis_relocation_options_t::key_prefix` | 코드 기본값은 빈 문자열이지만 유효한 구성에는 반드시 비어 있지 않은 값을 지정해야 한다(둘이 같은 Redis를 사용하면 서로 달라야 함) | Redis key namespace |
...
**선택 기준.** Manual peer만 사용하고 분산 location 기능이 필요 없는 node는 이 항목을 생략하고 시작할
...
locations.message_follow_duration = std::chrono::seconds{30};
```
...
[Relocation Flow §5.3](../../common/spec/server/05-location-relocation/04-relocation-flow.ko.md#53-relocation-전용-capacity-제한을-두지-않는다)을
참고한다.
...
**선택 기준.** 특정 역할의 peer가 준비될 때까지 기다리는 startup 순서 제어나 헬스체크에 사용한다.
...
**선택 기준.** Location 인프라 자체의 건강 상태를 진단할 때 사용한다. 특정 peer 준비 여부는
`is_peer_ready`를 사용한다.
...
**선택 기준.** 운영 도구에서 등록된 node나 서비스 현황을 사람이 볼 수 있는 형태로 조회할 때 사용한다.
