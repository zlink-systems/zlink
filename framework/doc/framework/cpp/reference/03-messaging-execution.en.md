...
| Modifier | Default | Meaning |
| --- | --- | --- |
| Omitting the topic argument | Uses the event's `packet_name` as the topic | A topic that Channel messaging §7 forbids is rejected at the call with `framework_exception_t` |
