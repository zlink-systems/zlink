...
HTTP를 열지 않는 process라 `SpringApplication`의 `isKeepAlive`를 `true`로 두어야 한다. 없으면
...
| server가 바로 종료된다 | `isKeepAlive`를 `true`로 두었는지 확인한다 |
...
| Server | `addRouteMesh` → `listen` → `channelName(...).server().addRequestHandler(...)`, 그리고 `isKeepAlive = true` |
