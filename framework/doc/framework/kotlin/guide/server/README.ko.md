...
다른 언어로 보기 — [C++](../../../cpp/guide/server/README.ko.md) · [C#/.NET](../../../dotnet/guide/server/README.ko.md) · [Java](../../../java/guide/server/README.ko.md) · **Kotlin** · [Node/TypeScript](../../../node/guide/server/README.ko.md)
{ .zlink-langswitch }
<!-- language-switch:end -->

**실시간 메시징이 중요한 서버 시스템**을 여러 프로세스로 나눠 만드는 Kotlin
...
**Kotlin은 Java 런타임을 그대로 사용한다.** `zlink-framework-kotlin`은 별도 구현이 아니라
coroutine idiom을 추가하는 얇은 레이어다. 그래서 이 가이드는 **Java와 다른 지점만** 사용하고
나머지는 [Java 가이드](../../../java/guide/server/README.ko.md)를 가리킨다.

- **01** — 의존성과 등록 코드 모양이 달라 Kotlin 전용으로 사용한다.
- **11 · 13 · 16** — 표면은 Java와 같고 idiom만 다르다. 차이만 사용하고 나머지는 Java 장을
  가리킨다.
