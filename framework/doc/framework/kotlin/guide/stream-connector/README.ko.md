...
Kotlin STREAM client connector의 가이드다. Java connector를 coroutine 관용구로
감싼 것이며, 서버 애플리케이션과 시험 도구가 대상이다.
...
| 8 | [게임 엔진 통합](12-engine-integration.ko.md) | 엔진별 connector 선택과 Engine Lobby sample |

파일 번호는 언어에 상관없이 같은 장을 가리키는 식별자다. 1~7장은 다섯 언어가 공유한다.

## 관련 문서

- 공개 계약: [Kotlin 공개 계약](../../../common/spec/stream-connector/languages/java/03-stream-connector.ko.md)
- 서버 가이드: [Kotlin 서버 가이드](../server/README.ko.md)
