...
- 세부 동작: [Flow correlation §7](../spec/server/06-observability/04-flow-correlation.ko.md)을 검증한다.
...
- 세부 동작: [Flow correlation §4](../spec/server/06-observability/04-flow-correlation.ko.md)을 검증한다.
...
- 검증: 정상 subscriber는 application payload marker를 한 번 처리하며 정상 fanout message-flow trace는 없다.
  ...
  handler가 한 번 실행된다.
- 세부 동작: [Message flow tracing — attribute 포함 조건](../spec/server/06-observability/03-message-flow-tracing.ko.md#32-attribute-포함-조건)과
  [Flow correlation — Flow를 만드는 시점](../spec/server/06-observability/04-flow-correlation.ko.md#4-flow를-만드는-시점)을 검증한다.
...
- 세부 동작: [Message-flow tracing — 실행 중에 기록 수준 변경](../spec/server/06-observability/03-message-flow-tracing.ko.md#5-실행-중-기록-수준-변경과-비용-규칙)을 검증한다.
...
- 세부 동작: [Runtime metrics §4](../spec/server/06-observability/02-runtime-metrics.ko.md)를 검증한다.
...
- 검증: Completed counter delta는 1이고 `object_kind=actor`다. Duration과 interruption sample이 각각
  ...
  실패로 바꾸지 않는다.
- 세부 동작: [Object와 STREAM metric](../spec/server/06-observability/02-runtime-metrics.ko.md)과
  [Host relocation과 shutdown metric](../spec/server/06-observability/02-runtime-metrics.ko.md#8-host-relocation과-shutdown)을 검증한다.
...
- 세부 동작: [Runtime metrics §6](../spec/server/06-observability/02-runtime-metrics.ko.md)과
  [§7](../spec/server/06-observability/02-runtime-metrics.ko.md)을 검증한다.
...
- 세부 동작: [Runtime metrics §8](../spec/server/06-observability/02-runtime-metrics.ko.md)을 검증한다.
...
- 검증: Held 구간에 source는 `Relocating`, not-ready, not-accepting이며 신규 object는 source에 배치되지
  ...
  같은 닫힌 state를 반영한다.
- 세부 동작: [Host maintenance §13](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)을 검증한다.
...
- 세부 동작: [Actor와 Spot의 공통 handoff 순서](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)와
  [Session Actor relocation route barrier](../spec/server/04-session/02-session-actor-binding.ko.md)를
  검증한다.
...
- 세부 동작: [Host maintenance §8.5](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)를
  검증한다.
...
- 세부 동작: [Target 선택 전 조건](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#4-target을-선택하기-전에-확인하는-조건)과
  [Mode에 맞는 target 선택](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#5-mode에-맞는-target을-선택한다)을 검증한다.
...
- 세부 동작: [Concurrent 호출과 cancellation](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#6-concurrent-호출과-cancellation)을
  검증한다.
...
- 세부 동작: [Host maintenance §11](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)을
  검증한다.
