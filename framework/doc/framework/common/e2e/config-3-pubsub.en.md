...
- Detailed behavior: verifies fanout delivery in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Verification: Each marker is recorded exactly once, only in the corresponding handler's evidence,
  ...
  field.
- Detailed behavior: verifies the handler namespace in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Detailed behavior: verifies non-replay delivery in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Detailed behavior: verifies fanout reconnect in [Transport Liveness §6](../spec/server/02-channel-transport/05-transport-liveness.en.md#6-connection-loss-and-reconnect).
...
- Detailed behavior: verifies subscriber-process isolation in
  [Channel Topology](../spec/server/02-channel-transport/01-channel-topology.en.md) and fanout dispatch isolation in
...
- Detailed behavior: verifies automatic discovery in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Detailed behavior: verifies fanout status in [Runtime Monitoring §2.2](../spec/server/06-observability/01-runtime-monitoring.en.md).
...
- Detailed behavior: verifies [Transport Liveness §5](../spec/server/02-channel-transport/05-transport-liveness.en.md#5-ready-and-failure-determination).
...
- Detailed behavior: verifies [Transport Liveness §5](../spec/server/02-channel-transport/05-transport-liveness.en.md#5-ready-and-failure-determination)
  and [§7](../spec/server/02-channel-transport/05-transport-liveness.en.md#7-location-store-and-host-termination).
...
- Detailed behavior: verifies [Transport Liveness §7](../spec/server/02-channel-transport/05-transport-liveness.en.md#7-location-store-and-host-termination).
...
- Detailed behavior: verifies [Network Listener Identity §4](../spec/server/02-channel-transport/04-network-listener-identity.en.md).
...
- Verification: The automatic subscriber's ready-publisher identity is preserved, and both markers
  ...
  sequence.
- Detailed behavior: verifies mode separation in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Detailed behavior: verifies manual mode in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Verification: The host does not expose a listener or ready status, and exits with a public
  configuration error.
- Detailed behavior: verifies the automatic Store prerequisite in [Framework API — Classic Fanout](../spec/server/00-foundation/06-framework-api.en.md).
...
- Detailed behavior: verifies mode validation in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Verification: Both hosts exit with a public configuration error matching the cause, before the
  listener binds.
- Detailed behavior: verifies publisher identity in [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md).
...
- Detailed behavior: verifies [Transport Liveness §4](../spec/server/02-channel-transport/05-transport-liveness.en.md#4-classic-fanout)
  and [§5](../spec/server/02-channel-transport/05-transport-liveness.en.md#5-ready-and-failure-determination).
...
- Verification: While B alone is excluded from the ready list, A's markers keep being processed. The
  ...
  markers.
- Detailed behavior: verifies per-publisher liveness in [Transport Liveness §4](../spec/server/02-channel-transport/05-transport-liveness.en.md#4-classic-fanout).
...
- Detailed behavior: verifies [Framework API §11](../spec/server/00-foundation/06-framework-api.en.md)
  and [Message Flow Tracing §2.2](../spec/server/06-observability/03-message-flow-tracing.en.md#22-the-public-behavior-recorded).
