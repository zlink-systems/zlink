...
- Detailed behavior: verifies [Spot Actor §2](../spec/server/03-spot-actor/05-spot-actor-membership.en.md).
...
- Detailed behavior: verifies [Work Put On The Spot Application Queue](../spec/server/03-spot-actor/02-spot-messaging.en.md#53-work-put-on-the-spot-application-queue)
  and [Spot Turn And Callback Order](../spec/server/03-spot-actor/02-spot-messaging.en.md#54-spot-turn-and-callback-order).
...
- Detailed behavior: verifies [Spot Messaging §3](../spec/server/03-spot-actor/02-spot-messaging.en.md).
...
- Detailed behavior: verifies [Async Execution Policy](../spec/server/01-execution/README.en.md).
...
- Detailed behavior: verifies [Actor Membership](../spec/server/03-spot-actor/05-spot-actor-membership.en.md#3-actor-membership-for-entry-spot-and-user-spot)
  and [Spot Termination](../spec/server/03-spot-actor/02-spot-messaging.en.md#62-spot-termination).
...
- Detailed behavior: verifies the publication boundary in [Spot Actor §2](../spec/server/03-spot-actor/05-spot-actor-membership.en.md).
...
- Detailed behavior: verifies [Network Listener Identity §7.3](../spec/server/02-channel-transport/04-network-listener-identity.en.md).
...
- Detailed behavior: verifies the same-global-ID rule and Spot ID contract in [Actor Model §2.1](../spec/server/03-spot-actor/04-actor-model.en.md).
...
- Detailed behavior: verifies [Actor Model §3](../spec/server/03-spot-actor/04-actor-model.en.md).
...
- Verification: Target `OnActorJoin`, `OnJoinedActor`, and source `OnLeaveActor` each run exactly
  ...
  once on play-a.
- Detailed behavior: verifies [Spot Actor §4](../spec/server/03-spot-actor/05-spot-actor-membership.en.md).
...
- Detailed behavior: verifies [Spot Actor §5](../spec/server/03-spot-actor/05-spot-actor-membership.en.md).
...
- Detailed behavior: verifies [Message Model](../spec/server/00-foundation/05-message-model.en.md).
...
- Verification: Only the play-b handler records the marker exactly once, and the caller receives a
  matching reply.
- Detailed behavior: verifies [Actor Model §5](../spec/server/03-spot-actor/04-actor-model.en.md).
...
- Detailed behavior: verifies [Session Actor Dispatch §6](../spec/server/04-session/02-session-actor-binding.en.md).
...
- Verification: The results are true, false, `InvalidOperation` in order, and the recreated Actor
  processes requests.
- Detailed behavior: verifies [Failover Policy §4.1](../spec/server/05-location-relocation/06-failure-failover-policy.en.md).
...
- Detailed behavior: verifies [MeshNode §4](../spec/server/03-spot-actor/03-mesh-node.en.md).
...
- Detailed behavior: verifies [Channel Messaging §3.2](../spec/server/02-channel-transport/02-channel-messaging.en.md).
...
- Verification: The matching Spots each receive the marker exactly once, and the nonmatching Spot
  does not.
- Detailed behavior: verifies [Spot Messaging §4](../spec/server/03-spot-actor/02-spot-messaging.en.md).
...
- Detailed behavior: verifies [Session Actor Dispatch §5](../spec/server/04-session/02-session-actor-binding.en.md).
...
- Detailed behavior: verifies [Actor Model §2.3](../spec/server/03-spot-actor/04-actor-model.en.md).
...
- Detailed behavior: verifies [Session Actor Dispatch §3](../spec/server/04-session/02-session-actor-binding.en.md).
...
- Detailed behavior: verifies [Session Actor Dispatch §4](../spec/server/04-session/02-session-actor-binding.en.md).
...
- Detailed behavior: verifies [Stream Session §3](../spec/server/04-session/01-stream-session.en.md).
...
- Verification: All 100 replies exactly correspond to their operation ID and input surface, with no
  cross-delivery.
- Detailed behavior: verifies [Interaction Model](../spec/server/00-foundation/04-interaction-model.en.md).
...
- Detailed behavior: verifies [Connection Loss And Reconnect](../spec/server/02-channel-transport/05-transport-liveness.en.md#6-connection-loss-and-reconnect)
  and [The STREAM Error Boundary](../spec/server/04-session/01-stream-session.en.md#7-error-boundary).
...
- Detailed behavior: verifies [STREAM TLS](../spec/server/04-session/01-stream-session.en.md#31-tls)
  and [From Session To Actor](../spec/server/04-session/01-stream-session.en.md#8-from-session-to-actor).
...
- Detailed behavior: verifies [Flow Correlation §5](../spec/server/06-observability/04-flow-correlation.en.md).
...
- Detailed behavior: verifies [Message Flow Tracing §2.2](../spec/server/06-observability/03-message-flow-tracing.en.md).
...
- Verification: The callback count and counter delta are both 1, and the client push is also exactly
  one.
- Detailed behavior: verifies [Async Execution Policy — Spot Timer](../spec/server/03-spot-actor/10-spot-timer.en.md).
...
- Detailed behavior: verifies [Spot Timer](../spec/server/03-spot-actor/10-spot-timer.en.md)
  and [Spot Termination](../spec/server/03-spot-actor/02-spot-messaging.en.md#62-spot-termination).
...
- Detailed behavior: verifies [Spot-Timer Overrun Policy And Tick Fields](../spec/server/03-spot-actor/10-spot-timer.en.md).
...
- Verification: Each handler processes only its own marker, and the caller receives matching
  replies.
- Detailed behavior: verifies [Interaction Model §3](../spec/server/00-foundation/04-interaction-model.en.md).
...
- Detailed behavior: verifies [Failover Policy §5](../spec/server/05-location-relocation/06-failure-failover-policy.en.md) and
  [§6](../spec/server/05-location-relocation/06-failure-failover-policy.en.md).
...
- Detailed behavior: verifies [MeshNode §5](../spec/server/03-spot-actor/03-mesh-node.en.md).
