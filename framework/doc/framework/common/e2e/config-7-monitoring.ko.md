...
- 세부 동작: [Runtime monitoring §2](../spec/server/06-observability/01-runtime-monitoring.ko.md)를
  검증한다.
...
- 세부 동작: [Runtime monitoring §2.2](../spec/server/06-observability/01-runtime-monitoring.ko.md)와
  [§3](../spec/server/06-observability/01-runtime-monitoring.ko.md)을 검증한다.
...
- 세부 동작: [Runtime monitoring §2.2](../spec/server/06-observability/01-runtime-monitoring.ko.md)와
  [Channel topology §7](../spec/server/02-channel-transport/01-channel-topology.ko.md)을 검증한다.
...
- 세부 동작: [Location runtime §8](../spec/server/05-location-relocation/01-location-runtime.ko.md)과
...
- 검증: Active count는 완료된 public lifecycle 결과와 단계마다 일치한다. Limit 초과 create는
  ...
  create가 성공한다.
- 세부 동작: [Runtime monitoring §2.2](../spec/server/06-observability/01-runtime-monitoring.ko.md)와
  [MeshNode §5](../spec/server/03-spot-actor/03-mesh-node.ko.md)를 검증한다.
...
- 검증: Request는 deadline 안에 reply를 받고 정상 observer는 current status를 제공한다. 느린 observer의
  ...
  observer를 종료하지 않는다.
- 세부 동작: [Runtime monitoring §3](../spec/server/06-observability/01-runtime-monitoring.ko.md)의
  observer 격리를 검증한다.
...
- 세부 동작: [Runtime monitoring §6](../spec/server/06-observability/01-runtime-monitoring.ko.md)를 검증한다.
