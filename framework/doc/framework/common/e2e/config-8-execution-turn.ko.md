...
- 세부 동작: [비동기 실행 정책 §1.1](../spec/server/01-execution/README.ko.md)을
  검증한다.
...
- 세부 동작: [비동기 실행 정책 §2](../spec/server/01-execution/README.ko.md)의 serial turn을
  검증한다.
...
- 검증: Handler가 reply를 받아 재개하고 deadline 전에 정상 완료한다. 같은 Spot의 다음 callback은 그 뒤
  실행된다.
- 세부 동작: [비동기 실행 정책 §3](../spec/server/01-execution/README.ko.md)을
  검증한다.
...
- 검증: Evidence 순서는 `async-held, async-completed, timer-started, timer-completed`이며 callback active
  count는 1을 넘지 않는다.
- 세부 동작: [비동기 실행 정책 §5](../spec/server/01-execution/README.ko.md)을
  검증한다.
...
- 세부 동작: [비동기 실행 정책 §1.1](../spec/server/01-execution/README.ko.md)의 shared Spot gate 반환과
  [§5](../spec/server/01-execution/README.ko.md)의 timer application turn을 검증한다.
...
- 세부 동작: [Worker offload](../spec/server/01-execution/02-handler-turn-and-execution-gate.ko.md)의 I/O worker를
  검증한다.
...
- 세부 동작: [Worker offload](../spec/server/01-execution/02-handler-turn-and-execution-gate.ko.md)를 검증한다.
...
- 검증: 모든 operation이 정상 reply를 하나씩 받고 `CapacityExceeded`가 없다. 다른 Spot probe도 대기
  중에 완료된다.
- 세부 동작: [Worker offload](../spec/server/01-execution/02-handler-turn-and-execution-gate.ko.md)의 CPU execution slot과 I/O wait 분리를
  검증한다.
...
- 검증: I/O operation은 CPU gate 해제 전에 정상 완료한다. CPU operation도 gate 해제 뒤 각자 terminal을
  하나만 반환한다.
- 세부 동작: [Worker offload](../spec/server/01-execution/02-handler-turn-and-execution-gate.ko.md)의 실행 자원 분리를
  검증한다.
...
- 검증: B, Spot과 timer evidence가 A continuation보다 먼저 나타나며 callback active count는 shared gate
  안에서 1을 넘지 않는다.
- 세부 동작: [Handler turn과 claim](../spec/server/01-execution/02-handler-turn-and-execution-gate.ko.md)을
  검증한다.
...
- 세부 동작: [Framework API §12](../spec/server/00-foundation/06-framework-api.ko.md)의 context validation을
  검증한다.
...
- 세부 동작: [같은 turn에서의 대기](../spec/server/01-execution/02-handler-turn-and-execution-gate.ko.md)를
  검증한다.
...
- 세부 동작: [Actor Join의 deferred terminal](../spec/server/01-execution/README.ko.md)의
...
- 검증: 두 completion callback이 Accepted이고 public current Spot이 서로 바뀐다. 각 Actor callback은 한
  번씩 실행되며 timeout이 없다.
- 세부 동작: [Spot actor §4](../spec/server/03-spot-actor/05-spot-actor-membership.ko.md)의 Actor별 독립성을
  검증한다.
...
- 세부 동작: [비동기 실행 정책 §2](../spec/server/01-execution/README.ko.md)을 검증한다.
...
- 세부 동작: [Framework API §12](../spec/server/00-foundation/06-framework-api.ko.md)을 검증한다.
...
- 세부 동작: [Session Actor execution과 lifecycle](../spec/server/04-session/02-session-actor-binding.ko.md)과
...
- 세부 동작: [Cancellation과 shutdown](../spec/server/01-execution/03-cancellation-and-shutdown.ko.md)을
  검증한다.
...
- 세부 동작: [Cancellation과 shutdown](../spec/server/01-execution/03-cancellation-and-shutdown.ko.md)과
...
- 세부 동작: [Public contract governance](../spec/server/00-foundation/01-public-contract-governance.ko.md)의 언어 parity를
  검증한다.
