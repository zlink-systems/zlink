...
- Remote topology, Session relay, 공통 timeout·Shutdown과 exact interface가 지원하는 cancellation 결과
...
- 세부 동작: [비동기 실행 정책 §1.1](../spec/server/05-async-execution-policy.ko.md)을
  검증한다.
...
- 세부 동작: [비동기 실행 정책 §2](../spec/server/05-async-execution-policy.ko.md)의 serial turn을
  검증한다.
...
- 검증: Handler가 reply를 받아 재개하고 deadline 전에 정상 완료한다. 같은 Spot의 다음 callback은 그 뒤
  실행된다.
- 세부 동작: [비동기 실행 정책 §3](../spec/server/05-async-execution-policy.ko.md)을
  검증한다.
...
- 검증: Evidence 순서는 `async-held, async-completed, timer-started, timer-completed`이며 callback active
  count는 1을 넘지 않는다.
- 세부 동작: [비동기 실행 정책 §5](../spec/server/05-async-execution-policy.ko.md)을
  검증한다.
...
- 세부 동작: [비동기 실행 정책 §1.1](../spec/server/05-async-execution-policy.ko.md)의 shared Spot gate 반환과
  [§5](../spec/server/05-async-execution-policy.ko.md)의 timer application turn을 검증한다.
...
- 검증: 첫 continuation은 current value 20을 다시 읽고 그 값을 기준으로 처리한다. Yield 전에 읽은 10을
  그대로 쓰지 않는다.
- 세부 동작: [Handler turn과 claim](../spec/server/05-async-execution-policy.ko.md#3-handler-turn과-claim)과
  [`Yield` 시 gate와 claim](../spec/server/05-async-execution-policy.ko.md#yield-시-gate와-claim)을
  검증한다.
...
- 세부 동작: [Worker offload](../spec/server/05-async-execution-policy.ko.md#12-worker-offload)의 I/O worker를
  검증한다.
...
- 세부 동작: [Worker offload](../spec/server/05-async-execution-policy.ko.md#12-worker-offload)를 검증한다.
...
- 검증: 모든 operation이 정상 reply를 하나씩 받고 `CapacityExceeded`가 없다. 다른 Spot probe도 대기
  중에 완료된다.
- 세부 동작: [Worker offload](../spec/server/05-async-execution-policy.ko.md#12-worker-offload)의 CPU execution slot과 I/O wait 분리를
  검증한다.
...
- 검증: I/O operation은 CPU gate 해제 전에 정상 완료한다. CPU operation도 gate 해제 뒤 각자 terminal을
  하나만 반환한다.
- 세부 동작: [Worker offload](../spec/server/05-async-execution-policy.ko.md#12-worker-offload)의 실행 자원 분리를
  검증한다.
...
- 검증: B, Spot과 timer evidence가 A continuation보다 먼저 나타나며 callback active count는 shared gate
  안에서 1을 넘지 않는다.
- 세부 동작: [Handler turn과 claim](../spec/server/05-async-execution-policy.ko.md#3-handler-turn과-claim)을
  검증한다.
...
- 세부 동작: [Framework API §12](../spec/server/06-framework-api.ko.md)의 context validation을
  검증한다.
...
- 세부 동작: [같은 turn에서의 대기](../spec/server/05-async-execution-policy.ko.md#같은-turn에서의-대기)를
  검증한다.
...
한 handler가 여러 Actor의 Join을 defer했더라도 handler가 exception으로 끝나면 아직
...
- 절차: Exception variant를 실행하고, handler cancellation 표면이 있는 언어만 cancellation variant를 fresh fixture에서 실행한다. Handler terminal 뒤
...
- 세부 동작: [Actor Join의 deferred terminal](../spec/server/05-async-execution-policy.ko.md#31-actor-join의-deferred-terminal)의
...
- 검증: 두 completion callback이 Accepted이고 public current Spot이 서로 바뀐다. 각 Actor callback은 한
  번씩 실행되며 timeout이 없다.
- 세부 동작: [Spot actor §4](../spec/server/15-spot-actor.ko.md)의 Actor별 독립성을
  검증한다.
...
- 세부 동작: [비동기 실행 정책 §2](../spec/server/05-async-execution-policy.ko.md)을 검증한다.
...
- 세부 동작: [Framework API §12](../spec/server/06-framework-api.ko.md)을 검증한다.
...
- 세부 동작: [Session Actor execution과 lifecycle](../spec/server/20-session-actor-dispatch.ko.md#7-execution과-lifecycle)과
...
#### TD-F5 Waiter 종료 뒤 owner를 계속 사용한다
...
- 절차: 공통 variant는 deadline 또는 Shutdown으로 waiter를 끝내고, 지원 언어만 public cancellation을
  별도 실행한 뒤 같은 owner에 새 request를 보낸다.
  마지막으로 remote reply를 해제한다.
- 검증: 첫 awaitable은 variant의 terminal 하나를 반환한다. Follow-up request는 정상 reply를 받고
  late reply가 새 operation을 완료하지 않는다.
- 세부 동작: [Cancellation과 shutdown](../spec/server/05-async-execution-policy.ko.md#4-cancellation과-shutdown)을
  검증한다.
...
- 세부 동작: [Cancellation과 shutdown](../spec/server/05-async-execution-policy.ko.md#4-cancellation과-shutdown)과
...
- 세부 동작: [Public contract governance](../spec/server/00-public-contract-governance.ko.md)의 언어 parity를
  검증한다.
