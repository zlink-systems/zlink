...
- 세부 동작: [Transport liveness §6](../spec/server/02-channel-transport/05-transport-liveness.ko.md)을
  검증한다.
...
- 세부 동작: [Host maintenance §5](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)을 검증한다.
...
- 세부 동작: [오류 모델 §5](../spec/server/00-foundation/07-framework-error-model.ko.md)를 검증한다.
...
- 세부 동작: [Shutdown과 Relocate의 경쟁](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#14-shutdown과-relocate의-경쟁)을 검증한다.
...
- 검증: First 구간은 A만 처리하고 directed verification은 B가 처리한다. B process와 connection은 전체
  구간 유지된다.
- 세부 동작: [Channel topology §4.3](../spec/server/02-channel-transport/01-channel-topology.ko.md)을
  검증한다.
...
- 세부 동작: [Weight 변경은 연결을 다시 만들지 않는다](../spec/server/02-channel-transport/01-channel-topology.ko.md#weight-변경은-연결을-다시-만들지-않는다)의
...
- 세부 동작: [Channel messaging](../spec/server/02-channel-transport/02-channel-messaging.ko.md)과
  [오류 모델](../spec/server/00-foundation/07-framework-error-model.ko.md)의 request correlation과 provider failure 격리를 검증한다.
...
- 검증: Public status에는 replacement RID만 ready이고 follow-up requests가 모두 replacement evidence에
  기록된다.
- 세부 동작: [Runtime monitoring §2.2](../spec/server/06-observability/01-runtime-monitoring.ko.md)를 검증한다.
...
- 세부 동작: [Message flow tracing §5](../spec/server/06-observability/03-message-flow-tracing.ko.md)을
  검증한다.
...
- 검증: Logger provider는 negative operation의 `zlink.dispatch_error`, `no_handler`, `reply_error` fields를 제공하고 normal
  request는 성공한다.
- 세부 동작: [Message-flow tracing의 attribute 포함 조건](../spec/server/06-observability/03-message-flow-tracing.ko.md#32-attribute-포함-조건)과
  [완료, 실패와 수명](../spec/server/06-observability/03-message-flow-tracing.ko.md#6-완료-실패와-수명)을 검증한다.
...
- 세부 동작: [오류 모델 §2](../spec/server/00-foundation/07-framework-error-model.ko.md)를 검증한다.
...
- 검증: 각 checkpoint의 public status와 directed request가 expected target set을 확인한다. Requests는
  ...
  threshold로 사용하지 않는다.
- 세부 동작: [Transport liveness](../spec/server/02-channel-transport/05-transport-liveness.ko.md)의 반복 lifecycle 수렴을 검증한다.
...
- 세부 동작: [Mode에 맞는 target 선택](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#5-mode에-맞는-target을-선택한다)과
  [Relocation unit과 실행량 제한](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)을 검증한다.
...
- 세부 동작: [오류 모델 §8](../spec/server/00-foundation/07-framework-error-model.ko.md)을 검증한다.
...
- 세부 동작: [Target이 새 message를 받기 시작하는 시점](../spec/server/05-location-relocation/01-location-runtime.ko.md#92-target이-새-message를-받기-시작하는-시점)과
  [Unit 하나를 이전하는 순서](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#9-unit-하나를-이전하는-순서--04를-따른다)를 검증한다.
...
- 세부 동작: [Actor join과 commit 순서](../spec/server/03-spot-actor/05-spot-actor-membership.ko.md#4-actor-join과-commit-순서)와
  [모든 이동 경로가 공유하는 relocation policy](../spec/server/03-spot-actor/05-spot-actor-membership.ko.md#6-모든-이동-경로가-공유하는-relocation-policy)를 검증한다.
...
- 세부 동작: [Host maintenance §4](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)와
  [§10](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)을 검증한다.
...
- 검증: First는 `Blocked/DeadlineExceeded`이고 Host는 Serving이다. Second는
  ...
  오인하지 않는다.
- 세부 동작: [Relocate 완료와 실패](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#13-relocate-완료와-실패)와
  [Shutdown과 Relocate의 경쟁](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#14-shutdown과-relocate의-경쟁)을 검증한다.
...
- 세부 동작: [Host maintenance §8](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)을
  검증한다.
...
- 세부 동작: [Graceful drain — 대기 중인 message, timer와 session을 옮긴다](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md#12-대기-중인-message-timer와-session을-옮긴다)를 검증한다.
...
- 세부 동작: [Host maintenance §7](../spec/server/05-location-relocation/05-host-relocation-flow.ko.md)과
  [Actor와 Spot relocation 전체 흐름](../spec/server/05-location-relocation/04-relocation-flow.ko.md)의 chunk·예산 경계를 검증한다.
