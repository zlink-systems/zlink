...
- framework의 목표 public contract는 구현보다 먼저 정식 spec과 언어별 exact
  interface에 고정한다. 현재 구현과의 차이는 `spec/30-implementation-gap.ko.md`에서만
  추적한다.
...
현재 구현과의 차이와 후속 계획은 `spec/30-implementation-gap.ko.md`에서만 추적한다.
...
`spec/30-implementation-gap.ko.md`에서만 추적한다.
