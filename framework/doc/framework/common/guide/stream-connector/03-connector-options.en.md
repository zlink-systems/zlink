...
=== "Kotlin"
    ...
    ```kotlin
    // Starting from the default instance leaves only the changed items to write.
    val options = ZLinkStreamConnectorOptions
        .createDefault(URI.create("wss://game.example.com:443/stream"))
...
=== "C++"
    ...
    ```cpp
    options.reconnect.max_attempts = std::nullopt;   // An empty value means unlimited.
    ```

=== "C#/.NET"

    ```csharp
    Reconnect = new ZlinkStreamReconnectOptions { MaxAttempts = null }   // null means unlimited.
    ```

=== "Java"

    ```java
    int maxAttempts = ZLinkStreamConnectorOptions.UNLIMITED_RECONNECT_ATTEMPTS;   // -1
    ```

=== "Kotlin"

    ```kotlin
    val maxAttempts = ZLinkStreamConnectorOptions.UNLIMITED_RECONNECT_ATTEMPTS   // -1
    ```
...
=== "Java"
    ...
    ```java
    connector.setDiagnosticsLevel(ZLinkStreamDiagnosticsLevel.OFF);
    ZLinkStreamDiagnosticsLevel level = connector.diagnosticsLevel();
    ```

=== "Kotlin"

    ```kotlin
    connector.diagnosticsLevel = ZLinkStreamDiagnosticsLevel.OFF
    val level = connector.diagnosticsLevel
    ```
...
=== "Node/TypeScript"
    ...
    ```typescript
    connector.setDiagnosticsLevel(ZlinkStreamDiagnosticsLevel.Off);
    const level = connector.diagnosticsLevel;
    ```
