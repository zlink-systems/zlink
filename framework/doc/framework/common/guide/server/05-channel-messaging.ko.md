...
=== "Java"
    ...
    ```java
    // one-way — 제출을 받아들일 때까지. 값이 없는 완료다.
...
다른 언어의 등록 표면은 [framework-api §9](../../../common/spec/server/00-foundation/06-framework-api.ko.md#12-codec) 표를 본다.
...
- **handler 없는 packet으로 보냈을 때(런타임)** → **request는 error reply로 실패**(client는
  ...
  정식 structured record(`no_handler` / `reply_error`·`drop`)로 남는다(`11. Monitoring` 장).
