...
|---|---|---|
| Connect the client connection to an actor. | STREAM session binding | Delivers server push through the current binding route. [STREAM session](../../spec/server/04-session/01-stream-session.en.md) |
| Prepare the identity actor. | Actor GetOrCreate | Reuses the existing actor by its stable ActorId and type. [Interaction Model §2.1](../../spec/server/00-foundation/04-interaction-model.en.md#2-the-public-interface-that-starts-an-interaction) |
