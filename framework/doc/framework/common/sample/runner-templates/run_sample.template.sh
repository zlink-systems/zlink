...
#   samples/<language>/<Sample>/config/<role>.json   <- checked-in defaults
#
...
#   5. wait for readiness, then run the client self-check with CLI options
...
write_role_config() {
  ...
  sed -e "s|@API_ENDPOINT@|tcp://127.0.0.1:${API_PORT}|g" \
...
# API_CONFIG="$(write_role_config api)"

# 4-5. start role servers with a config file path, then run the client
#      self-check. Each server role has its own entry point -- do not switch
#      roles with --role/--mode on one executable. The client is not a framework
#      host, so it takes explicit CLI options instead of a config file.
#
#      The self-check's exit status is the verdict. Do not print a pass line
...
# SERVER_PID="$!"
# wait_for_readiness
# run_client_self_check \
#   --endpoint "tcp://127.0.0.1:${API_PORT}" \
