...
| `Relocation Store` | The first Instance activation envelope and planned-relocation operation records | Uses a provider and key prefix separate from the Location Store. It may share a Redis deployment, but not a keyspace. |
...
The `OrderWorkflowSpot` factory selects `RecreateOnRelocation`. It doesn't duplicate Application
state in a relocation payload; the target rebuilds the aggregate by replaying `OrderEventStore`.
The Framework preserves the queue, accepted journal, and owner fence, and the event stream doesn't
record Framework-internal state.
...
The runner closes a gate on the background continuation immediately after
`InventoryReservedEvent` is appended with the expected version. It starts the public planned
relocation operation on the source host and opens the gate after the target owner becomes Ready
with the same `OrderId` and `ObjectGeneration`. The target replays the stream, observes
`InventoryReserved`, and continues with `AuthorizePaymentReq`.

Server-side evidence records `ObjectGeneration`, owner identity, factory execution, and external
module calls before and after relocation. Assertions confirm that `ReserveInventoryReq` ran once,
the existing `ReservationId` was retained, and `AuthorizePaymentReq` started at the target. Final
event order and projection must match the success path without relocation. This flow isn't crash
failover or owner-loss recovery.
...
10. Confirm that after termination or an idle condition, a valid command can activate a new
    ...
    authority release.
11. Stop the continuation immediately after `InventoryReservedEvent`, run planned relocation, and
    confirm that the target with the same `ObjectGeneration` replays the stream and starts at
    `AuthorizePaymentReq`, without repeating `ReserveInventoryReq`.

### 9.3 Routing And The Failure Boundary

12. Process different orders concurrently on `CommerceApi x2` and `OrderWorkflow x2`, and confirm
    each order's state and event stream are the same regardless of which API is queried.
13. Terminate an already-Ready owner process, then confirm the same order command doesn't
    automatically create it on a different node — it ends in `Unavailable`.
14. Send continue or rebuild to an `OrderId` with neither a runtime instance nor an event stream,
    and confirm neither an empty order nor an `OrderStartedEvent` is created.
15. Confirm the caller configuration, message fields, and reservation contain no owner `NodeRid`,
...
2. Start test instances of the `Location Store`, `Relocation Store`, `OrderEventStore`, `OrderReadModelStore`, and
