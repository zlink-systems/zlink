...
방법이다. 두 결정은 application 호출을 받기 시작하는 시점에 직접 영향을 준다.

## 1. Liveness는 하나의 기준으로 판단한다

**결정 — mesh peer의 liveness 판단은 runtime 전체에서 하나의 구조가 소유한다.**
...
느린 구독자 때문에 message 처리가 지연되면 관측 기능이 서비스 처리량에 영향을 준다.
구독자별 대기 공간에 한도를 두고, 한도를 넘으면 **중간 상태를 합쳐서** 최신 상태를
전달한다. Message 처리 경로는 구독자를 기다리지 않는다.
...
**결정 — 공개하는 상태는 특정 시점의 복사본이다.** 현재 변경 가능한 내부 자료구조를
그대로 넘기면 읽는 쪽에서도 잠금이 필요하고, 같은 잠금이 message 처리 경로까지 영향을
준다.
...
상태가 자주 바뀔 때 모든 변화를 그대로 보내면 느린 구독자의 대기 공간이 빠르게 찬다. 그런데
...
쓰면 시계열 수가 객체 수만큼 늘어나 metric 수집기가 감당할 수 있는 한도를 넘는다.

**언어별 재량.** 구독을 밀어 주는 방식으로 만들지 끌어가는 방식으로 만들지는 자유다.
위의 "구독자 callback은 상태를 만드는 잠금 바깥에서 실행한다"를 지키면 어느 쪽이든 관찰
...
message 흐름 추적은 실행 중에 켜고 끌 수 있다. 꺼진 상태에서도 작업이 남아 있으면
**평상시 모든 message 처리에 그 비용이 발생한다.**
...
- mesh peer liveness 판단이 runtime 전체에서 하나의 기준을 쓴다.
