...
> This document defines the contract for the timer through which a [Spot](../00-foundation/02-glossary.en.md#spot)
> registers repeating or deferred execution, and the implementation that keeps resources from
...
keeping the following meaning regardless of backend.
...
before re-registration ran after re-registration, it would run something other than the new
period and new callback the caller expects.
...
scheduler's wall-clock error and nanosecond-precision timing are not a public result — these three
...
normalize an ignored value to a valid default instead of turning it into public meaning as is.
...
If lease renewal
stops and the monotonic deadline is exceeded, no new tick is queued and no callback is started
after resuming, even if the Framework process had been suspended.

Pending ticks from a
previous object/owner authority are not run either — if a timer kept running outside the
...
queue entries is the same, but only one scheduler and core thread are needed. The scheduler
...
count) don't grow in proportion to the number of registered timers is confirmed only by an
internal resource count. It cannot be observed from the public surface of timer
registration and callback execution alone.*

## 5. Internal Implementation Of Late-Tick Handling

How to handle a passed tick when execution runs late past the period is the public option
§2 defines. The implementation doesn't pick and fix one of the three policies — in particular,
...
scheduler implements as-is the §1 rule that "duplicate expirations may be collapsed into one
...
authority; `PerActor` uses authority per timer name. If a timer can't acquire its own
authority, that tick stays in the holding slot and retries next time. The rules for
...
Even a high-frequency timer does not round-trip the native callback boundary in a managed
...
- After cancel, a tick from that generation onward doesn't start its callback, but a callback
