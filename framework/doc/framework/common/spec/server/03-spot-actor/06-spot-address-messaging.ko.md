...
10. Store에 owner가 없고 target에도 현재 사용할 Spot이 없으면 target은 complete
    ...
    transaction에서 recovery receipt, provider가 발급한 reservation fence와 생성 중 capacity를
    기록한다.
11. 이 예약에 성공한 target만 factory와 initialize를 실행한다. 최초 message를 durable
    activation inbox의 첫 record로 확정할 때까지 handler 실행은 barrier로 차단한다(내부 확인
