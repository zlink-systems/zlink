...
[Spot·Actor 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [다음: 02. Spot 메시징](02-spot-messaging.ko.md)

> Entry Spot, User Spot과 Instance Spot의 공통점과 차이점을 정의한다. 세 종류 모두
> 주소와 상태를 가지고 순서대로 callback을 실행하는 [Spot](../00-foundation/02-glossary.ko.md#spot)이지만
> 생성 목적, Actor가 현재 어느 Entry Spot 또는 User Spot에 속하는지 나타내는 관계인
> [Actor membership](../00-foundation/02-glossary.ko.md#actor-membership), 종료와 relocation 계약이 서로
> 다르다.

## 1. Spot 모델 개요

이 문서는 "어떤 Spot 종류를 사용해야 하는가?"와 "Entry Spot이 어떤 역할을 하는가?"에
답한다.

여러 node가 참여하는 연결 topology에서 message를 보내거나 받는 runtime node를
[MeshNode](../00-foundation/02-glossary.ko.md#meshnode)라 한다. 다음 계약은 다른 문서가 소유한다.
...
| 등록·생성 | Object Server builder에 Spot 구현 type을 등록하고 startup에서 초기화한다. | 배포 version이나 실행 node가 바뀌어도 같은 종류의 Spot임을 식별하는 고정 이름인 [Stable type](../00-foundation/02-glossary.ko.md#stable-type)의 factory를 등록하고 manager `Create`·`GetOrCreate`로 만든다. | Stable type의 factory를 등록하고, target이 없을 때 새 Instance Spot을 준비해도 된다는 caller의 명시적 선택인 [Instance intent](../00-foundation/02-glossary.ko.md#instance-intent)를 가진 첫 direct call로 준비한다. |
| Spot을 식별하는 전역 논리 주소인 [Spot ID](../00-foundation/02-glossary.ko.md#spot-id) | Framework가 발급한다. 형식과 충돌 처리 규칙은 [MeshNode](03-mesh-node.ko.md)가 정의한다. | `Create`는 Framework가 발급하고 `GetOrCreate`는 caller가 지정한다. | Caller가 direct message의 target Spot ID를 지정한다. |
...
| 기본 실행 mode | Entry 고정 — 실행 계약 §2 참조 | `SpotWide` — 실행 계약 §2 참조 | Instance 고정 — 실행 계약 §2 참조 |
| Optional 실행 방식 | 제공하지 않는다. | Factory 등록에서 `PerActor`를 선택할 수 있다. | 제공하지 않는다. |
...
| ChannelName과 topic으로 같은 Channel의 여러 Spot에 message 하나를 전달하는 [Logical Multicast](../00-foundation/02-glossary.ko.md#logical-multicast) subscription | 지원한다. | 지원한다. | 지원하지 않는다. |
| Application의 명시적 close | Entry Spot context와 manager에 close operation을 제공하지 않는다. | 지정한 `SpotRef`를 manager `Close`에 전달하거나 local context에서 닫는다. | 자신의 handler나 timer context에서 닫는다. |
...
[Spot application queue](../00-foundation/02-glossary.ko.md#spot-application-queue)에 넣는다.
Actor에 전달된 업무 payload는 Spot queue를 거치지 않고 해당 Actor의 queue에
바로 넣는다.

### 3.1 Relocation 중에는 temporary queue를 먼저 확인한다

일반 dispatch는 기존처럼, 생성·초기화와, 각 Spot의 현재 owner와 상태를 여러 node가 함께
확인할 수 있도록 보관하는 저장소인 [Location Store](../00-foundation/02-glossary.ko.md#location-store)
기록이 끝나 message를 받을 수 있는 상태인 [Ready](../00-foundation/02-glossary.ko.md#ready) Actor나
Spot의 execution queue를 찾아 message를 넣는다.
Target runtime이 Restore 요청을 받으면 다음 packet을 dispatch하기 전에
[relocation temporary queue](../00-foundation/02-glossary.ko.md#relocation-temporary-queue)를 등록한다. 이후
Actor나 Spot message의 dispatch 순서는 다음과 같다.

1. Object 종류, ID와, 같은 논리 ID의 서로 다른 incarnation을 구분하는 번호인
   [`ObjectGeneration`](../00-foundation/02-glossary.ko.md#objectgeneration)을 검사한다.
...
`Yield`의 call별 제공 범위는 [§16](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-call-eligibility)이 소유한다.
...
규칙은 [Framework API의 handler 수명](../00-foundation/06-framework-api.ko.md#11-handler-실행-객체와-dependency-수명)을
따른다.
...
commit 순서는
[Spot과 Actor membership §4](05-spot-actor-membership.ko.md)가
정의한다.
...
| 3 | `IdleEvicted` | X | X | O | Instance Spot이 사용하지 않고 남아 있는 시간 기준을 넘겨 local instance를 내릴 때 호출한다. |
...
descriptor와 resolver에 Entry Spot을 공개하지 않는다. Entry Spot ID의 발급 규칙,
형식과 충돌 처리는 [MeshNode](03-mesh-node.ko.md)가 소유한다.
...
[Ready](../00-foundation/02-glossary.ko.md#ready) barrier 안에서 완료한다. Actor가 Entry Spot에
...
ActorId·ObjectGeneration을 유지한 채 target의, 하나의 물리 연결 그룹을 식별하는 이름인
[MeshName](../00-foundation/02-glossary.ko.md#meshname)·NodeRid로 갱신한다. 같은 Session에 bind된
relocation 대상에 포함되지 않은 다른 Actor의 route와 physical STREAM connection은 바꾸지 않는다.
Session owner는 Session·binding·Actor generation이 일치하는지와 relocation identity를 확인하고 held
message를 target route로 제출한 뒤 matching seal을 해제한다. 적용 reply는 없으며 그 command
44가 `SessionRelocationSealTimeout` 안에 없으면 physical Session과 관련 state를 정리한다. Target
Actor는 command 44 적용 reply 없이 message를 처리하며 이전 route의 message는, Actor나 Spot이
다른 MeshNode로 옮겨진 뒤에도 이전 owner에 도착한 message를 새 owner에게 대신 전달하는
[Message Follow](../00-foundation/02-glossary.ko.md#message-follow) route가 전달한다. Route 갱신은 같은
`ObjectGeneration`에만 적용하며, application은 relocation을 알기 위해 rebind하지 않는다. 새
incarnation은 application이 명시적으로 다시 bind해야 한다.
...
User Spot의 기본 [execution mode](../00-foundation/02-glossary.ko.md#user-spot-execution-mode)는
`SpotWide`다. 등록된 stable type에 맞는 Spot instance를 만드는 application 코드인
[Factory](../00-foundation/02-glossary.ko.md#factory)를 등록할 때 `PerActor`를 선택할 수 있다.
Mode는 MeshNode lifecycle 시작 전에 고정하고 실행 중에는 바꾸지 않는다.
Mode별 gate와 Yield는 [실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)을 따른다.

### 5.1 SpotWide relocation 경계

[`Spot relocation coordination mode`](../00-foundation/02-glossary.ko.md#spot-relocation-coordination-mode)의
...
열기 전에 호출하며, `Continued`이면 source가 다음 job 전에 호출한다. Callback이 완료되면 보류한
message와 timer를 다시 처리한다. Application은 다음 round를 이 callback에서 시작할 수 있다.
...
Creation request, placement, `SpotRef`와 close의 generation 일치 검사는
...
Session owner는 aggregate에 포함된 각 Actor의 [binding route](../00-foundation/02-glossary.ko.md#binding-route)를
...
connection은 바꾸지 않는다. 각 Session owner는 binding을 한 번 갱신하고 held message를
target route로 제출한 뒤 seal을 해제한다. 적용 reply는 없으며 timeout이면 physical Session을
정리한다. Target User Spot과 member Actor는 command 44 적용 reply 없이 message를 처리한다. Route
갱신은 같은 `ObjectGeneration`에만 적용하며, application은 relocation을 알기 위해 rebind하지
않는다. 새 incarnation은 application이 명시적으로 다시 bind해야 한다.
...
Actor는 normal host scheduler에서 각각 이전하며 relocation 전용 동시 unit 상한을 두지 않는다.
Actor의 `ObjectGeneration`과 logical User Spot membership은 유지하고 Actor owner generation만
바꾼다. Infrastructure relocation은 `OnActorJoinAsync`, `OnJoinedActorAsync`,
`OnLeaveActorAsync` 또는 `OnDisconnectActorAsync`를 호출하지 않는다. 마지막 Actor와 source에서
이미 수락한 Spot 작업을 모두 정리한 뒤 source shell에 `RelocationOut`을 전달하고 종료한다. 각
Actor가 Session에 bind되어 있으면 target runtime이 command 44 `sessionRelocationRoute` commit을
one-way로 보내 해당 binding route와 bound-session current Actor location snapshot을 같은
ActorId·ObjectGeneration을 유지한 채 target owner 및 target MeshName·NodeRid로 갱신한다. 이
control은 command 44 commit의 one-way send이며 적용 reply나 재전송 journal이 없다. Target
Actor는 command 44 적용 reply 없이 message를 처리한다. Application은 이 갱신을 위해 rebind하지
않는다.
...
Global Spot ID 하나를 지정해 send나 request를 보내는
[Spot direct](../00-foundation/02-glossary.ko.md#spot-direct) call은 기본적으로 실행 중인 Spot만
찾는다. Missing RID에서 Instance
...
[Spot 주소 메시징](06-spot-address-messaging.ko.md)이
정의한다.

Instance Spot은 application handler나 timer가 자신의 context에서 닫을 수 있다.
Host `Relocate`에서는 Actor가 없는 Spot 하나를 relocation unit으로 처리한다.
Instance Spot callback의 실행과 Yield는
[실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)을 따른다.
...
### 6.2 사용하지 않고 남아 있는 Instance Spot 정리

Framework는 Instance Spot을 사용하지 않고 남아 있는 시간 기준으로 정리할 수 있다.
**User Spot과 Entry Spot은 정리하지 않는다** — 일반 message는 없는 object를 만들지
않으므로([Spot·Actor routing](08-routing.ko.md)) 정리한 User Spot으로 온 message는
되살아나지 못하고 실패한다. Instance Spot은 Instance intent를 명시한 call이 다시
cold activation하므로 정리해도 다음 call에서 복구된다.

정리는 다음 두 조건을 **함께** 만족할 때만 시작한다.

| 조건 | 내용 |
|---|---|
| 사용하지 않고 남아 있는 시간 | 마지막 application 작업 완료 이후 `InstanceSpotIdleTimeout`을 넘겼다. 기본값은 `0`이며 `0`은 정리하지 않음을 뜻한다. |
...
전에 `MeshName`, `SpotId`, `ObjectGeneration`, `NodeRid`와 owner fence가 결합된 Context를 만든다.
...
[.NET Spot interface](../languages/dotnet/interfaces/05-spots.ko.md)가 소유한다.

## 8. 문서 경계

| 문서 | 소유하는 상세 계약 |
|---|---|
| [Spot 메시징](02-spot-messaging.ko.md) | Spot direct, Logical Multicast, queue admission과 dispatch |
| [MeshNode](03-mesh-node.ko.md) | Object role, Entry Spot ID와 factory 등록, placement capability |
| [Spot과 Actor membership](05-spot-actor-membership.ko.md) | Actor 생성, Entry·User Spot membership과 callback·commit 순서 |
| [Spot 주소 메시징](06-spot-address-messaging.ko.md) | User·Instance Spot의 ID, 생성, cold activation, route와 close |
| [Host Relocate, Shutdown과 handoff](../05-location-relocation/05-host-relocation-flow.ko.md) | 세 Spot 종류의 shutdown, relocation과 recovery 순서 |

## 9. 검증 요구

공개 표면(Spot lifecycle callback, manager `Create`·`GetOrCreate`, application이 관찰하는
relocation 결과)만으로 다음을 확인한다.
