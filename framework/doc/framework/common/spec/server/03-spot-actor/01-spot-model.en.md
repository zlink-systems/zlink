...
| Default application execution | Spot handler and timer are serialized on the [Spot turn](../00-foundation/02-glossary.en.md#spot-turn) — the unit in which one callback occupies the application queue's execution gate to run; Actors run per Actor. | `SpotWide`: serializes the whole Spot/member Actor/timer/lifecycle callback. | Serializes the direct handler and timer across the whole Spot. |
...
doesn't go through the Spot queue, and multiple queues aren't merged into
one. However, since every queue shares one common execution gate, the same
User Spot only runs one of the Spot handler, timer callback, or member Actor
handler at a time.
...
unit. An Instance Spot's direct handler and timer share one Spot execution
gate. Returning this turn via `Yield` lets the next Instance Spot record
run, and the continuation resumes as a new turn on the same gate.
