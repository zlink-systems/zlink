...
[Spot·Actor 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 04. Actor 모델](04-actor-model.ko.md) · [다음: 06. Spot 주소 메시징](06-spot-address-messaging.ko.md)

> **이 장이 정의하는 것** — Actor 생성, Spot membership, relocation과 aggregate
> relocation. 자동 failover는 범위 밖이다.

Process 종료 뒤 다른 runtime이 relocation을 이어받는 자동 failover는 이 문서의
계약에 포함하지 않는다.

[Core](../00-foundation/02-glossary.ko.md#location-store)는 raw socket과 transport만 제공한다. Object가
현재 어느 [Spot](../00-foundation/02-glossary.ko.md#spot) — 주소와 상태를 가진 논리 instance인 Entry
Spot 또는 User Spot — 에 속하는지 나타내는 관계인
[Actor membership](../00-foundation/02-glossary.ko.md#actor-membership), relocation 상태와 lifecycle은
각 언어의 Framework runtime이 관리한다.

## 1. Identity와 authority

Entry·User·Instance 세 종류가 있는 Spot의 ID는 ActorId와 함께 Location Store
namespace 전체에서 전역인 logical key다. [`MeshName`](../00-foundation/02-glossary.ko.md#meshname)은
object를 처음 배치할 곳을 정할 때 사용하는 속성이며 authority key에는 포함되지 않는다.

[Location Store](../00-foundation/02-glossary.ko.md#location-store)는 각 logical key마다 현재 object를 어느 node가 처리하는지와 Actor가
어느 [Spot](../00-foundation/02-glossary.ko.md#spot)에 속하는지를 기록한다. 이 현재 처리 권한과 위치 기록을 authority라
한다. Object가 다른 node로 이동하면 logical key는 그대로 유지하면서 현재 owner
정보만 새 값으로 바꾼다.

`ActorRef`와 `SpotRef`의 [`ObjectGeneration`](../00-foundation/02-glossary.ko.md#objectgeneration)은
같은 logical identity로 다시 만들어진 incarnation을 구분하는, 0이 아닌 unsigned 63-bit
conceptual value다. 같은 incarnation에서 membership이나 [owner](../00-foundation/02-glossary.ko.md#owner) MeshNode가 바뀌어도
`ObjectGeneration`은 유지한다. 대신 provider가 더 큰, 같은 incarnation에서 authority owner가
바뀐 순서를 나타내는
[`AuthorityOwnerGeneration`](../00-foundation/02-glossary.ko.md#authorityownergeneration)을
발급하여 새 owner를 구분한다.
...
| 현재 owner lease | Authority에 기록된 owner lifecycle이 아직 유효한지 검증한다. |

Runtime route cache는 [authority](../00-foundation/02-glossary.ko.md#authority) row의 snapshot일
...
Actor와 User Spot을 manager로 만들 때와 [Instance Spot](../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot)의 첫 message로 생성할 때는
...
| [Spot direct](../00-foundation/02-glossary.ko.md#spot-direct)(Spot을 식별하는 전역 논리 주소인 global [Spot ID](../00-foundation/02-glossary.ko.md#spot-id) 하나를 지정해 그 Spot에 send/request를 보내는 방식)로 만드는 Instance Spot [cold activation](../00-foundation/02-glossary.ko.md#cold-activation) | Source가 최초 message와 생성 정보를 target에 먼저 보낸다. Target에 현재 사용할 수 있는 Spot이 없으면 target이 자신에게 이 Spot을 만들어도 되는지 Location Store에 요청한다. |

두 방식의 공통 결과는 같다. Location Store가 한 target에만 생성 권한을 주고, 다른
target은 별도 [factory](../00-foundation/02-glossary.ko.md#factory)를 시작하지 않는다.

Object factory를 등록하고 같은 type인지 비교할 때 사용하는 변경되지 않는 이름을
stable type이라 한다.

Remote User Spot manager create는 reservation 뒤 그 target에 별도 terminal service operation을 보낸다.
이 operation은 source와 target node lifecycle, global Spot key·[stable type](../00-foundation/02-glossary.ko.md#stable-type),
provider가 발급한 reservation,
`StoreVersion`과 deadline을 고정한다. Target은 `Reserved` allocation의 `pendingCreation`을 Location Store에서
Store에서 직접 읽은 뒤 factory·initialize·Commit을 실행한다. Location row polling이나 application control packet은
terminal result가 아니다.

Remote User Spot close도 종료 대상 `SpotRef`, owner generation·`StoreVersion`과 target lifecycle을 가진 별도
...
5. 생성 callback이 승인하면 Store terminal `Commit`이 같은 fence를, Spot 생성·초기화와
   Location Store 기록이 끝나 application message를 받을 수 있는 상태인
   [`Ready`](../00-foundation/02-glossary.ko.md#ready)로
...
target descriptor, typed capacity bundle, 현재 owner lease와 `StoreVersion`을
...
[Ready](../00-foundation/02-glossary.ko.md#ready) 준비 전체에 적용된다. [Deadline](../00-foundation/02-glossary.ko.md#deadline)이 끝나면,
operation에 허용된 deadline까지 완료 조건을 만족하지 못했을 때 발생하는 Framework
exception인 [`DeadlineExceeded`](../00-foundation/02-glossary.ko.md#deadlineexceeded)다. Creating 상태를 재확인하던 caller가 이 deadline에 도달하면 해당
caller는 `DeadlineExceeded`로 끝나지만, 생성 attempt 자체가 실패했다고 간주하지는 않는다.
다음 call은 Store의 현재 authority와 retained terminal record를 다시 확인하여 중단된
...
[terminal result](../00-foundation/02-glossary.ko.md#creation-terminal-result)다. Callback exception은
...
authority와 reserved capacity를 정리한다. Terminal record는 source Node
RID·lifecycle generation·`OperationId`가 모두 일치해야 식별하며 같은 operation의 재전송에만 사용한다.
...
[Spot 모델](01-spot-model.ko.md)이 정의한다. 이 절은 Entry·User Spot이 Actor
membership을 처리하는 순서만 정의한다.

Actor를 만들면 selected owner [MeshNode](../00-foundation/02-glossary.ko.md#meshnode)의 Entry Spot이 initial membership을 처리한다. Actor 업무 message는
...
`NotifyDisconnected`의 명시적 logical notification에서 실행된다. Framework는 binding identity마다
...
authority를 유지한다. Caller가 명시적 leave 또는 destroy를 끝낸 뒤에만 닫을 수 있다. Framework는 Actor를
숨겨서 이동하거나 파괴하지 않는다.
...
[.NET Actor interface](../languages/dotnet/interfaces/06-actors.ko.md)가
정의한다.
...
[Spot ID](../00-foundation/02-glossary.ko.md#spot-id)와 admission 판단에 필요한 request만 지정한다. Framework가
...
대기 중인 message를 처리한다. Bound Session이 있으면 relocation을 시작하기 전에 Session
owner가 해당 binding을 seal한다. Target 준비와 target-only Location Store CAS가 끝난 뒤
Session owner는 binding route와 current `ActorRef` 위치 snapshot을 target으로 바꾸고 seal을
해제한다. Session은 target을 선택하거나 Location Store를 변경하지 않는다. 갱신 전의 route로
도착한 message는, Actor가 relocation된 뒤에도 이전 owner에 도착한 message를 새 owner에게
대신 전달하는 동작인 source의 [Message Follow](../00-foundation/02-glossary.ko.md#message-follow)
route가 target Actor에 전달한다. Session owner가
이 seal·검증·route 전환을 어떻게 수행하는지는
...
Session owner에 target route 적용과 seal 해제를 one-way로 알린다.
...
기다린다. `Relocate` seal이 먼저면 Join은 `Unavailable`로 끝난다. Runtime이 종료를 진행
중이어서 새 operation admission을 받을 수 없는 상태인
[Shutdown](../00-foundation/02-glossary.ko.md#shutdown) admission
...
2. Handler가 정상적으로 끝나면 Framework가 target을 확인한다. Target이 User Spot이면
   ...
   [relocation temporary queue](../00-foundation/02-glossary.ko.md#relocation-temporary-queue) 등록과 factory
   ...
   ([51 §9](../02-channel-transport/06-wire-protocol.ko.md) 참조). 이후 Restore 요청에서 이 준비를 반복하지
   않으므로 왕복 수는 그대로이고 seal 뒤 처리 시간만 줄어든다. 승인 reply에는 relocation payload를 나눠 보내는
   단위인 [relocation state chunk](../00-foundation/02-glossary.ko.md#relocation-state-chunk)의 target 유효
...
4. Source runtime이 target runtime에 **Actor Restore 요청을 먼저 보낸다**. Restore 요청에는
   ...
   [Actor와 Spot relocation 전체 흐름](../05-location-relocation/04-relocation-flow.ko.md)이 정의한다. Target
...
target attempt로 구성한 identity로 식별하며, 이동이 시작되지 않으면 준비
...
다른 identity의 승인이나 Restore가 도착하면 target은 기존 준비 상태를 먼저
...
sequenceDiagram
    ...
    Handler->>SourceRuntime: [local] JoinSpot(...) 호출 뒤 반환 객체에서 Defer() 호출
    Handler-->>SourceRuntime: [local] Handler 정상 종료
    ...
    alt Accepted
        ...
        SourceRuntime->>SourceActor: [local] 기존 source membership 유지
    end
```
...
유지한다(위 규칙 참고). `OnLeaveActor`는 owner commit 뒤에만 보내므로 이 source 복구 경로에서는 호출하지 않는다. User Spot target은 승인 처리에서
...
source로 되돌리지 않는다. 같은 target process가
...
[ObjectGeneration](../00-foundation/02-glossary.ko.md#objectgeneration)은 그대로 유지한다. Cross-node 이동으로
...
record를 도착 순서대로 source queue에 되돌리고 target temporary queue를 폐기한다(위 규칙 참고).
Owner commit이 성공하면 저장된 기존 작업 뒤에
...
흐름](../05-location-relocation/04-relocation-flow.ko.md#3-무엇을-한-번에-옮기는가)을 참고한다. Node의 Entry Spot을
...
scope를 정리하고 Location authority와 resource를 정리한다.
...
## 6. 모든 이동 경로가 공유하는 relocation policy

Actor·User Spot·Instance Spot의 [Object Server](../00-foundation/02-glossary.ko.md#object-client와-object-server-role) factory는 다음 policy 중 하나를 반드시 등록한다.
...
| `PreserveStateWith` | Handler가 정상적으로 끝난 경계의 application state를 object 종류에 맞는 relocation adapter로 opaque byte sequence에 갈무리하고 target에 복원한다. Framework queue·timer 정보도 함께 유지한다. |
...
동시 전송량을 제한하는
[in-flight payload budget](../00-foundation/02-glossary.ko.md#in-flight-payload-budget)은
[Actor와 Spot relocation 전체 흐름](../05-location-relocation/04-relocation-flow.ko.md)이 정의한다.
...
[Graceful drain과 handoff §8](../05-location-relocation/05-host-relocation-flow.ko.md#9-unit-하나를-이전하는-순서--04를-따른다)이
정의한다.

## 7. User Spot과 member Actor를 함께 이동하는 maintenance aggregate
...
1. Spot queue turn 경계에서 source User Spot의 join·leave와 모든 participant application
   admission을 reversible하게 seal한다.
2. Seal 시점의 participant inventory를 그대로 immutable tree로 저장하고 root·count·digest를 검증한다.
3. 모든 relocation 구성과 target type·state 보존 adapter capability를 확인한다.
4. `PreserveStateWith` participant의 state, 실행하지 않은 message queue와 timer logical
   registration·pending tick을 갈무리하고 target factory·restore를 admission이 닫힌 상태로 준비한다.
5. 준비를 끝낸 target이 Location Store의 단일 CAS로 aggregate owner, generation과 inventory
   root를 전환한다. 이 CAS가 성공하면 Spot과 모든 member Actor가 함께 새 owner를 사용한다.
6. Authority commit 뒤 target lifecycle callback, 저장한 message replay와 Framework timer 자동
   복원을 끝내고 target User Spot과 member Actor가 message를 처리하기 시작한다. Bound Actor의
   target runtime은 bound Actor마다 Session owner에 target route 적용과 seal 해제를 one-way로
   알린다. Session owner는 그 update를 받으면 route를 바꾸고 seal을 해제한다. 같은 Session에서
   aggregate 밖 Actor의 route와 physical STREAM connection은 유지한다.
...
queue, timer와 bound-session current Actor location
...
변경 callback이 아니며 member Actor에 전달하지 않는다. Callback을 재정의한
...
[AuthorityOwnerGeneration](../00-foundation/02-glossary.ko.md#authorityownergeneration), owner lease와 local
admission deadline 때문에 message, timer, phase update와 cleanup을 수행하지 못한다.

## 9. Message Follow

Commit 뒤 source는 `MessageFollowDuration` 안에서 이전 주소로 도착한 message를 committed
target으로 relay한다. Relay는 Store를 읽거나 application handler를 실행하지 않으며 original
operation identity, generation, payload와 reply route를 유지한다. Route duration이 끝난 뒤 이전
주소로 도착한 request는 `Unavailable`로 끝난다.

Message Follow는 Session route update를 기다리기 위한 재전송 queue가 아니다. Target이
Session route update를 보내더라도, 이미 이전 주소로 전송된 server message를 처리하기
위해 정해진 기간 동안 유지한다. 서로 다른 connection의 message 사이에 전역 순서는 보장하지
않는다.

## 10. Bound Session

Actor가 이동해도 physical STREAM connection, Session identity와 ObjectGeneration은
유지된다. Relocation 전체 흐름에서 Session owner가 담당하는 seal 설치, held message 보관,
route 검증과 전환, timeout 처리는 §4.2의 8-step 흐름(특히 8번)이 Actor·target 쪽 절차를
정의하며,
[Session–Actor binding §8](../04-session/02-session-actor-binding.ko.md#8-actor-relocation-중-session의-책임)이
Session owner 쪽 검증·처리 절차를 소유한다 — 이 문서는 그 문서를 다시 서술하지 않고
가리킨다.

정확한 Session route 계약은
...
- Object role이 Store 없이 시작하지 않고 hidden local manager를 만들지 않는다.
...
- 서로 다른 operation은 Ready 뒤 `Existing`을 받고 cleanup 뒤 새 reservation을
  경쟁하며, 같은 source lifecycle·`OperationId`의 재전송만 terminal을 다시 적용한다.
- `Rejected`와 `Failed`가 Ready authority와 active capacity를 만들지 않고 pending
  capacity를 반환한다.
...
- `Defer()` 뒤 source seal 전 message는 barrier 뒤 Actor queue에 두고, seal 뒤
  message만 [relocation ingress hold](../00-foundation/02-glossary.ko.md#relocation-ingress-hold)에 보관한다.
...
- 기존 준비가 남은 상태에서 다른 identity의 승인·Restore가 도착하면 기존 준비가
...
- User Spot과 member Actor는 target이 실행하는 Location Store conditional batch CAS 한 번으로
  함께 전환된다.
- Commit 뒤 failure가 participant 일부를 source로 되돌리지 않는다.
- Message Follow는 commit된 route만 사용한다. Relay queue에는 relocation 전용 record 수나
  byte 상한을 두지 않으며, [operation identity](../00-foundation/02-glossary.ko.md#operation-identity)를
  그대로 보존한다.
- Bound STREAM connection은 이동하지 않는다. Session owner 한 곳에서 current Session, binding
  generation과 relocation identity를 확인하고 해당 Actor route와 location snapshot만 target으로
  바꾼다. ActorId·ObjectGeneration은 유지한다.
