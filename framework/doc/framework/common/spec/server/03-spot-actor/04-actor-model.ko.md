...
[Spot·Actor 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 03. MeshNode](03-mesh-node.ko.md) · [다음: 05. Spot과 Actor membership](05-spot-actor-membership.ko.md)
...
Actor가 주소와 상태를 가진 논리 instance인 [Spot](../00-foundation/02-glossary.ko.md#spot) —
[Entry Spot](../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot)이나 User Spot —
또는 여러 node가 참여하는 연결 topology에서 message를 보내거나 받는 runtime node인
remote [MeshNode](../00-foundation/02-glossary.ko.md#meshnode)에 있더라도
application payload는 Actor 자신의 queue에 제출한다. Queue에 들어온 handler를 실행할
gate는, Actor가 현재 어느 Entry Spot 또는 User Spot에 속하는지 나타내는 관계인
[Actor membership](../00-foundation/02-glossary.ko.md#actor-membership)과 User Spot execution
mode가 결정한다.
...
| 관련 계약 | 정의하는 문서 |
|---|---|
| MeshNode route와 peer admission | [MeshNode](03-mesh-node.ko.md) |
| [Spot](../00-foundation/02-glossary.ko.md#spot) membership transaction과 relocation | [Spot과 Actor membership](05-spot-actor-membership.ko.md) |
| [STREAM session](../00-foundation/02-glossary.ko.md#stream-session)(하나의 client connection을 수락한 때부터 닫을 때까지 유지하는 서버 실행 단위) 연동 | [Session–Actor binding](../04-session/02-session-actor-binding.ko.md) |
| Payload와 metadata | [메시지 모델](../00-foundation/05-message-model.ko.md) |
| Callback 실행과 completion | [비동기 실행 정책](../01-execution/01-submit-and-completion.ko.md) |

## 2. Actor identity와 서로 독립적인 상태

### 2.1 ActorId와 stable type

Actor는 각 Spot의 현재 owner, ObjectGeneration과 lifecycle 상태를 여러 node가 함께
확인하도록 보관하는 저장소인 [Location Store](../00-foundation/02-glossary.ko.md#location-store)
namespace 전체에서 유일한 logical `ActorId`로 식별하는 stateful object다.

`ActorId`는 UTF-8 `1..255` bytes이며 대소문자를 구분해 그대로 비교하는 값이다. Framework는
Unicode normalization이나 case folding을 적용하지 않는다.

[`MeshName`](../00-foundation/02-glossary.ko.md#meshname)은 Actor를 처음 배치할 곳을
선택할 때 사용하는 속성이며 Actor identity에 포함되지 않는다. 따라서 같은 `ActorId`를 서로 다른 Mesh마다 중복 생성할 수 없다.
...
| [`ObjectGeneration`](../00-foundation/02-glossary.ko.md#objectgeneration) | 같은 ActorId의 서로 다른 logical incarnation을 구분하는 0이 아닌 unsigned 63-bit 값이다. Relocation 중 target에서 Actor 객체를 다시 만드는 `RecreateOnRelocation`은 같은 incarnation을 계속 사용하므로 이 값을 바꾸지 않는다. |
| 현재 `MeshName` | 현재 owner가 속한 Mesh다. |
| 현재 `NodeRid` | 현재 [owner](../00-foundation/02-glossary.ko.md#owner) node의 RID다. |

`ActorRef`는 Actor message의 target으로 사용하는 값이 아니다. Actor가 이동하거나
다시 만들어지면 이전 `ActorRef`는 stale할 수 있다.

[`ObjectGeneration`](../00-foundation/02-glossary.ko.md#objectgeneration)은 JSON에서 decimal
string으로 표현한다. 별도의 `ActorRefSnapshot` public type은 제공하지 않는다.
...
|---|---|---|
| Spot membership | [Entry Spot](../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot), user Spot, 이동 중 | Actor의 logical 위치와 Spot membership을 나타낸다. |
...
[비동기 실행 정책](../01-execution/01-submit-and-completion.ko.md)이
정의한다. Entry Spot과 User Spot이 Actor membership을 처리하는 순서 자체는
[Spot과 Actor membership](05-spot-actor-membership.ko.md#3-entry-spot과-user-spot의-actor-membership)이
정의하며, 이 절의 gate·`Yield` 규칙이 membership 상태와 관계없이 그대로 적용된다.
...
먼저 실행되지 않도록 [Deferred Join barrier](../00-foundation/02-glossary.ko.md#deferred-join-barrier)(비활성
queue barrier)만 등록한다.
...
stable type에 맞는 Actor instance를 생성하는 application 제공 코드), `Configure`, lifecycle callback, relocation adapter, detached task,
...
[Message Follow](../00-foundation/02-glossary.ko.md#message-follow)로 새 owner에 전달한다.
...
Spot은 Actor application payload를 처리하지 않는다. [Spot turn](../00-foundation/02-glossary.ko.md#spot-turn)에서 처리하는 Actor
...
|---|---|---|
| owner 점유 상한 | **서로 다른 owner** 사이 | [Framework API](../00-foundation/06-framework-api.ko.md) |
...
([Spot 메시징 §5.4](02-spot-messaging.ko.md)).
...
> 최상위 스펙 README는 owner 점유 상한과 lifecycle 연속 실행 상한을 이 절의 기준으로
> 인용한다. 이 문서가 새 경로로 옮겨가면 절 번호가 `## 4`로 바뀌므로, 최상위 README의
> 해당 인용도 이동 단계(§5)에서 함께 갱신해야 한다.

## 5. Actor 메시징

Actor send/request의 target은 global `ActorId`다. Framework는 [Ready](../00-foundation/02-glossary.ko.md#ready) 상태인 현재
incarnation과 [authority](../00-foundation/02-glossary.ko.md#authority)가 가리키는 owner route를
positive route cache 또는 [Location Store](../00-foundation/02-glossary.ko.md#location-store)에서 찾는다.
그 뒤 owner fence를 확인하고 target queue에 message를 제출한다. 찾을 때 확인한
...
[Framework API의 handler 수명](../00-foundation/06-framework-api.ko.md#11-handler-실행-객체와-dependency-수명)을
따른다.

Actor와 Actor Context는 composition 관계다. Framework는 factory를 호출하기 전에 `ActorId`,
`ObjectGeneration`, current `MeshName`, nullable current `SpotId`와 bound-session capability를 가진 Context를 만든다. Factory는 ID를 별도 인자로 받지 않고 이 Context만 받는다. 반환한 Actor는 전달받은
...
- Actor [stable type](../00-foundation/02-glossary.ko.md#stable-type)
- [Factory](../00-foundation/02-glossary.ko.md#factory)
...
같은 object incarnation에서 authority owner가 바뀐 순서를 나타내는 provider 발급
...
Terminal submit을 시작할 때 end-to-end [deadline](../00-foundation/02-glossary.ko.md#deadline) 하나를 고정한다. 이 deadline은
...
Factory가 만든 Actor는 아직 외부에 공개되지 않은 staging instance다. 승인 여부는
Entry Spot의 `OnCreateActor`가 결정하며, 거절된 Actor는 `Find`로 조회할 수 없고
message를 받을 수 없으며 active capacity를 소비하지 않는다. Actor를 Ready로 공개한 뒤
destroy하는 방식으로 거절을 구현하지 않는다.
...
여러 process가 동시에 `GetOrCreate`를 호출해도 Location Store의
[reservation](../00-foundation/02-glossary.ko.md#reservation-id) CAS에 성공한 caller만 생성 실행을
소유한다.

`Created`·`Rejected`·`Failed`·`Existing` 각 상태의
의미, 동시 생성 경쟁에서 나중 operation이 어떻게 합류하는지, 그리고 `OperationId` 재전송이
terminal record를 얼마나 보존하는지는
[Spot과 Actor membership §2](05-spot-actor-membership.ko.md#2-object를-하나만-생성하도록-확정하는-과정)가
정의한다.
...
[Spot과 Actor membership](05-spot-actor-membership.ko.md)의 fencing과 barrier를 따른다.
...
[Spot과 Actor membership](05-spot-actor-membership.ko.md)이 정의한다.
...
[Session–Actor binding](../04-session/02-session-actor-binding.ko.md)이 정의한다.
...
| ActorRef로 직접 지정한 호출에서 mapping이 없다. | `Unavailable`로 끝난다. |
...
- Actor payload가 Spot callback이나 [Spot application queue](../00-foundation/02-glossary.ko.md#spot-application-queue)를 거치지 않는다.
...
- Restore 중 도착한 message를 temporary queue에서 실행하지 않는다. Commit 뒤 saved work,
  boundary 전 relay와 나머지 temporary 작업을 실제 Actor queue에 순서대로 넣고 regular route로
  전환한다. 필요한 lifecycle 작업을 끝낸 뒤 dispatch를 연다.
...
- Actor messaging이 ActorId만 받고 [owner route](../00-foundation/02-glossary.ko.md#owner-route)와 generation을 application에 요구하지 않는다.
