...
> **What this document defines** — the scope in which the Framework
...
This document defines the scope in which the Framework automatically continues the same work
...
The Framework doesn't judge an operation's execution as confirmed merely because it selected a
processing target. Before transport or the target queue accepts the operation, it can confirm
the operation hasn't run, so it can select a different allowed target. Once acceptance status is
unclear or already accepted, the same operation isn't automatically submitted to a different
target, to prevent duplicate execution.

In this document, failover means switching a failed processing target to a different target and
...
A logical target that stays reachable at the same ID even when the node actually executing it
changes is a [Spot](../00-foundation/02-glossary.en.md#spot). The node currently processing an
...
4. For a stateful relocation, checks whether it's before relay-ready reply becomes accepted,
...
change again as the earlier work, the application protocol must prevent duplicate impact via an
...
called [select-one](../00-foundation/02-glossary.en.md#select-one). A
[RouteMesh](../00-foundation/02-glossary.en.md#routemesh) — the scope where multiple MeshNodes
participate to exchange node and Channel messages — Channel uses a server that's in
...
failure doesn't stop processing on a different ready peer and local owner, or turn the whole host
`Error`.
...
reused. If whether transport accepted an operation before the connection loss is unknown, that
...
is called [`ObjectGeneration`](../00-foundation/02-glossary.en.md#objectgeneration). Where it's
used and where it isn't is set by
[Spot/Actor Routing §2.5](../03-spot-actor/08-routing.en.md#26-where-objectgeneration-is-used-and-where-its-not).
The per-operation application table and the result when an owner has disappeared are there.

This distinction makes one difference for failure handling. **A regular Actor/Spot message only
uses the global logical ID as target.** Since it targets the current Ready object of the logical
ID, not a generation, an object re-created under the same ID and an object that lost its owner
end with different results. `ObjectGeneration` is **excluded from a regular message's
...
If the current `Ready` Actor's or Spot's owner process terminates, the Framework doesn't
...
applies identically to Instance Spot. Being of the Instance Spot kind alone doesn't release the
[authority](../00-foundation/02-glossary.en.md#authority) — the reference information that
determines which node an Actor or Spot is on and which node is currently the owner — after owner
lease expiry, or convert the next message into cold activation.
...
This behavior is recovery before creation is exposed as `Ready` — not failover recovering an
...
An Instance Spot doesn't call a separate create API — it's created from the `Missing` state by
the first message. This trait only decides **when** the object is created — it doesn't add a
failover policy that automatically restores the object on a different node after a `Ready` owner
failure. The caller's explicit choice that it's fine to create a new Instance Spot when the target
...
would require defining a separate failover contract: under what condition to release a failed
...
storage and resume order are defined by [Location Runtime](01-location-runtime.en.md)'s
first-message storage-and-resume section.

## 5. Host Relocation Failure

`Relocate` is a planned action where the running source and the selected target hand off state
and not-yet-executed work. It isn't an operation that finds an owner to substitute for a failed
host. The current version only supports a graceful handoff that runs until the source runtime,
the selected target runtime, the Location Store, and the Relocation Store finish the operation.
...
operation before its irreversible boundary. Source isn't restored afterward even before owner
...
and the application must redo authentication and bind on the new Session. The previous
...
liveness checking of existing connections. It doesn't create a new outbound connection. Grace
doesn't extend the owner lease or relocation deadline. Once owner eligibility ends, new
message/timer processing and state changes stop. Once the Store recovers, owner and the whole
descriptor are re-confirmed, and only needed connection changes are applied.

The re-confirmation and payload order for a Store request are defined by
[Location Runtime](01-location-runtime.en.md)'s Store-response-loss section.

## 8. The Application's Retry Decision

The Framework returns a failed operation's `ErrorKind` but doesn't provide whether to retry. This
is because, on a timeout or connection loss, whether the remote handler ran may be unknown. The
application decides whether to start a new operation after preventing duplicate impact via
operation idempotency, a business-level idempotency key, result lookup, or state comparison.

The Framework re-selecting a target before acceptance, or re-confirming a Store result, is
internal processing of the same operation. It's distinguished from a new operation the
...
**This document is authoritative for public failure behavior.** Implementation structure
continues in the following documents, and those documents don't redefine this chapter's
...
Confirmed by the public surface (target selection result, returned `ErrorKind`/outcome, Location
Store record lookup, Session binding state) alone. Each item leads to one test.
...
- One peer's liveness failure doesn't turn a different ready peer or the host state `Error`.
- A reconnect redoes handshake and identity verification, and doesn't reuse a previous
...
- Even if a `Ready` authority still has an activation recovery pointer, it is used only to resume
  the incomplete first cold-activation operation on the same target node and lifecycle designated
  by authority. It is never a basis for selecting another target after a steady `Ready` owner
  failure.
