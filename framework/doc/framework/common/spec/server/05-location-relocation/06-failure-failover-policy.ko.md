...
[Location·Relocation 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 05. Host relocation 전체 흐름](05-host-relocation-flow.ko.md)

> **이 문서가 정의하는 것** — connection·message operation·object 생성과 relocation
...
실제 실행하는 node가 바뀌어도 같은 ID로 계속 접근할 수 있는 논리 대상인
[Spot](../00-foundation/02-glossary.ko.md#spot)이나 Actor를 현재 처리하는 node를
[owner](../00-foundation/02-glossary.ko.md#owner)라고 한다. Owner를 사용하는 operation은 target node의
물리 connection만 보지 않고, 각 object의 현재 owner와 generation을 여러 node가 함께 확인할 수
있도록 보관하는 저장소인 [Location Store](../00-foundation/02-glossary.ko.md#location-store)의 object
...
3. Object operation이면 Location Store에서 확인한 owner와 generation이 아직 유효한지
   확인한다.
4. Stateful relocation이면 relay-ready reply가 accepted 상태가 되기 전인지, 그 뒤 owner 변경
   commit 전인지, commit 뒤인지 확인한다. 이 accepted 경계의 정의는
   [Actor와 Spot relocation 전체 흐름 「4.4」](04-relocation-flow.ko.md#44-ordered-relay와-one-way-cutover)가
   소유한다.
...
[select-one](../00-foundation/02-glossary.ko.md#select-one)이라고 한다. 여러 MeshNode가 참여해 node와
Channel message를 주고받는 범위인 [RouteMesh](../00-foundation/02-glossary.ko.md#routemesh) Channel은 새
operation을 받을 수 있는 [Ready](../00-foundation/02-glossary.ko.md#ready) 상태이고 weight가 0보다 큰
server를 후보로 사용하고, ClientServer Channel은 `Ready`인 server를 후보로 사용한다.
...
Caller가 MeshName과 target RID를 직접 지정해 특정 node에 message를 보내는
[Node direct](../00-foundation/02-glossary.ko.md#node-direct)는 이 재선택 규칙을 사용하지 않는다. 지정한
node가 없거나 connection이 준비되지 않으면 `NotFound` 또는 `Unavailable`로 끝난다. 자세한
선택과 완료 결과는 [상호작용 모델 §3](../00-foundation/04-interaction-model.ko.md#3-node-direct와-channel-select-one)과
[Framework API §13](../00-foundation/06-framework-api.ko.md#19-오류-kind)이 정의한다.
...
[Transport liveness §6](../02-channel-transport/05-transport-liveness.ko.md)이
정의한다.
...
ID만 target으로 사용한다.** generation이 아니라 logical ID의 current Ready object를
대상으로 하므로, object가 같은 ID로 다시 만들어진 것과 owner를 잃은 것이 서로 다른 결과로
끝난다. `ObjectGeneration`은 일반 message의 **Target 일치 조건에서 제외한다.** 전자는 새
incarnation이 처리하고 후자는 `Unavailable`이다. 아래 절들은 후자만 다룬다.

### 4.2 기존 Actor와 Spot

Actor와 Spot message는 Location Store에서 확인한 현재 `Ready` owner를 사용한다. Cache가
...
[Spot과 Actor membership §2](../03-spot-actor/05-spot-actor-membership.ko.md#2-object를-하나만-생성하도록-확정하는-과정)가
정의한다.
...
특징은 object를 **언제 만드는지** 정할 뿐, `Ready` owner 장애 뒤 다른 node에서 object를
자동 복원하는 failover 정책을 추가하지 않는다. Caller가 target Spot이 없을 때 새 Instance Spot을
만들어도 된다고 명시적으로 선택하는 것을
[Instance intent](../00-foundation/02-glossary.ko.md#instance-intent)라고 한다. 다음 표는 message가
도착했을 때 현재 authority에 따라 Framework가 어떤 동작을 하는지 구분한다.
...
[Location runtime](01-location-runtime.ko.md)의 최초 message 저장·재개 절이 정의한다.

## 5. Host relocation 장애

`Relocate`는 실행 중인 source와 선택한 target이 상태와 아직 실행하지 않은 작업을 넘기는
계획된 동작이다. 장애가 발생한 host를 대신할 owner를 찾는 operation이 아니다. 현재 version은
source runtime, 선택한 target runtime, Location Store와 Relocation Store가 operation이 끝날
때까지 실행되는 graceful handoff만 지원한다.
...
[Spot과 Actor membership §7](../03-spot-actor/05-spot-actor-membership.ko.md#8-실패-처리-범위)이 정의한다.

## 6. Session과 binding

Actor가 계획된 relocation으로 이동하면 Session의 physical STREAM connection은 유지된다.
Target runtime이 Session owner에 위치 갱신 message를 보내 해당 Actor의 binding route와
current `ActorRef` 위치 snapshot을 바꾼다. 같은 ObjectGeneration이 유지된 relocation에만 이 갱신을
적용하며 Application은 relocation을 알기 위해 다시 bind하지 않는다.
...
relay는 current binding token이 필요하므로 Application이 새 `ActorRef`를 bind해야 한다.
이전 Session에서 늦게 도착한 relay·unbind·disconnect는 새 binding에 적용하지 않는다.

Session owner process가 종료되면 Framework는 physical connection, Session identity와
binding을 다른 process로 이전하지 않는다. Client reconnect는 새 Session을 만들며,
Application은 새 Session에서 인증과 bind를 다시 수행해야 한다. 이전 connection의 reply와
binding update는 새 Session에 적용하지 않는다. 자세한 종료 경계는
[Session과 Actor binding 「9. 재접속과 이동의 구분」](../04-session/02-session-actor-binding.ko.md#9-재접속과-이동의-구분)이
정의한다.

## 7. Store 장애

Framework가 Location Store 변경 결과를 받지 못하면 성공 또는 실패를 추측하지 않는다. 같은
key와 처음 사용한 `StoreVersion`으로 record를 다시 읽고, 적용된 변경인지 확인한 뒤 필요한
...
Store 요청의 재확인과 payload 순서는
[Location runtime](01-location-runtime.ko.md)의 Store 응답 유실 절이 정의한다.

## 8. Application의 재시도 판단

Framework는 실패한 operation의 `ErrorKind`를 반환하지만 재시도 여부는 제공하지 않는다.
Timeout이나 connection loss가 발생하면 remote handler의 실행 여부를 알 수 없을 수 있기
때문이다. Application은 operation의 idempotency, 업무 단위 idempotency key, 결과 조회 또는
상태 비교로 중복 영향을 막은 뒤 새 operation을 시작할지 결정한다.

Framework가 수락 전 target을 다시 선택하거나 Store 결과를 재확인하는 동작은 같은 operation의
내부 처리다. Application이 실패 결과를 받은 뒤 시작하는 새 operation과 구분한다. 자세한
오류와 완료 조건은 [Framework 오류 모델](../00-foundation/07-framework-error-model.ko.md)이 정의한다.

## 9. 관련 내부 구조

**이 문서가 공개 장애 동작의 소유 문서다.** 그 동작을 만드는 구현 구조는 다음 문서가 이어서
설명하며, 어느 문서도 이 장의 오류 의미나 failover 범위를 다시 정의하지 않는다.

- [Spot·Actor routing](../03-spot-actor/08-routing.ko.md)은 `Missing`과 `Unavailable`을
  resolver 결과 타입에서 보존하는 방법을 설명한다.
- [객체 종류와 활성화](../03-spot-actor/09-object-lifecycle.ko.md)는 resolver 결과를 activation
  state machine의 서로 다른 입력으로 전달하는 방법을 설명한다.
- [Transport liveness](../02-channel-transport/05-transport-liveness.ko.md)는 availability
  evidence와 authority release의 소유자를 분리한다.
- [Service wire protocol](../02-channel-transport/06-wire-protocol.ko.md#8-instance-spot-cold-activation-recovery)은
  같은 target의 최초 activation recovery에만 사용하는 durable root와 scan을 설명한다.

## 10. 검증 요구

공개 표면(target 선택 결과, 반환하는 `ErrorKind`·outcome, Location Store record 조회, Session
binding 상태)만으로 다음을 확인한다. 각 항목은 test 하나로 이어진다.
...
**Object routing과 생성 recovery**

- 생성 recovery는 같은 object ID와 generation만 계속하며 하나의 target만 factory를 실행한다.
- Actor·Spot direct message는 logical ID의 current Ready object를 대상으로 하며
...
- `Ready` authority에 activation recovery pointer가 남아 있더라도 최초 cold activation의
...
- Instance Spot cold activation recovery를 Actor, User Spot, 이미 `Ready`인 Instance Spot이나
  host relocation에 사용하지 않는다.
...
- Store 결과가 불분명하면 authority를 다시 읽기 전까지 source admission과 target dispatch를
  열지 않는다.
