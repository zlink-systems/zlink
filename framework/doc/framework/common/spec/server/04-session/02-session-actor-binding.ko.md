...
packet handler turn이 시작할 때 한 번 하며, turn 동안 그 결과를 유지한다. 연결 수립 callback은 그 연결의 첫
packet callback보다 먼저, 연결 종료 callback은 이미 수락한 packet callback 뒤에 완료한다.
**Handler turn 시작 시 현재 binding이 없는 slot을 실은 packet은 session handler에 전달하지
