...
| Actor가 다른 node로 이동한 뒤의 route와 위치 | Relocation commit 뒤 Framework가 session에 보관한 해당 Actor route와 bound-session current Actor location snapshot을 target으로 갱신한다. `ActorId`·`ObjectGeneration`은 유지한다. |
...
전 abort를 전달하는 one-way control이다. `sessionRelocationRoute` commit에는
...
않으므로 **언어별 재량**이다. Response는 보내지 않는다.

Abort에는 matching seal identity와 abort action만 넣는다. Session owner는 matching
seal만 해제하고 보관한 message를 source route로 제출하며 reply를 만들지 않는다.
