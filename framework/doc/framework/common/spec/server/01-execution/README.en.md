...
order, and completes with whichever of reply, timeout, cancellation, or shutdown is
decided first. This topic covers that entire path — from submit to completion.

- [Submit And Completion](01-submit-and-completion.en.md) covers when each terminator
  completes and what operation identity is.
...
The execution path has capacity limits at two places with different purposes. This is a
point that is easy to mistake for one, so it is distinguished with a diagram.
...
- **Core byte HWM** is the byte-level last-resort safeguard of the transport path. Even
...
The precise order at which the two authorities meet (the ordinary ingress permit order) is
owned by
