...
반납은 `SpotWide` User Spot과 Instance Spot에서만 사용할 수 있다. 그 밖의 자리에서
...
- Byte 합계는 보조로만 사용한다 — 처리 시간이 payload 크기에 비례할 때만 의미가 있다.
...
| 방식 | 상태 캐시 | 자원 활용 |
|---|---|---|
| 넘겨가며 실행 | 작업마다 잃을 수 있다 | 자원 수만큼 고르게 사용한다 |
