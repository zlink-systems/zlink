...
> 이 문서는 handler가 언제 실행되고 언제 다른 handler에게 자리를 내주는지, `Yield`와
> `Defer()`가 각각 무엇을 반납하고 무엇을 유지하는지, application 실행이 infrastructure
> 진행을 막지 않는 이유를 정의한다. 계약은 [Actor 모델](../03-spot-actor/04-actor-model.ko.md)과
> [Spot 위의 Stage wrapper](../03-spot-actor/07-stage-wrapper-on-spot.ko.md)가 소유하며, 이 문서는 그
> 계약을 만족시키는 실행 구조를 모든 언어 runtime이 따라야 하는 규칙으로 함께 담는다.

## 1. Queue와 gate 분리 원칙

Runtime의 작업은 두 축으로 나뉜다. 하나는 "어디에 줄 서는가"(queue)와 "누가 지금 실행할
권한을 쥐는가"(gate·execution 권한)의 분리이고, 다른 하나는 handler가 실행하는
...
Queue와 gate의 분리는 [Actor 모델](../03-spot-actor/04-actor-model.ko.md#3-actor-queue)이 요구하는
서로 다른 두 가지에서 나온다.

- Actor 앞으로 온 payload는 실행 모드와 무관하게 항상 그 Actor의 queue에 제출한다.
- `SpotWide`에서는 여러 Actor를 담는 실행 단위인 그 [Spot](../00-foundation/02-glossary.ko.md#spot)의
  Actor handler·Spot handler·timer·lifecycle callback이 전체에서 한 번에 하나만 실행된다.

**줄 서는 곳(queue)은 Actor마다 두고, 실행 권한(gate)은 Spot이 공유한다.**
...
`PerActor`는 Actor별·Spot별 분리로 끝나지 않고 timer마다도 따로 gate를 갖는다
([Spot 위의 Stage wrapper 「9. 구현 및 contract test 검증 요구」](../03-spot-actor/07-stage-wrapper-on-spot.ko.md#9-구현-및-contract-test-검증-요구)).
Timer 두 개를 같은 Spot 줄에 함께 넣으면 서로 다른 timer가 서로를 기다리게 된다.

## 2. Execution gate — owner 처리 순서

- Node handler, message를 보낼 Channel 범위를 식별하는 이름인
  [ChannelName](../00-foundation/02-glossary.ko.md#channelname) handler, 각 Spot과 각 Actor는
  자신에게 적용되는 execution gate의 순서에 따라 application record를 처리한다.
- 일반 비동기 terminal(`Async` 등)로 기다리는 handler는 완료 continuation이 끝날 때까지
  같은 gate의 다음 application record를 실행하지 않는다.
- `SpotWide` User Spot과 Instance Spot에서 `Yield`로 기다리면 shared Spot turn을
  반납하므로 같은 Spot의 다음 record를 실행할 수 있고, 완료 continuation은 같은 Spot
  queue에 들어가 새 turn으로 재개한다.
- Entry Spot Actor와 `PerActor` User Spot의 Actor는 Actor별 gate를 사용하며 `Yield`를
  제공하지 않는다.
- 어느 경우에도 같은 execution gate의 application turn 두 개를 동시에 실행하지 않는다.

## 3. `Yield` 시 gate와 claim

`Yield`는 `SpotWide` User Spot과 Instance Spot에서 실행하는 Channel·Spot·Actor
request, CPU·I/O worker와 Actor·Spot create·get-or-create call에만 제공한다.
Create·get-or-create의 `Yield`는 naming 적용 범위를 넓히는 규칙이 아니라 별도 object
execution 특례다.
...
- `SpotWide` User Spot과 Instance Spot에서 사용한 `Yield`는 shared Spot turn을 반환한
  뒤 completion이 확정되면 원래 Spot queue에 실행 재개 record 하나를 넣는다. Reply
  payload를 새 Spot packet으로 dispatch하지 않는다.
...
([Spot 위의 Stage wrapper 「5. Timer」](../03-spot-actor/07-stage-wrapper-on-spot.ko.md#5-timer와-yield)).
...
결과는 제출 계열, 대기열이 어느 runtime에 있는가, 호출의 public 결과가 이미 확정됐는가로
갈린다([Spot 메시징 「5.3 Spot application queue에 들어가는 작업」](../03-spot-actor/02-spot-messaging.ko.md#53-spot-application-queue에-들어가는-작업)이
이 판단 기준을 소유한다).

| 계열 | 대기열 위치 | 결과 |
|---|---|---|
| Send·one-way | 같은 runtime | send timeout까지 기다린다. 내부 대기 자리까지 차면, operation의 완료 조건을 deadline까지 만족하지 못했을 때 발생하는 [`DeadlineExceeded`](../00-foundation/02-glossary.ko.md#deadlineexceeded) |
...
기다리는 것과 즉시 끝내는 것을 가르는 기준은 호출자가 결과를 받아 판단할 수 있는가다.
Request는 받을 수 있으므로 기다리지 않고, send 계열은 받을 수 없으므로 기다린다. 그
자리에서 실행하는 것은 어느 경우에도 선택지가 아니다.
...
8 turn이다. 시간 예산은 handler 하나가 끝난 경계에서 확인한다 — 이 시간 예산과
[§9](#9-시간-예산과-batch-처리-구현)의 owner 점유 예산이 같은 메커니즘을 가리키는지는 문면상
명시돼 있지 않으며 spec-gap 후보로 남아 있다(ledger 참고).
...
이 시간 예산이 [§7](#7-lane-분리와-우선순위-구현)의 owner 점유 상한(기본 10 ms)과 같은
메커니즘인지, `PerActor`처럼 각 실행 단위가 이미 분리된 gate를 가진 경우에도 같은 값이
적용되는지는 spec-gap 후보로 남아 있다.
...
두 도메인은 owner마다 물리적으로 다른 FIFO로 분리된다 — 정확한 한도와 우선순위 규칙은
[§7](#7-lane-분리와-우선순위-구현)이 소유한다.
...
- `SpotWide` 문맥에서 `Yield`를 허용하는 call과 미제공 대상 call이 [§3](#3-yield-시-gate와-claim)의
  표대로 구분된다.
...
- 대기열이 가득 찬 상태에서 제출한 작업이 그 자리에서 실행되지 않고 [§6](#6-처리-권한-획득의-함정-구현)의
  표대로 완료한다.
