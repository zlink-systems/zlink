...
Owner마다 하나씩 두며, 그 컴포넌트의 상태를 읽고 쓰는 모든 코드는 이 lane 위에서만
실행된다.
...
state lane이 막는 형태는 하나다 — **lock 같은 배타적 접근 안에서 상태의 스냅샷을 꺼내고,
그 접근을 해제한 뒤, async 경계 너머에서 그 스냅샷으로 결정을 내리는 형태.**
...
컴포넌트의 mutable 상태는 다음 세 가지 중 하나로 분류한다. 분류가 정해지면 그 상태를
...
**재진입은 데드락이 아니라 예외로 검출되어야 한다.** 같은 lane의 turn 안에서 그 lane에
다시 진입하는 호출은 처리할 방법이 없다 — 앞선 turn이 끝나기를 기다리는 것 자체가 그
turn 안에서 일어나므로 자기 자신을 기다리는 것과 같다. 이걸 그대로 두면 대기가 영원히
끝나지 않는 hang이 된다. Lane은 이 상황을 hang이 아니라 그 호출 지점에서 즉시 진단
가능한 예외로 검출해야 한다 — 어느 lane에서 재진입했는지 알 수 있는 오류를 그 자리에서
내야, 재진입이 실행 중 어딘가에서 조용히 멈추는 대신 원인을 즉시 지목한다.

## 6. 재진입 제거 규칙

C2로 분류한 컴포넌트를 lane으로 전환할 때 재진입을 먼저 걷어낸다. 실제 전환에서 나타나는
재진입은 두 유형이다.
...
- **전환 단위는 클래스 하나다.** 한 컴포넌트(그 클래스가 가진 배타적 접근 하나)를
  한 번에 옮긴다.
- **전환마다 검증을 통과해야 다음으로 간다.** 그 언어의 단위 테스트 전체가 그린이어야
  하고, 샘플 게이트가 유지되어야 한다.
...
state lane의 정본 구현이다. 상태 접근은 이 lane에 제출한 작업으로 실행하며, lane이
