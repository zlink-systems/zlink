...
대기열에 남은 작업이 이미 확정되어 있으므로 그때 만들 수 있다.
...
## 8. Retained Core lease와 1:N child 소유권

Core receive에서 retain한 record는 payload와 Core receive-credit lease의 shared owner
하나를 가진다.
...
Shared retained owner는 모든 child terminal과 필요한 record-level reply attempt가
terminal이 된 뒤 Core lease를 정확히 한 번 반환한다.
...
callback)만으로 다음을 확인한다. 각 항목은 test 하나로 이어진다. 이 중 "buffer 전체
복사·view·객체 생성 횟수"를 다루는 항목은 공개 표면이 아니라 내부 할당·복사 계측으로
측정하는 **내부 확인 조건**이며, 아래에 그 항목만 따로 표시한다.
...
- 첫 typed 접근의 값이나 실패를 message에 저장하며, 같은 message에서는 codec을 최대 한
  번만 호출한다.
...
**내부 확인 조건(측정 기반, 공개 표면으로는 확인할 수 없음)**
...
- 수신 codec table이 시작 시점에 확정되어 있고, codec 정보를 읽는 데 잠금이 필요하지
  않다.
