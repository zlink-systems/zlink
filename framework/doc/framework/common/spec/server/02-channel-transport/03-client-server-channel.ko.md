...
[Channel·Transport 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 02. Channel 메시징](02-channel-messaging.ko.md) · [다음: 04. Network listener identity](04-network-listener-identity.ko.md)

> [ClientServer Channel](../00-foundation/02-glossary.ko.md#clientserver-channel)은 Client가 send나 request를 시작하고 Server가 handler를 실행하여
> request에 reply하는 단방향 service 경계다. 이 문서는 role 등록, endpoint
> 발견, weight 기반 target 선택, drain과 재시작 계약을 정의한다.

## 1. ClientServer Channel 개요

| Role | 시작할 수 있는 업무 호출 | 받은 message 처리 |
|---|---|---|
| `Client` | [Ready](../00-foundation/02-glossary.ko.md#ready)(초기화와 discovery 기록이 끝나 application message를 받을 수 있는 상태)인 server 하나를 선택하여 send 또는 request를 시작한다. | Client request 없이 Server가 먼저 보낸 message는 받지 않는다. Client가 시작한 request와 일치하는 reply만 그 request의 결과로 받는다. |
| `Server` | 연결된 Client를 대상으로 새로운 업무 send나 request를 시작할 수 없다. | Client가 보낸 send와 request의 handler를 실행한다. Request handler는 받은 reply token으로 응답한다. |
...
[ClientServer Channel](../00-foundation/02-glossary.ko.md#clientserver-channel)은
[RouteMesh](../00-foundation/02-glossary.ko.md#routemesh)의 option이 아니다. 다음 기능을 제공하지
않는다.

- 여러 [MeshNode](../00-foundation/02-glossary.ko.md#meshnode)(RouteMesh에 참여해 message를 주고받는 runtime node) 사이의 [Node direct](../00-foundation/02-glossary.ko.md#node-direct)(caller가 MeshName과 target RID를 함께 지정해 특정 MeshNode로 message를 보내는 방식)
- Spot과 Actor 메시징
- [Logical Multicast](../00-foundation/02-glossary.ko.md#logical-multicast)(ChannelName과 topic으로 같은 Channel의 여러 Spot에 message 하나를 전달하는 방식)
- 다른 RouteMesh를 대신 연결하거나 message를 중계하는 기능

이 제한은 ClientServer transport가 위 기능을 자동으로 대신하지 않는다는 뜻이다.
같은 process의 application은 message를 보낼 Channel 범위를 식별하는 이름인
[ChannelName](../00-foundation/02-glossary.ko.md#channelname)을 서로 다르게 지정하여 ClientServer와 RouteMesh를
각각 등록하고 두 송신 경로에서 별도 호출을 시작할 수 있다. ClientServer handler도
application 판단에 따라 등록된 RouteMesh, 다른 ClientServer Channel, [Spot](../00-foundation/02-glossary.ko.md#spot) 또는
...
[Channel 메시징](02-channel-messaging.ko.md)이 정의한다.

이 문서의 C# 코드는 공통 계약이 .NET public API에서 어떻게 나타나는지 보여주는
참고 자료다. 다른 언어에 같은 signature를 요구하지 않는다. 정확한 .NET signature는
[.NET topology 공개 인터페이스](../languages/dotnet/interfaces/03-configuration-topology.ko.md)와
[.NET Channel 메시징 공개 인터페이스](../languages/dotnet/interfaces/04-channel-messaging.ko.md)가
정의한다.

## 2. Client와 Server role 등록
...
Client의 endpoint와 Server의 listener·handler 설정은 [§3](#3-server-endpoint를-찾고-ready로-만드는-방법)과
[§4](#4-weight와-target-선택)에서 설명한다.
...
## 3. Server endpoint를 찾고 ready로 만드는 방법

Client는 다음 source 중 하나 이상에서 Server endpoint를 얻는다.

| 발견 방식 | Endpoint source | [Location Store](../00-foundation/02-glossary.ko.md#location-store)(각 Spot·descriptor의 현재 상태를 여러 node가 함께 확인하도록 보관하는 저장소) |
|---|---|---|
| Manual | Application이 `Connect(endpoint)`로 등록한다. | Manual endpoint만 사용하면 필요하지 않다. |
| Automatic | 같은 [ChannelName](../00-foundation/02-glossary.ko.md#channelname)의 ClientServer Server descriptor를 조회한다. | Location Store가 필요하다. |

Manual과 automatic source가 같은 Server RID와
[lifecycle generation](../00-foundation/02-glossary.ko.md#lifecycle-generation)을 가리키면 Framework는
연결 후보 하나로 합친다.

Endpoint 발견부터 target 선택까지는 Client, discovery source(manual 설정 또는
automatic descriptor)와 Server 세 주체가 순서대로 관여한다.

```mermaid
sequenceDiagram
    participant Client as Client role
    participant Discovery as Discovery source<br/>(manual 설정 또는 automatic descriptor)
    participant Server as Server role

    Client->>Discovery: endpoint 조회(manual Connect 또는 automatic 조회)
    Discovery-->>Client: Server RID·lifecycle generation과 endpoint
    Client->>Server: transport 연결 시작(Client만 connection 시작)
    Server-->>Client: identity·generation 확인 후 ready
    Note over Client: ready + weight>0 + non-draining Server만 후보
    Client->>Server: weight 비율로 선택한 Server에 send 또는 request
```

이 흐름에서 discovery는 endpoint 후보를 만들 뿐이며, 실제 transport 연결에서
identity와 generation을 다시 확인해야 [ready target](../00-foundation/02-glossary.ko.md#ready-target)(listener와
transport 연결, identity 검사와 필요한 handler 등록이 끝나 새 message를 받을 수
있는 target)이 된다([§3.2](#32-server-descriptor를-찾은-것만으로-ready가-되지는-않는다)).
Ready target 가운데 실제로 호출을 받는 Server 하나를 고르는 weight 규칙은
[§4](#4-weight와-target-선택)가 정의한다.

### 3.1 Client만 Server로 connection을 시작한다
...
[Automatic discovery](../00-foundation/02-glossary.ko.md#automatic-discovery)에서는 같은 ChannelName의 유효한 Server [descriptor](../00-foundation/02-glossary.ko.md#descriptor)마다 Server RID와
lifecycle generation으로 구분한 connection intent를 하나 만든다. 여러 Server를 발견하면
각 Server의 [ready](../00-foundation/02-glossary.ko.md#ready) connection을 독립적으로 유지하고, 업무 호출을 시작할 때 그중 하나를
[§4의 weight 규칙](#4-weight와-target-선택)으로 선택한다.

따라서 ClientServer Channel은 연결 방향이 정해진 비대칭 topology다. RouteMesh처럼 두
[MeshNode](../00-foundation/02-glossary.ko.md#meshnode)의 RID를 비교하여 어느 쪽이 connection을 시작할지 정하지 않는다.
...
### 3.2 Server descriptor를 찾은 것만으로 ready가 되지는 않는다

Automatic discovery Server는 다음 정보를 담은 ClientServer 전용
[ClientServer Server descriptor](../00-foundation/02-glossary.ko.md#clientserver-server-descriptor)와
owner lease를 게시한다. [Owner](../00-foundation/02-glossary.ko.md#owner) lease는 이 Server가 ClientServer Server descriptor를
...
- [Descriptor revision](../00-foundation/02-glossary.ko.md#descriptor-revision)(같은 lifecycle 안에서 weight나
  drain state처럼 바뀔 수 있는 descriptor 정보의 version을 나타내는 증가하는 번호)

Client는 같은 ChannelName의 유효한 ClientServer Server descriptor만 사용한다.
이 등록 정보에서 endpoint를 찾은 뒤 실제 transport 연결에서 [Server identity](../00-foundation/02-glossary.ko.md#server-identity)와
...
### 3.3 Location Store가 필요한 시점

[Manual endpoint](../00-foundation/02-glossary.ko.md#manual-endpoint)만 사용하면 Location Store가 없어도 된다. Automatic discovery를
...
- Weight와 [drain state](../00-foundation/02-glossary.ko.md#drain과-draining)
- Security identity

이 정보는 ClientServer 연결만 제어하며 [MeshNode descriptor](../00-foundation/02-glossary.ko.md#meshnode-descriptor)나 RouteMesh peer
정보로 변환하지 않는다.
...
- **Local Server가 선택되어도 handler를 직접 호출하지 않는다.** Client `DEALER`에서
  Server `ROUTER`로 실제 transport message를 전달하므로 codec, HWM, timeout,
...
### 4.2 실행 중 target 선택 정보가 바뀌는 경우

Server가 실행 중에 weight를 바꾸거나 drain을 시작하면 Client가 사용하는 target
선택 정보가 달라진다. Server는 이 변경을 ClientServer Server descriptor에 기록하고
[descriptor revision](../00-foundation/02-glossary.ko.md#descriptor-revision)을 증가시킨다. Client는 같은 lifecycle generation에서 더 큰
...
[reply correlation](../00-foundation/02-glossary.ko.md#reply-correlation)을 만든다. Reply, error,
...
Server request handler가 받는 [reply token](../00-foundation/02-glossary.ko.md#reply-token)은 현재
...
### 5.2 Handler가 다른 target을 호출하는 경우

ClientServer handler는 다른 RouteMesh, ClientServer Channel, Spot 또는 Actor에
request를 보낼 수 있다. 이
[downstream request](../00-foundation/02-glossary.ko.md#downstream-request)는 원래 ClientServer
...
3. 이미 수락한 handler와 request reply를 [deadline](../00-foundation/02-glossary.ko.md#deadline)까지 진행한다.
4. 모든 최종 결과가 확정되면 ClientServer Server descriptor와
   [owner lease](../00-foundation/02-glossary.ko.md#owner-lease)를 해제하고 listener를 닫는다.

Manual Client에는 같은 drain state를 연결 control message로 알린다.

- **Client가 drain state를 확인하기 직전에 request를 제출할 수 있다.** Server가 이
  request를 거부하면 무한히 기다리지 않고 유한한 rejected 결과로 완료한다. 제출
  시점과 확인 시점 사이의 경합에서도 caller가 결과를 무한정 기다리지 않게 하기
  위해서다.
...
- **이 값은 숫자 크기로 순서를 판단하지 않는다.** Endpoint가 같아도 이전 generation의
  연결과 ClientServer Server descriptor를 새 target으로 사용하지 않는다. Generation은
  값이 같은지만 비교하는 fence이기 때문이다.

Client는 다음 순서로 교체한다.

1. 새 generation의 ClientServer Server descriptor를 찾는다.
2. Transport 연결에서 identity와 generation을 다시 확인한다.
3. 새 generation을 [ready target](../00-foundation/02-glossary.ko.md#ready-target)으로 만든다.
4. 이전 generation의 연결을 제거한다.

Client는 reply에 포함된 reply correlation을 현재 대기 중인 request의 값과
비교한다.

- **값이 일치할 때만 그 request의 결과로 처리한다.** 따라서 이전 generation에서 보낸
  reply라도 원래 request가 아직 대기 중이면 그 request의 결과가 될 수 있다 — reply
  correlation만으로 판단하고 어느 generation에서 왔는지는 판단에 쓰지 않기 때문이다.
...
- **이미 ready인 연결과 이미 수락한 request는 Store 장애만으로 취소하지 않는다.**
  Discovery 갱신이 멈춘 것과 이미 확인한 transport 연결의 유효성은 별개이기
  때문이다.

Server가 owner lease를 갱신하지 못한 상태에서 허용된 시간이 지나면 새로운 업무
message를 받지 않는다. 이 마지막 시점을
[`fencing deadline`](../00-foundation/02-glossary.ko.md#fencing-deadline)이라 한다. Store가 복구되면
최신 descriptor revision과 lifecycle generation을 기준으로 target 목록을 다시
맞춘다.

## 9. 검증 요구

Role 등록 builder, Client·Server가 관찰하는 send·request 결과와 reply, ClientServer
Server descriptor 조회와 owner lease 상태, monitoring이 제공하는 connection snapshot만으로
다음을 확인한다.

**Role과 등록**

- Server에는 Client를 대상으로 새 업무 호출을 시작하는 public API가 없다.
- Client request와 일치하지 않는 Server message를 application handler에 전달하지 않는다.
- 같은 process의 동일한 ClientServer `ChannelName`에 `Client`와 `Server` 역할을 각각
  한 번 등록할 수 있다.
- 동일한 ClientServer `ChannelName`과 동일한 역할의 중복 등록, 그리고 RouteMesh와의
  `ChannelName` 충돌은 startup configuration error다.

**Endpoint 발견과 target 선택**
...
- ClientServer Channel의 connection 시작 방향을 정할 때 RouteMesh의 RID 비교 규칙을
  사용하지 않는다.
- Automatic discovery는 ClientServer Server descriptor를 사용하며, MeshNode descriptor와
  서로 바꾸어 사용하지 않는다.
