...
>
> **계약 소유** — `framework/runtime/protocol/service-wire-v1.schema.json`이 command ID, frame
> 배치, field 관계와 검증 순서를 정하는 유일한 기준이다. 다른 구현이 상호운용을 위해 의존하는
> byte 배치(frame 순서, field 순서, 크기 상한, enum 값)는 이 장에서 **계약 서술**이다 — schema와
> 다르면 schema에 맞춰 이 장을 고친다. 그 byte를 만드는 절차(어느 함수가 buffer를 재사용하고
> codec을 언제 생성하는가)는 **구현 서술**이다 — schema가 바뀌면 절차도 따라간다. 이 장은
> 문장마다 결정·재량 표시를 따로 붙이지 않는다. 모든 command와 field 규칙이 schema가 소유한
> 계약이기 때문이다.
>
> **함께 보는 계약** — [계층 경계와 식별자](../00-foundation/08-layering.ko.md) ·
> [Location runtime](../05-location-relocation/01-location-runtime.ko.md) ·
> [Redis Relocation Store](../05-location-relocation/03-relocation-store-redis.ko.md) ·
> [Transport liveness](05-transport-liveness.ko.md) ·
> [Relocation handoff 상태 전이](../05-location-relocation/04-relocation-flow.ko.md)

| 절 | 다루는 내용 |
|---|---|
| [1. Schema와 생성 경계](#1-schema와-생성-경계) | 규범 generated-codec 기준, 계층별 형식 소유, schema 관례, validator, Location Store authority key 형식 |
| [2. Record framing과 decode](#2-record-framing과-decode) | multipart frame 구성, decode 검증, payload 크기 상한 |
| [3. Command space](#3-command-space) | 53개 command 목록과 역할, Message Follow와 session 교체 notification |
...
| [12. 검증 요구](#12-검증-요구) | schema self-test·generated codec·golden fixture로 확인하는 관찰 결과 |

> §7~§11은 durable authority, cold activation recovery, relocation manifest·CAS, membership과
> Ready, terminal identity를 다룬다. 내용이 05-location-relocation·03-spot-actor 주제와
> 직접 겹치므로, 재배치 여부는 그 주제를 재작성할 때 다시 판단한다. 이 장에서는 문장 정리만
> 하고 절을 옮기지 않는다.
...
`framework/runtime/protocol/service-wire-v1.schema.json`은 Framework service wire의 유일한 규범
기준이다. 이 schema가 command ID, frame·logical stream layout, enum 값, field bound, durable format과
...
| 계층 | 규범 형식 | 소유자와 해석 |
|---|---|---|
| Location Store record | canonical JSON envelope | [Location runtime §2.4](../05-location-relocation/01-location-runtime.ko.md)가 JSON record의 byte 배치를 정확히 정의하며 provider는 opaque bytes로 취급한다. |
| ClientServer application record | JSON `0xF2` channel envelope | Client가 send/request를 시작하고 Server가 handler 실행과 reply를 담당하는 단방향 service 경계인 [ClientServer Channel](../00-foundation/02-glossary.ko.md#clientserver-channel)([상세](03-client-server-channel.ko.md))의 application-record 계약이 이 envelope와 JSON 의미를 소유한다. |
...
[Location Store](../00-foundation/02-glossary.ko.md#location-store)라고 한다. Actor와, 주소와 상태를 가지고
실행 node가 바뀌어도 같은 global ID로 계속 접근할 수 있는 논리 instance인
[Spot](../00-foundation/02-glossary.ko.md#spot)의 authority key를 만드는 규칙도 같은
schema와 golden fixture가 고정한다.

| 객체 | key 형식 |
|---|---|
| Actor | `zla1:a:<byte-length>:<encoded-ActorId>` |
| [Spot](../00-foundation/02-glossary.ko.md#spot) | `zla1:s:<byte-length>:<encoded-SpotRid>` |
...
복사하지 않는다. Service record는 다음 순서의 multipart로 구성한다 — 이 순서와 각 frame의 byte 배치는 다른
구현이 상호운용을 위해 그대로 의존하는 계약 서술이고, 그 bytes를 만드는 절차는 schema에서 생성한 codec이
소유하는 구현 서술이다.

```text
frame 0                     frame 1              그다음 frame
+------------------------+  +----------------+  +------------------------+
| head prefix (5 byte)   |  | metadata       |  | typed payload envelope |
| + command body         |  | flag 0x01일 때 |  | command이 허용할 때    |
+------------------------+  +----------------+  +------------------------+
   항상 있다                  flag가 정한다        command이 정한다
```

Frame 0 맨 앞 5 byte는 모든 record가 같다.
...
byte    0      1      2         3          4
     ...
     |  'Z' |  'M' |  major  |    ID    | 비트 합 |
     +------+------+---------+----------+---------+
      \___ magic ___/       각 1 byte
```

이어지는 command body의 배치는 schema의 `commands[].body` 선언 순서를 따른다. 2 byte 이상인
정수는 모두 big-endian이다.

Flags는 뒤에 무엇이 더 붙는지를 정한다.

| 비트 | 이름 | 세우면 | 비우면 |
|---|---|---|---|
| `0x01` | `metadata` | frame 1에 metadata frame이 **반드시 있다** | metadata frame이 있으면 거부한다 |
| `0x02` | `boundSession` | command body 뒤에 bound session tail이 붙는다 | 붙지 않는다 |
...
- 여러 runtime node가 이름으로 서로를 찾아 message를 주고받는 연결 그룹인
  [RouteMesh](../00-foundation/02-glossary.ko.md#routemesh)의 ServerServer에서는 schema·wire 표현
  절대 상한만 적용하고 Framework-level message-size 상한은 두지 않는다.
...
Payload의 byte 배치는 다음과 같다.

```text
+-------------+  +--------------+------------------+  +--------------+-----+
| part count  |  | part 0 길이  | part 0 bytes     |  | part 1 길이  | ... |
| u32 (>= 1)  |  | u32          | 길이만큼 opaque  |  | u32          |     |
+-------------+  +--------------+------------------+  +--------------+-----+
                 \____________ part count번 반복 ____________________/
```
...
| 3 | `reject` | infrastructure | 없음 | — | admission 거부 |
| 4 | `update` | infrastructure | 없음 | — | admitted descriptor revision 갱신 |
| 5 | `livenessProbe` | infrastructure | 없음 | — | 현재 connection의 round-trip 확인 |
| 6 | `livenessAck` | infrastructure | 없음 | — | 같은 probe ID 응답 |
| 16 | `nodeSend` | application | 필수 | `metadata` | node one-way |
| 17 | `nodeRequest` | application | 필수 | `metadata` | node request |
| 18 | `channelSend` | application | 필수 | `metadata` | channel one-way (RouteMesh connection 전용) |
| 19 | `channelRequest` | application | 필수 | `metadata` | channel request (RouteMesh connection 전용) |
| 20 | `reply` | infrastructure | 선택 | — | request terminal result (RouteMesh connection 전용) |
| 21 | `spotSend` | application | 필수 | `metadata` | Spot one-way |
| 22 | `spotRequest` | application | 필수 | `metadata` | Spot request |
| 23 | `logicalMulticast` | application | 필수 | `metadata` | logical multicast |
| 24 | `actorSend` | application | 필수 | `metadata`·`boundSession`·`sourceSpotId` | Actor one-way |
| 25 | `actorRequest` | application | 필수 | `metadata`·`boundSession`·`sourceSpotId` | Actor request |
| 26 | `actorLookup` | infrastructure | 없음 | — | Actor route lookup |
| 27 | `actorDestroy` | infrastructure | 없음 | — | Actor destroy coordination |
| 28 | `actorJoin` | infrastructure | 선택 | — | Actor가 어느 Spot에 속하는지 나타내는 [Actor membership](../00-foundation/02-glossary.ko.md#actor-membership) proposal |
| 29 | `actorLeft` | infrastructure | 없음 | — | Actor leave commit |
| 30 | `relocationReady` | infrastructure | 없음 | — | temporary queue·Restore와 relay 수신 준비 reply |
| 31 | `relocationData` | infrastructure | 없음 | — | capture 뒤 ingress-hold relay record 전달 |
| 32 | reserved (`relocationAck`) | — | — | — | 제거된 per-message ACK·numeric high-water command |
| 33 | `replyRelay` | infrastructure | 선택 | — | terminal completion relay |
| 34 | `relocationCutover` | infrastructure | 없음 | — | boundary 전 relay 전송 완료를 알리는 one-way control |
| 35 | reserved (`relocationComplete`) | — | — | — | 제거된 target completion reply command |
| 36 | `boundSessionSend` | application | 필수 | — | bound, STREAM client connection 하나를 수락한 때부터 닫을 때까지 유지하는 서버 실행 단위인 [STREAM session](../00-foundation/02-glossary.ko.md#stream-session) egress |
| 37 | `actorJoined` | infrastructure | 없음 | — | Actor join commit |
| 38 | `boundSessionBind` | infrastructure | 없음 | — | session binding commit |
| 39 | `instanceSpot` | application | 필수 | `metadata` | logical Instance Spot operation |
| 40 | `relocationPrepare` | infrastructure | 없음 | — | temporary queue 설치·final-stage(과 선택적 base-stage) payload manifest 선언·relay 준비 request |
| 41 | reserved (`relocationReserved`) | — | — | — | 제거된 relocation별 capacity reservation ACK |
| 42 | `sessionRelocationSeal` | infrastructure | 없음 | — | session ingress seal 요청 |
| 43 | `sessionRelocationSealed` | infrastructure | 없음 | — | Session seal 응답 |
| 44 | `sessionRelocationRoute` | infrastructure | 없음 | — | target route switch 또는 source의 relay-ready accepted 전 abort를 적용하는 one-way Session route update control |
| 45 | reserved (`sessionRelocationRouted`) | — | — | — | 제거된 Session route 적용 응답 command |
| 46 | `replyRelayAck` | infrastructure | 없음 | — | relayed terminal result ACK |
| 47 | `userSpotCreate` | infrastructure | 없음 | — | 미리 확보한 자리에 remote User Spot을 만든다 |
| 48 | `userSpotClose` | infrastructure | 없음 | — | 지정한 세대의 remote User Spot만 닫는다 |
| 49 | `actorCreate` | infrastructure | 없음 | — | 미리 확보한 자리에 remote Actor를 만든다 |
| 50 | `messageFollow` | infrastructure | 없음 | — | relay 성공 뒤 source runtime에 보내는 위치 cache 무효화 통지 |
| 51 | `boundSessionReplaced` | infrastructure | 없음 | — | 새 binding 확정 뒤 이전 session에 보내는 교체 통지 |
...
Command별 body 배치와 direction은 schema의 closed definition을 따른다.
...
infrastructure record다. Actor authority source fence와 이전 session owner의 lifecycle·binding identity를
...
- Manual 구성의 lifecycle token은 예측하기 어려운 난수를 만드는 [CSPRNG](../00-foundation/02-glossary.ko.md#csprng)로 만든 non-zero opaque equality token이다.
...
- 같은 revision의 같은 bytes는 멱등하고, 같은 revision의 다른 bytes나 낮은 revision은 protocol error다.
...
[Classic fanout](../00-foundation/02-glossary.ko.md#classic-fanout)이라고 한다. 이 publisher는 ACK를 받을 수 없으므로
...
규칙은 [Message model §2.3](../00-foundation/05-message-model.ko.md#5-framework-json-v1-typed-payload-profile)이 단독으로
...
- Store-backed authority는 provider가 발급한 `StoreVersion`, 같은 논리 ID의 서로 다른 incarnation을
  구분하는 번호인
  [`ObjectGeneration`](../00-foundation/02-glossary.ko.md#objectgeneration), 같은 incarnation
  안에서 authority owner가 바뀐 순서를 나타내는
  [`AuthorityOwnerGeneration`](../00-foundation/02-glossary.ko.md#authority-owner-generation)과
  current host의 `OwnerId`, host process lifecycle을 구분하는
  [`OwnerLeaseGeneration`](../00-foundation/02-glossary.ko.md#owner-lease-generation)을
  분리해 보존한다.
- Object generation은 delete 뒤 같은 key로 새 object를 만들 때만 바뀐다.
- 저장소가 기록한 현재 owner와 그 자격을 [Authority](../00-foundation/02-glossary.ko.md#authority)라고 하며, authority owner generation은 같은 object의 owner가 바뀔 때마다 올라가 낡은 owner의 변경을 막는다.
...
[장애 대응과 failover 범위 §4.4](../05-location-relocation/06-failure-failover-policy.ko.md#44-instance-spot-cold-activation과-owner-장애를-구분한다)가
...
Normal Instance send·request는 global SpotRid만 포함하며 create command가 아니다. Target Spot이 없을 때
새 Instance Spot을 준비해도 된다는 caller의 명시적 선택인
[Instance intent](../00-foundation/02-glossary.ko.md#instance-intent)를 실은 Missing+Instance intent의
target은 command 39의 optional metadata presence·frame까지 보존한 complete
`instance-activation-recovery-v1` envelope를, Instance Spot cold activation에 필요한 activation
envelope와 relocation 뒤 완료되는 reply payload를 보관하는 저장소인
[Relocation Store](../00-foundation/02-glossary.ko.md#relocation-store)에 저장하고 receipt를 Reserve에
연결한다. 이 format과 durable activation inbox는 target-owned
...
Recovery pointer를 지우기까지의 순서는 다음과 같다.

1. `Ready`를 게시하기 전에 durable inbox의 first record를 확정한다. handler는 barrier로 막는다.
2. Startup은 queue head를 복원하기 전에 Serving을 게시하지 않는다.
3. 첫 handler의 terminal completion을 durable하게 기록하고, replay cursor를 inbox sequence까지
   갱신한다.
4. 그 뒤에만 recovery pointer를 Preserve CAS로 제거한다. **Queue admission만으로는 지우지
   않는다.**

### Cold activation recovery 실패 처리

- authority가 Missing이고 caller가 Instance intent를 지정했을 때 새 Instance Spot을 만들고 초기화하는
  과정인 [Cold activation](../00-foundation/02-glossary.ko.md#cold-activation) recovery 실패는
  local barrier를 seal하고 request를 한 번만 terminal 처리한 뒤 one-way drop event를 기록한다.
...
- Target은 current authority와 [Actor membership](../00-foundation/02-glossary.ko.md#actor-membership)과 relocation state를 수용 판단 전에 검사한다.
...
[15. Spot과 Actor 모델 §4.2](../03-spot-actor/05-spot-actor-membership.ko.md#42-다른-node의-spot으로-actor를-join하는-순서)에
...
[21. Location Runtime §2.4](../05-location-relocation/01-location-runtime.ko.md) 참조) 이미 `allocation.stableType`을
가진다. admission 시 수신자는 body의 `ActorId`에 해당하는 그 row를 **반드시 읽고**, row가 actor
route fence와 정확히 일치할 때만 join을 수용한다.

| Authority row에서 읽는 값 | 맞춰 보는 fence 값 | 조건 |
|---|---|---|
| `allocation.state` | — | `active`여야 한다 |
| `allocation.objectKind` | — | `actor`여야 한다 |
| `objectGeneration` | `ObjectGeneration` | 일치 |
| owner node RID | target node RID | 일치 |
| descriptor lifecycle generation | node generation | 일치 |
| `authorityOwnerGeneration` | `expectedAuthorityOwnerGeneration` | 일치 |
| `ownerLeaseGeneration` | `expectedOwnerLeaseGeneration` | 일치 |

일곱 줄 중 하나라도 어긋나면 join을 받아들이지 않는다.
...
- Target은 각 chunk를 순서대로 CRC-32C 누적과 생성 incremental decoder(07 §7.2 `replay`)에
  공급하며 complete encoded stream을 별도 buffer로 조립하지 않는다. 마지막 chunk에서 누적 길이·
  CRC-32C를 Prepare가 선언한 값과, decode 완료를 함께 확인한다. 어느 하나라도 어긋나면 명시적
  실패이며, target은 부분 복원을 시도하지 않고 투명하게 재시도하지 않는다. 실패하면 target은 자신의 부분 chunk와 준비 자원을 정리한 뒤 대응하는
...
수락한 순서만 유지한다. 현재 Actor·Spot을 실제로 실행하며 그 application queue를 관리하는
[Owner](../00-foundation/02-glossary.ko.md#owner) 전환 뒤 이전 주소로 도착한 message는 Message
Follow가 target에 전달한다.
...
- Command 42는 current binding을 seal하고 command 43은 seal 설치 결과만 반환한다.
...
- `SessionRelocationSealTimeout`의 기본값은 3,000 ms다. 대응하는 command 44가 그 안에 오지 않으면
...
- `OperationId`는 두 `u64` word(`high`, `low`)로 이루어진 non-zero identity다. `ReplyRouteId`는
  별도 non-zero `u64`다. 둘 다 source owner lifecycle 안에서 고유하며 wrap과 reuse는
...
- Cleanup은 Location authority에서 reference를 해제한 뒤 Relocation Store delete를 수행한다.
- Published reference의 permanent missing, checksum mismatch 또는 inventory digest mismatch는 non-retriable `RelocationDataLost`이며 commit된 owner·membership을 source로 되돌리지 않는다.
...
Schema self-test, 생성된 codec의 golden fixture decode 결과와 checked-in codec table만으로 다음을
확인한다.

**Schema와 codec**
- 생성 결과와 checked-in codec table이 schema와 일치한다.
- 모든 runtime은 typed application message의 `framework-json-v1` golden fixture에서 같은 value와 failure를 만든다.

**Decode와 admission**
- 완결되지 않은 길이, item count, UTF-8, enum·flag 값 또는 topology 방향을 담은 record는 application
