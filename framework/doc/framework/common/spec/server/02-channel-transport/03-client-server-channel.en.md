...
[Channel·Transport topic table of contents](README.en.md) · [Spec table of contents](../README.en.md) · [Previous: 02. Channel Messaging](02-channel-messaging.en.md) · [Next: 04. Network Listener Identity](04-network-listener-identity.en.md)

> A [ClientServer Channel](../00-foundation/02-glossary.en.md#clientserver-channel) is a one-directional service boundary where the Client
> starts a send or request and the Server runs a handler and replies to the
> request. This document defines role registration, endpoint discovery, weight-
> based target selection, and the drain and restart contract.
...
A [ClientServer Channel](../00-foundation/02-glossary.en.md#clientserver-channel) isn't a
[RouteMesh](../00-foundation/02-glossary.en.md#routemesh) option. It doesn't provide the
following functionality.

- [Node direct](../00-foundation/02-glossary.en.md#node-direct) (where a caller targets one specific
  [MeshNode](../00-foundation/02-glossary.en.md#meshnode) — a runtime node that sends or receives
  messages within a RouteMesh — by giving both the MeshName and the target RID) between
  MeshNodes
- Spot and Actor messaging
- [Logical Multicast](../00-foundation/02-glossary.en.md#logical-multicast) (delivering one message to
  multiple Spots of the same Channel by ChannelName and topic)
...
Actor. This call is a separate operation the application chose — it isn't
ClientServer automatically relaying the message.

The logical address a Channel caller specifies and the meaning of call
completion are defined by [Channel Messaging](02-channel-messaging.en.md).
...
languages. The precise .NET signature is defined by
...
The Client's endpoint and the Server's listener/handler configuration are
explained in [§3](#3-how-to-find-a-server-endpoint-and-make-it-ready) and
[§4](#4-weight-and-target-selection).
...
| Discovery method | Endpoint source | [Location Store](../00-foundation/02-glossary.en.md#location-store) (a store that keeps each Spot's and descriptor's current state where multiple nodes can check it together) |
|---|---|---|
| Manual | The application registers it via `Connect(endpoint)`. | Not needed if only manual endpoints are used. |
| Automatic | Queries the ClientServer Server descriptor of the same [ChannelName](../00-foundation/02-glossary.en.md#channelname). | Location Store is required. |

If the manual and automatic sources point to the same Server RID and
...
In this flow, discovery only produces a candidate endpoint. It becomes a
[ready target](../00-foundation/02-glossary.en.md#ready-target) (a target that finished the
listener/transport connection, identity check, and required handler
registration, and so can receive new messages) only once identity and
generation are re-verified on the actual transport connection
([§3.2](#32-finding-the-server-descriptor-alone-does-not-make-it-ready)). The
...
In [automatic discovery](../00-foundation/02-glossary.en.md#automatic-discovery), one
connection intent, distinguished by Server RID and lifecycle generation, is
created per valid Server [descriptor](../00-foundation/02-glossary.en.md#descriptor) for the
same ChannelName. If multiple Servers are discovered, each Server's
[ready](../00-foundation/02-glossary.en.md#ready) connection is kept independently, and one
is selected by [§4's weight rule](#4-weight-and-target-selection) when
...
connection by comparing the two [MeshNode](../00-foundation/02-glossary.en.md#meshnode)s'
RIDs.
...
[owner](../00-foundation/02-glossary.en.md#owner) lease is information proving, by renewing
on a fixed schedule, that this Server still has the right to keep using the
...
- [Descriptor revision](../00-foundation/02-glossary.en.md#descriptor-revision) (an
  increasing number marking the version of mutable descriptor information,
  such as weight or drain state, within the same lifecycle)
...
must re-verify that [Server identity](../00-foundation/02-glossary.en.md#server-identity)
and lifecycle generation match on the actual transport connection before using
it as a ready target.
...
- Weight and [drain state](../00-foundation/02-glossary.en.md#drain)
- Security identity
...
- **Even if the local Server is selected, its handler isn't called directly.**
  The actual transport message is delivered from the Client `DEALER` to the
  Server `ROUTER`. This is so that codec, HWM, timeout, cancellation, the
  identifying information linking request and reply, and the
  terminal-completion rule aren't bypassed.
...
- **Even if a connection closure, timeout, or cancellation occurs after
  submit, the same request isn't automatically resent to a different Server.**
...
[descriptor revision](../00-foundation/02-glossary.en.md#descriptor-revision). The Client
only applies a larger revision within the same lifecycle generation. So a
...
[reply correlation](../00-foundation/02-glossary.en.md#reply-correlation). It completes with
...
The [reply token](../00-foundation/02-glossary.en.md#reply-token) a Server request handler
...
[downstream request](../00-foundation/02-glossary.en.md#downstream-request) uses separate
...
received ones, processed in the following order.
...
- **The Client can submit a request right before confirming the drain
  state.** If the Server rejects this request, it doesn't wait indefinitely —
  it completes with a finite rejected result. This is so that, even in the
  race between the submit point and the confirmation point, the caller
...
- **This value's order isn't judged by numeric magnitude.** Even at the same
  endpoint, a previous generation's connection and ClientServer Server
  descriptor aren't used as a new target. This is because the generation is a
  fence that only compares whether values are equal.
...
3. Makes the new generation a [ready target](../00-foundation/02-glossary.en.md#ready-target).
4. Removes the previous generation's connection.

The Client compares the reply correlation included in a reply against the
value of the currently pending request.

- **Only if the values match is it treated as that request's result.** So even
  a reply sent from a previous generation can become the result of the
  original request, if that request is still pending — because it's judged
  only by reply correlation, not by which generation it came from.
...
- **An already-ready connection and an already-accepted request aren't
  canceled merely due to a Store failure.** This is because discovery updates
...
- If multiple Servers are discovered, each ready connection is kept
  independently.
- RouteMesh's RID-comparison rule isn't used to decide the connection-start
  direction on a ClientServer Channel.
- Automatic discovery uses a ClientServer Server descriptor, and it isn't used
