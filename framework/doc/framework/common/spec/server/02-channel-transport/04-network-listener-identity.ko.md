...
[Channel·Transport 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 03. ClientServer Channel](03-client-server-channel.ko.md) · [다음: 05. Transport liveness](05-transport-liveness.ko.md)

> Network listener가 목적별로 서로 다른 주소 두 개를 가질 때의 계약과, 시스템 전체에서
> transport RID·Spot ID를 발급하는 정책을 정의한다.

## 1. Listener 주소 개요

[Network listener](../00-foundation/02-glossary.ko.md#network-listener)(현재 process에서
network endpoint를 bind하고 remote 연결을 받는 transport 구성 요소)에는 서로 다른
목적의 주소가 두 개 필요할 수 있다.
...
[RouteMesh](../00-foundation/02-glossary.ko.md#routemesh)(여러 MeshNode가 참여해 node와 Channel
message를 주고받는 범위), [ClientServer Channel](../00-foundation/02-glossary.ko.md#clientserver-channel)(Client가
send/request를 시작하고 Server가 handler 실행과 reply를 담당하는 단방향 service 경계),
classic fanout publisher와 STREAM server는 같은
...
자동 [Location Store](../00-foundation/02-glossary.ko.md#location-store)(각 Spot의 현재 owner와
상태를 여러 node가 함께 확인하도록 보관하는 저장소) record 대상이 아니다.

아래 C# 코드는 공통 계약이 .NET public API에서 어떻게 나타나는지 보여주는 참고
자료다. 다른 언어에 같은 signature를 요구하지 않는다. 정확한 .NET signature는
...
Listener를 여는 local 주소와 remote process가 접속할 주소가 다르면
[`BindHost`](../00-foundation/02-glossary.ko.md#bindhost)(listener가 현재 host의 어느 local
network interface에서 연결을 받을지 지정하는 주소)와
[`AdvertiseHost`](../00-foundation/02-glossary.ko.md#advertisehost)(remote process가 이
listener에 실제로 연결할 때 사용하는 host/주소)를 각각 지정한다. 이 구분은 container에 한정되지 않는다. 여러
...
## 2. Process 기본값과 listener override
...
한 [RouteMesh](../00-foundation/02-glossary.ko.md#routemesh) listener의 override가 다른 ClientServer, fanout 또는 STREAM listener의
endpoint를 바꾸지 않는다.
...
[wildcard 주소](../00-foundation/02-glossary.ko.md#wildcard-address)다.
...
다음 그림은 물리 층에서 bind가 확정한 port가 remote peer가 실제로 보는 advertised
endpoint로 이어지는 과정을 보여준다.

```mermaid
flowchart LR
    A["BindHost와 configured port<br/>(local socket이 bind하는 값)"] -->|Listener에 bind| B["실제 bound port 확정"]
    B -->|AdvertiseHost와 결합| C["Remote가 사용할 advertised endpoint<br/>(remote peer가 보는 값)"]
...
[Automatic discovery](../00-foundation/02-glossary.ko.md#automatic-discovery)(Location Store에
게시된 descriptor를 조회해 remote endpoint와 identity를 찾는 방식) listener는 port를
생략하면 port `0`을 사용한다. Framework가
...
- **Wildcard host와 port `0`은 local bind 입력에만 사용할 수 있다.** [Location Store](../00-foundation/02-glossary.ko.md#location-store)
  record나 manual peer 설정처럼 remote가 읽는 자리에 남아 있으면 startup 설정
  오류다. Remote는 실제로 접속 가능한 확정된 값만 읽어야 하기 때문이다.

### 3.1 Publisher가 확인하는 listener 상태

Publisher application은 publisher capability가 제공하는 listener 상태 조회로
현재 listener가 remote process에 제공하는 endpoint를 확인할 수 있다. 반환되는 port는 설정에
...
Listener를 다시 시작하면 조회 결과의 endpoint가 달라질 수 있다.

- **Application은 이 값을 subscriber 설정에 복사하지 않는다.** Automatic subscriber가
  current descriptor를 따라가는지 확인하는 관찰 자료로만 사용한다.
...
확정된 [advertised endpoint](../00-foundation/02-glossary.ko.md#advertised-endpoint)는 RouteMesh의
[MeshNode](../00-foundation/02-glossary.ko.md#meshnode)(RouteMesh에 참여해 node 사이 message를
주고받는 runtime node)처럼 listener 종류에 맞는
[descriptor](../00-foundation/02-glossary.ko.md#descriptor)나 설정에만 기록한다.

| Listener | Remote에 제공하는 위치 | 기록하면 안 되는 위치 |
|---|---|---|
| RouteMesh MeshNode | [MeshName](../00-foundation/02-glossary.ko.md#meshname)(하나의 RouteMesh 물리 연결 그룹을 식별하는 이름)과 RID로 식별하는 [MeshNode descriptor](../00-foundation/02-glossary.ko.md#meshnode-descriptor)에 endpoint를 기록한다. | ClientServer Server descriptor에 기록하지 않는다. |
| ClientServer Server | ChannelName과 Server identity로 식별하는 [ClientServer Server descriptor](../00-foundation/02-glossary.ko.md#clientserver-server-descriptor)에 endpoint를 기록한다. | MeshNode descriptor나 [Spot](../00-foundation/02-glossary.ko.md#spot)(주소와 상태를 가진 논리 instance)·Actor location row에 기록하지 않는다. |
| [Classic fanout](../00-foundation/02-glossary.ko.md#classic-fanout) publisher | [ChannelName](../00-foundation/02-glossary.ko.md#channelname)과 Publisher RID로 식별하는 [fanout publisher descriptor](../00-foundation/02-glossary.ko.md#fanout-publisher-descriptor)에 endpoint를 기록한다. | MeshNode나 ClientServer Server descriptor에 기록하지 않는다. |
...
[Automatic discovery](../00-foundation/02-glossary.ko.md#automatic-discovery)에 참여하는 classic fanout publisher는 fanout publisher
...
## 5. Listener 재시작과 lifecycle

AdvertiseHost나 실제 bound port가 바뀐 listener가 재시작되면 새 endpoint와 새
lifecycle generation을 같은
[descriptor revision](../00-foundation/02-glossary.ko.md#descriptor-revision)에 기록한다.

- **Endpoint만 바꾸면서 이전 [lifecycle generation](../00-foundation/02-glossary.ko.md#lifecycle-generation)을
  유지하지 않는다.** Remote runtime은 descriptor에서 읽은 identity와 generation이
  실제 transport 연결의 값과 같은지 확인한 뒤 ready 상태로 사용하기 때문이다.
...
## 6. 시스템 전체 transport RID와 Spot ID 정책

이 절은 개별 listener 설정이 아니라 MeshNode RID, Framework [진단 prefix](../00-foundation/02-glossary.ko.md#routing-id-prefix)와 Entry
[Spot ID](../00-foundation/02-glossary.ko.md#spot-id)(Spot을 식별하는 전역 논리 주소) 발급까지
포함하는 시스템 전체 identity 발급 정책을 다룬다.

Core에는 `NID`라는 별도 public type이 없다. [Routing ID](../00-foundation/02-glossary.ko.md#routing-id)는 1..255-byte
...
| Caller가 지정하는 User·Instance Spot ID | UTF-8 encoded 크기 1..255 bytes의 대소문자를 구분해 그대로 비교하는 문자열ring |
...
### 6.1 Diagnostic prefix와 UUID 표현
...
### 6.2 RID 충돌과 lifecycle

MeshNode descriptor를 게시할 때 Location Store는 같은 `(MeshName, RID)`가 이미 사용 중인지 확인한다.

- **UUID 충돌은 정상적인 운영 상황으로 간주하지 않는다.** [Active conflict가 확인되면](../00-foundation/02-glossary.ko.md#routing-id-conflict)
  Framework는 기존 descriptor를 변경하지 않고 새 UUID claim을 시도하지 않으며
...
- **Caller가 User·Instance Spot ID로 이 형식을 지정하면 Location Store나 factory를
  실행하기 전에 `InvalidOperation`으로 거부한다.** Prefix와 `entry` marker는 진단
  정보이며 stable host identity, shard나 application domain identifier가 아니기
  때문이다.
...
- **개별 RID, [Server identity](../00-foundation/02-glossary.ko.md#server-identity), weight, message
  수락과 drain을 구분해야 하는 listener는 여러 Pod를 하나의 일반 Service 가상 주소로
  대신하지 않는다.** Remote runtime이 각 Pod endpoint를 별도로 발견하고 연결할 수
  있어야 하기 때문이다.

## 8. 검증 요구

Network 설정 API, listener 상태 조회, MeshNode·ClientServer·fanout descriptor에 기록된
endpoint, 그리고 RID·Spot ID 발급 결과만으로 다음을 확인한다.

**Bind와 advertise**

- RouteMesh, ClientServer, classic fanout과 STREAM listener가 process 기본값과 listener
  override 우선순위를 같은 방식으로 적용한다.
...
- Listener endpoint를 [Spot](../00-foundation/02-glossary.ko.md#spot)·Actor location row에 복제하지 않는다.
- Advertised endpoint가 바뀐 재시작에서는 새 generation만 [ready](../00-foundation/02-glossary.ko.md#ready) 상태가 된다.
