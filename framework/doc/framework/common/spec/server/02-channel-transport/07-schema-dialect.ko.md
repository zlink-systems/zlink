...
chunk를 나눌 수 있음을 뜻한다. `replay`는 생성 decoder의 incremental 계약이며 `replay` object의
다섯 field가 다음 값을 정의한다: chunk를 순서대로 공급받아 decode하고 complete encoded stream용
입력 buffer를 할당하지 않으며(`wholeStreamInputAllocation`), 성공은 final chunk에서만 반환하고
(`completion`), final chunk에서 root가 미완성이면 `incompleteFinalChunk`, root 뒤에 byte가 남으면
`bytesAfterRoot`의 실패 종류로 끝난다. Chunk command와 checksum 규약, target이 chunk를 공급하는
절차는 [06 §9](06-wire-protocol.ko.md#9-maintenance-capture와-relocation-envelope)가 소유한다.
