...
[Observability 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 03. Message flow tracing](03-message-flow-tracing.ko.md)

> `correlation_id`·`flow_id`·`flow_origin`의 생성·형식·전파·소유권·수명을 정의한다.
> 이 주제의 네 문서가 나누는 소유 범위는
> [주제 목차 「2. 이 주제의 문서」](README.ko.md#2-이-주제의-문서)를 따른다.
...
[reply correlation](../00-foundation/02-glossary.ko.md#reply-correlation)이라고 하며, 공개 field 이름은
...
[Message model](../00-foundation/05-message-model.ko.md)이 정의한다. Trace에 field를 넣는 조건과
sampling은 [03. Message flow tracing](03-message-flow-tracing.ko.md)이 정의한다.
...
[downstream request](../00-foundation/02-glossary.ko.md#downstream-request)라고 한다. Downstream
...
시작으로 보고 값을 만든다. 아래 목록의 실행 node가 바뀌어도 같은 ID로 계속
message를 받는 논리 대상을 [Spot](../00-foundation/02-glossary.ko.md#spot)이라고
한다.
...
[03. Message flow tracing 「5. 실행 중 기록 수준 변경과 비용 규칙」](03-message-flow-tracing.ko.md#5-실행-중-기록-수준-변경과-비용-규칙)을
따른다. 각 처리 지점이 `Off`를 확인한 뒤에는 flow ID 생성, validation, context
capture, envelope field 추가와 전파용 내부 message 생성을 하지 않는다. 이미
만들어진 outbound frame에는 지나간 처리 단계에 되돌려 적용하지 않는다.
...
두 flow field를 보존하는 처리 경계의 표준 목록은
[03. Message flow tracing 「2.2 기록하는 public 동작」](03-message-flow-tracing.ko.md#22-기록하는-public-동작)이
정의하는 경계와 같다 — Node direct와 Channel, Global Spot ID로 보내는
[Spot direct](../00-foundation/02-glossary.ko.md#spot-direct), Instance Spot direct,
Actor direct와 STREAM Actor dispatch, Actor relocation, 현재 session에 연결된 push,
그리고 Logical Multicast와, 별도 socket으로 event를 전달하는
[Classic fanout](../00-foundation/02-glossary.ko.md#classic-fanout). 각 경계에서 두 flow field는 그 경계가
기록하는 마지막 처리 지점까지 보존된다 — 예를 들어 Instance Spot direct는 source
조회부터 첫 application turn까지, Actor relocation은 relocation control과 target
Actor의 관련 lifecycle 작업까지 보존한다.
...
않는다. 연결이 교체된 뒤 이전 [STREAM session](../00-foundation/02-glossary.ko.md#stream-session)의
reply와 push도 새 session의 flow와 연결하지 않는다. Actor와 현재 STREAM session의
연결을 식별하는 [binding token](../00-foundation/02-glossary.ko.md#binding-token)이 더 이상
...
[03. Message flow tracing 「3.2 Attribute 포함 조건」](03-message-flow-tracing.ko.md#32-attribute-포함-조건)이
정의한다. Metric label에는 세 값을 모두 사용하지 않는다.

세 field에는 user ID, Actor ID, Spot을 식별하는 전역 주소인
[Spot ID](../00-foundation/02-glossary.ko.md#spot-id), endpoint, payload와 application metadata를
encode하지 않는다. 외부 trace adapter도 Framework가 정한 형식과 소유권을 바꾸지
않는다.

## 9. 검증 요구

공개 표면(`correlation_id`·`flow_id`·`flow_origin`을 주고받는 request·reply, execution
context를 캡처하는 public interface)만으로 다음을 확인한다. 각 항목은 contract
test 하나로 이어진다.
...
- 생성 권한을 얻지 못한 target이 Ready owner로 message를 전달해도 새 식별자를
  만들지 않는다.
- 이전 STREAM session의 reply와 늦은 reply가 새 correlation에 연결되지 않는다.
- Downstream request가 새 `correlation_id`를 사용하고 원래 Spot·Actor activation에는
  terminal completion이 정확히 한 번만 전달된다.
...
- Tracing이 켜져 있을 때 Logical Multicast와 Classic fanout의 모든 branch가 원래
  `flow_id`를 보존한다.
- Callback 종료 뒤 관련 없는 callback에 이전 flow context가 남지 않는다.

**Tracing을 끈 상태**

- Tracing을 끈 runtime이 trace 전용 `flow_id`, `flow_origin`, flow context와
  전파용 내부 message를 만들지 않는다.
