...
[Observability 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 02. Runtime metric](02-runtime-metrics.ko.md) · [다음: 04. Request correlation](04-flow-correlation.ko.md)

> message 한 건의 진행 기록(trace)과 그 attribute·기록 수준을 정의한다. 이 주제의
> 네 문서가 나누는 소유 범위는 [주제 목차 「2. 이 주제의 문서」](README.ko.md#2-이-주제의-문서)를 따른다.

## 1. 무엇을 확인할 수 있는가

이 문서는 message 한 건이 Framework의 어느 처리 단계에 도달했고 어디에서
실패했는지를 trace와 structured log로 확인하는 계약을 정의한다. Node, Channel,
message를 받는 논리 대상인 [Spot](../00-foundation/02-glossary.ko.md#spot), Actor와
STREAM은 같은 단계 이름, 처리 결과와 attribute 이름을 사용한다.
...
Request와 reply를 같은 작업으로 연결하는 식별 정보인
[reply correlation](../00-foundation/02-glossary.ko.md#reply-correlation)과 여러 message가 같은
원인에서 이어졌음을 나타내는 `flow_id`의 생성·전파·수명은
[04. Request correlation](04-flow-correlation.ko.md)이 정의한다. 이 문서는 두 식별자를
기록에 넣는 조건만 정의한다.
...
| `backpressured` | 송신 경로나 queue의 공간이 부족한 [backpressure](../00-foundation/02-glossary.ko.md#backpressured)가 발생했거나 제한 시간까지 공간을 확보하지 못했다. |
...
논리 Channel 범위를 식별하는 [ChannelName](../00-foundation/02-glossary.ko.md#channelname)과 그 안의
수신 대상을 고르는 [topic](../00-foundation/02-glossary.ko.md#topic)으로 여러 Spot에 message를
전달하는 [Logical Multicast](../00-foundation/02-glossary.ko.md#logical-multicast)와 별도 PUB/SUB
경로로 event를 전달하는 [Classic fanout](../00-foundation/02-glossary.ko.md#classic-fanout)은
...
- MeshName과 target RID로 node 하나를 지정하는
  [Node direct](../00-foundation/02-glossary.ko.md#node-direct)와, 여러 MeshNode가 참여해 node와
  Channel message를 주고받는 범위인
  [RouteMesh](../00-foundation/02-glossary.ko.md#routemesh)·ClientServer Channel의 제출, 수신,
  handler 분배와 reply
- Global Spot ID로 message를 보내는 [Spot direct](../00-foundation/02-glossary.ko.md#spot-direct)의
...
- [STREAM session](../00-foundation/02-glossary.ko.md#stream-session)의 수신, Actor handler 분배,
...
사용하는지를 [message kind](../00-foundation/02-glossary.ko.md#message-kind)라고 한다. 다음 값은
...
[상태](../00-foundation/02-glossary.ko.md#shutdown)를 뜻한다.
...
`flow_id`가 없으면 RouteMesh에 참여해 message를 보내거나 받는 runtime node인
[MeshNode](../00-foundation/02-glossary.ko.md#meshnode)의 generation과 local sequence로
기록 여부를 결정한다.

다음 C#은 공통 동작을 설명하는 발췌다. 다른 언어에 같은 signature를
...
않는다. 각 언어의 public interface는 process의 현재 level을 읽고 바꾸는
...
**`Off`에서는 message마다 찍는 hot path가 log message를 만드는 비용 자체를 없앤다.**
현재 level을 확인하는 읽기와 분기 외에는 trace 전용 작업을 하지 않는다.
...
- Structured log message를 만들거나 형식화하지 않는다.
...
**Message마다 찍는 hot path는 `if (enabled(outcome))`로 감싸 event도 lambda도 만들지
않는다.** 실패·abort 등 드문 전이는 lazy 형태(`trace(outcome, build)` /
`traceLazy`)로 gate를 통과한 뒤에만 event를 만든다. Lazy 형태는 호출부에서 `if`를
없애 주지만 lambda 하나를 할당하므로(C++는 인라인되어 할당이 없다), hot path에서는
lazy 형태라도 `if`로 한 번 더 감싸 lambda 생성까지 막는다. 문자열 연결을 gate
...
Provider failure 자체를 기록할 때는 실패한 provider가 아닌 application의 fallback
logger 또는 process stderr를 사용한다. 이 fallback 기록의 실패도 message operation
결과, dispatch와 lifecycle을 바꾸지 않는다.
...
[04. Request correlation](04-flow-correlation.ko.md)을 따르며, 세 값을 metric label로
사용하지 않는다.
...
attribute key, diagnostics level·sampling rate 설정 interface)만으로 다음을 확인한다.
...
- Telemetry provider failure가 handler 분배, reply와 lifecycle 결과를 바꾸지 않는다.
