...
- **client 접속 라이브러리와 framework가 같은 프로토콜 스택을 따로 구현하지 않는다.** 같은
  ...
  처리한다. 프로토콜 처리는 한 곳에서 구현하고 양쪽이 그것을 사용한다.
...
- **resource는 만든 쪽이 닫는다.** 자식이 부모의 resource를 사용하는 동안에는 그 부모가 아직
...
식별자를 그대로 이어받아 사용하고, 바깥 층은 안쪽 층의 식별자를 모른다.
...
- **진행 중 호출 식별자 형식은 runtime 안에서 하나만 둔다.** 형식을 여러 개 두면 서로
  변환하는 코드가 생기고, 어느 경로가 어느 형식을 사용하는지 알려면 호출 그래프를 따라가야
  한다.
...
식별자에 같은 이름을 사용하면 두 개념이 문서와 코드에서 섞인다. 구현 코드에서는 다른
이름을 사용한다.

### 밖으로 내보내지 않는 값

- **물리 연결 식별자, 저장소 record version, 실행 queue의 내부 순번은 공개 DTO에 넣지
  않는다.** 이 값들은 runtime이 같은 대상을 다시 확인할 때만 사용하는 값이고, 밖으로 나가는
