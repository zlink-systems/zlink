...
- a fixed routing ID or an automatic RID's diagnostic prefix; [MeshNode §3.3](../03-spot-actor/03-mesh-node.en.md#33-fixed-rid)
  defines the fixed RID contract
...
This infrastructure work proceeds in an execution area an application handler
...
The same Entry Spot ID is kept within the same lifecycle, and a new Entry Spot ID is issued on a
...
4. Even if multiple targets or duplicate messages compete, only the runtime that secures
   ...
   envelope's message as the first record of the [durable activation inbox](02-glossary.en.md#durable-activation-inbox).
...
[Handler turn and execution gate §16](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-call-eligibility)
owns `Yield` availability by call and execution context;
[§3 of that document](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-gate-and-claim)
owns gate and Actor-claim lifetime. This API document defines no separate eligibility or execution rules.
...
- a fixed RID and an automatic RID prefix are not set together; the
