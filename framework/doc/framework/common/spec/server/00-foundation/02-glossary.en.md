...
[Foundation topic table of contents](README.en.md) · [Spec table of contents](../README.en.md) · [Previous: 01. Public Contract Governance](01-public-contract-governance.en.md) · [Next: 03. Framework Overview](03-overview.en.md)
...
| .NET notation | The precise type, if a formal public .NET type exists. |
...
[.NET Server per-language interface](../languages/dotnet/interfaces/README.en.md) and the
[.NET Stream Connector per-language interface](../../stream-connector/languages/dotnet/03-stream-connector.en.md).
The .NET notation in this glossary is a supplementary notation for reading the common
contract concretely.

## Rules This Glossary Follows

This glossary is the single source of truth for domain term definitions shared across
multiple specs. An individual spec introduces a term at its first use with one sentence
suited to the current context, then links to this document's entry — it doesn't
redefine the term for the whole spec
([spec documentation writing guide](../../../../../../../doc/principal/documentation/spec-writing-guide.ko.md#32-용어집-정의와-첫-링크) §3.2).

- A term denoting a value or a record first gets a summary table (Shape / .NET notation
  / Public composition / Creation-management / Lifetime), and shows the real `.NET`
  declaration below it when one exists.
- For an entry whose `.NET notation` says "no public type," the code block is contract
  pseudocode meant to illustrate structure — it isn't a real API name.
- The single source of truth for the actual `.NET` declarations is the `.NET`
  language-specific interface document. The `.NET` notation in this glossary is only a supplementary
  notation for reading the common contract concretely.
- The glossary is divided into 11 topic sections (`## N. Title`), and each entry has a
  fixed `<a id>` anchor and `### Term` heading. Other specs link directly into these
  anchors, so the anchor and heading strings never change.
- The same name is never reused for a different concept. When two contracts need the
  same name (for example, `OperationId`), the entry states within itself that they are
  different contracts to distinguish them.
- An entry doesn't restate a procedure, table, or error condition already owned by
  another contract document. This document owns the definition itself (shape, public
  composition, creation/management, lifetime), but the flow in which the value is used
  links to the document that owns that procedure.
- A new term is added only when all four of the following conditions hold — it can't be
  described precisely with an existing public identifier or industry term, several
  documents or APIs repeatedly need to distinguish the same concept, naming it is
  easier to understand than repeatedly explaining the behavior, and it doesn't overlap
...
| .NET notation | `IZLinkSpot`, `IZLinkInstanceSpot`, `IZLinkEntrySpot`; the location snapshot pinned to one specific incarnation is `SpotRef` |
...
| Shape | UTF-8 string |
| .NET notation | `string` |
| Public composition | Compared byte-for-byte, case-sensitive; UTF-8-encoded, 1 to 255 bytes. MeshName, Spot kind, and stable type aren't part of the ID. |
...
public readonly record struct SpotRef(
    ...
    string MeshName,         // the Mesh this specific incarnation belongs to
...
An execution object identified by Actor ID, with its own state and mailbox. The [Actor model](../03-spot-actor/04-actor-model.en.md) owns Spot membership and lifecycle.

| Item | Content |
|---|---|
| Shape | Logical execution object |
| .NET notation | `IZLinkActor`; declarations follow the [Actor interface](../languages/dotnet/interfaces/06-actors.en.md). |
| Public composition | Refs and contexts identify the Actor ID and incarnation. The Actor interface owns the full member list. |
| Creation/management | The runtime creates and manages it through a registered factory. Identity scope follows the Actor model. |
| Transfer | Actor messages address a global Actor ID; the object itself is not sent on the wire. |
| Lifetime | From creation until that logical incarnation ends. Continuity during a move follows the Actor lifecycle contract. |
| Application authority | The application registers Actor implementations and handlers, and uses public managers and clients. |
...
|---|---|
| `SpotWide` | Gate, turn, and Yield follow the [execution contract §2–§3](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate). |
| `PerActor` | Follows the mode-specific scope in the same execution contract. |
...
The relocation procedure per mode, and how the Spot instance is re-created on a
`PerActor` target, are owned by
[Spot Model "5. User Spot"](../03-spot-actor/01-spot-model.en.md#5-user-spot).
...
the same owner. Calling `Defer()` under `FrameworkManaged`, `PerActor`, Entry Spot, or
Instance Spot fails with `InvalidOperation` before the queue changes.

The timing of the `OnRelocationReadyCompleted` callback call after `Defer()`, and its
default no-op implementation, are owned by
[Spot Model "5.1 SpotWide Relocation Boundary"](../03-spot-actor/01-spot-model.en.md#51-spotwide-relocation-boundary).
...
The storage that keeps, as an opaque byte payload, the activation envelope needed for
an Instance Spot's cold activation and the reply payload/completion result of a
request that completes after relocation. It doesn't handle the authority the Location
Store manages, and stores only the payload before it's published to the Location Store
and the already-published payload, under a reference the framework issues.

| Item | Content |
|---|---|
| Shape | Distributed provider capability |
| .NET notation | `IZLinkRelocationStore`; a provider interface offering `Put`/`Read`/`Renew`/`Delete` operations. |
| Public composition | An immutable payload identified by a framework-issued opaque reference, managing retention and a provider-relative expiration time (`StoreNow`). |
| Creation/management | A provider package implements the SPI, and the framework calls the registered provider instance. The application doesn't call this SPI directly. |
...
current owner lease together. The framework checks these values to distinguish:
...
A lifecycle action that closes new application-work admission for a defined scope.
[Host relocation §14](../05-location-relocation/05-host-relocation-flow.en.md#14-the-race-between-shutdown-and-relocate)
owns the Shutdown host seal, the Relocate unit seal, and handling of accepted work.
This decision differs from an [execution gate](#execution-gate), which orders execution, or a permit, which limits capacity.
...
| Public composition | Compares that operation's `ObjectGeneration`, `AuthorityOwnerGeneration`, `OwnerId`, and `OwnerLeaseGeneration` against current authority, requiring all of them to match. Depending on the operation, the expected `StoreVersion` is also checked. |
...
| `Failed` | A normal approve/reject result wasn't produced due to node shutdown, timeout, or a callback exception. Includes a typed creation failure. | Doesn't create Ready authority or active capacity; returns reserved capacity. |
...
The order of each stage, the coordination among competing targets, and the sequence
diagram are owned by
[Framework API "16. Creating A Missing Object — Cold Activation Sequence"](06-framework-api.en.md#16-creating-a-missing-object--cold-activation-sequence).
...
| Delivery | Included in `Accepted`, `Rejected`, and `Failed` Actor Join completions. |
| Lifetime | [Actor Join completion](../03-spot-actor/05-spot-actor-membership.en.md#actor-join-completion) owns retention and retry for every outcome. |
...
together with the creation reservation and returns it on a direct read.
...
| Creation/management | Issued by the Location Store provider on a successful `Reserve`. Commit and Abort compare the same fence for equality. |
| Lifetime | Closes when the corresponding Creating authority commits to Ready or is cleaned up via that abort. |
...
A runtime mechanism that assigns execution authority so callbacks in one scope do not access state concurrently. [Handler turn and execution gate §2](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate) owns its scope.

| Item | Content |
|---|---|
| Shape | Runtime execution authority |
| .NET notation | No public type. |
| Public composition | No public fields; observed through callback execution order. |
| Creation/management | Managed by the runtime according to execution mode; not a value transmitted by ID. |
| Transfer | Process-local execution context, not transmitted on the wire. |
| Lifetime | Used within the execution object's lifetime. Authority transfer across turns and Yield follows execution contract §2–§3. |
| Application authority | The application uses registration options and public calls rather than creating or acquiring gates directly. |

<a id="handler-turn"></a>
### Handler Turn

The interval in which a handler acquires an execution gate, runs, and releases that authority. It describes Node, Channel, Actor, and Session execution contexts as well as Spots.

| Item | Content |
|---|---|
| Shape | Callback execution interval |
| .NET notation | No public type. |
| Public composition | No public fields; observed through handler and continuation execution. |
| Creation/management | The runtime associates it with the handler's execution authority. |
| Transfer | Process-local execution context, not a message field. |
| Lifetime | Start, wait, release, and resumption boundaries follow [execution contract §2–§3](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate). |
| Application authority | The application uses handlers and eligible terminals; it does not create or change turn identifiers. |

<a id="spot-turn"></a>
### Spot Turn

A [handler turn](#handler-turn) that runs a Spot callback. The
[execution contract §2–§3](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate) owns gate sharing and turn boundaries around Yield.
...
| Public composition | Both return the same request result. Availability by call follows [execution contract §16](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-call-eligibility). |
| Lifetime | Only one terminal method can run per fluent request call. |

Gate and [Actor queue claim](#actor-queue-claim) lifetime follow
[execution contract §3](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-gate-and-claim).

<a id="source-local-admission"></a>
### Source-Local Admission

The boundary where the sending runtime's queue for an operation family accepts work. It distinguishes one-way completion from remote handler processing.

| Item | Content |
|---|---|
| Shape | Operation acceptance phase |
| .NET notation | No public type. |
| Public composition | Returns no separate status or remote-receipt confirmation value. |
| Creation/management | Managed by the sending runtime; [Submit and completion §4](../01-execution/01-submit-and-completion.en.md#4-one-way-submit--the-admission-boundary) owns queue boundaries by target kind. |
| Transfer | A source-local completion boundary, not a wire receipt. |
| Lifetime | Applies through acceptance of one submission operation. Waiting and failure follow that family's completion contract. |
| Application authority | The application awaits a public call's terminal rather than directly selecting an internal queue. |
...
The component that delivers a finalized Framework operation's completion callback in a new execution interval. It is distinct from the pending registry that decides completion races.

| Item | Content |
|---|---|
| Shape | Process-shared runtime execution component |
| .NET notation | No public type. |
| Public composition | No public fields; observed through callback execution and completion. |
| Creation/management | Managed per process by the runtime. Its Framework service-operation responsibility differs from the native completion owner. |
| Transfer | Not transmitted on the wire; receives finalized callbacks inside the runtime. |
| Lifetime | [Submit and completion §11](../01-execution/01-submit-and-completion.en.md#11-the-execution-turn-of-the-completion-callback-implementation) owns slot reservation and callback-return boundaries. |
| Application authority | The application receives public-operation results rather than configuring or posting to the dispatcher directly. |

<a id="backpressure"></a>
### Backpressure

Flow control in which Core limits new send admission using local byte HWM and remote receive-flow
state. The local HWM value and the remote PAUSED/RUNNING state are separate inputs.
Their composition follows the [Core socket receive-flow contract](../../../../../../../core/doc/spec/core/socket/README.en.md#zlink_socket_set_receive_flow_state);
Framework permit pressure is forwarded as specified in
...
owns one-way call waiting and completion. The internal waiting state is
[Backpressured](#backpressured), not a public terminal result.
...
The byte budget a framework host instance forwards to Core for directional queue
HWMs. The framework forwards `CoreHwmMemoryLimitBytes`, `CoreHwmBudgetBytes`, and
`CoreHwmProfile` unchanged; Core computes the effective budget and per-queue HWMs.
The `CoreHwmProfile` name here is an independent type and calculation from the
same-named profile used by the application job queue.
...
| Lifetime | Kept for the Spot handler registry's lifecycle; the same key can't be registered twice on the same Spot. |

<a id="publish-target-snapshot"></a>
### Publish Target Snapshot

A [Snapshot](#snapshot) of the publish target set. Creation timing and target selection are owned by
[Spot messaging §4](../03-spot-actor/02-spot-messaging.en.md#4-channel-scoped-logical-multicast).
...
An execution unit serializing code that reads and writes one component's mutable state. It differs from application and lifecycle lanes that classify handler work.

| Item | Content |
|---|---|
| Shape | Component-internal state-ownership primitive |
| .NET notation | No public type. |
| Public composition | No public fields. Common internal entry points follow [State ownership and state lanes §7](../01-execution/06-state-ownership-and-lanes.en.md#7-per-language-mapping). |
| Creation/management | Managed by the component owning the state. Collections participating in one invariant follow the same ownership boundary. |
| Transfer | A process-local execution unit, not transmitted on the wire. |
| Lifetime | Valid while the component accepts state access; closing and reentrancy follow the state-lane contract. |
| Application authority | Not a public API through which the application directly selects a lane. |
...
| Application authority | The application uses messaging and timer APIs rather than changing internal lanes directly. |
...
| Shape | Runtime FIFO per execution object |
| .NET notation | No public type. |
| Public composition | No public fields. [Execution contract §7](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-lanes) owns separation from the application lane, priority, and bounds. |
| Creation/management | The runtime manages lifecycle work for the execution object. |
| Transfer | Not transmitted on the wire; retains work the runtime creates or classifies from received input. |
| Lifetime | Used during the execution object's lifecycle; work-reservation lifetime follows execution contract §7. |
| Application authority | The application uses lifecycle APIs and callbacks rather than changing internal lanes directly. |

<a id="yield-debt"></a>
### Yield Debt

Per-execution-object state marking the need to prioritize an application turn after consecutive lifecycle selections. It is a scheduler concept distinct from a public `Yield` call.

| Item | Content |
|---|---|
| Shape | Runtime fairness state |
| .NET notation | No public type. |
| Public composition | No public fields. [Execution contract §7](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-lanes) owns the bound and clearing conditions. |
| Creation/management | The scheduler manages it when selecting lanes for each execution object. |
| Transfer | Not transmitted on the wire or in a public snapshot. |
| Lifetime | From reaching the consecutive-lifecycle bound until an application turn runs and clears it. |
| Application authority | Not a value the application reads or sets. |
...
The application queue retaining Spot business payloads and timer callbacks.
[Handler turn and execution gate §7](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-lanes)
owns work classification and the separate lifecycle lane for control claims.

| Item | Content |
|---|---|
| Shape | Framework-owned serialized application queue |
| .NET notation | No public queue type |
| Public composition | Retains Spot direct, matching publish, and timer work items. Classification of Actor business payloads and lifecycle control follows the execution contract above. |
| Lifetime | Exists for the Spot incarnation; close and relocation follow their lifecycle contracts. |
...
| Lifetime | Registered when the target accepts a Restore request. Work is moved to the real object queue and this queue removed after owner commit and any needed callbacks. Discarded without running and source restored only on an abort before relay-ready is accepted. |
...
| Public composition | `PlannedMaintenance` uses the same effective target version as the source. `RollingUpdate` also specifies the `TargetApplicationVersion` greater than the source's. |
...
The version the mode determines is applied first, and then capability, policy,
...
connection and whether the identity the chunk carries matches.
...
| Lifetime | Core's byte charge ends when the complete chunk message is dequeued into the binding/framework. The target feeds the chunk data to the assembly ([06 §9](../02-channel-transport/06-wire-protocol.en.md#9-maintenance-capture-and-relocation-envelope): the running CRC-32C and the incremental decoder), then releases the input message buffer under ordinary framework ownership. A missing or duplicate ordinal, a chunk of an already-ended attempt, or exceeding the declared length isn't added to the assembly and is treated as an explicit failure. |
...
| Public composition | Exposed through the host status query and change observation of [Runtime monitoring](../06-observability/01-runtime-monitoring.en.md). |
...
| Public composition | Uses `RelocateAsync`, which takes a mode and the target application version specified; a separate `ShutdownAsync`; and host runtime state and deadline. |
...
The per-ChannelName registration roles of a [RouteMesh](#routemesh).
...
- Server role registers the send path plus remote target membership, and provides
  handlers and weight. A RouteMesh Server role also includes the Client's send capability.

The Client and Server of a [ClientServer Channel](#clientserver-channel) share the names but
are a different contract. The rule that a Server cannot start new business calls toward a Client
is owned by [ClientServer Channel §1](../02-channel-transport/03-client-server-channel.en.md#1-clientserver-channel-overview).

| Item | Content |
|---|---|
| Shape | Closed registration role |
| .NET notation | Expressed by the RouteMesh builder's Client/Server registration methods. `ZLinkClientServerRole` of ClientServer monitoring follows the ClientServer Channel document. |
...
| Creation/management | The framework determines the value to use for each new listener/server lifecycle. Issued values stay within the wire `nonzero-u64` range `1..long.MaxValue` (2^63-1) — values using the top bit of a full-width `ulong` must not be issued. Receivers compare only whether the values are equal. |
| Lifetime | Kept until that run ends. The remote compares the descriptor's and transport admission's values for equality. |
...
| Public composition | Commonly has identity, lifecycle, advertised endpoint, state, and the current owner lease; each per-topology field is defined by its own descriptor entry. |
...
- If this is an Object Server, the Entry Spot ID issued in the same lifecycle
...
public sealed record ZLinkMeshNodeDescriptor(
    ...
    string? EntrySpotId,                        // Entry Spot ID for the Object Server lifecycle
...
prefix. The default is `64 KiB` (`65,536` bytes).

`0` adds no separate Framework cap and
maps to Core `ZLINK_OPT_MAXMSGSIZE = -1`. A positive value is finite, and a negative value
is a startup configuration error.

This cap isn't applied to a message sent from server
to client.

An over-limit message isn't partly delivered to the session handler; the
...
| Lifetime | Kept for the host handler registry's lifecycle; the same key can't be registered twice within the same namespace. |
...
| Item | Content |
|---|---|
| Shape | A two-frame internal multipart record |
| .NET notation | No public type |
...
| Lifetime | The subscriber processes only a record matching the defined format as a liveness signal. If the topic matches but the payload or frame count differs, it's a protocol error and isn't delivered to an application handler. |
...
A capability the Framework provides to reply once to the current request.
A typed handler's reply return and an explicit STREAM Reply call use this capability through different surfaces.
...
| Lifetime | Valid until `LeaseExpiresAt`. Stale if the stored owner token differs or the store-relative expiry has passed. |
...
public abstract record ZLinkOwnerLeaseReadResult
{
    ...
    public sealed record Found(
        ZLinkLocationOwnerToken Token, // the current owner identity and generation
...
A byte value identifying a MeshNode within the same RouteMesh. The framework generates
a new value per lifecycle (automatic RID), or the application specifies a fixed RID.
...
| Creation/management | Generated by the framework per lifecycle in automatic mode. On an active conflict, it doesn't generate a new UUID or a second claim. The application can specify a fixed RID ([MeshNode §3.3](../03-spot-actor/03-mesh-node.en.md#33-fixed-rid)). |
| Lifetime | Unchanged for a MeshNode's lifecycle. An automatic RID is a new value for each replacement lifecycle even at the same endpoint; fixed RID lifecycle rules are defined by [MeshNode §3.3](../03-spot-actor/03-mesh-node.en.md#33-fixed-rid). |
...
| Public composition | A single opaque token. Internal encoding isn't part of the public contract. The binding relationship is verified together with the bound `ActorRef`, current authority/lease generation, session identity, and binding generation. |
...
Actor's route changes, and the conditions under which other Actors' routes and
connections are kept, are owned by
...
Command 44 route update that the target runtime sends to the Session owner as `[send]`
after the Location Store CAS and target queue opening. The Session owner uses this
update to change the binding route and submit messages held during the seal. There is
no reply, and reserved command 45 `sessionRelocationRouted` is neither sent nor
accepted.

Command 44's precise fields, timeout, abort handling, and the Message Follow condition
are owned by
[Session And Actor Binding "8.2 Control Messages 42, 43, 44"](../04-session/02-session-actor-binding.en.md#82-control-messages-42-43-44).
...
| Public composition | A single positive generation value that, together with `OwnerId`, forms the owner token. |
...
A STREAM transport unit. [Stream Connector §4](../../stream-connector/32-stream-connector.en.md#4-wire-contract)
owns its wire composition and sequence constraints.

<a id="dispatch-mode"></a>
### Dispatch Mode

A Connector callback execution mode. [Stream Connector §7](../../stream-connector/32-stream-connector.en.md#7-dispatch-mode)
owns its values and execution contract.
