...
[.NET per-language interface table of contents](README.en.md) · [Topology Configuration](03-configuration-topology.en.md)

## 1. Scope

This document defines the contract for registering ZLink Framework with
the ASP.NET Core host and DI. The signature of the RouteMesh
...
The framework root is registered once on one `IServiceCollection`. The
 member of `IZLinkFrameworkOptions` is owned by
...
new version, `RollingUpdate` and an target version greater than
...
different version. The option and result types are owned by
...
|---|---|---|
| `IZLinkRouteClient` | singleton | Node direct and [ChannelName](../../../00-foundation/02-glossary.en.md#channelname) send/request |
| `IZLinkSpotClient` | singleton | Global SpotId direct send/request and explicit Instance cold activation |
| `IZLinkSpotManager` | singleton | User Spot creation, resolve, and close |
| `IZLinkSpotPublisherClient` | singleton | [Spot](../../../00-foundation/02-glossary.en.md#spot) Logical Multicast publish |
...
| `IZLinkRouteMeshRuntimeOptions` | singleton | Query/set Mesh placement weight and ChannelName [weight](../../../00-foundation/02-glossary.en.md#weight) |
...
interface instance. The Location instance provides direct read,
...
A [MeshNode](../../../00-foundation/02-glossary.en.md#meshnode) that only uses manual
peers and doesn't use distributed location features can start without a
[location store](../../../00-foundation/02-glossary.en.md#location-store). Manual
peers also pass [MeshName](../../../00-foundation/02-glossary.en.md#meshname), RID,
...
[packet name](../../../00-foundation/02-glossary.en.md#packet-name) is determined
...
- The MeshNode's [Routing ID](../../../00-foundation/02-glossary.en.md#routing-id)
 and listener configuration. Server
 [membership](../../../00-foundation/02-glossary.en.md#membership) can be 0
...
- Duplicate Object role selection, Location Store registration for
 ...
 [factory](../../../00-foundation/02-glossary.en.md#factory) for None role
