...
[.NET 언어별 interface 목차](README.ko.md) · [Topology configuration](03-configuration-topology.ko.md)
...
시그니처는 [언어별 interface 목차](README.ko.md)의 기능별 문서가 소유한다.
...
`RollingUpdate`와 source보다 큰 target version을 함께 지정한다. 요청한 version의 eligible node가
...
| service | lifetime | 책임 |
|---|---|---|
| `IZLinkRouteClient` | singleton | Node direct와 [ChannelName](../../../00-foundation/02-glossary.ko.md#channelname) send/request |
| `IZLinkSpotClient` | singleton | global SpotId direct send/request와 명시적 Instance cold activation |
| `IZLinkSpotManager` | singleton | User Spot 생성, resolve와 종료 |
| `IZLinkSpotPublisherClient` | singleton | [Spot](../../../00-foundation/02-glossary.ko.md#spot) Logical Multicast publish |
...
| `IZLinkRouteMeshRuntimeOptions` | singleton | Mesh placement weight와 ChannelName [weight](../../../00-foundation/02-glossary.ko.md#weight) 조회·설정 |
...
각 interface instance를 하나씩 받는다. Location instance는 직접 read, conditional atomic batch와 bounded
...
Manual peer만 사용하고 분산 location 기능을 사용하지 않는 [MeshNode](../../../00-foundation/02-glossary.ko.md#meshnode)는 [location store](../../../00-foundation/02-glossary.ko.md#location-store) 없이 시작할 수 있다.
Manual peer도 [MeshName](../../../00-foundation/02-glossary.ko.md#meshname), RID, lifecycle generation, ChannelName set과 security identity admission을 통과한다.
...
Framework가 소유한다. [Packet name](../../../00-foundation/02-glossary.ko.md#packet-name)은 handler registration descriptor에서 결정하며 codec을 바꾸어도
...
- Actor·User Spot·[Instance Spot](../../../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot)의 stable type·구현 class 중복, explicit relocation policy와 type별 capacity
...
- [automatic discovery](../../../00-foundation/02-glossary.ko.md#automatic-discovery)·object role과 fixed routing ID의 잘못된 조합, RID prefix 형식
