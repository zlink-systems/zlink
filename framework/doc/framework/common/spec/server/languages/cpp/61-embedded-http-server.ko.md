...
[C++ 계약 목차](README.ko.md) | [언어별 interface 목차](../README.ko.md) | [이전: C++ HTTP Hosting](60-http-hosting.ko.md)
...
- [shutdown](../../00-foundation/02-glossary.ko.md#shutdown) 시작 뒤에는 새 connection을 받지 않는다.
...
HTTP server는 handler model을 새로 만들지 않는다. handler declaration과 호출 우선순위의
...
public TLS/server option builder 이름은 아래처럼 고정한다. `http_options_builder_t`의
