# C++ Actor 언어별 interface

[C++ 언어별 interface 목차](README.ko.md) · [Actor model](../../../03-spot-actor/04-actor-model.ko.md) ·
...
Actor가 사용하는 `actor_context_t`의 선언은
...
`actor_id_t`는 UTF-8 `1..255` byte global identity다. Constructor는 invalid 값을
...
concrete Actor를 만든 뒤 `configure()`를 호출한다. Factory는 전달받은
...
Framework는 payload를 Relocation Store에 기록하지 않고, source memory에서
`relocation_payload_chunk_limit_bytes` 이하의 chunk로 나눠 source–target ordered mesh 연결로 직접
전송한다. 반환한 byte vector의 소유권은 Framework로 이동하고, `restore(...)`에 전달한 byte
...
`create`와 `get_or_create`가 반환하는 `actor_create_call_t`의 single-use, 중복 option과
terminal 재호출 오류는 [Actor 모델 §6.2](../../../03-spot-actor/04-actor-model.ko.md#62-create와-getorcreate-입력)가 소유한다.

`in_mesh`를 생략했을 때 object role Mesh가
...
같은 stable type의 [Ready](../../../00-foundation/02-glossary.ko.md#ready) Actor를 callback 없이
...
membership의 Ready `spot_ref_t`만 반환하고 Entry [membership](../../../00-foundation/02-glossary.ko.md#membership) 또는 Missing Actor에는 빈 optional을 반환한다.
`Destroy`는 ActorRef만 변경한다.
...
Actor creation은 selected owner MeshNode의 [Entry Spot](../../../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot) membership을 Ready barrier 안에서 함께 확정한다. Actor
...
Bind는 caller가 제출한 ActorRef 위치로 한 번만 control request를 보낸다. Stale·moving 결과에서 global
...
`notify_disconnected()`는 physical connection을 닫지 않는 logical disconnect다. binding의 Spot
callback을 최대 한 번 실행하고 terminal 뒤 binding을 tombstone으로 확정하여 제거한다. Physical STREAM
connection과 Actor·Spot membership은 유지하며 새 public unbind API는 제공하지 않는다. Rebind는 새 identity를
current로 등록한 즉시 완료되며 이전 session의 처리를 기다리지 않는다. 이전 session의
`on_actor_binding_replaced(...)`에서 client 안내를 보낼 수 있다. Callback이 성공 또는 실패로 terminal이 되면
Framework가 `100 ms` 뒤 connection을 닫는다. Outbound queue가 먼저 비어도 이 시간을 줄이지 않는다.
Callback이나 close 실패는 새 binding을 제거하거나 이전 binding을 복원하지 않는다.
...
제출하지 않는다. Connection 종료는 Actor의 [Spot](../../../00-foundation/02-glossary.ko.md#spot) membership을 바꾸거나 Actor를 자동 종료하지 않는다.

## 5. Public trace category

이 문서의 declaration은 public trace의 `actor-relocation` category에 속한다. 공통 의미는
[Actor model](../../../03-spot-actor/04-actor-model.ko.md), [Spot·Actor membership](../../../03-spot-actor/05-spot-actor-membership.ko.md)과
