...
struct session_message_context_t {
    ...
    bool can_reply;
    std::shared_ptr<session_actor_t> actor; // the current binding the packet's Actor slot points at; nullptr when there is none
};
...
is kept, and waits until the callback terminal. `session_actor_t::ref() const noexcept` returns `const actor_ref_t &`.
