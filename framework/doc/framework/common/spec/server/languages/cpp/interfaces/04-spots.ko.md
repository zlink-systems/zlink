...
[cold activation](../../../00-foundation/02-glossary.ko.md#cold-activation)을 시작한다. Marker가 없는 일반 send·request에서
RID가 없으면 `not_found`로 끝나며 factory를 실행하거나 creation intent를 기록하지 않는다.

Source와 target은 다음 순서로 역할을 나눈다.

1. Source는 `Ready` [authority](../../../00-foundation/02-glossary.ko.md#authority)가 있으면 current owner에게 일반 message를 보낸다.
...
없다. 실패는 Abort로 authority와 reserved capacity를 함께 정리한다.
...
call에서만 missing RID의 [activation envelope](../../../00-foundation/02-glossary.ko.md#activation-envelope)를 target에 보낸다. Target runtime은 envelope를 근거로 자신을
owner로 creation claim을 시작한다. `instance_spot()`은 [stable type](../../../00-foundation/02-glossary.ko.md#stable-type)을 생략한 marker이고,
