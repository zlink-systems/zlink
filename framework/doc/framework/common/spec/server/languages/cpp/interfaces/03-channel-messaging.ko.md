# C++ channel messaging 언어별 interface

[C++ 언어별 interface 목차](README.ko.md) · [MeshNode](../../../03-spot-actor/03-mesh-node.ko.md) ·
...
[C++ monitoring 언어별 interface](08-monitoring.ko.md)가 정의한다.
...
[MeshNode](../../../00-foundation/02-glossary.ko.md#meshnode)도 시작할 수 있다. `add_client_server_channel(channel_name)`은 단방향
...
역할은 별도 registration으로 같은 ChannelName의 topology를 공유한다. [RouteMesh](../../../00-foundation/02-glossary.ko.md#routemesh)의 역할 단일 선택과
[ChannelName](../../../00-foundation/02-glossary.ko.md#channelname) 충돌 규칙은 바꾸지 않는다.
...
`set_instance_spot_idle_timeout(...)`은 유휴 [Instance Spot](../../../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot) 정리 기준 시간이다. 기본값은
...
Server와 같은 candidate 집합에 포함한다. [Ready](../../../00-foundation/02-glossary.ko.md#ready), positive weight, non-draining 조건을 동일하게 적용하고
...
intent 하나를 만든다.
`subscriber_connections()`는 builder의 `connect(endpoint)`와 같은 [manual endpoint](../../../00-foundation/02-glossary.ko.md#manual-endpoint) 집합을 가리키는
...
Missing Instance Spot의 cold activation intent를 표현하며 address에 [MeshName](../../../00-foundation/02-glossary.ko.md#meshname), stable type, owner RID 또는
