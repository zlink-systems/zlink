...
- **정리는 생성의 역순으로 한다.** provider나 scope를 닫으면 그 안에서 만든 인스턴스를 **만든
  순서의 역순으로** 정리한다. 의존 대상은 자신에게 의존하는 것보다 **나중에** 정리된다.

의존 대상은 주입되려면 먼저 만들어져 있어야 하므로, 생성 역순 하나면 의존 역순이 따라온다.
등록 순서나 의존 그래프를 따로 계산하지 않는다. 등록을 담는 자료구조는 이 순서를 보존해야 하며,
순회 순서가 정해지지 않은 자료구조에 정리 순서를 맡기지 않는다.
