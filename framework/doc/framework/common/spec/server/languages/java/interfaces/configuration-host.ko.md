...
[인터페이스 목차](README.ko.md) · [Transport liveness](../../../02-channel-transport/05-transport-liveness.ko.md)
...
capacity overflow는 bind 전에 실패하며 runtime 중 다시 계산하지 않는다.
...
[Location Store](../../../00-foundation/02-glossary.ko.md#location-store)가 필수다. Actor·User Spot·Instance Spot [factory](../../../00-foundation/02-glossary.ko.md#factory)는 stable type과 configure callback을
...
service-wire 표현 한계와 HWM은 별도 자원·wire guard로 유지한다.
...
`ROUTING_ID_CONFLICT`로 실패한다. Fixed RID의 사용 범위와 재시작 충돌은 [공통 MeshNode §3.3](../../../03-spot-actor/03-mesh-node.ko.md#33-fixed-rid)이 정한다. Slot count, allocation group과 public allocation provider는 제공하지 않는다.
