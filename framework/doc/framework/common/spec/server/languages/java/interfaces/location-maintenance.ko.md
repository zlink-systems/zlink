...
[Java 언어별 interface 목록](README.ko.md) · [공통 Location runtime](../../../05-location-relocation/01-location-runtime.ko.md) ·
[공통 Redis provider](../../../05-location-relocation/02-location-store-redis.ko.md)
...
- Key는 Framework가 발급하는 opaque UTF-8 `1..1024` bytes이며 case-sensitive 비교다.
...
- Version condition은 현재 version이 match일 때만 참이다.
...
- Conflict result는 실패한 condition이나 current value를 공개하지 않는다. Framework가 직접 read로
 필요한 key를 다시 확인한다.
...
- Prefix는 UTF-8 `0..1024` bytes이며 key와 같은 comparison을 사용한다.
...
transport 오류가 발생하면 commit 여부가 불확실할 수 있으며 Framework가 직접 read와 version 또는
