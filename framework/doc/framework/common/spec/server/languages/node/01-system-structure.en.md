...
[Node.js contract table of contents](README.en.md) | [Language interface table of contents](../README.en.md)
...
> [spot-messaging](../../03-spot-actor/02-spot-messaging.en.md), [MeshNode](../../03-spot-actor/03-mesh-node.en.md),
> [stream-session](../../04-session/01-stream-session.en.md), [actor-model](../../03-spot-actor/04-actor-model.en.md),
...
- **Codec implementations aren't mixed into core.** JSON is the default
 ...
 [Framework API §9](../../00-foundation/06-framework-api.en.md#12-codec).
- **The [location store](../../00-foundation/02-glossary.en.md#location-store)
...
provided. The contract is owned by
...
| [Spot](../../00-foundation/02-glossary.en.md#spot) packet/subscribe/actor/timer handler | Handler decorator + `zlinkDiscoverProviders(...)` | When processed in that Spot's execution context |
| Actor factory | `providers` + `addActorFactory(...)` | When ActorManager creates an actor |
| Stream session (or [factory](../../00-foundation/02-glossary.en.md#factory)) | `providers` + `streams` configuration | When a stream connection is activated as a session |
...
| `ZLinkDrainHealthIndicator` | [MeshNode](../../00-foundation/02-glossary.en.md#meshnode) readiness and health indicator |
...
If there's no [RouteMesh](../../00-foundation/02-glossary.en.md#routemesh) MeshNode
in a static `forRoot`, the RouteMesh runtime provider isn't created; if
there's no [ClientServer Channel](../../00-foundation/02-glossary.en.md#clientserver-channel),
...
The NestJS [shutdown](../../00-foundation/02-glossary.en.md#shutdown) hook uses
...
| `channel(name).client()` | Adds a ChannelName call role with no server [membership](../../00-foundation/02-glossary.en.md#membership) | Not required |
...
owned by
[Channel Topology §5](../../02-channel-transport/01-channel-topology.en.md) and
[Channel Messaging](../../02-channel-transport/02-channel-messaging.en.md).

## 7. Spot/Actor Registration

The Spot/Actor factory is registered on the owner MeshNode.
[Spot direct](../../00-foundation/02-glossary.en.md#spot-direct) and Logical
...
[MeshNode](../../03-spot-actor/03-mesh-node.en.md) and
...
[descriptor](../../00-foundation/02-glossary.en.md#descriptor). Actor create
finishes the selected owner MeshNode's Entry Spot membership and the
Actor [Ready](../../00-foundation/02-glossary.en.md#ready) barrier in the same
lifecycle.
...
[Spot Messaging §6](../../03-spot-actor/02-spot-messaging.en.md#6-failure-and-lifetime).
...
[stable type](../../00-foundation/02-glossary.en.md#stable-type) or the same
...
global [Spot ID](../../00-foundation/02-glossary.en.md#spot-id), stable type, and
...
global Spot ID and object generation. A regular message uses Spot
ID, not ref — the specified ref is only used for close. Store version and
...
[authority](../../00-foundation/02-glossary.en.md#authority). The application
doesn't additionally register [MeshName](../../00-foundation/02-glossary.en.md#meshname)
...
[Channel Messaging §9](../../02-channel-transport/02-channel-messaging.en.md#10-verification-requirements)
and [Spot Messaging §8](../../03-spot-actor/02-spot-messaging.en.md).
