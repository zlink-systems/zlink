...
[Spot](../../../00-foundation/02-glossary.en.md#spot) adapter is used for the Spot
...
the `"completed"` record don't block the target's message processing. Infrastructure
...
[deadline](../../../00-foundation/02-glossary.en.md#deadline). Relocation Store,
...
processing. [Session–Actor binding §8.2](../../../04-session/02-session-actor-binding.en.md#82-control-messages-42-43-44) owns relocation route updates for a bound Session. The `PerActor` Spot policy
...
operation ID, matching request-source fence (owner ID, lease generation,
...
publish part of the [descriptor](../../../00-foundation/02-glossary.en.md#descriptor).

`configureNetwork()`'s default BindHost is `127.0.0.1`. If AdvertiseHost is omitted, the
default of [Network listener identity §2.1](../../../02-channel-transport/04-network-listener-identity.en.md#21-defaults) applies. If the automatic discovery listener's port is
...
Server role needs a [Location Store](../../../00-foundation/02-glossary.en.md#location-store).
...
[weight](../../../00-foundation/02-glossary.en.md#weight) is an integer `0..10000`,
