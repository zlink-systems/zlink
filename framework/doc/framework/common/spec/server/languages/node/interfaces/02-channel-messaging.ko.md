...
Node application은 비동기 종결자 `submit(...)`(→ `Promise`)를 사용한다.
...
시작한다. Bound Session의 relocation route 갱신은 [Session–Actor binding §8.2](../../../04-session/02-session-actor-binding.ko.md#82-control-message-424344)가 소유한다.
...
진단으로 기록하지만 새 binding을 제거하거나 이전 binding을 복원하지 않는다. `ZLinkSessionActor.ref`의 타입은 `ActorRef`다.
