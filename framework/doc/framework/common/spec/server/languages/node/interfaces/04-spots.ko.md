...
받고 Spot 전용 call을 반환한다. [Instance intent](../../../00-foundation/02-glossary.ko.md#instance-intent)를 설정하지 않은 call은 existing-only이며 Missing에서
...
선택할 때만 사용하고 existing [owner](../../../00-foundation/02-glossary.ko.md#owner)의 current Mesh를 제한하지 않는다.

선택한 Mesh에 Instance type이 없으면 send와 request 모두 `NotFound`로 끝난다.
Distinct type이 여러 개인데 type을 생략하면 `InvalidOperation`이다. [Ready](../../../00-foundation/02-glossary.ko.md#ready) Instance authority가 있으면 저장된
