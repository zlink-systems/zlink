...
[Node.js 계약 목차](README.ko.md) | [언어별 interface 목차](../README.ko.md)
...
- **codec 구현을 core에 섞지 않는다.** JSON은 기본 codec이고, Protobuf·MessagePack은
 ...
 [Framework API §9](../../00-foundation/06-framework-api.ko.md#12-codec)을 따른다.
- **[location store](../../00-foundation/02-glossary.ko.md#location-store) 구현도 extension이다.** core는 store 계약만 알고 Redis 구현은 별도 package가
 제공한다(§10).
...
| [Spot](../../00-foundation/02-glossary.ko.md#spot) packet·subscribe·actor·timer handler | handler decorator + `zlinkDiscoverProviders(...)` | 그 Spot 실행 문맥에서 처리할 때 |
| actor factory | `providers` + `addActorFactory(...)` | ActorManager가 actor를 생성할 때 |
| stream session(또는 [factory](../../00-foundation/02-glossary.ko.md#factory)) | `providers` + `streams` 설정 | stream 연결을 session으로 활성화할 때 |
...
| `ZLinkDrainHealthIndicator` | [MeshNode](../../00-foundation/02-glossary.ko.md#meshnode) readiness와 health indicator |
...
정적 `forRoot`에서 [RouteMesh](../../00-foundation/02-glossary.ko.md#routemesh) MeshNode가 없으면 RouteMesh runtime provider를, [ClientServer Channel](../../00-foundation/02-glossary.ko.md#clientserver-channel)이 없으면
...
NestJS [shutdown](../../00-foundation/02-glossary.ko.md#shutdown) hook은 host 단위 `Shutdown`을 사용한다. Rolling maintenance에서 continuity가 필요하면
...
| `channel(name).client()` | server [membership](../../00-foundation/02-glossary.ko.md#membership) 없이 ChannelName 호출 역할을 추가한다 | 불필요 |
...
[channel-topology §5](../../02-channel-transport/01-channel-topology.ko.md)와
[channel-messaging](../../02-channel-transport/02-channel-messaging.ko.md)이 소유한다.

## 7. Spot·Actor 등록

Spot·Actor factory는 owner MeshNode에 등록한다. [Spot direct](../../00-foundation/02-glossary.ko.md#spot-direct)와 Logical Multicast는 Node·Channel
...
중복 등록과 타입 규칙은
[MeshNode](../../03-spot-actor/03-mesh-node.ko.md)와 [spot-messaging](../../03-spot-actor/02-spot-messaging.ko.md)이 소유한다.
...
[descriptor](../../00-foundation/02-glossary.ko.md#descriptor)를 게시한다. Actor create는 선택한 owner MeshNode의 Entry Spot membership과 Actor [Ready](../../00-foundation/02-glossary.ko.md#ready) barrier를
같은 lifecycle에서 완료한다.
...
[spot-messaging §6](../../03-spot-actor/02-spot-messaging.ko.md#6-실패와-수명)이 소유한다. 수동 outbound
...
relocation policy를 함께 등록한다. 같은 MeshNode에서 같은 [stable type](../../00-foundation/02-glossary.ko.md#stable-type)이나 같은 provider class를 User Spot
...
Spot direct fluent call의 Instance intent만 global [Spot ID](../../00-foundation/02-glossary.ko.md#spot-id), stable type과 최초 Mesh를 durable creation intent로
...
Ready location은 global Spot ID와 object generation을 포함하는 immutable `SpotRef`로 관측한다. 일반
message는 ref가 아니라 Spot ID를 사용하고, 지정한 ref는 close에만 사용한다. Store version과 owner fence는
...
[stream-session](../../04-session/01-stream-session.ko.md)이 소유한다.

## 9. Session actor dispatch 등록

계약은 [session-actor-dispatch](../../04-session/02-session-actor-binding.ko.md)가 소유한다. Stream node에서 Actor dispatch를
활성화하면 runtime이 global Actor ID와 current [authority](../../00-foundation/02-glossary.ko.md#authority)로 route를 결정한다. Application은 [MeshName](../../00-foundation/02-glossary.ko.md#meshname)이나 Spot
...
계약은 [runtime-monitoring](../../06-observability/01-runtime-monitoring.ko.md)과
[location-runtime](../../05-location-relocation/01-location-runtime.ko.md)이 소유한다.
...
검증 항목은
[channel-messaging §9](../../02-channel-transport/02-channel-messaging.ko.md#10-검증-요구)와
[spot-messaging §8](../../03-spot-actor/02-spot-messaging.ko.md)이 소유하는 규칙을 따른다.
