...
[Common Spot Contract](../../../03-spot-actor/05-spot-actor-membership.en.md)
...
[Spot](../../../00-foundation/02-glossary.en.md#spot) send/request only takes
SpotId. `SpotRef(spotId, objectGeneration, meshName, nodeRid)` is an
immutable snapshot only used when closing an specified incarnation.
`objectGeneration` is `1..Long.MAX_VALUE`, and a decimal string in
JSON. User and [Instance Spot](../../../00-foundation/02-glossary.en.md#entry-user-instance-spot)
type is a stable value of UTF-8 1..255 bytes. The Java enum's
...
[Spot ID](../../../00-foundation/02-glossary.en.md#spot-id). The manager doesn't
...
Spot send/request takes the global SpotId and returns `ZLinkKotlinSpotSendCall` or
`ZLinkKotlinSpotRequestCall<TReply>`. Marker overloads are `instanceSpot()` and
`instanceSpot(stableType: String)`; the Mesh input is `inMesh(meshName: String)`.
Send expresses completion as `suspend fun await(): Unit`; request uses
`suspend fun await(): TReply` and `suspend fun yield(): TReply`. Exact declarations
appear in [Kotlin Source Signature](#kotlin-source-signature).
...
`Blocked/StateIncompatible` if a [deadline](../../../00-foundation/02-glossary.en.md#deadline)
...
`onCreateActorSuspending` returns `ZLinkActorCreateResponse`, preserved by the JVM
bridge `onCreateActor` as `CompletionStage<ZLinkActorCreateResponse>`.
[Spot Model §4.2](../../../03-spot-actor/01-spot-model.en.md#42-entry-spots-actor-lifecycle) owns the common creation-response semantics;
[Java Spot Interface](../../java/interfaces/spots.en.md) owns the superinterface declaration.
...
public interface systems.zlink.framework.kotlin.ZLinkSuspendingSpotPacketHandler<TSpot, TMessage> {
 ...
 public default java.lang.Object handle(TSpot, TMessage, systems.zlink.framework.ZLinkMessageContext, kotlin.coroutines.Continuation<? super kotlin.Unit>);
}
public interface systems.zlink.framework.kotlin.ZLinkSuspendingSpotRequestHandler<TSpot, TRequest, TReply> {
 ...
 public default java.lang.Object handle(TSpot, TRequest, systems.zlink.framework.ZLinkMessageContext, kotlin.coroutines.Continuation<? super TReply>);
}
public interface systems.zlink.framework.kotlin.ZLinkSuspendingSpotSubscriptionHandler<TSpot, TEvent> {
 ...
 public default java.lang.Object handle(TSpot, TEvent, systems.zlink.framework.channels.ZLinkPublishMessageContext, kotlin.coroutines.Continuation<? super kotlin.Unit>);
}
...
public interface systems.zlink.framework.kotlin.ZLinkKotlinSpotRequestCall<TReply> {
 ...
 public abstract systems.zlink.framework.kotlin.ZLinkKotlinSpotRequestCall<TReply> timeout(java.time.Duration);
...
public interface systems.zlink.framework.kotlin.ZLinkKotlinSpotCreateCall {
 ...
 public abstract systems.zlink.framework.kotlin.ZLinkKotlinSpotCreateCall timeout(java.time.Duration);
...
processing. [Session–Actor binding §8.2](../../../04-session/02-session-actor-binding.en.md#82-control-messages-42-43-44) owns relocation route updates for a bound Session.
Infrastructure relocation doesn't call
...
[membership](../../../00-foundation/02-glossary.en.md#membership) callbacks.
