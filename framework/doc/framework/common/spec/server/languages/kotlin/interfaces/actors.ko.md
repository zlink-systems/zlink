...
[Actor 공통 계약](../../../03-spot-actor/04-actor-model.ko.md)
...
`ActorRef(actorId, objectGeneration, meshName, nodeRid)`는 지정한 incarnation을 destroy하거나 session에
bind할 때만 사용한다. `objectGeneration`은 `1..Long.MAX_VALUE`이고 JSON에서는 decimal string이다.

`ZLinkKotlinActorManager.create(actorId, actorType)`와 `getOrCreate(actorId, actorType)`는
`ZLinkKotlinActorCreateCall`을 반환한다. Single-use, 중복 option과 terminal 재호출 오류는
[Actor 모델 §6.2](../../../03-spot-actor/04-actor-model.ko.md#62-create와-getorcreate-입력)가 소유한다.

`inMesh`를 생략했을 때 object role Mesh가 하나면 자동 선택하고, 0개면
...
Actor type은 UTF-8 1..255 bytes의 stable value다. `Create`에서 Ready object가 있으면
`AlreadyExists`이며 새 attempt에서는 Java `ZLinkActorCreateResult`의 `Created`
또는 `Rejected`를 반환한다. `GetOrCreate`는 같은 type의
[Ready](../../../00-foundation/02-glossary.ko.md#ready) object를 callback 없이 `Existing`으로
...
`preserveStateWith(...)`로 등록한 Actor adapter는 maintenance cross-node materialization, remote User·Entry Spot join과 whole [User Spot](../../../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot)
...
Capture가 반환한 `ByteArray`에는 relocation adapter 전용 size 상한이 없다. Framework는 payload를 `relocationPayloadChunkLimitBytes` 이하의 chunk로 나눠 source–target
ordered mesh 연결로 직접 전송한다. Source memory가 복원 원본이며 handoff payload를 Relocation
Store에 저장하지 않는다. 배열의 소유권과 수명은 [공통 membership §6](../../../03-spot-actor/05-spot-actor-membership.ko.md#6-모든-이동-경로가-공유하는-relocation-policy)이 정한다. Empty `ByteArray`도 유효한 보존 state다.
[Factory](../../../00-foundation/02-glossary.ko.md#factory)는 target attempt마다 fresh Actor instance를 만들며 source나 이전 attempt instance를 재사용하지 않는다.
같은 attempt의 restore는 반복될 수 있다. Capture exception은 source [authority](../../../00-foundation/02-glossary.ko.md#authority)와 admission을 유지하고, restore
...
먼저 확정되지 않았으면 `Blocked/StateIncompatible`, [deadline](../../../00-foundation/02-glossary.ko.md#deadline)이 먼저 확정되면 `Blocked/DeadlineExceeded`다.
...
abstract class ZLinkSuspendingActor : ZLinkActor {
 ...
 // Java accessor를 같은 Context property에 연결한다.
...
adapter registration을 위한 reified helper, policy를 생략하는 overload와 default argument를 생성하지 않는다.
 `ActorRef`를 받는 public operation은
destroy와 session bind뿐이다. Missing 지정한 ref는 `false`, generation 불일치는 `InvalidOperation`, seal된
이관 구간은 `Unavailable`로 처리한다.

Actor Join에는 coroutine terminal을 추가하지 않는다. Java 언어별 interface의 동기 `defer()`를 handler
