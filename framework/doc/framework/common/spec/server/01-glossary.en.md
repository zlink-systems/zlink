...
[.NET Server exact interface](languages/dotnet/interfaces/README.en.md) and the
...
The update has no reply, and reserved command 45 `sessionRelocationRouted` is neither sent
nor accepted. On a failure before relay-ready is accepted, source coordinator sends command 44 abort one-way
so only the matching seal's held messages are submitted to the source route and its seal
is released. `SessionRelocationSealTimeout` defaults to 3,000 ms. Without
