...
| **Actor handle** | **The list and the lookup follow bound and unbound, the bound callback runs before that Actor's first packet callback, the registrations can be disposed, and a dropped transport closes every open handle with its unbound callback before the disconnected callback (§5.6, §7)** |
| **Actor handle send and receive** | **A handle's send and request carry that slot, a handle's receive registration gets only that Actor's messages, and send/request on a closed handle are `ValidationFailed` (§5.6)** |
