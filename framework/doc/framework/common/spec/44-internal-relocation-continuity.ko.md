...
[내부 구조 목차](README.ko.md) · [이전: 43. operation 완료 확정 — 한 번만 확정한다](43-internal-completion.ko.md) · [다음: 45. target 선택과 route cache](45-internal-routing-and-cache.ko.md)

> **이 장이 답하는 것** — 객체가 다른 node로 옮겨 가는 동안 그 객체로 향하던 message는 어디로 가는가.
>
> **계약 소유** — 이동 절차의 전체 순서는 [Actor와 Spot relocation 전체 흐름](28-relocation-flow.ko.md),
> host operation은 [Host relocation 전체 흐름](30-host-relocation-flow.ko.md),
> 저장소 계약은 [Location runtime](21-location-runtime.ko.md)이 소유한다.
...
[52. Relocation handoff 상태 전이](52-internal-relocation-handoff.ko.md)가 설명한다.
...
않는다**([Host relocation 전체 흐름 「8.2 모든 Actor와 Spot이 따르는 공통 순서」](30-host-relocation-flow.ko.md#82-모든-actor와-spot이-따르는-공통-순서)).
...
두지 않는다([Host relocation 전체 흐름 「9. 대기 중인 message, timer와 session을 옮긴다」](30-host-relocation-flow.ko.md#9-대기-중인-message-timer와-session을-옮긴다)).
개별 message 크기, transport, deadline과 cancellation 제한은 그대로 적용한다.

순서에 규칙이 하나 있다 — **복원된 이전 작업이 보관해 둔 message보다 먼저 실행된다**
([Spot 모델 「3.1 Relocation 중에는 temporary queue를 먼저 확인한다」](11-spot-model.ko.md#31-relocation-중에는-temporary-queue를-먼저-확인한다)). 반대로 하면 이동 전에 이미 큐에
...
[41. Spot·Actor 실행 직렬화](41-internal-serialization.ko.md)의 구조가 여기서 다시 걸린다.
...
주는 것이 **[Message Follow](01-glossary.ko.md#message-follow)**이며, 기본 동작 기간은 **30초**다
([Location runtime 「6.3 이전 owner로 도착한 message를 새 owner에게 전달한다」](21-location-runtime.ko.md#63-이전-owner로-도착한-message를-새-owner에게-전달한다)).
...
않으면 [43. operation 완료 확정](43-internal-completion.ko.md)의 완료 자리를 찾지 못해 caller가
timeout까지 완료되지 않는다.
...
Target의 saved-work·relay·temporary 구간은 dispatch 전 ordered durable backlog다. 이 구간으로 들어오는
ordinary record도 shared Application Job Queue reservation을 얻은 뒤 receive하고, target retained-byte
owner에 유한하게 handoff한 즉시 reservation을 반환한다. 아직 runnable하지 않은 backlog item은 live
queued-job permit을 보유하지 않는다. Target은 모든 staged payload의 byte owner가 확정된 뒤에만 relay
준비 reply를 보낸다.
...
Target은 기존 작업, cutover 앞의 relay, temporary queue에 들어온 작업 순서로 durable backlog를
확정하고 regular route로 전환한다. 필요한 lifecycle callback을 끝낸 뒤 dispatch를 runnable하게 만들고,
backlog의 handler turn마다 shared queued-job permit을 하나씩 얻어 live queue에 넣는다. Actual handler
start가 permit을 반환하므로 backlog 전체를 한 번에 예약하지 않는다. Permit을 기다리는 payload는 target
retained-byte owner가 계속 소유한다.
서로 다른 TCP connection에서 직접 target으로 들어온 message와
source relay 사이의 전역 순서는 보장하지 않는다. Target queue가 수락한 뒤의 순서만 유지한다.

Source는 cutover submit이 성공 또는 실패 terminal에 도달한 뒤 target 완료 응답을 기다리지 않고
Message Follow로 전환한다. Target은
...
Relay-ready reply가 accepted 상태가 되면 source 복구를 금지한다. Source는 그 뒤 한 번 수행하는
one-way cutover submit이 성공 또는 실패 terminal에 도달할 때까지 각 source queued job permit과
source payload byte owner를 유지한다. Terminal result 뒤 source dispatch를 영구 종료하고 이 owner들을
정확히 한 번 정리한다. Target completion ACK를 추가하지 않는다. Relay-ready 전 명시적 abort만
source owner를 유지하고 target staged byte owner를 정리한다.
...
[내부 구조 목차](README.ko.md) · [이전: 43. operation 완료 확정](43-internal-completion.ko.md) · [다음: 45. target 선택과 route cache](45-internal-routing-and-cache.ko.md)
