...
One relocation unit's temporary queue is at most 1,024 records, with no stored-size
cap. A request
...
| Resource | Move rule |
|---|---|
| A message arriving after new work is blocked | The source temporarily holds up to 1,024 records, with no stored-size cap. If the owner change succeeds, operation identity and ObjectGeneration are kept and delivered to the target. If the change is canceled, it's restored to the source queue in arrival order. |
