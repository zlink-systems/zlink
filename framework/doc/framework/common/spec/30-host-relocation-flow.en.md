...
Relocation correctness adds no separate cap on concurrent units, participant count, relay record
count, or encoded bytes. Existing runtime and Store provider memory, frame, and page-size limits
still apply. If a resource isn't immediately available, the Framework waits before blocking
...
sequenceDiagram
    ...
    Source->>Dispatch: [request] install temporary queue, Restore, prepare relay without dispatch
    Dispatch->>TempQueue: [local] register the temporary queue
...
sequenceDiagram
    ...
    TargetRuntime->>TargetTemp: [local] register the Actor temporary queue
    ...
    TargetRuntime->>TargetQueue: [local] add the stored queue/timer
...
sequenceDiagram
    ...
    loop for each member Actor independently
        ...
        TargetRuntime->>TargetActorQueue: [local] move stored queue/timer and temporary work
...
sequenceDiagram
    ...
    SourceRuntime->>RelocationStore: [request] store all Spot/Actor state, unexecuted queue, and timers
    RelocationStore-->>SourceRuntime: [reply] aggregate payload location and checksum fixed
    SourceRuntime->>TargetRuntime: [request] install aggregate temporary queue, Restore all, prepare relay
    TargetRuntime->>TargetTemp: [local] register temporary queue group for Spot and every Actor
...
sequenceDiagram
    ...
    SourceRuntime->>RelocationStore: [request] store Spot state, unexecuted queue, and timers
    RelocationStore-->>SourceRuntime: [reply] payload location and content checksum fixed
    SourceRuntime->>TargetRuntime: [request] install Instance Spot temporary queue, Restore, prepare relay
    TargetRuntime->>TargetTemp: [local] register the Instance Spot temporary queue
    TargetRuntime->>TargetSpot: [local] run factory and Restore state
    ...
    TargetRuntime->>TargetQueue: [local] move stored queue/timer and temporary work
    TargetRuntime->>TargetTemp: [local] remove registration, switch to existing dispatch
...
address. A deployment that needs the full configured `MessageFollowDuration` calls `Shutdown`
after that period. Calling `Shutdown` earlier removes the remaining Message Follow routes with
