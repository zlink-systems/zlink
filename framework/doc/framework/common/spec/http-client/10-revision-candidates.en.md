...
| --- | --- | --- | --- |
| R1 | Expose the error body on typed failure | The typed response terminator discards the response body at status ≥ 400, requiring a raw response terminator workaround to read the API error payload — a practical pitfall | The form that carries status+headers+raw body in the failure value (an exception field vs. a failure envelope), the 5-language expression |
...
| R5 | Deeper kotlin coroutine work | Cancellation doesn't propagate to the underlying request, streaming is only a callback sink (no `Flow`) | The `suspendCancellableCoroutine` propagation scope, whether to add a `Flow<ByteArray>` download |
