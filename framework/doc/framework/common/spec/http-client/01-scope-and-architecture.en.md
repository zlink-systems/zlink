...
- **The dependency direction is HTTP client → Framework contracts.** The HTTP client
  consumes the common Framework error model (`ZLinkFrameworkException` family) and
  codec contracts. It uses the same error classification for HTTP calls by sharing
  the exception hierarchy.
- **Framework core works without the HTTP client.** The HTTP client is a separate
  deliverable; each language documents the scope of its Framework package or target
  dependency. The `.NET` split into `Zlink.Framework.Contracts` does not require the
  same binary boundary in other languages.
- Integration connects server turns through the HTTP client execution scheduler
  injection point. [HTTP Client §3.2](12-http-client.en.md#32-the-turn-seam--a-single-injection-point) owns the completion/turn contract.
