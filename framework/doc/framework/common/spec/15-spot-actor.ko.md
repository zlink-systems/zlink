...
Owner generation은 hop마다 증가하며 최대 8 hops다. Route 하나의 queue에는 message 수와 저장
크기 어느 쪽에도 상한을 두지 않으며 negotiated message bound는 지킨다. Message Follow duration
만료, route 없음과 loop는 `Unavailable`, generation mismatch는 `InvalidOperation`이다. Framework는 failed operation을 fresh owner에게 hidden retry하지 않는다.
