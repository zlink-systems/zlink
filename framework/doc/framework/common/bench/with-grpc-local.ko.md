...
`Client CPU`는 모든 셀에서 기록한다. 선택 항목이 아니다.

client CPU가 95%를 넘은 셀은 포화 셀로 표시하고 처리량 우열 판정에 사용하지 않는다. 그 셀에서
상한을 정한 것은 transport가 아니라 client 런타임이기 때문이다. 포화 셀의 처리량은 "이 client
구성에서 관찰된 값"으로만 기록한다.

이 규칙은 Node에서 가장 자주 적용된다. Node client는 단일 스레드로 동작하므로 transport가
더 처리할 수 있는 구간에서도 client가 먼저 상한에 닿는다.
...
값을 KOPS로 남기고, 다른 언어의 with-grpc report는 `RESULT` 라인의 `throughput` 값을 초당
완료 수로 남긴다. C 값과 다른 언어 값을 직접 비교할 때는 다른 언어 값을 1000으로 나누어
KOPS로 맞춘 뒤 비율을 계산한다.
