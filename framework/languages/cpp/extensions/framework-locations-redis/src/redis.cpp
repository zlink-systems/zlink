...
class location_store_backend_t final : public location_store_t
{
  public:
    using options_type = redis_location_options_t;
    using options_builder_type = redis_location_options_builder_t;

    explicit location_store_backend_t (redis_location_options_t options) :
        _options (std::move (options))
    {
        if (_options.connection_string.empty () || _options.key_prefix.empty ())
            throw std::invalid_argument ("Redis location connection and key prefix are required");
        if (_options.key_prefix.find ('{') != std::string::npos
            || _options.key_prefix.find ('}') != std::string::npos)
            throw std::invalid_argument ("Redis location key prefix must not contain '{' or '}'");
    ...
    ~location_store_backend_t () override = default;
  ...
  private:
    ...
    task_t<store_scan_result_t> scan (store_scan_request_t request) override
    {
        ...
        return _worker.submit<store_scan_result_t> ([this, request = std::move (request)] {
            ...
            std::size_t offset = 0;
            if (request.cursor) {
                const auto first_separator = request.cursor->value.find (':');
                const auto second_separator =
                  first_separator == std::string::npos
                    ? std::string::npos
                    : request.cursor->value.find (':', first_separator + 1);
                if (first_separator == std::string::npos || second_separator == std::string::npos)
                    return store_scan_result_t{store_scan_expired_t {}};
                try {
                    const auto epoch =
                      std::stoull (request.cursor->value.substr (0, first_separator));
                    if (epoch != _scan_epoch)
                        return store_scan_result_t{store_scan_expired_t {}};
                    snapshot_id = std::stoull (request.cursor->value.substr (
                      first_separator + 1, second_separator - first_separator - 1));
                    offset = static_cast<std::size_t> (
                      std::stoull (request.cursor->value.substr (second_separator + 1)));
                }
                catch (...) {
                    return store_scan_result_t{store_scan_expired_t {}};
                }
            } else {
                if (_scan_snapshots.size () >= max_scan_snapshots)
                    _scan_snapshots.erase (_scan_snapshots.begin ());
                ...
                for (std::size_t index = 0; index + 1 < scanned.size (); index += 2) {
                    ...
                    if (decoded.tombstone
                        || (decoded.expires_at_ms > 0
                            && decoded.expires_at_ms <= static_cast<std::uint64_t> (unix_ms (now))))
                        continue;
                    snapshot.items.push_back (
                      {{decoded.original_key},
                       { string_to_bytes (decoded.raw_bytes),
                         ...
                         decoded.expires_at_ms > 0
                           ? std::optional{from_unix_ms (
                               static_cast<std::int64_t> (decoded.expires_at_ms))}
                           : std::nullopt,
                         now }});
                }
            ...
            if (snapshot == _scan_snapshots.end () || offset > snapshot->second.items.size ())
                return store_scan_result_t{store_scan_expired_t {}};
            store_scan_page_t page;
            page.store_now = now;
            auto end = offset;
            std::size_t encoded_size = 0;
            while (end < snapshot->second.items.size () && end - offset < request.limit) {
                const auto item_size = encoded_scan_item_size (snapshot->second.items[end]);
                if (end != offset && encoded_size + item_size > max_scan_page_bytes)
                    break;
            ...
            page.items.insert (page.items.end (),
                               ...
                               snapshot->second.items.begin () + static_cast<std::ptrdiff_t> (end));
            if (end < snapshot->second.items.size ())
                page.next_cursor =
                  store_scan_cursor_t{std::to_string (_scan_epoch) + ":"
                                      + std::to_string (snapshot_id) + ":" + std::to_string (end)};
            else
...
return result
    ...
    {
        if (prefix.size () > 1024)
            throw std::invalid_argument ("location scan prefix exceeds 1024 bytes");
    }

    static void validate_write (const store_write_request_t &request)
    {
        ...
        for (const auto &condition : request.conditions) {
            ...
            if (!conditions.insert (key.value).second)
                throw std::invalid_argument ("location write repeats a condition key");
        ...
        for (const auto &mutation : request.mutations) {
            std::visit (
              [&] (const auto &value) {
                  ...
                  if (!mutations.insert (value.key.value).second)
                      throw std::invalid_argument ("location write repeats a mutation key");
                  keys.insert (value.key.value);
                  encoded += value.key.value.size ();
                  using value_t = std::decay_t<decltype (value)>;
                  if constexpr (std::is_same_v<value_t, store_put_t>) {
                      if (value.bytes.size () > 1024u * 1024u)
                          throw std::invalid_argument ("location value exceeds 1 MiB");
                      if (value.retention && *value.retention <= std::chrono::milliseconds::zero ())
    ...
    static std::vector<std::byte> string_to_bytes (const std::string &value)
    {
        const auto *first = reinterpret_cast<const std::byte *> (value.data ());
        return {first, first + value.size ()};
    }

    static std::size_t encoded_scan_item_size (const store_scan_item_t &item) noexcept
    {
        return item.key.value.size () + item.value.version.value.size () + item.value.bytes.size ();
    }
    ...
    std::string hash_base () const { return _options.key_prefix + ":{zlink-location-v3}"; }
...
#if defined(ZLINK_FRAMEWORK_LOCATIONS_REDIS_HAS_ASYNC_CLIENT)
    static std::chrono::system_clock::time_point redis_now (sw::redis::Redis &redis)
    {
        const auto reply = redis.command<std::vector<std::string>> ("TIME");
        if (reply.size () != 2)
...
class relocation_store_backend_t final : public relocation_store_t
{
  public:
    using options_type = redis_relocation_options_t;
    using options_builder_type = redis_relocation_options_builder_t;

    explicit relocation_store_backend_t (redis_relocation_options_t options) :
        _options (std::move (options))
    {
        if (_options.connection_string.empty () || _options.key_prefix.empty ())
            throw std::invalid_argument ("Redis relocation connection and key prefix are required");
        if (_options.key_prefix.find ('{') != std::string::npos
            || _options.key_prefix.find ('}') != std::string::npos)
            throw std::invalid_argument ("Redis relocation key prefix must not contain '{' or '}'");
        if (_options.operation_timeout <= std::chrono::milliseconds::zero ())
            throw std::invalid_argument ("Redis relocation operation timeout must be positive");
#if defined(ZLINK_FRAMEWORK_LOCATIONS_REDIS_HAS_ASYNC_CLIENT)
        _redis = detail::make_redis (_options.connection_string, _options.operation_timeout);
#endif
    }

    ~relocation_store_backend_t () override = default;
  ...
  private:
                                   ...
                                   std::chrono::milliseconds retention) override
    {
        ...
        return _worker.submit<blob_put_result_t> (
          [this, reference = std::move (reference), payload = std::move (owned), retention] {
              auto &redis = *_redis;
              const auto keys = std::vector<std::string>{payload_key (reference.value)};
              const auto args = std::vector<std::string>{
                std::string{reinterpret_cast<const char *> (payload.data ()), payload.size ()},
                std::to_string (retention.count ())};
              const auto result = redis.eval<std::vector<std::string>> (
                std::string (put_script), keys.begin (), keys.end (), args.begin (), args.end ());
              if (result.size () != 3)
                  throw sw::redis::Error ("invalid opaque relocation put result");
              const auto now = from_unix_ms (std::stoll (result[1]));
              const auto expires_at = from_unix_ms (std::stoll (result[2]));
              if (result[0] == "stored")
                  return blob_put_result_t{blob_stored_t { expires_at, now }};
              if (result[0] == "same")
                  return blob_put_result_t{blob_already_stored_t { expires_at, now }};
              if (result[0] == "conflict")
                  return blob_put_result_t{blob_conflict_t { now }};
              throw sw::redis::Error ("unknown opaque relocation put result");
          });
#else
        (void) reference;
        (void) payload;
        ...
        return _worker.submit<blob_renew_result_t> (
          [this, reference = std::move (reference), retention] {
              ...
              if (!redis.pexpire (key, retention))
                  return blob_renew_result_t{blob_missing_t { now }};
              return blob_renew_result_t{blob_renewed_t { now + retention, now }};
          });
#else
        (void) reference;
        (void) retention;
    ...
    task_t<void> erase (blob_reference_t reference) override
    {
        validate_reference (reference);
#if defined(ZLINK_FRAMEWORK_LOCATIONS_REDIS_HAS_ASYNC_CLIENT)
        return _worker.submit<void> ([this, reference = std::move (reference)] {
            auto &redis = *_redis;
            (void) redis.del (payload_key (reference.value));
        });
#else
        (void) reference;
        return task_t<void> (
...
return {'stored', tostring(nowMs),
    ...
    static void validate (const blob_reference_t &reference, std::chrono::milliseconds retention)
    {
        validate_reference (reference);
        if (retention <= std::chrono::milliseconds::zero ())
            throw std::invalid_argument ("relocation retention must be positive");
    }
    ...
    {
        return _options.key_prefix + ":{zlink-relocation-v1}:blob:" + std::string (reference);
    }
...
redis_location_store_t::redis_location_store_t (redis_location_options_t options) :
    _store (std::make_unique<location_store_backend_t> (std::move (options)))
{
}

redis_location_store_t::~redis_location_store_t () = default;

task_t<store_read_result_t> redis_location_store_t::read (store_key_t key)
{
    return _store->read (std::move (key));
}

task_t<store_write_result_t> redis_location_store_t::write (store_write_request_t request)
{
    return _store->write (std::move (request));
}

task_t<store_scan_result_t> redis_location_store_t::scan (store_scan_request_t request)
{
    return _store->scan (std::move (request));
}

redis_relocation_store_t::redis_relocation_store_t (redis_relocation_options_t options) :
    _store (std::make_unique<relocation_store_backend_t> (std::move (options)))
{
}

redis_relocation_store_t::~redis_relocation_store_t () = default;

task_t<blob_put_result_t> redis_relocation_store_t::put (blob_reference_t reference,
                                                         std::span<const std::byte> payload,
                                                         std::chrono::milliseconds retention)
{
    return _store->put (std::move (reference), payload, retention);
}

task_t<blob_read_result_t> redis_relocation_store_t::read (blob_reference_t reference)
{
    return _store->read (std::move (reference));
}

task_t<blob_renew_result_t> redis_relocation_store_t::renew (blob_reference_t reference,
                                                             std::chrono::milliseconds retention)
{
    return _store->renew (std::move (reference), retention);
}

task_t<void> redis_relocation_store_t::erase (blob_reference_t reference)
{
    return _store->erase (std::move (reference));
}
