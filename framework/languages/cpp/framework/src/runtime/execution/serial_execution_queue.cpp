                                               ...
                                               item_origin_t origin)
{
    ...
    lane.queue.push_back (work_item_t{std::move (name), std::move (work), options.lane, bytes,
                                      submission_id, std::move (cancel), std::move (turn),
                                      std::move (origin.chain), std::move (origin.slot)});
    ++lane.messages;
    lane.bytes += bytes;
...
{
    return !_application.queue.empty () || !_lifecycle.queue.empty ();
}
...
{
    return !_application.queue.empty () || next_lifecycle_locked () != _lifecycle.queue.end ();
}
...
serial_execution_queue_t::work_item_t serial_execution_queue_t::take_next_locked ()
{
    ...
    const bool lifecycle_ready = lifecycle_next != _lifecycle.queue.end ();
