...
bool is_service_control_frame (const detail::backend::raw_bytes_t &frame) noexcept
{
    return frame.size () >= 5 && frame[0] == 0x5A && frame[1] == 0x4D;
}

detail::backend::raw_message_t envelope_wire_parts (runtime::messaging::message_parts_t parts)
{
...
std::size_t raw_message_bytes (const detail::backend::raw_message_t &parts) noexcept
{
    std::size_t result = 0;
...
liveness_connection_identity (const protocol::client_server_server_admission_t &server)
{
...
{
  public:
    explicit pending_request_guard_t (std::atomic_size_t &pending) : _pending (&pending)
    {
        _pending->fetch_add (1, std::memory_order_relaxed);
    }

    ~pending_request_guard_t () { _pending->fetch_sub (1, std::memory_order_relaxed); }
    ...
    return _lane
      .run ([this] {
          ...
          trace_client_server_lazy ("server-socket-options", [&] {
              return "endpoint=" + _options.descriptor.advertised_endpoint + " max_message_bytes="
                     + std::to_string (router->options ().max_message_size ().bytes ());
          });
          router->set_routing_id (
            zlink::routing_id_t::from (_options.descriptor.server_routing_id));
          ...
          _options.descriptor.advertised_endpoint = transport::advertised_tcp_endpoint (
  ...
  mesh::service_liveness_registry_t::clock_t::time_point now)
{
    ...
    for (;;) {
        ...
        trace_client_server_lazy ("server-monitor-event", [&] {
            return "event=" + std::to_string (static_cast<int> (event->event)) + " routing_id="
                   + (event->routing_id ? event->routing_id->to_string () : std::string ("-"))
                   + " value=" + std::to_string (event->value) + " local=" + event->local_addr
        ...
        if (event->event == zlink::monitor_event::connection_ready) {
            trace_client_server_lazy ("server-connection-ready", [&] {
  ...
  std::shared_ptr<application_job_queue_t::permit_t> application_permit)
{
    ...
    try {
        ...
        if (header.kind == protocol::command::hello) {
            ...
            //  protocol error.
            if (received->parts.size () != 1 || !received->reply_token) {
                co_return client_server_pump_result_t::protocol_error;
            }
            const auto client = protocol::decode_client_server_client_admission (
              received->parts.front (), protocol::command::hello);
            trace_client_server_lazy ("server-hello", [&] {
            ...
            if (client.channel_name != _options.descriptor.channel_name
                || client.security_identity != _options.descriptor.security_identity) {
                const detail::backend::raw_message_t reject_message{protocol::encode_reject (3)};
            ...
            trace_client_server_lazy ("server-admit-sent", [&] {
                return "channel=" + _options.descriptor.channel_name
                       + " client=" + routing_id_label (received->source_routing_id)
                       + " endpoint=" + _options.descriptor.advertised_endpoint;
            });
        ...
        if (header.kind == protocol::command::livenessProbe
            ...
            if (liveness.kind == protocol::command::livenessProbe) {
                ...
                if (received->reply_token) {
                    ...
                    ack_sent = co_await port->send (received->source_routing_id, ack_message);
                }
                if (!ack_sent) {
                    co_return client_server_pump_result_t::protocol_error;
                }
    ...
    if (prepared.port) {
        if (prepared.update) {
            const detail::backend::raw_message_t update_message{
              protocol::encode_client_server_server_admission (protocol::command::update,
                                                               *prepared.update)};
        ...
        for (const auto &probe : prepared.result.probes) {
            ...
            (void) co_await prepared.port->send (probe.node_routing_id, probe_message);
        }
    }
                                        ...
                                        const protocol::application_payload_t &payload)
{
    ...
    auto header =
      writer.create_reply_header (messaging::message_kind_t::response,
                                  request_header.value ().channel_name, request_header.value ());
    header.content_type = payload.content_type;
    auto parts = envelope_wire_parts (
      writer.reply_raw_envelope (header, zlink::message_t::from (payload.payload_bytes ())));
    const auto delivered =
      port->reply ({request.source_routing_id, request.reply_token, {}}, parts);
    trace_client_server_lazy ("server-reply", [&] {
        return "channel=" + request_header.value ().channel_name
               + " packet=" + request_header.value ().message_name
               + " client=" + routing_id_label (request.source_routing_id) + " reply_token="
...
raw_client_server_client_t::raw_client_server_client_t (raw_client_server_client_options_t options,
                                                        std::shared_ptr<zlink::context_t> context) :
    _options (std::move (options)),
    _context (context ? std::move (context) : std::make_shared<zlink::context_t> ()),
  ...
  mesh::service_liveness_registry_t::clock_t::time_point now)
{
    ...
    for (;;) {
        ...
        ++count;
        trace_client_server_lazy ("client-monitor-event", [&] {
            return "event=" + std::to_string (static_cast<int> (event->event))
                   + " value=" + std::to_string (event->value) + " local=" + event->local_addr
                   + " remote=" + event->remote_addr;
        });
        if (event->event == zlink::monitor_event::connection_ready) {
            trace_client_server_lazy ("client-connection-ready", [&] {
                return "endpoint=" + _options.expected_server.advertised_endpoint
                       + " channel=" + _options.admission.channel_name + " client="
                       + routing_id_label (_options.admission.channel_name.empty ()
                                             ? std::vector<std::uint8_t>{}
...
raw_client_server_client_t::pump_one (mesh::service_liveness_registry_t::clock_t::time_point now)
{
    ...
    try {
        ...
        if (header.kind == protocol::command::admit || header.kind == protocol::command::update) {
            if (received->size () != 1) {
                co_return client_server_pump_result_t::protocol_error;
            }
  ...
  const std::shared_ptr<detail::backend::raw_dealer_port_t> &port)
{
    ...
    detail::backend::raw_message_t hello_message{
      protocol::encode_client_server_client_admission (protocol::command::hello, admission)};
  ...
  mesh::service_liveness_registry_t::clock_t::time_point now)
{
    ...
    if (admission) {
        progressed = true;
        const bool stale = admission->connection != current_connection
    ...
    for (const auto &probe : probes) {
        ...
        if (probe.second.connection != current_connection
            ...
            trace_client_server_lazy ("client-probe-stale-discard",
        ...
        try {
            const auto liveness = protocol::decode_liveness (completion.parts.front ());
                                     ...
                                     std::chrono::milliseconds timeout)
{
    ...
    const auto protocol_failure = [] (std::string message) {
        client_server_request_completion_t failure{foundation::operation_terminal_t::completed};
    ...
    if (parts.size () != 2) {
        co_return protocol_failure ("ClientServer reply has an invalid part count");
    }
    const auto reply_header =
      messaging::envelope_codec_t{}.decode_header (zlink::message_t::from (parts.front ()), false);
    if (!reply_header) {
        co_return protocol_failure (reply_header.error () != nullptr
    ...
    if (reply.kind == messaging::message_kind_t::error) {
        ...
        failure.error_message = reply.error_message.value_or ("ClientServer request failed");
        co_return failure;
    }
    if (reply.kind != messaging::message_kind_t::response
        || reply.correlation_id != correlation_id) {
        co_return protocol_failure ("ClientServer reply kind or correlation does not match");
    }
    client_server_request_completion_t success{foundation::operation_terminal_t::completed};
