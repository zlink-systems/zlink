...
cleanup_source_created_reservation (const std::shared_ptr<location_repository_t> &store,
                                    ...
                                    bool source_created) noexcept
{
    ...
    try {
        const auto result = store->abort ({key, fence}).result ().value ();
        if (std::holds_alternative<object_aborted_t> (result))
            return source_creation_cleanup_t::aborted;
        if (std::holds_alternative<object_already_aborted_t> (result))
