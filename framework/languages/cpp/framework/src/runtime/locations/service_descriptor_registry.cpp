...
service_descriptor_publish_status_t
service_descriptor_registry_t::publish (service_descriptor_record_t record,
      ...
      _lane
        .run ([&] {
            ...
            if (found == _records.end ()) {
                ...
                ++_change_stamp;
                const auto [inserted, accepted] = _records.emplace (record.key, std::move (record));
                static_cast<void> (accepted);
                event = {service_descriptor_change_t::upserted, inserted->second, _change_stamp};
            } else {
                if (!expected_revision || *expected_revision != found->second.descriptor_revision
                    || record.descriptor_revision <= found->second.descriptor_revision
                ...
                event = {service_descriptor_change_t::upserted, found->second, _change_stamp};
            }
...
bool service_descriptor_registry_t::remove (const service_descriptor_key_t &key,
      ...
      _lane
        .run ([&] {
            ...
            if (found == _records.end () || found->second.descriptor_revision != expected_revision
                ...
                || found->second.owner_lease_generation != expected_owner_lease_generation
                || _change_stamp == std::numeric_limits<std::uint64_t>::max ()) {
                return false;
            }
            ++_change_stamp;
            event = {service_descriptor_change_t::removed, found->second, _change_stamp};
...
std::uint64_t service_descriptor_registry_t::watch (service_descriptor_watch_filter_t filter,
                                                    watch_callback_t callback)
{
    ...
    return _lane
      .run ([&] {
          ...
          _watchers.emplace (id, watcher_t{std::move (filter), std::move (callback)});
          return id;
      })
      .get ();
}
...
bool service_descriptor_registry_t::valid (const service_descriptor_record_t &record)
{
    return !record.key.scope.empty () && !record.key.routing_id.empty ()
           && record.lifecycle_generation != 0 && record.descriptor_revision != 0
           && !record.endpoint.empty () && !record.security_identity.empty ()
           && record.effective_max_message_bytes != 0 && record.weight >= 0
           && record.weight <= 10000 && !record.owner_id.empty ()
           && record.owner_lease_generation > 0;
}

bool service_descriptor_registry_t::immutable_identity_matches (
  const service_descriptor_record_t &current, const service_descriptor_record_t &candidate)
{
    return current.key == candidate.key
           && current.lifecycle_generation == candidate.lifecycle_generation
           && current.endpoint == candidate.endpoint
           && current.security_identity == candidate.security_identity
           && current.effective_max_message_bytes == candidate.effective_max_message_bytes
           && current.owner_id == candidate.owner_id
           && current.owner_lease_generation == candidate.owner_lease_generation;
}

bool service_descriptor_registry_t::matches (const service_descriptor_watch_filter_t &filter,
...
void service_descriptor_registry_t::notify (std::vector<watch_callback_t> callbacks,
