...
mesh::service_node_state_t service_state (framework_runtime_state_t state)
{
    switch (state) {
        ...
        default:
            throw std::invalid_argument ("invalid fanout runtime state");
    }
}
...
void fanout_location_runtime_t::start_publisher (const channel_snapshot_t &channel,
                                                 const std::optional<location_owner_token_t> &owner)
{
    ...
    if (owner) {
        const auto stored =
          _store->update_fanout_publisher (descriptor, location_write_intent_t::new_claim)
            .result ()
            .value ();
        if (stored.status != location_write_status_t::stored) {
            raw->close ();
            throw std::runtime_error ("fanout publisher descriptor publication was fenced");
        }
    }
    ...
    if (_listener_statuses)
        _listener_statuses->update (listener_kind_t::fanout, channel.name,
                                    entry->descriptor.endpoint);
    _publishers.emplace (channel.name, std::move (entry));
...
void fanout_location_runtime_t::wait_for_activity (std::chrono::milliseconds timeout) noexcept
{
...
void fanout_location_runtime_t::publish_descriptors ()
{
    ...
    std::vector<pending_descriptor_t> pending;
    {
        ...
        for (const auto &[channel_name, publisher] : _publishers) {
            const bool new_owner =
              ...
              || publisher->descriptor.lease_generation != owner->lease_generation;
            if (!new_owner && publisher->descriptor.state == state)
                continue;
            if (publisher->descriptor.descriptor_revision
                == static_cast<std::uint64_t> (std::numeric_limits<std::int64_t>::max ()))
                throw std::overflow_error ("fanout publisher descriptor revision is exhausted");
            ...
            pending.push_back (pending_descriptor_t{channel_name, std::move (descriptor),
    ...
    for (auto &item : pending) {
        const auto written =
          _store->update_fanout_publisher (item.descriptor, item.intent).result ().value ();
...
void fanout_location_runtime_t::reconcile_subscriber (subscriber_entry_t &subscriber)
{
    std::vector<fanout_publisher_intent_t> desired;
    location_page_request_t page;
    do {
        const auto listed =
          _store->list_fanout_publishers (subscriber.channel_name, page).result ().value ();
        for (const auto &descriptor : listed.items) {
            const auto live = owner_is_live (descriptor);
            desired.push_back (fanout_publisher_intent_t{
              descriptor.publisher_rid.to_bytes (), descriptor.lifecycle_generation,
              descriptor.endpoint,
              live ? service_state (descriptor.state) : mesh::service_node_state_t::stopped});
        }
...
fanout_channel_snapshot_t fanout_location_runtime_t::snapshot (std::string channel_name) const
{
...
bool fanout_location_runtime_t::is_observable_channel (std::string_view channel_name) const noexcept
{
    return std::any_of (_channels.begin (), _channels.end (), [channel_name] (const auto &channel) {
        return channel.name == channel_name
               && ((channel.publisher.enabled && channel.publisher.discovery)
                   || (channel.subscriber.enabled && channel.subscriber.discovery));
    });
}

fanout_channel_snapshot_t
fanout_location_runtime_t::build_snapshot_locked (const std::string &channel_name) const
{
...
void fanout_location_runtime_t::pump ()
{
    ...
    for (std::size_t offset = 0; offset < subscribers.size (); ++offset) {
        ...
        while (budget.can_receive ()) {
            ...
            auto [status, received] = subscriber.owner->try_receive (now);
            if (status == fanout_receive_status_t::no_data)
                break;
            budget.account (received ? received->payload.payload_bytes ().size () : 0);
            if (status != fanout_receive_status_t::application || !received) {
                if (budget.exhausted ())
                    break;
                continue;
            }
            try {
                ...
                const auto message = zlink::message_t::from (received->payload.payload_bytes ());
                detail::inbound_message_context_t inbound;
                inbound.before_application_handler = [permit = application_permit] () mutable {
                    if (!permit)
                        return;
                    permit->release_for_handler_entry ();
                    permit.reset ();
                };
                inbound.message.channel_name = subscriber.channel_name;
...
task_t<void> fanout_location_runtime_t::publish (const std::string &channel_name,
                                                 std::string topic,
                                                 ...
                                                 std::chrono::milliseconds timeout)
{
    ...
    std::shared_ptr<raw_fanout_publisher_t> publisher;
    {
        ...
        if (found == _publishers.end ()
            || found->second->descriptor.state != framework_runtime_state_t::serving)
    ...
    for (auto &[_, publisher] : publishers) {
        ...
        try {
            ...
            if (draining.descriptor_revision
                ...
                const auto written =
                  _store->update_fanout_publisher (draining, location_write_intent_t::renew)
                    .result ()
                    .value ();
                if (written.status == location_write_status_t::stored)
...
std::uint64_t fanout_location_runtime_t::make_lifecycle_generation ()
{
