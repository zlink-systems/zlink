...
struct mesh_node_runtime_t::message_follow_subscription_state_t
{
    ...
    {
        std::function<void (const runtime::protocol::message_follow_notice_t &)> retired_handler;
        {
            std::lock_guard lock (mutex);
            assert (in_flight != 0);
            --in_flight;
    ...
    void deactivate_and_wait () noexcept
    {
        ...
        for (auto *frame = current_dispatch; frame != nullptr; frame = frame->previous) {
            if (frame->state == this)
                                        ...
                                        serializer_registry_t *serializers)
{
    ...
    return result_t<std::optional<runtime::protocol::application_payload_t>>::success (
      runtime::protocol::application_payload_t{
        ...
        std::string (runtime::protocol::framework_multipart_content_type), std::move (multipart)});
}
...
bool same_bound_session_relocation_identity (const bound_session_relocation_route_t &left,
                                             const bound_session_relocation_route_t &right) noexcept
{
    return left.session_owner_node == right.session_owner_node
           && left.session_owner_node_generation == right.session_owner_node_generation
           && left.session_owner.owner_id == right.session_owner.owner_id
           && left.session_owner.lease_generation == right.session_owner.lease_generation
           && left.session == right.session && left.binding_generation == right.binding_generation;
}
...
bool framework_owned_node_message (const std::vector<zlink::message_t> &parts)
{
    try {
        ...
        const auto header = runtime::messaging::envelope_codec_t{}.decode_header (encoded, false);
...
class actor_message_follow_lease_t
{
  public:
    actor_message_follow_lease_t (spot_node_runtime_t runtime,
        ...
        _source_fence (std::move (source_fence)),
                       ...
                       serializer_registry_t *serializers)
{
    ...
    try {
        ...
        result.receive_chunk_limit_bytes =
          static_cast<std::uint32_t> (detail::spot_actor_join_advertised_receive_chunk_limit_bytes);
...
void mesh_node_runtime_t::start ()
{
    ...
    node->set_flow_capture_provider ([spot_state = _state->spot_state] {
    ...
    node->configure_actor_join_relocation (
      [spot_state = _state->spot_state] (const runtime::protocol::relocation_prepare_t &prepare) {
          return spot_node_runtime_t (spot_state).validate_actor_join_relocation_prepare (prepare);
      },
      [spot_state = _state->spot_state] (runtime::stateful::frozen_object_state_t &frozen,
      ...
      [spot_state = _state->spot_state] (const runtime::stateful::object_ref_t &target) {
          return spot_node_runtime_t (spot_state).actor_join_relocation_authority_spot (target);
      },
      [spot_state = _state->spot_state] (const std::string &stable_type,
                                         const runtime::stateful::object_ref_t &target) {
      ...
      [spot_state = _state->spot_state] (const runtime::stateful::object_ref_t &target,
          ...
          return spot_node_runtime_t (spot_state)
            .adopt_committed_actor_relocation_authority (target, target_node_generation,
    ...
    if (_relocation_authority && _relocation_store)
        node->configure_relocation (_relocation_authority, _relocation_store,
                                    _aggregate_relocation_authority, _relocation_limits);
    node->start ();
    if (_instance_spot_materializer)
  ...
  runtime::stateful::relocation_limits_t relocation_limits)
{
    ...
    if (relocation_limits.payload_chunk_limit_bytes == 0
        || relocation_limits.payload_chunk_limit_bytes > runtime::protocol::relocationChunkBytes
        || relocation_limits.cutover_wait_timeout <= std::chrono::milliseconds::zero ())
        throw configuration_error ("Relocation transfer options are out of range");
...
{
    if (!resolver)
        throw configuration_error ("Bound Session relocation resolver is required");
    if (_node)
        throw configuration_error (
  ...
  std::chrono::milliseconds timeout)
{
    ...
    for (const auto &[source, authority] : participants) {
        ...
        if (session->session_owner_node.to_bytes ().empty ()
            ...
            || session->session_owner.lease_generation <= 0 || session->session.to_bytes ().empty ()
        ...
        const runtime::protocol::session_relocation_seal_t seal{
          ...
          {source.key, source.object_generation, coordinator.node_routing_id,
           coordinator.node_generation, source.authority_owner_generation,
          ...
          static_cast<std::uint64_t> (session->session_owner.lease_generation),
        ...
        try {
            submitted = co_await _node->seal_session_remote (
              ...
              [seal] {
                  ...
                  return runtime::protocol::encode_session_relocation_seal (seal);
              },
              [completion] (operation_terminal_t terminal,
                            std::optional<host::session_relocation_seal_result_t> result) {
                  completion->complete (
                    result_t<completion_t>::success ({terminal, std::move (result)}));
              });
        }
        catch (...) {
            submitted = false;
        ...
        if (completed.terminal != operation_terminal_t::completed || !completed.result) {
...
runtime::protocol::session_relocation_route_t mesh_node_runtime_t::make_session_relocation_route (
  ...
  runtime::protocol::session_relocation_route_action_t action) const
{
    ...
    return runtime::protocol::session_relocation_route_t{
      ...
      static_cast<std::uint64_t> (checkpoint.session.session_owner.lease_generation),
      checkpoint.session.session.to_bytes (),
      checkpoint.session.binding_generation,
      {action, commit ? checkpoint.source.authority_owner_generation : 0,
                                                 ...
                                                 const authority_snapshot_t &authority)
{
    ...
    runtime::stateful::eligible_relocation_unit_t::canonical_wire_context_t wire{
      ...
      .prepare_target =
        [this, target, source_status = status, source = *source,
         ...
         relocation, coordinator] (
          ...
          const std::vector<runtime::protocol::session_relocation_route_t> &session_routes)
      ...
      .send_state_chunk =
        [this, target] (const runtime::protocol::relocation_state_t &chunk) -> task_t<bool> {
          co_return co_await _node->transport ().send_relocation_control (target.rid.to_bytes (),
                                                                          chunk);
      },
      .send_relocation_data =
        [this, target] (const std::vector<runtime::protocol::relocation_data_t> &records,
                        const runtime::stateful::relocation_ingress_batch_t &) -> task_t<bool> {
          for (const auto &record : records) {
              if (!co_await _node->transport ().send_relocation_control (target.rid.to_bytes (),
                                                                         record))
                  co_return false;
          }
          co_return true;
      ...
      .send_cutover = [this, target] (const runtime::protocol::relocation_cutover_t &cutover)
        -> task_t<runtime::stateful::eligible_relocation_unit_t::canonical_wire_context_t::
                    cutover_enqueue_t> {
          using context_t = runtime::stateful::eligible_relocation_unit_t::canonical_wire_context_t;
  ...
  const std::vector<authority_snapshot_t> &authorities)
{
    ...
    if (!_node || sources.empty () || sources.size () != stable_types.size ()
        || sources.size () != authorities.size () || target.lifecycle_generation == 0
    ...
    for (std::size_t index = 0; index != sources.size (); ++index) {
        input.push_back (
          {std::move (sources[index]), std::move (stable_types[index]), authorities[index]});
    }
    ...
    std::vector<std::pair<object_ref_t, authority_snapshot_t>> session_participants;
    session_participants.reserve (input.size ());
    for (const auto &participant : input)
        session_participants.emplace_back (participant.source, participant.authority);
    auto session_seal = std::make_shared<session_relocation_seal_outcome_t> ();
    auto session_checkpoint_attempted = std::make_shared<bool> (false);
    ...
    const auto receive_chunk_limit_bytes = negotiated_receive_chunk_limit_bytes (sources);
    if (sources.size () == 1) {
        auto result = co_await maintenance->relocate (
    ...
    auto result = co_await maintenance->relocate_aggregate (
      ...
      256u * 1024u * 1024u, digest, wire, {}, receive_chunk_limit_bytes);
    if (*session_checkpoint_attempted && !session_seal->completed) {
        result.terminal = session_seal->recovery_required ? relocation_terminal_t::recovery_required
                                                          : relocation_terminal_t::blocked;
        result.reason = runtime::stateful::relocation_reason_t::bound_session_fence_incomplete;
    }
    if (result.terminal == relocation_terminal_t::blocked
  ...
  const runtime::protocol::message_follow_notice_t &notice)
{
    ...
    for (const auto &subscription : subscriptions) {
        ...
        struct dispatch_guard_t
        {
            ...
            explicit dispatch_guard_t (message_follow_subscription_state_t *value) :
                state (value), frame{value, message_follow_subscription_state_t::current_dispatch}
            {
...
task_t<zlink::submit_result_t> mesh_node_runtime_t::send_to_actor (
    ...
    bound_session_source)
{
    ...
    co_return co_await _node->send_to_actor (target, parts, metadata, authority_owner_generation,
    ...
    bound_session_source)
{
    ...
    co_return co_await _node->request_to_actor (target, parts, operation_id, timeout, metadata,
...
mesh_node_runtime_t::request_actor_join_spot_route (const runtime::spot_address_t &target,
                                                    ...
                                                    std::chrono::milliseconds timeout)
{
    ...
    if (completed.record.terminal_result != static_cast<int> (zlink::request_result_t::ok)) {
...
struct mesh_node_runtime_t::remote_actor_join_state_t
{
    ...
    std::uint64_t committed_actor_authority_owner_generation = 0;
  ...
  std::string application_content_type)
{
    ...
    if (remote == false) {
        /* A canonical remote Join materializes the Actor in the target public
         * host without going through create_application_actor(), so the
         * create-time _actors convenience map is not authoritative here.
         * Rebind a handle to the current host object; this also keeps a later
         * same-node User-Spot Join on the ordinary local membership lane. */
        const auto current = _node ? _node->resolve_actor (actor) : std::nullopt;
        if (!current) {
    ...
    auto state = std::make_shared<remote_actor_join_state_t> (remote_actor_join_state_t{
      std::move (actor), target, request, timeout, std::move (bound_session_node_rid),
      std::move (bound_session_rid), std::move (application_packet_name),
    ...
    if (const auto observed =
        ...
        co_return co_await admit_remote_application_actor_join_via_wire (std::move (state),
                                                                         *observed);
    }
  ...
  std::shared_ptr<remote_actor_join_state_t> s)
{
    ...
    s->target_completion_operation_id_high = s->completion_operation_id_high;
    s->target_completion_operation_id_low = s->completion_operation_id_low;
    ...
    const auto request = spot_actor_admission_route_request_t{
      s->transfer_id,
      std::string (s->actor.node_rid ().value ()),
      std::string (::zlink::framework::detail::actor_ref_access_t::actor_type (s->actor)),
      std::string (s->actor.actor_id ().value ()),
      s->actor.object_generation (),
      source_status.lifecycle_generation (),
    ...
    auto parts =
      co_await request_actor_join_spot_route (s->target, std::move (encoded), s->timeout);
...
task_t<bool>
mesh_node_runtime_t::abort_remote_actor_join_seal (std::shared_ptr<remote_actor_join_state_t> s)
{
...
void mesh_node_runtime_t::observe_spot_authority (const zlink::routing_id_t &target_node_rid,
                                                  const std::string &target_spot_id,
                                                  std::uint64_t object_generation,
                                                  std::uint64_t target_node_generation,
                                                  std::uint64_t authority_owner_generation,
  ...
  const std::vector<runtime::stateful::object_ref_t> &sources) const
{
    ...
    for (const auto &source : sources) {
        ...
        if (found != _negotiated_receive_chunk_limits.end () && found->second != 0) {
            negotiated =
              negotiated == 0 ? found->second : std::min<std::uint64_t> (negotiated, found->second);
        }
    }
    return negotiated;
}
  ...
  std::shared_ptr<remote_actor_join_state_t> s, observed_spot_authority_t observed)
{
    ...
    s->source_actor = *source;
    s->actor_authority_owner_generation = source->authority_owner_generation;
    s->target_spot_authority_owner_generation = observed.authority_owner_generation;
    s->deadline = std::chrono::steady_clock::now () + s->timeout;
    ...
    if (tail.join_result == runtime::protocol::actor_join_result_t::accepted
        ...
        record_negotiated_receive_chunk_limit (s->actor, tail.receive_chunk_limit_bytes);
    }
    ...
    s->admission_reply_content_type = outcome.application_reply
  ...
  std::shared_ptr<remote_actor_join_state_t> s)
{
    ...
    try {
        ...
        do {
            const auto listed =
              co_await _user_spot_store->list_mesh_nodes (s->target.mesh_name, page);
            const auto descriptor = std::find_if (listed.items.begin (), listed.items.end (),
  ...
  std::shared_ptr<remote_actor_join_state_t> s)
{
    ...
    const auto prepared = spot.transfer_actor_out (s->actor, s->transfer_id, false);
    if (!prepared) {
        const auto failed =
    ...
    runtime::protocol::actor_join_recovery_t recovery{
      ...
      .request_content_type = s->application_content_type.empty () ? "application/octet-stream"
      ...
      .target_authority_owner_generation = s->actor_authority_owner_generation + 1,
      .target_spot_authority_owner_generation = s->target_spot_authority_owner_generation,
      .coordinator = s->relocation_coordinator,
      .operation = {s->target_completion_operation_id_high, s->target_completion_operation_id_low},
    ...
    try {
        recovery_frozen = runtime::protocol::encode_actor_join_recovery_saved_work (recovery);
    }
    catch (const std::exception &error) {
    ...
    if (!s->encoded_session_relocation_route.empty ()) {
        try {
            session_routes.push_back (runtime::protocol::decode_session_relocation_route (
              s->encoded_session_relocation_route));
        }
        catch (...) {
    ...
    const runtime::protocol::relocation_object_t object{
      runtime::protocol::relocation_object_kind_t::actor, recovery.actor_type, recovery.actor_id,
      recovery.actor_generation, recovery.actor_authority_owner_generation};
    runtime::stateful::eligible_relocation_unit_t::canonical_wire_context_t wire{
      .relocation = s->relocation,
      ...
      .augment_frozen =
        [recovery,
         recovery_frozen] (std::vector<runtime::stateful::frozen_object_state_t> &participants) {
            const auto actor = std::find_if (
              participants.begin (), participants.end (), [&recovery] (const auto &candidate) {
                  return candidate.owner.kind == runtime::stateful::object_kind_t::actor
                         && candidate.owner.key == recovery.actor_id
                         && candidate.owner.object_generation == recovery.actor_generation
            ...
            actor->pending_application.push_back (
              runtime::stateful::turn_record_t{sequence, recovery_frozen.canonical_bytes,
                                               std::nullopt, std::nullopt, recovery_frozen});
            return true;
        },
      .prepare_target =
        [this, s, object] (const std::vector<runtime::stateful::frozen_object_state_t> &,
                           const runtime::stateful::relocation_payload_manifest_t &manifest,
                           const std::vector<runtime::protocol::session_relocation_route_t> &routes)
        -> task_t<runtime::stateful::relocation_reason_t> {
          co_return co_await _node->prepare_relocation_remote (
            s->target.node_rid,
            runtime::protocol::relocation_prepare_t{
              ...
              {s->target.node_rid.to_bytes (), s->target.node_generation, s->target.owner.owner_id,
               static_cast<std::uint64_t> (s->target.owner.lease_generation)},
              runtime::protocol::relocation_role_t::source,
              object,
              s->source_node_rid,
              s->source_node_generation,
              manifest.total_length,
              manifest.chunk_count,
              manifest.checksum_crc32c,
              s->target_application_version},
      ...
      .send_state_chunk = [this,
                           s] (const runtime::protocol::relocation_state_t &chunk) -> task_t<bool> {
          co_return co_await _node->transport ().send_relocation_control (
            s->target.node_rid.to_bytes (), chunk);
      },
      .send_relocation_data =
        [this, s] (const std::vector<runtime::protocol::relocation_data_t> &records,
                   const runtime::stateful::relocation_ingress_batch_t &) -> task_t<bool> {
          for (const auto &record : records) {
      ...
      .send_cutover = [this, s] (const runtime::protocol::relocation_cutover_t &cutover)
        -> task_t<runtime::stateful::eligible_relocation_unit_t::canonical_wire_context_t::
                    cutover_enqueue_t> {
          using context_t = runtime::stateful::eligible_relocation_unit_t::canonical_wire_context_t;
          co_return co_await _node->cutover_relocation_remote (s->target.node_rid, cutover)
            ? context_t::cutover_enqueue_t::enqueued
            : context_t::cutover_enqueue_t::not_enqueued;
      },
      .abort_target_before_cutover = [] { return true; }};
    ...
    const auto relocated = co_await maintenance->relocate (
      ...
      {s->target.owner.owner_id, s->target.owner.lease_generation}, 256u * 1024u * 1024u,
  ...
  std::shared_ptr<remote_actor_join_state_t> s)
{
    ...
    const auto joined = actor_join_reply_t{
      ...
      zlink::message_t{}};
    const auto source =
      runtime::protocol::actor_route_fence_t{std::string (s->actor.actor_id ().value ()),
                                             s->actor.object_generation (),
                                             s->source_node_rid,
                                             s->source_node_generation,
                                             s->actor_authority_owner_generation,
                                                       ...
                                                       const actor_ref_t &actor)
{
    if (record.terminal_result != 0) {
        ...
        const auto failure = failure_mapper.reply_header_exception (
          ...
          static_cast<std::uint32_t> (record.failure_errno), "Local Actor join");
        return result_t<actor_join_reply_t>::failure (failure.kind (), failure.what ());
    }
  ...
  std::optional<bound_session_relay_source_t> bound_session_source)
{
    try {
        ...
        const bool has_exact_stale_route = stale_route.owner_lease_generation != 0;
        ...
        if (acquired_follow.value ()) {
            ...
            actor_message_follow_lease_t lease (spot_runtime, actor, follow_target.source_fence,
                                                payload_bytes);
            auto follow_path = advance_message_follow_path (
              header.metadata, local_node, follow_target.source_fence.authority_owner_generation);
            if (!follow_path) {
                co_return detail::propagate_failure<std::optional<zlink::message_t>> (
            ...
            target_identity +=
              std::to_string (follow_target.target_fence.authority_owner_generation);
            ...
            if (completed.record.terminal_result
                ...
                co_return result_t<std::optional<zlink::message_t>>::failure (failure.kind (),
                                                                              failure.what ());
            }
            ...
            if ((original_operation.high != 0 || original_operation.low != 0)
                ...
                && spot_runtime.try_begin_actor_message_follow_notification (
                  actor, follow_target.source_fence, follow_target.target_fence)) {
                ...
                (void) spot_runtime.complete_actor_message_follow_notification (
                  actor, follow_target.source_fence, follow_target.target_fence, accepted);
            }
        ...
        const auto wire_bound_session_source = [&] {
            ...
            return std::make_optional (
              runtime::protocol::actor_message_header_t::bound_session_source_t{
                ...
                bound_session_source->binding_generation, bound_session_source->session_sequence});
        }();
        ...
        auto reply_header = runtime::messaging::envelope_codec_t{}.decode_header (reply, false);
        if (!reply_header) {
            co_return result_t<std::optional<zlink::message_t>>::failure (
  ...
  const actor_ref_t &actor, const runtime::spot_address_t &stale_route) const
{
    if (_actor_route_invalidator) {
        _actor_route_invalidator (runtime::protocol::actor_route_fence_t{
          std::string (actor.actor_id ().value ()), stale_route.object_generation,
          stale_route.node_rid.to_bytes (), stale_route.node_generation,
  ...
  std::chrono::milliseconds timeout) const
{
    if (timeout <= std::chrono::milliseconds::zero ())
        co_return std::nullopt;
    const auto changed = [&stale_route] (const runtime::spot_address_t &candidate) {
        return candidate.node_rid != stale_route.node_rid
               || candidate.node_generation != stale_route.node_generation
               || candidate.object_generation != stale_route.object_generation
               || candidate.authority_owner_generation != stale_route.authority_owner_generation
               || candidate.owner.lease_generation != stale_route.owner.lease_generation;
    };
    const auto deadline = std::chrono::steady_clock::now () + timeout;
    do {
    ...
    try {
        ...
        const auto submitted = co_await _node->transport ().request_bound_session_bind (
          ...
          [completion] (runtime::foundation::operation_terminal_t terminal,
              ...
              if (terminal != runtime::foundation::operation_terminal_t::completed) {
                  const auto kind = terminal == runtime::foundation::operation_terminal_t::timed_out
                                      ? framework_error_kind_t::deadline_exceeded
                                      : framework_error_kind_t::unavailable;
                  completion->complete (result_t<runtime::protocol::reply_header_t>::failure (
                    kind, "Remote Actor session binding did not complete successfully"));
                  return;
              }
              try {
        ...
        const auto stale_route =
          reply.terminal_result
            == static_cast<std::uint32_t> (runtime::protocol::request_terminal_result::conflict)
          && reply.failure_code
        ...
        const auto actor_not_ready =
          reply.terminal_result
            == static_cast<std::uint32_t> (runtime::protocol::request_terminal_result::busy)
        ...
        co_return result_t<application_actor_session_bind_outcome_t>::failure (
          framework_error_kind_t::unavailable, "Remote Actor session binding was rejected");
    }
    catch (const framework_exception_t &error) {
        co_return detail::result_access_t::failure<application_actor_session_bind_outcome_t> (
          error);
    }
    catch (const std::exception &error) {
        co_return result_t<application_actor_session_bind_outcome_t>::failure (
...
task_t<void>
mesh_node_runtime_t::retire_application_actor_session (runtime::stateful::stream_binding_t binding,
                                                       ...
                                                       std::chrono::milliseconds timeout)
{
    if (binding.binding_generation == 0
        ...
        || binding.actor.authority_owner_generation == 0 || binding.actor.node_id.empty ()
        || binding.target_node_generation == 0 || binding.owner_lease_generation == 0
    ...
    const auto submitted = co_await _node->transport ().request_bound_session_bind (
      ...
      runtime::protocol::bound_session_bind_t{
        0,
        runtime::protocol::actor_route_fence_t{
          binding.actor.key, binding.actor.object_generation, actor_owner.to_bytes (),
          binding.target_node_generation, binding.actor.authority_owner_generation,
          binding.owner_lease_generation},
        session_rid.to_bytes (),
        {runtime::protocol::bound_session_binding_state_t::tombstone, binding.binding_generation}},
      timeout,
      [completion] (runtime::foundation::operation_terminal_t terminal,
                    std::vector<std::uint8_t> payload) {
          if (terminal != runtime::foundation::operation_terminal_t::completed) {
                                   ...
                                   const std::map<std::string, std::string> &metadata)
{
    ...
    co_return co_await send_to_node (target, parts, std::vector<std::uint8_t> (encoded));
}
                                      ...
                                      std::vector<std::uint8_t> metadata)
{
    ...
    (void) metadata;
    co_return co_await _node->request_to_node (target, parts, operation_id, timeout);
}
...
{
    const auto encoded = mesh_metadata_codec_t::encode (metadata);
    co_return co_await request_to_node (target, parts, operation_id, timeout,
...
{
    const auto encoded = mesh_metadata_codec_t::encode (metadata);
    co_return co_await send_to_channel (channel_name, parts, std::vector<std::uint8_t> (encoded));
}
                                         ...
                                         std::vector<std::uint8_t> metadata)
{
    ...
    (void) metadata;
    co_return co_await _node->request_to_channel (channel_name, parts, operation_id, timeout);
}
...
{
    const auto encoded = mesh_metadata_codec_t::encode (metadata);
    co_return co_await request_to_channel (channel_name, parts, operation_id, timeout,
                                           std::vector<std::uint8_t> (encoded));
}
