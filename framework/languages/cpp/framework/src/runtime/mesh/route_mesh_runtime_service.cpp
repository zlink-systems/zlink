...
{
    using observer_t = observation_detail::runtime_observer_state_t<mesh_node_snapshot_t>;
    ...
    for (const auto &[_, hub] : _state->hubs) {
        ...
        for (auto &peer : terminal->peers) {
            if (peer.state != peer_state_t::not_required) {
                ...
                peer.unavailable_reason = topology_reason_t::runtime_not_ready;
            }
        }
                ...
                std::string mesh_name)
{
    ...
    for (const auto &peer : peers) {
        ...
        if (location_is_healthy
            ...
            peer_snapshots.push_back (mesh_peer_snapshot_t{
              ...
              .unavailable_reason =
                draining ? std::optional<topology_reason_t>{topology_reason_t::draining}
                         : std::optional<topology_reason_t>{topology_reason_t::no_ready_peer}});
            continue;
        }
        if (location_is_healthy && location != location_by_rid.end ()) {
            for (const auto &[channel_name, weight] : location->second->channel_weights) {
                if (weight > 0)
        ...
        peer_snapshots.push_back (mesh_peer_snapshot_t{
          ...
          .state = peer.descriptor.state == mesh::service_node_state_t::draining
                     ...
                     : peer_state_t::ready,
          .unavailable_reason = peer.descriptor.state == mesh::service_node_state_t::draining
                                  ? std::optional<topology_reason_t>{topology_reason_t::draining}
                                  : std::nullopt});
    }
    ...
    for (const auto &remote : location_descriptors) {
        if (remote.rid == status.routing_id ()
            ...
            continue;
        const auto not_required = mesh::route_mesh_connection_not_required (
          local_role, !local_channels.empty (), remote.object_role,
        ...
        peer_snapshots.push_back (mesh_peer_snapshot_t{
          .node_rid = remote.rid,
          .state = not_required ? peer_state_t::not_required
                   : remote.state == framework_runtime_state_t::draining
                     ? peer_state_t::draining
                     : peer_state_t::not_connected,
          .unavailable_reason = not_required ? std::nullopt
    ...
    for (const auto &name : hub->node->channel_names ()) {
        ...
        channels.push_back (
          mesh_channel_snapshot_t{.channel_name = name,
                                  .is_ready = ready != 0,
    ...
    return mesh_node_snapshot_t{
      ...
      .placement =
        mesh_placement_snapshot_t{
          ...
          .active_actor_count = static_cast<std::uint32_t> (placement.capacity.actors.active),
          .active_spot_count = static_cast<std::uint32_t> (placement.capacity.spots.active),
          .unavailable_reason =
            placement_available
              ? std::nullopt
...
std::unique_ptr<mesh_runtime_observation_t> route_mesh_runtime_service_t::observe (
  std::string mesh_name,
  std::size_t capacity,
  std::function<void (const observed_status_t<mesh_node_snapshot_t> &)> observer)
{
    if (capacity == 0)
    ...
    auto registered = std::make_shared<state_t::observer_t> (capacity, std::move (observer));
    registered->start ();
    {
        std::lock_guard lock (hub->mutex);
