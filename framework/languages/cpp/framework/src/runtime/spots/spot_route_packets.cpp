...
std::string encode_base64 (const std::vector<std::uint8_t> &bytes)
{
    ...
    for (std::size_t offset = 0; offset < bytes.size (); offset += 3) {
        ...
        encoded.push_back (base64_alphabet[((first & 0x03U) << 4) | (second >> 4)]);
        encoded.push_back (remaining > 1 ? base64_alphabet[((second & 0x0fU) << 2) | (third >> 6)]
                                         : '=');
...
{
    json = nlohmann::json{{"topic", value.topic}, {"frame", encode_base64 (value.frame)}};
}
...
void to_json (nlohmann::json &json, const spot_actor_admission_route_request_t &value)
{
    json = nlohmann::json{{"transferId", value.transfer_id},
                          ...
                          {"completionOperationIdHigh", value.completion_operation_id_high},
                          {"completionOperationIdLow", value.completion_operation_id_low},
...
void from_json (const nlohmann::json &json, spot_actor_admission_route_request_t &value)
{
    ...
    value.actor_node_generation = json.value ("actorNodeGeneration", std::uint64_t{0});
    ...
    value.completion_operation_id_high = json.value ("completionOperationIdHigh", std::uint64_t{0});
    value.completion_operation_id_low = json.value ("completionOperationIdLow", std::uint64_t{0});
...
{
    json =
      nlohmann::json{{"transferId", value.transfer_id},
                     {"actorNodeRid", value.actor_node_rid},
                     {"actorType", value.actor_type},
                     {"actorId", value.actor_id},
                     {"actorGeneration", value.actor_generation},
                     ...
                     {"targetNodeLifecycleGeneration", value.target_node_lifecycle_generation},
                     ...
                     {"sessionRelocationRoute", encode_base64 (value.session_relocation_route)},
...
void from_json (const nlohmann::json &json, spot_actor_commit_route_request_t &value)
{
    ...
    value.completion_root_checksum = json.value ("completionRootChecksum", std::uint32_t{0});
    value.target_spot_id = json.at ("targetSpotId").get<std::string> ();
    value.target_spot_generation = json.value ("targetSpotGeneration", std::uint64_t{0});
    ...
    value.source_spot_generation = json.value ("sourceSpotGeneration", std::uint64_t{0});
    value.bound_session_node_rid = json.value ("boundSessionNodeRid", "");
    value.bound_session_rid = json.value ("boundSessionRid", "");
    value.session_relocation_route = json.contains ("sessionRelocationRoute")
    ...
    value.finalize_timeout_ms = json.value ("finalizeTimeoutMs", std::uint64_t{0});
...
void from_json (const nlohmann::json &json, spot_actor_leave_route_command_t &value)
{
    ...
    value.actor_generation = json.at ("actorGeneration").get<std::uint64_t> ();
    value.source_spot_id = json.at ("sourceSpotId").get<std::string> ();
    value.source_spot_generation = json.at ("sourceSpotGeneration").get<std::uint64_t> ();
    value.target_spot_id = json.at ("targetSpotId").get<std::string> ();
    value.target_node_rid = json.at ("targetNodeRid").get<std::string> ();
    value.target_node_generation = json.at ("targetNodeGeneration").get<std::uint64_t> ();
...
void from_json (const nlohmann::json &json, spot_actor_join_route_reply_t &value)
{
    ...
    value.actor_generation = json.at ("actorGeneration").get<std::uint64_t> ();
    value.payload = decode_base64_field (json, "payload");
    value.receive_chunk_limit_bytes = json.value ("receiveChunkLimitBytes", std::uint64_t{0});
}
...
void to_json (nlohmann::json &json, const spot_actor_packet_route_request_t &value)
{
    json = nlohmann::json{{"actorNodeRid", value.actor_node_rid},
                          ...
                          {"actorNodeGeneration", value.actor_node_generation},
                          {"actorAuthorityOwnerGeneration", value.actor_authority_owner_generation},
                          {"actorOwnerLeaseGeneration", value.actor_owner_lease_generation},
...
void from_json (const nlohmann::json &json, spot_actor_packet_route_request_t &value)
{
    ...
    value.actor_authority_owner_generation =
      json.value ("actorAuthorityOwnerGeneration", std::uint64_t{0});
    value.actor_owner_lease_generation = json.value ("actorOwnerLeaseGeneration", std::uint64_t{0});
    value.spot_id = json.at ("spotId").get<std::string> ();
    value.packet_name_value = json.at ("packetName").get<std::string> ();
    value.content_type = json.value ("contentType", "application/json");
    value.message_follow_hop_count = json.value ("messageFollowHopCount", std::uint8_t{0});
  ...
  std::optional<runtime::protocol::actor_route_fence_t> target_fence)
{
    ...
    if (const auto hop = metadata.find ("__zlink.messageFollowHopCount")) {
