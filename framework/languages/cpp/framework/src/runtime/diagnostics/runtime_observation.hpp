...
inline void increment_runtime_observation_loss (std::uint64_t &value) noexcept
{
...
{
  public:
    using callback_t = std::function<void (const observed_status_t<TStatus> &)>;

    runtime_observer_state_t (std::size_t terminal_capacity, callback_t callback) :
    ...
    void enqueue (std::string source_key, TStatus status, bool terminal = false) noexcept
    {
        bool schedule = false;
        try {
            {
                ...
                if (terminal)
                    retain_terminal_locked (std::move (source_key), std::move (status));
                else
                    retain_intermediate_locked (std::move (source_key), std::move (status));
  ...
  private:
    ...
    {
        return !_terminal_fifo.empty () || !_intermediate_by_source.empty ();
    }

    void retain_intermediate_locked (std::string source_key, TStatus status)
    {
        const auto terminal = _retained_terminal_count_by_source.find (source_key);
        if (terminal != _retained_terminal_count_by_source.end () && terminal->second != 0) {
        ...
        queued_status_t queued{source_key, std::move (status), false};
        _intermediate_by_source.emplace (std::move (source_key), std::move (queued));
    }

    void retain_terminal_locked (std::string source_key, TStatus status)
    {
        if (_intermediate_by_source.erase (source_key) != 0)
            increment_runtime_observation_loss (_loss.coalesced_count);

        if (_terminal_fifo.size () == _terminal_capacity) {
            release_terminal_source_locked (_terminal_fifo.front ().source_key);
            _terminal_fifo.pop_front ();
            increment_runtime_observation_loss (_loss.discarded_terminal_count);
        }

        ++_retained_terminal_count_by_source[source_key];
        _terminal_fifo.push_back (
          queued_status_t{std::move (source_key), std::move (status), true});
    }

    void release_terminal_source_locked (const std::string &source_key)
    {
        const auto found = _retained_terminal_count_by_source.find (source_key);
    ...
    static void schedule_delivery (std::shared_ptr<runtime_observer_state_t> state) noexcept
    {
        if (schedule_runtime_observation_work ([state] { state->deliver_one (); }))
            return;
        state->fail_closed ();
    }
    ...
    void deliver_one () noexcept
    {
        ...
        std::optional<observed_status_t<TStatus>> delivered;
        {
            ...
            delivered.emplace (observed_status_t<TStatus>{std::move (next->status), _loss});
    ...
    std::unordered_map<std::string, std::size_t> _retained_terminal_count_by_source;
