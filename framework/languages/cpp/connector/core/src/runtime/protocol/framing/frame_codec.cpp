...
result_t<std::vector<std::uint8_t>> frame_codec_t::encode (const std::vector<std::uint8_t> &header,
                                                           ...
                                                           const connector_options_t &options)
{
    /* stream-connector 32 §4.7: the send payload bound is applied before the
     * transport write and reported as a validation error (§9). `FrameTooLarge`
     * names the receive bound only, so it must not be used here; the wire
     * ceilings below are a separate rule owned by encode_prefix. */
    if (payload.size () > options.max_send_payload_size) {
        return result_t<std::vector<std::uint8_t>>::failure (
          error_code_t::validation_failed, "Payload exceeds max_send_payload_size.");
    }
