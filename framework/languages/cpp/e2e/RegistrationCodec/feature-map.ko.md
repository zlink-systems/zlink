...
| `RC-B6` | 구현 | 별도 codec extension 없이 기본 JSON으로 camelCase property, signed 64-bit decimal string, padded Base64 bytes, 32-bit number, finite floating-point 값과 nullable field를 별도 typed handler process 사이에서 왕복하고 복원된 값을 확인한다. 실행 결과는 `scenario RC-B6 passed`로 남긴다. |
