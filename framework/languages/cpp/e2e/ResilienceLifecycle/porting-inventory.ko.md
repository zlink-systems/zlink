...
| `run_e2e.sh` | `run_e2e.sh` | runner | done | 전용 role target을 빌드하고 Consumer HTTP smoke, RL-A1/A2/A3/A4/A5, RL-B1/B2/B3/B4/B5/B6, RL-C1/C3/C4, RL-D1/D2/D3/D4/D5 slice를 실행한다. RL-A4는 green provider endpoint를 별도 process로 띄우고, RL-B1/B2/B4/B5/C1/C3/D1/D3/D4/D5는 Consumer HTTP endpoint와 provider evidence로 실행한다. RL-C4는 Redis location store outage 중 established manual channel 유지와 Redis 복구 뒤 new-client recovery를 검증한다. |
