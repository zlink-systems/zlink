...
std::string run_atd_b1_other_actor_progress_scenario (
  TConnector &connector,
  const await_actor_scenario_context_t &actors)
{
    const auto request_id = unique_id ("ATD-B1");
    ...
    auto actor_fast =
      connector.request (actor_fast_req_t{.request_id = request_id, .marker = "b1-fast"})
        ...
        .timeout (std::chrono::milliseconds (30000))
        .template submit<actor_await_res_t> ();
    ensure (static_cast<bool> (actor_fast), "ATD-B1 ActorFastReq failed");
    auto actor_await_reply = actor_yield.get ();
    ensure (static_cast<bool> (actor_await_reply), "ATD-B1 ActorYieldReq failed");
    auto evidence =
    ...
    ensure (static_cast<bool> (evidence), "ATD-B1 evidence request failed");
    ...
    std::cout << "scenario ATD-B1 passed\n";
    return request_id;
}
