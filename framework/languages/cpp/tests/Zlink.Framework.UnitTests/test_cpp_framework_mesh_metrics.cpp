...
std::vector<metric_sample_t> samples (metric_test::provider_t &provider)
{
    ...
    for (const auto &metric : provider.collect ()) {
        for (const auto &point : metric.point_data_attr_) {
            metric_sample_t sample{metric.instrument_descriptor.name_,
                                   metric.instrument_descriptor.unit_,
                                   {},
                                   labels (point.attributes),
                                   0,
                                   0};
            if (const auto *sum = opentelemetry::nostd::get_if<metric_test::sdk::SumPointData> (
                  &point.point_data)) {
                ...
                sample.value = opentelemetry::nostd::get<double> (sum->value_);
                         ...
                         opentelemetry::nostd::get_if<metric_test::sdk::LastValuePointData> (
                           &point.point_data)) {
                sample.kind = "gauge";
                sample.value = opentelemetry::nostd::get<double> (gauge->value_);
                ...
                sample.histogram_sum = opentelemetry::nostd::get<double> (histogram->sum_);
            } else {
                continue;
            }
...
std::optional<metric_sample_t> find_sample (metric_test::provider_t &provider,
                                            std::string_view name,
                                            const labels_t &expected_labels)
{
    const auto collected = samples (provider);
    const auto found =
      std::find_if (collected.begin (), collected.end (), [&] (const auto &sample) {
          return sample.name == name && sample.labels == expected_labels;
      });
    return found == collected.end () ? std::nullopt : std::make_optional (*found);
}

void require_positive_duration (metric_test::provider_t &provider, labels_t expected_labels)
{
    const auto sample = find_sample (provider, "zlink.mesh_node.request.duration", expected_labels);
    const auto identity =
      std::string ("zlink.mesh_node.request.duration ") + describe_labels (expected_labels);
    ...
    require (std::isfinite (sample->histogram_sum) && sample->histogram_sum > 0,
             "duration sum was not finite and positive for " + identity
               + ": actual=" + std::to_string (sample->histogram_sum));
}
                     ...
                     double expected_value)
{
    ...
    const auto identity = std::string (name) + " " + describe_labels (expected_labels);
    require (sample.has_value (), "missing metric " + identity);
    require (sample->kind == kind, "unexpected instrument kind for " + identity + ": expected="
                                     + std::string (kind) + " actual=" + sample->kind);
    require (sample->unit == unit, "unexpected instrument unit for " + identity + ": expected="
                                     + std::string (unit) + " actual=" + sample->unit);
    require (sample->value == expected_value, "unexpected metric value for " + identity
                                                + ": expected=" + std::to_string (expected_value)
                                                + " actual=" + std::to_string (sample->value));
}

mesh::service_node_descriptor_t
descriptor (std::string mesh_name,
...
bool pump_until (mesh::raw_mesh_node_owner_t &first,
...
void admit_pair (mesh::raw_mesh_node_owner_t &source, mesh::raw_mesh_node_owner_t &target)
{
...
void verify_topology_metrics_and_selection_reasons ()
{
    ...
    mesh::raw_mesh_node_owner_t target ({descriptor (
      std::string (mesh_name), "metrics-topology-target", {{"alpha", 100}, {"zero", 0}})});
    target.start ();
    ...
    const labels_t peer_labels{{"mesh_name", std::string (mesh_name)},
    ...
    require_metric (provider, "zlink.mesh_node.peers.connected", "gauge", "{peer}", peer_labels, 1);
    require_metric (provider, "zlink.mesh_node.peers.ready", "gauge", "{peer}", peer_labels, 0);
    ...
    require_metric (provider, "zlink.mesh_node.peers.configured", "gauge", "{peer}", peer_labels,
                    1);
    require_metric (provider, "zlink.mesh_node.peers.connected", "gauge", "{peer}", peer_labels, 1);
    require_metric (provider, "zlink.mesh_node.peers.ready", "gauge", "{peer}", peer_labels, 1);
    ...
    require_metric (
      ...
      1);
    require (!find_sample (provider, "zlink.mesh_node.requests.inflight",
                           {{"mesh_name", std::string (mesh_name)}, {"surface", "channel"}}),
             "rejected channel selection emitted an inflight request");
    require (
      !find_sample (
        provider, "zlink.mesh_node.request.duration",
        {{"mesh_name", std::string (mesh_name)}, {"outcome", "failed"}, {"surface", "channel"}}),
    ...
    require_metric (
      provider, "zlink.mesh_node.channel.selection_failures", "counter", "{failure}",
      {{"channel_name", "zero"}, {"mesh_name", std::string (mesh_name)}, {"reason", "not_ready"}},
      1);
    require (!find_sample (provider, "zlink.mesh_node.channel.selection_failures",
    ...
    require (
      pump_until (source, target,
                  [&] {
                      const auto peer = source.topology ().peer (target_descriptor.node_routing_id);
                      return peer && peer->descriptor.state == mesh::service_node_state_t::draining;
                  }),
    ...
    require_metric (provider, "zlink.mesh_node.peers.configured", "gauge", "{peer}", peer_labels,
                    1);
    source.forget_peer (target_descriptor.node_routing_id, target.endpoint ());
    require_metric (provider, "zlink.mesh_node.peers.configured", "gauge", "{peer}", peer_labels,
                    0);
}
...
{
    auto target_descriptor = descriptor ("metrics-not-required", "metrics-client-target");
    ...
    metric_test::provider_t provider;
    auto source_descriptor = descriptor ("metrics-not-required", "metrics-client-source");
    ...
    require (source.admit_peer (remote, bytes ("not-required-connection"),
    ...
    const labels_t peer_labels{{"mesh_name", "metrics-not-required"}, {"source", "manual"}};
    require_metric (provider, "zlink.mesh_node.peers.configured", "gauge", "{peer}", peer_labels,
                    1);
    require_metric (provider, "zlink.mesh_node.peers.connected", "gauge", "{peer}", peer_labels, 0);
    require_metric (provider, "zlink.mesh_node.peers.ready", "gauge", "{peer}", peer_labels, 0);
...
std::optional<mesh::service_mailbox_claim_t> claim_request (mesh::raw_mesh_node_owner_t &target,
...
void complete_remote_request (mesh::raw_mesh_node_owner_t &source,
                              ...
                              Reply reply)
{
    ...
    require (await_task (submit ([&promise] (foundation::operation_terminal_t terminal,
    ...
    require (claim && claim->records.size () == 1, surface + " request was not delivered");

    const labels_t inflight_labels{{"mesh_name", "metrics-requests"}, {"surface", surface}};
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    inflight_labels, 1);
    require (reply (claim->records.front ()), surface + " request reply failed");
    require (target.mailbox ().release (*claim), surface + " request mailbox release failed");
    ...
    require (completion.get ().first == foundation::operation_terminal_t::completed,
             surface + " request did not complete successfully");
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    inflight_labels, 0);
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-requests"}, {"outcome", "completed"}, {"surface", surface}}, 1);
    require_positive_duration (
      provider,
      {{"mesh_name", "metrics-requests"}, {"outcome", "completed"}, {"surface", surface}});
    require (!find_sample (provider, "zlink.mesh_node.request.timeouts",
...
{
    mesh::raw_mesh_node_owner_t target (
      {descriptor ("metrics-requests", "metrics-request-target", {{"work", 100}})});
    target.start ();
    ...
    const auto remote = target.topology ().local_descriptor ();
    const protocol::application_payload_t request_payload{"MetricRequest", "application/json",
                                                          bytes ("request")};
    const protocol::application_payload_t reply_payload{"MetricReply", "application/json",
                                                        bytes ("reply")};

    complete_remote_request (
      source, target, provider, "node", mesh::service_mailbox_domain_t::application,
      [&] (auto callback) {
          return source.request_to_node (remote.node_routing_id, request_payload, 2s,
                                         std::move (callback));
      },
      [&] (const auto &record) { return target.reply (record, reply_payload); });
    complete_remote_request (
      source, target, provider, "channel", mesh::service_mailbox_domain_t::application,
      [&] (auto callback) {
          return source.request_to_channel ("work", request_payload, 2s, std::move (callback));
      },
      [&] (const auto &record) { return target.reply (record, reply_payload); });

    const protocol::spot_route_fence_t spot{
      "spot-1", 1, remote.node_routing_id, remote.lifecycle_generation, 1, 1};
    complete_remote_request (
      source, target, provider, "spot", mesh::service_mailbox_domain_t::application,
      [&] (auto callback) {
          return source.request_to_spot (remote.node_routing_id, "source-spot", spot,
                                         request_payload, 2s, std::move (callback));
      },
      [&] (const auto &record) { return target.reply (record, reply_payload); });

    const protocol::actor_route_fence_t actor{
      "actor-1", 1, remote.node_routing_id, remote.lifecycle_generation, 1, 1};
    complete_remote_request (
      source, target, provider, "actor", mesh::service_mailbox_domain_t::application,
      [&] (auto callback) {
          return source.request_to_actor (remote.node_routing_id, std::nullopt, actor,
                                          request_payload, 2s, std::move (callback));
      },
      [&] (const auto &record) { return target.reply (record, reply_payload); });
    ...
    protocol::instance_spot_activation_header_t activation{
      {remote.node_routing_id, remote.lifecycle_generation, "instance-1", "metrics-requests",
       "room", "descriptor-1", deadline_unix_ms},
      ...
      false};
    complete_remote_request (
      source, target, provider, "instance_spot", mesh::service_mailbox_domain_t::infrastructure,
      [&] (auto callback) {
          return source.request_instance_spot_activation (remote.node_routing_id, activation,
    ...
    require (
      await_task (source.request_instance_spot_activation (
        remote.node_routing_id, std::move (failed_activation), std::nullopt, request_payload, 2s,
        [&activation_failure_promise] (foundation::operation_terminal_t terminal,
                                       std::vector<std::uint8_t> payload) mutable {
            activation_failure_promise.set_value ({terminal, std::move (payload)});
        })),
      "failed instance activation was not submitted");
    auto failed_activation_claim =
      claim_request (target, mesh::service_mailbox_domain_t::infrastructure);
    require (failed_activation_claim && failed_activation_claim->records.size () == 1,
             "failed instance activation was not delivered");
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "instance_spot"}}, 1);
    require (target.reply_instance_spot_activation (failed_activation_claim->records.front (), 104,
                                                    16, std::nullopt),
    ...
    const auto activation_failure_deadline = std::chrono::steady_clock::now () + 2s;
    while (activation_failure.wait_for (0ms) != std::future_status::ready
           && std::chrono::steady_clock::now () < activation_failure_deadline) {
        (void) await_task (
          source.pump_one (mesh::service_liveness_registry_t::clock_t::now (), false));
        std::this_thread::yield ();
    }
    require (activation_failure.wait_for (0ms) == std::future_status::ready,
             "failed instance activation terminal was not delivered");
    const auto failed_activation_result = activation_failure.get ();
    require (failed_activation_result.first == foundation::operation_terminal_t::completed,
             "instance activation reply changed its callback terminal");
    const auto failed_activation_parts =
      protocol::unpack_infrastructure_reply (failed_activation_result.second);
    ...
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "instance_spot"}}, 0);
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-requests"}, {"outcome", "failed"}, {"surface", "instance_spot"}}, 1);
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-requests"}, {"outcome", "completed"}, {"surface", "instance_spot"}},
      1);
    require (!find_sample (provider, "zlink.mesh_node.request.timeouts",
    ...
    require (await_task (source.request_to_node (
               ...
               [&failed_promise] (foundation::operation_terminal_t terminal,
                                  std::vector<std::uint8_t> payload) mutable {
                   failed_promise.set_value ({terminal, std::move (payload)});
               })),
             "failed-outcome request was not submitted");
    auto failed_claim = claim_request (target, mesh::service_mailbox_domain_t::application);
    ...
    require (target.mailbox ().release (*failed_claim), "failed request mailbox release failed");
    ...
    require (failed.get ().first == foundation::operation_terminal_t::transport_failed,
             "failed request reported the wrong terminal");
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "node"}}, 0);
    require_metric (provider, "zlink.mesh_node.request.duration", "histogram", "s",
                    {{"mesh_name", "metrics-requests"}, {"outcome", "failed"}, {"surface", "node"}},
                    1);
    require_positive_duration (
      provider, {{"mesh_name", "metrics-requests"}, {"outcome", "failed"}, {"surface", "node"}});
    require (!find_sample (provider, "zlink.mesh_node.request.timeouts",
    ...
    require (await_task (source.request_to_node (
               remote.node_routing_id, request_payload, 2s,
               [&timeout_promise] (foundation::operation_terminal_t terminal,
                                   std::vector<std::uint8_t> payload) mutable {
                   timeout_promise.set_value ({terminal, std::move (payload)});
               })),
             "timeout request was not submitted");
    auto late_claim = claim_request (target, mesh::service_mailbox_domain_t::application);
    require (late_claim && late_claim->records.size () == 1,
             "timeout request was not parked at its target");
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "node"}}, 1);
    require (source.expire_requests (foundation::operation_registry_t::clock_t::now () + 3s) == 1,
             "timeout request did not expire");
    require (timeout.wait_for (2s) == std::future_status::ready,
             "timeout callback was not delivered");
    require (timeout.get ().first == foundation::operation_terminal_t::timed_out,
             "timeout request reported the wrong terminal");
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "node"}}, 0);
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-requests"}, {"outcome", "timed_out"}, {"surface", "node"}}, 1);
    require_positive_duration (
      provider, {{"mesh_name", "metrics-requests"}, {"outcome", "timed_out"}, {"surface", "node"}});
    require_metric (provider, "zlink.mesh_node.request.timeouts", "counter", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "node"}}, 1);

    require (target.reply (late_claim->records.front (), reply_payload),
             "late reply could not be sent");
    require (target.mailbox ().release (*late_claim), "late request mailbox release failed");
    ...
    require (await_task (source.request_to_node (
               remote.node_routing_id, request_payload, 2s,
               [&wire_timeout_promise] (foundation::operation_terminal_t terminal,
                                        std::vector<std::uint8_t> payload) mutable {
                   wire_timeout_promise.set_value ({terminal, std::move (payload)});
               })),
             "wire timeout request was not submitted");
    auto wire_timeout_claim = claim_request (target, mesh::service_mailbox_domain_t::application);
    require (wire_timeout_claim && wire_timeout_claim->records.size () == 1,
             "wire timeout request was not delivered");
    require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "node"}}, 1);
    require (target.reply_failure (wire_timeout_claim->records.front (), 101, 0),
             "wire timeout reply could not be sent");
    require (target.mailbox ().release (*wire_timeout_claim),
             "wire timeout mailbox release failed");
    const auto wire_timeout_deadline = std::chrono::steady_clock::now () + 2s;
    ...
    require (wire_timeout_result.first == foundation::operation_terminal_t::transport_failed,
             "wire timeout changed its callback terminal");
    const auto wire_timeout_header = protocol::decode_reply_header (wire_timeout_result.second);
    ...
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-requests"}, {"outcome", "timed_out"}, {"surface", "node"}}, 2);
    require_metric (provider, "zlink.mesh_node.request.timeouts", "counter", "{request}",
                    {{"mesh_name", "metrics-requests"}, {"surface", "node"}}, 2);
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-requests"}, {"outcome", "completed"}, {"surface", "node"}}, 1);
}

void verify_remaining_terminal_outcomes ()
{
    metric_test::provider_t provider;
    auto metrics = std::make_shared<runtime::mesh_request_metrics_t> ("metrics-terminals");
    foundation::operation_registry_t registry;
    const auto register_operation = [&] (std::uint64_t id,
                                         runtime::mesh_request_surface_t surface) {
        require (registry.register_operation (
                   {1, id}, foundation::operation_registry_t::clock_t::now () + 2s,
                   [] (auto, auto) {}, {}, runtime::mesh_request_metric_t (metrics, surface)),
    ...
    require (registry.fail ({1, 2}, foundation::operation_terminal_t::shutdown),
             "request shutdown terminal failed");
    register_operation (3, runtime::mesh_request_surface_t::actor);
    require (registry.fail ({1, 3}, foundation::operation_terminal_t::transport_failed),
             "request failure terminal failed");

    for (const auto &[surface, outcome] : std::vector<std::pair<std::string, std::string>>{
           {"node", "cancelled"}, {"channel", "shutdown"}, {"actor", "failed"}}) {
        require_metric (provider, "zlink.mesh_node.requests.inflight", "updown", "{request}",
                        {{"mesh_name", "metrics-terminals"}, {"surface", surface}}, 0);
        require_metric (
          provider, "zlink.mesh_node.request.duration", "histogram", "s",
          {{"mesh_name", "metrics-terminals"}, {"outcome", outcome}, {"surface", surface}}, 1);
        require (!find_sample (provider, "zlink.mesh_node.request.timeouts",
...
std::shared_ptr<zlink::framework::detail::mesh_node_builder_state_t> make_public_node_state ()
{
    ...
    state->routing_id = zlink::routing_id_t::from (std::string ("metrics-local-node"));
...
{
    metric_test::provider_t provider;
    zlink::framework::detail::mesh_node_runtime_t node (make_public_node_state ());
    ...
    const std::vector<zlink::message_t> reply_parts{zlink::message_t::from (std::string ("reply"))};
    ...
    require (await_task (source_spot.request_to_spot (
               node_id, target_spot.spot_id (), target_spot.status ().lifecycle_generation (),
               request_parts, spot_operation, zlink::send_flags_t::none, 2s))
    ...
    (void) await_task (
      node.dispatch_ready ([&] (const host::ready_record_t &, const host::receive_record_t &record,
                                std::vector<zlink::message_t>) {
          if (record.kind == host::record_kind_t::spot_request) {
              spot_replied =
                host::reply (record.reply_token, reply_parts) == zlink::submit_result_t::ok;
          }
      }));
    ...
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-local-public"}, {"outcome", "completed"}, {"surface", "spot"}}, 1);
    require_positive_duration (
      provider,
      {{"mesh_name", "metrics-local-public"}, {"outcome", "completed"}, {"surface", "spot"}});
    require (!find_sample (provider, "zlink.mesh_node.request.timeouts",
                           {{"mesh_name", "metrics-local-public"}, {"surface", "spot"}}),
             "local public Spot completion emitted a timeout counter");

    auto source_actor = node.create_actor ("metric.actor", "source-actor", {}, 2s);
    auto target_actor = node.create_actor ("metric.actor", "target-actor", {}, 2s);
    host::pending_operation_t actor_operation;
    require (await_task (source_actor.request_to (target_actor.ref (), request_parts,
                                                  actor_operation, zlink::send_flags_t::none, 2s))
               == zlink::submit_result_t::ok,
    ...
    require_metric (
      provider, "zlink.mesh_node.request.duration", "histogram", "s",
      {{"mesh_name", "metrics-local-public"}, {"outcome", "completed"}, {"surface", "actor"}}, 1);
    require_positive_duration (
      provider,
      {{"mesh_name", "metrics-local-public"}, {"outcome", "completed"}, {"surface", "actor"}});
    require (!find_sample (provider, "zlink.mesh_node.request.timeouts",
