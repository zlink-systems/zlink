...
codec::codec_context_t operation_context (const nlohmann::json &item)
{
    ...
    if (values.contains ("effectiveCompleteMessageBytesMinusActualEnvelopeOverhead"))
        context.effectiveCompleteMessageBytesMinusActualEnvelopeOverhead =
          values.at ("effectiveCompleteMessageBytesMinusActualEnvelopeOverhead")
            .get<std::int64_t> ();
...
std::vector<std::uint8_t> from_hex (std::string_view hex)
{
    ...
    for (std::size_t index = 0; index < hex.size (); index += 2)
        result.push_back (
          static_cast<std::uint8_t> ((digit (hex[index]) << 4) | digit (hex[index + 1])));
    return result;
}
...
nlohmann::json load_json (const std::filesystem::path &path)
{
    ...
    for (std::size_t position = 0;
         (position = source.find ("\\ud800", position)) != std::string::npos; position += 7)
        source.replace (position, 6, "\\\\ud800");
    return nlohmann::json::parse (source);
}

const nlohmann::json &
pointer (const nlohmann::json &fixture, const nlohmann::json &pointers, const char *name)
{
    return fixture.at (nlohmann::json::json_pointer (pointers.at (name).get<std::string> ()));
}

std::vector<std::vector<std::uint8_t>> bytes (const nlohmann::json &fixture,
...
outcome exact (Result decoded, Encode encode, const std::vector<std::uint8_t> &input)
{
    ...
    return {encoded.value == input,
            encoded.value == input ? codec::error_code::ok : codec::error_code::constraint};
}

template <class Result, class Encode>
outcome
exact_frames (Result decoded, Encode encode, const std::vector<std::vector<std::uint8_t>> &input)
{
    if (!decoded)
        return {false, decoded.error};
    const auto encoded = encode (decoded.value);
...
template <class Result> outcome encoded (Result result, const std::vector<std::uint8_t> &input)
{
    if (!result)
...
outcome command (std::uint8_t id,
                 ...
                 const codec::codec_context_t &context)
{
    switch (id) {
        case 16:
            return exact_frames (
              codec::decode_nodeSend_16_frames (frames, context),
              [&] (const auto &value) { return codec::encode_nodeSend_16_frames (value, context); },
              frames);
        case 24:
            return exact_frames (
              codec::decode_actorSend_24_frames (frames, context),
              [&] (const auto &value) {
                  return codec::encode_actorSend_24_frames (value, context);
              },
              frames);
        case 28:
            return exact_frames (
              codec::decode_actorJoin_28_frames (frames, context),
              [&] (const auto &value) {
                  return codec::encode_actorJoin_28_frames (value, context);
              },
              frames);
        case 47:
            return exact_frames (codec::decode_userSpotCreate_47_frames (frames),
                                 codec::encode_userSpotCreate_47_frames, frames);
        case 48:
            return exact_frames (codec::decode_userSpotClose_48_frames (frames),
                                 codec::encode_userSpotClose_48_frames, frames);
        case 49:
            return exact_frames (codec::decode_actorCreate_49_frames (frames),
                                 codec::encode_actorCreate_49_frames, frames);
        default:
            return {false, codec::error_code::header};
    }
}

outcome type (std::string_view name,
              const std::vector<std::uint8_t> &input,
              const codec::codec_context_t &context)
{
    if (name == "application-version")
        return exact (codec::decode_application_version (input), codec::encode_application_version,
                      input);
    if (name == "bool8")
    ...
    if (name == "optional-actor-ref")
        return exact (codec::decode_optional_actor_ref (input), codec::encode_optional_actor_ref,
                      input);
    if (name == "actor-ref")
        return exact (codec::decode_actor_ref (input), codec::encode_actor_ref, input);
    if (name == "sorted-text8-vector")
        return exact (codec::decode_sorted_text8_vector (input), codec::encode_sorted_text8_vector,
                      input);
    ...
    if (name == "authority-payload-v1")
        return exact (codec::decode_durable_authority_payload_v1 (input),
                      codec::encode_durable_authority_payload_v1, input);
    if (name == "instance-activation-recovery-v1")
        return exact (
          codec::decode_durable_instance_activation_recovery_v1 (input, context),
          [&] (const auto &value) {
              return codec::encode_durable_instance_activation_recovery_v1 (value, context);
          },
          input);
    if (name == "relocation-data-chunk-v1")
        return exact (codec::decode_durable_relocation_data_chunk_v1 (input),
                      codec::encode_durable_relocation_data_chunk_v1, input);
    if (name == "relocation-manifest-v1")
        return exact (codec::decode_durable_relocation_manifest_v1 (input),
                      codec::encode_durable_relocation_manifest_v1, input);
    if (name == "relocation-envelope-v1")
        return exact (
          codec::decode_relocation_envelope_v1 (input, context),
          [&] (const auto &value) { return codec::encode_relocation_envelope_v1 (value, context); },
          input);
    if (name == "descriptor-extension")
        return exact (codec::decode_descriptor_extension (input),
                      codec::encode_descriptor_extension, input);
    if (name == "relocation-object-identity")
        return exact (codec::decode_relocation_object_identity (input),
                      codec::encode_relocation_object_identity, input);
    if (name == "aggregate-participant-vector")
        return exact (codec::decode_aggregate_participant_vector (input),
                      codec::encode_aggregate_participant_vector, input);
    if (name == "saved-work-vector")
        return exact (
          codec::decode_saved_work_vector (input, context),
          [&] (const auto &value) { return codec::encode_saved_work_vector (value, context); },
          input);
    if (name == "text8")
        return exact (codec::decode_text8 (input), codec::encode_text8, input);
    if (name == "application-payload-bytes")
        return exact (
          codec::decode_application_payload_bytes (input, context),
          [&] (const auto &value) {
              return codec::encode_application_payload_bytes (value, context);
          },
          input);
    if (name == "application-payload-envelope-v1")
        return exact (
          codec::decode_application_payload_envelope_v1 (input, context),
          [&] (const auto &value) {
              return codec::encode_application_payload_envelope_v1 (value, context);
          },
          input);
    if (name == "creation-operation-terminal-v1")
        return exact (
          codec::decode_creation_operation_terminal_v1 (input, context),
          [&] (const auto &value) {
              return codec::encode_creation_operation_terminal_v1 (value, context);
          },
          input);
    return {false, codec::error_code::header};
}

bool pilot_oracle (const nlohmann::json &surface,
                   const std::vector<std::vector<std::uint8_t>> &frames,
                   bool &available)
{
    available = true;
    try {
        ...
        if (format == "user-spot-create-v1")
            return pilot::encode_user_spot_create_47 (
                     pilot::decode_user_spot_create_47 (frames.front ()))
                   == frames.front ();
        if (format == "user-spot-close-v1")
            return pilot::encode_user_spot_close_48 (
                     pilot::decode_user_spot_close_48 (frames.front ()))
                   == frames.front ();
        if (format == "actor-create-v1")
            return pilot::encode_actor_create_49 (pilot::decode_actor_create_49 (frames.front ()))
                   == frames.front ();
        if (format == "relocation-envelope-v1")
            return pilot::encode_relocation_envelope_v1 (
                     pilot::decode_relocation_envelope_v1 (frames))
                   == frames.front ();
        if (format == "relocation-data-chunk-v1")
            return pilot::encode_relocation_data_chunk_v1 (
                     pilot::decode_relocation_data_chunk_v1 (frames.front ()))
                   == frames.front ();
        if (format == "relocation-manifest-v1")
            return pilot::encode_relocation_manifest_v1 (
                     pilot::decode_relocation_manifest_v1 (frames.front ()))
                   == frames.front ();
        available = false;
        return false;
    }
...
outcome negotiated_case (const nlohmann::json &item,
...
codec::relocation_object_identity_t relocation_identity (const nlohmann::json &input)
{
    ...
    if (input.at ("variant") == "actor") {
        ...
        selected.actor.objectGeneration.value =
          input.at ("actor").at ("objectGeneration").get<std::uint64_t> ();
...
codec::descriptor_extension_t descriptor (const nlohmann::json &input)
{
    ...
    value.applicationVersion =
      codec::application_version_t{input.at ("applicationVersion").get<std::int64_t> ()};
    ...
    value.activeCapacityLimit =
      codec::object_capacity_limit_t{input.at ("activeCapacityLimit").get<std::uint32_t> ()};
    ...
    value.pendingCapacityUsed =
      codec::u32_t{input.at ("pendingCapacityUsed").get<std::uint32_t> ()};
    return value;
}

outcome semantic_encode (const nlohmann::json &item,
                         const std::vector<std::uint8_t> &expected,
                         const codec::codec_context_t &context)
{
    const auto name = item.at ("surface").at ("type").is_null ()
                        ? std::string{}
                        : item.at ("surface").at ("type").get<std::string> ();
    ...
    if (name == "relocation-object-identity")
        return encoded (codec::encode_relocation_object_identity (relocation_identity (input)),
                        expected);
    if (name == "sorted-text8-vector") {
    ...
    if (name == "aggregate-participant-vector") {
        ...
        for (const auto &entry : input) {
            ...
            participant.expectedStoreVersion.value =
              entry.at ("expectedStoreVersion").get<std::string> ();
    ...
    if (name == "application-payload-bytes") {
        ...
        value.value.assign (input.at ("count").get<std::size_t> (),
                            input.at ("repeatByte").get<std::uint8_t> ());
...
outcome operation_case (const nlohmann::json &item, std::string_view direction)
{
    ...
    if (item.at ("operation") == "logical-stream") {
        codec::logical_relocation_envelope_v1_decoder_t decoder{maximum_codec_context ()};
        if (item.contains ("syntheticPriorInputByteCount"))
            decoder.set_synthetic_prior_input_byte_count_for_test (
              item.at ("syntheticPriorInputByteCount").get<std::uint64_t> ());
        std::optional<codec::logical_relocation_envelope_v1_t> complete;
        std::vector<std::uint8_t> canonical;
        std::uint64_t supplied{};
        const auto finalIndex = item.at ("finalChunkIndex").get<std::size_t> ();
        for (std::size_t index = 0; index < item.at ("chunksHex").size (); ++index) {
            auto chunk = from_hex (item.at ("chunksHex").at (index).get<std::string> ());
            canonical.insert (canonical.end (), chunk.begin (), chunk.end ());
            supplied += chunk.size ();
            const auto step = decoder.push (chunk, index == finalIndex);
            std::fill (chunk.begin (), chunk.end (), 0);
            if (!step) {
                const auto expectedKind = item.at ("failureKind").get<std::string> ();
                if (index != item.at ("failureChunkIndex").get<std::size_t> ()
                    || item.at ("chunkOutcomes").at (index).get<std::string> () != expectedKind)
                    throw std::runtime_error{item.at ("name").get<std::string> ()
                                             + ": logical failure chunk mismatch"};
                const auto expectedError =
                  expectedKind == "truncated"  ? codec::error_code::truncated
                  : expectedKind == "trailing" ? codec::error_code::trailing
                  : expectedKind == "capacity" ? codec::error_code::capacity
                  : expectedKind == "limit"    ? codec::error_code::limit
                                               : codec::error_code::constraint;
                const auto actualKind = step.error == codec::error_code::truncated
                                            || step.error == codec::error_code::trailing
                                            || step.error == codec::error_code::capacity
                                            || step.error == codec::error_code::limit
                                          ? step.error
                                          : codec::error_code::constraint;
                if (actualKind != expectedError
                    || decoder.failure_offset () != item.at ("failureOffset").get<std::uint64_t> ())
                    throw std::runtime_error{
                      item.at ("name").get<std::string> () + ": logical failure expected="
                      + expectedKind + " actual=" + std::to_string (static_cast<int> (step.error))
                      + " offset=" + std::to_string (decoder.failure_offset ())};
                const auto failureOffset = decoder.failure_offset ();
                if (item.value ("assertChunkReleasedAfterFailure", false)) {
                    const auto terminal = decoder.push ({}, false);
                    if (terminal || terminal.error != codec::error_code::invalid_state
                        || decoder.failure_offset () != failureOffset)
                        throw std::runtime_error{item.at ("name").get<std::string> ()
                                                 + ": caller chunk retained after failure"};
                }
                return {false, step.error};
            }
            if (item.contains ("failureChunkIndex")
                && index == item.at ("failureChunkIndex").get<std::size_t> ())
                return {false, codec::error_code::constraint};
            if (step.value.buffered_input_byte_count
                > item.at ("maximumBufferedInputBytes").get<std::size_t> ())
                return {false, codec::error_code::constraint};
            if (item.contains ("expectedBufferedInputByteCounts")) {
                const auto &expectedBuffered =
                  item.at ("expectedBufferedInputByteCounts").at (index);
                if (!expectedBuffered.is_null ()
                    && step.value.buffered_input_byte_count != expectedBuffered.get<std::size_t> ())
                    return {false, codec::error_code::constraint};
            }
            if (step.value.consumed_byte_count != supplied
                || step.value.continuation_depth
                     > codec::logical_relocation_envelope_v1_decoder_t::maximum_continuation_depth)
                return {false, codec::error_code::constraint};
            const auto expected = item.at ("chunkOutcomes").at (index).get<std::string> ();
            const auto actual = step.value.progress == codec::logical_decode_progress_t::need_more
                                  ? "need-more"
                                  : "complete";
            if (expected != actual)
                return {false, codec::error_code::constraint};
            if (step.value.progress == codec::logical_decode_progress_t::complete)
                complete = step.value.value;
        }
        if (!complete)
            return {false, codec::error_code::truncated};
        const auto terminal = decoder.push ({}, true);
        if (terminal || terminal.error != codec::error_code::invalid_state)
            return {false, codec::error_code::constraint};
        return exact (
          codec::result_t<codec::logical_relocation_envelope_v1_t>{codec::error_code::ok,
                                                                   std::move (*complete)},
          [&] (const auto &value) {
              return codec::encode_logical_relocation_envelope_v1 (value, maximum_codec_context ());
          },
          canonical);
    }
    ...
    if (direction == "encode" && item.contains ("encodeInputHex")) {
        auto seed = codec::decode_saved_work_vector (
          from_hex (item.at ("encodeInputHex").get<std::string> ()), context);
        if (!seed || item.at ("encodeMutation") != "duplicate-first-item")
            return {false, seed ? codec::error_code::constraint : seed.error};
        seed.value.items.push_back (seed.value.items.front ());
        return encoded (codec::encode_saved_work_vector (seed.value, context), frames.front ());
    }
...
int main ()
{
    ...
    for (const auto &entry : index.at ("fixtures")) {
        const auto fixture =
          load_json (protocolRoot / entry.at ("goldenFixture").get<std::string> ());
        ...
        const auto verify = [&] (const nlohmann::json &item, bool expected) {
            ...
            if (result.accepted != expected) {
                std::cerr << kind << ':' << surface.at ("format").get<std::string> () << ':'
                          ...
                          << ": operation=fixture, expected " << (expected ? "accept" : "reject")
    ...
    for (const auto &item : index.at ("operationCases")) {
        ...
        const auto directions = item.contains ("directions") ? item.at ("directions")
        ...
        for (const auto &directionValue : directions) {
            ...
            if (item.contains ("expectedFailure")) {
                const auto expectedError = item.at ("expectedFailure").at ("cppError");
                if (expectedError != "duplicate" || result.error != codec::error_code::duplicate) {
                    std::cerr << "operation-case:" << item.at ("name").get<std::string> ()
                              << ": direction=" << direction
                              << ", expected duplicate error-code, actual="
    ...
    if (index.at ("version") != 4 || acceptedCases != 33 || rejectedCases != 60
        || boundaryPairs.size () != 25) {
        std::cerr << "fixture catalog v4 coverage mismatch: accept=" << acceptedCases
