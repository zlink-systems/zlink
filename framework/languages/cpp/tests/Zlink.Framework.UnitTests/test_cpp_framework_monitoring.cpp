...
bool unsubscribed_metrics_remain_disabled ()
{
    ...
    for (std::size_t index = 0; index < 10'000; ++index) {
        metrics.counter ("zlink.test.unsubscribed", "{operation}", 1, {{"operation", "request"}});
    }
...
bool unsubscribed_metric_storage_unchanged ()
{
    ...
    for (std::size_t index = 0; index < 50'000; ++index) {
        ...
        metrics.updown ("zlink.test.storage.active", "{operation}", 1, {{"operation", "request"}});
    }

    if (state.use_count () != baseline_owners || !state->metric_instruments.empty ()) {
        std::cerr << "unsubscribed metric emission retained state owners: " << baseline_owners
                  << " -> " << state.use_count () << '\n';
        return false;
    }
    if (metrics.enabled ()) {
...
bool public_spot_event_surface_reports_internal_timer_failure ()
{
    ...
    monitoring.add_spot_events ("zone-node-1")
      .on_spot_event (
        [&observed] (const zlink::framework::spot_event_t &event) { observed.push_back (event); });
    const auto runtime = zlink::framework::detail::monitoring_runtime_t::from (monitoring);
    runtime.publish_timer_failure (
      "unregistered-node", zlink::framework::spot_id_t ("zone-ignored"),
      zlink::framework::timer_failure_event_t{"ignored", std::type_index (typeid (int)), 1, false,
                                              "ignored"});
    runtime.publish_timer_failure ("zone-node-1", zlink::framework::spot_id_t ("zone-a"),
                                   zlink::framework::timer_failure_event_t{
                                     "zone-tick", std::type_index (typeid (int)), 7, true, "boom"});
    return observed.size () == 1 && observed.front ().source_name == "zone-node-1"
           && observed.front ().event
                == zlink::framework::spot_event_kind_t::timer_stopped_after_unhandled_exception
           && observed.front ().diagnostic.spot_id == zlink::framework::spot_id_t ("zone-a")
