...
int main ()
{
    ...
    const std::vector<std::string_view> relocation_control_golden{
      ...
      /* relocationPrepareRestore (golden): baseChecksumCrc32c=0 (no base
       * stage), inserted after payloadChecksumCrc32c and before
       * applicationVersion. */
      "",
      /* relocationStateChunk (golden): payloadStage=final(01), inserted
       * after object and before chunkOrdinal. */
      "5a4d0134000000000000000004000000000000000500000000000000060b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a010000000000000010000102030405060708090a0b0c0d0e0f",
      "5a4d0134000000000000000004000000000000000500000000000000060b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a0100000001000000081011121314151617",
      /* relocationPrepareRestoreWithBase (golden): non-zero
       * baseChecksumCrc32c means the final-stage payload is a delta
       * against a base snapshot with this checksum. */
      "",
      /* relocationStateBaseChunk (golden): payloadStage=base(00), its own
       * independent ordinal space starting at chunkOrdinal=0. */
      "5a4d0134000000000000000004000000000000000500000000000000060b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a00000000000000000c62617365736e617073686f74"};
    ...
    assert (prepare.base_checksum_crc32c == 0);
    assert (state_chunk.sender_role == protocol::relocation_role_t::source);
    assert (state_chunk.payload_stage
            == protocol::relocation_payload_stage_t::final);
    assert (state_chunk.chunk_ordinal == 0);
    assert (state_chunk.chunk_data.size () == 16);
    assert (state_final_chunk.payload_stage
            == protocol::relocation_payload_stage_t::final);
    assert (state_final_chunk.chunk_ordinal == 1);
    assert (state_final_chunk.chunk_data.size () == 8);
    {
        /* relocationPrepareRestoreWithBase: non-zero baseChecksumCrc32c
         * signals a delta payload against a base snapshot with that
         * checksum. */
        const auto &prepare_with_base = std::get<protocol::relocation_prepare_t> (
          decoded_controls[6]);
        assert (prepare_with_base.base_checksum_crc32c == 0x6e3e3b64u);
        assert (prepare_with_base.payload_checksum_crc32c == 0x29bc8795u);
    }
    {
        /* relocationStateBaseChunk: payloadStage=base has its own
         * independent chunkOrdinal space (28's chunkOrdinal-space-is-
         * independent-per-payloadStage), starting again at 0. */
        const auto &state_base_chunk = std::get<protocol::relocation_state_t> (
          decoded_controls[7]);
        assert (state_base_chunk.payload_stage
                == protocol::relocation_payload_stage_t::base);
        assert (state_base_chunk.chunk_ordinal == 0);
        const std::string_view base_text{"basesnapshot"};
        assert (state_base_chunk.chunk_data.size () == base_text.size ());
        assert (std::equal (state_base_chunk.chunk_data.begin (),
                            state_base_chunk.chunk_data.end (),
                            base_text.begin ()));
    }
    {
    ...
    }
    {
         ...
         * data, a trailing byte, and an invalid payloadStage value are all
         * rejected. */
        const std::vector<std::string_view> malformed{
          "5a4d0134000000000000000004000000000000000500000000000000000b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a010000000000000010000102030405060708090a0b0c0d0e0f",
          "5a4d0134000000000000000004000000000000000500000000000000060b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a010000000000000010000102030405060708090a0b0c0d0e",
          "5a4d0134000000000000000004000000000000000500000000000000060b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a010000000000000010000102030405060708090a0b0c0d0e0f00",
          "5a4d0134000000000000000004000000000000000500000000000000060b636f6f7264696e61746f720000000000000007066e6f64652d61000000000000000b000773746f72652d33010200170673706f742d310000000000000009000000000000000a020000000000000010000102030405060708090a0b0c0d0e0f"};
