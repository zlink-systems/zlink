...
int main ()
{
    ...
    input >> golden;
    require (golden.at ("format").get<std::string> () == "relocation-envelope-v1",
             "golden format tag must match");

    const auto logical = from_hex (golden.at ("logicalHex").get<std::string> ());
    ...
    require (envelope.relocation.high == u64 (decoded.at ("relocationHigh"))
               && envelope.relocation.low == u64 (decoded.at ("relocationLow")),
    ...
    require (envelope.object.object_id == object.at ("spotIdUtf8Fixture").get<std::string> (),
             "object id must match");
    require (envelope.object.object_generation == u64 (object.at ("spotGeneration")),
    ...
    require (envelope.application_version
               == static_cast<std::int64_t> (u64 (decoded.at ("applicationVersion"))),
    ...
    for (std::size_t index = 0;
         index < states.size () && index < envelope.application_states.size (); ++index) {
        const auto &expected = states[index];
        const auto &actual = envelope.application_states[index];
        require (actual.participant_id == u64 (expected.at ("participantId")),
                 "state participant id must match");
        const auto &application = expected.at ("applicationState");
        const auto has_state = application.at ("hasState").get<bool> ();
        require (actual.has_state == has_state, "state presence must match");
        if (has_state)
            require (actual.state == fixture_bytes (application.at ("payloadUtf8Fixture")),
                     "state payload must match");
        else
            require (actual.state.empty (), "an absent state must decode empty");
    }

    const auto &saved_work = decoded.at ("savedWork");
    require (envelope.saved_work.size () == saved_work.size (), "saved work count must match");
    for (std::size_t index = 0; index < saved_work.size () && index < envelope.saved_work.size ();
         ++index) {
        const auto &expected = saved_work[index];
        const auto &actual = envelope.saved_work[index];
        require (actual.participant_id == u64 (expected.at ("participantId")),
                 "saved work participant must match");
        require (actual.order == u64 (expected.at ("order")), "saved work order must match");
        const auto &record = expected.at ("record");
        require (record.at ("recordKind").get<std::string> () == "spotRequest"
                   && actual.record.kind == protocol::frozen_record_kind_t::spot_request,
                 "record kind must match");
        const auto &source = record.at ("source");
        require (source.at ("sourceKind").get<std::string> () == "node"
                   && actual.record.source_kind == protocol::frozen_source_kind_t::node,
                 "record source kind must match");
        require (actual.record.source.node_routing_id
                   == fixture_bytes (source.at ("sourceNodeRidUtf8Fixture")),
                 "record source node rid must match");
        require (actual.record.source.node_generation == u64 (source.at ("sourceNodeGeneration")),
                 "record source node generation must match");
        require (actual.record.source.owner_id == source.at ("sourceOwnerId").get<std::string> (),
        ...
        require (record.at ("metadata").is_null () == !actual.record.has_metadata,
                 "record metadata presence must match");
        require (actual.record.operation.high == u64 (record.at ("operationId").at ("high"))
                   && actual.record.operation.low == u64 (record.at ("operationId").at ("low")),
                 "record operation id must match");
        require (record.at ("operationKind").get<std::string> () == "spotRequest"
                   && actual.record.operation_kind == 3,
                 "record operation kind must match");
        require (actual.record.reply_route_id
                   == std::optional<std::uint64_t>{u64 (record.at ("replyRouteId"))},
                 "record reply route must match");
        const auto &body = record.at ("body");
        const auto &target_spot = body.at ("targetSpot");
        require (actual.record.target.has_value (), "record target must decode");
        if (actual.record.target) {
            require (actual.record.target->object_id
                         == target_spot.at ("spotIdUtf8Fixture").get<std::string> ()
                       && actual.record.target->object_generation
                            == u64 (target_spot.at ("spotGeneration"))
                       && actual.record.target->target_node_routing_id
                            == fixture_bytes (target_spot.at ("targetNodeRidUtf8Fixture"))
                       && actual.record.target->target_node_generation
                            == u64 (target_spot.at ("targetNodeGeneration"))
                       && actual.record.target->authority_owner_generation
                            == u64 (target_spot.at ("expectedAuthorityOwnerGeneration"))
                       && actual.record.target->owner_lease_generation
                            == u64 (target_spot.at ("expectedOwnerLeaseGeneration")),
                     "record target fence must match");
        }
        const auto &payload = body.at ("payload");
        require (actual.record.application.has_value (), "record payload must decode");
        if (actual.record.application) {
            require (actual.record.application->packet_name
                       ...
                       && actual.record.application->payload_bytes ()
                            == fixture_bytes (payload.at ("payloadUtf8Fixture")),
    ...
    for (std::size_t index = 0;
         index < timers.size () && index < envelope.timer_registrations.size (); ++index) {
        ...
        require (actual.participant_id == u64 (expected.at ("participantId"))
                   ...
                   && actual.handler_type == expected.at ("handlerType").get<std::string> ()
                   && actual.period_milliseconds == u64 (expected.at ("periodMilliseconds"))
                   && (expected.at ("overrunPolicy").get<std::string> () != "skipLateTicks"
                       || actual.overrun_policy == 1)
                   && actual.max_catch_up_ticks == u64 (expected.at ("maxCatchUpTicks"))
    ...
    for (std::size_t index = 0;
         index < pending.size () && index < envelope.pending_timer_ticks.size (); ++index) {
        const auto &expected = pending[index];
        const auto &actual = envelope.pending_timer_ticks[index];
        require (actual.participant_id == u64 (expected.at ("participantId"))
                   && actual.order == u64 (expected.at ("order"))
                   && actual.timer_name == expected.at ("timerName").get<std::string> ()
                   && actual.delivery_index == u64 (expected.at ("deliveryIndex"))
                   && actual.scheduled_index == u64 (expected.at ("scheduledIndex"))
    ...
    {
        const auto &inventory = golden.at ("participantAuthorityKeyInventory");
        require (inventory.size () == 3
                   && inventory[0].at ("authorityKeyUtf8Fixture").get<std::string> () == "actor:z"
                   && inventory[1].at ("authorityKeyUtf8Fixture").get<std::string> () == "actor:é"
                   && inventory[2].at ("authorityKeyUtf8Fixture").get<std::string> () == "spot:a",
                 "golden inventory fixture must be the declared order");
        const auto identity = [] (stateful::object_kind_t kind, std::string key) {
