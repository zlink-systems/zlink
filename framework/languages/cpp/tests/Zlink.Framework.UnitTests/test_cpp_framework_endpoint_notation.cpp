...
int main ()
{
    ...
    expect_eq (transport::normalize_endpoint ("TCP://Example.COM:80"), "tcp://example.com:80",
               "scheme+host lowercase");
    ...
    expect_eq (transport::normalize_endpoint ("tcp://[2001:DB8::1]:443"), "tcp://[2001:db8::1]:443",
               "IPv6 hex digits lowercased");

    /* IPv6 zone id preserved verbatim (not lowercased, not dropped) */
    expect_eq (transport::normalize_endpoint ("tcp://[fe80::1%eth0]:80"), "tcp://[fe80::1%eth0]:80",
               "IPv6 zone id preserved");
