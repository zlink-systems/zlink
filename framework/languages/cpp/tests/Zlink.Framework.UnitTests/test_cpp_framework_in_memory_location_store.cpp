...
location_owner_token_t claim_owner (in_memory_location_repository_t &store,
                                    std::string owner_id,
...
make_mesh_node (std::string rid, location_owner_token_t owner, std::int32_t actor_limit = 10000)
{
    ...
    return mesh_node_descriptor_t{
      ...
      .capacity = {.actors = {.limit = actor_limit}, .spots = {.limit = 128}},
...
void publish_mesh_node (in_memory_location_repository_t &store,
                        std::string rid,
                        ...
                        std::int32_t actor_limit = 10000)
{
    EXPECT_EQ (
      ...
      store
        .update_mesh_node (make_mesh_node (std::move (rid), std::move (owner), actor_limit),
      ...
      AuthorityRetargetPreservesExactOwnerAndMovesActiveCapacity)
{
    ...
    object_reserve_request_t reservation{.key = {placement_object_kind_t::actor, "actor-authority"},
                                         .intent = {.stable_type = "player",
                                                    .request_content_reference = "request-root",
                                                    .request_encoded_size = 4},
                                         .target = {.mesh_name = "play",
                                                    .node_rid = node_rid_t::from_string ("node-a"),
                                                    .node_lifecycle_generation = 1,
                                                    .owner = owner_a},
                                         .creating_payload = {std::byte{0x01}, std::byte{0x02}},
                                         .capacity_bundle = {.actor_slots = 1}};
    const auto reserved = store.reserve (reservation).result ().value ();
    const auto *reserved_value = std::get_if<object_reserved_t> (&reserved);
    ASSERT_NE (nullptr, reserved_value);
    EXPECT_EQ (reservation.creating_payload, reserved_value->creating.payload);
    EXPECT_EQ (owner_a.lease_generation, reserved_value->creating.owner.lease_generation);

    const std::vector<std::byte> ready_payload{std::byte{0x03}, std::byte{0x04}};
    const auto committed =
      store.commit ({reservation.key, reserved_value->fence, ready_payload}).result ().value ();
    const auto *committed_value = std::get_if<object_committed_t> (&committed);
    ASSERT_NE (nullptr, committed_value);
    EXPECT_EQ (ready_payload, committed_value->ready.payload);

    const authority_key_t authority_key = actor_authority_key ("actor-authority");
    const auto preserved =
      store
        .compare_exchange_authority (authority_key, committed_value->ready.store_version,
                                     authority_put_t{{std::byte{0x05}}})
        .result ()
        .value ();
    const auto *preserved_value = std::get_if<authority_stored_t> (&preserved);
    ASSERT_NE (nullptr, preserved_value);
    EXPECT_EQ (owner_a.lease_generation, preserved_value->snapshot.owner.lease_generation);
    EXPECT_EQ (committed_value->ready.authority_owner_generation,
               preserved_value->snapshot.authority_owner_generation);

    const object_creation_target_t target{.mesh_name = "play",
                                          .node_rid = node_rid_t::from_string ("node-b"),
                                          .node_lifecycle_generation = 1,
                                          .owner = owner_b};
    const auto moved =
      store
        .compare_exchange_authority (authority_key, preserved_value->snapshot.store_version,
                                     authority_retarget_t{{std::byte{0x06}}, target})
        .result ()
        .value ();
    const auto *moved_value = std::get_if<authority_stored_t> (&moved);
    ASSERT_NE (nullptr, moved_value);
    EXPECT_EQ (owner_b.lease_generation, moved_value->snapshot.owner.lease_generation);
    EXPECT_GT (moved_value->snapshot.authority_owner_generation,
               preserved_value->snapshot.authority_owner_generation);
    const auto nodes = store.list_mesh_nodes ("play").result ().value ();
    const auto source =
      std::find_if (nodes.items.begin (), nodes.items.end (),
...
TEST (ZLinkFrameworkInMemoryLocationStore, DescriptorFencesCapabilityProfileAndPendingCapacity)
{
    using namespace zlink::framework;
    in_memory_location_repository_t store;
    const auto owner =
      std::get<owner_lease_claimed_t> (
        store.claim_owner_lease ("owner-target", std::chrono::seconds (15)).result ().value ())
        .token;

    object_reserve_request_t request{.key = {placement_object_kind_t::actor, "profiled-a"},
                                     .intent = {.stable_type = "player"},
                                     .target = {.mesh_name = "play",
                                                ...
                                                .owner = owner},
                                     .capacity_bundle = {.actor_slots = 1}};
    EXPECT_NE (nullptr, std::get_if<object_reserve_conflict_t> (
                          &store.reserve (request).result ().value ()));

    publish_mesh_node (store, "node-target", owner, 1);
    EXPECT_NE (nullptr,
               std::get_if<object_reserved_t> (&store.reserve (request).result ().value ()));
    auto over_limit = request;
    over_limit.key.global_id = "profiled-b";
    EXPECT_NE (nullptr, std::get_if<object_placement_capacity_exhausted_t> (
                          &store.reserve (over_limit).result ().value ()));

    auto changed = make_mesh_node ("node-target", owner, 2);
    changed.descriptor_revision = 2;
    EXPECT_EQ (
      location_write_status_t::rejected_conflict,
      store.update_mesh_node (changed, location_write_intent_t::renew).result ().value ().status);
    const auto listed = store.list_mesh_nodes ("play").result ().value ();
    ASSERT_EQ (1u, listed.items.size ());
    EXPECT_EQ (1, listed.items.front ().capacity.actors.limit);
}

TEST (ZLinkFrameworkInMemoryLocationStore, AuthorityRestorePreservesIdentityWithoutLiveOwnerLease)
{
    using namespace zlink::framework;
    in_memory_location_repository_t store;
    ...
    const auto committed =
      store.commit ({request.key, reservation->fence, {std::byte{0x02}}}).result ().value ();
    ...
    EXPECT_EQ ((std::vector<std::byte>{std::byte{0x04}}), stored->snapshot.payload);
    EXPECT_EQ (ready->ready.object_generation, stored->snapshot.object_generation);
    EXPECT_EQ (ready->ready.authority_owner_generation,
               stored->snapshot.authority_owner_generation);
    EXPECT_EQ (ready->ready.owner.owner_id, stored->snapshot.owner.owner_id);
    EXPECT_EQ (ready->ready.owner.lease_generation, stored->snapshot.owner.lease_generation);
    EXPECT_EQ (ready->ready.allocation.target.node_rid.value (),
               stored->snapshot.allocation.target.node_rid.value ());

    const auto stale = store
                         .compare_exchange_authority (actor_authority_key ("restore-actor"),
      ...
      StoreRevisionExhaustionDoesNotReuseVersionOrMutateAuthority)
{
    ...
    const auto owner =
      std::get<owner_lease_claimed_t> (
        store.claim_owner_lease ("owner-max", std::chrono::seconds (15)).result ().value ())
        .token;
    publish_mesh_node (store, "node-max", owner);
    object_reserve_request_t request{.key = {placement_object_kind_t::actor, "max-revision"},
                                     .intent = {.stable_type = "player"},
                                     .target = {.mesh_name = "play",
...
TEST (ZLinkFrameworkInMemoryLocationStore, CreationTerminalDedupesOnlyTheSameOperation)
{
    using namespace zlink::framework;
    in_memory_location_repository_t store;
    ...
    const std::vector<std::byte> envelope{std::byte{'r'}, std::byte{'e'}, std::byte{'j'}};
    creation_terminal_publication_t publication{
      first_operation, envelope, std::chrono::system_clock::now () + std::chrono::seconds (30)};
    const auto completed =
      store
        .complete_creation ({request.key, reserved.fence, object_creation_rejected_t{publication}})
        .result ()
        .value ();
    ASSERT_NE (nullptr, std::get_if<object_creation_completed_result_t> (&completed));
    const auto terminal = store.read_creation_terminal (first_operation).result ().value ();
    ASSERT_TRUE (terminal);
    EXPECT_EQ (envelope, terminal->terminal_envelope);

    auto second = request;
    EXPECT_NE (nullptr,
               std::get_if<object_reserved_t> (&store.reserve (second).result ().value ()));
}

TEST (ZLinkFrameworkInMemoryLocationStore, AggregateRequiresExactCapacityBundle)
{
    using namespace zlink::framework;
    in_memory_location_repository_t store;
    const auto owner_a = claim_owner (store, "owner-a");
    ...
    const auto create = [&] (placement_object_kind_t kind, std::string stable_type, std::string id,
                             std::byte marker) -> authority_snapshot_t {
        object_reserve_request_t request{
          .key = {kind, id},
          ...
          .capacity_bundle = kind == placement_object_kind_t::actor
                               ? placement_capacity_bundle_t{.actor_slots = 1}
                               : placement_capacity_bundle_t{
                                   .spot_slots = 1,
                                   .spot_type = spot_type_capacity_delta_t{
                                     .object_kind = kind, .stable_type = stable_type, .slots = 1}}};
        const auto reserved = store.reserve (request).result ().value ();
        const auto &fence = std::get<object_reserved_t> (reserved).fence;
        return std::get<object_committed_t> (
                 store.commit ({request.key, fence, {marker}}).result ().value ())
          .ready;
    };
    const auto first =
      create (placement_object_kind_t::user_spot, "room", "aggregate-room", std::byte{0x11});
    const auto second =
      create (placement_object_kind_t::actor, "player", "aggregate-actor", std::byte{0x12});

    aggregate_prepare_request_t request;
    request.aggregate_id.value[15] = std::byte{0x21};
    request.aggregate_generation = 1;
    request.participants = {{spot_authority_key ("aggregate-room"),
                             first.store_version,
    ...
    std::sort (
      ...
      [] (const auto &left, const auto &right) { return left.key.value < right.key.value; });
    request.target_descriptor = {.mesh_name = "play", .rid = zlink::routing_id_t::from ("node-b")};
    ...
    EXPECT_NE (nullptr, std::get_if<aggregate_prepare_conflict_t> (
    ...
    EXPECT_EQ (first.store_version,
               std::get<authority_snapshot_t> (
                 store.read_authority (spot_authority_key ("aggregate-room")).result ().value ())
                 .store_version);
    EXPECT_EQ (second.store_version,
               std::get<authority_snapshot_t> (
                 store.read_authority (actor_authority_key ("aggregate-actor")).result ().value ())
                 .store_version);

    store.release_owner_lease (owner_a).result ().value ();
    const auto prepared = store.prepare_aggregate (request).result ().value ();
    const auto *prepared_value = std::get_if<aggregate_prepared_t> (&prepared);
    ASSERT_NE (nullptr, prepared_value);
    EXPECT_EQ (aggregate_commit_result_t::committed,
               store.commit_aggregate (prepared_value->fence).result ().value ());
    EXPECT_EQ (owner_b.lease_generation,
               std::get<authority_snapshot_t> (
                 store.read_authority (spot_authority_key ("aggregate-room")).result ().value ())
...
TEST (ZLinkFrameworkInMemoryLocationStore, AggregateMovesCapacityOnce)
{
    using namespace zlink::framework;
    in_memory_location_repository_t store;
    ...
    const auto create = [&] (placement_object_kind_t kind, std::string stable_type, std::string id,
        ...
        request.capacity_bundle =
          kind == placement_object_kind_t::actor
            ? placement_capacity_bundle_t{.actor_slots = 1}
            : placement_capacity_bundle_t{.spot_slots = 1,
                                          .spot_type = spot_type_capacity_delta_t{
                                            placement_object_kind_t::user_spot, stable_type, 1}};
        const auto reserved = store.reserve (request).result ().value ();
        const auto *reservation = std::get_if<object_reserved_t> (&reserved);
        EXPECT_NE (reservation, nullptr);
        const auto committed =
          store.commit ({request.key, reservation->fence, {marker}}).result ().value ();
        return std::get<object_committed_t> (committed).ready;
    };

    const auto spot =
      create (placement_object_kind_t::user_spot, "room", "fenced-room", std::byte{0x11});
    const auto actor =
      create (placement_object_kind_t::actor, "player", "fenced-actor", std::byte{0x12});

    aggregate_prepare_request_t request;
    request.aggregate_id.value[15] = std::byte{3};
    request.aggregate_generation = 1;
    request.participants = {{actor_authority_key ("fenced-actor"),
                             actor.store_version,
                             authority_generation_transition_t::new_owner,
                             {std::byte{0x21}},
                             {}},
                            {spot_authority_key ("fenced-room"),
                             ...
                             {std::byte{0x22}},
                             {}}};
    request.target_descriptor = {"play", zlink::routing_id_t::from ("fence-target-node")};
    request.target_descriptor_lifecycle_generation = 1;
    request.capacity_bundle = {
      .actor_slots = 1,
      .spot_slots = 1,
    ...
    const auto target_before =
      std::find_if (before_commit.items.begin (), before_commit.items.end (),
                    [] (const auto &node) { return node.rid.to_string () == "fence-target-node"; });
    ...
    EXPECT_EQ (store.commit_aggregate (fence->fence).result ().value (),
               aggregate_commit_result_t::committed);
    const auto after_commit = store.list_mesh_nodes ("play").result ().value ();
    const auto target_after =
      std::find_if (after_commit.items.begin (), after_commit.items.end (),
                    [] (const auto &node) { return node.rid.to_string () == "fence-target-node"; });
      ...
      AggregateInventoryUsesBoundedLeafPagesWithoutTotalCountCap)
{
    ...
    for (std::size_t index = 0; index < tree->pages.size (); ++index) {
        ...
        const auto decoded = decode_page (tree->pages[index].encoded, index);
        ASSERT_TRUE (decoded);
        EXPECT_EQ (decoded->size (), tree->pages[index].participants.size ());
        EXPECT_FALSE (decode_page (tree->pages[index].encoded, index + 1));
    }
...
TEST (ZLinkFrameworkInMemoryLocationStore, MaintainsOwnerLeasesAndUsesPollingWithoutStampHint)
{
    ...
    EXPECT_EQ (owner.lease_generation, found->token.lease_generation);
    EXPECT_GT (found->lease_expires_at, found->store_now);

    const auto stamp = store.get_mesh_node_change_stamp ("play").result ().value ();
    EXPECT_FALSE (stamp.has_value ());
    const auto released = store.release_owner_lease (owner).result ().value ();
    EXPECT_TRUE (std::holds_alternative<zlink::framework::owner_lease_released_t> (released));
...
TEST (ZLinkFrameworkInMemoryLocationStore, FanoutPublisherRowsFenceIdentityRevisionAndCleanup)
{
    ...
    const fanout_publisher_descriptor_t original{.channel_name = "events",
                                                 .publisher_rid =
                                                   zlink::routing_id_t::from ("publisher-a"),
    ...
    EXPECT_EQ (location_write_status_t::stored,
               store.update_fanout_publisher (original, location_write_intent_t::new_claim)
                 .result ()
                 .value ()
                 .status);
    ...
    EXPECT_EQ (location_write_status_t::rejected_conflict,
               store.update_fanout_publisher (conflicting, location_write_intent_t::new_claim)
                 .result ()
                 .value ()
                 .status);
    ...
    EXPECT_EQ (location_write_status_t::rejected_conflict,
               store.update_fanout_publisher (immutable_change, location_write_intent_t::renew)
                 .result ()
                 .value ()
                 .status);
    ...
    EXPECT_EQ (location_write_status_t::stored,
               store.update_fanout_publisher (draining, location_write_intent_t::renew)
                 .result ()
                 .value ()
                 .status);
    auto same_revision_conflict = draining;
    same_revision_conflict.state = framework_runtime_state_t::serving;
    EXPECT_EQ (
      location_write_status_t::rejected_conflict,
      store.update_fanout_publisher (same_revision_conflict, location_write_intent_t::renew)
        .result ()
        .value ()
        .status);
    EXPECT_EQ (location_write_status_t::ignored_stale,
               store.update_fanout_publisher (original, location_write_intent_t::renew)
                 .result ()
                 .value ()
                 .status);

    EXPECT_NE (nullptr, std::get_if<zlink::framework::owner_lease_released_t> (
                          &store.release_owner_lease (owner_a).result ().value ()));
    ...
    EXPECT_EQ (location_write_status_t::stored,
               store.update_fanout_publisher (replacement, location_write_intent_t::new_claim)
                 .result ()
                 .value ()
                 .status);
    ...
    EXPECT_EQ (location_write_status_t::stored,
               store.update_fanout_publisher (second, location_write_intent_t::new_claim)
                 .result ()
                 .value ()
                 .status);

    const auto first_page =
      store.list_fanout_publishers ("events", location_page_request_t{.page_size = 1})
        .result ()
        .value ();
    ...
    EXPECT_EQ (location_write_status_t::ignored_stale,
               store
                 .remove_fanout_publisher (
                   fanout_publisher_descriptor_key_t{.channel_name = original.channel_name,
    ...
    EXPECT_EQ (location_write_status_t::stored,
               store
                 .remove_fanout_publisher (
                   fanout_publisher_descriptor_key_t{.channel_name = replacement.channel_name,
                                                     .publisher_rid = replacement.publisher_rid},
                   owner_b)
                 .result ()
                 .value ());
    EXPECT_EQ (1u, store.remove_all_by_owner (owner_c).result ().value ());
    EXPECT_TRUE (store.list_fanout_publishers ("events").result ().value ().items.empty ());
}

TEST (ZLinkFrameworkInMemoryLocationStore, FanoutPublisherPageStopsAtFourMiB)
{
    in_memory_location_repository_t store;
    const std::string maximal_escaped_text (255, '\x01');
    const auto owner = claim_owner (store, maximal_escaped_text, std::chrono::seconds (60));
    for (std::size_t index = 0; index < 1000; ++index) {
        const fanout_publisher_descriptor_t descriptor{
          .channel_name = maximal_escaped_text,
          .publisher_rid = zlink::routing_id_t::from ("page-publisher-" + std::to_string (index)),
          .lifecycle_generation = 1,
          .descriptor_revision = 1,
          .endpoint = maximal_escaped_text,
          .state = framework_runtime_state_t::serving,
          .security_identity = maximal_escaped_text,
          .owner_id = owner.owner_id,
          .lease_generation = owner.lease_generation};
        ASSERT_EQ (location_write_status_t::stored,
                   store.update_fanout_publisher (descriptor, location_write_intent_t::new_claim)
                     .result ()
                     .value ()
                     .status);
    }
    ...
    const auto first =
      store
        .list_fanout_publishers (maximal_escaped_text, location_page_request_t{.page_size = 1000})
        .result ()
        .value ();
    ...
    for (const auto &descriptor : first.items) {
        ...
        encoded_upper_bound +=
          512u + escaped_text_bytes * 6u + descriptor.publisher_rid.size () * 2u;
    }
    EXPECT_LE (encoded_upper_bound, 4u * 1024u * 1024u);

    const auto second = store
                          .list_fanout_publishers (
                          ...
                          .result ()
                          .value ();
    EXPECT_EQ (1000u, first.items.size () + second.items.size ());
    EXPECT_FALSE (second.continuation_token.has_value ());
    EXPECT_EQ (1000, store.remove_all_by_owner (owner).result ().value ());
}

TEST (ZLinkFrameworkInMemoryLocationStore, SignedWeightBoundariesAreValidatedWithoutNarrowing)
{
    in_memory_location_repository_t store;
    const auto owner = claim_owner (store, "weight-owner");

    auto upper = make_mesh_node ("weight-upper", owner);
    upper.channel_weights = {{"default", 10000}};
    upper.placement_weight = 10000;
    EXPECT_EQ (
      location_write_status_t::stored,
      store.update_mesh_node (upper, location_write_intent_t::new_claim).result ().value ().status);
    const auto listed =
      store.list_mesh_nodes ("play", location_page_request_t{.page_size = 1000}).result ().value ();
    ASSERT_EQ (1u, listed.items.size ());
    EXPECT_EQ (10000, listed.items.front ().placement_weight);
    EXPECT_EQ (10000, listed.items.front ().channel_weights.at ("default"));

    auto negative = make_mesh_node ("weight-negative", owner);
    negative.channel_weights = {{"default", -1}};
    EXPECT_THROW ((void) store.update_mesh_node (negative, location_write_intent_t::new_claim),
                  std::invalid_argument);

    auto over = make_mesh_node ("weight-over", owner);
    over.placement_weight = 10001;
    EXPECT_THROW ((void) store.update_mesh_node (over, location_write_intent_t::new_claim),
                  std::invalid_argument);

    zlink::framework::client_server_server_descriptor_t server{
      .channel_name = "orders",
      .server_rid = zlink::routing_id_t::from ("weight-server"),
      .lifecycle_generation = 1,
      .descriptor_revision = 1,
      ...
      .lease_generation = owner.lease_generation};
    EXPECT_EQ (location_write_status_t::stored,
               store.update_client_server (server, location_write_intent_t::new_claim)
                 .result ()
                 .value ()
                 .status);
    server.server_rid = zlink::routing_id_t::from ("weight-server-over");
    server.weight = 10001;
    EXPECT_THROW ((void) store.update_client_server (server, location_write_intent_t::new_claim),
                  std::invalid_argument);
}
