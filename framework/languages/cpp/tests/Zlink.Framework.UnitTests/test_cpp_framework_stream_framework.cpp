...
int main ()
{
    ...
    zlink::framework::runtime::stream_host_service_t mutual_tls_host (
      ...
      std::chrono::milliseconds{30'000}, nullptr, {}, mutual_tls_listeners);
    mutual_tls_host.start (mutual_tls_provider);
    const auto mutual_tls_endpoint =
      mutual_tls_listeners->find (zlink::framework::listener_kind_t::stream, "mutual-tls-listener")
        ->endpoint;
    const auto mutual_tls_port = static_cast<std::uint16_t> (
      std::stoi (mutual_tls_endpoint.substr (mutual_tls_endpoint.rfind (':') + 1)));
    ...
    const auto transport_endpoint =
      transport_listeners->find (zlink::framework::listener_kind_t::stream, "transport-stream")
        ->endpoint;
    const auto transport_port = static_cast<std::uint16_t> (
      std::stoi (transport_endpoint.substr (transport_endpoint.rfind (':') + 1)));
    ...
    zlink::framework::runtime::stream_host_service_t limited_host (
      ...
      std::chrono::milliseconds{30'000}, nullptr, {}, limited_listeners);
    limited_host.start (transport_provider);
    const auto limited_endpoint =
      limited_listeners->find (zlink::framework::listener_kind_t::stream, "limited-stream")
        ->endpoint;
    const auto limited_port = static_cast<std::uint16_t> (
      std::stoi (limited_endpoint.substr (limited_endpoint.rfind (':') + 1)));
    ...
    zlink::framework::runtime::stream_host_service_t rejected_host (
      ...
      std::chrono::milliseconds{30'000}, nullptr, {}, rejected_listeners);
    rejected_host.start (transport_provider);
    const auto rejected_endpoint =
      rejected_listeners->find (zlink::framework::listener_kind_t::stream, "rejected-stream")
        ->endpoint;
    const auto rejected_port = static_cast<std::uint16_t> (
      std::stoi (rejected_endpoint.substr (rejected_endpoint.rfind (':') + 1)));
    ...
    core_connector_options.endpoint = core_stream_endpoint;
    ...
    zlink::framework::runtime::stream_host_service_t error_close_host (
      ...
      std::chrono::milliseconds{30'000}, error_close_mesh, {}, error_close_listeners);
    error_close_host.start (error_close_provider);
    const auto error_close_endpoint =
      error_close_listeners->find (zlink::framework::listener_kind_t::stream, "error-close-stream")
        ->endpoint;
    const auto error_close_stream_port = static_cast<std::uint16_t> (
      std::stoi (error_close_endpoint.substr (error_close_endpoint.rfind (':') + 1)));
