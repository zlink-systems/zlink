...
std::vector<std::uint8_t> decode_hex_vector (std::string_view value)
{
    ...
    for (std::size_t index = 0; index != value.size (); index += 2)
        result.push_back (
          static_cast<std::uint8_t> ((nibble (value[index]) << 4u) | nibble (value[index + 1])));
    return result;
}

void test_spot_lifecycle_domain_rejects_invalid_kind_combinations (test_context_t &test)
{
    namespace detail = zlink::framework::detail;

    static_assert (!std::default_initializable<detail::spot_lifecycle_domain_t>);
    ...
    test.require (entry.is_entry () && !entry.allows_relocation ()
                    && !entry.allows_idle_eviction (),
                  "Entry Spot domain must exclude relocation and idle eviction");
    test.require (!user.is_entry () && user.allows_relocation () && !user.allows_idle_eviction (),
                  "User Spot domain must allow relocation but exclude idle eviction");
    test.require (!instance.is_entry () && instance.allows_relocation ()
...
bool wait_until_bounded (Predicate &&condition, std::chrono::milliseconds timeout)
{
...
void test_relocation_ready_completion_runs_once_on_spot_turn (test_context_t &test)
{
    ...
    state->lifecycle.on_relocation_ready_completed =
      [&] (void *, const zlink::framework::spot_relocation_ready_completion_t &completion) {
          outcomes.push_back (completion.outcome);
          completion_owned_spot_turn = state->owns_current_serial_turn ();
          completions.fetch_add (1);
      };
    auto context = detail::spot_context_access_t::create (state);
    auto manager_before_defer = context.manager ();
    auto worker_before_defer = context.run_cpu_worker ([] { return 7; });
    ...
    test.require (
      deferred && completions.load () == 1 && completion_owned_spot_turn && close_rejected
        && manager_rejected && worker_rejected && outbound_rejected
        && outcomes
             == std::vector<
               spot_relocation_ready_outcome_t>{spot_relocation_ready_outcome_t::continued},
    ...
    test.require (prepared_deferred && completions.load () == 2
                    && outcomes.back () == spot_relocation_ready_outcome_t::relocated,
...
void test_temporary_channel_request_yield_owns_call_state (test_context_t &test)
{
    ...
    const auto submitted = state->run_serial_sync ("temporary-channel-call-yield", [&] {
        auto pending =
          channel_request_call_t ("temporary.reply", &serializers,
    ...
    reply_source->complete (
      zlink::framework::result_t<zlink::message_t>::success (zlink::message_t::from ("451")));
...
void to_json (nlohmann::json &json, const accepted_decode_payload_t &payload)
{
    json = nlohmann::json{{"value", payload.value}};
}

void to_json (nlohmann::json &json, const rejected_decode_payload_t &payload)
{
    json = nlohmann::json{{"value", payload.value}};
}

void to_json (nlohmann::json &json, const failed_decode_payload_t &payload)
{
    json = nlohmann::json{{"value", payload.value}};
}

void from_json (const nlohmann::json &json, accepted_decode_payload_t &payload)
{
    ++successful_payload_deserializations;
    payload.value = json.at ("value").get<int> ();
}

void from_json (const nlohmann::json &json, rejected_decode_payload_t &payload)
{
    ++other_payload_deserializations;
    payload.value = json.at ("value").get<int> ();
}

void from_json (const nlohmann::json &, failed_decode_payload_t &)
{
    ++failed_payload_deserializations;
    throw std::runtime_error ("expected accepted-payload decode failure");
}

class payload_decode_spot_t final : public zlink::framework::spot_t<zlink::framework::actor_t>
{
  public:
    explicit payload_decode_spot_t (zlink::framework::spot_context_t context) :
        _context (std::move (context))
    {
    }
    ...
    on_create (const zlink::framework::message_t &request) override
    {
        if (!failure_mode) {
            ...
            const auto second = copied_request.decode<accepted_decode_payload_t> ();
            ...
            success_observed = first.value == 73 && second.value == 73 && other_rejected;
        } else {
            ...
            failure_observed = !first_error.empty () && repeated_error == first_error;
        }
...
void test_accepted_message_payload_is_deserialized_once (test_context_t &test)
{
    ...
    mesh.add_spot_factory<payload_decode_spot_t> (
      "decode-cache",
      [] (zlink::framework::spot_context_t context) {
          return std::make_shared<payload_decode_spot_t> (std::move (context));
      },
    ...
    auto found_runtime =
      zlink::framework::detail::spot_node_runtime_t::from (builder, "decode-cache-mesh");
    test.require (found_runtime.has_value (),
    ...
    const auto success =
      runtime.get_or_create_spot ("decode-cache", zlink::framework::spot_id_t ("decode-success"),
                                  zlink::message_t::from (R"({"value":73})"));
    test.require (success.state == zlink::framework::spot_create_state_t::created
                    && payload_decode_spot_t::success_observed
                    && successful_payload_deserializations == 1
                    && other_payload_deserializations == 0,
                  "one admitted payload must preserve its first successful decode across repeated "
                  "and differently typed reads");

    payload_decode_spot_t::failure_mode = true;
    const auto failure =
      runtime.get_or_create_spot ("decode-cache", zlink::framework::spot_id_t ("decode-failure"),
                                  zlink::message_t::from (R"({"value":91})"));
    test.require (failure.state == zlink::framework::spot_create_state_t::created
                    && payload_decode_spot_t::failure_observed
                    && failed_payload_deserializations == 1 && other_payload_deserializations == 0,
                  "one admitted payload must preserve its first decode failure without invoking "
                  "another serializer");
...
class fail_first_restore_spot_t final : public zlink::framework::spot_t<zlink::framework::actor_t>
{
  public:
    explicit fail_first_restore_spot_t (zlink::framework::spot_context_t context) :
        _context (std::move (context))
    {
    }
...
{
  public:
    zlink::framework::task_t<std::vector<std::byte>> capture (fail_first_restore_spot_t &,
                                                              std::stop_token) override
    {
        co_return std::vector<std::byte>{};
    }
    ...
    zlink::framework::task_t<void>
    restore (fail_first_restore_spot_t &, std::vector<std::byte> payload, std::stop_token) override
    {
...
class concurrent_restore_spot_t final : public zlink::framework::spot_t<zlink::framework::actor_t>
{
  public:
    explicit concurrent_restore_spot_t (zlink::framework::spot_context_t context) :
        _context (std::move (context))
    {
        ++factory_count;
    }
...
class concurrent_restore_adapter_t final
    : public zlink::framework::spot_relocation_adapter_t<concurrent_restore_spot_t>
{
  public:
    zlink::framework::task_t<std::vector<std::byte>> capture (concurrent_restore_spot_t &,
                                                              std::stop_token) override
    {
        co_return std::vector<std::byte>{};
    }
    ...
    zlink::framework::task_t<void>
    restore (concurrent_restore_spot_t &, std::vector<std::byte> payload, std::stop_token) override
    {
        concurrent_restore_spot_t::restore_count.fetch_add (1);
        std::unique_lock lock (concurrent_restore_spot_t::restore_mutex);
        ...
        concurrent_restore_spot_t::restore_condition.wait (
          lock, [] { return concurrent_restore_spot_t::release_restore; });
        concurrent_restore_spot_t::restored_payload = std::move (payload);
        co_return;
    }
};
    ...
    : public zlink::framework::runtime::in_memory_location_repository_t
{
  public:
    ...
    read_authority (zlink::framework::authority_key_t key, std::stop_token) override
    {
        ...
        if (!snapshot)
            return completed (zlink::framework::authority_read_result_t{
              zlink::framework::authority_missing_t{std::chrono::system_clock::now ()}});
        return completed (zlink::framework::authority_read_result_t{*snapshot});
    }
    ...
    {
        observed_keys.push_back (std::move (key.value));
        const auto *put = std::get_if<zlink::framework::authority_put_t> (&mutation);
        const auto *retarget = std::get_if<zlink::framework::authority_retarget_t> (&mutation);
        if (!snapshot || (!put && !retarget) || expected_store_version != snapshot->store_version)
        ...
        if (retarget) {
            ...
            snapshot->authority_owner_generation += retarget_authority_generation_advance;
    ...
    list_authorities (std::string,
                      std::optional<zlink::framework::authority_scan_cursor_t>,
                      std::size_t,
                      std::stop_token) override
    {
        return completed (
          zlink::framework::authority_scan_result_t{zlink::framework::authority_page_t{}});
    }
  ...
  public:
      ...
      std::vector<std::byte> payload, std::chrono::hours retention, std::stop_token) override
    {
        ...
        for (const auto value : payload)
            checksum_input.push_back (std::to_integer<std::uint8_t> (value));
        const auto reference = "public-root";
        roots[reference] = std::move (payload);
        return completed (zlink::framework::relocation_stored_t{
          reference, maintenance_runtime_t::crc32c (checksum_input),
          std::chrono::system_clock::now () + retention, std::chrono::system_clock::now ()});
    }

    zlink::framework::task_t<zlink::framework::relocation_read_result_t>
    get_relocation (std::string reference, std::stop_token) override
    {
        const auto found = roots.find (reference);
        if (found == roots.end ())
            return completed (
              zlink::framework::relocation_read_result_t{zlink::framework::relocation_missing_t{}});
        return completed (zlink::framework::relocation_read_result_t{
          zlink::framework::relocation_found_t{found->second}});
    }

    zlink::framework::task_t<zlink::framework::relocation_renew_result_t>
    renew_relocation (std::string reference, std::chrono::hours retention, std::stop_token) override
    {
        if (!roots.contains (reference))
            return completed (zlink::framework::relocation_renew_result_t{
              zlink::framework::relocation_renew_missing_t{}});
        const auto now = std::chrono::system_clock::now ();
        return completed (zlink::framework::relocation_renew_result_t{
          zlink::framework::relocation_renewed_t{now + retention, now}});
    }

    zlink::framework::task_t<zlink::framework::relocation_delete_result_t>
    delete_relocation (std::string reference, std::stop_token) override
    {
        return completed (roots.erase (reference) > 0
...
class memory_relocation_repository_t final : public relocation_store_port_t
{
  public:
    ...
    std::optional<std::vector<std::uint8_t>> get (const std::string &reference) override
    {
        std::lock_guard lock (mutex);
        const auto found = roots.find (reference);
        return found == roots.end () ? std::optional<std::vector<std::uint8_t>>{}
...
class memory_authority_store_t final : public authority_relocation_port_t
{
  public:
                                                                 ...
                                                                 const std::string &key)
    {
        ...
        return found == rows.end () ? std::optional<authority_relocation_reference_t>{}
                                    : std::make_optional (found->second);
    }
...
{
  public:
    explicit memory_aggregate_authority_t (std::shared_ptr<memory_authority_store_t> authority) :
        authority (std::move (authority))
    {
    }

    aggregate_publish_result_t prepare (const std::vector<object_ref_t> &sources,
                                        ...
                                        inventory_digest_t inventory_digest) override
    {
        ...
        for (const auto &source : sources) {
            ...
            pending.push_back ({.source = source,
                                .target = target,
        ...
        prepared = true;
        return {aggregate_publish_status_t::prepared, {++next_fence}, pending};
    }

    aggregate_publish_result_t commit (aggregate_relocation_fence_t fence) override
    {
        std::lock_guard lock (mutex);
        ++commit_count;
            ...
            return {aggregate_publish_status_t::conflict, fence, {}};
        {
            ...
            for (const auto &reference : pending) {
                authority->rows[authority_key (reference.source.kind, reference.source.key)] =
                  reference;
            }
        }
        prepared = false;
        return {aggregate_publish_status_t::committed, fence, pending};
    }
...
{
  public:
    target_preflight_result_t preflight (const std::vector<relocation_unit_t> &units) override
    {
        std::function<void ()> callback;
        {
            std::lock_guard lock (mutex);
        ...
        if (status != target_preflight_status_t::eligible)
            return {status, {}};
        target_preflight_result_t result{target_preflight_status_t::eligible, {}};
        for (std::size_t index = 0; index != units.size (); ++index) {
            result.units.push_back (
              {.unit = units[index],
               ...
               .inventory_digest = digest_with (static_cast<std::uint8_t> (index + 10))});
        }
        return result;
    }
    ...
    target_preflight_status_t status = target_preflight_status_t::eligible;
    int calls = 0;
};

object_ref_t
create_actor (stateful_object_runtime_t &runtime, std::string key, std::string node = "node-a")
{
    runtime.replace_placement_candidates ({{.mesh_name = "mesh",
                                            ...
                                            .pending_count = 0}});
    auto created = runtime.begin_create ({.kind = object_kind_t::actor,
                                          .key = std::move (key),
                                          .stable_type = "actor",
...
object_ref_t create_spot (stateful_object_runtime_t &runtime, object_kind_t kind, std::string key)
{
    runtime.replace_placement_candidates ({{.mesh_name = "mesh",
                                            .node_id = "node-a",
                                            ...
                                            .pending_capacity = 100,
                                            .pending_count = 0}});
    auto created = runtime.begin_create ({.kind = kind,
                                          .key = std::move (key),
                                          .stable_type = "spot",
                                          .mesh_name = std::optional<std::string>{"mesh"},
                                          .creation_request = {},
...
void test_generation_barrier_quiesces_yield_spot_and_timer (test_context_t &test)
{
    ...
    test.require (
      objects.enqueue (actor, turn_domain_t::application, {10, {10}}) == stateful_error_t::none
        && objects.enqueue (actor, turn_domain_t::application, {20, {20}}) == stateful_error_t::none
        && objects.enqueue (spot, turn_domain_t::application, {40, {40}}) == stateful_error_t::none,
    ...
    test.require (actor_claim_error == stateful_error_t::none && actor_claim
                    && actor_claim->sequence == 10 && spot_claim_error == stateful_error_t::none
                    && spot_claim && spot_claim->sequence == 40,
                  "Actor and Spot lanes must both be active before sealing");
    test.require (objects.yield_claim (actor, {11, {11}}) == stateful_error_t::none,
    ...
    std::thread sealing ([&] {
        auto result = await_task (objects.try_seal_relocation_aggregate ({actor, spot}));
    ...
    const bool sealed = wait_until_bounded (
      [&] { return objects.cancel_timer (actor, 999) == stateful_error_t::moving; },
      std::chrono::seconds (5));
    test.require (sealed && !seal_completed.load (std::memory_order_acquire),
                  "seal must close timer admission and wait for active lanes");
    test.require (objects.enqueue_timer_tick (actor, 9, {30}) == stateful_error_t::moving,
                  "timer dispatch must not mutate the sealed generation");

    const auto [continuation_error, continuation] =
      objects.try_claim (actor, turn_domain_t::application);
    test.require (continuation_error == stateful_error_t::none && continuation
                    && continuation->sequence == 11,
                  "yielded continuation must reacquire its Actor lane while sealed");
    test.require (objects.complete_claim (actor, turn_domain_t::application)
    ...
    test.require (seal_error == stateful_error_t::none && seal.participants.size () == 2
    ...
    test.require (objects.enqueue (actor, turn_domain_t::application, {21, {21}})
                    == stateful_error_t::none,
                  "sealed ingress must be retained for same-generation abort");
    test.require (objects.abort_relocation (seal.token) == stateful_error_t::none,
                  "same-generation abort must reopen the seal");
    test.require (objects.commit_relocation_aggregate (seal.token, "node-b").first
                    == stateful_error_t::not_found,
                  "stale commit must not mutate an aborted generation");

    const auto [first_error, first] = objects.try_claim (actor, turn_domain_t::application);
    test.require (first_error == stateful_error_t::none && first && first->sequence == 20,
                  "abort must restore the captured queue before held ingress");
    test.require (objects.complete_claim (actor, turn_domain_t::application)
                    == stateful_error_t::none,
                  "restored captured turn must complete");
    const auto [held_error, held] = objects.try_claim (actor, turn_domain_t::application);
    test.require (held_error == stateful_error_t::none && held && held->sequence == 21,
    ...
    test.require (second_error == stateful_error_t::none
                    && objects.abort_relocation (seal.token) == stateful_error_t::not_found,
                  "stale abort must not reopen a newer generation");
    test.require (objects.abort_relocation (second_seal.token) == stateful_error_t::none
                    && objects.enqueue (actor, turn_domain_t::application, {22, {22}})
                         == stateful_error_t::none,
...
void test_envelope_round_trip (test_context_t &test)
{
    ...
    admitted.body = protocol::frozen_spot_application_body_t{
      ...
      protocol::application_payload_t{
        "ChatRequest", "application/json", {'{', '"', 'i', 'd', '"', ':', '1', '}'}}};
    const auto canonical = protocol::encode_frozen_application_record (admitted);
    ...
    frozen_object_state_t frozen{
      ...
      .pending_application = {turn_record_t{
        4, canonical.canonical_bytes,
        protocol::application_payload_hwm_bytes (*canonical.application), std::nullopt, canonical}},
      .timers = {{11, 100, 50, 3}}};

    const protocol::relocation_id_t relocation{0, 9};
    const auto encoded = maintenance_runtime_t::encode_envelope ({frozen}, relocation);
    test.require (!encoded.empty (), "the schema relocation envelope must encode");
    const auto envelope = maintenance_runtime_t::decode_envelope (encoded);
    test.require (envelope.has_value (), "the schema relocation envelope must decode");
    test.require (
      envelope && envelope->relocation == relocation
        && envelope->object.kind == protocol::relocation_object_kind_t::user_spot
        && envelope->object.object_id == "spot-a" && envelope->object.object_generation == 3
        && envelope->object.expected_authority_owner_generation == 9
        && envelope->application_states.size () == 1 && envelope->saved_work.size () == 1
        && envelope->saved_work.front ().order == 4 && envelope->timer_registrations.size () == 1,
      "the stream must carry the declared schema sections");
    // Re-encoding the decoded model must be byte-exact.
    test.require (envelope && protocol::encode_relocation_envelope (*envelope) == encoded,
    ...
    const auto materialized = maintenance_runtime_t::materialize_envelope (
      *envelope,
      {relocation_participant_identity_t{frozen.owner, frozen.stable_type, std::nullopt}});
    test.require (materialized.has_value () && materialized->size () == 1
                    && materialized->front () == frozen,
                  "queue and timer state must round-trip through the schema stream");

    auto unordered = frozen;
    unordered.pending_application.push_back (unordered.pending_application.front ());
    test.require (maintenance_runtime_t::encode_envelope ({unordered}, relocation).empty (),
    ...
    frozen_object_state_t actor{.owner = {.kind = object_kind_t::actor,
                                          .key = "actor-b",
                                          .object_generation = 5,
                                ...
                                .pending_application = {},
                                .timers = {}};
    const auto aggregate_encoded =
      maintenance_runtime_t::encode_envelope ({frozen, actor}, relocation);
    test.require (!aggregate_encoded.empty (), "the aggregate schema envelope must encode");
    const auto aggregate_envelope = maintenance_runtime_t::decode_envelope (aggregate_encoded);
    test.require (aggregate_envelope && aggregate_envelope->application_states.size () == 2
    ...
    const auto aggregate_materialized = maintenance_runtime_t::materialize_envelope (
      *aggregate_envelope,
      {relocation_participant_identity_t{frozen.owner, frozen.stable_type, std::nullopt},
       relocation_participant_identity_t{actor.owner, actor.stable_type,
                                         std::pair{std::string ("spot-a"), std::uint64_t{3}}}});
    test.require (aggregate_materialized && aggregate_materialized->size () == 2
...
{
    namespace protocol = zlink::framework::runtime::protocol;
    const std::string handoff_id = "00112233445566778899aabbccddeeff";
    const auto relocation = protocol::actor_join_relocation_id (handoff_id);
    protocol::actor_join_recovery_t recovery{
      ...
      .relocation_content_type = std::string (protocol::actor_join_snapshot_content_type),
      .request_content_type = "application/json",
      .request = {'{', '}'},
      .reservation_token = handoff_id,
      .reserved_payload_bytes =
        protocol::actor_join_reserved_payload_bytes (2, protocol::actor_join_snapshot_content_type),
      ...
      .reply = {'[', ']'}};
    const auto frozen = protocol::encode_actor_join_recovery_saved_work (recovery);
    ...
    constexpr std::string_view node_zljr_frozen_hex =
      "0101001f06373337323633000000000000000d076f776e65722d6100000000000000110000000000000000000000"
      "0000000000000000000000000100"
      "000767225f5f7a6c696e6b2e6163746f722e726f757465645f6a6f696e2e7265636f76657279316170706c696361"
      "74696f6e2f782d7a6c696e6b2d61"
      "63746f722d726f757465642d6a6f696e2d7265636f766572792d76310000070e5a4c4a5201000006f90000000200"
      "0000027b2252657175657374223a"
      "7b224163746f724964223a226163746f722d61222c224163746f7254797065223a2273616d706c652e4163746f72"
      "222c2248616e646f66664964223a"
      "223030313132323333343435353636373738383939616162626363646465656666222c22426f756e645365737369"
      "6f6e4e6f6465526964223a6e756c"
      "6c2c22426f756e6453657373696f6e526964223a6e756c6c2c2252656c6f636174696f6e436f6e74656e74547970"
      "65223a226170706c69636174696f"
      "6e2f766e642e7a6c696e6b2e6163746f722d72656c6f636174696f6e2e736e617073686f74222c2252656c6f6361"
      "74696f6e5265666572656e636522"
      "3a2270656e64696e67222c2252656c6f636174696f6e436865636b73756d437263333263223a302c2252656c6f63"
      "6174696f6e416767726567617465"
      "4964223a2230303131323233332d343435352d363637372d383839392d616162626363646465656666222c225265"
      "6c6f636174696f6e416767726567"
      "61746547656e65726174696f6e223a312c2252656c6f636174696f6e496e76656e746f7279446967657374223a22"
      "4141414141414141414141414141"
      "41414141414141414141414141414141414141414141414141414141413d222c2252657175657374436f6e74656e"
      "7454797065223a226170706c6963"
      "6174696f6e2f6a736f6e222c2252657175657374223a22222c2248616e646f66664672616d6573223a5b5d2c2253"
      "6f7572636553706f744964223a22"
      "736f757263652d73706f74222c22536f757263654e6f6465526964223a2263334a6a222c224163746f7247656e65"
      "726174696f6e223a372c22416374"
      "6f72417574686f726974794f776e657247656e65726174696f6e223a31312c22426f756e6453657373696f6e4269"
      "6e64696e67546f6b656e223a6e75"
      "6c6c2c22426f756e6453657373696f6e42696e64696e6747656e65726174696f6e223a302c22426f756e64536573"
      "73696f6e4f626a65637447656e65"
      "726174696f6e223a302c22426f756e6453657373696f6e417574686f726974794f776e657247656e65726174696f"
      "6e223a302c22426f756e64536573"
      "73696f6e4d6573684e616d65223a6e756c6c2c22426f756e6453657373696f6e5461726765744e6f646547656e65"
      "726174696f6e223a302c22426f75"
      "6e6453657373696f6e4f776e65724c6561736547656e65726174696f6e223a302c22426f756e6453657373696f6e"
      "4f776e65724e6f646547656e6572"
      "6174696f6e223a302c22426f756e6453657373696f6e4163636570746564486967685761746572223a302c22426f"
      "756e6453657373696f6e53657373"
      "696f6e4f776e65724964223a6e756c6c2c22426f756e6453657373696f6e53657373696f6e4f776e65724c656173"
      "6547656e65726174696f6e223a30"
      "2c225265736572766174696f6e546f6b656e223a2230303131323233333434353536363737383839396161626263"
      "63646465656666222c2252657365"
      "727665645061796c6f61644279746573223a38333935313631382c225461726765744e6f6465526964223a225a48"
      "4e30222c225461726765744e6f64"
      "6547656e65726174696f6e223a31392c2254617267657453706f7447656e65726174696f6e223a32332c22546172"
      "676574417574686f726974794f77"
      "6e657247656e65726174696f6e223a31322c2254617267657453706f74417574686f726974794f776e657247656e"
      "65726174696f6e223a32392c2252"
      "656c6f636174696f6e436f6f7264696e61746f724f776e65724964223a226f776e65722d61222c2252656c6f6361"
      "74696f6e436f6f7264696e61746f"
      "724c6561736547656e65726174696f6e223a31372c2252656c6f636174696f6e436f6f7264696e61746f724e6f64"
      "65526964223a2263334a6a222c22"
      "52656c6f636174696f6e436f6f7264696e61746f724e6f646547656e65726174696f6e223a31332c2252656c6f63"
      "6174696f6e436f6f7264696e6174"
      "6f724578706563746564417574686f7269747953746f726556657273696f6e223a2273746f72652d7631222c2241"
      "63746f724e6f646547656e657261"
      "74696f6e223a31332c2245787065637465644f776e65724c6561736547656e65726174696f6e223a31377d2c2254"
      "617267657453706f744964223a22"
      "7461726765742d73706f74222c225461726765744e6f6465526964223a225a484e30222c225461726765744e6f64"
      "6547656e65726174696f6e223a31"
      "392c2254617267657453706f7447656e65726174696f6e223a32332c22546172676574417574686f726974794f77"
      "6e657247656e65726174696f6e22"
      "3a31322c224f7065726174696f6e496448696768223a33312c224f7065726174696f6e49644c6f77223a33372c22"
      "5265706c79436f6e74656e745479"
      "7065223a226170706c69636174696f6e2f6a736f6e222c225265706c79223a22227d7b7d5b5d";
    test.require (frozen.canonical_bytes == decode_hex_vector (node_zljr_frozen_hex),
                  "ZLJR saved-work must match the fixed Node byte vector");
    const auto decoded = protocol::decode_actor_join_recovery_saved_work (frozen);
    test.require (decoded && decoded->actor_id == recovery.actor_id
                    && decoded->actor_type == recovery.actor_type
                    && decoded->handoff_id == handoff_id && decoded->relocation == relocation
                    && decoded->source_spot_id == recovery.source_spot_id
                    && decoded->source_node_routing_id == recovery.source_node_routing_id
                    && decoded->target_spot_id == recovery.target_spot_id
                    && decoded->target_node_routing_id == recovery.target_node_routing_id
                    && decoded->coordinator == recovery.coordinator
                    && decoded->request == recovery.request && decoded->reply == recovery.reply,
    ...
    const auto independently_issued = protocol::decode_actor_join_recovery_saved_work (
      protocol::encode_actor_join_recovery_saved_work (independent_recovery));
    test.require (
      independently_issued && independently_issued->relocation == independent_recovery.relocation
        && independently_issued->handoff_id == independent_recovery.handoff_id
        && independently_issued->reservation_token == independent_recovery.reservation_token
    ...
    frozen_object_state_t actor{.owner = {object_kind_t::actor, "actor-a", 7, 11, "mesh", "src"},
                                .stable_type = "sample.Actor",
                                .application_state = {1, 2, 3},
                                .pending_application = {turn_record_t{
                                  1, frozen.canonical_bytes, std::nullopt, std::nullopt, frozen}},
                                .timers = {}};
    const auto envelope_bytes = maintenance_runtime_t::encode_envelope ({actor}, relocation, 42);
    const auto envelope = maintenance_runtime_t::decode_envelope (envelope_bytes);
    test.require (
      envelope && envelope->application_version == 42 && envelope->saved_work.size () == 1
        && protocol::decode_actor_join_recovery_saved_work (envelope->saved_work.front ().record)
             .has_value (),
      "Join relocation must carry descriptor applicationVersion and one canonical ZLJR record");
}

void test_spot_restore_stages_before_publication (test_context_t &test)
{
    ...
    builder.add_spot_factory<fail_first_restore_spot_t> (
      ...
      [] (zlink::framework::spot_context_t context) {
          ...
          return std::make_shared<fail_first_restore_spot_t> (std::move (context));
      },
      [] (auto &factory) {
          factory.template preserve_state_with<fail_first_restore_adapter_t> ();
      });
    auto runtime = zlink::framework::detail::spot_node_runtime_t::from (builder);
    const frozen_object_state_t frozen{.owner = {.kind = object_kind_t::user_spot,
                                                 .key = "staged-spot",
                                                 .object_generation = 7,
                                                 .authority_owner_generation = 9,
                                                 .mesh_name = "mesh",
                                       ...
                                       .pending_application = {},
                                       .timers = {}};
    const object_ref_t target{.kind = object_kind_t::user_spot,
                              .key = "staged-spot",
                              .object_generation = 7,
    ...
    oversized.application_state.assign (64u * 1024u * 1024u + 1u, 0x5a);
    test.require (!runtime.restore_spot_relocation_state (oversized, target)
                    && fail_first_restore_spot_t::factory_count == 0
                    && fail_first_restore_spot_t::restore_count == 0,
                  "oversized state must be rejected before Spot materialization");

    test.require (!runtime.restore_spot_relocation_state (frozen, target)
                    && !runtime.find_spot (zlink::framework::spot_id_t ("staged-spot"))
...
void test_concurrent_spot_restore_owns_one_reservation (test_context_t &test)
{
    concurrent_restore_spot_t::factory_count.store (0);
    concurrent_restore_spot_t::restore_count.store (0);
    {
        std::lock_guard lock (concurrent_restore_spot_t::restore_mutex);
    ...
    builder.add_spot_factory<concurrent_restore_spot_t> (
      "concurrent-spot",
      [] (zlink::framework::spot_context_t context) {
          return std::make_shared<concurrent_restore_spot_t> (std::move (context));
      },
      [] (auto &factory) {
          factory.template preserve_state_with<concurrent_restore_adapter_t> ();
      });
    auto runtime = zlink::framework::detail::spot_node_runtime_t::from (builder);
    const frozen_object_state_t frozen{.owner = {.kind = object_kind_t::user_spot,
                                                 .key = "concurrent-id",
    ...
    bool first = false;
    std::thread owner ([&] { first = runtime.restore_spot_relocation_state (frozen, target); });
    {
        std::unique_lock lock (concurrent_restore_spot_t::restore_mutex);
        concurrent_restore_spot_t::restore_condition.wait (
          lock, [] { return concurrent_restore_spot_t::restore_entered; });
    }
    ...
    std::thread contender ([&] {
        ...
        const auto activated = runtime.get_or_create_spot (
          "concurrent-spot", zlink::framework::spot_id_t ("concurrent-id"));
        contender_observed_existing =
          activated.state == zlink::framework::spot_create_state_t::existing;
    });
    while (!contender_started.load ())
        std::this_thread::yield ();
    std::this_thread::sleep_for (std::chrono::milliseconds (10));
    test.require (concurrent_restore_spot_t::factory_count.load () == 1
    ...
    test.require (
      first && contender_observed_existing && concurrent_restore_spot_t::factory_count.load () == 1
        && concurrent_restore_spot_t::restore_count.load () == 1
        && concurrent_restore_spot_t::restored_payload == std::vector<std::byte>{std::byte{1}}
        && runtime.find_spot (zlink::framework::spot_id_t ("concurrent-id")),
...
void test_restore_validates_generation_before_spot_publication (test_context_t &test)
{
    stateful_object_runtime_t single;
    const auto old_spot = create_spot (single, object_kind_t::user_spot, "stale-single");
    test.require (single.close_spot (old_spot) == std::pair{stateful_error_t::none, true},
    ...
    auto single_frozen = frozen_object_state_t{.owner = {.kind = object_kind_t::user_spot,
                                                         .key = "stale-single",
                                                         .object_generation = 1,
                                                         .authority_owner_generation = 1,
                                                         .mesh_name = "mesh",
    ...
    const auto holder = create_spot (aggregate, object_kind_t::user_spot, "entry:holder");
    const auto old_actor = create_actor (aggregate, "stale-actor");
    const auto [join_error, join] = aggregate.begin_membership_move (old_actor, holder);
    const auto [commit_error, joined_actor] = aggregate.commit_membership_move (join);
    test.require (join_error == stateful_error_t::none && commit_error == stateful_error_t::none
                    && aggregate.destroy_actor (joined_actor) == stateful_error_t::none
                    && aggregate.close_spot (holder) == std::pair{stateful_error_t::none, true},
    ...
    std::vector<frozen_object_state_t> participants{{.owner = {.kind = object_kind_t::user_spot,
                                                               .key = "aggregate-spot",
                                                               .object_generation = 1,
                                                               .authority_owner_generation = 1,
                                                               .mesh_name = "mesh",
                                                    ...
                                                    {.owner = {.kind = object_kind_t::actor,
                                                     ...
                                                     .pending_application = {},
                                                     .timers = {}}};
    std::vector<object_ref_t> aggregate_targets{participants[0].owner, participants[1].owner};
    ...
    test.require (fresh_aggregate == stateful_error_t::none && aggregate_restore_count == 1
                    ...
                    && std::any_of (aggregate_inventory.begin (), aggregate_inventory.end (),
...
/* 28 §2/§8 return relocation. A node that hands an object away keeps its
 * record in `moving` forever — `finalize_relocation_cutover` never removes it
 * and no source-cleanup path does either. When the object relocates back, the
 * incoming restore carries a Store-validated authority generation strictly
 * newer than the remnant's, so the remnant must be replaced, never allowed to
 * veto. Failure-first: a restore that fails after replacing the remnant must
 * leave the node clean enough for the next attempt to be admitted on its own
 * (15 §4.2), or the object is wedged off this node permanently. */
void test_return_relocation_replaces_departed_remnant (test_context_t &test)
{
    stateful_object_runtime_t objects;
    bool restore_succeeds = false;
    ...
    const auto spot = create_spot (objects, object_kind_t::user_spot, "returning-spot");

    auto sealed = await_task (objects.try_seal_relocation_aggregate ({spot}));
    test.require (sealed.error == stateful_error_t::none, "departing Spot must seal");
    test.require (objects.begin_relocation_boundary (sealed.seal.token).first
                    == stateful_error_t::none,
                  "departing Spot must cross the ingress boundary");
    test.require (objects.finalize_relocation_cutover (sealed.seal.token) == stateful_error_t::none,
                  "cutover must finalize and leave the source record behind");
    test.require (objects.inventory ().size () == 1,
                  "the departed record stays on the source as a `moving` remnant");

    frozen_object_state_t frozen{.owner = spot,
                                 .stable_type = "spot",
                                 .application_state = {1},
                                 .pending_application = {},
                                 .timers = {}};
    frozen.owner.node_id = "node-b";
    auto returning = spot;
    returning.authority_owner_generation = spot.authority_owner_generation + 2;
    const relocation_restore_identity_t identity{"return-root", 4, digest_with (4)};

    /* A restore that is NOT newer than the remnant is still a real conflict. */
    auto not_newer = returning;
    ...
    test.require (retried == stateful_error_t::none && returned_inventory.size () == 1
                  ...
                  "the return retry must replace the remnant instead of wedging");
}

void test_pending_restore_holds_ingress_before_rollback (test_context_t &test)
{
    ...
    const auto ingress = target.enqueue (restored_target, turn_domain_t::application, {1, {9}});
    {
    ...
    callback_condition.notify_all ();
    restoring.join ();
    test.require (ingress == stateful_error_t::none && restore_result == stateful_error_t::conflict
...
void test_host_preflight_is_all_or_none (test_context_t &test)
{
    ...
    auto aggregates = std::make_shared<memory_aggregate_authority_t> (authority);
    ...
    maintenance_runtime_t relocation (
      objects, maintenance_provider_set_t{authority, aggregates, roots, targets});
    stream_session_registry_t sessions (
      [&] (const std::string &key) { return objects.find (object_kind_t::actor, key); });
    host_maintenance_runtime_t host (objects, sessions, relocation, targets);
    host.mark_serving ();
    const auto result = await_task (host.terminate (termination_intent_t::retire));
    test.require (result
                    == termination_result_t{termination_intent_t::retire,
    ...
    test.require (roots->roots.empty ()
                    && objects.enqueue (actor, turn_domain_t::application, {1, {1}})
                         == stateful_error_t::none,
                  "failed preflight must not seal an object or write relocation data");
    const auto after_blocked =
      objects.begin_create ({.kind = object_kind_t::actor,
                             .key = "after-blocked",
                             .stable_type = "actor",
                             .mesh_name = std::optional<std::string>{"mesh"},
                             .creation_request = {},
                             .exclusive = true,
                             .instance_intent = false});
    test.require (host.state () == maintenance_admission_state_t::serving
...
void test_shutdown_wins_during_retire_preflight (test_context_t &test)
{
    stateful_object_runtime_t objects;
    ...
    std::thread shutdown (
      [&] { shutdown_result = await_task (host.terminate (termination_intent_t::shutdown)); });
    while (host.intent_snapshot ()
           != std::optional<termination_intent_t>{termination_intent_t::shutdown}) {
        std::this_thread::yield ();
    }
    {
    ...
    retire.join ();
    shutdown.join ();
    const termination_result_t expected{termination_intent_t::shutdown,
                                        termination_outcome_t::stopped, termination_reason_t::none};
    test.require (retire_result == expected && shutdown_result == expected,
                  "Shutdown seal claim during Retire preflight must win for all waiters");
    test.require (roots->roots.empty () && aggregates->commit_count == 0,
                  "winning Shutdown must not start continuity relocation");
    test.require (observations == 1,
...
void test_public_relocation_store_adapter (test_context_t &test)
{
    ...
    test.require (adapter.get (stored.reference)
...
void test_public_authority_store_adapter (test_context_t &test)
{
    ...
    const auto source_application_payload =
      zlink::framework::runtime::encode_actor_authority_payload (
        {.state = zlink::framework::runtime::actor_authority_state_t::ready,
         ...
         .node_generation = 16});
    store.snapshot = zlink::framework::authority_snapshot_t{
      "1", source_application_payload, 7, 11, {"owner-a", 3}, std::chrono::system_clock::now ()};
    public_authority_store_adapter_t adapter (store);
    const object_ref_t source{object_kind_t::actor, "actor-public", 7, 11, "mesh", "node-a"};
    const zlink::framework::location_owner_token_t target_owner{"owner-b", 5};
    const zlink::framework::object_creation_target_t target_placement{
      "mesh-b", zlink::framework::node_rid_t::from_string ("node-b"), 17, target_owner};
    auto target = source;
    ...
    test.require (
      published.status == authority_publish_status_t::published && published.current
    ...
    test.require (store.snapshot && !begins_with_zlra3 (store.snapshot->payload)
                    && stored_projection && stored_projection->actor.object_generation () == 7
                    && stored_projection->actor.actor_id ().value () == "actor-public"
                    && stored_projection->actor.node_rid ().value () == "node-b",
                  "remote actor commit must store a canonical authority row resolvable by "
                  "actor_directory_t::find");
    test.require (
      store.observed_target_owner && store.observed_target_owner->owner_id == "owner-b"
        && store.observed_target_owner->lease_generation == 5 && store.observed_target_placement
        ...
        && store.snapshot->owner.owner_id == "owner-b" && !store.observed_keys.empty ()
        && std::all_of (store.observed_keys.begin (), store.observed_keys.end (),
                        [] (const auto &key) { return key == "zla1:a:12:actor-public"; }),
      "public authority adapter must pass the exact target placement");
    const auto read = adapter.read (object_kind_t::actor, "actor-public");
    test.require (read && read->target.key == "actor-public" && read->target.object_generation == 7
                    && read->target.node_id == "node-b" && read->target_owner.lease_generation == 5,
                  "public authority adapter must decode its canonical actor authority row");

    const std::vector<std::byte> application_payload{std::byte{0x31}, std::byte{0x32}};
    store.snapshot = zlink::framework::authority_snapshot_t{
      "10", application_payload, 7, 12, {"owner-b", 5}, std::chrono::system_clock::now ()};
    const auto completion_published = adapter.publish_completion (
      object_kind_t::actor, "actor-public", "mesh", 7, "completion-prepared", 51);
    const auto completion_replaced =
      adapter.replace_completion (object_kind_t::actor, "actor-public", 7, "completion-prepared",
                                  51, "completion-delivered", 52);
    const auto completion_read = adapter.read (object_kind_t::actor, "actor-public");
    test.require (
      completion_published.status == authority_publish_status_t::published
        && completion_replaced.status == authority_publish_status_t::published && completion_read
        && completion_read->relocation_reference == "completion-delivered"
        && completion_read->checksum_crc32c == 52 && completion_read->source.object_generation == 7
        && completion_read->source.authority_owner_generation == 12,
      "completion cursor roots must use exact preserve-generation authority CAS");
    const auto completion_released = adapter.release_completion (
      object_kind_t::actor, "actor-public", 7, "completion-delivered", 52);
    test.require (completion_released && store.snapshot
...
void test_source_relocation_failure_stops_state_chunks_and_unseals (test_context_t &test)
{
    ...
    eligible_relocation_unit_t::canonical_wire_context_t wire{
      ...
      .send_state_chunk = [&] (const protocol::relocation_state_t &) -> framework::task_t<bool> {
          ++sent_chunks;
          if (sent_chunks == 1) {
              target_terminal->complete (framework::result_t<relocation_reason_t>::success (
                relocation_reason_t::checksum_mismatch));
          }
          co_return true;
      },
      .send_relocation_data = [] (const std::vector<protocol::relocation_data_t> &,
    ...
    const auto result = await_task (relocation.relocate (
      actor, "target-node", {"target-owner", 9}, 1024 * 1024, digest_with (0x31), wire, {}, 16));
    const auto source_resumed = objects.enqueue (actor, turn_domain_t::application, {1, {0x41}});
    test.require (planned_chunks > 1 && sent_chunks == 1
...
void test_application_relocation_remote_production_path (test_context_t &test)
{
    ...
    source.configure_stateful_dispatch ([] (const accepted_record_authority_query_t &query)
        ...
        return accepted_record_authority_t{
          {"target-owner", 9, query.source_node_routing_id, query.source_node_generation}, 1};
    });
    target.configure_stateful_dispatch ([] (const accepted_record_authority_query_t &query)
                                          -> std::optional<accepted_record_authority_t> {
        return accepted_record_authority_t{
          {"source-owner", 1, query.source_node_routing_id, query.source_node_generation}, 9};
    });
    source.configure_session_route_owner (
      [] { return std::optional<framework::location_owner_token_t>{{"source-owner", 1}}; });
    ...
    session_owner.configure_session_route_owner ([&target, &route_owner_observed_target_ready] {
        if (target.native_node ().objects ().find (object_kind_t::actor,
                                                   "production-remote-actor")) {
            route_owner_observed_target_ready.store (true, std::memory_order_release);
        }
        return std::optional<framework::location_owner_token_t>{{"session-owner", 17}};
    });
    source.start ();
    target.start ();
    session_owner.start ();
    source.connect_peer (target.status ().routing_id (), target.status ().local_endpoint ());
    source.connect_peer (session_owner.status ().routing_id (),
                         session_owner.status ().local_endpoint ());
    target.connect_peer (session_owner.status ().routing_id (),
                         session_owner.status ().local_endpoint ());
    ...
    while ((!source.has_admitted_peer (target.status ().routing_id (),
            ...
            || !source.has_admitted_peer (session_owner.status ().routing_id (),
                                          session_owner.status ().lifecycle_generation ())
            || !session_owner.has_admitted_peer (source.status ().routing_id (),
                                                 source.status ().lifecycle_generation ())
            || !target.has_admitted_peer (session_owner.status ().routing_id (),
                                          session_owner.status ().lifecycle_generation ())
            || !session_owner.has_admitted_peer (target.status ().routing_id (),
                                                 target.status ().lifecycle_generation ()))
        ...
        (void) session_owner.dispatch_ready ([] (const auto &, const auto &, auto) {});
        std::this_thread::sleep_for (1ms);
    }
    test.require (source.has_admitted_peer (target.status ().routing_id (),
                                            target.status ().lifecycle_generation ())
                    && source.has_admitted_peer (session_owner.status ().routing_id (),
                                                 session_owner.status ().lifecycle_generation ())
                    && target.has_admitted_peer (session_owner.status ().routing_id (),
    ...
    const auto created = source.create_application_actor (
      "production.actor", "production-remote-actor", std::nullopt, 1s);
    test.require (static_cast<bool> (created),
    ...
    framework::authority_snapshot_t snapshot{
      ...
      .allocation = {
        ...
        {"production-relocation-mesh",
         framework::node_rid_t::from_string (source.status ().routing_id ().to_string ()),
         source.status ().lifecycle_generation (),
         {"source-owner", 1}},
        {1, 0, std::nullopt}}};
    framework::mesh_node_descriptor_t target_descriptor;
    target_descriptor.mesh_name = "production-relocation-mesh";
    ...
    const auto bound_source_object = source.native_node ().resolve_actor (actor);
    test.require (bound_source_object.has_value (),
    ...
    const auto session_connection = session_owner.native_node ().sessions ().open (
      zlink::routing_id_t::from ("production-bound-session").to_hex ());
    const auto [session_bind_error, session_binding] =
      session_owner.native_node ().sessions ().bind_remote (
        session_connection, *bound_source_object, source.status ().lifecycle_generation (), 1);
    const auto [initial_inbound_error, initial_inbound] =
      session_owner.native_node ().sessions ().admit_inbound (session_binding);
    test.require (
      session_bind_error == stateful_error_t::none
        && initial_inbound_error == stateful_error_t::none && initial_inbound
    ...
    const auto initial_inbound_completed =
      initial_inbound
      && session_owner.native_node ().sessions ().complete_inbound (*initial_inbound)
           == stateful_error_t::none;
    test.require (initial_inbound_completed,
    ...
    };
    {
        ...
        authority->participant_identities = {relocation_participant_identity_t{
    ...
    while (std::chrono::steady_clock::now () < route_deadline) {
        const auto current =
          session_owner.native_node ().sessions ().current_binding ("production-remote-actor");
        if (current && current->actor.node_id == target.status ().routing_id ().to_string ())
            break;
        std::this_thread::sleep_for (1ms);
    }
    ...
    const auto [continued_inbound_error, continued_inbound] =
      committed_binding
        ? session_owner.native_node ().sessions ().admit_inbound (*committed_binding)
        : std::pair{stateful_error_t::not_found, std::optional<stream_dispatch_t>{}};
    const auto continued_inbound_completed =
      continued_inbound
      && session_owner.native_node ().sessions ().complete_inbound (*continued_inbound)
           == stateful_error_t::none;
    ...
    object_ref_t expected_target{
      ...
      actor.object_generation (),   2,
      "production-relocation-mesh", target.status ().routing_id ().to_string ()};
    const auto restored_target = target.native_node ().objects ().find (
      object_kind_t::actor, std::string (actor.actor_id ().value ()));
    test.require (
      result.terminal == relocation_terminal_t::completed && !result.authority
        && result.target_handoff
        && result.target_handoff->sources == std::vector<object_ref_t>{*bound_source_object}
        && result.target_handoff->target_node_id == target.status ().routing_id ().to_string ()
        && restored_target == std::optional<object_ref_t>{expected_target}
        && roots->roots.size () == 1
        && route_owner_observed_target_ready.load (std::memory_order_acquire) && committed_binding
      ...
      "Session route, and continue ingress");
    test.require (
      source_relocation_gate == relocation_gate_snapshot_t{},
      "cutover submit terminal must release relocation execution permits while post-cutover "
      "retransmission retention remains active");
    source.stop ();
    target.stop ();
    session_owner.stop ();
}

class aggregate_materialized_actor_t final : public zlink::framework::actor_t
{
  public:
    explicit aggregate_materialized_actor_t (zlink::framework::actor_context_t context) :
        _context (std::move (context))
    {
    }
...
class aggregate_materialized_actor_factory_t final
    : public zlink::framework::actor_factory_t<aggregate_materialized_actor_t>
{
  public:
    zlink::framework::task_t<std::shared_ptr<aggregate_materialized_actor_t>>
    create (zlink::framework::actor_context_t context, std::stop_token) override
    {
        ...
        co_return std::make_shared<aggregate_materialized_actor_t> (std::move (context));
    }
...
class aggregate_materialized_actor_adapter_t final
    : public zlink::framework::actor_relocation_adapter_t<aggregate_materialized_actor_t>
{
  public:
    zlink::framework::task_t<std::vector<std::byte>> capture (aggregate_materialized_actor_t &actor,
                                                              std::stop_token) override
    {
    ...
    zlink::framework::task_t<void> restore (aggregate_materialized_actor_t &actor,
                                            std::vector<std::byte> payload,
                                            std::stop_token) override
    {
        actor.value = payload.empty () ? -1 : std::to_integer<int> (payload.front ());
        restored_value.store (actor.value, std::memory_order_release);
        restore_order.store (++aggregate_materialized_actor_factory_t::sequence,
                             std::memory_order_release);
        co_return;
    }
...
class aggregate_materialized_spot_t final
    : public zlink::framework::spot_t<aggregate_materialized_actor_t>
{
  public:
    explicit aggregate_materialized_spot_t (zlink::framework::spot_context_t context) :
        _context (std::move (context))
    {
    }
    ...
    zlink::framework::spot_context_t &context () noexcept override { return _context; }
    const zlink::framework::spot_context_t &context () const noexcept override { return _context; }
    void configure () override { configure_lifecycle_probe (); }

    struct lifecycle_probe_message_t
    {
    };

    zlink::framework::task_t<void> lifecycle_probe (aggregate_materialized_actor_t &,
    ...
    {
        _context.handlers ().add_actor_send<&aggregate_materialized_spot_t::lifecycle_probe> (
    ...
    zlink::framework::task_t<void> on_actor_joined (aggregate_materialized_actor_t &actor) override
    {
        ...
        joined_held_ingress.store (
          target_objects && target_actor
            && target_objects->enqueue (*target_actor, turn_domain_t::application, {201, {0x51}})
                 == stateful_error_t::none,
          std::memory_order_release);
        joined_order.store (++aggregate_materialized_actor_factory_t::sequence,
                            std::memory_order_release);
        co_return;
    }
    zlink::framework::task_t<void> on_leave_actor (aggregate_materialized_actor_t &) override
    {
        co_return;
    }
    ...
    {
        ready_after_cas.store (completion.outcome
        ...
        ready_before_dispatch.store (
          target_objects && target_actor
            && target_objects->try_claim (*target_actor, turn_domain_t::application).first
                 == stateful_error_t::moving,
          std::memory_order_release);
        ready_order.store (++aggregate_materialized_actor_factory_t::sequence,
                           std::memory_order_release);
        co_return;
    }
...
class aggregate_materialized_spot_adapter_t final
    : public zlink::framework::spot_relocation_adapter_t<aggregate_materialized_spot_t>
{
  public:
    zlink::framework::task_t<std::vector<std::byte>> capture (aggregate_materialized_spot_t &spot,
                                                              std::stop_token) override
    {
        capture_count.fetch_add (1, std::memory_order_acq_rel);
        co_return std::vector<std::byte>{static_cast<std::byte> (spot.value)};
    }
    zlink::framework::task_t<void> restore (aggregate_materialized_spot_t &spot,
                                            std::vector<std::byte> payload,
                                            std::stop_token) override
    {
        ...
        if (aggregate_materialized_actor_factory_t::record_target.load (
            ...
            aggregate_materialized_spot_t::restore_order.store (
              ++aggregate_materialized_actor_factory_t::sequence, std::memory_order_release);
        }
        co_return;
    }
...
void test_application_user_spot_aggregate_remote_production_path (test_context_t &test)
{
    using namespace std::chrono_literals;
    namespace detail = zlink::framework::detail;
    namespace framework = zlink::framework;
    ...
    const auto make_state = [core_context] (const std::string &rid) {
        ...
        state->listen_endpoint = "tcp://127.0.0.1:0";
        state->routing_id = zlink::routing_id_t::from (rid);
        state->spot_builder.add_spot_factory<aggregate_materialized_spot_t> (
          "production.aggregate.spot",
          [] (framework::spot_context_t context) {
              return std::make_shared<aggregate_materialized_spot_t> (std::move (context));
          },
          [] (auto &factory) {
        ...
        state->spot_builder.add_actor_factory<aggregate_materialized_actor_t> (
          "production.aggregate.actor", std::make_shared<aggregate_materialized_actor_factory_t> (),
          [] (auto &factory) {
              factory.template preserve_state_with<aggregate_materialized_actor_adapter_t> ();
          });
        return state;
    };
    ...
    const auto configure_materialization =
      [authority] (detail::mesh_node_runtime_t &node,
                   const std::shared_ptr<detail::spot_node_builder_state_t> &spot_state) {
          detail::spot_node_runtime_t (spot_state).bind_relocation_authority (authority);
          auto &objects = node.native_node ().objects ();
          objects.configure_relocation_state (
            [spot_state] (const object_ref_t &object, const std::string &stable_type,
                          std::stop_token cancellation) {
                return detail::spot_node_runtime_t (spot_state)
                  .capture_spot_relocation_state (object, stable_type, cancellation);
            },
            [spot_state] (const frozen_object_state_t &frozen, const object_ref_t &object,
                          std::stop_token cancellation) {
                return detail::spot_node_runtime_t (spot_state)
          ...
          objects.configure_relocation_materialization (
            [spot_state] (const frozen_object_state_t &frozen, const object_ref_t &object,
                          const std::optional<object_ref_t> &spot, std::stop_token cancellation) {
                return detail::spot_node_runtime_t (spot_state)
            ...
            [spot_state] (const std::vector<object_ref_t> &objects) {
                detail::spot_node_runtime_t (spot_state).abort_relocation_materialization (objects);
            });
      };
    ...
    test.require (source_admitted && target_admitted,
    ...
    const auto created_actor = source.create_application_actor (
      "production.aggregate.actor", "production-aggregate-actor", std::nullopt, 1s);
    test.require (static_cast<bool> (created_actor), "production aggregate Actor must be created");
    if (!created_actor) {
        source.stop ();
        target.stop ();
        return;
    ...
    source_objects.replace_placement_candidates (
      {{.mesh_name = "production-aggregate-mesh",
        .node_id = source.status ().routing_id ().to_string (),
        .stable_types = {"production.aggregate.actor", "production.aggregate.spot"},
        .weight = 100,
        .active_capacity = 100,
        .active_count = 1,
        .pending_capacity = 100,
    ...
    auto created_spot = source_objects.begin_create (
      {.kind = object_kind_t::user_spot,
       .key = "production-aggregate-spot",
       .stable_type = "production.aggregate.spot",
       .mesh_name = std::optional<std::string>{"production-aggregate-mesh"},
       .creation_request = {},
       .exclusive = true,
       .instance_intent = false});
    test.require (created_spot.status == create_status_t::reserved
    ...
    const auto spot = source_objects.find (object_kind_t::user_spot, "production-aggregate-spot");
    if (!actor || !spot) {
        test.require (false, "production aggregate participants must resolve");
        source.stop ();
        target.stop ();
        return;
    }
    const auto [join_error, join] = source_objects.begin_membership_move (*actor, *spot);
    const auto [commit_error, joined_actor] = source_objects.commit_membership_move (join);
    test.require (join_error == stateful_error_t::none && commit_error == stateful_error_t::none,
                  "production aggregate Actor must join its User Spot");

    const auto application_actor_ref = ::zlink::framework::detail::actor_ref_access_t::make (
      framework::node_rid_t::from_string (joined_actor.node_id), "production.aggregate.actor",
      joined_actor.key, joined_actor.object_generation);
    ...
    test.require (source_objects.register_timer (*spot, {101, 1000, 250, 7})
                      == stateful_error_t::none
                    && source_objects.register_timer (joined_actor, {102, 2000, 0, 8})
                         == stateful_error_t::none,
    ...
    aggregate_materialized_spot_t::authority_commit_count = [aggregates] {
        std::lock_guard lock (aggregates->mutex);
        return aggregates->commit_count;
    };
    aggregate_materialized_spot_t::target_objects = &target.native_node ().objects ();
    ...
    try {
        const auto source_spot_state =
          source_spots.capture_spot_relocation_state (*spot, "production.aggregate.spot");
        test.require (source_spot_state == std::vector<std::uint8_t>{19},
                      "production aggregate source Spot adapter must be capturable");
    }
    catch (const std::exception &error) {
        std::cerr << "V11-M6C-CPP source Spot capture diagnostic: " << error.what () << '\n';
        test.require (false, "production aggregate source Spot adapter must be capturable");
    }
    aggregate_materialized_spot_adapter_t::capture_count.store (0);
    aggregate_materialized_actor_factory_t::record_target.store (true, std::memory_order_release);
    {
        ...
        authority->participant_identities = {
          relocation_participant_identity_t{*spot, "production.aggregate.spot", std::nullopt},
          relocation_participant_identity_t{
            joined_actor, "production.aggregate.actor",
            std::pair{std::string ("production-aggregate-spot"), spot->object_generation}}};
    }

    const auto make_authority = [&] (const object_ref_t &object,
    ...
    std::thread relocation_thread ([&] {
        result = await_task (source.relocate_application_unit (
          {*spot, joined_actor}, {"production.aggregate.spot", "production.aggregate.actor"},
          target_descriptor,
    ...
    const auto target_committed = wait_until_bounded (
      [&] {
          return aggregates->commit_count == 1
                 && target.native_node ().objects ().find (object_kind_t::user_spot,
                                                           "production-aggregate-spot")
                 && target.native_node ().objects ().find (object_kind_t::actor,
    ...
    const auto restored_spot =
      target.native_node ().objects ().find (object_kind_t::user_spot, expected_spot.key);
    const auto restored_actor =
      target.native_node ().objects ().find (object_kind_t::actor, expected_actor.key);
    const auto [saved_claim_error, saved_claim] =
      target.native_node ().objects ().try_claim (expected_actor, turn_domain_t::application);
    const auto saved_completed = saved_claim
                                 && target.native_node ().objects ().complete_claim (
                                      expected_actor, turn_domain_t::application)
                                      == stateful_error_t::none;
    const auto lifecycle_ordered =
      aggregate_materialized_spot_t::restore_order.load (std::memory_order_acquire) == 1
      && aggregate_materialized_actor_factory_t::factory_order.load (std::memory_order_acquire) == 2
      && aggregate_materialized_actor_adapter_t::restore_order.load (std::memory_order_acquire) == 3
      && aggregate_materialized_spot_t::joined_order.load (std::memory_order_acquire) == 4
      && aggregate_materialized_spot_t::ready_order.load (std::memory_order_acquire) == 5;
    if (result.terminal != relocation_terminal_t::completed || !result.target_handoff
        || !target_committed || aggregates->prepare_count != 1 || aggregates->commit_count != 1
        || restored_spot != std::optional<object_ref_t>{expected_spot}
        || restored_actor != std::optional<object_ref_t>{expected_actor}
        || target.native_node ().objects ().actor_membership (expected_actor)
             != std::optional<std::string>{expected_spot.key}
        || target.native_node ().objects ().timers (expected_spot).size () != 1
        || target.native_node ().objects ().timers (expected_actor).size () != 1)
        std::cerr
          << "V11-M6C-CPP aggregate diagnostic: terminal=" << static_cast<int> (result.terminal)
          << " reason=" << static_cast<int> (result.reason)
          << " authority=" << result.authority.size () << " prepare=" << aggregates->prepare_count
          << " commit=" << aggregates->commit_count << " spot=" << restored_spot.has_value ()
          << " actor=" << restored_actor.has_value () << " membership="
          << target.native_node ().objects ().actor_membership (expected_actor).value_or ("<none>")
          << " spot-timers=" << target.native_node ().objects ().timers (expected_spot).size ()
          << " actor-timers=" << target.native_node ().objects ().timers (expected_actor).size ()
          << " lifecycle-order=" << lifecycle_ordered
          << " spot-capture=" << aggregate_materialized_spot_adapter_t::capture_count.load ()
          << " actor-capture=" << aggregate_materialized_actor_adapter_t::capture_count.load ()
          << " saved-claim=" << static_cast<int> (saved_claim_error) << '\n';
    test.require (result.terminal == relocation_terminal_t::completed && result.target_handoff
                    && target_committed && aggregates->prepare_count == 1
                    ...
                    && target.native_node ().objects ().timers (expected_spot).size () == 1
                    && target.native_node ().objects ().timers (expected_actor).size () == 1,
                  "production aggregate relocation must commit and restore the User Spot, member "
                  "Actor, membership, and timers");
    test.require (
      aggregate_materialized_spot_adapter_t::capture_count.load (std::memory_order_acquire) == 1
        && aggregate_materialized_actor_adapter_t::capture_count.load (std::memory_order_acquire)
             == 1
        && aggregate_materialized_spot_adapter_t::restored_value.load (std::memory_order_acquire)
             == 19
        && aggregate_materialized_actor_adapter_t::restored_value.load (std::memory_order_acquire)
             == 37
        && aggregate_materialized_spot_t::joined_saw_membership.load (std::memory_order_acquire)
        && aggregate_materialized_spot_t::joined_saw_state.load (std::memory_order_acquire)
        && aggregate_materialized_spot_t::joined_before_cas.load (std::memory_order_acquire)
        && aggregate_materialized_spot_t::joined_held_ingress.load (std::memory_order_acquire)
        && aggregate_materialized_spot_t::ready_after_cas.load (std::memory_order_acquire)
        && aggregate_materialized_spot_t::ready_before_dispatch.load (std::memory_order_acquire)
        && lifecycle_ordered && saved_claim_error == stateful_error_t::none && saved_claim
        && saved_claim->sequence == 201 && saved_completed,
      "target materialization must restore Spot then Actor, publish membership before joined, run "
      "target-only CAS before ready completion, and open saved ingress last");
...
void test_relocation_adapter_single_capture_restore_path (test_context_t &test)
{
    ...
    builder.add_spot_factory<fail_first_restore_spot_t> (
      ...
      [] (auto &factory) { factory.template preserve_state_with<single_capture_adapter_t> (); });
    auto runtime = zlink::framework::detail::spot_node_runtime_t::from (builder);

    const object_ref_t source_spot{.kind = object_kind_t::user_spot,
    ...
    const frozen_object_state_t frozen{.owner = source_spot,
...
void to_json (nlohmann::json &json, const entry_relocation_test_payload_t &payload)
{
    json = nlohmann::json{{"value", payload.value}};
}

void from_json (const nlohmann::json &json, entry_relocation_test_payload_t &payload)
{
    payload.value = json.at ("value").get<int> ();
}
...
{
  public:
    explicit entry_relocation_test_actor_t (zlink::framework::actor_context_t context) :
        _context (std::move (context))
    {
        ++create_count;
    }
...
class entry_relocation_test_actor_factory_t final
    : public zlink::framework::actor_factory_t<entry_relocation_test_actor_t>
{
  public:
    zlink::framework::task_t<std::shared_ptr<entry_relocation_test_actor_t>>
    create (zlink::framework::actor_context_t context, std::stop_token) override
    {
        co_return std::make_shared<entry_relocation_test_actor_t> (std::move (context));
    }
};
...
{
  public:
    explicit entry_relocation_test_entry_spot_t (zlink::framework::entry_spot_context_t context) :
        _context (std::move (context))
    {
    }
    ...
    void configure () override
    {
        _context.handlers ().add_actor_send<&entry_relocation_test_entry_spot_t::on_message> (
          "entry-relocation-test-message");
    }

    zlink::framework::task_t<void> on_message (entry_relocation_test_actor_t &,
                                               zlink::framework::message_context_t &,
    ...
    zlink::framework::task_t<void> on_actor_joined (entry_relocation_test_actor_t &actor) override
    {
        ++joined_count;
        joined_spot = this;
        joined_actor = &actor;
...
void test_entry_spot_actor_relocation_restore_resolves_local_entry_spot (test_context_t &test)
{
    ...
    mesh.add_actor_factory<entry_relocation_test_actor_t, entry_relocation_test_actor_factory_t> (
      "entry-relocation-test-actor", std::make_shared<entry_relocation_test_actor_factory_t> (),
      [] (auto &factory) { factory.recreate_on_relocation (); });

    auto found_runtime = zlink::framework::detail::spot_node_runtime_t::from (
    ...
    test.require (entry_created.state == zlink::framework::spot_create_state_t::created,
                  "the fixture must create its own local Entry Spot before restore");

    const object_ref_t source_actor{.kind = object_kind_t::actor,
                                    .key = "entry-relocation-test-actor-id",
                                    .object_generation = 1,
                                    .authority_owner_generation = 1,
                                    .mesh_name = "mesh",
                                    .node_id = "source"};
    const frozen_object_state_t frozen{.owner = source_actor,
                                       .stable_type = "entry-relocation-test-actor",
                                       .application_state = {},
                                       .pending_application = {},
                                       .timers = {}};
    const object_ref_t target_actor{.kind = object_kind_t::actor,
                                    .key = "entry-relocation-test-actor-id",
                                    .object_generation = 1,
                                    .authority_owner_generation = 2,
                                    .mesh_name = "mesh",
    ...
    test.require (entry_relocation_test_entry_spot_t::joined_spot != nullptr
                    && entry_relocation_test_entry_spot_t::joined_actor != nullptr,
    ...
    if (entry_relocation_test_entry_spot_t::joined_spot
        ...
        const auto served =
          entry_relocation_test_entry_spot_t::joined_spot->context ()
            .handlers ()
            .invoke_actor_packet ("entry-relocation-test-message",
                                  *entry_relocation_test_entry_spot_t::joined_spot,
                                  *entry_relocation_test_entry_spot_t::joined_actor, services,
                                  serializers, zlink::message_t::from (R"({"value":42})"));
        test.require (served && entry_relocation_test_entry_spot_t::message_served_count == 1
                        && entry_relocation_test_entry_spot_t::last_message_value == 42,
...
/* A->B->A can return before B's one-way OnLeave notification has cleaned A's
 * retained source instance. The exact A->B Message Follow fence proves that
...
void test_return_actor_relocation_replaces_departed_spot_instance (test_context_t &test)
{
    ...
    auto found_runtime = zlink::framework::detail::spot_node_runtime_t::from (
      builder, "return-relocation-remnant-mesh");
    test.require (found_runtime.has_value (),
                  "return relocation fixture must resolve its Spot runtime");
    if (!found_runtime)
        return;
    auto runtime = *found_runtime;
    const auto entry_created = runtime.create_spot ("entry");
    test.require (entry_created.state == zlink::framework::spot_create_state_t::created,
                  "return relocation fixture must create its local Entry Spot");
    const auto state = runtime.weak_state ().lock ();
    ...
    const object_ref_t first_source{.kind = object_kind_t::actor,
                                    .key = "returning-entry-actor",
                                    .object_generation = 1,
                                    .authority_owner_generation = 1,
                                    .mesh_name = "mesh",
                                    .node_id = "node-x"};
    const frozen_object_state_t first_frozen{.owner = first_source,
                                             .stable_type = "entry-relocation-test-actor",
                                             .application_state = {},
                                             .pending_application = {},
                                             .timers = {}};
    const object_ref_t first_target{.kind = object_kind_t::actor,
                                    .key = "returning-entry-actor",
                                    .object_generation = 1,
                                    .authority_owner_generation = 2,
                                    .mesh_name = "mesh",
    ...
    const auto old_instance = state->actor_instances.at (key);
    const auto source_actor = zlink::framework::detail::actor_ref_access_t::make (
      zlink::framework::node_rid_t::from_string ("node-a"), "entry-relocation-test-actor",
      "returning-entry-actor", 1);
    const auto returned_source = zlink::framework::detail::actor_ref_access_t::make (
      zlink::framework::node_rid_t::from_string ("node-b"), "entry-relocation-test-actor",
      "returning-entry-actor", 1);
    const auto source_fence = zlink::framework::runtime::protocol::actor_route_fence_t{
      "returning-entry-actor", 1, zlink::routing_id_t::from ("node-a").to_bytes (), 1, 2, 4};
    const auto target_fence = zlink::framework::runtime::protocol::actor_route_fence_t{
      "returning-entry-actor", 1, zlink::routing_id_t::from ("node-b").to_bytes (), 1, 3, 5};
    const auto entry_id = state->spot_ids_by_name.at ("entry");
    state->actor_authority_fences.erase (key);
    zlink::framework::detail::record_actor_route_unlocked (
      *state, key,
      zlink::framework::spot_route_t{zlink::framework::node_rid_t::from_string ("node-b"),
                                     zlink::framework::spot_id_t ("spot-b"), "entry"},
      1);
    state->actor_transfer_coordinator.activate_message_follow (
      key, source_fence, returned_source,
      zlink::framework::spot_route_t{zlink::framework::node_rid_t::from_string ("node-b"),
                                     zlink::framework::spot_id_t ("spot-b"), "entry"},
      target_fence, std::chrono::steady_clock::now () + std::chrono::seconds (30), "return-a-b");
    state->pending_remote_source_cleanups.push_back (
      zlink::framework::detail::spot_node_builder_state_t::pending_remote_source_cleanup_t{
        .source_actor = source_actor,
        .source_fence = source_fence,
        .transfer_id = "return-a-b",
        .source_spot_id = entry_id,
        .source_spot_generation = 1,
        .target_spot_id = zlink::framework::spot_id_t ("spot-b"),
        .not_before = std::chrono::steady_clock::now () + std::chrono::seconds (30),
        .leave_submitted = false,
        .leave_completed = false,
        .leave_deadline = std::chrono::steady_clock::now () + std::chrono::seconds (30)});

    const object_ref_t returning_source{.kind = object_kind_t::actor,
                                        .key = "returning-entry-actor",
                                        .object_generation = 1,
                                        .authority_owner_generation = 3,
                                        .mesh_name = "mesh",
                                        .node_id = "node-b"};
    const frozen_object_state_t returning_frozen{.owner = returning_source,
                                                 .stable_type = "entry-relocation-test-actor",
                                                 .application_state = {},
                                                 .pending_application = {},
                                                 .timers = {}};
    ...
    const auto cleanup = std::find_if (
      ...
      [] (const auto &candidate) { return candidate.transfer_id == "return-a-b"; });
    test.require (returned && state->actor_instances.at (key) != old_instance
                    && entry_relocation_test_actor_t::create_count == 2
                    && entry_relocation_test_entry_spot_t::joined_count == 2
                    && entry_relocation_test_entry_spot_t::leave_count == 1
                    && cleanup != state->pending_remote_source_cleanups.end ()
    ...
    // remains. That exact remote route is cache state, not a competing local
    // Actor, and a still newer Store-validated return replaces it directly.
    state->actor_authority_fences.erase (key);
    state->pending_remote_source_cleanups.clear ();
    zlink::framework::detail::record_actor_route_unlocked (
      *state, key,
      zlink::framework::spot_route_t{zlink::framework::node_rid_t::from_string ("node-b"),
                                     zlink::framework::spot_id_t ("spot-b"), "entry"},
      1);
    auto route_only_source = returning_source;
    route_only_source.authority_owner_generation = 5;
    auto route_only_frozen = returning_frozen;
    route_only_frozen.owner = route_only_source;
    ...
    test.require (route_only_returned && entry_relocation_test_actor_t::create_count == 3
                    ...
                    && state->actor_routes.at (key).node_rid.value () == "node-a",
                  "an exact departed remote route without local authority, even when its "
                  "instance erase lagged, must not veto a newer Store-validated return");

    // Message Follow expiry can remove the remote route before its membership
    // cache. With no instance, route, authority, or cleanup left, the exact-
    // generation actor_spot_ids entry is an orphan and cannot veto the next
    // Store-validated return.
    zlink::framework::detail::erase_actor_instance_index_unlocked (
      *state, "entry-relocation-test-actor", "returning-entry-actor");
    state->actor_instances.erase (key);
    state->actor_authority_fences.erase (key);
    state->actor_routes.erase (key);
    state->pending_remote_source_cleanups.clear ();
    auto orphan_source = route_only_source;
    orphan_source.authority_owner_generation = 7;
    auto orphan_frozen = route_only_frozen;
    orphan_frozen.owner = orphan_source;
    ...
    test.require (orphan_returned && entry_relocation_test_actor_t::create_count == 4
                    ...
                    && state->actor_instances.contains (key)
                    && state->actor_routes.at (key).node_rid.value () == "node-a",
                  "an exact-generation orphan Spot membership cache must not veto a "
                  "newer Store-validated return");

    auto committed_orphan_target = orphan_target;
    committed_orphan_target.authority_owner_generation = 11;
    state->actor_authority_fences.at (key).target_node_generation = 23;
    const auto staged_fence = state->actor_authority_fences.at (key);
    const auto committed_adopted = runtime.adopt_committed_actor_relocation_authority (
      committed_orphan_target, staged_fence.target_node_generation, 17);
    auto stale_committed_target = committed_orphan_target;
    --stale_committed_target.authority_owner_generation;
    const auto stale_adopted = runtime.adopt_committed_actor_relocation_authority (
      stale_committed_target, staged_fence.target_node_generation, 19);
    test.require (committed_adopted && !stale_adopted
                    && state->actor_authority_fences.at (key).authority_owner_generation == 11
                    && state->actor_authority_fences.at (key).owner_lease_generation == 17,
                  "post-CAS target adoption must install the exact Store fence before route "
                  "publication and reject a stale generation");
    {
        std::lock_guard lock (authority->mutex);
        authority->rows[authority_key (object_kind_t::actor, orphan_target.key)] =
          authority_relocation_reference_t{.source = orphan_source,
                                           .target = committed_orphan_target,
                                           .target_owner = {"return-target-owner", 17}};
    }
    const auto materialization_committed =
      runtime.commit_relocation_materialization ({orphan_target});
    const auto committed_fence = state->actor_authority_fences.find (key);
    test.require (materialization_committed
                    && committed_fence != state->actor_authority_fences.end ()
                    && committed_fence->second.authority_owner_generation == 11
                    && committed_fence->second.owner_lease_generation == 17,
                  "materialization commit must retain the Store-confirmed target authority "
                  "generation and lease when the Store advances by more than one");
...
void test_entry_spot_actor_relocation_restore_fails_without_local_entry_spot (test_context_t &test)
{
    entry_relocation_test_actor_t::create_count = 0;

    zlink::framework::spot_node_builder_t builder;
    builder
      .add_actor_factory<entry_relocation_test_actor_t, entry_relocation_test_actor_factory_t> (
        "entry-relocation-test-actor", std::make_shared<entry_relocation_test_actor_factory_t> (),
        [] (auto &factory) { factory.recreate_on_relocation (); });
...
void test_stateful_application_reservation_includes_active_work (test_context_t &test)
{
    ...
    test.require (multipart_claim_error == stateful_error_t::none && multipart_claim,
    ...
    object_ref_t restore_target{
      object_kind_t::actor, "progressive-restore-actor", 1, 2, "mesh", "node-b"};
    progressive_restore.configure_relocation_materialization (
      ...
      [&] (const std::vector<object_ref_t> &targets) {
          ...
          const auto [claim_error, claimed] =
            progressive_restore.try_claim (restore_target, turn_domain_t::application);
          lifecycle_saw_closed_dispatch = claim_error == stateful_error_t::moving && !claimed;
          return lifecycle_committed && lifecycle_saw_closed_dispatch;
      },
      [&] (const std::vector<object_ref_t> &) { relocation_aborted = true; });
    const relocation_restore_identity_t restore_identity{"restore-byte-root", 1,
                                                         digest_with (0x61)};
    const frozen_object_state_t frozen{
      ...
      .stable_type = "actor",
      .application_state = {},
      .pending_application = {{1, std::vector<std::uint8_t> (4, 0x41)},
    ...
    if (restored == stateful_error_t::none) {
        relay_ingress =
          progressive_restore.enqueue (restore_target, turn_domain_t::application, {4, {0x44}});
        temporary_ingress =
          progressive_restore.enqueue (restore_target, turn_domain_t::application, {5, {0x45}});
        backlog_records_before_commit =
          progressive_restore.pending (restore_target, turn_domain_t::application);
        backlog_bytes_before_commit =
          progressive_restore.pending_bytes (restore_target, turn_domain_t::application);
        std::tie (pre_commit_claim_error, pre_commit_claim) =
          progressive_restore.try_claim (restore_target, turn_domain_t::application);
        committed =
          progressive_restore.commit_relocation_restore (restore_target, restore_identity);
        progressive_fifo = committed == stateful_error_t::none;
        for (std::uint64_t sequence = 1; progressive_fifo && sequence <= 5; ++sequence) {
            const auto [claim_error, claimed] =
              progressive_restore.try_claim (restore_target, turn_domain_t::application);
            const auto [concurrent_error, concurrent] =
              progressive_restore.try_claim (restore_target, turn_domain_t::application);
            progressive_fifo = claim_error == stateful_error_t::none && claimed
                               && claimed->sequence == sequence
                               && concurrent_error == stateful_error_t::none && !concurrent;
            if (claimed) {
                ++handler_turns;
                progressive_fifo =
                  progressive_restore.complete_claim (restore_target, turn_domain_t::application)
                    == stateful_error_t::none
                  && progressive_fifo;
            }
        }
    ...
    test.require (
      restored == stateful_error_t::none && materialized && relay_ingress == stateful_error_t::none
        && temporary_ingress == stateful_error_t::none && backlog_records_before_commit == 5
        && backlog_bytes_before_commit > fixed + 1
        && pre_commit_claim_error == stateful_error_t::moving && !pre_commit_claim
        && progressive_restore.pending_bytes (restore_target, turn_domain_t::application) == 0
    ...
    std::vector<frozen_object_state_t> aggregate_frozen{
      ...
      {.owner = {.kind = object_kind_t::actor,
       ...
       .timers = {}}};
    std::vector<object_ref_t> aggregate_targets{aggregate_frozen[0].owner,
                                                aggregate_frozen[1].owner};
    for (auto &target : aggregate_targets) {
        target.authority_owner_generation = 2;
        target.node_id = "node-b";
    }
    ...
    const auto aggregate_committed = aggregate_restored == stateful_error_t::none
                                       ...
                                       : stateful_error_t::conflict;
    bool aggregate_fifo = aggregate_committed == stateful_error_t::none;
    for (const auto &target : aggregate_targets) {
        for (std::uint64_t sequence = 1; aggregate_fifo && sequence <= 3; ++sequence) {
            const auto [claim_error, claimed] =
              aggregate_restore.try_claim (target, turn_domain_t::application);
            aggregate_fifo =
              claim_error == stateful_error_t::none && claimed && claimed->sequence == sequence;
            if (claimed) {
                aggregate_fifo =
                  aggregate_restore.complete_claim (target, turn_domain_t::application)
                    == stateful_error_t::none
                  && aggregate_fifo;
            }
        }
    }
    test.require (aggregate_restored == stateful_error_t::none && aggregate_materialized == 2
                    && aggregate_commit_count == 1 && aggregate_fifo,
                  "aggregate relocation must restore and progressively drain every "
                  "participant backlog beyond the live count and byte limits");
}

void test_aggregate_seal_failure_preserves_earlier_application_work (test_context_t &test)
{
    stateful_object_runtime_t objects;
    const auto first = create_spot (objects, object_kind_t::user_spot, "seal-failure-first");
    const auto second = create_spot (objects, object_kind_t::user_spot, "seal-failure-second");
    test.require (
      objects.enqueue (first, turn_domain_t::application, {1, {1, 2}}) == stateful_error_t::none
        && objects.enqueue (first, turn_domain_t::application, {2, {3}}) == stateful_error_t::none
        && objects.enqueue (second, turn_domain_t::application, {1, {4}}) == stateful_error_t::none,
      "aggregate seal failure setup must retain application records");
    const auto first_bytes = objects.pending_bytes (first, turn_domain_t::application);
    const auto second_bytes = objects.pending_bytes (second, turn_domain_t::application);
    objects.configure_relocation_state (
    ...
    const auto [error, seal] = await_task (objects.try_seal_relocation_aggregate ({first, second}));
    test.require (error == stateful_error_t::conflict && seal.participants.empty (),
                  "aggregate capture failure must return conflict without a seal");
    test.require (objects.pending (first, turn_domain_t::application) == 2
                    && objects.pending_bytes (first, turn_domain_t::application) == first_bytes,
                  "aggregate capture failure must preserve the earlier participant queue");
    test.require (objects.pending (second, turn_domain_t::application) == 1
                    && objects.pending_bytes (second, turn_domain_t::application) == second_bytes,
                  "aggregate capture failure must preserve the failing participant queue");
}

void test_relocation_hold_restores_without_dedicated_limits (test_context_t &test)
{
    namespace limits = zlink::framework::runtime::dispatch_limits;
    namespace protocol = zlink::framework::runtime::protocol;
    ...
    const auto bound_session_bytes = [] {
        ...
        held.body = protocol::frozen_actor_application_body_t{
          ...
          {protocol::framework_multipart_packet_name, protocol::framework_multipart_content_type,
           multipart ({'h', 'e', 'l', 'd'})}};
    ...
    test.require (failed_capture.enqueue (failed_spot, turn_domain_t::application, {1, {1}})
                      == stateful_error_t::none
                    && failed_capture.enqueue (failed_spot, turn_domain_t::application, {2, {2}})
                         == stateful_error_t::none,
                  "held capture failure setup must retain the initial FIFO records");
    const auto [active_error, active] =
      failed_capture.try_claim (failed_spot, turn_domain_t::application);
    test.require (active_error == stateful_error_t::none && active && active->sequence == 1,
                  "held capture failure setup must keep the first record active");
    failed_capture.configure_relocation_state (
      [] (const object_ref_t &, const std::string &, std::stop_token) -> std::vector<std::uint8_t> {
    ...
    std::thread capture ([&] {
        auto result = await_task (failed_capture.try_seal_relocation_aggregate ({failed_spot}));
    ...
    const bool moving = wait_until_bounded (
      [&] {
          return failed_capture.register_timer (failed_spot, {91, 1000, 1000, 1})
                 == stateful_error_t::moving;
      },
      std::chrono::seconds (5));
    test.require (moving && !capture_done.load (std::memory_order_acquire),
                  "capture failure test must enqueue while the source is moving");
    test.require (
      failed_capture.enqueue (failed_spot, turn_domain_t::application, {3, bound_session_bytes})
        == stateful_error_t::none,
      "application ingress must enter the relocation hold before capture");
    if (active) {
        test.require (failed_capture.complete_claim (failed_spot, turn_domain_t::application)
                        == stateful_error_t::none,
                      "capture failure test must release the active turn");
    }
    capture.join ();
    test.require (capture_error == stateful_error_t::conflict && capture_seal.participants.empty (),
                  "capture failure must return conflict without a relocation seal");
    const auto [first_error, first] =
      failed_capture.try_claim (failed_spot, turn_domain_t::application);
    test.require (first_error == stateful_error_t::none && first && first->sequence == 2,
    ...
    const auto [second_error, second] =
      failed_capture.try_claim (failed_spot, turn_domain_t::application);
    test.require (second_error == stateful_error_t::none && second && second->sequence == 3,
                  "capture failure must not strand or reorder held work");
    {
        bool tail_intact = false;
        if (second && second->payload == bound_session_bytes) {
            const auto decoded = protocol::decode_frozen_record (second->payload);
            tail_intact =
              decoded.source_kind == protocol::frozen_source_kind_t::bound_session
              && decoded.source_session_routing_id
              && *decoded.source_session_routing_id == std::vector<std::uint8_t>{'h', 'e', 'l', 'd'}
              && decoded.source_binding_generation == 5 && decoded.source_session_sequence == 9
              && decoded.source_actor && decoded.source_actor->first == "held-actor"
              && decoded.source_actor->second == 5;
        }
        test.require (tail_intact, "held bound-session records must merge with the session tail "
                                   "intact");
    }
    (void) failed_capture.complete_claim (failed_spot, turn_domain_t::application);
    ...
    const auto count_second =
      create_spot (count_limited, object_kind_t::user_spot, "held-count-second");
    test.require (count_limited.enqueue (count_first, turn_domain_t::application, {0, {}})
                    == stateful_error_t::none,
                  "aggregate hold count setup must reserve an active turn");
    const auto [count_claim_error, count_claim] =
      count_limited.try_claim (count_first, turn_domain_t::application);
    test.require (count_claim_error == stateful_error_t::none && count_claim,
    ...
    std::thread count_sealing ([&] {
        auto result =
          await_task (count_limited.try_seal_relocation_aggregate ({count_first, count_second}));
    ...
    const bool count_moving = wait_until_bounded (
      [&] {
          return count_limited.register_timer (count_first, {92, 1000, 1000, 1})
                 == stateful_error_t::moving;
      },
      std::chrono::seconds (5));
    test.require (count_moving && !count_done.load (std::memory_order_acquire),
                  "aggregate hold count test must observe the moving barrier");
    const auto membership_actor = create_actor (count_limited, "membership-only-hold-actor");
    const object_ref_t membership_target{
      object_kind_t::user_spot, "membership-only-target", 1, 1, "mesh", "node-b"};
    const auto [membership_error, membership_move] =
      count_limited.begin_remote_membership_move (membership_actor, membership_target);
    test.require (
      membership_error == stateful_error_t::none
        && count_limited.enqueue (membership_actor, turn_domain_t::application, {1, {7}})
             == stateful_error_t::none,
    ...
    for (std::uint64_t sequence = 1; sequence <= 600; ++sequence) {
        count_records_accepted =
          count_limited.enqueue (count_first, turn_domain_t::application, {sequence, {}})
            == stateful_error_t::none
          && count_records_accepted;
        count_records_accepted =
          count_limited.enqueue (count_second, turn_domain_t::application, {3000 + sequence, {}})
            == stateful_error_t::none
          && count_records_accepted;
    }
    test.require (count_records_accepted
                    && count_limited.pending (count_first, turn_domain_t::application)
                           + count_limited.pending (count_second, turn_domain_t::application)
                         == 1200,
    ...
    test.require (count_records_accepted
                    && count_limited.pending (count_first, turn_domain_t::application) == 2049,
    ...
    if (membership_error == stateful_error_t::none) {
        test.require (count_limited.abort_membership_move (membership_move)
                        == stateful_error_t::none,
                      "membership-only movement must release its independent hold");
    }
    if (count_claim) {
        test.require (count_limited.complete_claim (count_first, turn_domain_t::application)
                        == stateful_error_t::none,
                      "aggregate hold count test must release its active turn");
    }
    count_sealing.join ();
    test.require (count_error == stateful_error_t::none
                    && count_limited.abort_relocation (count_seal.token) == stateful_error_t::none,
    ...
    for (std::uint64_t sequence = 1; sequence <= 2049; ++sequence) {
        const auto [claim_error, claimed] =
          count_limited.try_claim (count_first, turn_domain_t::application);
        source_abort_fifo = source_abort_fifo && claim_error == stateful_error_t::none && claimed
                            && claimed->sequence == sequence;
        if (claimed) {
            source_abort_fifo =
              count_limited.complete_claim (count_first, turn_domain_t::application)
                == stateful_error_t::none
              && source_abort_fifo;
        }
    }
    ...
    test.require (byte_limited.enqueue (byte_first, turn_domain_t::application, {0, {}})
                    == stateful_error_t::none,
                  "aggregate hold byte setup must reserve an active turn");
    const auto [byte_claim_error, byte_claim] =
      byte_limited.try_claim (byte_first, turn_domain_t::application);
    test.require (byte_claim_error == stateful_error_t::none && byte_claim,
    ...
    std::thread byte_sealing ([&] {
        auto result =
          await_task (byte_limited.try_seal_relocation_aggregate ({byte_first, byte_second}));
    ...
    const bool byte_moving = wait_until_bounded (
      [&] {
          return byte_limited.register_timer (byte_first, {93, 1000, 1000, 1})
                 == stateful_error_t::moving;
      },
      std::chrono::seconds (5));
    test.require (byte_moving && !byte_done.load (std::memory_order_acquire),
                  "aggregate hold byte test must observe the moving barrier");
    const auto held_payload_bytes = 8u * 1024u * 1024u - limits::fixed_work_byte_cost;
    test.require (
      byte_limited.enqueue (byte_first, turn_domain_t::application,
                            {1, std::vector<std::uint8_t> (held_payload_bytes, 0x41)})
          == stateful_error_t::none
        && byte_limited.enqueue (byte_second, turn_domain_t::application,
    ...
    test.require (byte_limited.enqueue (
                    byte_first, turn_domain_t::application,
                    {2, std::vector<std::uint8_t> (former_bound_excess_payload_bytes, 0x43)})
                    == stateful_error_t::none,
                  "relocation hold must accept bytes beyond the former aggregate "
                  "16 MiB limit");
    const auto application_capacity_fill_payload_bytes =
      11u * 1024u * 1024u - 2u * limits::fixed_work_byte_cost;
    test.require (byte_limited.enqueue (
                    byte_first, turn_domain_t::application,
                    {3, std::vector<std::uint8_t> (application_capacity_fill_payload_bytes, 0x44)})
                      == stateful_error_t::none
                    && byte_limited.enqueue (byte_first, turn_domain_t::application, {4, {}})
                         == stateful_error_t::none
                    && byte_limited.enqueue (byte_first, turn_domain_t::application, {5, {}})
                         == stateful_error_t::none
                    && byte_limited.pending_bytes (byte_first, turn_domain_t::application)
                         > 20u * 1024u * 1024u,
                  "source relocation ingress must remain held beyond the configured "
                  "application-lane byte capacity");
    if (byte_claim) {
        test.require (byte_limited.complete_claim (byte_first, turn_domain_t::application)
                        == stateful_error_t::none,
                      "aggregate hold byte test must release its active turn");
    }
    byte_sealing.join ();
    test.require (byte_error == stateful_error_t::none
    ...
    target_hold.configure_relocation_state (
      ...
      [&] (const frozen_object_state_t &, const object_ref_t &, std::stop_token) {
          ...
          target_condition.wait (lock, [&] { return release_target_restore; });
          return true;
      });
    const frozen_object_state_t target_frozen{.owner = {.kind = object_kind_t::user_spot,
    ...
    const relocation_restore_identity_t target_identity{"target-unbounded-root", 7,
                                                        digest_with (0x71)};
    ...
    {
        std::unique_lock lock (target_mutex);
        test.require (target_condition.wait_for (lock, std::chrono::seconds (5),
                                                 [&] { return target_restore_entered; }),
                      "target temporary hold test must enter restore before ingress");
    }
    bool target_records_held =
      target_hold.enqueue (target_ref, turn_domain_t::application,
                           {1, std::vector<std::uint8_t> (17u * 1024u * 1024u, 0x51)})
      == stateful_error_t::none;
    for (std::uint64_t sequence = 2; sequence <= 1025; ++sequence) {
        target_records_held =
          target_hold.enqueue (target_ref, turn_domain_t::application, {sequence, {0x52}})
            == stateful_error_t::none
          && target_records_held;
    }
    {
    ...
    test.require (target_records_held && target_restore_result == stateful_error_t::none
                    && target_hold.pending (target_ref, turn_domain_t::application) == 1025
                    && target_hold.pending_bytes (target_ref, turn_domain_t::application)
                         > 16u * 1024u * 1024u
                    && target_hold.commit_relocation_restore (target_ref, target_identity)
                         == stateful_error_t::none,
    ...
    for (std::uint64_t sequence = 1; sequence <= 1025; ++sequence) {
        const auto [claim_error, claimed] =
          target_hold.try_claim (target_ref, turn_domain_t::application);
        test.require (claim_error == stateful_error_t::none && claimed
                        && claimed->sequence == sequence,
                      "target relocation hold must resume in FIFO order after commit");
        if (claimed) {
            (void) target_hold.complete_claim (target_ref, turn_domain_t::application);
        }
    }
    ...
    target_abort.configure_relocation_state (
      ...
      [&] (const frozen_object_state_t &, const object_ref_t &, std::stop_token) {
          ...
          abort_condition.wait (lock, [&] { return release_abort_restore; });
          return true;
      });
    ...
    const relocation_restore_identity_t abort_identity{"target-abort-root", 8, digest_with (0x72)};
    ...
    {
        std::unique_lock lock (abort_mutex);
        test.require (abort_condition.wait_for (lock, std::chrono::seconds (5),
                                                [&] { return abort_restore_entered; }),
                      "target abort test must enter restore before ingress");
    }
    test.require (target_abort.enqueue (abort_target, turn_domain_t::application, {1, {0x71}})
                      == stateful_error_t::none
                    && target_abort.enqueue (abort_target, turn_domain_t::application, {2, {0x72}})
                         == stateful_error_t::none,
    ...
    test.require (abort_restore_result == stateful_error_t::none
                    && target_abort.abort_relocation_restore (abort_target, abort_identity)
                         == stateful_error_t::none
                    && !target_abort.find (abort_target.kind, abort_target.key),
    ...
    const auto [normal_move_error, normal_move] =
      normal_lane_caps.begin_remote_membership_move (membership_capped, remote_membership_target);
    test.require (
...
void test_advertised_receive_chunk_limit_wiring (test_context_t &test)
{
     ...
     * join reply. */
    test.require (spots::spot_actor_join_advertised_receive_chunk_limit_bytes == 32768,
    ...
    /* Round-trip: to_json/from_json preserve receiveChunkLimitBytes. */
    {
        ...
        test.require (decoded.receive_chunk_limit_bytes == 32768,
                      "join reply round-trip must preserve receiveChunkLimitBytes");
        test.require (decoded.actor_id == "actor-1" && decoded.actor_generation == 7,
    ...
    {
        nlohmann::json legacy{{"resultCode", 0},           {"actorNodeRid", "node-a"},
                              {"actorType", "demo.actor"}, {"actorId", "actor-1"},
                              {"actorGeneration", 7},      {"payload", ""}};
        const auto decoded = legacy.get<spots::spot_actor_join_route_reply_t> ();
        test.require (
    ...
    test.require (maintenance_runtime_t::apply_advertised_receive_chunk_limit (262144, 0) == 262144,
                  "advertised=0 (not advertised) must leave the local chunk cap unchanged");
    test.require (maintenance_runtime_t::apply_advertised_receive_chunk_limit (262144, 32768)
                    == 32768,
                  "a smaller advertised cap must win over a larger local budget");
    test.require (maintenance_runtime_t::apply_advertised_receive_chunk_limit (16384, 32768)
                    == 16384,
    ...
    const auto uncapped_limit =
      maintenance_runtime_t::apply_advertised_receive_chunk_limit (262144, 0);
    const auto capped_limit =
      maintenance_runtime_t::apply_advertised_receive_chunk_limit (262144, 8192);
    const auto chunk_count = [] (std::size_t total, std::uint64_t limit) {
        return limit == 0 ? std::size_t{0} : (total + limit - 1) / limit;
    };
    test.require (chunk_count (payload.size (), uncapped_limit) == 1
                    && chunk_count (payload.size (), capped_limit) == 3,
                  "an advertised cap smaller than the local budget must split the same payload "
                  "into more chunks");
}
...
void test_actor_join_wire_gate_records_target_authority (test_context_t &test)
{
    namespace detail = zlink::framework::detail;
    ...
    test.require (!node.observed_spot_authority (target_rid, "gate-spot", 5).has_value (),
                  "a freshly started node's cache has no entry for a Spot address "
    ...
    test.require (observed && observed->target_node_generation == 9
    ...
    test.require (!node.observed_spot_authority (target_rid, "gate-spot", 6).has_value (),
...
int main ()
{
    ...
    test_return_relocation_replaces_departed_remnant (test);
