...
int main ()
{
    ...
    for (const auto &required : {include_root, root / "framework/src", root / "connector/core"}) {
    ...
    const auto call_hpp =
      read_text_file (include_root / "zlink/framework/contracts/channels/call.hpp");
    ...
    const auto m6a_sources_begin = cmake.find ("set(ZLINK_FRAMEWORK_CPP_M6A_RUNTIME_SOURCES");
    ...
    const auto location_auto_connect = read_text_file (
      root / "framework/src/runtime/locations/location_auto_connect_host_service.hpp");
    const auto client_server_location_runtime = read_text_file (
      root / "framework/src/runtime/client_server/client_server_location_runtime.cpp");
    ...
    const auto dispatch_events =
      read_text_file (root / "framework/src/runtime/diagnostics/dispatch_events.hpp");
    const auto mesh_node_runtime =
      read_text_file (root / "framework/src/runtime/mesh/mesh_node_runtime.cpp");
    const auto actor_transfer_coordinator =
      read_text_file (root / "framework/src/runtime/spots/actor_transfer_coordinator.cpp");
    const auto mesh_node_host_service =
      read_text_file (root / "framework/src/runtime/mesh/mesh_node_host_service.cpp");
    const auto framework_message =
      read_text_file (include_root / "zlink/framework/contracts/messaging/message.hpp");
    const auto serializer_header =
    ...
    const auto service_topology_registry =
      read_text_file (root / "framework/src/runtime/mesh/service_topology_registry.hpp");
    ...
    const auto generated_service_wire_constants =
      read_text_file (root / "../../runtime/protocol/generated/cpp/service_wire_constants.hpp");
    const auto public_host_runtime =
      read_text_file (root / "framework/src/runtime/stateful/public_host_runtime.cpp");
    const auto raw_stateful_dispatch =
      read_text_file (root / "framework/src/runtime/stateful/raw_stateful_dispatch.cpp");
    const auto monitoring_unit =
      read_text_file (root / "tests/Zlink.Framework.UnitTests/test_cpp_framework_monitoring.cpp");
    const auto actor_gateway_unit = read_text_file (
      root / "tests/Zlink.Framework.UnitTests/test_cpp_framework_actor_gateway.cpp");
    ...
    const auto spot_hpp =
      read_text_file (include_root / "zlink/framework/contracts/spots/spot.hpp");
    const auto app_hpp =
    ...
    const auto framework_options_hpp = read_text_file (
      include_root / "zlink/framework/contracts/configuration/framework_options.hpp");
    ...
    const auto local_session_route =
      stream_host.find ("local_node->to_hex () == actor_route->node_rid.to_hex ()");
    const auto remote_session_route = stream_host.find ("bind_application_actor_session");
    gate.require (
      bound_session_registration != std::string::npos && local_session_route != std::string::npos
        && remote_session_route != std::string::npos && local_session_route < remote_session_route,
      "E2E-CP-56", "local actor session binding does not have a local path before remote binding");
    bool every_pubsub_scenario_checks_evidence = true;
    gate.require (every_pubsub_scenario_checks_evidence, "E2E-CP-04",
    ...
    gate.require (zlink_builder_hpp.find ("on_retry") == std::string::npos
                  ...
                  "IMP-CP-30", "zlink builder still exposes C++-only reliability hooks");
    gate.require (channel_hpp.find ("channel_reliability_event_t") == std::string::npos
                  ...
                  "IMP-CP-30", "channel contract still exposes C++-only reliability event types");
    ...
    gate.require (execution_hpp.find ("unhandled_dispatch_options_t") == std::string::npos
                    && execution_hpp.find ("unhandled_dispatch_action_t") == std::string::npos,
                  "IMP-CP-29", "fixed unhandled dispatch policy remains configurable");
    gate.require (execution_hpp.find ("unhandled;") == std::string::npos, "IMP-CP-29",
                  "dispatch options still expose the no-op unhandled policy");

    /* IMP-CP-08 — session-owned transport failures reach the session callback. */
    gate.require (stream_host.find ("stream_session_error_t::transport_error") != std::string::npos,
                  "IMP-CP-08", "STREAM host does not classify a session transport failure");
    gate.require (stream_host.find ("_runtime.dispatch_error (") != std::string::npos, "IMP-CP-08",
    ...
    gate.require (location_auto_connect.find ("list_mesh_nodes") != std::string::npos, "IMP-CP-05",
                  "RouteMesh discovery does not read MeshNode descriptors");
    ...
    gate.require (
      location_auto_connect.find ("const bool manual_endpoint") != std::string::npos
        && location_auto_connect.find ("Manual routes still use the discovered descriptor")
             != std::string::npos
        && location_auto_connect.find ("mesh_node->expect_peer") != std::string::npos,
      "CPP-TOPO-001", "manual RouteMesh endpoints do not install descriptor admission fences");
    ...
    gate.require (framework_options_hpp.find ("STREAM node '") != std::string::npos
                    && framework_options_hpp.find ("' is already registered") != std::string::npos,
    ...
    gate.require (store_location_resolvers.find ("version < observed") != std::string::npos,
    ...
    gate.require (store_location_resolvers.find ("row.actor_ref") != std::string::npos
                    && store_location_resolvers.find ("actor_ref_access_t::empty (*row.actor_ref)")
                         != std::string::npos,
                  "IMP-CP-07", "actor resolver does not reject pending actor rows");
    gate.require (app_runtime.find ("actor_location_observer") != std::string::npos, "IMP-CP-07",
                  "actor resolver and runtime query do not share generation state");
    ...
    gate.require (actor_hpp.find ("task_t<void> async") != std::string::npos, "CPP-G0-ASYNC-001",
    ...
    gate.require (actor_hpp.find ("task_t<void> notify_disconnected") != std::string::npos,
                  "CPP-G0-ASYNC-002",
                  "session_actor_t::notify_disconnected does not return "
                  "task_t<void>");
    ...
    for (const std::string forbidden :
         {"onCreateActor", "onLeaveActor", "onDisconnectActor", "destroyActor"}) {
    ...
    for (const std::string required :
         {"on_create_actor", "on_leave_actor", "on_disconnect_actor", "destroy_actor"}) {
    ...
    for (const std::string forbidden :
         {"actor_stale_generation", "timeout", "shutdown", "disconnected", "closed", "cancelled"}) {
    ...
    for (const std::string required : {"spot_ref_t", "send_to_spot", "request_to_spot"}) {
    ...
    gate.require (actor_hpp.find ("void defer ()") != std::string::npos, "CPP-G0-ACTOR-002",
                  "Actor Join defer terminal is missing");
    for (const std::string required :
         {"struct actor_join_accepted_t", "struct actor_join_rejected_t",
          "struct actor_join_failed_t", "on_join_completed"}) {
    ...
    for (const std::string alternative :
        ...
        gate.require (completion_block.find (alternative) != std::string::npos, "CPP-G0-ACTOR-002",
    ...
    gate.require (join_call_block.find ("void defer ()") != std::string::npos, "CPP-G0-ACTOR-002",
                  "actor_join_call_t declaration is missing or was not extracted whole");
    for (const std::string forbidden : {"submit (", " async (", "yield ("}) {
        gate.require (join_call_block.find (forbidden) == std::string::npos, "CPP-G0-ACTOR-002",
    ...
    for (const std::string removed : {"actor_join_result_t", "task_t<actor_join"}) {
        gate.require (actor_hpp.find (removed) == std::string::npos, "CPP-G0-ACTOR-002",
    ...
    for (const std::string forbidden :
         {"dispatch_mode_t", "spot_dispatch_mode", "stream_dispatch_mode"}) {
    ...
    gate.require (mesh_node_hpp.find ("max_message_size") == std::string::npos, "CPP-WIRE-002",
                  "MeshNode socket config still exposes max_message_size");
    gate.require (
      service_topology_registry.find ("effective_max_message_bytes") == std::string::npos,
      "CPP-WIRE-002", "RouteMesh topology still negotiates a framework message-size limit");
    ...
    gate.require (tree_contains (include_root, "typed_session_packet_handler"), "CPP-G0-STREAM-001",
                  "typed stream session handler contract is missing");
    ...
    for (const std::string required :
         {"\n    off = 0", "\n    errors = 1", "\n    normal = 2", "\n    detailed = 3"}) {
        gate.require (execution_hpp.find (required) != std::string::npos, "CPP-CONTRACT-DIAG-001",
    ...
    for (const std::string removed :
         {"message_flow_event_t", "message_dispatch_error_event_t", "message_flow_observer_t",
          ...
          "effective_message_flow", "live_mode ()", "log_file ()", "label ()"}) {
        gate.require (execution_hpp.find (removed) == std::string::npos, "CPP-CONTRACT-DIAG-002",
    ...
    for (const std::string required : {"location_object_kind_t", "location_object_state_t",
                                       "location_object_entry_t", "location_object_filter_t"}) {
        gate.require (location_diagnostics_hpp.find (required) != std::string::npos,
                      "CPP-CONTRACT-QUERY-001",
                      "location diagnostics header is missing " + required);
    }
    for (const std::string required :
         {"find_actor_location", "find_spot_location", "list_object_locations"}) {
        gate.require (location_runtime_query_hpp.find (required) != std::string::npos,
                      "CPP-CONTRACT-QUERY-001",
                      "location runtime query header is missing " + required);
    }
    gate.require (execution_hpp.find ("std::optional<logger_t<>> diagnostics_logger")
                    == std::string::npos,
                  "CPP-CONTRACT-DIAG-002",
                  "dispatch execution header still exports a diagnostics logger field");
    for (const std::string forbidden : {"\n    errors_only =", "\n    key_transitions =",
                                        "\n    verbose =", "\n    diagnostic ="}) {
        gate.require (execution_hpp.find (forbidden) == std::string::npos, "CPP-CONTRACT-DIAG-001",
                      "legacy diagnostics level remains public: " + forbidden);
    }

    /* CPP-G0-METRIC-001 — generic raw events and metric DTOs stay private. */
    gate.require (
      !std::filesystem::exists (include_root / "zlink/framework/contracts/eventing/events.hpp"),
      "CPP-G0-METRIC-001", "raw event contract header is still public");
    for (const std::string forbidden :
         {"metrics_builder_t", "metric_event_payload_t", "socket_event_payload_t"}) {
        gate.require (!tree_contains (include_root, forbidden), "CPP-G0-METRIC-001",
    ...
    for (const std::string required :
         {"relocation_options_t", "relocation_result_t", "termination_result_t"}) {
        gate.require (tree_contains (include_root, required), "CPP-G0-DRAIN-001",
                      "host lifecycle contract type is missing: " + required);
    }
    for (const std::string forbidden : {"drain_result_t", "await_drained", "retire ("}) {
        gate.require (app_hpp.find (forbidden) == std::string::npos, "CPP-G0-DRAIN-001",
                      "legacy host lifecycle surface is still public: " + forbidden);
    }
    gate.require (
      location_records_hpp.find ("framework_runtime_state_t state") != std::string::npos,
      "CPP-G0-DRAIN-001", "private location descriptor lacks the typed lifecycle state");
    gate.require (!tree_contains (include_root, "mesh_node_drain_policy_t"), "CPP-G0-DRAIN-001",
    ...
    const auto relocation_dispatch = app_runtime.find ("join_application_actor_to_entry_spot");
    ...
    gate.require (relocation_dispatch != std::string::npos && accepted_barrier != std::string::npos
                    && accepted_barrier < stream_barrier && stream_barrier < spot_close
                    && spot_close < owner_cleanup,
                  "CPP-G0-DRAIN-001",
     ...
     * boundary. */
    gate.require (
      mesh_node_runtime.find ("\"commit_ack\", actor, transfer_id") == std::string::npos
        && spot_runtime.find ("emit_actor_transfer_marker (\"source_cleanup\"")
             != std::string::npos,
      "E2E-CP-51",
    ...
    const auto source_finalizer_end =
      source_finalizer_begin == std::string::npos
        ? std::string::npos
        : mesh_node_runtime.find ("mesh_node_runtime_t::reserve_application_actor_join_barrier (",
                                  source_finalizer_begin);
    const auto source_finalizer =
      source_finalizer_begin == std::string::npos || source_finalizer_end == std::string::npos
        ? std::string{}
        : mesh_node_runtime.substr (source_finalizer_begin,
                                    source_finalizer_end - source_finalizer_begin);
    const auto source_fence =
      source_finalizer.find ("const auto source =\n      runtime::protocol::actor_route_fence_t{");
    const auto target_fence =
      source_finalizer.find ("const auto target = runtime::protocol::actor_route_fence_t{");
    const auto source_follow_publish =
      source_finalizer.find ("co_await spot.complete_remote_actor_transfer (");
    ...
    const auto duplicate_target_finalize =
      target_finalize == std::string::npos
        ? std::string::npos
        : spot_route_dispatcher.find ("finalize_remote_actor_to_spot_async (", target_finalize + 1);
    gate.require (
      target_authority_commit != std::string::npos && target_backlog_stage != std::string::npos
        && target_backlog_stage < target_authority_commit && target_finalize != std::string::npos
        && target_finalize > target_authority_commit && source_fence != std::string::npos
        && target_fence > source_fence && source_follow_publish > target_fence
        ...
        && source_finalizer.find ("request_actor_join_spot_route") == std::string::npos
        && source_finalizer.find ("send_to_spot (") == std::string::npos
        && spot_route_packets_hpp.find ("__zlink.spot.actor.leave") != std::string::npos
        && spot_route_dispatcher.find ("send_actor_leave_notification (") != std::string::npos
        && mesh_node_runtime.find ("completion_request") == std::string::npos
        && spot_runtime.find ("poll_deferred_actor_join_completions") == std::string::npos
        && spot_route_packets.find ("coreReserveMessageCount") == std::string::npos
        && spot_route_packets.find ("coreReserveByteCount") == std::string::npos
    ...
    const auto target_finalizer_begin =
      spot_runtime.find ("void spot_node_runtime_t::finalize_remote_actor_to_spot_async (");
    ...
    const auto target_finalizer =
      target_finalizer_begin == std::string::npos || target_finalizer_end == std::string::npos
        ? std::string{}
        : spot_runtime.substr (target_finalizer_begin,
                               target_finalizer_end - target_finalizer_begin);
    gate.require (
      mesh_node_runtime.find ("deliver_remote_actor_join(*s,accepted)") == std::string::npos
        && target_finalizer.find ("actor_join_accepted_t{") != std::string::npos
        && target_finalizer.find ("deliver_actor_join_completion_async (") != std::string::npos
        && target_finalizer.find ("stage_commit_backlog (") == std::string::npos,
      "E2E-CP-53",
      "target does not exclusively own the Accepted OperationId or the finalizer restages the "
      "source prefix");
    const auto actor_client_runtime =
      read_text_file (root / "framework/src/runtime/actors/actor_client.cpp");
    const auto send_begin =
      actor_client_runtime.find ("task_t<void> send_erased (actor_id_t actor_id");
    ...
    gate.require (actor_send.find ("stale_policy_t::location_stale") == std::string::npos
                  ...
                  "E2E-CP-54", "explicit actor send still re-resolves and retries stale refs");
    gate.require (spot_runtime.find ("ZLINK_FRAMEWORK_CPP_ACTOR_HANDOFF_MARKERS")
                  ...
                  "E2E-CP-57", "spot runtime still emits environment-gated stderr handoff markers");
    ...
    gate.require (spot_runtime.find ("\"location_committed\"") != std::string::npos,
                  "CPP-G0-E2E-004", "spot runtime does not emit committed Location evidence");
    gate.require (mesh_node_runtime.find ("ZLINK_FRAMEWORK_CPP_ACTOR_HANDOFF_MARKERS")
                      == std::string::npos
                    && mesh_node_runtime.find ("emit_backlog_enqueued_marker") == std::string::npos,
                  "E2E-CP-57", "actor bridge still emits environment-gated stderr handoff markers");
    ...
    gate.require (stream_bind_begin != std::string::npos && stream_remote_bind != std::string::npos
                    && stream_host.find ("request_to_node", stream_bind_begin) == std::string::npos,
                  "E2E-CP-14", "SM-D2 Session binding still depends on a RouteMesh request path");
    ...
    gate.require (cmake.find ("add_zlink_framework_extension") == std::string::npos, "IMP-CP-28",
                  "unsupported no-op framework extension targets remain exported");
    ...
    gate.require (messaging_test.find (R"("errorCode":"not_found")") != std::string::npos,
    ...
    gate.require (location_auto_connect.find ("failure_started_at") != std::string::npos
                    ...
                    && location_auto_connect.find ("retry_pending_targets") != std::string::npos,
    ...
    gate.require (redis_hpp.find ("owner_is_live") == std::string::npos, "E2E-CP-39",
                  "Redis store still filters rows by owner lease");
    gate.require (live_location_reader.find ("list_owner_leases") == std::string::npos
                    && live_location_reader.find ("read_owner_lease") != std::string::npos
    ...
    gate.require (redis_hpp.find ("parse_scan_state") == std::string::npos
                  ...
                  "IMP-CP-36", "Redis public Store still exposes a provider-specific paging codec");

    /* IMP-CP-01 — subscription dispatch key is topic plus decoded packet name. */
    gate.require (spot_runtime.find ("descriptor.packet_name == *packet_name") != std::string::npos,
    ...
    gate.require (zlink_builder_hpp.find ("channels () const") == std::string::npos
                    && zlink_builder_hpp.find ("route_channels () const") == std::string::npos
    ...
    gate.require (mesh_node_host_service.find ("framework_error_kind_t::capacity_exceeded")
                    == std::string::npos,
                  "CPP-DISP-001",
    ...
    gate.require (
      app_runtime.find ("result.outcome == relocation_outcome_t::relocated") != std::string::npos
        && app_runtime.find ("operation.started = operation.terminal") != std::string::npos
        && app_runtime.find ("completed_worker.join ()") != std::string::npos,
      "CPP-RELOC-001", "blocked relocation results are still retained as terminal state");
    ...
    gate.require (app_runtime.find ("relocation_topology_preflight_until") != std::string::npos
                    && app_runtime.find ("wait_for_relocation_target") != std::string::npos
                    && app_runtime.find ("relocation_topology_poll_interval") != std::string::npos
                    && app_runtime.find ("options ().polling_interval") != std::string::npos,
                  "CPP-RELOC-002",
    ...
    gate.require (public_host_runtime.find ("_transport->signal_activity ()") != std::string::npos,
                  "CPP-DISP-005",
    ...
    gate.require (raw_mesh_node_owner.find ("protocol::framework_error_code::workerQueueFull")
                      == std::string::npos
                    && raw_mesh_node_owner.find ("reply-worker-queue-full") == std::string::npos,
                  "CPP-DISP-003", "pre-admission still sends a terminal queue-capacity reply");
    ...
    gate.require (
      legacy_submit_runtime_absent
        && m6a_sources.find ("framework/src/runtime/messaging/call_facade_runtime.cpp")
             != std::string::npos
        && m6a_sources.find ("framework/src/runtime/messaging/logical_multicast_runtime.cpp")
             != std::string::npos
        && !std::filesystem::exists (messaging_runtime / "logical_multicast_runtime.hpp")
      ...
      "CPP-HWM-ASYNC-001",
      "Framework still owns legacy submit retry state or the replacement runtimes are not "
      "production members");
    ...
    gate.require (call_facade_runtime.find ("return task_t<void> (terminal_result (submit ()))")
                      != std::string::npos
    ...
    gate.require (
      m6a_sources.find ("framework/src/runtime/messaging/logical_multicast_runtime.cpp")
          != std::string::npos
        && logical_multicast_runtime.find ("job.completion->complete (result_t<void>::success ())")
             != std::string::npos
        && logical_multicast_runtime.find ("bool stopping = false;") != std::string::npos
        && logical_multicast_runtime.find ("if (stopping) {\n            job.completion->complete")
             != std::string::npos
    ...
    gate.require (
      spot_runtime.find ("auto queue = state_sync ([this] {") != std::string::npos
        ...
        && spot_runtime.find ("admission_token->inherit_materialized_context") != std::string::npos
    ...
    gate.require (
      app_runtime.find ("make_instance_spot_activation_trace_context") != std::string::npos
        && app_runtime.find ("if (!may_emit)") != std::string::npos
        && app_runtime.find ("return std::nullopt;") != std::string::npos
        && app_runtime.find ("message_flow_tracer_t (dispatch).trace") != std::string::npos,
      "CPP-OBS-001", "Instance Spot activation tracing is not gated before event allocation");
    ...
    gate.require (flow_context.find ("diagnostics_mode") != std::string::npos
                    && flow_context.find ("scope_t (std::nullopt)") != std::string::npos
                    && message_flow_tracer.find ("current->diagnostics_mode") == std::string::npos
                    && message_flow_tracer.find ("effective_message_flow (") != std::string::npos,
                  "CPP-OBS-002",
    ...
    gate.require (
      message_flow_tracer.find ("sample_current (") != std::string::npos
        ...
        && diagnostic_event_sink.find ("log_if_configured") != std::string::npos
    ...
    gate.require (
      actor_gateway_runtime.find ("auto task = completion->task ()") != std::string::npos
        && actor_gateway_runtime.find ("pending.completion->complete (result)") != std::string::npos
        && actor_gateway_runtime.find (
             "[state, actor_id, pending = std::move (*pending), dispatch, dispatched,")
             != std::string::npos
        && actor_gateway_runtime.find ("continue_gate] (const result_t<void> &result) mutable {")
             != std::string::npos
        && actor_gateway_runtime.find (
             "relay_source = std::move (relay_source)] () mutable -> task_t<void> {")
             != std::string::npos
        && actor_gateway_runtime.find ("const auto dispatched = co_await dispatcher (")
             != std::string::npos
    ...
    gate.require (authority_key_codec.find ("\"zla1:\"") != std::string::npos
                    && authority_key_codec.find ("0123456789ABCDEF") != std::string::npos
                    && authority_key_codec.find ("object_id.size ()") != std::string::npos,
                  "CPP-WIRE-001",
                  "canonical authority key codec is missing its version, byte length, or uppercase "
                  "percent encoding");
    for (const auto &[name, source] : std::array{
           ...
           std::pair{"Actor client", &actor_client}, std::pair{"host runtime", &app_runtime},
        ...
        gate.require (source->find ("\"1:\"") == std::string::npos
     ...
     * codec. */
    gate.require (
      generated_service_wire_constants.find ("enum class request_terminal_result")
          != std::string::npos
        && generated_service_wire_constants.find ("valid_terminal_failure") != std::string::npos
        && generated_service_wire_constants.find ("blobBytes") != std::string::npos
        && service_wire_codec.find ("bool valid_terminal_failure") == std::string::npos
        && service_wire_codec.find ("maximum_bytes = blobBytes") != std::string::npos
        && service_wire_codec.find ("offset - start > metadataBytes") != std::string::npos,
      "CPP-WIRE-006", "C++ service codec still redefines schema terminal or bound knowledge");
    ...
    gate.require (
      client_server_location_runtime.find ("framework_error_kind_t::not_configured")
          != std::string::npos
    ...
    gate.require (
      mesh_node_hpp.find ("std::optional<std::chrono::milliseconds> receive_timeout")
        ...
        && raw_mesh_node_owner.find ("router->options ().recv_timeout (*_options.receive_timeout);")
             != std::string::npos,
    ...
    gate.require (call_id.find ("struct call_id_t") != std::string::npos
                    && call_id.find ("struct operation_id_t") == std::string::npos,
                  "CPP-LAYER-002", "in-flight runtime calls still use the OperationId name");
    gate.require (service_wire_codec_header.find ("struct wire_operation_id_t") != std::string::npos
                    && service_wire_codec_header.find ("using wire_operation_id_t")
                         == std::string::npos,
                  "CPP-LAYER-002", "Actor Join OperationId is not a distinct wire type");
    ...
    gate.require (framework_message.find ("with_encoded_payload") != std::string::npos
                    && framework_message.find ("return *_encoded;") == std::string::npos,
                  "CPP-OWN-006",
                  "framework message access still copies an already encoded payload");
    gate.require (mesh_node_runtime.find ("payload.to_bytes ().size ()") == std::string::npos
                    && raw_fanout_owner.find ("parts.front ().to_bytes ()") == std::string::npos,
                  "CPP-OWN-006",
    ...
    gate.require (
      ...
      "CPP-OWN-004", "default JSON serialization still round-trips through a binding message");
    ...
    gate.require (serializer_header.find ("const std::string &content_type ()") != std::string::npos
                    && tree_contains (
                      include_root, "inbound content type does not match the typed handler codec"),
                  "CPP-OWN-001",
    ...
    gate.require (
      serializer_header.find ("[serialize = std::move (serialize)]") != std::string::npos
        && serializer_header.find ("[this, type]") == std::string::npos,
      "CPP-OWN-008", "cached custom serializers still look up erased functions per message");
     ...
     * bytes again. */
    gate.require (raw_stateful_dispatch.find ("auto frozen = std::move (pending->second.frozen)")
                      != std::string::npos
                    && raw_stateful_dispatch.find ("auto payload = std::move (*frozen.application)")
                         != std::string::npos
                    && raw_stateful_dispatch.find ("owner, *turn,") == std::string::npos,
                  "CPP-OWN-003",
    ...
    for (const std::string required :
         {"framework_error_kind_t::invalid_operation", "framework_error_kind_t::internal_failure",
          "framework_error_kind_t::unavailable"}) {
        gate.require (actor_transfer_coordinator.find (required) != std::string::npos,
                      "CPP-FOLLOW-001",
                      "Message Follow admission omits typed failure: " + required);
    }
    gate.require (
      mesh_node_runtime.find ("__zlink.messageFollowVisitedNodes") != std::string::npos
        && mesh_node_runtime.find ("follow_path.value ().visited.contains") != std::string::npos,
      "CPP-FOLLOW-001", "Message Follow still relies only on the hop ceiling for loop detection");
    gate.require (
      actor_transfer_coordinator.find ("candidate.source_fence == source_fence")
          != std::string::npos
        && actor_transfer_coordinator.find ("source_fence.object_generation != generation")
             != std::string::npos,
      "CPP-FOLLOW-001", "Message Follow does not preserve its exact Actor source fence");
    ...
    gate.require (
      raw_stateful_dispatch.find ("matches_application_route") != std::string::npos
        && raw_stateful_dispatch.find ("accepted_target.object_generation =") != std::string::npos
        && raw_stateful_dispatch.find ("actor.target.owner_lease_generation") != std::string::npos
        && raw_stateful_dispatch.find ("spot.target.owner_lease_generation") != std::string::npos,
      "CPP-LIFE-001",
      "general application admission does not normalize generation or fence the current owner "
      "lease");
    gate.require (spot_runtime.find ("const bool admitted =") != std::string::npos
                    && spot_runtime.find ("targets_local_actor") != std::string::npos
                    && spot_runtime.find ("targets_exact_incarnation") != std::string::npos
                    && spot_runtime.find ("requires_exact_incarnation") != std::string::npos
                    && spot_runtime.find ("framework_error_kind_t::invalid_operation")
                         != std::string::npos,
                  "CPP-LIFE-001",
    ...
    gate.require (
      replacement_source.find ("asio::steady_timer") != std::string::npos
        && replacement_source.find ("expires_after (std::chrono::milliseconds (100))")
             != std::string::npos
        && replacement_source.find ("async_wait") != std::string::npos
        && replacement_source.find ("std::this_thread::sleep_for") == std::string::npos
    ...
    for (const std::string required :
         {"spot_lane_policy_t", "spot_lane_lifecycle_t", "return_wait", "relocation_sealed",
          "session_lane_policy_t", "session_lane_lifecycle_t", "connection_closed",
          "actor_delivery_lane_policy_t", "std::variant<"}) {
        gate.require (serial_execution_queue.find (required) != std::string::npos, "CPP-SESS-003",
    ...
    for (const std::string required : {"execute_application (", "execute_control (",
                                       "execute_infrastructure (", "execute_final ("}) {
        gate.require (session_serial_executor.find (required) != std::string::npos, "CPP-SESS-003",
                      "STREAM session executor omits entrypoint: " + required);
    }
    gate.require (stream_runtime_surface.find ("dispatch_queue") == std::string::npos
