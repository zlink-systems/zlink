...
class game_room_t
  : public fw::spot_t<fw::actor_t>
{
  public:
    explicit game_room_t (
    ...
    const fw::spot_context_t &context () const
      noexcept override
    {
      return _context;
    }
    ...
    void configure () override
    {
        _context.handlers ()
          .add_handler<
            &game_room_t::post_chat> ();
        _context.handlers ()
          .add_handler<
            &game_room_t::get_room_state> ();
    }

    void post_chat (const post_chat_t &message)
    {
        auto line = message.player_id + ": "
          + message.text;
        _chat.push_back (line);
    }

    // The return value is the reply. This handler only reads.
    room_state_t get_room_state (
      const get_room_state_t &) const
    {
    ...
    {
        const auto &body =
          request.decode<open_room_t> ();
        _title = body.title;
        co_return
          fw::spot_create_response_t::accept ();
    }
    ...
    {
        co_return fw::spot_actor_join_result_t
          ::reject ();
    }

    fw::task_t<void> on_actor_joined (
      fw::actor_t &) override
    {
      co_return;
    }

    fw::task_t<void> on_leave_actor (
      fw::actor_t &) override
    {
      co_return;
    }
