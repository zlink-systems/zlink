...
class hello_http_handler_t
{
  public:
    ...
    fw::task_t<fw::http_response_t> handle (const fw::http_request_t &request)
    {
        ...
        auto reply = co_await _routes.request_to_channel ("greeting", hello_t{name}).template async<greeting_t> ();
...
int main (int argc, char **argv)
{
    ...
    app.add_zlink_framework ([] (fw::zlink_framework_options_t &options) {
        ...
        options.http ().listen ("http://0.0.0.0:5083").map_get<hello_http_handler_t> ("/hello/{name}");
    });
