...
struct player_view_t { std::string player_id; int x = 0; int y = 0; std::string zone_id; bool is_bot = false; };
struct node_view_t { std::string node_id; bool registered = false; bool connected = false; bool maintenance = false; std::vector<std::string> zones; int player_count = 0; };
...
struct enter_world_req_t { ZW_PACKET (enter_world_req_t, "EnterWorldReq"); int x = 0; int y = 0; bool is_bot = false; int dir_x = 0; int dir_y = 0; };
...
struct enter_zone_msg_t { ZW_PACKET (enter_zone_msg_t, "EnterZoneMsg"); std::string player_id; int x = 0; int y = 0; bool is_bot = false; bool initial_entry = false; std::optional<std::string> from_node_id; };
...
struct message_follow_probe_res_t { ZW_PACKET (message_follow_probe_res_t, "MessageFollowProbeRes"); std::string probe_id; std::vector<std::uint8_t> payload; };
...
inline void to_json (nlohmann::json &j, const watch_nodes_req_t &) { j = nlohmann::json::object (); }
inline void from_json (const nlohmann::json &, watch_nodes_req_t &) {}
#define NODES_FIELDS(F) F (nodes, "nodes")
#define NODE_STATUS_FIELDS(F) NODE_VIEW_FIELDS(F)
...
ZW_JSON_FIELDS (node_status_notify_t, NODE_STATUS_FIELDS)
...
inline void to_json (nlohmann::json &j, const bot_tick_msg_t &) { j = nlohmann::json::object (); }
inline void from_json (const nlohmann::json &, bot_tick_msg_t &) {}
...
#define ENTER_ZONE_FIELDS(F) F (player_id, "playerId") F (x, "x") F (y, "y") F (is_bot, "isBot") F (initial_entry, "initialEntry") F (from_node_id, "fromNodeId")
...
#define FOLLOW_RES_FIELDS(F) F (probe_id, "probeId") F (payload, "payload")
...
ZW_JSON_FIELDS (enter_zone_msg_t, ENTER_ZONE_FIELDS)
