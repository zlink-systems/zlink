...
class ops_console_registry_t
{
  public:
    ...
    {
        co_await send (node_status_notify_t{
          node.node_id, node.registered, node.connected,
          node.maintenance, node.zones, node.player_count});
    }

    fw::task_t<void> publish_alert (const node_alert_notify_t &alert)
    {
        co_await send (alert);
    }
  ...
  private:
    template <typename T> fw::task_t<void> send (const T &message)
    {
        ...
        {
            std::lock_guard lock (_mutex);
            for (const auto &[_, stream] : _streams) streams.push_back (stream);
        }
        ...
        for (auto &stream : streams) {
            try {
                writes.push_back (
                  stream.write_packet (zlink::message_t::from_json (message))
...
{
  public:
    node_view_t report (const report_node_status_msg_t &report,
                        const std::string &source_rid)
    {
        std::lock_guard lock (_mutex);
    ...
    std::vector<node_view_t> nodes () const
    {
        ...
        for (const auto &[_, node] : _nodes) result.push_back (node);
        return result;
    }
    ...
    std::optional<node_view_t> find (const std::string &node_id) const
    {
        ...
        return found == _nodes.end () ? std::nullopt
                                     : std::optional<node_view_t> (found->second);
    }

    std::optional<node_view_t> set_maintenance (const std::string &node_id,
                                                bool enabled)
    {
        std::lock_guard lock (_mutex);
        const auto found = _nodes.find (node_id);
        ...
        for (const auto &[source_id, source] : _nodes) {
            ...
            for (const auto &source_zone : {std::string ("zone-nw")}) {
                for (const auto &[target_id, target] : _nodes) {
                    if (source_id == target_id || !target.registered) continue;
                    for (const auto &target_zone : target.zones) {
                        if (!adjacent (source_zone, target_zone)) continue;
                        const auto source_rid = _rid_by_node.find (source_id);
                        const auto target_rid = _rid_by_node.find (target_id);
                        if (source_rid == _rid_by_node.end ()
                            || target_rid == _rid_by_node.end ())
                            continue;
                        return {source_zone, target_zone, source_rid->second,
                                target_rid->second, std::nullopt};
                    }
                }
            }
        }
...
{
  public:
    using dependency_types = fw::dependency_list_t<
      ops_state_t, ops_console_registry_t, ops_notification_queue_t>;
    report_node_status_handler_t (
      ops_state_t &state,
    ...
    fw::task_t<void> handle (
      const report_node_status_msg_t &report,
      const fw::route_message_context_t &context)
    {
        ...
        std::cout << "zoneworld-node-report node=" << node.node_id
                  << " rid=" << rid << " registered=true" << std::endl;
...
class report_spot_event_handler_t
{
  public:
    ...
    explicit report_spot_event_handler_t (ops_console_registry_t &consoles) :
        _consoles (consoles) {}
    fw::task_t<void> handle (const report_spot_event_msg_t &report)
    {
        std::cout << "zoneworld-node-alert node=" << report.node_id
                  << " kind=" << report.kind << " " << report.detail
                  << std::endl;
...
class ops_monitor_service_t final : public fw::hosted_service_t
{
  public:
    void start (fw::service_provider_t &services) override
    {
        ...
        try { apply_snapshot (runtime.snapshot (names_t::mesh)); } catch (...) {}
        _running.store (true);
    ...
    void request_stop () noexcept override
    {
        ...
        if (_observation) _observation->close ();
    }
...
class ops_session_t final : public fw::packet_stream_session_t
{
  public:
    using dependency_types = fw::dependency_list_t<
      ops_state_t, ops_console_registry_t, fw::publisher_t,
      fw::route_client_t, maintenance_store_t>;

    ops_session_t (ops_state_t &state, ops_console_registry_t &consoles,
                   fw::publisher_t &publisher, fw::route_client_t &routes,
                   maintenance_store_t &maintenance) :
        _state (state), _consoles (consoles), _publisher (publisher),
        _routes (routes), _maintenance (maintenance)
    {
    }
    ...
    fw::task_t<void> on_error (fw::stream_t &, const fw::stream_error_t &) override
    { co_return; }
                                ...
                                const zlink::message_t &payload) override
    {
        ...
        if (packet == watch_nodes_req_t::packet_name) {
            stream.reply_packet (zlink::message_t::from_json (
              watch_nodes_res_t{_state.nodes ()})).submit ();
            co_return;
        }
        if (packet == relocation_pair_req_t::packet_name) {
            stream.reply_packet (zlink::message_t::from_json (
              _state.relocation_pair ())).submit ();
            co_return;
        }
        if (packet == announce_world_req_t::packet_name) {
            ...
            co_await _publisher.publish (
              names_t::broadcast_channel, names_t::announce_topic,
              world_announce_event_t{id, request.text}).submit ();
            stream.reply_packet (zlink::message_t::from_json (
              announce_world_res_t{id})).submit ();
            co_return;
        }
            ...
            try {
                ...
                const auto applied = co_await _routes
                  .request_to_channel (
                    ...
                    apply_node_maintenance_req_t{request.node_id, request.enabled})
                  .timeout (std::chrono::seconds (10))
                  .submit<apply_node_maintenance_res_t> ();
                co_await _publisher.publish (
                  names_t::broadcast_channel, names_t::maintenance_topic,
                  node_maintenance_changed_event_t{request.node_id, request.enabled}).submit ();
                if (const auto changed = _state.set_maintenance (
                      request.node_id, request.enabled))
                    co_await _consoles.publish (*changed);
                stream.reply_packet (zlink::message_t::from_json (
                  set_maintenance_res_t{applied.node_id, applied.enabled,
                                        applied.zones, std::nullopt})).submit ();
            }
            catch (const fw::framework_exception_t &error) {
                stream.reply_packet (zlink::message_t::from_json (
                  set_maintenance_res_t{request.node_id, request.enabled, {},
                                        error.kind () == fw::framework_error_kind_t::deadline_exceeded
                                          ? errors_t::deadline_exceeded : errors_t::unavailable})).submit ();
            }
            co_return;
        }
        if (packet == node_diagnostics_req_t::packet_name) {
            ...
            try {
                ...
                stream.reply_packet (zlink::message_t::from_json (
                  node_diagnostics_res_t{result.node_id, result.zones,
                                         result.player_count, result.maintenance,
                                         std::nullopt})).submit ();
            }
            catch (const fw::framework_exception_t &) {
                stream.reply_packet (zlink::message_t::from_json (
                  node_diagnostics_res_t{request.node_id, {}, 0, false,
                                         errors_t::unavailable})).submit ();
            }
            co_return;
        }
...
int main (int argc, char **argv)
{
    ...
    app.add_zlink_framework ([&] (fw::zlink_framework_options_t &options) {
        options.services ()
          ...
          .add_singleton<maintenance_store_t> (
            std::make_unique<maintenance_store_t> (configuration));
        ...
        mesh.set_automatic_routing_id_prefix ("ops")
          .set_object_role (fw::object_role_t::client)
          .listen (configuration.mesh_endpoint);
        ...
        mesh.channel_name (names_t::ops_channel ("zone-node-1")).client ();
        mesh.channel_name (names_t::ops_channel ("zone-node-2")).client ();
