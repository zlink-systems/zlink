...
| RL-A3 | 구현 | 공통 시나리오와 비교해 100개 worker, 정상 SIGTERM, replacement readiness, worker별 unique request를 구현했다. readiness 판정은 `status.IsReady`, channel `IsReady`, `ReadyTargetCount`를 public status에서 확인한다. 같은 endpoint의 replacement 과정에서 이전 `Connecting` physical transport가 재생성되던 구현 gap을 exact physical RID 정리, endpoint reconnect 취소, stale `Admit/Update` 차단으로 수정했다. 100개 worker의 replacement readiness와 unique reply를 확인했다(`logs/20260806-191757-1446836`). 구현 세부는 `framework/doc/framework/dotnet/internals/graceful-drain-bounded-teardown.ko.md`에 기록했다. |
| RL-A4 | 구현 | drain 뒤 green endpoint 전환과 요청 복구를 확인했다(`logs/20260720-033205-1968652`). |
| RL-A5 | 구현 | Provider B 종료·replacement를 5회 반복하고, down 구간의 Provider A 처리, current replacement의 topology 수렴, Provider A weight 0 뒤 Provider B directed request와 weight 복원을 확인했다(`logs/20260806-175549-2881034`). 구현 세부는 `framework/doc/framework/dotnet/internals/resilience-a5-provider-flapping-e2e.ko.md`에 기록했다. |
