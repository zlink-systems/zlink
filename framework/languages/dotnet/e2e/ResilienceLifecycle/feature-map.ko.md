...
| RL-A3 | 재검증 중 | 공통 시나리오와 비교해 100개 worker, 정상 SIGTERM, replacement readiness, worker별 unique request를 구현했다. readiness 판정은 `status.IsReady`, channel `IsReady`, `ReadyTargetCount`를 public status에서 확인한다. 현재 fresh 실행에서 Provider B의 정상 SIGTERM이 `Draining` 이후 60초 안에 완료되지 않아 완료로 판정하지 않는다(`logs/20260806-181543-1345260`). |
