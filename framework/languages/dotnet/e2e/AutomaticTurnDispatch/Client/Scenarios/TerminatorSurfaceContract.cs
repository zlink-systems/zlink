// Verifies the public terminator surface used by TD-G1.
...
internal static class TerminatorSurfaceContract
{
    public static Task RunAsync(ExecutionTurnScenarioContext context)
    {
        ...
        ZlinkStreamAssert.Ensure(methods.Contains("Async"),
            "TD-G1 standalone HTTP request builder is missing Async.");
        ZlinkStreamAssert.Ensure(!methods.Contains("Submit") && !methods.Contains("Yield"),
            "TD-G1 standalone HTTP request builder exposes server-only terminators.");
    ...
    private static void AssertHttpServerTerminators(Type type)
    {
        ...
        ZlinkStreamAssert.Ensure(methods.Contains("Async"),
            $"TD-G1 {type.Name} is missing Async.");
        ...
        ZlinkStreamAssert.Ensure(methods.Contains("Yield"),
            $"TD-G1 {type.Name} is missing Yield.");
        ZlinkStreamAssert.Ensure(!methods.Contains("Submit"),
            $"TD-G1 {type.Name} exposes Submit.");
    }
    ...
    private static void AssertAwaitedTerminators(Type type)
    {
        ...
        foreach (var name in new[] { "Async", "Yield" })
            ZlinkStreamAssert.Ensure(methods.Contains(name), $"TD-G1 {type.Name} is missing {name}.");
        ZlinkStreamAssert.Ensure(!methods.Contains("Submit"),
            $"TD-G1 {type.Name} exposes the fire-and-forget Submit terminator.");
    }
    ...
    private static void AssertDeferredJoinTerminators(Type type)
    {
        var methods = type.GetMethods()
            ...
            .Select(method => method.Name)
            .ToHashSet(StringComparer.Ordinal);
        ZlinkStreamAssert.Ensure(methods.Contains("Defer"), $"TD-G1 {type.Name} is missing Defer.");
        foreach (var name in new[] { "Async", "Yield", "Submit", "Fetch" })
            ZlinkStreamAssert.Ensure(!methods.Contains(name), $"TD-G1 {type.Name} exposes {name}.");
    }
}
