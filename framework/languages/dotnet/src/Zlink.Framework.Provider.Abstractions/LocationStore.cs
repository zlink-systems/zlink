...
public abstract record ZLinkStoreReadResult
{
    ...
    public sealed record Missing(DateTimeOffset StoreNow) : ZLinkStoreReadResult;

    public sealed record Found(ZLinkStoreValue Value) : ZLinkStoreReadResult;
}
...
public abstract record ZLinkStoreCondition
{
    ...
    public sealed record Missing(ZLinkStoreKey Key) : ZLinkStoreCondition;
...
public abstract record ZLinkStoreMutation
{
    ...
    public sealed record Put(ZLinkStoreKey Key, ReadOnlyMemory<byte> Bytes, TimeSpan? Retention)
        : ZLinkStoreMutation;

    public sealed record Delete(ZLinkStoreKey Key) : ZLinkStoreMutation;
}
...
public abstract record ZLinkStoreWriteResult
{
    ...
    public sealed record Conflict(DateTimeOffset StoreNow) : ZLinkStoreWriteResult;
}

public sealed record ZLinkStoreScanRequest(string Prefix, ZLinkStoreScanCursor? Cursor, int Limit);
...
public abstract record ZLinkStoreScanResult
{
    ...
    public sealed record Page(ZLinkStoreScanPage Value) : ZLinkStoreScanResult;
