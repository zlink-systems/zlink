...
public static class ServiceCollectionExtensions
{
    ...
    )
    {
        ...
        if (
            services.Any(static descriptor =>
                descriptor.ServiceType == typeof(ZLinkFrameworkRegistration)
            )
        )
    ...
    )
    {
        ...
        services.AddKeyedSingleton<ZLinkHttpServerClient>(
            name,
            (provider, _) =>
            {
                ...
                return builder.BuildServer(
                    provider.GetRequiredService<IZLinkHttpExecutionScheduler>()
                );
            }
        );
        return services;
    ...
    public static IHealthChecksBuilder AddZLinkDrainHealthCheck(this IHealthChecksBuilder builder)
    {
