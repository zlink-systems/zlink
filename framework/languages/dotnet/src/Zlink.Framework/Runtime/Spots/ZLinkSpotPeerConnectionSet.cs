...
internal sealed class ZLinkSpotPeerConnectionSet
{
    ...
    private readonly Dictionary<string, RoutingId?> _routerAuto = new(StringComparer.Ordinal);
    private readonly HashSet<string> _routerManual = new(StringComparer.Ordinal);
    private readonly Dictionary<string, RoutingId> _retainedManualPeerRids = new(
    ...
    public void RetainManualPeerRid(string endpoint, RoutingId peerRid)
    {
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                _retainedManualPeerRids[endpoint] = peerRid;
            })
        );
    }
    ...
    {
        if (!source.Remove(endpoint))
            return false;
        return !IsOwned(endpoint);
    }

    private bool IsOwned(string endpoint) =>
        _routerManual.Contains(endpoint) || _routerAuto.ContainsKey(endpoint);
