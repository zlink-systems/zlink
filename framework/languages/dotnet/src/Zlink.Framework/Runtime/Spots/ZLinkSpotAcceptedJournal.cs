...
internal static class ZLinkSpotAcceptedJournal
{
    ...
    )
    {
        ...
        length = checked(
            ...
            + 8
            + 4
            + Encoding.UTF8.GetByteCount(requestSource.OwnerId)
            + 8
            + 1
            + (
    ...
    internal static ZLinkSpotAcceptedJournalRecord Decode(ReadOnlySpan<byte> encoded)
    {
        if (encoded.Length is <= 0 or > MaxRecordBytes)
            throw new InvalidDataException("The accepted Spot journal record size is invalid.");
        ...
        if (version >= 6)
        {
            ...
            if (
                sourceNodeRid is not { } exactSourceNodeRid
        ...
        var requestSequence = reader.ReadBoolean() ? reader.ReadUInt64() : (ulong?)null;
        ...
        if (!ZLinkMeshMetadataCodec.TryDecode(metadataFrame, out var metadata))
            throw new InvalidDataException("The accepted Spot journal metadata is invalid.");
        var partCount = reader.ReadInt32();
        if (partCount < 0 || partCount > MaxParts)
            throw new InvalidDataException("The accepted Spot journal part count is invalid.");
    ...
    {
        return reader.ReadBoolean() ? RoutingId.From(ReadBytes(reader)) : null;
    }
    ...
    private static void WriteSpotId(BinaryWriter writer, string? value)
    {
        ...
        if (!present)
            return;
        WriteBytes(writer, Encoding.UTF8.GetBytes(ZLinkSpotId.Require(value!, nameof(value))));
    }
    ...
    private static string ReadText(BinaryReader reader)
    {
        ...
        {
            throw new InvalidDataException("The accepted Spot journal text is invalid.", error);
        }
    }
