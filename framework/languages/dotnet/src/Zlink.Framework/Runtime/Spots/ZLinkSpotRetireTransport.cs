...
internal sealed record ZLinkSpotRetireHeldRecord(ulong AcceptedSequence, byte[] Payload);

internal sealed class ZLinkCanonicalRelocationDurablyAbortedException(string message)
    : Exception(message);
...
) : IZLinkSpotRetireTarget
{
    ...
    private readonly ConcurrentDictionary<ZLinkAggregateFence, ITargetStageEntry> _staged = new();
    ...
    internal int TerminalTombstoneCount => _staged.Values.OfType<TargetStageTombstone>().Count();

    internal bool TryTrackStage(ZLinkAggregateFence fence, TargetStage stage) =>
        _staged.TryAdd(fence, stage);
    ...
    public async ValueTask<ZLinkSpotRetireReservation?> TryReserveAsync(
        ...
        await ResolveReservationAsync(inventory, null, selection, cancellationToken)
            .ConfigureAwait(false);

    public async ValueTask<ZLinkSpotRetireReservation?> TryReserveForPreflightAsync(
        ZLinkSpotRetireInventory inventory,
        ZLinkRetirePreflightPlan plan,
        ZLinkRelocationTargetSelection selection,
        CancellationToken cancellationToken
    ) =>
        await ResolveReservationAsync(inventory, plan, selection, cancellationToken)
            .ConfigureAwait(false);
    ...
    )
    {
        ...
        var descriptors = await resolver
            .ListLiveMeshNodesAsync(inventory.MeshName, cancellationToken)
            .ConfigureAwait(false);
        ...
        var capacity = new ZLinkCapacityVector(
            plan is not null || !inventory.PerActorShell ? inventory.ActorIds.Count : 0,
            1,
            new ZLinkSpotTypeCapacityDelta(kind, inventory.StableType, 1)
        );
        ...
        var target = targets.FirstOrDefault(candidate =>
            candidate.LeaseGeneration > 0 && (plan is null || plan.TryReserve(candidate, capacity))
        );
        if (target is null)
            return null;
        return new ZLinkSpotRetireReservation(
            ...
            capacity,
            new ZLinkLocationOwnerToken(target.OwnerId, checked((ulong)target.LeaseGeneration))
        );
    }
    ...
    )
    {
        if (
            source.MaintenanceWave is { } sourceWave
                && StringComparer.Ordinal.Equals(sourceWave, candidate.MaintenanceWave)
            || !HasHeadroom(candidate.Capacity.Actors, inventory.ActorIds.Count)
            || !HasHeadroom(candidate.Capacity.Spots, 1)
            || candidate.ActivationConcurrency.Limit - candidate.ActivationConcurrency.Active < 1
        )
            return false;
        ...
        foreach (var required in inventory.RequiredCapabilities)
        {
            var compatible = candidate.ObjectCapabilities.Any(capability =>
                ...
                && StringComparer.Ordinal.Equals(capability.StableType, required.StableType)
        ...
        var typeCapacity = candidate.Capacity.SpotTypes.SingleOrDefault(capacity =>
            ...
            && StringComparer.Ordinal.Equals(capacity.StableType, inventory.StableType)
        );
    ...
    internal static ZLinkServiceWireCodec.RelocationObjectRecord CreateCanonicalRelocationObject(
    ...
    {
        if (
            kind is not (ZLinkPlacementObjectKind.UserSpot or ZLinkPlacementObjectKind.InstanceSpot)
        )
            throw new ArgumentOutOfRangeException(nameof(kind));
        return new ZLinkServiceWireCodec.RelocationObjectRecord(
            (byte)kind,
            kind == ZLinkPlacementObjectKind.InstanceSpot ? stableType : string.Empty,
            spotId,
            objectGeneration,
            kind == ZLinkPlacementObjectKind.InstanceSpot ? 0 : authorityOwnerGeneration
        );
    }

    internal static bool HasHeadroom(ZLinkPopulationCapacity capacity, int required) =>
        required == 0
        || capacity.Limit == 0
    ...
    {
        if (
            runtime.GetSpotNodeRuntime(reservation.Inventory.SourceNodeRid).Node
        ...
        )
        {
            ...
            var idBytes = relocation.Envelope.AggregateId.ToByteArray(bigEndian: true);
            ...
            var targetAttemptGeneration = relocation.Envelope.AggregateGeneration;
            ...
            var prepare = new ZLinkServiceWireCodec.RelocationPrepareRecord(
                ...
                new ZLinkServiceWireCodec.RelocationCoordinatorFence(
                    ...
                    checked((ulong)reservation.Inventory.SourceOwner.LeaseGeneration),
    ...
    public ValueTask AbortAsync(ZLinkSpotRetireReservation reservation, ZLinkAggregateFence? fence)
    {
        _ = reservation;
        if (fence is { } exactFence && _sourceAttempts.TryGetValue(exactFence, out var prepare))
    ...
    )
    {
        ...
        while (true)
        {
            ...
            if (
                ...
                && found.Snapshot.AuthorityOwnerGeneration > spot.AuthorityOwnerGeneration
                && found.Snapshot.OwnerId == reservation.TargetOwner.OwnerId
                && found.Snapshot.OwnerLeaseGeneration == reservation.TargetOwner.LeaseGeneration
                && (
                    ZLinkRelocationAuthorityPayloadCodec.TryDecode(
                        ...
                        && publication.AggregateGeneration == relocation.Fence.AggregateGeneration
    ...
    {
        return CompleteCanonicalSourceAsync(reservation, relocation, held, cancellationToken);
    }
    ...
    )
    {
        ...
        if (!_sourceAttempts.TryGetValue(relocation.Fence, out var prepare))
            return ValueTask.CompletedTask;
        _sourceAttempts.TryRemove(
            new KeyValuePair<ZLinkAggregateFence, ZLinkServiceWireCodec.RelocationPrepareRecord>(
                relocation.Fence,
                prepare
            )
        );
    ...
    )
    {
        ...
        var spotAuthority = candidate.Authorities.Single(static entry =>
        ...
        var instanceSpot =
            spotAuthority.Snapshot.Allocation.ObjectKind == ZLinkPlacementObjectKind.InstanceSpot;
        string spotId;
        string meshName;
        ...
        var actorAuthorities = candidate
            .Authorities.Where(static entry =>
        ...
        var actors = candidate
            .Envelope.Participants.Where(static participant =>
                participant.ObjectKind == ZLinkPlacementObjectKind.Actor
            )
            .Select(participant =>
            {
                var authority = actorAuthorities[participant.AuthorityKey.Value];
                if (
                    !ZLinkRelocationAuthorityPayloadCodec.TryDecode(
        ...
        var existing = _staged
            .Values.OfType<TargetStage>()
            .SingleOrDefault(stage =>
                stage.Envelope.AggregateId == candidate.Envelope.AggregateId
                && candidate.Envelope.AggregateGeneration == stage.Envelope.AggregateGeneration
            );
        if (existing is not null)
        {
            ...
            if (
                !existing.Matches(stagingRequest, sourceNodeRid)
                || !IsStagingPrefix(
                    existing.Envelope,
                    candidate.Envelope,
            ...
            await ReconcilePublishedStageAsync(existing, candidate, cancellationToken)
                .ConfigureAwait(false);
            return;
        }
        ...
        {
            stage = await runtime
                .StageInboundSpotAggregateAsync(
                    request,
                    candidate.Envelope,
        ...
        if (!TryTrackStage(fence, stage))
        {
            ...
            if (
                !_staged.TryGetValue(fence, out var winningEntry)
        ...
        await ReconcilePublishedStageAsync(stage, candidate, cancellationToken)
            .ConfigureAwait(false);
    ...
    )
    {
        ...
        var states = candidate
            .Envelope.Participants.OrderBy(static participant => participant.CanonicalParticipantId)
            .ToArray();
        var authorities = candidate
            .Authorities.OrderBy(static authority =>
    ...
    )
    {
        ...
        stage.RememberFinalRoot(candidate.Reference.Reference, candidate.Reference.ChecksumCrc32c);
        cancellationToken.ThrowIfCancellationRequested();

        await FinalizeStageAsync(stage, normalizeAuthority: false, cancellationToken)
            .ConfigureAwait(false);
        await CompleteInboundThenNormalizeAsync(stage, cancellationToken).ConfigureAwait(false);
    }

    private static void RestoreHeldRecords(TargetStage stage, ZLinkRelocationEnvelope authoritative)
    {
        var stagedSpot = stage.SpotParticipant;
        var authoritativeSpot = authoritative.Participants.Single(static participant =>
            participant.ObjectKind
                is ZLinkPlacementObjectKind.UserSpot
                    or ZLinkPlacementObjectKind.InstanceSpot
        );
        var held = authoritativeSpot
            .AcceptedJobs.Skip(stagedSpot.AcceptedJobs.Count)
    ...
    internal static void ValidateHeldRecords(IReadOnlyList<ZLinkSpotRetireHeldRecord> records)
    {
        long bytes = 0;
        ulong previous = 0;
        foreach (var record in records)
        {
            if (record.AcceptedSequence == 0 || record.AcceptedSequence <= previous)
                throw new InvalidDataException(
    ...
    internal static byte[] ComputeStageRequestDigest(ZLinkCanonicalSpotStageContext request)
    {
        ...
        foreach (var actor in request.Actors)
        {
            ...
            BinaryPrimitives.WriteInt32BigEndian(payloadLength, actor.AuthorityPayload.Length);
    ...
    )
    {
        ...
        var recoveries = envelope
            .Participants.Where(static participant => !participant.RecoveryPayload.IsEmpty)
            .Select(participant =>
                ZLinkCanonicalParticipantRecoveryCodec.Decode(participant.RecoveryPayload.Span)
            )
            .ToArray();
        var actors = recoveries
            .Where(static recovery => recovery.ObjectKind == ZLinkPlacementObjectKind.Actor)
            .Select(recovery =>
            {
                if (
                    !ZLinkActorAuthorityPayloadCodec.TryDecodeRelocating(
                    ...
                    throw new InvalidDataException("Canonical Actor recovery payload is invalid.");
                return new ZLinkCanonicalSpotActorDescriptor(
                    actor.ActorId,
                    actor.StableType,
        ...
        var stableType =
            ...
            ?? await ResolveCanonicalSpotStableTypeAsync(prepare, cancellationToken)
                .ConfigureAwait(false);
        var context = new ZLinkCanonicalSpotStageContext(
            envelope.AggregateId,
            envelope.AggregateGeneration,
            prepare.TargetAttemptGeneration,
            runtime.GetSpotNodeRuntime(prepare.Target.NodeRid).Node.MeshStatus().MeshName,
        ...
        var staged = await StageInboundAsync(context, envelope, sourceNodeRid, cancellationToken)
            .ConfigureAwait(false);
        if (!staged)
            throw new ZLinkFrameworkException(
                ZLinkFrameworkErrorKind.Unavailable,
                "Canonical target staging was rejected.",
                ZLinkRetryAdvice.RetryAfterBackoff
            );
    }
    ...
    )
    {
        ...
        var authorityStore =
            registration.Locations.ResolveStore()
            ?? throw new ZLinkConfigurationException("Location Store is not registered.");
        var key = ZLinkUserSpotAuthorityPayloadCodec.AuthorityKey(prepare.Object.ObjectId);
    ...
    )
    {
        ...
        if (
            !_staged.TryGetValue(fence, out var candidate)
            ...
            || !_staged.TryRemove(
                new KeyValuePair<ZLinkAggregateFence, ITargetStageEntry>(fence, stage)
            )
        )
            return;
        await AbortTargetStageAsync(stage).ConfigureAwait(false);
    ...
    )
    {
        ...
        var expectedStableType =
            spot.ObjectKind == ZLinkPlacementObjectKind.InstanceSpot
                ? stage.StableType
                : string.Empty;
        ...
        if (
            ...
            || coordinator.NodeGeneration != stage.SourceNodeLifecycleGeneration
            || !StringComparer.Ordinal.Equals(coordinator.OwnerId, stage.SourceOwner.OwnerId)
            || coordinator.LeaseGeneration != checked((ulong)stage.SourceOwner.LeaseGeneration)
            ...
            || !StringComparer.Ordinal.Equals(relocationObject.StableType, expectedStableType)
            || !StringComparer.Ordinal.Equals(
    ...
    )
    {
        ...
        var targetOwner =
            ...
            ?? throw new ZLinkConfigurationException("The target owner lease is not established.");
        var participants = stage
            .Envelope.Participants.Select(state =>
            {
                ...
                return new ZLinkAggregateRelocationParticipant(
                    state with
                    {
                        RecoveryPayload = stage.SourceRecoveries[state.AuthorityKey],
                    },
    ...
    private static Guid DecodeRelocationId(ZLinkServiceWireCodec.RelocationWireId relocationId)
    {
        ...
        BinaryPrimitives.WriteUInt64BigEndian(bytes[..8], relocationId.High);
        BinaryPrimitives.WriteUInt64BigEndian(bytes[8..], relocationId.Low);
    ...
    )
    {
        ...
        try
        {
            await CompleteInboundThenNormalizeCoreAsync(stage, cancellationToken)
                .ConfigureAwait(false);
        }
        finally
        {
    ...
    )
    {
        ...
        await runtime
            ...
            .ConfigureAwait(false);
        runtime.ScheduleRelocationSessionRouteConvergence(stage);
        await NormalizeAuthorityAsync(stage, cancellationToken, releaseFinalRoot: false)
            .ConfigureAwait(false);
    ...
    )
    {
        ...
        if (sourceNodeRid.ToHex() != request.SourceNodeRid)
            return false;
        var targetNode = runtime.GetSpotNodeRuntime(RoutingId.FromHex(request.TargetNodeRid));
        ...
        if (
            ...
            || targetStatus.LifecycleGeneration != request.TargetNodeLifecycleGeneration
            || localOwner is not { } exactLocalOwner
            || exactLocalOwner.OwnerId != request.TargetOwnerId
            || exactLocalOwner.LeaseGeneration != checked((long)request.TargetOwnerLeaseGeneration)
        )
            return false;
        ...
        if (_staged.TryGetValue(fence, out var existing))
            ...
            {
                TargetStage active => active.AbortState == TargetStageAbortState.Staged
        ...
        if (!ValidateActorDescriptors(envelope, request))
            return false;
        if (
            !await IsSourceFenceCurrentAsync(request, envelope, cancellationToken)
                .ConfigureAwait(false)
        )
            return false;
        TargetStage stage;
        ...
        {
            await AbortTargetStageAsync(stage).ConfigureAwait(false);
            return _staged.TryGetValue(fence, out var winner)
    ...
    )
    {
        if (envelope.CanonicalLogicalStream.IsEmpty)
            return (
                envelope,
                envelope.Participants.ToDictionary(
                    static state => state.AuthorityKey,
                    static state => state.RecoveryPayload
                )
            );
        var orderedStates = envelope
            .Participants.OrderBy(static participant => participant.CanonicalParticipantId)
            .ToArray();
        if (orderedStates.Length != request.Actors.Length + 1)
            throw new ZLinkRelocationDataLostException(
                "Canonical relocation state count does not match its authority inventory."
            );
        var keys = new[] { ZLinkUserSpotAuthorityPayloadCodec.AuthorityKey(request.SpotId) }
            .Concat(
                request
                    .Actors.Select(static actor =>
        ...
        var sourceRecoveries = new Dictionary<ZLinkAuthorityKey, ReadOnlyMemory<byte>>();
        for (var index = 0; index < keys.Length; index++)
        {
            var read = await authorityStore
                .ReadAuthorityAsync(keys[index], cancellationToken)
                .ConfigureAwait(false);
            ...
            var stableType = index == 0 ? request.StableType : request.Actors[index - 1].StableType;
            var authorityPayload =
                index == 0 ? found.Snapshot.Payload : request.Actors[index - 1].AuthorityPayload;
    ...
    )
    {
        ...
        if (authorityStore is null)
            return false;
        var spot = envelope.Participants.Single(static participant =>
            participant.ObjectKind
                is ZLinkPlacementObjectKind.UserSpot
                    or ZLinkPlacementObjectKind.InstanceSpot
        );
        var requestAggregateBytes = request.AggregateId.ToByteArray(bigEndian: true);
        ...
        foreach (var participant in envelope.Participants)
        {
            ...
            if (
                read is not ZLinkAuthorityReadResult.Found found
                || found.Snapshot.ObjectGeneration != participant.ObjectGeneration
                || found.Snapshot.AuthorityOwnerGeneration != participant.AuthorityOwnerGeneration
            ...
            if (
                ...
                && !string.IsNullOrEmpty(recordedRelocation.State.TargetNodeRid)
                && !StringComparer.Ordinal.Equals(
            ...
            if (participant.ObjectKind == ZLinkPlacementObjectKind.Actor)
            {
                if (
                    ...
                    || actor.NodeGeneration != request.SourceNodeLifecycleGeneration
                )
                    return false;
            }
            else
        ...
        if (request.InstanceSpot)
            return ZLinkInstanceSpotAuthorityPayloadCodec.TryDecode(
                ...
                && instance.NodeGeneration == request.SourceNodeLifecycleGeneration;
        return ZLinkUserSpotAuthorityPayloadCodec.TryDecode(spotSnapshot.Payload.Span, out var user)
            ...
            && user.NodeGeneration == request.SourceNodeLifecycleGeneration;
    }
    ...
    )
    {
        var participants = envelope
            .Participants.Where(static participant =>
                participant.ObjectKind == ZLinkPlacementObjectKind.Actor
            )
            .ToDictionary(
        ...
        {
            if (
                !actorIds.Add(descriptor.ActorId)
                || !ZLinkActorAuthorityPayloadCodec.TryDecodeRelocating(
                ...
                || !participants.ContainsKey(
                    ZLinkActorAuthorityPayloadCodec.AuthorityKey(descriptor.ActorId)
                )
            )
                return false;
        }
    ...
    )
    {
        ...
        if (entry is TargetStageTombstone terminal)
            return terminal.SourceNodeRid == sourceNodeRid
        ...
        {
            Volatile.Write(ref stage.AuthorityPublished, 1);
            await FinalizeStageAsync(stage, normalizeAuthority: true, cancellationToken)
                .ConfigureAwait(false);
            CompleteStage(stage, TargetStageTerminalOutcome.Completed);
            return true;
        }
        return await ReconcileStageAuthorityAsync(stage, cancellationToken).ConfigureAwait(false)
            is not null;
    }
    ...
    )
    {
        ...
        var stagedSpot = stage.SpotParticipant;
        var read = await authorityStore
            .ReadAuthorityAsync(stagedSpot.AuthorityKey, cancellationToken)
            .ConfigureAwait(false);
        if (
            read is not ZLinkAuthorityReadResult.Found found
            || !ZLinkRelocationAuthorityPayloadCodec.TryDecode(
            ...
            || publication.AggregateGeneration != stage.Envelope.AggregateGeneration
            || string.IsNullOrEmpty(publication.Reference)
        )
            throw new ZLinkRelocationDataLostException(
                "Canonical relocation publication is unavailable."
            );
        var durable = await ZLinkRelocationTreeStore
            .GetAsync(
                relocationStore,
        ...
        if (
            ...
            || !durable.InventoryDigest.Span.SequenceEqual(stage.Envelope.InventoryDigest.Span)
        )
            throw new ZLinkRelocationDataLostException(
                "Canonical relocation publication does not match target staging."
            );
        stage.RememberFinalRoot(publication.Reference, publication.ChecksumCrc32c);
    }

    private async ValueTask AbortTargetStageAsync(TargetStage stage)
    {
        await runtime.AbortInboundSpotAggregateAsync(stage).ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        if (candidate is not TargetStage stage || stage.SourceNodeRid != sourceNodeRid)
            return false;
    ...
    internal static bool IsStagingPrefix(
        ...
        Func<ZLinkRelocationParticipantEnvelope, ulong>? targetAuthorityOwnerGeneration = null
    )
    {
        if (
            staging.AggregateId != authoritative.AggregateId
            || !staging.InventoryDigest.Span.SequenceEqual(authoritative.InventoryDigest.Span)
            || staging.Participants.Count != authoritative.Participants.Count
            || staging.CanonicalLogicalStream.IsEmpty
                != authoritative.CanonicalLogicalStream.IsEmpty
            || !staging.CanonicalLogicalStream.IsEmpty
                && (
                    staging.CanonicalRelocationHigh != authoritative.CanonicalRelocationHigh
                    || staging.CanonicalRelocationLow != authoritative.CanonicalRelocationLow
        ...
        {
            if (
                !authoritativeByKey.TryGetValue(expected.AuthorityKey, out var actual)
                || expected.ObjectKind != actual.ObjectKind
                || expected.ObjectGeneration != actual.ObjectGeneration
                || !AuthorityGenerationMatches(expected, actual, targetAuthorityOwnerGeneration)
                || !expected.ApplicationState.Span.SequenceEqual(actual.ApplicationState.Span)
                || !expected.RecoveryPayload.Span.SequenceEqual(actual.RecoveryPayload.Span)
                || expected.ObjectKind
                    is not (
                        ZLinkPlacementObjectKind.UserSpot
                        or ZLinkPlacementObjectKind.InstanceSpot
                    )
                    && !expected.CompletionPayload.Span.SequenceEqual(actual.CompletionPayload.Span)
                || !SameTimers(expected.LogicalTimers, actual.LogicalTimers)
                || actual.AcceptedJobs.Count < expected.AcceptedJobs.Count
                || expected.ObjectKind
                    is not (
                        ZLinkPlacementObjectKind.UserSpot
                        or ZLinkPlacementObjectKind.InstanceSpot
                ...
                return false;
            if (canonical && expected.CanonicalParticipantId != actual.CanonicalParticipantId)
                return false;
            for (var index = 0; index < expected.AcceptedJobs.Count; index++)
                if (
                    expected.AcceptedJobs[index].AcceptedSequence
                        != actual.AcceptedJobs[index].AcceptedSequence
                    || !expected
                        .AcceptedJobs[index]
                        .Payload.Span.SequenceEqual(actual.AcceptedJobs[index].Payload.Span)
                )
                    return false;
            for (var index = 1; index < actual.AcceptedJobs.Count; index++)
                if (
                    actual.AcceptedJobs[index - 1].AcceptedSequence
                    >= actual.AcceptedJobs[index].AcceptedSequence
                )
                    return false;
        }
        ...
        static bool AuthorityGenerationMatches(
            ...
            Func<ZLinkRelocationParticipantEnvelope, ulong>? targetAuthorityOwnerGeneration
        )
        {
            if (targetAuthorityOwnerGeneration is not null)
                return actual.AuthorityOwnerGeneration == targetAuthorityOwnerGeneration(expected);
            return expected.AuthorityOwnerGeneration == actual.AuthorityOwnerGeneration;
        }
        ...
        )
        {
            ...
            for (var index = 0; index < expected.Count; index++)
                if (
                    expected[index].TimerId != actual[index].TimerId
                    || expected[index].DueUnixTimeMilliseconds
                        != actual[index].DueUnixTimeMilliseconds
                    || expected[index].PeriodMilliseconds != actual[index].PeriodMilliseconds
                    || !expected[index].Payload.Span.SequenceEqual(actual[index].Payload.Span)
                )
                    return false;
            return true;
        }
    ...
    )
    {
        ...
        var reads = await Task.WhenAll(
                stage.Envelope.Participants.Select(async participant =>
                    (
        ...
        foreach (var (participant, read) in reads)
        {
            if (
                ...
                || found.Snapshot.OwnerLeaseGeneration != exactLocalOwner.LeaseGeneration
                || !ZLinkAggregateRelocationCoordinator.IsExactNormalizedTargetAuthority(
                    found.Snapshot,
                    participant with
                    {
                    ...
                    stage.Envelope.AggregateId,
                    new ZLinkMeshNodeDescriptorKey(stage.SourceMeshName, stage.Node.Node.RoutingId),
                    stage.Node.Node.MeshStatus().LifecycleGeneration,
                    exactLocalOwner,
                    allowCurrentAuthorityGeneration: stage.TargetAuthorityOwnerGenerationFor(
                        participant
    ...
    )
    {
        if (await IsAuthorityNormalizedAsync(stage, cancellationToken).ConfigureAwait(false))
        {
            if (releaseFinalRoot)
                await ReleaseFinalRootAfterNormalizationAsync(
                        ...
                        reference => DeleteFinalRootBestEffortAsync(reference, cancellationToken)
                    )
                    .ConfigureAwait(false);
            return;
        }
        ...
        foreach (var (participant, read) in authorityReads)
        {
            if (
                ...
                || publication.AggregateId != stage.Envelope.AggregateId
                || publication.AggregateGeneration != expectedPublicationGeneration
                || publication.TargetOwnerId != localOwner.OwnerId
                || publication.TargetOwnerLeaseGeneration != localOwner.LeaseGeneration
            )
                throw new ZLinkRelocationDataLostException(
            ...
            if (publicationReference is null)
            {
                ...
                publication.Reference != publicationReference
                || publication.ChecksumCrc32c != publicationChecksum
                || canonicalPublication.GetValueOrDefault() != publication.IsCanonical
                || !publication.IsCanonical
        ...
        stage.RememberFinalRoot(publicationReference, publicationChecksum);
        ...
        var prepare = await authorityStore
        ...
        if (releaseFinalRoot)
            await ReleaseFinalRootAfterNormalizationAsync(
                    ...
                    () => authorityStore.CommitAggregateForRecoveryAsync(fence, cancellationToken),
                    reference => DeleteFinalRootBestEffortAsync(reference, cancellationToken)
                )
                .ConfigureAwait(false);
        else
            await CommitNormalizationRetainingRootAsync(authorityStore, fence, cancellationToken)
                .ConfigureAwait(false);
    }
    ...
    )
    {
        var commit = await authorityStore
            .CommitAggregateForRecoveryAsync(fence, cancellationToken)
            .ConfigureAwait(false);
    ...
    )
    {
        var relocationStore =
            registration.Locations.ResolveRelocationStore()
            ?? throw new ZLinkConfigurationException("Relocation Store is not registered.");
        await ZLinkRelocationTreeStore
            .DeleteTreeAsync(relocationStore, reference, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    )
    {
        try
        {
            await DeleteFinalRootAsync(reference, cancellationToken).ConfigureAwait(false);
        }
        catch (Exception exception)
        {
            runtime.ErrorSink.ReportRuntimeTaskException("spot-relocation-root-cleanup", exception);
        }
    }
    ...
    )
    {
        ...
        if (
            ...
            && ZLinkUserSpotAuthorityPayloadCodec.TryDecode(sourcePayload.Span, out var user)
        )
        ...
        if (
            ...
            && relocation.Phase == ZLinkActorRelocationAuthorityPhase.Activated
            && ZLinkActorAuthorityPayloadCodec.TryDecodeRelocating(
            ...
            return ZLinkActorRelocationAuthorityPayloadCodec.Encode(
                relocation with
                {
                    ...
                    ApplicationPayload = ZLinkActorAuthorityPayloadCodec.Encode(
                        actor with
                        {
    ...
    private void ScheduleReconciliation()
    {
        ...
        if (Interlocked.CompareExchange(ref _reconciliationRunning, 1, 0) != 0)
            return;
        if (
            !runtime.TryRunDetached("spot-retire-target-reconciliation", RunReconciliationLoopAsync)
        )
            Interlocked.Exchange(ref _reconciliationRunning, 0);
    }

    private async ValueTask RunReconciliationLoopAsync(CancellationToken cancellationToken)
    {
        try
        {
            Task? woken = null;
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                foreach (var stage in _staged.Values.OfType<TargetStage>().ToArray())
                {
                    ...
                    try
                    {
                        await ReconcileStageAuthorityAsync(stage, cancellationToken)
                            .ConfigureAwait(false);
                    }
                    ...
                    catch (Exception exception)
                    {
                        ...
                        {
                            services
                                .GetService<IZLinkRuntimeTerminalFailureSink>()
                                ?.SealError(exception);
                            return;
                        }
                    }
    ...
    private async ValueTask<ZLinkRelocationRecoveryCandidate?> ReconcileStageAuthorityAsync(
        TargetStage stage,
        CancellationToken cancellationToken
    )
    {
    ...
    {
        while (
            _terminalOrder.TryPeek(out var terminalFence)
            && (
                !_staged.TryGetValue(terminalFence, out var terminalEntry)
                || terminalEntry is TargetStageTombstone tombstone && tombstone.ExpiresAt <= now
            )
        )
        {
    ...
    {
        foreach (
            var entry in _staged
                .Where(entry => entry.Value is TargetStage stage && IsCleanupCandidate(stage, now))
                .ToArray()
        )
        {
            var stage = (TargetStage)entry.Value;
            if (
                !_staged.TryGetValue(entry.Key, out var current) || !ReferenceEquals(current, stage)
            )
                continue;
            var authorityStore =
                registration.Locations.ResolveStore()
    ...
    )
    {
        ...
        if (Volatile.Read(ref stage.Published) == 0)
        {
            ...
            if (published is not null || Volatile.Read(ref stage.AuthorityPublished) != 0)
                return false;
    ...
    private static byte[] ComputeHeldDigest(IReadOnlyList<ZLinkSpotRetireHeldRecord> records)
    {
        using var hash = IncrementalHash.CreateHash(HashAlgorithmName.SHA256);
        Span<byte> number = stackalloc byte[8];
        BinaryPrimitives.WriteUInt64BigEndian(number, checked((ulong)records.Count));
        hash.AppendData(number);
        foreach (var record in records)
        {
            BinaryPrimitives.WriteUInt64BigEndian(number, record.AcceptedSequence);
            hash.AppendData(number);
...
) : ITargetStageEntry
{
    internal bool Matches(
        ...
        && (!requireCompleted || Outcome == TargetStageTerminalOutcome.Completed)
        && StageRequestDigest
            .AsSpan()
            .SequenceEqual(ZLinkSpotRetireTargetRuntime.ComputeStageRequestDigest(request));

    internal bool MatchesRelay(RoutingId sourceNodeRid, ReadOnlySpan<byte> relayDigest) =>
        SourceNodeRid == sourceNodeRid
        && Outcome == TargetStageTerminalOutcome.Completed
...
internal sealed record TargetStage(
    ...
    IReadOnlyDictionary<ZLinkActorId, ulong> ActorTargetAuthorityOwnerGenerations,
...
) : ITargetStageEntry
{
    ...
    internal ZLinkRelocationParticipantEnvelope SpotParticipant =>
        _spotParticipant ??= Envelope.Participants.Single(static participant =>
            participant.ObjectKind
                is ZLinkPlacementObjectKind.UserSpot
                    or ZLinkPlacementObjectKind.InstanceSpot
        );
    ...
    {
        return AwaitStateLane(_lane.RunAsync(() => HeldRecords.ToArray()));
    }
    ...
    {
        get => AwaitStateLane(_lane.RunAsync(() => (TargetStageAbortState)_abortState));
        set => AwaitStateLane(_lane.RunAsync(() => _abortState = (int)value));
    }

    internal ulong TargetActorAuthorityOwnerGeneration(string actorId) =>
        TargetActorAuthorityOwnerGeneration(ZLinkActorId.FromBoundary(actorId, nameof(actorId)));

    internal ulong TargetActorAuthorityOwnerGeneration(ZLinkActorId actorId) =>
        ActorTargetAuthorityOwnerGenerations.TryGetValue(actorId, out var generation)
            ? generation
            : throw new ZLinkRelocationDataLostException(
                $"Actor '{actorId}' does not have a prepared target authority generation."
            );

    internal ulong TargetAuthorityOwnerGenerationFor(ZLinkRelocationParticipantEnvelope participant)
    {
        if (
            participant.ObjectKind
            is ZLinkPlacementObjectKind.UserSpot
        ...
        var actor = ActorTargetAuthorityOwnerGenerations.SingleOrDefault(candidate =>
    ...
    )
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var preparation = await _lane.RunAsync(PrepareAbortCleanup).ConfigureAwait(false);
            ...
            try
            {
                ...
                await _lane
                    .RunAsync(() => CompleteAbortCleanup(preparation.Claim!, removed))
                    .ConfigureAwait(false);
                return removed;
            }
            catch (Exception exception)
            {
                await _lane
                    .RunAsync(() => FailAbortCleanup(preparation.Claim!, exception))
                    .ConfigureAwait(false);
                throw;
            }
        }
    ...
    internal bool TryBeginSessionRouteConvergence() =>
        Interlocked.CompareExchange(ref _sessionRouteConvergenceRunning, 1, 0) == 0;
    ...
    internal bool TrySetHeldRecords(
        ...
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                HeldDigest = digest;
                HeldRecords = records
                    .Select(static record => new ZLinkRelocationQueuedJob(
    ...
    internal byte[]? GetHeldRelayDigest() =>
        AwaitStateLane(_lane.RunAsync(() => HeldRelayDigest?.ToArray()));
    ...
    private void CompleteAbortCleanup(TaskCompletionSource<bool> claim, bool removed)
    {
        ...
        _abortState = (int)(removed ? TargetStageAbortState.Aborted : TargetStageAbortState.Staged);
        _abortCleanup = null;
    ...
    internal bool Matches(ZLinkCanonicalSpotStageContext request, RoutingId sourceNodeRid) =>
        SourceNodeRid == sourceNodeRid
        && StageRequestDigest
            .AsSpan()
            .SequenceEqual(ZLinkSpotRetireTargetRuntime.ComputeStageRequestDigest(request));
