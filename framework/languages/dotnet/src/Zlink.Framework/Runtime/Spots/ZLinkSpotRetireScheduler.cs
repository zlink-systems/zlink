...
internal interface IZLinkSpotRetireTarget
{
    ...
    ValueTask AbortAsync(ZLinkSpotRetireReservation reservation, ZLinkAggregateFence? fence);
...
{
    private readonly Dictionary<(ZLinkMeshName MeshName, RoutingId NodeRid), Usage> _usage = [];
    ...
    )
    {
        ...
        if (
            ...
            || descriptor.ActivationConcurrency.Limit - descriptor.ActivationConcurrency.Active
        ...
        if (capacity.SpotType is { } delta)
        {
            var typeCapacity = descriptor.Capacity.SpotTypes.SingleOrDefault(candidate =>
                ...
                && StringComparer.Ordinal.Equals(candidate.StableType, delta.StableType)
            );
            used.SpotTypes.TryGetValue((delta.ObjectKind, delta.StableType), out var usedForType);
            if (
                typeCapacity is null
                || !ZLinkSpotRetireTargetRuntime.HasHeadroom(
        ...
        if (capacity.SpotType is { } committedType)
            used.SpotTypes[(committedType.ObjectKind, committedType.StableType)] = checked(
                used.SpotTypes.GetValueOrDefault(
    ...
    private sealed class Usage
    {
        ...
        internal Dictionary<
            (ZLinkPlacementObjectKind Kind, string StableType),
            int
        > SpotTypes { get; } = [];
    }
}
...
)
{
    ...
    private static CancellationTokenSource CreateDeadlineTokenSource(DateTimeOffset deadline)
    {
        var remaining = deadline - DateTimeOffset.UtcNow;
        var source = new CancellationTokenSource();
        source.CancelAfter(remaining > TimeSpan.Zero ? remaining : TimeSpan.FromTicks(1));
        return source;
    }
    ...
    )
    {
        foreach (var unit in units)
        {
            ...
            if (
                inventory.RequiredCapabilities.Any(static capability =>
            ...
            if (
                await target
                    .TryReserveForPreflightAsync(inventory, plan, selection, cancellationToken)
    ...
    )
    {
        ...
        return new ZLinkSpotRetireInventory(
            ...
            actorIds,
            activation.ResolveRetireCapabilities(instanceSpot, includeActors)
        );
    }
    ...
    {
        var policy =
            activation.ResolveSpotRelocationRegistrationForRetire().PolicyKind == 2
        ...
        try
        {
            ...
            metric.Complete(
                ...
                {
                    ZLinkRelocationUnitOutcome.Completed => ZLinkRelocationMetricOutcome.Completed,
                    ZLinkRelocationUnitOutcome.Pending => ZLinkRelocationMetricOutcome.Aborted,
    ...
    )
    {
        ...
        var perActorShell =
            !instanceSpot && activation.ExecutionMode == ZLinkUserSpotExecutionMode.PerActor;
        var actorIds = perActorShell ? Array.Empty<string>() : activation.SnapshotActorIds();
            ...
            admittedSeal =
                ...
                == ZLinkSpotRelocationCoordinationMode.ApplicationSignaled
                    ? await activation
                        ...
                        .ConfigureAwait(false)
                    : await activation
                        .SealRelocationAsync(allowActorClaims: perActorShell, cancellationToken)
                        .ConfigureAwait(false);
        }
        catch
        {
            activation.CancelRelocationReadyWait();
            await target.AbortAsync(reservation, fence: null).ConfigureAwait(false);
            throw;
        }

        ZLinkFrameworkDebugLog.SpotDiscovery($"relocation_sealed spot={activation.SpotId}");
        var interruption = activation.StartRelocationInterruption(instanceSpot);
        ...
        var actorCaptures = new Dictionary<ZLinkActorId, SourceActorCapture>();
        ...
        var sealedSessionRoutes = new Dictionary<ZLinkActorId, ZLinkRemoteActorBoundSessionRoute>();
        var sessionRelocationContext = default(ZLinkSessionRelocationContext);
        try
        {
            var sealedActorIds = activation.SnapshotActorIds();
            if (!perActorShell && !sealedActorIds.SequenceEqual(actorIds, StringComparer.Ordinal))
            ...
            var coordinatorAuthority = await ReadOwnedAsync(
                    ZLinkUserSpotAuthorityPayloadCodec.AuthorityKey(activation.SpotId),
                    cancellationToken
                )
                .ConfigureAwait(false);
            ...
            var sourceActorStates = actorIds.Select(runtime.GetOrCreateActorState).ToArray();
            foreach (var actorState in sourceActorStates)
            {
                ...
                var actorKey = ZLinkActorId.FromBoundary(actorState.ActorId, nameof(actorState));
                actorCaptures.Add(actorKey, new SourceActorCapture(actorState, sourceActor));
            }
            for (
                var first = 0;
            ...
            {
                var count = Math.Min(MaxConcurrentParticipantIo, sourceActorStates.Length - first);
                await Task.WhenAll(
                        sourceActorStates
                            .Skip(first)
                            .Take(count)
                            ...
                            {
                                _ = await state
                                    .BeginHandoffCaptureAsync(cancellationToken)
                                    .ConfigureAwait(false);
                                actorCaptures[
            ...
            var sealedRoutes = new ZLinkRemoteActorBoundSessionRoute[actorIds.Count];
            for (var first = 0; first < actorIds.Count; first += MaxConcurrentParticipantIo)
            {
                var count = Math.Min(MaxConcurrentParticipantIo, actorIds.Count - first);
                await Task.WhenAll(
                        Enumerable
                            .Range(first, count)
                            .Select(async index =>
            ...
            foreach (var capture in actorCaptures.Values)
            {
                ...
                //  attach.
                capture.State.Handoff.SealCapture(runtime.BeginPendingRelocationUnit());
                capture.CommitBoundary = capture.State.Handoff.FreezeCaptureCommitBoundary();
            }
            ...
            var stagingEnvelope = ZLinkCanonicalSpotRelocationWriter.CreateInitial(
                new ZLinkRelocationEnvelope(
                    ...
                    participants.Select(static participant => participant.Envelope).ToArray()
                ),
                activation.SpotId,
            ...
            var stagingRoot = await new ZLinkRelocationPublicationCoordinator(
                ...
                .ConfigureAwait(false);
            staging = new ZLinkPreparedSpotRetireStaging(stagingRoot, participants);
            ZLinkFrameworkDebugLog.SpotDiscovery(
            ...
            stageStarted = true;
            await target.StageAsync(reservation, staging, cancellationToken).ConfigureAwait(false);
            ZLinkFrameworkDebugLog.SpotDiscovery(
        ...
        catch (ZLinkCanonicalRelocationDurablyAbortedException)
        {
            ...
            try
            {
                await ExecutePrecommitAbortAsync(
                        null,
                        () =>
                            target.AbortAsync(reservation, new ZLinkAggregateFence(aggregateId, 1)),
                        DiscardStagingAsync,
                        async () =>
                        {
                            if (seal is not null)
                            ...
                            await RestoreSourceActorsAsync().ConfigureAwait(false);
                        },
                        async () =>
                        {
                            var handoffId = aggregateId.ToString("N");
        ...
        catch (Exception exception)
        {
            ...
            {
                return await ReconcileUncertainCommitAsync().ConfigureAwait(false);
            }
            if (committed)
            {
        ...
        async ValueTask<ZLinkRelocationUnitResult> ReconcileUncertainCommitAsync()
        {
            try
            {
            ...
            {
                return ZLinkRelocationUnitResult.Terminal(
                    MapFailureReason(exception, committed: targetPublished),
                    targetPublished
                        ? ZLinkRelocationCommitKnowledge.Committed
                        : ZLinkRelocationCommitKnowledge.Unknown,
                    sourceTerminalized: sourceCompleted
        ...
        async ValueTask CompleteCommittedAsync(CancellationToken completionToken)
        {
        ...
        async ValueTask CompleteCommittedWithinDeadlineAsync()
        {
            ...
            await CompleteCommittedAsync(completionDeadline.Token).ConfigureAwait(false);
        }
        ...
        {
            var followTasks = activeActorCaptures.Values.Select(async capture =>
            {
                var key = ZLinkActorAuthorityPayloadCodec.AuthorityKey(capture.State.ActorId);
                var participant = committedPublication.Envelope.Participants.Single(candidate =>
                    candidate.ObjectKind == ZLinkPlacementObjectKind.Actor
                    && candidate.AuthorityKey == key
                );
                var targetRead = await authorityStore
                    .ReadAuthorityAsync(key, completionToken)
                    .ConfigureAwait(false);
                ...
                var trailing = capture.State.Handoff.CutoverCaptureToMessageFollow(
                    ...
                    checked((ulong)activeInventory.SourceOwner.LeaseGeneration),
                    checked((ulong)activeReservation.TargetOwner.LeaseGeneration)
                );
                var backlog = runtime.RelayStandaloneActorRelocationTrailing(
                    capture.State,
                    capture.SourceActor,
                    trailing
                );
                capture.State.Handoff.CommitMessageFollow(
                    runtime.Registration.Locations.Options.MessageFollowDuration,
                    runtime.Registration.Locations.Options.RelocationCutoverWaitTimeout
                );
                return backlog;
            });
            foreach (var backlog in await Task.WhenAll(followTasks).ConfigureAwait(false))
        ...
        {
            foreach (
                var capture in actorCaptures.Values.OrderBy(
            ...
            )
            {
                if (!capture.CaptureStarted)
                    continue;
                await runtime
                    .RestoreStandaloneActorRelocationSourceAsync(capture.State)
                    .ConfigureAwait(false);
            }
        }
    ...
    private async ValueTask<ZLinkAggregateRelocationParticipant[]> BuildParticipantsAsync(
        ZLinkSpotActivation activation,
        ZLinkSpotRelocationSeal seal,
        ZLinkSpotRelocationApplicationState application,
        IReadOnlyList<string> actorIds,
        Guid aggregateId,
        IReadOnlyDictionary<ZLinkActorId, ZLinkRemoteActorBoundSessionRoute> sealedSessionRoutes,
    ...
    )
    {
        ...
        var spotKey = ZLinkUserSpotAuthorityPayloadCodec.AuthorityKey(activation.SpotId);
        var spotTask = ReadOwnedAsync(spotKey, cancellationToken).AsTask();
        var capturedJobsTask = ResolveAcceptedJobsAsync(
            ...
            .AsTask();
        var actorAuthoritiesTask = ReadActorAuthoritiesAsync(actorIds, cancellationToken).AsTask();
        var spot = await spotTask.ConfigureAwait(false);
        var capturedJobs = await capturedJobsTask.ConfigureAwait(false);
        var actorAuthorities = await actorAuthoritiesTask.ConfigureAwait(false);
        var spotKind = activation.PlacementKind;
        var spotStableType =
            spotKind == ZLinkPlacementObjectKind.InstanceSpot
                ? ZLinkInstanceSpotAuthorityPayloadCodec.TryDecode(
                    spot.Payload.Span,
                    out var instanceAuthority
                )
        ...
        var spotRecovery = ZLinkCanonicalParticipantRecoveryCodec.Encode(
            new ZLinkCanonicalParticipantRecovery(
                ...
                MaintenancePolicy: requiredCapabilities
                    .Single(capability =>
                        capability.ObjectKind == spotKind
                        && StringComparer.Ordinal.Equals(capability.StableType, spotStableType)
                    )
                    .Policy
            )
        );
        participants.Add(
            new ZLinkAggregateRelocationParticipant(
                new ZLinkRelocationParticipantEnvelope(
                    ...
                    capturedJobs,
                    includeSpotTimers ? seal.LogicalTimers : [],
        ...
        for (var index = 0; index < actorIds.Count; index++)
        {
            ...
            var boundRoute = sealedSessionRoutes.TryGetValue(actorKey, out var sealedRoute)
                ? sealedRoute
                : default;
            var relocationAuthority = ZLinkActorRelocationAuthorityPayloadCodec.Encode(
            ...
            participants.Add(
                new ZLinkAggregateRelocationParticipant(
                    new ZLinkRelocationParticipantEnvelope(
                        ...
                        CreateActorAcceptedJobs(actorCaptures[actorKey], actor.ObjectGeneration),
                        [],
    ...
    private static IReadOnlyList<ZLinkRelocationQueuedJob> CreateActorAcceptedJobs(
    ...
    )
    {
        ...
        return ZLinkStandaloneActorRelocationRuntime
            .CreateAcceptedRecords(capture.CommitBoundary.Frames)
            .OrderBy(static accepted => accepted.Frame.ArrivalIndex)
            .Select(
                (accepted, index) =>
                    new ZLinkRelocationQueuedJob(
                        checked((ulong)index + 1),
                        ZLinkCanonicalActorAcceptedJournal.Encode(accepted, capture.SourceActor)
                    )
            )
            .ToArray();
    }

    private async ValueTask<ZLinkAuthoritySnapshot[]> ReadActorAuthoritiesAsync(
        IReadOnlyList<string> actorIds,
        CancellationToken cancellationToken
    )
    {
        ...
        for (var first = 0; first < actorIds.Count; first += MaxConcurrentParticipantIo)
        {
            ...
            await Task.WhenAll(
                    Enumerable
                        ...
                        .Select(async index =>
                        {
                            var key = ZLinkActorAuthorityPayloadCodec.AuthorityKey(actorIds[index]);
                            authorities[index] = await ReadOwnedAsync(key, cancellationToken)
                                .ConfigureAwait(false);
                        })
                )
                .ConfigureAwait(false);
    ...
    private async ValueTask<ZLinkAggregateRelocationParticipant[]> AppendHeldIngressAsync(
        string meshName,
        IReadOnlyList<ZLinkAggregateRelocationParticipant> participants,
        IReadOnlyList<ZLinkAcceptedWorkRecord> held,
        CancellationToken cancellationToken
    )
    {
        ...
        var heldJobs = await ResolveAcceptedJobsAsync(meshName, held, cancellationToken)
            .ConfigureAwait(false);
        var accepted = spot
            .Envelope.AcceptedJobs.Concat(heldJobs)
            .OrderBy(static record => record.AcceptedSequence)
            .ToArray();
        for (var index = 1; index < accepted.Length; index++)
            if (accepted[index - 1].AcceptedSequence >= accepted[index].AcceptedSequence)
                throw new ZLinkRelocationDataLostException(
        ...
        return participants
            .Select(participant =>
                participant == spot
                    ? participant with
                    {
                        Envelope = participant.Envelope with { AcceptedJobs = accepted },
                    }
                    : participant
            )
            .ToArray();
    ...
    private ValueTask<IReadOnlyList<ZLinkRelocationQueuedJob>> ResolveAcceptedJobsAsync(
        string meshName,
    ...
    internal static IReadOnlyList<ZLinkRelocationQueuedJob> ResolveFrozenAcceptedJobs(
        IReadOnlyList<ZLinkAcceptedWorkRecord> records
    )
    {
        return records
            .Select(record =>
            {
                ...
                return new ZLinkRelocationQueuedJob(record.AcceptedSequence, record.Payload)
                {
    ...
    )
    {
        ...
        for (var index = 0; index < expected.Count; index++)
            if (
                expected[index].AcceptedSequence != actual[index].AcceptedSequence
