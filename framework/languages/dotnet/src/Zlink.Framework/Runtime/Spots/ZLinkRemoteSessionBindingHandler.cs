...
internal static class ZLinkRemoteSessionBindingHandler
{
    ...
    )
    {
        ...
        var request =
            JsonSerializer.Deserialize<ZLinkRemoteSessionBindRequest>(
        ...
        if (
            !string.Equals(request.ActorId, actor.Context.ActorId, StringComparison.Ordinal)
        ...
        response = await runtime
            .BindRemoteBoundSessionRouteAsync(request, frame.SourceNodeRid, cancellationToken)
    ...
    private static ZLinkActorReply AcknowledgedReply(ZLinkRemoteSessionBindResponse response) =>
        new(
