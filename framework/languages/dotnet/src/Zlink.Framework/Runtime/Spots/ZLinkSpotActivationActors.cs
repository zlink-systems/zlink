...
internal abstract partial class ZLinkSpotActivation
{
    ...
    public bool TryGetJoinedActor(string actorId, out IZLinkActor? actor)
    {
        return _actors.TryGetActor(ZLinkActorId.FromBoundary(actorId, nameof(actorId)), out actor);
    }
    ...
    )
    {
        ...
        var state = new ActorJoinCallState(actor, request, descriptor, absoluteDeadline);
    ...
    {
        if (!_runtime.Flow.Enabled(outcome))
            return;
    ...
    )
    {
        ...
        if (ReferenceEquals(ZLinkSpotAmbientContext.CurrentOrDefault, this))
        {
            state.Result = await HandlerInvoker
    ...
    {
        ArgumentNullException.ThrowIfNull(actor);
        return CommitActorJoinCoreAsync(actor, cancellationToken, absoluteDeadline);
    }

    internal void StageRelocatedPerActorMember(IZLinkActor actor, ZLinkActorRuntimeState actorState)
    {
        ArgumentNullException.ThrowIfNull(actor);
        ...
        return ReferenceEquals(ZLinkSpotAmbientContext.CurrentOrDefault, this)
            ...
            : ExecuteSerializedAsync(
                static (activation, state, ct) =>
                    activation.PrepareTransferredActorJoinAndReplayCoreAsync(
                        state.Actor,
                        state.ActorState,
                        ct
                    ),
    ...
    )
    {
        try
        {
            ...
            _ = await StageActorMembershipAsync(actor, cancellationToken).ConfigureAwait(false);
        }
        catch
        {
            _actors.RemoveIfCurrent(actor);
    ...
    )
    {
        ...
        return _serial.ExecuteApplicationCallbackAsync(
            static (activation, state, ct) =>
                activation.CompleteTransferredActorJoinLifecycleCoreAsync(
                    state.Actor,
                    state.ActorState,
                    state.HandoffId,
                    state.Reply,
    ...
    )
    {
        ...
        return _serial.ExecuteSealedRelocationAsync(
            seal.QueueSeal,
            (activation, ct) => activation.NotifyJoinedActorCoreAsync(actor, ct),
            cancellationToken
        );
    }
    ...
    )
    {
        ...
        await _dispatcher
            .DispatchActorReplayFramesAsync(
    ...
    )
    {
        return ReferenceEquals(ZLinkSpotAmbientContext.CurrentOrDefault, this)
            ...
            : ExecuteSerializedAsync(
                static (activation, state, ct) =>
                    activation.RestoreActorAfterFailedHandoffCoreAsync(state, ct),
                actor,
                cancellationToken
            );
    }
    ...
    )
    {
        ...
        return ReferenceEquals(ZLinkSpotAmbientContext.CurrentOrDefault, this)
            ...
            : ExecuteSerializedAsync(
                static (activation, state, ct) =>
                    activation.NotifyActorLeftAfterJoinCommitCoreAsync(state, ct),
                actor,
                cancellationToken
            );
    }
    ...
    )
    {
        ...
        try
        {
            ...
            previousActivation = await _runtime
                .CommitActorToSpotAsync(
                    this,
                    actor,
                    async ct =>
                    {
    ...
    )
    {
        ...
        while (true)
        {
            ...
            if (localDeadline is { } deadline && deadline <= Stopwatch.GetElapsedTime(0))
                throw new ZLinkFrameworkException(
    ...
    )
    {
        ...
        return await _runtime
            .JoinActorToSpotAsync(this, actor, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        {
            var entrySpot =
                _runtime.GetMeshNodeRuntime(SpotNodeName).EntrySpotActivation
                ?? throw new ZLinkFrameworkException(
                    ZLinkFrameworkErrorKind.NotFound,
                    $"MeshNode '{SpotNodeName}' does not have an Entry Spot activation."
                );
            await locations
                .ActorOwnership.NotifyActorLeftSpotAsync(
                    ZLinkActorId.FromBoundary(actor.Context.ActorId, nameof(actor.Context.ActorId)),
    ...
    {
        if (PerActorShellRelocationPlan is not null && JoinedActorCount == 0)
    ...
    {
        if (!_actors.RemoveIfCurrent(actor))
            return;
    ...
    {
        if (_actorsLeavingForEntrySpot.Remove(RuntimeActorId(actor)))
            return;
    ...
    )
    {
        return await HandlerInvoker
            .InvokeActorJoinAsync(descriptor, actor.Context.ActorId, request, cancellationToken)
            .ConfigureAwait(false);
    }
