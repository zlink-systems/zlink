...
internal sealed record ZLinkSessionBindingEntry(
    ...
    ZLinkServiceWireCodec.SessionRelocationSealRecord? CanonicalRelocationSeal = null,
    ZLinkServiceWireCodec.SessionRelocationSealedRecord? CanonicalRelocationSealResult = null,
    ZLinkServiceWireCodec.SessionRelocationRouteRecord? AppliedCanonicalRelocationRoute = null
)
{
...
internal readonly record struct ZLinkSessionBindingRoute
{
    ...
    )
    {
        ...
        var runtimeMeshName = ZLinkMeshName.FromBoundary(meshName, nameof(meshName));
        if (!string.Equals(actor.MeshName, runtimeMeshName.Value, StringComparison.Ordinal))
        {
            route = default;
            return false;
        }
...
internal readonly record struct ZLinkSessionBindingKey(ZLinkActorId ActorId, string BindingToken)
{
    internal static ZLinkSessionBindingKey FromBoundary(string actorId, string bindingToken) =>
        new(ZLinkActorId.FromBoundary(actorId, nameof(actorId)), bindingToken);
}
...
internal sealed class ZLinkSessionActorBindingTable
{
    ...
    private readonly HashSet<CanonicalRelocationKey> _timedOutCanonicalSeals = [];
    private readonly Queue<CanonicalRelocationKey> _timedOutCanonicalSealOrder = new();
    ...
    {
        internal ZLinkServiceWireCodec.SessionRelocationSealRecord Seal { get; } = seal;
    ...
    )
    {
        internal static CanonicalRelocationKey From(
            ZLinkServiceWireCodec.SessionRelocationSealRecord seal
        ) => new(seal.RelocationId, seal.Coordinator, seal.Actor.Actor, seal.Session);

        internal static CanonicalRelocationKey From(
            ZLinkServiceWireCodec.SessionRelocationRouteRecord route
        ) => new(route.RelocationId, route.Coordinator, route.Actor, route.Session);
    }
    ...
    )
    {
        ...
        if (
            _canonicalRelocationSealTimeout <= TimeSpan.Zero
            || _canonicalRelocationSealTimeout == Timeout.InfiniteTimeSpan
            || _canonicalRelocationSealTimeout.Ticks % TimeSpan.TicksPerMillisecond != 0
        )
    ...
    {
        if (_canonicalSealTimeouts.ContainsKey(key))
            return;
    ...
    {
        if (!_canonicalSealTimeouts.Remove(key, out var timeout))
            return;
        timeout.Cancellation.Cancel();
    }

    private void AddTimedOutCanonicalSeal(ZLinkServiceWireCodec.SessionRelocationSealRecord seal)
    {
        var key = CanonicalRelocationKey.From(seal);
        if (!_timedOutCanonicalSeals.Add(key))
            return;
        _timedOutCanonicalSealOrder.Enqueue(key);
        while (_timedOutCanonicalSeals.Count > _maxTombstones)
            _timedOutCanonicalSeals.Remove(_timedOutCanonicalSealOrder.Dequeue());
    }
    ...
    )
    {
        ...
        catch (OperationCanceledException) when (timeout.Cancellation.IsCancellationRequested)
        {
        ...
        await _lane
            ...
            {
                ownsTimeout =
                    _canonicalSealTimeouts.TryGetValue(key, out var current)
                ...
                if (ownsTimeout)
                {
                    ...
                    )
                    {
                        ...
                        var sessionKeys = _entries
                            .Where(candidate =>
                                ReferenceEquals(candidate.Value.Context, entry.Context)
                            )
        ...
        timeout.Cancellation.Dispose();
        if (!ownsTimeout || timedOut.Count == 0)
            return;
        ...
        {
            timedOut[0]
                .Context.Runtime.ErrorSink.ReportRuntimeTaskException(
                    $"session-relocation-seal-timeout:" + timedOut[0].Context.SessionId,
                    failure
                );
        }
    }
    ...
    )
    {
        ...
        {
            var key = ZLinkSessionBindingKey.FromBoundary(tenure.ActorId, tenure.BindingToken);
            if (!_entries.TryGetValue(key, out var entry))
                return new ZLinkSessionOutboundAdmission(
                    _entries.Keys.Any(candidate => candidate.ActorId.Value == tenure.ActorId)
    ...
    )
    {
        ...
        return _lane.RunAsync(() =>
        {
            ...
            {
                var exact =
                    tombstone.SessionRid == actorRef.SessionRid
                    && tombstone.BindingGeneration == bindingGeneration
                    && tombstone.SessionOwnerNodeGeneration == sessionOwnerNodeGeneration
            ...
            _entries[key] = new ZLinkSessionBindingEntry(
                ...
                SessionOwnerId: string.IsNullOrWhiteSpace(sessionOwnerId)
                    ...
                    : sessionOwnerId,
                SessionOwnerLeaseGeneration: sessionOwnerLeaseGeneration == 0
                    ? sessionOwnerNodeGeneration
    ...
    public ValueTask TombstoneAsync(
        ...
        _lane.RunAsync(() =>
        {
            ...
            {
                if (
                    existingTombstone.SessionRid != sessionRid
                    || existingTombstone.BindingGeneration != bindingGeneration
                    || existingTombstone.SessionOwnerNodeGeneration != sessionOwnerNodeGeneration
            ...
            if (!_tombstones.ContainsKey(key) && _tombstones.Count >= _maxTombstones)
                throw new ZLinkFrameworkException(
                    ZLinkFrameworkErrorKind.Unavailable,
    ...
    )
    {
        var wait = await _lane
            .RunAsync(() =>
            {
                ...
                if (entry.RelocationHandoffId is null && entry.CanonicalRelocationSeal is null)
                    return ValueTask.FromResult(true);

                var signal =
                    entry.RouteAvailableSignal
    ...
    public ValueTask CompleteAcceptedAsync(string actorId, string bindingToken) =>
        _lane.RunAsync(() =>
        {
            var key = ZLinkSessionBindingKey.FromBoundary(actorId, bindingToken);
            if (!_entries.TryGetValue(key, out var entry) || entry.ActiveFrames == 0)
                return;
    ...
    internal async ValueTask<ZLinkServiceWireCodec.SessionRelocationSealedRecord> SealCanonicalRouteAsync(
        ZLinkServiceWireCodec.SessionRelocationSealRecord request,
        CancellationToken cancellationToken
    )
    {
        Task? drain = null;
        var installedResult = await _lane
            .RunAsync(() => PrepareCanonicalRouteSeal(request, out drain))
            .ConfigureAwait(false);
        ...
        return await _lane
            .RunAsync(() =>
            {
                ...
                _entries[key] = current with { CanonicalRelocationSealResult = result };
                return result;
            })
            .ConfigureAwait(false);
    }

    private ZLinkServiceWireCodec.SessionRelocationSealedRecord? PrepareCanonicalRouteSeal(
        ZLinkServiceWireCodec.SessionRelocationSealRecord request,
        out Task? drain
    )
    {
        ...
        if (entry.CanonicalRelocationSeal is { } installed && installed != request)
            throw new InvalidDataException(
    ...
    )
    {
        ...
        var routed = await _lane
            .RunAsync(() =>
            {
                if (_timedOutCanonicalSeals.Contains(CanonicalRelocationKey.From(request)))
                {
                ...
                {
                    if (
                        request.Route.PreviousAuthorityOwnerGeneration
                            != seal.Actor.AuthorityOwnerGeneration
                        || authenticatedRoute.NodeRid != request.Route.TargetNodeRid
                        || authenticatedRoute.NodeGeneration != request.Route.TargetNodeGeneration
                        || authenticatedRoute.AuthorityOwnerGeneration
                            != request.Route.TargetAuthorityOwnerGeneration
                        || string.IsNullOrWhiteSpace(authenticatedRoute.MeshName)
                    )
                        return false;
                ...
                else
                {
                    if (
                        ...
                        || authenticatedRoute.NodeRid != seal.Coordinator.NodeRid
    ...
    internal ValueTask<bool> IsCanonicalRouteAppliedAsync(
        ...
        {
            if (
                !TryFindCanonicalBinding(request.Actor, request.Session, out _, out var entry)
                || entry.AppliedCanonicalRelocationRoute != request
            )
                return false;

            var expectedNode =
                request.Route.Action == ZLinkServiceWireCodec.SessionRelocationRouteAction.Commit
                    ? request.Route.TargetNodeRid
            ...
            var expectedAuthority =
                request.Route.Action == ZLinkServiceWireCodec.SessionRelocationRouteAction.Commit
                    ? request.Route.TargetAuthorityOwnerGeneration
                    : request.Route.CurrentAuthorityOwnerGeneration;
            if (
                authenticatedCandidate.NodeRid != expectedNode
                || authenticatedCandidate.NodeGeneration != expectedGeneration
                || authenticatedCandidate.AuthorityOwnerGeneration != expectedAuthority
            )
                throw new InvalidDataException(
    ...
    )
    {
        foreach (var candidate in _entries)
        {
            ...
            if (
                candidate.Key.ActorId.Value == actor.ActorId
                ...
                && value.SessionOwnerNodeGeneration == session.SessionOwnerNodeGeneration
                && string.Equals(
                    value.SessionOwnerId,
                    session.SessionOwnerId,
                    StringComparison.Ordinal
                )
                && value.SessionOwnerLeaseGeneration == session.SessionOwnerLeaseGeneration
            )
            {
                key = candidate.Key;
                entry = value;
    ...
    private static bool MatchesCanonicalActorRoute(
        ...
        string.Equals(entry.ActorRef.ActorId, actor.Actor.ActorId, StringComparison.Ordinal)
    ...
    )
    {
        ...
        var immediate = await _lane
            .RunAsync(() => PrepareRouteSeal(request, out drain, out acceptedHighWater))
            .ConfigureAwait(false);
        ...
        return await _lane
            .RunAsync(() =>
            {
                ...
                return new ZLinkSessionRouteSealResult(true, current.AcceptedHighWater);
            })
            .ConfigureAwait(false);
    }
    ...
    )
    {
        drain = null;
        acceptedHighWater = 0;
        var key = ZLinkSessionBindingKey.FromBoundary(request.ActorId, request.BindingToken);
        if (!_entries.TryGetValue(key, out var entry))
        {
            Diagnostics.ZLinkFrameworkDebugLog.SpotDiscovery(
                $"route_seal_refused actor={request.ActorId} entry=false "
        ...
        var sessionOwnerMatches =
            entry.SessionOwnerNodeGeneration == request.SessionOwnerNodeGeneration;
        ...
        if (
            entry.RelocationHandoffId is { } current
    ...
    public async ValueTask<bool> UnsealCommittedRouteAsync(ZLinkSessionRouteCommit request)
    {
        ...
        var unsealed = await _lane
            .RunAsync(() =>
            {
                ...
                var sessionOwnerMatches =
                    entry.SessionOwnerNodeGeneration == request.SessionOwnerNodeGeneration;
                var highWaterMatches = entry.AcceptedHighWater >= request.AcceptedHighWater;
                var handoffMatches = string.Equals(
                    entry.RelocationHandoffId,
                    request.HandoffId,
                    StringComparison.Ordinal
    ...
    public ValueTask<ZLinkSessionRouteCommitResult> CommitRouteAsync(
        ...
        _lane.RunAsync(() =>
        {
            ...
            var targetObjectMatches =
                targetRouteCreated && targetRoute.Ref.ObjectGeneration == request.ObjectGeneration;
            var authorityOrderMatches =
                request.TargetAuthorityOwnerGeneration > request.PreviousAuthorityOwnerGeneration;
    ...
    public ValueTask<ZLinkSessionContext?> GetContextAsync(string actorId, string bindingToken) =>
    ...
    public ValueTask<ZLinkSessionContext?> GetContextByActorIdAsync(string actorId) =>
        _lane.RunAsync<ZLinkSessionContext?>(() =>
        {
    ...
    public ValueTask<ZLinkSessionBindingEntry?> GetEntryByActorIdAsync(string actorId) =>
        _lane.RunAsync<ZLinkSessionBindingEntry?>(() =>
        {
            var actorKey = ZLinkActorId.FromBoundary(actorId, nameof(actorId));
            foreach (var candidate in _entries)
    ...
    )
    {
        foreach (var candidate in _entries)
        {
            ...
            if (
                ...
                || value.SessionOwnerNodeGeneration != sessionOwnerNodeGeneration
                || !string.Equals(value.SessionOwnerId, sessionOwnerId, StringComparison.Ordinal)
                || value.SessionOwnerLeaseGeneration != sessionOwnerLeaseGeneration
            )
                continue;
            return value;
        }
    ...
    public ValueTask<IReadOnlyCollection<IZLinkSessionActor>> SnapshotActorsAsync(
        ...
        {
            return _entries
                .Values.Where(entry => ReferenceEquals(entry.Context, context))
        ...
        _lane.RunAsync(() =>
        {
            return _entries
                ...
                .FirstOrDefault(entry =>
                    string.Equals(entry.ActorRef.ActorId, actorId, StringComparison.Ordinal)
                )
                ?.ActorRef;
        });
