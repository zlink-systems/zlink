...
)
{
    ...
    private readonly Dictionary<long, Task?> _canonicalMaintenanceReplayReservations = [];
    ...
    public void BeginCapture()
    {
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                if (
                    _sourcePhase
                        is ZLinkActorSourceHandoffPhase.Capturing
                    ...
                    || _targetPhase
                        is ZLinkActorTargetHandoffPhase.Importing
        ...
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                if (
                    ...
                    || _targetPhase
                        is ZLinkActorTargetHandoffPhase.Failed
                            or ZLinkActorTargetHandoffPhase.Quarantined
                )
                {
                    diagnostic?.Invoke(
    ...
    public IReadOnlyList<ZLinkActorHandoffFrame> EndDeferredJoinCapture()
    {
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                if (!_deferredJoinCapture || _sourcePhase != ZLinkActorSourceHandoffPhase.Idle)
                    return [];

                var frames = _frames.OrderBy(static frame => frame.ArrivalIndex).ToArray();
                _deferredJoinCapture = false;
                _sourceIngressAdmission.ReleaseAll();
                _sourceHoldAdmission.ReleaseAll();
                _frames.Clear();
    ...
    internal bool CapturesSourceIngress
    {
        get
        {
            return AwaitStateLane(
                _lane.RunAsync(() =>
                    _deferredJoinCapture || _sourcePhase == ZLinkActorSourceHandoffPhase.Capturing
                )
            );
        }
    }
    ...
    {
        get { return AwaitStateLane(_lane.RunAsync(() => _staleSourceActor is not null)); }
    }

    public bool BlocksLocalDispatch
    {
        get { return AwaitStateLane(_lane.RunAsync(BlocksLocalDispatchOnStateLane)); }
    }
    ...
    public void CompleteSourceMigration()
    {
        ...
        completion = AwaitStateLane(
            _lane.RunAsync(() =>
            {
                if (
                    _sourcePhase
                    is not (
                        ZLinkActorSourceHandoffPhase.CutoverPending
                        or ZLinkActorSourceHandoffPhase.MessageFollowCommitted
                    )
                )
                    throw new InvalidOperationException(
        ...
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                if (
                    _sourcePhase
                    is not (
                        ZLinkActorSourceHandoffPhase.Idle
    ...
    internal TaskCompletionSource? TryBeginSourceMembershipLeave(string handoffId)
    {
        return AwaitStateLane(
            ...
            {
                if (
                    _sourceMembershipLeave is { } pending
    ...
    {
        return AwaitStateLane(
            _lane.RunAsync(() => TryCaptureOnStateLane(frame, prepareState, prepareCapture))
        );
    }
    ...
    )
    {
        var capturesSourceIngress =
            _deferredJoinCapture || _sourcePhase == ZLinkActorSourceHandoffPhase.Capturing;
        var capturesSourceHold = capturesSourceIngress && _sourceCaptureSealed;
        ...
        if (!frame.RouteContext.IsDirectRoute)
        {
            ...
            frame.RequestSource is not { } source
            || frame.SourceNodeGeneration == 0
        ...
        var encodedBytes =
            capturesSourceIngress || capturesTargetIngress
                ? ZLinkActorHandoffFrames.CanonicalEncodedLength(captured, frame.Actor)
                : 0;
        if (capturesSourceIngress || capturesTargetIngress)
            captured = captured with { CanonicalEncodedLength = encodedBytes };
        if (
            capturesSourceIngress
            && !capturesSourceHold
            && !_sourceIngressAdmission.TryAcquire(encodedBytes)
        )
            return ZLinkActorHandoffCaptureResult.Full;
        if (capturesSourceHold && !_sourceHoldAdmission.TryAcquire(encodedBytes))
            return ZLinkActorHandoffCaptureResult.Full;
        try
        {
    ...
    )
    {
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                var route = ResolveMessageFollowRouteOnStateLane(currentActor, frame.Actor);
                var capture =
                    route.Route == ZLinkActorFrameRoute.MessageFollow
        ...
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                var capture = allowCapture
                    ...
                    : ZLinkActorHandoffCaptureResult.NotSealed;
                var route = capture
                    is ZLinkActorHandoffCaptureResult.Captured
                        or ZLinkActorHandoffCaptureResult.Full
                    ? new ZLinkActorMessageFollowRouteResolution(ZLinkActorFrameRoute.Current, null)
                    : ResolveMessageFollowRouteOnStateLane(currentActor, frame.Actor);
                return new ZLinkActorHandoffIngressStateSnapshot(
                    capture,
                    BlocksLocalDispatchOnStateLane(),
                    route
    ...
    public ZLinkActorHandoffImportResult Import(ZLinkRemoteActorJoinRequest request)
    {
        ...
        var imported = AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                if (_targetPhase == ZLinkActorTargetHandoffPhase.Importing)
                {
                    ...
                    _targetPhase
                    is ZLinkActorTargetHandoffPhase.AuthorityCommitted
                        or ZLinkActorTargetHandoffPhase.NotifyingJoined
                        or ZLinkActorTargetHandoffPhase.Prepared
                        or ZLinkActorTargetHandoffPhase.Replaying
                        or ZLinkActorTargetHandoffPhase.AdmissionOpenDraining
                ...
                _arrivalIndex = 0;
                foreach (
                    var frame in request.HandoffFrames.OrderBy(static frame => frame.ArrivalIndex)
                )
                {
                    _frames.Add(
                        frame with
        ...
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                try
                {
                    ...
                    {
                        if (
                            frame.ArrivalIndex <= previousSequence
    ...
    )
    {
        ...
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                var ordered = frames.OrderBy(static frame => frame.ArrivalIndex).ToArray();
                var expectedSequence = _arrivalIndex;
                foreach (var frame in ordered)
                {
                    if (frame.ArrivalIndex != expectedSequence || frame.CanonicalEncodedLength <= 0)
                        throw new ZLinkRelocationDataLostException(
    ...
    public void CompleteJoinedNotification(string handoffId, ZLinkRemoteActorJoinReply reply)
    {
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
    ...
    {
        if (sourceObjectGeneration == 0 || targetObjectGeneration != sourceObjectGeneration)
    ...
    public bool IsJoinedNotificationCompleted(string handoffId)
    {
        return AwaitStateLane(
            _lane.RunAsync(() =>
                string.Equals(_handoffId, handoffId, StringComparison.Ordinal)
                && _targetPhase
                    is ZLinkActorTargetHandoffPhase.Prepared
                        or ZLinkActorTargetHandoffPhase.Replaying
                        or ZLinkActorTargetHandoffPhase.AdmissionOpenDraining
                        or ZLinkActorTargetHandoffPhase.Completed
            )
    ...
    public bool FailJoinedNotification(string handoffId, ZLinkRemoteActorJoinReply reply)
    {
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                if (_targetPhase == ZLinkActorTargetHandoffPhase.Failed)
                    return false;
                if (
                    _targetPhase
                    is not (
    ...
    {
        return AwaitStateLane(
            _lane.RunAsync(() => string.Equals(_handoffId, handoffId, StringComparison.Ordinal))
        );
    }
    ...
    private void AbortImportLocked(string handoffId, Exception? preparationFailure = null)
    {
    ...
    private bool TryCompleteTargetHandoffLocked(string handoffId)
    {
        ...
        if (_targetPhase != ZLinkActorTargetHandoffPhase.Replaying || _frames.Count != 0)
            return false;
        ...
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                if (sourceMembershipLeaveHandoffId is not null)
                    _sourceMembershipLeave = new SourceMembershipLeave(
                        sourceMembershipLeaveHandoffId
                    );
    ...
    private bool CanSealCaptureLocked() =>
        _sourcePhase == ZLinkActorSourceHandoffPhase.Capturing && !_sourceCaptureSealed;
    ...
    )
    {
        ...
        if (
            ...
            || targetAuthorityOwnerGeneration is 0 or > long.MaxValue
        ...
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                if (
                    ...
                    || (!_sourceCaptureSealed && committedFrameCount > _frames.Count)
                    || (
                        _sourceCaptureSealed
                        && (
                            _sourceCommittedFrameCount < 0
                            || committedFrameCount != _sourceCommittedFrameCount
                        )
                    )
                )
    ...
    /// </param>
    public void CommitMessageFollow(TimeSpan duration, TimeSpan retransmissionWindow = default)
    {
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                {
                    _ = Task.Run(() =>
                        RemoveMessageFollowRouteAfterDelayAsync(
                            duration,
            ...
            _lane.RunAsync(() =>
            {
                ...
                if (!_sourceTrailingImported)
                {
                    var trailing = sourceTrailingFrames
                        .OrderBy(static frame => frame.ArrivalIndex)
                        .Select(static frame => frame with { CanonicalEncodedLength = 0 })
                        .ToArray();
    ...
    internal IReadOnlyList<ZLinkActorHandoffFrame> PrepareCanonicalMaintenanceReplay(
        string handoffId
    )
    {
        return AwaitStateLane(
    ...
    internal Task WaitForTargetReplayCompletionAsync(CancellationToken cancellationToken)
    {
        Task completion;
        completion = AwaitStateLane(
            _lane.RunAsync(() =>
        ...
        return AwaitStateLane(
            _lane.RunAsync(() =>
            {
                if (
                    ...
                    return false;
                if (_frames.Any(frame => frame.ArrivalIndex > queuedThroughArrivalIndex))
                    return false;
                _targetPhase = ZLinkActorTargetHandoffPhase.AdmissionOpenDraining;
                return true;
            })
        );
    }
    ...
    internal IReadOnlyList<Task> ReserveCanonicalMaintenanceTrailingAndOpenAdmission(
        string handoffId,
        long queuedThroughArrivalIndex,
        ...
        var claimed = AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                var trailing = _frames
                    .Where(frame => frame.ArrivalIndex > queuedThroughArrivalIndex)
                ...
                // completion.
                var reservations = _canonicalMaintenanceReplayReservations
                ...
                {
                    if (
                        _canonicalMaintenanceReplayReservations.TryGetValue(
                            frame.ArrivalIndex,
                            out _
                        )
                    )
                    ...
                    _canonicalMaintenanceReplayReservations.Add(frame.ArrivalIndex, null);
    ...
    internal void RegisterCanonicalMaintenanceDrain(string handoffId, Task drain)
    {
    ...
    internal void AcknowledgeReplayedFrame(long arrivalIndex)
    {
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                var index = _frames.FindIndex(frame => frame.ArrivalIndex == arrivalIndex);
                if (index < 0)
                    throw new InvalidOperationException(
    ...
    public IReadOnlyList<ZLinkActorHandoffFrame> AbortCapture()
    {
        ...
        foreach (var frame in frames)
            AwaitStateLane(
                _lane.RunAsync(() => AcknowledgeAbortRestoreEnqueuedCore(frame.ArrivalIndex))
            );
    ...
    {
        AwaitStateLane(_lane.RunAsync(() => AcknowledgeAbortRestoreEnqueuedCore(arrivalIndex)));
    }
    ...
    private void AcknowledgeAbortRestoreEnqueuedCore(long arrivalIndex)
    {
        ...
        var holdIndex =
            _sourceHoldFrames.Count == 0
                ? -1
                : _sourceHoldFrames.FindIndex(frame => frame.ArrivalIndex == arrivalIndex);
        var nextArrival = _frames
            .Concat(_sourceHoldFrames)
            .Select(static frame => frame.ArrivalIndex)
            .DefaultIfEmpty(long.MinValue)
            .Min();
        if (nextArrival != arrivalIndex || sourceIndex < 0 && holdIndex < 0)
            throw new InvalidOperationException(
        ...
        if (sourceIndex >= 0)
        {
            ...
            if (!_abortRestoreAdmissionsReleased)
                _sourceIngressAdmission.Release(frame.CanonicalEncodedLength);
        }
        else
        {
            ...
            if (!_abortRestoreAdmissionsReleased)
                _sourceHoldAdmission.Release(frame.CanonicalEncodedLength);
        }
    }
    ...
    {
        return AwaitStateLane(
            _lane.RunAsync(() => ResolveMessageFollowRouteOnStateLane(currentActor, frameActor))
        );
    }

    private ZLinkActorMessageFollowRouteResolution ResolveMessageFollowRouteOnStateLane(
        ZLinkBackendActorRef? currentActor,
        ZLinkBackendActorRef frameActor
    )
    {
        ...
        var messageFollowRoute =
            route == ZLinkActorFrameRoute.MessageFollow ? _messageFollowRoute : null;
        return new ZLinkActorMessageFollowRouteResolution(route, messageFollowRoute);
    }
    ...
    public void Reset()
    {
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                ...
                _preparation = null;
                _targetCompletion?.TrySetException(
                    new InvalidOperationException($"Actor '{actorId}' target handoff was reset.")
                );
                _targetCompletion = null;
                _targetReplayCompletion?.TrySetException(
                    new InvalidOperationException($"Actor '{actorId}' target handoff was reset.")
    ...
    )
    {
        ...
        if (_messageFollowRoute is { } messageFollowRoute)
        {
            if (!messageFollowRoute.Lease.IsActive)
            {
                ...
                messageFollowRoute.SourceActor.NodeRid == frameActor.NodeRid
        ...
        if (
            targetActor.NodeRid == frameActor.NodeRid
    ...
    )
    {
        try
        {
            ...
            ZLinkRuntimeMetrics.RecordRelocationRouteConvergence(
                timeProvider.GetElapsedTime(committedAt),
                "actor"
            );
            ...
            if (remainder > TimeSpan.Zero)
                await Task.Delay(remainder, timeProvider, expiry.Token).ConfigureAwait(false);
            await _lane
                .RunAsync(() =>
                {
                    if (!ReferenceEquals(_messageFollowExpiry, expiry))
                        return;
...
internal sealed class ZLinkActorMessageFollowLease(TimeProvider timeProvider)
{
    ...
    public bool IsActive => AwaitStateLane(_lane.RunAsync(IsActiveCore));

    public bool IsCommitted => AwaitStateLane(_lane.RunAsync(IsCommittedCore));
        ...
        AwaitStateLane(
            _lane.RunAsync(() =>
            {
                if (_phase != ZLinkActorMessageFollowLeasePhase.Pending)
                    throw new InvalidOperationException(
                        "Actor Message Follow lease is not pending."
                    );
    ...
    {
        return AwaitStateLane(
            _lane.RunAsync(() => IsCommittedCore() && _suppression.TryBegin(fence))
        );
    }
    ...
    private bool IsActiveCore() =>
        _phase switch
        {
            ...
            ZLinkActorMessageFollowLeasePhase.Committed => timeProvider.GetElapsedTime(_committedAt)
                < _duration,
            _ => false,
        };
