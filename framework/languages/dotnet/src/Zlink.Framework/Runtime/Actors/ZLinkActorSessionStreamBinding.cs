...
internal sealed partial class ZLinkActorSessionManager
{
    ...
    {
        await state
            .ExecuteLockedAsync(() => state.AttachStream(stream), cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    {
        return await state
            .ExecuteLockedAsync(() => state.DetachStreamIfCurrent(stream), cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    {
        if (getActorSpotNode() is null || state.NativeActorRef is not { } actorRef)
            return;
    ...
    {
        if (getActorSpotNode() is null || state.NativeActorRef is null)
            return;
