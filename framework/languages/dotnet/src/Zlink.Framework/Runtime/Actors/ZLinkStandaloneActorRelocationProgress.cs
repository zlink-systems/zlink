...
)
{
    ...
    internal async ValueTask<ZLinkStandaloneActorRelocationProgress> AdvanceCanonicalPhaseAsync(
        ZLinkRelocationEnvelope identity,
    ...
    {
        if (
            (byte)expected < (byte)ZLinkStandaloneActorCanonicalPhase.Committed
        ...
        {
            var current = await ReadCurrentAsync(identity, targetOwner, cancellationToken)
            ...
            var exchanged = await authorityStore
        ...
        throw Moving("canonical phase advance conflicted after the bounded retry limit");
    }
    ...
    )
    {
        for (var attempt = 0; attempt < MaxConflictRetries; attempt++)
        {
            ...
            if (
                current.Canonical is { } directCanonical
            ...
            {
                var direct = await authorityStore
                    .CompareExchangeAuthorityAsync(
                        identity.Participants.Single().AuthorityKey,
                        current.Authority.StoreVersion,
                        new ZLinkAuthorityMutation.Put(
            ...
            var relocationReference =
                current.Canonical?.RelocationReference ?? publication.Reference;
            var exchanged = await authorityStore
                .CompareExchangeAuthorityAsync(
                    identity.Participants.Single().AuthorityKey,
                    current.Authority.StoreVersion,
    ...
    )
    {
        ...
        {
            if (
                directCanonical.RelocationHigh != identity.CanonicalRelocationHigh
        ...
        var root = await ZLinkRelocationTreeStore
            ...
            .ConfigureAwait(false);
        if (
            root.AggregateId != identity.AggregateId
        ...
        root = root with { AggregateGeneration = publication.AggregateGeneration };
        ZLinkActorRelocationAuthorityPayload phase;
        ...
        return new ZLinkStandaloneActorRelocationProgress(found.Snapshot, phase, root)
        {
            Canonical = canonical,
        };
    }
    ...
    )
    {
        ...
        if (
            ...
            || canonical.TargetAttemptGeneration != expected.TargetAttemptGeneration
            || !StringComparer.Ordinal.Equals(
                canonical.State.TargetNodeRid,
                expected.NodeRid.ToHex()
            )
            || canonical.State.TargetNodeGeneration != expected.NodeGeneration
            || !StringComparer.Ordinal.Equals(canonical.TargetOwnerId, expected.Owner.OwnerId)
            || canonical.TargetOwnerLeaseGeneration
                != checked((ulong)expected.Owner.LeaseGeneration)
            || !StringComparer.Ordinal.Equals(snapshot.OwnerId, expected.Owner.OwnerId)
    ...
    {
        if (!ZLinkRelocationAuthorityPayloadCodec.TryDecode(current.Span, out var publication))
            throw new ZLinkRelocationDataLostException(
    ...
    private static ZLinkFrameworkException Moving(string reason) =>
        new(
            ZLinkFrameworkErrorKind.Unavailable,
