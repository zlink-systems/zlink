...
)
{
    ...
    public void TryRemove(ZLinkActorId actorId, ZLinkActorRuntimeState state)
    {
        AwaitStateLane(
            ...
            {
                if (
                    _states.TryGetValue(actorId, out var existing)
    ...
    )
    {
        ...
        var cleanup = Task.WhenAll(
            states.Select(static state =>
                state.InvalidateRuntimeGenerationAfterDispatchesAsync().AsTask()
            )
        );
        ...
        {
            _ = ObserveDetachedCleanupAsync(cleanup, detachedCleanupFailure);
            throw;
        }
    }
