...
internal static class ZLinkCanonicalActorRelocationWriter
{
    ...
    )
    {
        ...
        if (
            !ZLinkActorAuthorityPayloadCodec.TryGetActorId(
        ...
        var accepted = participant
            .AcceptedJobs.OrderBy(static job => job.AcceptedSequence)
            .ToList();
        ...
        )
        {
            accepted.Add(
                ZLinkActorRemoteJoinRecoverySavedWork.Create(
                    accepted.Count == 0 ? 1UL : checked(accepted[^1].AcceptedSequence + 1),
                    source,
                    remote
                )
            );
        }
        var savedWork = new List<ServiceWirePilotCodec.RelocationSavedWork>(accepted.Count);
        foreach (var job in accepted)
        {
            if (!ZLinkRelocationEnvelopeCodec.TryValidateCanonicalFrozenRecord(job.Payload.Span))
                throw new ZLinkRelocationDataLostException(
    ...
    )
    {
        ...
        try
        {
            ...
            if (recovery.OperationRecovery.IsEmpty && source.LegacyRemoteJoinRecovery.IsEmpty)
                return false;
