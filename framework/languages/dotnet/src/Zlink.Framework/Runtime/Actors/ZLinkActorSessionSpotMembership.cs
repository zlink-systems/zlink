...
internal sealed partial class ZLinkActorSessionManager
{
    ...
    )
    {
        ...
        var previousActivation = await state
            .ExecuteLockedAsync(() => state.Activation, cancellationToken)
        ...
        await state
            .ExecuteLockedAsync(() => state.JoinSpot(activation), cancellationToken)
            .ConfigureAwait(false);
        return previousActivation;
    }
    ...
    )
    {
        ...
        await state
            .ExecuteLockedAsync(
                () =>
                {
                    if (!ReferenceEquals(state.Activation, failedTarget))
                        return;
