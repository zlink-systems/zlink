...
internal static class ZLinkActorRelocationRoot
{
    ...
    )
    {
        ...
        if (
            envelope.AggregateId != wire.RelocationAggregateId
            || envelope.AggregateGeneration != wire.RelocationAggregateGeneration
        ...
        if (
            ...
            throw DataLost($"Actor '{wire.ActorId}' relocation participant fences are invalid.");
                ...
                .Select(
                    (job, index) =>
                        ZLinkCanonicalActorAcceptedJournal
                            .Decode(job.Payload.Span, checked((long)index))
                            .Frame
                )
                .ToArray();
        }
        ...
        if (
            ...
            && (
                ...
                || !StringComparer.Ordinal.Equals(canonical.StableType, wire.ActorType)
            )
        )
            throw DataLost(
                $"Actor '{wire.ActorId}' canonical participant recovery fences are invalid."
            );
        var expectedInventoryDigest = canonical is null
        ...
        if (
            ...
            && !wire.RelocationInventoryDigest.AsSpan().SequenceEqual(expectedInventoryDigest.Span)
        )
            throw DataLost($"Actor '{wire.ActorId}' relocation inventory mutation is invalid.");
        envelope = envelope with { InventoryDigest = expectedInventoryDigest };
        if (
            !string.Equals(recovery.Request.ActorId, wire.ActorId, StringComparison.Ordinal)
            || !string.Equals(recovery.Request.ActorType, wire.ActorType, StringComparison.Ordinal)
            || !string.Equals(recovery.Request.HandoffId, wire.HandoffId, StringComparison.Ordinal)
            || !string.Equals(
                recovery.TargetSpotId,
                ZLinkSpotId.Require(recovery.TargetSpotId, nameof(recovery.TargetSpotId)),
            ...
            || !recovery.TargetNodeRid.AsSpan().SequenceEqual(recovery.Request.TargetNodeRid)
            || recovery.TargetNodeGeneration != recovery.Request.TargetNodeGeneration
            || recovery.TargetSpotGeneration != recovery.Request.TargetSpotGeneration
            || recovery.TargetAuthorityOwnerGeneration
                != recovery.Request.TargetAuthorityOwnerGeneration
        )
            throw DataLost($"Actor '{wire.ActorId}' relocation recovery identity is invalid.");
        var normalizedWire = wire with
        {
            RelocationReference = recovery.Request.RelocationReference,
            RelocationChecksumCrc32c = recovery.Request.RelocationChecksumCrc32c,
            RelocationAggregateId = recovery.Request.RelocationAggregateId,
            RelocationAggregateGeneration = recovery.Request.RelocationAggregateGeneration,
            RelocationInventoryDigest = recovery.Request.RelocationInventoryDigest,
            HandoffFrames = recovery.Request.HandoffFrames,
        };
        if (!ZLinkActorHandoffRequestIdentity.Matches(recovery.Request, normalizedWire))
            throw DataLost(
                $"Actor '{wire.ActorId}' relocation wire changed its durable route or session fences."
            );
        return new ZLinkLoadedActorRelocation(envelope, participant, recovery, frames);
    }
    ...
    ) => wire with { HandoffFrames = loaded.AcceptedFrames };

    private static ZLinkFrameworkException DataLost(string message) =>
        new(ZLinkFrameworkErrorKind.DataLost, message, retryAdvice: ZLinkRetryAdvice.DoNotRetry);
}
