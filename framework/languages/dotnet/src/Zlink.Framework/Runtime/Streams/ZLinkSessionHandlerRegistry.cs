...
{
    private readonly Dictionary<string, ZLinkSessionPacketHandlerDescriptor> _handlers = new(
    ...
    )
    {
        ...
        var context =
            ...
            ?? throw new InvalidOperationException(
                "Session handler registry is not bound to a session."
            );

        await handler
            .HandleAsync(handlerInstances, _session, context, dispatch, payload, cancellationToken)
    ...
    internal void BindContext(IZLinkSessionContext context)
    {
        if (_context is not null)
            throw new InvalidOperationException("Session handler registry is already bound.");
    ...
    internal void BindSession(IZLinkSession session)
    {
        if (_session is not null)
            throw new InvalidOperationException("Session handler owner is already bound.");
        _session = session;
    }
    ...
    {
        if (
            _handlers.TryGetValue(descriptor.PacketName, out var existing)
...
) : ZLinkSessionPacketHandlerDescriptor(sessionType, typeof(TMessage), packetName)
{
    ...
    )
    {
        ...
        object? arg1 =
            ...
            : passCancellationToken ? cancellationToken
            : null;
        object? arg2 = passDispatch && passCancellationToken ? cancellationToken : null;
        await ZLinkHandlerInvocationEngine
            .InvokeAsync(session, invoker, message, arg1, arg2)
            .ConfigureAwait(false);
    }
}
...
internal abstract record ZLinkScannedSessionHandler(ZLinkSessionPacketHandlerDescriptor Descriptor);
    ...
    )
    {
        ...
        foreach (var handlerType in assembly.GetTypes())
        {
            ...
            if (sessionTypes.Contains(handlerType))
                foreach (
                    var method in handlerType.GetMethods(
                        BindingFlags.Instance | BindingFlags.Public
                    )
                )
                    if (method.GetCustomAttribute<ZLinkStreamPacketAttribute>() is not null)
                        candidates.Add(
                            new ZLinkScannedAttributedSessionHandler(
                                handlerType,
                                ZLinkSessionPacketHandlerDescriptorFactory.CreateAttributed(
                                    handlerType,
                                    method
                                )
                            )
            ...
            foreach (
                var (
                    definition,
                    arguments
                ) in ZLinkHandlerContractInspector.EnumerateGenericInterfaces(handlerType)
            )
            {
                if (definition != typeof(IZLinkSessionPacketHandler<,>))
                    continue;

                candidates.Add(
                    new ZLinkScannedInterfaceSessionHandler(
                        arguments[0],
...
{
    public static ZLinkSessionPacketHandlerDescriptor CreateAttributed(
        Type sessionType,
        MethodInfo method
    )
    {
        if (
            !typeof(IZLinkSession).IsAssignableFrom(sessionType)
        ...
        var passCancellationToken =
            parameters.Length > cancellationIndex
            && parameters[cancellationIndex].ParameterType == typeof(CancellationToken);
        if (
            parameters.Length != 1 + (passDispatch ? 1 : 0) + (passCancellationToken ? 1 : 0)
            || messageType.IsByRef
            || method.ReturnType != typeof(Task) && method.ReturnType != typeof(ValueTask)
        )
            throw InvalidShape(sessionType, method);

        return (ZLinkSessionPacketHandlerDescriptor)
            Activator.CreateInstance(
                typeof(ZLinkAttributedSessionPacketHandlerDescriptor<>).MakeGenericType(
                    messageType
                ),
                sessionType,
