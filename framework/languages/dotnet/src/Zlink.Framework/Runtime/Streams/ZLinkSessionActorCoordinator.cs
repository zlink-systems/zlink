...
)
{
    ...
    )
    {
        ...
        var operation = await _actorOperationGatesLane
            .RunAsync(() => AcquireActorOperationGate(actorId))
        ...
        {
            lease?.Dispose();
            await _actorOperationGatesLane
                .RunAsync(() => ReleaseActorOperationGate(actorId, operation))
                .ConfigureAwait(false);
        }
    }
    ...
    private void ReleaseActorOperationGate(ZLinkActorId actorId, ActorOperationGate operation)
    {
        operation.Users--;
        if (
            operation.Users == 0
    ...
    private sealed class ActorOperationGate
    {
        ...
        internal void Dispose() => AwaitStateLane(_lane.DisposeAsync());
    ...
    )
    {
        if (_bindings.FindActor(actor.ActorId) is { } existing)
        {
            ...
            catch (Exception replacementFailure)
            {
                ...
                if (runtime.IsStarted || !IsLocalActorRef(existing.Ref))
                    throw;
                try
                {
                    await BindNativeActorAsync(existing.Ref.ToBackend(), CancellationToken.None)
                        .ConfigureAwait(false);
                }
                catch (Exception rollbackFailure)
                {
                    throw new AggregateException(replacementFailure, rollbackFailure);
                }
    ...
    )
    {
        ...
        catch (Exception bindingFailure)
        {
            try
            {
                if (!runtime.IsStarted && confirmedIdentity is { } localIdentity)
                    runtime.UnbindActorSession(actor.ActorId, localIdentity.BindingToken);
                if (nativeBound)
    ...
    )
    {
        ...
        if (
            response.ObjectGeneration != actor.ObjectGeneration
    ...
    )
    {
        ...
        try
        {
            if (stream is ZLinkManagedStream)
            {
                ...
                {
                    var requestId =
                        header.Kind == ZlinkStreamMessageKind.Request ? header.RequestSeq : null;
    ...
    private bool IsLocalActorRef(ZLinkSessionBindingRoute route)
    {
        ...
        return route.Ref.NodeRid == local.RoutingId
            && route.TargetNodeGeneration == local.MeshStatus().LifecycleGeneration;
    }
    ...
    )
    {
        ...
        if (!found || binding.Context.RoutingId is null)
        {
            //  Returning quietly here is indistinguishable from a disconnect
            //  that was delivered, so name the condition that skipped it.
            return ValueTask.CompletedTask;
        }
        return runtime.NotifyActorDisconnectedAsync(binding, cancellationToken);
    }

    public ValueTask CleanupAsync(ZLinkSessionContext context, CancellationToken cancellationToken)
    {
    ...
    )
    {
        ...
        {
            await using var boundSessionScope = ZLinkBoundSessionDispatchScope.Enter(
                actorRef.ActorId
            );
            try
            {
                var reply = await runtime
                    ...
                    .ConfigureAwait(false);
                await replyRawAsync(header, reply, cancellationToken).ConfigureAwait(false);
            }
            finally
            {
                await boundSessionScope.DrainAsync(cancellationToken).ConfigureAwait(false);
            }
            return;
        }

        await runtime
            .SubmitActorByIdAsync(actorRef.ActorId, header, actorPayload, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    {
        await ZLinkNativeActorStreamBinding
            .BindAsync(
                stream,
                actorRef,
