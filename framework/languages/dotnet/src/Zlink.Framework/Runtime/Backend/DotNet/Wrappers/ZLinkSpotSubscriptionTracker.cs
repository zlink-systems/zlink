...
internal sealed class ZLinkSpotSubscriptionTracker
{
    private readonly ConcurrentDictionary<
        ZLinkSpotId,
        ConcurrentDictionary<SubscriptionKey, byte>
    > _targets = new();
    ...
    public void Add(string spotId, string channelName, string topic)
    {
        ...
        _targets
            .GetOrAdd(key, static _ => new ConcurrentDictionary<SubscriptionKey, byte>())
            .TryAdd(new SubscriptionKey(channelName, topic), 0);
    }

    public void RemoveSpot(string spotId)
    {
        _targets.TryRemove(ZLinkSpotId.FromBoundary(spotId, nameof(spotId)), out _);
    }
