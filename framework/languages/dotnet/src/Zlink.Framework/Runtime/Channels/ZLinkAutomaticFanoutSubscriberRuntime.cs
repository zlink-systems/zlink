...
internal sealed class ZLinkAutomaticFanoutSubscriberRuntime : IAsyncDisposable
{
        ...
        var removed = await _lane
            .RunAsync(() =>
            {
                ...
                _excluded = plans
                    .Where(static plan => !plan.Connect)
                    .Select(static plan =>
                        Snapshot(plan.Descriptor, false, false, plan.State, plan.LastFailure)
                    )
                    .ToArray();
                ...
                {
                    if (
                        desired.Remove(key, out var plan)
                        && string.Equals(
                            connection.Endpoint,
    ...
    ) : IAsyncDisposable
    {
        ...
        private readonly CancellationTokenSource _stop =
            CancellationTokenSource.CreateLinkedTokenSource(owner._runtimeStopToken);
        ...
        internal void UpdateDescriptor(ZLinkFanoutPublisherDescriptor value)
        {
        ...
        internal ZLinkFanoutPublisherConnectionSnapshot Read()
        {
            return AwaitStateLane(
                _lane.RunAsync(() => Snapshot(_descriptor, true, _ready, _state, _lastFailure))
            );
        }
        ...
        private async Task RunAsync()
        {
            ...
            while (!_stop.IsCancellationRequested)
            {
                ...
                try
                {
                    await Task.Delay(ReconnectDelay, owner._time, _stop.Token)
                        .ConfigureAwait(false);
                }
        ...
        private async Task RunAttemptAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                socket.Connect(
                    await _lane.RunAsync(() => _descriptor.Endpoint).ConfigureAwait(false)
                );
                await _lane
                    .RunAsync(() =>
                    {
                ...
                _ = await Task.WhenAny(receive, watchdog).ConfigureAwait(false);
            ...
            {
                await _lane.RunAsync(() => _lastFailure = exception.Message).ConfigureAwait(false);
            }
            finally
            {
        ...
        private async Task WatchInboundAsync(CancellationTokenSource attempt)
        {
            while (!attempt.IsCancellationRequested)
            {
                ...
                if (
                    !ZLinkFanoutLivenessProtocol.IsInboundTimedOut(
                        owner._time.GetElapsedTime(lastActivity)
                    )
                )
                    continue;
                await _lane
                    .RunAsync(() => _lastFailure = "fanout publisher inbound timeout")
                    .ConfigureAwait(false);
                await attempt.CancelAsync().ConfigureAwait(false);
                return;
            }
        ...
        private ValueTask<string?> ReadLastFailureAsync() => _lane.RunAsync(() => _lastFailure);
