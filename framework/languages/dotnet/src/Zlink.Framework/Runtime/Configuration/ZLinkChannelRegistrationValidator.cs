...
internal static partial class ZLinkFrameworkRegistrationValidator
{
    ...
    )
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(publisher.BindEndpoint) && publisher.ListenPort is null)
                throw new ZLinkConfigurationException(
                    $"fanout channel '{channel.ChannelName}' publisher must define a listener."
                );
            if (publisher.FixedRoutingId.Size > 0 && publisher.RoutingIdPrefix is not null)
                throw new ZLinkConfigurationException(
    ...
    {
        var hasHandlers =
            exposedKinds.Contains(ZLinkMessageKind.Command)
            || exposedKinds.Contains(ZLinkMessageKind.Request);
        if (channel.HasClientServerClient && !channel.HasClientServerServer && hasHandlers)
            throw new ZLinkConfigurationException(
    ...
    private static void ValidateChannelShape(ZLinkChannelRegistration channel)
    {
        if (channel.AutoConnectType == ZLinkLocationAutoConnectType.ClientServer)
        {
            ...
            if (
                (!channel.HasClientServerClient && !channel.HasClientServerServer)
                || !validClient
                || !validServer
            )
                throw new ZLinkConfigurationException(
