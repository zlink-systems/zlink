...
internal static partial class ZLinkFrameworkRegistrationValidator
{
    public static void Validate(ZLinkFrameworkRegistration registration)
    {
        ...
        foreach (var channel in registration.Channels.Values)
            ValidateChannel(channel, registration.Locations.Enabled, handlerExposure);
        ...
        var clientServerChannels = registration
    ...
    private static void ValidateUniqueStreamSessionTypes(ZLinkFrameworkRegistration registration)
    {
    ...
    private static void ValidateRouteMeshChannelNames(ZLinkFrameworkRegistration registration)
    {
        ...
        {
            if (
                !owners.TryAdd(membership.ChannelName, node.SpotNodeName)
                && !string.Equals(
    ...
    )
    {
        ...
        if (
            options.CoreHwmMemoryLimitBytes is { } memoryLimit
    ...
    private static void ValidateRelocationStoreRequirement(ZLinkFrameworkRegistration registration)
    {
    ...
    private static void ValidateDispatchOptions(ZLinkDispatchOptionsModel options)
    {
        ...
        if (
            double.IsNaN(options.Diagnostics.SampleRate)
    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(streamNode.BindEndpoint) && streamNode.ListenPort is null)
            throw new ZLinkConfigurationException(
        ...
        var objectMeshes = registration
            .SpotNodes.Values.Where(static node => node.ObjectRoleSelected)
            .ToArray();
