...
internal static partial class ZLinkFrameworkRegistrationValidator
{
    ...
    {
        if (options.PollingInterval <= TimeSpan.Zero || options.StoreFailureGrace <= TimeSpan.Zero)
            throw new ZLinkConfigurationException(
    ...
    {
        if (
            options.OwnerLeaseRenewInterval <= TimeSpan.Zero
    ...
    {
        if (
            options.RouteCacheMaxAge < TimeSpan.Zero
        ...
        if (
            options.RouteCacheMaxAge > TimeSpan.Zero
            && options.MessageFollowDuration > TimeSpan.Zero
            && options.RouteCacheMaxAge > options.MessageFollowDuration - TimeSpan.FromSeconds(5)
        )
            throw new ZLinkConfigurationException(
    ...
    )
    {
        var frameLimit =
            registration
                .SpotNodes.Values.Select(static node => node.Router?.SocketConfig.MaxMessageSize)
                .Where(static size => size is not null)
                .DefaultIfEmpty(ZLinkSocketConfig.DefaultMaxMessageSize)
                .Min()
            ?? ZLinkSocketConfig.DefaultMaxMessageSize;
        if (
            options.RelocationPayloadChunkLimit <= 0
        ...
        if (
            options.RelocationInFlightPayloadBudget < 0
        ...
        if (cutoverWait < TimeSpan.Zero || cutoverWait == Timeout.InfiniteTimeSpan)
            throw new ZLinkConfigurationException(
                "RelocationCutoverWaitTimeout must be a non-negative finite " + "duration."
            );
    }
}
