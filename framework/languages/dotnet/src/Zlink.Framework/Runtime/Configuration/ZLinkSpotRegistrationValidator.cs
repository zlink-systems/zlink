...
internal static partial class ZLinkFrameworkRegistrationValidator
{
    ...
    )
    {
        ...
        if (spotNode.Router is { } router)
        {
            ...
            var automatic = router.AcquisitionMode == ZLinkPeerAcquisitionMode.AutoConnect;
    ...
    private static void ValidateObjectClientBoundary(ZLinkSpotNodeRegistration spotNode)
    {
        if (spotNode.ObjectRole != ZLinkMeshNodeObjectRole.Client)
            return;
        if (spotNode.RouteSendHandlers.Count != 0 || spotNode.RouteRequestHandlers.Count != 0)
            throw new ZLinkConfigurationException(
    ...
    {
        if (spotNode.EntrySpotType is { } entrySpotType && !globalEntrySpots.Add(entrySpotType))
            throw new ZLinkConfigurationException(
