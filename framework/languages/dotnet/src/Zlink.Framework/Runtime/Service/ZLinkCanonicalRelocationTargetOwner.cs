...
) : ICanonicalRelocationTarget
{
    ...
    )
    {
        ...
        await WaitForTargetReadinessAsync(cancellationToken).ConfigureAwait(false);

        switch (ObjectKind(prepare.Object.Kind))
        {
            case ZLinkPlacementObjectKind.Actor:
                if (standaloneActorRuntime is null)
                    throw Unavailable("Standalone Actor target relocation is not configured.");
                await standaloneActorRuntime
                    .StageTargetAsync(
                        prepare,
                        envelope,
                        authenticatedSourceNodeRid,
                        checked(prepare.Object.ExpectedAuthorityOwnerGeneration + 1),
                        cancellationToken
                    )
            ...
            case ZLinkPlacementObjectKind.InstanceSpot:
                if (targetRuntime is null)
                    throw Unavailable("SPOT target relocation is not configured.");
                await targetRuntime
                    .StageCanonicalInboundAsync(
                        prepare,
                        envelope,
                        authenticatedSourceNodeRid,
                        cancellationToken
    ...
    )
    {
        ...
        switch (ObjectKind(prepare.Object.Kind))
        {
            case ZLinkPlacementObjectKind.Actor when standaloneActorRuntime is not null:
                if (
                    standaloneActorRuntime.MarkTargetReadySubmitted(
                        prepare,
                        authenticatedSourceNodeRid
                    )
                )
    ...
    )
    {
        ValidateAttempt(prepare, authenticatedSourceNodeRid);
        if (
            ObjectKind(prepare.Object.Kind) == ZLinkPlacementObjectKind.Actor
    ...
    )
    {
        ...
        return ObjectKind(prepare.Object.Kind) switch
        {
            ZLinkPlacementObjectKind.Actor when standaloneActorRuntime is not null =>
                standaloneActorRuntime.AbortTargetAsync(prepare, authenticatedSourceNodeRid),
    ...
    )
    {
        ...
        return ObjectKind(data.Object.Kind) switch
        {
            ...
            _ => throw Unavailable("The relocation data target is not configured."),
        };
    }
    ...
    )
    {
        ...
        return ObjectKind(cutover.Object.Kind) switch
        {
            ...
            _ => throw Unavailable("The relocation cutover target is not configured."),
        };
    }

    private async ValueTask WaitForTargetReadinessAsync(CancellationToken cancellationToken)
    {
        ...
        {
            throw Unavailable("The relocation target RouteMesh is not ready.");
        }
    }
    ...
    private static ZLinkFrameworkException Unavailable(string message) =>
        new(ZLinkFrameworkErrorKind.Unavailable, message, ZLinkRetryAdvice.RetryAfterBackoff);
}
