...
internal static partial class ZLinkServiceWireCodec
{
    ...
    )
    {
        ...
        if (
            ...
            return false;
        if (
            generated.Context is not ServiceWirePilotCodec.MaintenanceRelocationReplyContext context
    ...
    private static ServiceWirePilotCodec.OperationId ToGenerated(MeshOperationId value) =>
        new(value.High, value.Low);

    private static MeshOperationId FromGenerated(ServiceWirePilotCodec.OperationId value) =>
        new(value.High, value.Low);

    private static ServiceWirePilotCodec.RelocationId ToGenerated(RelocationWireId value) =>
        new(value.High, value.Low);

    private static RelocationWireId FromGenerated(ServiceWirePilotCodec.RelocationId value) =>
        new(value.High, value.Low);
    ...
    private static ServiceWirePilotCodec.RequestSourceFence ToGenerated(RequestSourceFence value) =>
        new(
            value.OwnerId,
            value.LeaseGeneration,
            value.NodeRid.ToBytes().ToArray(),
    ...
    private static ServiceWirePilotCodec.TargetFence ToGenerated(RelocationTargetRecord value) =>
        new(
            value.NodeRid.ToBytes().ToArray(),
            value.NodeGeneration,
            value.OwnerId,
    ...
    private static RelocationTargetRecord FromGenerated(ServiceWirePilotCodec.TargetFence value) =>
        new(
    ...
    private static ServiceWirePilotCodec.RelocationRole ToGeneratedRole(byte value) =>
        value switch
        {
    ...
    private static byte FromGeneratedRole(ServiceWirePilotCodec.RelocationRole value) =>
        value switch
        {
    ...
    private static bool IsTerminalFailureValid(
        uint terminalResult,
        ServiceWireConstants.FrameworkErrorCode failureCode
    ) => ServiceWireConstants.ValidTerminalFailure(terminalResult, (uint)failureCode);
    ...
    private static void ValidateCoordinator(RelocationCoordinatorFence fence)
    {
        ...
        if (fence.LeaseGeneration == 0 || fence.NodeRid.IsEmpty || fence.NodeGeneration == 0)
    ...
    private static bool TryRelocationId(ref WireReader reader, out RelocationWireId relocation)
    {
        relocation = default;
