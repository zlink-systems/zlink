...
)
{
    ...
    )
    {
        ...
        var high = System.Buffers.Binary.BinaryPrimitives.ReadUInt64BigEndian(id);
        var low = System.Buffers.Binary.BinaryPrimitives.ReadUInt64BigEndian(id[8..]);
        return current.RelocationHigh == high
            ...
            && current.State.TargetNodeGeneration == targetAuthority.NodeGeneration
            && StringComparer.Ordinal.Equals(current.TargetOwnerId, targetAuthority.OwnerId)
            && current.TargetOwnerLeaseGeneration == targetAuthority.OwnerLeaseGeneration
    ...
    )
    {
        if (
            ...
            throw DataLost("Standalone Actor recovery owner fence is malformed.");
        ...
        {
            throw DataLost("Standalone Actor recovery owner RID is malformed.");
        }

        var local = runtime.TryGetSpotNodeRuntime(rid);
        if (local is not null && local.Node.MeshStatus().LifecycleGeneration == nodeGeneration)
            return false;
        ...
        if (
            ...
            || found.Token.LeaseGeneration != checked((long)ownerLeaseGeneration)
        ...
        return descriptors.Any(descriptor =>
            ...
            && descriptor.LeaseGeneration == checked((long)ownerLeaseGeneration)
        );
    }
