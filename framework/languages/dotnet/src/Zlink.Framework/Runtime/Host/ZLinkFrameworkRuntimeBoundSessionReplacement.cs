...
internal sealed partial class ZLinkFrameworkRuntime
{
    ...
    )
    {
        ...
        if (
            retired.SessionOwnerNodeRid != receivingNodeRid
            ...
            : AwaitStateLane(
                state.RunStateAsync(() =>
                    state.SpotNodes.Values.SingleOrDefault(node =>
                        node.Node.RoutingId == receivingNodeRid
                    )
                )
            );
        if (
        ...
        if (
            _actorBoundSessionCoordinator.GetExactRetiredSessionBinding(
    ...
    )
    {
        try
        {
            ...
            return string.Equals(local.OwnerId, retired.SessionOwnerId, StringComparison.Ordinal)
                && local.LeaseGeneration == retired.SessionOwnerLeaseGeneration;
        }
        catch (InvalidOperationException)
        {
            return string.Equals(
                ...
                && retired.SessionOwnerLeaseGeneration == retired.SessionOwnerNodeGeneration;
        }
    }
    ...
    )
    {
        ...
        if (
            !TryRunDetached(
                ...
                {
                    _ = await ZLinkBoundSessionReplacementAdmissionRetry
                        .RunAsync(
                            () =>
                            {
                                try
                                {
                                    if (
                                        sender.TrySendBoundSessionReplacedNotification(
                                            targetNodeRid,
                                            record
                                        )
                                    )
