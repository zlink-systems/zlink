...
internal static partial class ZLinkRelocationTreeStore
{
    ...
    private static readonly byte[] ParticipantManifestMagic = "ZLPM"u8.ToArray();

    private static bool CanUseParticipantComponents(ZLinkRelocationEnvelope envelope) =>
    ...
    private static bool IsParticipantManifest(ReadOnlySpan<byte> encoded) =>
        ...
        && encoded[..ParticipantManifestMagic.Length].SequenceEqual(ParticipantManifestMagic);

    private static async ValueTask<ZLinkRelocationTreeStored> PutParticipantComponentsAsync(
    ...
    )
    {
        ...
        try
        {
            ...
            var components = new ParticipantComponentBuilder[componentCount];
            var measuredLengths = new long[componentCount];
            for (var index = 0; index < componentCount; index++)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var length = MeasureCanonicalSlice(sourceLength, componentCount, index);
                measuredLengths[index] = length;
                components[index] = new ParticipantComponentBuilder(index, length);
            }
            var preflight = ValidateParticipantComponentBounds(measuredLengths);
            var preflightLogicalLength = preflight.LogicalLength;
            var preflightChunkCount = preflight.ChunkCount;

            var pending = new List<ParticipantEncodedChunk>(MaxConcurrentComponentIo);
            long pendingBytes = 0;
            var globalOrder = 0;
            ...
            async ValueTask FlushAsync()
            {
                ...
                var stored = await Task.WhenAll(
                        pending.Select(chunk =>
                            PutVerifiedAsync(store, chunk.Encoded, retention, cancellationToken)
                                .AsTask()
                        )
                    )
                ...
                for (var index = 0; index < pending.Count; index++)
                {
                    ...
                    components[chunk.ParticipantOrder]
                        .Chunks.Add(
                            new ChunkEntry(
            ...
            for (var componentIndex = 0; componentIndex < componentCount; componentIndex++)
            {
                ...
                while (input.Position < input.Length)
                {
                    ...
                    var dataLength = checked(
                        (int)Math.Min(ChunkBytes, input.Length - input.Position)
                    );
                    var encodedLength = checked(
                        FrameHeaderBytes + ChunkBodyPrefixBytes + dataLength + FrameChecksumBytes
                    );
                    if (
                        pending.Count == MaxConcurrentComponentIo
                        || pending.Count != 0 && pendingBytes > MaxComponentIoBytes - encodedLength
                    )
                    ...
                    ZLinkCrc32C.Append(ref componentCrcState, data);
                    ZLinkCrc32C.Append(ref logicalCrcState, data);
                    var encoded = ServiceWirePilotCodec.EncodeRelocationDataChunkV1(
                        new(checked((uint)globalOrder), data)
                    );
                    pending.Add(
                        new ParticipantEncodedChunk(
                            source.Order,
                            localChunkOrder++,
                            globalOrder++,
                            dataLength,
                    ...
                    logicalLength = checked(logicalLength + (uint)dataLength);
                }
                components[source.Order].LogicalChecksumCrc32c = ~componentCrcState;
            }
            await FlushAsync().ConfigureAwait(false);
            if (logicalLength != preflightLogicalLength || globalOrder != preflightChunkCount)
            ...
            var root = await PutVerifiedAsync(store, manifest, retention, cancellationToken)
                .ConfigureAwait(false);
    ...
    ) ValidateParticipantComponentBounds(IReadOnlyList<long> componentLengths)
    {
        ArgumentNullException.ThrowIfNull(componentLengths);
        if (componentLengths.Count < 2)
            throw new ArgumentOutOfRangeException(nameof(componentLengths));
    ...
    private static long MeasureCanonicalSlice(long logicalLength, int stripeCount, int order)
    {
    ...
    private static int CalculateOrderedStripeCount(long logicalLength, int participantCount)
    {
        if (logicalLength <= 0)
            throw new InvalidOperationException("A canonical relocation stream must not be empty.");
        return checked((int)Math.Min(logicalLength, Math.Min(participantCount, MaxOrderedStripes)));
    }
    ...
    {
        var length = MeasureCanonicalSlice(logicalLength, stripeCount, order);
        var quotient = logicalLength / stripeCount;
        var remainder = logicalLength % stripeCount;
        var offset = checked(order * quotient + Math.Min(order, remainder));
        return ParticipantComponentSource.FromFileSlice(order, path, offset, length);
    }
    ...
    )
    {
        ...
        return ParticipantComponentSource.FromMemorySlice(
            order,
            bytes.Slice(checked((int)offset), checked((int)length))
        );
    }

    private static async ValueTask<ZLinkRelocationTreeRead> ReadParticipantComponentsAsync(
        IZLinkRelocationRepository store,
        ...
        try
        {
            var chunks = manifest
                .Components.SelectMany(component =>
                    component.Chunks.Select(chunk => new ManifestParticipantChunk(
                        component.Order,
                        chunk
                    ))
                )
                .ToArray();
            var logicalCrcState = uint.MaxValue;
            var componentCrcStates = Enumerable
                .Repeat(uint.MaxValue, manifest.Components.Count)
                .ToArray();
            ulong logicalLength = 0;
            await using (
                var output = new FileStream(
            ...
            {
                for (var firstIndex = 0; firstIndex < chunks.Length; )
                {
                    var count = CalculateParticipantBatchCount(chunks, firstIndex);
                    var batch = await Task.WhenAll(
                            chunks
                                .Skip(firstIndex)
                                .Take(count)
                                .Select(item =>
                                    ReadChunkAsync(store, item.Chunk, cancellationToken).AsTask()
                                )
                        )
                        .ConfigureAwait(false);
                    for (var index = 0; index < batch.Length; index++)
                    {
                        ...
                        if (chunk.Order != item.Chunk.Order)
                            throw DataLost("Relocation component completion order is invalid.");
                        await output
                            .WriteAsync(chunk.Data, cancellationToken)
                            .ConfigureAwait(false);
                        ZLinkCrc32C.Append(
                            ref componentCrcStates[item.ParticipantOrder],
                            chunk.Data.Span
                        );
                        ZLinkCrc32C.Append(ref logicalCrcState, chunk.Data.Span);
                        logicalLength = checked(logicalLength + (uint)chunk.Data.Length);
                    }
                    firstIndex += count;
                }
                await output.FlushAsync(cancellationToken).ConfigureAwait(false);
            ...
            if (
                ...
                throw DataLost("Relocation participant component stream is invalid.");

            var participants = new ZLinkRelocationParticipantEnvelope[manifest.Components.Count];
            ...
            for (var index = 0; index < manifest.Components.Count; index++)
            {
                if (~componentCrcStates[index] != manifest.Components[index].LogicalChecksumCrc32c)
                    throw DataLost("Relocation participant component checksum is invalid.");
            }
            if (manifest.CanonicalSlices)
            {
                var decoded = ZLinkRelocationEnvelopeCodec.Decode(input, manifest.InventoryDigest);
                if (
                    decoded.AggregateId != manifest.AggregateId
                    || !decoded.InventoryDigest.Span.SequenceEqual(manifest.InventoryDigest.Span)
                )
                    throw DataLost("Canonical relocation component identity is invalid.");
            ...
            for (var index = 0; index < manifest.Components.Count; index++)
            {
                ...
                if (
                    decoded.AggregateId != manifest.AggregateId
                    || decoded.AggregateGeneration != manifest.AggregateGeneration
                    || !decoded.InventoryDigest.Span.SequenceEqual(manifest.InventoryDigest.Span)
                    || decoded.Participants.Count != 1
                )
                    throw DataLost("Relocation participant component identity is invalid.");
                participants[index] = decoded.Participants[0];
                componentOffset = checked(componentOffset + component.LogicalLength);
            }
    ...
    {
        var manifest = DecodeParticipantManifest(encodedManifest.Span);
        var chunks = manifest.Components.SelectMany(static component => component.Chunks).ToArray();
        for (var firstIndex = 0; firstIndex < chunks.Length; )
        {
            ...
            await Task.WhenAll(
                    chunks
                        ...
                        .Take(count)
                        .Select(chunk =>
                            VerifyAndRenewChunkAsync(store, chunk, retention, cancellationToken)
                                .AsTask()
                        )
                )
                .ConfigureAwait(false);
    ...
    )
    {
        var references = components
            ...
            .ToArray();
        if (references.Any(static value => value.Length is < 1 or > MaxReferenceBytes))
            throw new InvalidOperationException("A relocation reference exceeds its UTF-8 bound.");
        ...
        BinaryPrimitives.WriteUInt64BigEndian(encoded.AsSpan(offset, 8), logicalLength);
        offset += 8;
        BinaryPrimitives.WriteUInt32BigEndian(encoded.AsSpan(offset, 4), logicalChecksum);
        offset += 4;
        encoded[offset++] = InventoryDigestBytes;
        envelope.InventoryDigest.Span.CopyTo(encoded.AsSpan(offset, InventoryDigestBytes));
        ...
        foreach (var component in components)
        {
            ...
            foreach (var chunk in component.Chunks)
            {
                ...
                BinaryPrimitives.WriteUInt32BigEndian(encoded.AsSpan(offset, 4), chunk.Order);
                offset += 4;
                ...
                BinaryPrimitives.WriteUInt64BigEndian(encoded.AsSpan(offset, 8), chunk.Length);
                offset += 8;
                BinaryPrimitives.WriteUInt32BigEndian(
                    encoded.AsSpan(offset, 4),
    ...
    private static ParticipantManifest DecodeParticipantManifest(ReadOnlySpan<byte> encoded)
    {
        ...
        if (version is not (1 or 2))
            throw DataLost("Relocation participant manifest version is invalid.");
        var canonicalSlices =
            version == 2
            && ReadByte(body, ref offset) switch
            {
                0 => false,
                1 => true,
                _ => throw DataLost("Relocation participant manifest component mode is invalid."),
            };
        ...
        if (ReadByte(body, ref offset) != InventoryDigestBytes)
            throw DataLost("Relocation participant manifest digest is invalid.");
        ...
        if (
            ...
            || participantCount < 2
            || participantCount
                > (uint)Math.Min(int.MaxValue, remaining / MinimumParticipantManifestBytes)
        )
            throw DataLost("Relocation participant manifest bounds are invalid.");

        var components = new ParticipantComponentManifest[checked((int)participantCount)];
        ...
        for (var index = 0; index < components.Length; index++)
        {
            ...
            if (
                ...
                || chunkCount > (uint)((body.Length - offset) / MinimumParticipantChunkBytes)
            )
                throw DataLost("Relocation participant component bounds are invalid.");
            ...
            for (var chunkIndex = 0; chunkIndex < chunks.Length; chunkIndex++)
            {
                ...
                if (
                    globalOrder != expectedGlobalOrder++
                    || referenceLength is 0 or > MaxReferenceBytes
                )
                    throw DataLost("Relocation participant chunk order is invalid.");
                ...
                if (
                    ...
                    || chunkIndex < chunks.Length - 1 && length != ChunkBytes
                )
                    throw DataLost("Relocation participant chunk length is invalid.");
                componentTotal = checked(componentTotal + length);
                chunks[chunkIndex] = new ChunkEntry(globalOrder, reference, length, checksum);
            }
            if (componentTotal != componentLength)
                throw DataLost("Relocation participant component length is invalid.");
        ...
        if (
            ...
            throw DataLost("Relocation participant manifest length is invalid.");
    ...
    private sealed class ParticipantComponentSource : IDisposable
    {
        ...
        {
            if (_path is null)
                return new MemoryStream(_memory.ToArray(), writable: false);
            ...
            return new BoundedReadStream(input, _offset, Length, leaveOpen: false);
        }
    ...
    ) : Stream
    {
        ...
        {
            var bounded = checked((int)Math.Min(count, length - Position));
            return bounded == 0 ? 0 : inner.Read(buffer, offset, bounded);
        }

        public override int Read(Span<byte> buffer)
        {
            var bounded = checked((int)Math.Min(buffer.Length, length - Position));
            return bounded == 0 ? 0 : inner.Read(buffer[..bounded]);
        }
    ...
    private sealed class ParticipantComponentBuilder(int order, long logicalLength)
    {
    ...
    private sealed record ManifestParticipantChunk(int ParticipantOrder, ChunkEntry Chunk);
}
