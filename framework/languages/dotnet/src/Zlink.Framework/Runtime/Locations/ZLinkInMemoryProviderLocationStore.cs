    ...
    : IZLinkLocationStore
{
    ...
    )
    {
        ...
        return _lane.RunAsync<ZLinkStoreWriteResult>(() =>
        {
            ...
            foreach (var condition in request.Conditions)
            {
                ...
                {
                    ZLinkStoreCondition.Missing missing => !_entries.ContainsKey(missing.Key),
                    ZLinkStoreCondition.Version version => _entries.TryGetValue(
                        version.Key,
                        out var current
                    )
        ...
        return _lane.RunAsync<ZLinkStoreScanResult>(() =>
        {
            ...
            if (request.Cursor is { } cursor)
            {
                ...
                if (
                    ...
                    || !_snapshots.TryGetValue(cursorValue[..separator], out snapshot!)
                )
                {
            ...
            else
            {
                ...
                snapshot = new Snapshot(
                    _entries
                        .Where(pair => pair.Key.Value.StartsWith(prefix, StringComparison.Ordinal))
                        .OrderBy(static pair => pair.Key.Value, StringComparer.Ordinal)
                        .Select(pair => new KeyValuePair<ZLinkStoreKey, ZLinkStoreValue>(
                            pair.Key,
            ...
            {
                var id = _snapshots.First(pair => ReferenceEquals(pair.Value, snapshot)).Key;
    ...
    {
        if (_nextEntryExpiry is null || now < _nextEntryExpiry)
            return;
        ...
        foreach (var pair in _entries.ToArray())
        {
            ...
            if (
                pair.Value.ExpiresAt is { } expiresAt
        ...
        var mutationKeys = request
            .Mutations.Select(static mutation =>
                mutation switch
                {
                    ...
                    _ => throw new ArgumentException("Unknown Store mutation.", nameof(request)),
                }
            )
            .ToArray();
        if (
