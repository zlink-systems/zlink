...
internal sealed class ZLinkFanoutDiscovery : IAsyncDisposable
{
    ...
    )
    {
        foreach (var (channelName, registration) in state.Registration.Channels)
        {
            ...
            if (
                registration.Publisher is not null
            ...
            )
            {
                ...
                await PublishAsync(publisher, ZLinkLocationWriteIntent.NewClaim, cancellationToken)
                    .ConfigureAwait(false);
                _publishers.Add(publisher);
            }

            if (
                registration.Subscriber?.AcquisitionMode != ZLinkPeerAcquisitionMode.AutoConnect
    ...
    )
    {
        ...
        var result = await _store
            .UpdateFanoutPublisherAsync(descriptor, intent, cancellationToken)
            .ConfigureAwait(false);
        if (
    ...
    ) : IAsyncDisposable
    {
        ...
        private async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            while (await timer.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false))
                ...
                catch (Exception exception)
                {
                    ...
                    runtime.RecordLocationFailure(lastSuccessAt: null, failureAt: observed);
        ...
        private async ValueTask ReconcileAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var row in rows)
            {
                ...
                if (
                    _observedRevisions.TryGetValue(key, out var observed)
        ...
        private async ValueTask<IReadOnlyList<ZLinkFanoutPublisherDescriptor>> ListAllAsync(
            CancellationToken cancellationToken
        )
        {
