...
internal sealed class ZLinkSpotMeshLocationResolver
{
    ...
    {
        _rows = rows;
        _meshNames = registration
            .SpotNodes.Values.Select(static node => node.SpotMeshChannelName ?? node.SpotNodeName)
    ...
    internal async ValueTask<(
        ...
        bool RowPresent
    )> ResolveActorWithPresenceAsync(string actorId, CancellationToken cancellationToken)
    {
        var present = false;
        foreach (var meshName in _meshNames)
        {
            var (row, rowPresent) = await _rows
    ...
    )
    {
        ...
        {
            var row = await _rows
                .ResolveSpotRowAsync(new ZLinkSpotLocationKey(spotId), cancellationToken)
                .ConfigureAwait(false);
            if (row is not null)
                return row.ToPublic();
        }
