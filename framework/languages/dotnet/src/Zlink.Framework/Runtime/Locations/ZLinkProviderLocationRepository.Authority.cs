...
internal sealed partial class ZLinkProviderLocationRepository
{
    ...
    )
    {
        ...
        var stored = await ReadAuthorityRecordAsync(key, cancellationToken).ConfigureAwait(false);
        ...
        var result = await provider
            .ScanAsync(
                new ZLinkStoreScanRequest(
                    ...
                    cursor is null ? null : new ZLinkStoreScanCursor(cursor.Value.Encoded),
                    limit
                ),
                cancellationToken
            )
    ...
    )
    {
        ...
        var terminal = await ReadTerminalAsync(operation, cancellationToken).ConfigureAwait(false);
    ...
    )
    {
        ...
        await EnsureAggregateStagingRecoveryAsync(cancellationToken).ConfigureAwait(false);
    ...
    private async ValueTask<ZLinkAggregatePrepareResult> PrepareAggregateCoreAsync(
        ZLinkAggregatePrepareRequest request,
        int counterRetry,
        TimeSpan retryDeadline,
        CancellationToken cancellationToken
    )
    {
        ...
        var existing = await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
            .ConfigureAwait(false);
        if (existing is not null && existing.Record.Status != AggregateStatus.Staging)
        ...
        if (
            ...
            && (
                ...
                || !AggregateInventoryRootEquals(existing.Record.Inventory, inventory.Root)
            )
        )
        ...
        if (
            newOwnerCount > 0
            && authorityCounter.Value > (ulong)long.MaxValue - (ulong)newOwnerCount
        )
            return new ZLinkAggregatePrepareResult.GenerationExhausted();
        var nextIssuedGeneration = authorityCounter.Value;
        var targetAuthorityOwnerGenerations = new Dictionary<ZLinkAuthorityKey, ulong>(
            request.Participants.Count
        );
        for (var index = 0; index < request.Participants.Count; index++)
        {
            var participant = request.Participants[index];
            var currentGeneration = authorities[index]!.Meta.AuthorityOwnerGeneration;
        ...
        try
        {
            ...
            if (staging is null)
            {
                var raced = await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
                    .ConfigureAwait(false);
                if (raced is null)
                    return new ZLinkAggregatePrepareResult.Conflict();
                if (raced.Record.Status != AggregateStatus.Staging)
                    return await ReconcileExistingAggregateAsync(
                        ...
                        .ConfigureAwait(false);
                if (
                    !AggregateHeaderEquals(raced.Record.Header, header)
                    || !CryptographicOperations.FixedTimeEquals(
                        raced.Record.RequestFingerprint,
                        requestFingerprint
                    )
                    || !AggregateInventoryRootEquals(raced.Record.Inventory, inventory.Root)
                )
                    return new ZLinkAggregatePrepareResult.Conflict();
                staging = raced;
            }
            await StageAggregateInventoryAsync(fence, inventory, cancellationToken)
                .ConfigureAwait(false);
            var stagedParticipants = request
                .Participants.Select(
                    (participant, index) =>
                        new AggregateParticipantRecord(
                            index,
                ...
                .ToArray();
            if (
                !await StageAggregateParticipantsAsync(
                        fence,
            ...
            {
                await AbortAggregateStagingAsync(fence, staging, cancellationToken)
                    .ConfigureAwait(false);
                return new ZLinkAggregatePrepareResult.Conflict();
            }
            await UpdateAggregateParticipantGenerationsAsync(
                ...
                .ConfigureAwait(false);
            if (
                !await InstallAggregateParticipantFencesAsync(
                        fence,
                        request,
                        authorities,
                        cancellationToken
            ...
            var nextCapacity = capacity.Record.Clone();
            ApplyCapacityVector(nextCapacity, request.Capacity, pendingDelta: 1);
            var conditions = new List<ZLinkStoreCondition>
            {
            ...
            var mutations = new List<ZLinkStoreMutation>
            {
                new ZLinkStoreMutation.Put(
                    key,
                    Encode(staging.Record with { Status = AggregateStatus.Prepared }),
                    null
                ),
                new ZLinkStoreMutation.Put(capacity.Key, Encode(nextCapacity), null),
                new ZLinkStoreMutation.Put(
            ...
            {
                var terminal = await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
                    .ConfigureAwait(false);
                if (terminal is null || terminal.Record.Status == AggregateStatus.Staging)
                {
                    var unchanged = true;
            ...
            if (result is ZLinkStoreWriteResult.Applied)
                ...
                {
                    TargetAuthorityOwnerGenerations = targetAuthorityOwnerGenerations,
                };
            ...
            {
                var stillStaging = await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
                    .ConfigureAwait(false);
                if (stillStaging?.Record.Status == AggregateStatus.Staging)
                    await AbortAggregateStagingAsync(fence, stillStaging, cancellationToken)
                        .ConfigureAwait(false);
            }
        ...
        {
            var recovered = await ReconcileOrAbortClaimedAggregatePrepareAsync(
                    fence,
                    header,
                    requestFingerprint,
                    inventory.Root,
    ...
    public async ValueTask<ZLinkAggregateCommitResult> CommitAggregateForRecoveryAsync(
        ZLinkAggregateFence fence,
        CancellationToken cancellationToken = default
    )
    {
    ...
    )
    {
        ...
        {
            await CleanupAggregateParticipantsAsync(fence, cancellationToken).ConfigureAwait(false);
            return;
        }
        ...
        await Parallel
            .ForEachAsync(
                ...
                async (storedParticipant, token) =>
                {
                    ...
                    for (var attempt = 0; attempt < 8; attempt++)
                    {
                        ...
                        var nextGeneration = authority.Meta.AuthorityOwnerGeneration;
                        var conditions = new List<ZLinkStoreCondition>
                        {
                            new ZLinkStoreCondition.Version(
                        ...
                        if (changesOwner)
                        {
                            ...
                            if (
                                (
                                    nextGeneration = storedParticipant
                                        .Metadata
                                        .TargetAuthorityOwnerGeneration
                                ) == 0
                                || nextGeneration <= authority.Meta.AuthorityOwnerGeneration
            ...
            .ConfigureAwait(false);
        await MarkAggregateParticipantsNormalizedAsync(fence, cancellationToken)
            .ConfigureAwait(false);
        await CleanupAggregateParticipantsAsync(fence, cancellationToken).ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        {
            var aggregate =
                await ReadRecordAsync<AggregateRecord>(key, cancellationToken).ConfigureAwait(false)
                ?? throw new ZLinkRelocationDataLostException(
                    $"Committed aggregate '{fence.AggregateId:N}' is missing."
                );
            if (aggregate.Record.ParticipantsNormalized)
                return;
            if (aggregate.Record.Status != AggregateStatus.Committed)
                throw new ZLinkRelocationDataLostException(
                ...
                .WriteAsync(
                    new ZLinkStoreWriteRequest(
                        ...
                        [
                            new ZLinkStoreMutation.Put(
                                key,
                                Encode(aggregate.Record with { ParticipantsNormalized = true }),
                                null
                            ),
                        ]
                    ),
    ...
    )
    {
        ...
        if (aggregate is null || aggregate.Record.ParticipantsCleaned)
            return;
        if (
            aggregate.Record.Status == AggregateStatus.Committed
            && !aggregate.Record.ParticipantsNormalized
        )
            return;
        if (aggregate.Record.Status is not (AggregateStatus.Committed or AggregateStatus.Aborted))
            return;
        ...
        for (var attempt = 0; attempt < 8; attempt++)
        {
            aggregate = await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
                .ConfigureAwait(false);
            if (aggregate is null || aggregate.Record.ParticipantsCleaned)
                return;
            var result = await provider
                .WriteAsync(
                    new ZLinkStoreWriteRequest(
                        ...
                        [
                            new ZLinkStoreMutation.Put(
                                key,
                                Encode(aggregate.Record with { ParticipantsCleaned = true }),
                                null
                            ),
                        ]
                    ),
    ...
    )
    {
        ...
        for (var offset = 0; offset < inventory.Count; offset += maximumDeletesPerBatch)
        {
            var count = Math.Min(maximumDeletesPerBatch, inventory.Count - offset);
                ...
                .Select(entry =>
                    (ZLinkStoreMutation)
                        new ZLinkStoreMutation.Delete(
                            payloadRows
                                ? AggregateParticipantPayloadKey(fence, checked((int)entry.Index))
                                : AggregateParticipantMetaKey(fence, checked((int)entry.Index))
                        )
                )
                .ToArray();
            await provider
    ...
    )
    {
        ...
        for (var level = 0; level < inventory.PageCountsByLevel.Count; level++)
        {
            var pageCount = inventory.PageCountsByLevel[level];
            if (pageCount < 1 || pageCount > inventory.TotalCount)
                throw InventoryDataLost(fence, $"level {level} page count is invalid");
            for (var offset = 0; offset < pageCount; offset += maximumDeletesPerBatch)
            {
                var count = Math.Min(maximumDeletesPerBatch, pageCount - offset);
                var mutations = Enumerable
                    .Range(offset, count)
                    .Select(index =>
                        (ZLinkStoreMutation)
                            new ZLinkStoreMutation.Delete(
                                AggregateInventoryPageKey(fence, level, index)
                            )
                    )
                    .ToArray();
                await provider
    ...
    )
    {
        ...
        return generation > 0
            ...
            && authority.Meta.OwnerLeaseGeneration == aggregate.Header.TargetOwner.LeaseGeneration
            && authority.Meta.Allocation.Descriptor == aggregate.Header.TargetDescriptor
            && authority.Meta.Allocation.DescriptorLifecycleGeneration
                == aggregate.Header.TargetDescriptorLifecycleGeneration;
    }

    private async ValueTask<StoredRecord<AggregateRecord>?> ClaimAggregateStagingAsync(
        ZLinkStoreKey key,
    ...
    {
        var storeNow = await ReadStoreNowAsync(cancellationToken).ConfigureAwait(false);
            ...
            var claimed = await provider
                .WriteAsync(
                    new ZLinkStoreWriteRequest(
                        ...
                        [new ZLinkStoreMutation.Put(key, Encode(staged), null)]
                    ),
                    cancellationToken
                )
                .ConfigureAwait(false);
        ...
        catch
        {
            ...
            if (
                ...
                && AggregateInventoryRootEquals(reconciled.Record.Inventory, inventory)
            )
                return reconciled;
            throw;
        }
    ...
    )
    {
            ...
            .WriteAsync(
                new ZLinkStoreWriteRequest(
                    ...
                    [
                        new ZLinkStoreMutation.Put(
                            key,
                            Encode(staging.Record with { Status = AggregateStatus.Aborted }),
                            null
                        ),
                    ]
                ),
        ...
        var aborted =
            result is ZLinkStoreWriteResult.Applied
                ...
                : (
                    await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
                        .ConfigureAwait(false)
                )?.Record;
        if (aborted?.Status != AggregateStatus.Aborted)
            return;
        await ReleaseAggregateParticipantFencesAsync(fence, aborted, cancellationToken)
            .ConfigureAwait(false);
        await CleanupAggregateParticipantsAsync(fence, cancellationToken).ConfigureAwait(false);
    }

    private async ValueTask<ZLinkAggregatePrepareResult?> ReconcileOrAbortClaimedAggregatePrepareAsync(
        ZLinkAggregateFence fence,
        AggregateHeader header,
        byte[] requestFingerprint,
        AggregateInventoryRoot inventory,
        ...
        try
        {
            for (var attempt = 0; attempt < 8; attempt++)
            {
                ...
                if (aggregate is null)
                    return null;
                if (
                    !AggregateHeaderEquals(aggregate.Record.Header, header)
                    || !CryptographicOperations.FixedTimeEquals(
                        aggregate.Record.RequestFingerprint,
                        requestFingerprint
                    )
                    || !AggregateInventoryRootEquals(aggregate.Record.Inventory, inventory)
                )
                    return new ZLinkAggregatePrepareResult.Conflict();

                if (
                    aggregate.Record.Status is AggregateStatus.Prepared or AggregateStatus.Committed
                )
                {
                    if (request is null)
                        return new ZLinkAggregatePrepareResult.Prepared(fence);
                ...
                if (aggregate.Record.Status == AggregateStatus.Aborted)
                {
                    try
                    {
                        await ReleaseAggregateParticipantFencesAsync(
                            ...
                            .ConfigureAwait(false);
                        await CleanupAggregateParticipantsAsync(fence, deadline.Token)
                            .ConfigureAwait(false);
                    }
                    catch (ZLinkRelocationDataLostException)
                    {
                        ...
                        await CleanupUnfencedAggregateChildrenAsync(fence, deadline.Token)
                            .ConfigureAwait(false);
                    }
                    return new ZLinkAggregatePrepareResult.Conflict();
                }
        ...
        await Parallel
            .ForEachAsync(
                ...
                async (index, token) =>
                {
                    ...
                    installed[index] = authority.Meta.AggregateFence == fence;
                }
            )
            .ConfigureAwait(false);
    ...
    )
    {
        ...
        const int maximumDeletesPerBatch = 1024;
        for (var offset = 0; offset < keys.Count; offset += maximumDeletesPerBatch)
        {
            var mutations = keys.Skip(offset)
    ...
    private async ValueTask EnsureAggregateStagingRecoveryAsync(CancellationToken cancellationToken)
    {
        if (Volatile.Read(ref aggregateStagingRecoveryCompleted) != 0)
            return;
        await aggregateRecoveryGate.WaitAsync(cancellationToken).ConfigureAwait(false);
        try
        {
            if (aggregateStagingRecoveryCompleted != 0)
                return;
            var abandoned =
                new List<(ZLinkAggregateFence Fence, StoredRecord<AggregateRecord> Aggregate)>();
            var preparedToAbort = new List<ZLinkAggregateFence>();
            var terminalToFinish =
                new List<(ZLinkAggregateFence Fence, AggregateRecord Aggregate)>();
            ZLinkStoreScanCursor? cursor = null;
            ...
            do
            {
                ...
                {
                    if (pair.Key.Value.Contains(":participant:", StringComparison.Ordinal))
                        continue;
                    if (pair.Key.Value.Contains(":inventory:", StringComparison.Ordinal))
                        continue;
                    AggregateRecord aggregate;
                    try
                    {
                    ...
                    if (
                        aggregate.StagedAt > pair.Value.StoreNow - AggregateStagingRetention
                        || await ReadLiveOwnerAsync(aggregate.Header.TargetOwner, cancellationToken)
                            .ConfigureAwait(false)
                            is not null
                    )
                        continue;
            ...
            foreach (var item in abandoned)
                await AbortAggregateStagingAsync(item.Fence, item.Aggregate, cancellationToken)
                    .ConfigureAwait(false);
            foreach (var fence in preparedToAbort)
                await AbortAggregateAsync(fence, cancellationToken).ConfigureAwait(false);
            foreach (var item in terminalToFinish)
            {
                ...
                else
                {
                    await ReleaseAggregateParticipantFencesAsync(
                        ...
                        .ConfigureAwait(false);
                    await CleanupAggregateParticipantsAsync(item.Fence, cancellationToken)
                        .ConfigureAwait(false);
                }
            }
        ...
        await Parallel
            .ForEachAsync(
                ...
                async (index, token) =>
                {
                        ...
                        result = await provider
                            .WriteAsync(
                                new ZLinkStoreWriteRequest(
                                    ...
                                    [
                                        new ZLinkStoreMutation.Put(metaKey, Encode(record), null),
                                        new ZLinkStoreMutation.Put(
                                            payloadKey,
                    ...
                    {
                        exact[index] = await IsExactStagedParticipantForReconciliationAsync(
                                metaKey,
                                payloadKey,
                                record,
    ...
    )
    {
        ...
        var level = 0;
        while (
            references.Count > MaxInventoryPageEntries
            || EncodeAggregateInventoryRoot(
                new AggregateInventoryRoot(
                    entries.Length,
        ...
        {
            level++;
            references = PackInventoryUpperPages(level, references, pages);
        }
    ...
    private static byte[] EncodeAggregateInventoryRoot(AggregateInventoryRoot root)
    {
    ...
    private static byte[] EncodeAggregateInventoryPage(AggregateInventoryPage page)
    {
        using var stream = new MemoryStream();
    ...
    private static byte[] EncodeAggregateInventoryLeafEntry(AggregateInventoryLeafEntry entry)
    {
        using var stream = new MemoryStream();
    ...
    private static AggregateInventoryPage DecodeAggregateInventoryPage(ReadOnlyMemory<byte> encoded)
    {
        ...
        var leafCount = reader.ReadBoundedCount(MaxInventoryPageEntries);
        ...
        var childCount = reader.ReadBoundedCount(MaxInventoryPageEntries);
        var children = new AggregateInventoryPageReference[childCount];
        for (var childIndex = 0; childIndex < childCount; childIndex++)
            children[childIndex] = reader.ReadReference();
        reader.RequireEnd();
        return new AggregateInventoryPage(level, index, startIndex, entryCount, entries, children);
    }

    private static void WriteInventoryHeader(Stream stream, byte recordKind)
    {
        Span<byte> header = stackalloc byte[6];
        BinaryPrimitives.WriteUInt32BigEndian(header, InventoryCodecMagic);
        header[4] = InventoryCodecVersion;
        header[5] = recordKind;
        stream.Write(header);
    }

    private static void WriteInventoryInt32(Stream stream, int value)
    {
    ...
    private static void WriteInventoryString(Stream stream, string value)
    {
    ...
    private static void WriteInventoryHash(Stream stream, byte[] hash)
    {
        if (hash.Length != SHA256.HashSizeInBytes)
            throw new InvalidDataException("An aggregate inventory SHA-256 value is invalid.");
        stream.Write(hash);
    }

    private static void WriteInventoryLeafEntry(Stream stream, AggregateInventoryLeafEntry entry)
    {
    ...
    private static IReadOnlyList<AggregateInventoryPageReference> PackInventoryLeafPages(
    ...
    )
    {
        ...
        {
            var count = Math.Min(MaxInventoryPageEntries, entries.Count - offset);
    ...
    private static IReadOnlyList<AggregateInventoryPageReference> PackInventoryUpperPages(
        int level,
    ...
    )
    {
        ...
        {
            var count = Math.Min(MaxInventoryPageEntries, children.Count - offset);
            AggregateInventoryPage page;
            byte[] encoded;
            do
            {
    ...
    ) => new(page.Level, page.Index, page.StartIndex, page.EntryCount, SHA256.HashData(encoded));
    ...
    )
    {
        await Parallel
            .ForEachAsync(
                ...
                async (page, token) =>
                {
                    var key = AggregateInventoryPageKey(fence, page.Level, page.Index);
    ...
    private async ValueTask<AggregateInventoryLeafEntry[]> ReadAndValidateAggregateInventoryAsync(
        ZLinkAggregateFence fence,
        AggregateRecord aggregate,
        CancellationToken cancellationToken
    )
    {
        ...
        if (
            ...
            || root.PageCountsByLevel.Any(count => count < 1 || count > root.TotalCount)
            || EncodeAggregateInventoryRoot(root).Length > MaxInventoryPageBytes
        )
            throw InventoryDataLost(fence, "root metadata is invalid");

        var leafPageCount = root.PageCountsByLevel[0];
        if (
            leafPageCount > root.TotalCount
            || (long)root.TotalCount > (long)leafPageCount * MaxInventoryPageEntries
        )
            throw InventoryDataLost(fence, "leaf page count cannot contain the declared total");
        for (var level = 1; level < root.PageCountsByLevel.Count; level++)
        {
            var lowerCount = root.PageCountsByLevel[level - 1];
            var currentCount = root.PageCountsByLevel[level];
            var minimum =
                ((long)lowerCount + MaxInventoryPageEntries - 1) / MaxInventoryPageEntries;
        ...
        if (root.TopPages.Count != root.PageCountsByLevel[root.TopLevel])
            throw InventoryDataLost(fence, "top page count does not match the root");
        ...
        foreach (var reference in root.TopPages)
        {
            if (reference.Level != root.TopLevel || reference.StartIndex != expectedStart)
                throw InventoryDataLost(fence, "top page references are reordered");
        ...
        if (
            ...
            throw InventoryDataLost(fence, "page counts do not match the root");
        ...
        for (var index = 0; index < entries.Count; index++)
        {
            ...
            if (
                ...
                || entry.AuthorityPayloadSha256.Length != SHA256.HashSizeInBytes
                || entry.MembershipMutationSha256 is null
                || entry.MembershipMutationSha256.Length != SHA256.HashSizeInBytes
            )
                throw InventoryDataLost(fence, $"leaf entry {index} is invalid or reordered");
            digest.AppendData(EncodeAggregateInventoryLeafEntry(entry));
        }
        if (!CryptographicOperations.FixedTimeEquals(digest.GetHashAndReset(), root.Digest))
            throw InventoryDataLost(fence, "digest does not match");
        return entries.ToArray();

        async ValueTask ReadInventoryPageAsync(AggregateInventoryPageReference reference)
        {
            if (
                reference.Level < 0
            ...
            if (page.Level == 0)
            {
                if (
                    ...
                    || page.Entries.Count is < 1 or > MaxInventoryPageEntries
                    || page.Children is null
                    || page.Children.Count != 0
                    || page.Entries.Count != page.EntryCount
                )
                    throw InventoryDataLost(fence, $"leaf page {page.Index} has invalid bounds");
                for (var offset = 0; offset < page.Entries.Count; offset++)
                {
                    var entry = page.Entries[offset];
                    if (entry.Index != (long)page.StartIndex + offset)
                        throw InventoryDataLost(fence, $"leaf page {page.Index} is reordered");
                    if (entries.Count >= root.TotalCount)
                        throw InventoryDataLost(fence, "leaf entries exceed the declared total");
                    entries.Add(entry);
                }
                return;
            }
    ...
    private static ZLinkRelocationDataLostException InventoryDataLost(
        ZLinkAggregateFence fence,
        string reason
    ) => new($"Aggregate '{fence.AggregateId:N}' authoritative inventory {reason}.");
    ...
    )
    {
        await Parallel
            .ForEachAsync(
                ...
                {
                    var key = AggregateParticipantMetaKey(fence, participant.Index);
                    var expectedGeneration = generations[new ZLinkAuthorityKey(participant.Key)];
                    for (var attempt = 0; attempt < 8; attempt++)
                    {
                        var stored =
                            await ReadRecordAsync<AggregateParticipantRecord>(key, token)
                                .ConfigureAwait(false)
                            ?? throw new ZLinkRelocationDataLostException(
    ...
    private async ValueTask<bool> IsExactStagedParticipantForReconciliationAsync(
        ZLinkStoreKey metaKey,
        ZLinkStoreKey payloadKey,
        AggregateParticipantRecord expected,
        ReadOnlyMemory<byte> expectedPayload
    ...
    private async ValueTask<StoredAggregateParticipant[]> ReadAggregateParticipantsAsync(
        ZLinkAggregateFence fence,
        AggregateRecord aggregate,
        CancellationToken cancellationToken
    )
    {
        ...
        var inventory = await ReadAndValidateAggregateInventoryAsync(
            ...
            .ConfigureAwait(false);
        var participants = new StoredAggregateParticipant[aggregate.ParticipantCount];
        await Parallel
            .ForEachAsync(
                ...
                async (index, token) =>
                {
                    ...
                    var payload = await provider
                        ...
                        .ConfigureAwait(false);
                    if (
                        payload is not ZLinkStoreReadResult.Found found
                        || meta.Record.Index != index
                        || !AggregateInventoryEntryEquals(inventory[index], meta.Record)
                        || !CryptographicOperations.FixedTimeEquals(
                            meta.Record.RequestFingerprint,
    ...
    private async ValueTask<StoredAggregateParticipant> ReadAggregateParticipantAsync(
        ZLinkAggregateFence fence,
        AggregateRecord aggregate,
        int index,
        CancellationToken cancellationToken
    )
    {
        ...
        var inventoryEntry = await ReadAndValidateAggregateInventoryEntryAsync(
                fence,
                aggregate,
                index,
                cancellationToken
        ...
        if (
            payload is not ZLinkStoreReadResult.Found found
            || meta.Record.Index != index
            || !AggregateInventoryEntryEquals(inventoryEntry, meta.Record)
            || !CryptographicOperations.FixedTimeEquals(
                meta.Record.RequestFingerprint,
                aggregate.RequestFingerprint
            )
    ...
    private async ValueTask<AggregateInventoryLeafEntry> ReadAndValidateAggregateInventoryEntryAsync(
        ZLinkAggregateFence fence,
        AggregateRecord aggregate,
    ...
    )
    {
        ...
        foreach (var reference in root.TopPages)
        {
            if (
                ...
                throw InventoryDataLost(fence, "top page references are invalid or reordered");
            var end = checked((long)reference.StartIndex + reference.EntryCount);
            if (participantIndex >= reference.StartIndex && participantIndex < end)
                selected = reference;
            expectedStart = end;
        }
        if (expectedStart != root.TotalCount || selected is null)
            throw InventoryDataLost(fence, "top page references do not contain the participant");
        ...
        )
        {
            ...
            foreach (var candidate in page.Children)
            {
                ...
                var childEnd = checked((long)candidate.StartIndex + candidate.EntryCount);
                if (participantIndex >= candidate.StartIndex && participantIndex < childEnd)
                    child = candidate;
                childStart = childEnd;
            }
            if (childStart != (long)page.StartIndex + page.EntryCount || child is null)
                throw InventoryDataLost(
                    fence,
    ...
    )
    {
        ...
        await Parallel
            .ForEachAsync(
                ...
                {
                    var authority =
                        await ReadAuthorityRecordAsync(participants[index].Key, token)
                            .ConfigureAwait(false)
                        ?? throw new InvalidDataException(
                    ...
                    generations[index] = authority.Meta.AuthorityOwnerGeneration;
                }
            )
            .ConfigureAwait(false);
        return participants
            .Select(
                (participant, index) => new { participant.Key, Generation = generations[index] }
            )
            .ToDictionary(static pair => pair.Key, static pair => pair.Generation);
    }

    private async ValueTask<ZLinkAggregatePrepareResult> ReconcileExistingAggregateAsync(
        ZLinkAggregateFence fence,
        AggregateRecord aggregate,
        ZLinkAggregatePrepareRequest request,
        CancellationToken cancellationToken
    ...
    {
        if (
            aggregate.Status is not (AggregateStatus.Prepared or AggregateStatus.Committed)
            || !AggregateHeaderEquals(aggregate.Header, CreateAggregateHeader(request))
            || !CryptographicOperations.FixedTimeEquals(
                aggregate.RequestFingerprint,
        ...
        if (!aggregate.ParticipantsNormalized)
        {
            var participants = await ReadAggregateParticipantsAsync(
                ...
                .ConfigureAwait(false);
            if (!AggregateParticipantsEqual(participants, request.Participants))
                return new ZLinkAggregatePrepareResult.Conflict();
            generations = participants.ToDictionary(
                static participant => new ZLinkAuthorityKey(participant.Metadata.Key),
                static participant => participant.Metadata.TargetAuthorityOwnerGeneration
            );
        }
        else
    ...
    private async ValueTask<ZLinkAggregatePrepareResult> ReconcileExistingAggregateForAmbiguousAsync(
        ZLinkAggregateFence fence,
        AggregateRecord aggregate,
        ZLinkAggregatePrepareRequest request
    )
    {
        using var deadline = new CancellationTokenSource(AmbiguousReconciliationTimeout);
        return await ReconcileExistingAggregateAsync(fence, aggregate, request, deadline.Token)
            .AsTask()
            .WaitAsync(deadline.Token)
            .ConfigureAwait(false);
    }
    ...
    {
        using var deadline = new CancellationTokenSource(AmbiguousReconciliationTimeout);
        await NormalizeCommittedAggregateAsync(fence, aggregate, deadline.Token)
            .AsTask()
            .WaitAsync(deadline.Token)
            .ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        for (var index = 0; index < stored.Count; index++)
        {
            ...
            if (
                !string.Equals(actual.Metadata.Key, expected.Key.Value, StringComparison.Ordinal)
                || !string.Equals(
                    actual.Metadata.ExpectedStoreVersion,
                    expected.ExpectedStoreVersion,
                    StringComparison.Ordinal
                )
                || actual.Metadata.OwnerTransition != expected.OwnerTransition
                || !CryptographicOperations.FixedTimeEquals(
                    actual.Payload.Span,
    ...
    private static ZLinkAggregatePrepareRequest RehydrateAggregateRequest(
        ...
        new(
            ...
            aggregate.Header.AggregateGeneration,
            participants
                .Select(static participant => new ZLinkAggregateParticipant(
    ...
    private static AggregateHeader CreateAggregateHeader(ZLinkAggregatePrepareRequest request) =>
        new(
            request.AggregateId,
    ...
    private static bool AggregateHeaderEquals(AggregateHeader left, AggregateHeader right) =>
        EncodeAggregateHeader(left).AsSpan().SequenceEqual(EncodeAggregateHeader(right));

    private static byte[] AggregateRequestFingerprint(ZLinkAggregatePrepareRequest request)
    {
        using var hash = IncrementalHash.CreateHash(HashAlgorithmName.SHA256);
        hash.AppendData(EncodeAggregateHeader(CreateAggregateHeader(request)));
        using var stream = new MemoryStream();
        WriteAggregateFingerprintHeader(stream, AggregateRequestRecordKind);
        WriteInventoryInt32(stream, request.Participants.Count);
        foreach (
            var participant in request.Participants.OrderBy(
                static participant => participant.Key.Value,
                StringComparer.Ordinal
            )
        )
        {
            ...
            WriteInventoryString(stream, participant.ExpectedStoreVersion);
            stream.WriteByte((byte)participant.OwnerTransition);
            WriteInventoryHash(stream, Sha256(participant.AuthorityPayload));
            WriteInventoryHash(stream, Sha256(participant.MembershipMutation));
        }
        hash.AppendData(stream.GetBuffer().AsSpan(0, checked((int)stream.Length)));
        return hash.GetHashAndReset();
    }

    private static byte[] EncodeAggregateHeader(AggregateHeader header)
    {
        using var stream = new MemoryStream();
        WriteAggregateFingerprintHeader(stream, AggregateHeaderRecordKind);
        ...
        WriteInventoryString(stream, header.TargetDescriptor.MeshName);
        WriteInventoryString(stream, header.TargetDescriptor.Rid.ToHex());
        WriteInventoryUInt64(stream, header.TargetDescriptorLifecycleGeneration);
        ...
        WriteInventoryInt64(stream, header.TargetOwner.LeaseGeneration);
        return stream.ToArray();
    }

    private static void WriteAggregateFingerprintHeader(Stream stream, byte recordKind)
    {
        Span<byte> header = stackalloc byte[6];
        BinaryPrimitives.WriteUInt32BigEndian(header, AggregateFingerprintMagic);
        header[4] = AggregateFingerprintVersion;
        header[5] = recordKind;
        stream.Write(header);
    }

    private static void WriteInventoryUInt64(Stream stream, ulong value)
    {
    ...
    private static void WriteInventoryInt64(Stream stream, long value)
    {
    ...
    private static ZLinkAggregatePrepareRequest CanonicalizeAggregateRequest(
        ZLinkAggregatePrepareRequest request
    )
    {
    ...
    private async ValueTask<ZLinkAggregatePrepareResult> ReconcileAggregatePrepareAsync(
        ZLinkStoreKey key,
        ZLinkAggregateFence fence,
        ZLinkAggregatePrepareRequest request,
        CancellationToken cancellationToken
    ...
    {
        var stored = await ReadRecordAsync<AggregateRecord>(key, cancellationToken)
            .ConfigureAwait(false);
        return stored is null
    ...
    )
    {
        ...
        if (aggregate.Record.Status == AggregateStatus.Aborted)
        {
            await ReleaseAggregateParticipantFencesAsync(fence, aggregate.Record, cancellationToken)
                .ConfigureAwait(false);
            await CleanupAggregateParticipantsAsync(fence, cancellationToken).ConfigureAwait(false);
        ...
        var capacity = await ReadCapacityAsync(header.TargetDescriptor, cancellationToken)
            .ConfigureAwait(false);
        var nextCapacity = capacity.Record.Clone();
        ApplyCapacityVector(nextCapacity, header.Capacity, pendingDelta: -1);
        var conditions = new List<ZLinkStoreCondition>
        {
            new ZLinkStoreCondition.Version(key, aggregate.Version),
            capacity.Condition,
    ...
    )
    {
        ...
        for (var attempt = 0; attempt < 8; attempt++)
        {
            ...
            if (projectCommittedAggregate && meta.AggregateFence is { } aggregateFence)
            {
                var aggregate = await ReadRecordAsync<AggregateRecord>(
                ...
                )
                {
                    ...
                    catch (ZLinkRelocationDataLostException)
                    {
                        ...
                        var currentMeta = await provider
                            ...
                            .ConfigureAwait(false);
                        if (
                            currentMeta is not ZLinkStoreReadResult.Found current
                    ...
                    var targetAuthorityOwnerGeneration = participant
                        .Metadata
                        .TargetAuthorityOwnerGeneration;
                    ...
                    meta = meta with
                    {
                        ...
                        Allocation = changesOwner
                            ...
                            {
                                Descriptor = aggregate.Record.Header.TargetDescriptor,
            ...
            return new StoredAuthority(
                ...
                meta,
                found.Version,
                Snapshot(meta, found.Version, found.StoreNow, payloadBytes)
            );
        }
        throw new IOException($"Authority '{key.Value}' changed continuously while it was read.");
    }
    ...
    )
    {
        ...
        var ownerRead = await ReadLiveOwnerAsync(owner, cancellationToken).ConfigureAwait(false);
    ...
    )
    {
        ...
        if (found.Value.ExpiresAt is not { } expiresAt)
            throw new InvalidDataException("The creation terminal store value has no expiration.");
        var envelope = found.Value.Bytes.ToArray();
        return new StoredTerminal(
            new ZLinkCreationTerminalRecord(operation, envelope, expiresAt, found.Value.StoreNow),
            found.Value.Version
        );
    }
    ...
    )
    {
        ...
        if (remaining <= TimeSpan.Zero)
            return;
        var exponentialMilliseconds = Math.Min(100, 2 << Math.Min(retry, 5));
        var delay = TimeSpan.FromMilliseconds(
            Math.Min(
                remaining.TotalMilliseconds,
                exponentialMilliseconds + Random.Shared.Next(0, exponentialMilliseconds + 1)
            )
        );
    ...
    )
    {
        ...
        try
        {
            ...
            when (IsAggregateDurableRecord<T>()
                && error
                    is JsonException
                        or InvalidDataException
                        or NotSupportedException
    ...
    private static bool IsAggregateDurableRecord<T>() =>
        typeof(T) == typeof(AggregateRecord) || typeof(T) == typeof(AggregateParticipantRecord);

    private async ValueTask<StoredRecord<T>?> ReadRecordForReconciliationAsync<T>(ZLinkStoreKey key)
    {
        using var deadline = new CancellationTokenSource(AmbiguousReconciliationTimeout);
    ...
    private async ValueTask<AuthorityOwnerGenerationCounterState> ReadAuthorityOwnerGenerationCounterAsync(
        CancellationToken cancellationToken
    )
    {
        ...
        return read switch
        {
            ...
            ZLinkStoreReadResult.Found found => new AuthorityOwnerGenerationCounterState(
    ...
    // mechanism).
    private async ValueTask<ObjectGenerationCounterState> ReadObjectGenerationCounterAsync(
        CancellationToken cancellationToken
    )
    {
        ...
        return read switch
        {
            ...
            ZLinkStoreReadResult.Found found => new ObjectGenerationCounterState(
                DecodeGenerationCounter(found.Value.Bytes),
                new ZLinkStoreCondition.Version(key, found.Value.Version)
            ),
            _ => throw new InvalidOperationException(
    ...
    private static bool MatchesReservation(
        ...
        && current.Snapshot.AuthorityOwnerGeneration == reservation.AuthorityOwnerGeneration
        ...
        && current.Snapshot.OwnerLeaseGeneration == reservation.TargetOwner.LeaseGeneration
        && current.Snapshot.Allocation.State == ZLinkPlacementAllocationState.Reserved;

    private static ZLinkAuthorityCompareExchangeResult Conflict(StoredAuthority? current) =>
    ...
    )
    {
        ...
        var limit = descriptor
            .ObjectCapabilities.SingleOrDefault(capability =>
                capability.ObjectKind == spotType.ObjectKind
        ...
        return requested.Spots == spotType.Count
            && (limit == 0 || count.Active + count.Pending + spotType.Count <= limit);
    }
    ...
    private static void ApplyCapacity(
        ...
        int pendingDelta = 0,
        int activeDelta = 0
    ) => ApplyCapacityVector(record, allocation.Capacity, pendingDelta, activeDelta);
    ...
    {
        record.ActorsPending = checked(record.ActorsPending + vector.Actors * pendingDelta);
        record.ActorsActive = checked(record.ActorsActive + vector.Actors * activeDelta);
    ...
    {
        if (
            record.ActorsActive < 0
            || record.ActorsPending < 0
            || record.SpotsActive < 0
            || record.SpotsPending < 0
            || record.SpotTypes.Values.Any(value => value.Active < 0 || value.Pending < 0)
        )
            throw new InvalidDataException("The Location Store capacity record is inconsistent.");
    }
    ...
    private static void ValidateAuthorityMutation(ZLinkAuthorityMutation mutation)
    {
        ...
        if (
            ...
            || preserve
                && (
                    put.TargetOwner is not null
                    || put.TargetAllocation is not null
                    || put.TargetAuthorityOwnerGeneration != 0
                )
            || changesOwner
                && (
                    put.TargetOwner is null
                    || put.TargetAllocation
                        is not {
                            State: ZLinkPlacementAllocationState.Active,
    ...
    private static void ValidateReservation(ZLinkObjectReservationRequest request)
    {
    ...
    private static void ValidateAggregate(ZLinkAggregatePrepareRequest request)
    {
        ...
        if (
            ...
            || request.TargetOwner.LeaseGeneration <= 0
            || request
                .Participants.Select(value => value.Key.Value)
                .Distinct(StringComparer.Ordinal)
                .Count() != request.Participants.Count
        )
            throw new ArgumentOutOfRangeException(nameof(request));
        if (
    ...
    private static void ValidateTerminal(ZLinkCreationTerminalPublication publication)
    {
    ...
    private static byte[] Sha256(ReadOnlyMemory<byte> payload) => SHA256.HashData(payload.Span);
    ...
    )
    {
        ...
        writer.WriteStartObject();
        foreach (var pair in spotTypes.OrderBy(static pair => pair.Key, StringComparer.Ordinal))
        {
    ...
    private static CapacityRecord DecodeCapacity(ReadOnlyMemory<byte> bytes)
    {
        ...
        var spotTypes = new Dictionary<string, CapacityCount>(StringComparer.Ordinal);
        ...
        foreach (var pair in pending.SpotTypes)
            spotTypes[pair.Key] = spotTypes.GetValueOrDefault(pair.Key) with
            {
                Pending = pair.Value,
            };
    ...
    private static DecodedCapacityUsage DecodeCapacityUsage(JsonElement root, string name)
    {
        ...
        if (
            !usage.TryGetProperty("spotTypes", out var types)
            || types.ValueKind != JsonValueKind.Object
        )
            throw new InvalidDataException(
                "The Location Store capacity record has invalid " + $"{name}.spotTypes."
            );
        ...
        return new DecodedCapacityUsage(
            DecodeCapacityCount(RequireCapacityProperty(usage, "actors", name), $"{name}.actors"),
            DecodeCapacityCount(RequireCapacityProperty(usage, "spots", name), $"{name}.spots"),
            decodedTypes
        );
    }

    private static JsonElement RequireCapacityProperty(JsonElement usage, string field, string name)
    {
    ...
    private static AuthorityMeta DecodeAuthorityMeta(ReadOnlyMemory<byte> bytes)
    {
        var meta = Decode<AuthorityMeta>(bytes);
        if (meta.RecordVersion != 1)
            throw new InvalidDataException(
                "The Location Store authority record has an unrecognized" + " recordVersion."
            );
        return meta;
    }
    ...
    // counter.
    private static ZLinkStoreKey ObjectGenerationCounterKey() => Key($"{Prefix}object-counter");
    ...
    private static string AuthorityMetaPrefix(string prefix) =>
        prefix switch
        {
            ...
            _ => throw new ArgumentException(
                $"unsupported authority scan prefix '{prefix}'",
                nameof(prefix)
            ),
        };
    ...
    private static ZLinkAuthorityKey DecodeAuthorityKey(ZLinkStoreKey key)
    {
        ...
        if (!preimage.StartsWith(AuthorityCanonicalPreimagePrefix, StringComparison.Ordinal))
        {
            throw new InvalidDataException(
                $"Authority scan returned a non-authority logical key " + $"'{preimage}'."
            );
        }
        ...
        if (separator < 0)
        {
            throw new InvalidDataException("Authority logical key preimage is malformed.");
        }
        ...
        return kind switch
        {
            ...
            _ => throw new InvalidDataException($"Unrecognized authority key kind '{kind}'."),
        };
    }

    internal static ZLinkStoreKey TerminalKey(ZLinkCreationOperationId operation) =>
        OpaqueRecordKey(
            "creation-terminal",
    ...
    private static string CreationOperationIdHex(ZLinkCreationOperationId operation) =>
    ...
    private static ZLinkStoreKey AggregateKey(ZLinkAggregateFence fence) =>
        Key($"{AggregatePrefix}{fence.AggregateId:N}:" + fence.AggregateGeneration);
    ...
    private static ZLinkStoreKey AggregateParticipantMetaKey(
        ...
        Key(
            $"{AggregatePrefix}{fence.AggregateId:N}:"
    ...
    private static ZLinkStoreKey CapacityKey(ZLinkMeshNodeDescriptorKey descriptor) =>
        Key(
            $"{Prefix}capacity:{EncodeUriComponent(descriptor.MeshName)}"
    ...
    // <stableType>".
    private static string CapacityTypeKey(ZLinkPlacementObjectKind kind, string stableType) =>
        $"{CapacityObjectKindSegment(kind)}\0{stableType}";

    private static string CapacityObjectKindSegment(ZLinkPlacementObjectKind kind) =>
        kind switch
        {
    ...
    private sealed record AuthorityMeta(
        ...
        [property: JsonConverter(
            typeof(Messaging.ZLinkJsonSerializerOptions.FrameworkUnsigned64JsonConverter)
        )]
        ...
        [property: JsonPropertyName("allocation")] ZLinkPlacementAllocation Allocation,
    ...
    private ref struct AggregateInventoryReader
    {
        ...
        {
            if (
                ReadUInt32() != InventoryCodecMagic
                || ReadByte() != InventoryCodecVersion
                || ReadByte() != expectedRecordKind
            )
                throw new InvalidDataException("The aggregate inventory codec header is invalid.");
        }
        ...
        internal AggregateInventoryPageReference ReadReference() =>
            new(ReadInt32(), ReadInt32(), ReadInt32(), ReadInt32(), ReadHash());
        ...
        private string ReadString()
        {
            ...
            if (length < 1 || length > remaining.Length)
                throw new InvalidDataException("An aggregate inventory string length is invalid.");
            return InventoryUtf8.GetString(ReadBytes(length));
        }

        private byte[] ReadHash() => ReadBytes(SHA256.HashSizeInBytes).ToArray();

        private ReadOnlySpan<byte> ReadBytes(int count)
        {
            if (count < 0 || count > remaining.Length)
                throw new InvalidDataException("The aggregate inventory record is truncated.");
    ...
    private sealed record ObjectGenerationCounterState(ulong Value, ZLinkStoreCondition Condition);
