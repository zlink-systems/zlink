...
internal static class ZLinkRelocationEnvelopeCodec
{
    ...
    )
    {
        if (
            ...
            || !ServiceWireConstants.ValidTerminalFailure(terminalResult, errorCode)
    ...
    internal static void EncodeTo(Stream stream, ZLinkRelocationEnvelope envelope)
    {
    ...
    )
    {
        ...
        if (!stream.CanSeek)
        {
            ...
            if (memory.Length < sizeof(uint))
                throw new InvalidDataException("The relocation logical stream is truncated.");
            if (BinaryPrimitives.ReadUInt32LittleEndian(memory.Span) != Magic)
    ...
    private static ReadOnlyMemory<byte> ReadNonSeekableOwnedMemory(Stream stream)
    {
        ...
        try
        {
            ...
            var owned = GC.AllocateUninitializedArray<byte>(checked((int)total));
            var offset = 0;
            foreach (var segment in segments)
            {
                segment.Buffer.AsSpan(0, segment.Count).CopyTo(owned.AsSpan(offset));
    ...
    private static ZLinkRelocationEnvelope DecodeVersioned(ReadOnlyMemory<byte> encoded)
    {
        ...
        for (var participantIndex = 0; participantIndex < participantCount; participantIndex++)
        {
            ...
            var jobs = new ZLinkRelocationQueuedJob[
                reader.ReadCountWithinRemaining(MinEncodedAcceptedJobBytes, "accepted job")
            ];
            ...
            var timers = new ZLinkRelocationLogicalTimer[
                reader.ReadCount(MaxTimerItems, "logical timer")
            ];
            ...
            for (var timerIndex = 0; timerIndex < timers.Length; timerIndex++)
            {
                ...
                if (!timerIds.Add(timerId))
                    throw new InvalidDataException($"Duplicate logical timer '{timerId}'.");
                var due = reader.ReadInt64();
                var period = reader.ReadInt64();
                if (period < 0)
                    throw new InvalidDataException("A logical timer period cannot be negative.");
    ...
    private static ZLinkRelocationEnvelope DecodeVersioned(Stream input)
    {
        ...
        for (var participantIndex = 0; participantIndex < participantCount; participantIndex++)
        {
            ...
            var jobs = new ZLinkRelocationQueuedJob[
                ReadCountWithinRemaining(reader, input, MinEncodedAcceptedJobBytes, "accepted job")
            ];
            ulong previousSequence = 0;
            for (var jobIndex = 0; jobIndex < jobs.Length; jobIndex++)
            {
        ...
        if (input.Position != input.Length)
            throw new InvalidDataException("The relocation root contains trailing bytes.");
    ...
    {
        var participants = new ZLinkRelocationParticipantEnvelope[envelope.Participants.Count];
        for (var index = 0; index < participants.Length; index++)
        {
            var participant = envelope.Participants[index];
            try
            {
                _ = ZLinkCanonicalParticipantRecoveryCodec.Decode(participant.RecoveryPayload.Span);
            }
        ...
        envelope.AggregateId.TryWriteBytes(id, bigEndian: true, out _);
        return envelope with
        {
            Participants = participants,
            CanonicalRelocationHigh = BinaryPrimitives.ReadUInt64BigEndian(id[..8]),
            CanonicalRelocationLow = BinaryPrimitives.ReadUInt64BigEndian(id[8..]),
        };
    }
    ...
    )
    {
        ...
        try
        {
            generated = ServiceWirePilotCodec.DecodeRelocationEnvelopeV1(encoded.ToArray());
        }
        ...
        var canonical = ServiceWirePilotCodec.EncodeRelocationEnvelopeV1(generated);
        var (objectKind, objectId, objectGeneration, ownerGeneration) = ProjectObject(
            generated.Object
        );
        var authorityKey = objectKind switch
        {
            ZLinkPlacementObjectKind.Actor => ZLinkAuthorityKeyCodec.EncodeActor(objectId),
        ...
        var participantIds = generated
            .ApplicationStates.Select(static state => state.ParticipantId)
            .ToArray();
        ...
        foreach (var work in generated.SavedWork)
        {
            if (!acceptedOrders.Add((work.ParticipantId, work.Order)))
                throw new InvalidDataException("The relocation saved-work order is invalid.");
            var reader = new CanonicalReader(work.FrozenRecord);
            var projection = ReadCanonicalFrozenRecordProjection(ref reader, out var operation);
            reader.RequireEnd("relocation frozen record");
            if (operation is { } exactOperation && !operations.Add(exactOperation))
                throw new InvalidDataException(
                    "The relocation saved work contains a duplicate operation identity."
                );
            jobs[work.ParticipantId]
                .Add(
                    new ZLinkRelocationQueuedJob(work.Order, work.FrozenRecord)
                    {
        ...
        {
            if (
                timer.PeriodMilliseconds > long.MaxValue
                || timer.NextScheduledAtUnixMilliseconds > long.MaxValue
            )
                throw new InvalidDataException("The relocation timer registration is invalid.");
            timers[timer.ParticipantId]
                .Add(
        ...
        {
            if (
                !acceptedOrders.Add((pending.ParticipantId, pending.Order))
            ...
            var timer = timers[pending.ParticipantId]
                .Single(candidate =>
                    string.Equals(candidate.TimerId, pending.TimerName, StringComparison.Ordinal)
                );
        ...
        var participants = generated
            .ApplicationStates.Select(
                (state, index) =>
        ...
        var result = new ZLinkRelocationEnvelope(
            ...
            generated.ApplicationVersion == 0 ? 1UL : checked((ulong)generated.ApplicationVersion),
            digest,
            participants
        )
        {
    ...
    {
        var generated = ServiceWirePilotCodec.DecodeRelocationEnvelopeV1(encoded.ToArray());
    ...
    ) ProjectObject(ServiceWirePilotCodec.RelocationObjectIdentity value) =>
        value switch
        {
    ...
    private static void ReadCanonicalFrozenRecord(ref CanonicalReader reader)
    {
        ...
        if (source.ReadUInt64() == 0)
            throw new InvalidDataException(
                "The relocation saved work source generation is invalid."
            );
        _ = source.ReadText8();
        if (source.ReadUInt64() == 0)
        ...
        ReadCanonicalFrozenRecordBody(ref reader, recordKind, operationKind, operationNonzero);
    }

    private static ZLinkCanonicalAcceptedRequest? ReadCanonicalFrozenRecordProjection(
    ...
    )
    {
        switch (recordKind)
        {
            ...
            case 11:
                ...
                if (
                    !IsCanonicalTerminalResult(completionTerminal)
    ...
    private static void ReadCanonicalActorFence(ref CanonicalReader reader)
    {
        ReadCanonicalActorRef(ref reader);
        _ = reader.ReadText8();
        if (reader.ReadUInt64() == 0 || reader.ReadUInt64() == 0 || reader.ReadUInt64() == 0)
    ...
    private static void ReadCanonicalRelocationControl(ref CanonicalReader reader)
    {
        ...
        if (
            !IsCanonicalTerminalResult(controlTerminal)
            || !ServiceWireConstants.ValidTerminalFailure(controlTerminal, controlFailure)
        )
    ...
    private static void ReadCanonicalInstanceRoute(ref CanonicalReader reader)
    {
        ...
        if (kind == 1)
        {
            if (route.ReadUInt64() == 0)
                throw new InvalidDataException(
                    "The Instance relocation object generation is invalid."
                );
            _ = route.ReadText8();
        ...
        else
        {
            ...
            _ = route.ReadText8();
            if (route.ReadUInt64() == 0)
                throw new InvalidDataException("The Instance relocation deadline is invalid.");
        }
    ...
    private static void ReadCanonicalApplicationPayload(ref CanonicalReader reader)
    {
        if (reader.ReadByte() != 1)
            throw new InvalidDataException(
                "The relocation application payload version is invalid."
            );
    ...
    private static ZLinkCanonicalApplicationPayload ReadCanonicalApplicationPayloadProjection(
        ref CanonicalReader reader
    )
    {
        if (reader.ReadByte() != 1)
    ...
    private static ZLinkMessageMetadata ReadCanonicalMetadataProjection(ref CanonicalReader reader)
    {
        if (reader.ReadByte() != 1)
            throw new InvalidDataException("The relocation metadata version is invalid.");
        var count = reader.ReadByte();
    ...
    )
    {
        ...
        internal void AppendPendingTick(ZLinkCanonicalPendingTimerTick pendingTick)
        {
            _pendingTick = pendingTick;
        }

        internal ZLinkRelocationLogicalTimer ToRelocationTimer() =>
            new(TimerId, dueUnixTimeMilliseconds, periodMilliseconds, ReadOnlyMemory<byte>.Empty)
            {
    ...
    private ref struct VersionedMemoryReader(ReadOnlyMemory<byte> source)
    {
        ...
        internal ushort ReadUInt16() =>
            BinaryPrimitives.ReadUInt16LittleEndian(ReadBytes(sizeof(ushort)).Span);

        internal uint ReadUInt32() =>
            BinaryPrimitives.ReadUInt32LittleEndian(ReadBytes(sizeof(uint)).Span);

        internal ulong ReadUInt64() =>
            BinaryPrimitives.ReadUInt64LittleEndian(ReadBytes(sizeof(ulong)).Span);

        internal long ReadInt64() =>
            BinaryPrimitives.ReadInt64LittleEndian(ReadBytes(sizeof(long)).Span);

        internal int ReadInt32() =>
            BinaryPrimitives.ReadInt32LittleEndian(ReadBytes(sizeof(int)).Span);
        ...
        internal ReadOnlyMemory<byte> ReadByteField(int maximum)
        {
            ...
            if (size < 0 || size > maximum)
                throw new InvalidDataException("A relocation byte field exceeds its bound.");
        ...
        internal int ReadCount(int maximum, string name)
        {
            ...
            if (count < 0 || count > maximum)
                throw new InvalidDataException($"The relocation {name} count exceeds its bound.");
            return count;
        }

        internal int ReadCountWithinRemaining(int minimumItemBytes, string name)
        {
    ...
    private ref struct CanonicalReader(ReadOnlyMemory<byte> source)
    {
        ...
        internal ushort ReadUInt16() =>
            BinaryPrimitives.ReadUInt16BigEndian(ReadBytes(sizeof(ushort)).Span);

        internal uint ReadUInt32() =>
            BinaryPrimitives.ReadUInt32BigEndian(ReadBytes(sizeof(uint)).Span);

        internal ulong ReadUInt64() =>
            BinaryPrimitives.ReadUInt64BigEndian(ReadBytes(sizeof(ulong)).Span);
    ...
    private static void ValidateEnvelope(ZLinkRelocationEnvelope envelope)
    {
        ...
        foreach (var participant in envelope.Participants)
        {
            ...
            if (
                participant.ObjectGeneration is 0 or > long.MaxValue
            ...
            foreach (var job in participant.AcceptedJobs)
            {
                if (job.AcceptedSequence == 0 || job.AcceptedSequence <= previousSequence)
                    throw new ArgumentException(
                        "Accepted job sequences must be strictly increasing.",
                        nameof(envelope)
                    );
...
internal static class ZLinkCrc32C
{
    ...
    internal static void Append(ref uint crc, ReadOnlySpan<byte> data)
    {
