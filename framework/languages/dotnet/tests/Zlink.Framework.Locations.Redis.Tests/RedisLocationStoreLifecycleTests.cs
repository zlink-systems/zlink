...
public sealed class RedisLocationStoreLifecycleTests
{
    ...
    {
        Assert.Equal(TimeSpan.FromSeconds(5), new ZLinkRedisLocationOptions().OperationTimeout);
        Assert.Equal(TimeSpan.FromSeconds(5), new ZLinkRedisRelocationOptions().OperationTimeout);
    ...
    public async Task Operation_timeout_bounds_the_redis_command()
    {
        ...
        await Assert.ThrowsAsync<TimeoutException>(() =>
            store.ReadAsync(new ZLinkStoreKey("timeout:read")).AsTask()
        );
    ...
    {
        var locationDatabase = DispatchProxy.Create<IDatabase, CapturingRedisDatabaseProxy>();
        var locationCommand = (CapturingRedisDatabaseProxy)(object)locationDatabase;
        ...
        var relocationDatabase = DispatchProxy.Create<IDatabase, CapturingRedisDatabaseProxy>();
        var relocationCommand = (CapturingRedisDatabaseProxy)(object)relocationDatabase;
        ...
        Assert.IsType<ZLinkBlobReadResult.Missing>(
            await relocationStore.ReadAsync(new ZLinkBlobReference("snapshot-reference"))
        );
        ...
        Assert.Equal(
            "zlink:relocation-original:{zlink-relocation-v1}:blob:" + "snapshot-reference",
    ...
    {
        var database = DispatchProxy.Create<IDatabase, CapturingWriteRedisDatabaseProxy>();
        ...
        await using var store = new ZLinkRedisLocationStore(
            ...
            _ =>
                ValueTask.FromResult<IZLinkRedisConnection>(
        ...
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await store.WriteAsync(
                new ZLinkStoreWriteRequest(
                    [],
                    [new ZLinkStoreMutation.Put(key, new byte[] { 1 }, TimeSpan.FromTicks(1))]
                )
            )
        );
    ...
    {
        var database = DispatchProxy.Create<IDatabase, CapturingRelocationPutRedisDatabaseProxy>();
        var command = (CapturingRelocationPutRedisDatabaseProxy)(object)database;
    ...
    public async Task Normalized_connection_pool_releases_the_multiplexer_last()
    {
        ...
        var key = $"{Guid.NewGuid():N}:" + ZLinkRedisConnectionPool.CreateKey(configuration);
        var connectCount = 0;
        ...
        var first = await ZLinkRedisConnectionPool.RentAsync(key, configuration, Connect);
        var second = await ZLinkRedisConnectionPool.RentAsync(key, configuration, Connect);
    ...
    public async Task Failed_pooled_connect_does_not_poison_the_next_rent()
    {
        ...
        await Assert.ThrowsAsync<IOException>(() =>
            ZLinkRedisConnectionPool.RentAsync(key, configuration, Connect).AsTask()
        );

        var lease = await ZLinkRedisConnectionPool.RentAsync(key, configuration, Connect);
    ...
    public async Task Caller_cancellation_completes_its_waiter_before_timeout()
    {
        ...
        var read = store
            .ReadAsync(new ZLinkStoreKey("cancelled:read"), cancellation.Token)
            .AsTask();
    ...
    {
        var database = DispatchProxy.Create<IDatabase, AmbiguousRelocationPutRedisDatabaseProxy>();
        var command = (AmbiguousRelocationPutRedisDatabaseProxy)(object)database;
        var connection = new TestRedisConnection(database: database);
        ...
        await Assert.ThrowsAsync<TimeoutException>(() =>
            store.PutAsync(reference, payload, TimeSpan.FromMinutes(1)).AsTask()
        );

        command.CompleteFirstPutAsStored();
        await command.FirstPutCompleted.Task.WaitAsync(TimeSpan.FromSeconds(5));
        Assert.IsType<ZLinkBlobPutResult.AlreadyStored>(
            await store.PutAsync(reference, payload, TimeSpan.FromMinutes(1))
        );
    ...
    public async Task Dispose_Waits_For_First_Connect_And_Disposes_The_Published_Connection_Once()
    {
        ...
        var read = store.ReadAsync(new ZLinkStoreKey("lifecycle:read")).AsTask();
        ...
        Assert.IsType<ZLinkStoreReadResult.Missing>(await read.WaitAsync(TimeSpan.FromSeconds(5)));
        await dispose.WaitAsync(TimeSpan.FromSeconds(5));

        Assert.Equal(1, connection.DisposeCount);
        await Assert.ThrowsAsync<ObjectDisposedException>(() =>
            store.ReadAsync(new ZLinkStoreKey("lifecycle:after-dispose")).AsTask()
        );
    ...
    public async Task Dispose_Stops_New_Admission_And_Waits_For_An_Admitted_Command()
    {
        ...
        var admitted = store.ReadAsync(new ZLinkStoreKey("lifecycle:blocked")).AsTask();
        await command.CommandStarted.Task.WaitAsync(TimeSpan.FromSeconds(5));
        ...
        await Assert.ThrowsAsync<ObjectDisposedException>(() =>
            store.ReadAsync(new ZLinkStoreKey("lifecycle:rejected")).AsTask()
        );
    ...
    private sealed class TestRedisConnection(Task? disposeBlock = null, IDatabase? database = null)
        : IZLinkRedisConnection
    {
        private readonly IDatabase _database =
            database ?? DispatchProxy.Create<IDatabase, RedisDatabaseProxy>();
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            if (disposeBlock is not null)
                await disposeBlock.ConfigureAwait(false);
        }
    }
    ...
    private class RedisDatabaseProxy : DispatchProxy
    {
        ...
        {
            if (
                targetMethod?.Name == nameof(IDatabase.ScriptEvaluateAsync)
    ...
    {
        private readonly TaskCompletionSource<RedisResult> _firstPut = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
    ...
    private static RedisResult MissingReadResult() =>
        RedisResult.Create([
            ...
            RedisResult.Create((RedisValue)DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()),
        ]);
