...
public sealed class HandlerExposureTests : RegistrationValidationSupport
{
    ...
    public void MeshNode_RecordsExplicitDirectRouteHandlers()
    {
        ...
        services.AddZLinkFramework(options =>
        {
            ...
            mesh.AddRouteSendHandler<TestRouteSendHandler, TestMessage>("route-send")
                .AddRouteRequestHandler<ExplicitRouteRequestHandler, TestRequest, TestReply>(
                    "route-request"
                );
        });
    ...
    public void MeshMembership_RejectsDuplicateExplicitRequestPacket()
    {
        ...
        var exception = Assert.Throws<ZLinkConfigurationException>(() =>
            services.AddZLinkFramework(options =>
            {
                ...
                mesh.Channel("profile")
                    .Server()
                    .AddRequestHandler<TestRequestHandler, TestRequest, TestReply>("request")
                    .AddRequestHandler<AlternateTestRequestHandler, TestRequest, TestReply>(
                        "request"
                    );
            })
        );
        ...
        Assert.Contains(
            "Duplicate request handler 'profile:profile:request'",
            exception.Message,
    ...
    public void MeshNode_RejectsDuplicateExplicitRouteRequestPacket()
    {
        ...
        var exception = Assert.Throws<ZLinkConfigurationException>(() =>
            services.AddZLinkFramework(options =>
            {
                var mesh = options
                    ...
                    .SetRoutingId(RoutingId.From("profile"));
                mesh.Channel("profile").Server();
                mesh.AddRouteRequestHandler<ExplicitRouteRequestHandler, TestRequest, TestReply>(
                        "route-request"
                    )
                    .AddRouteRequestHandler<AlternateRouteRequestHandler, TestRequest, TestReply>(
                        "route-request"
                    );
            })
        );
        ...
        Assert.Contains(
            "Duplicate routed request handler 'profile:route-request'",
            exception.Message,
            StringComparison.Ordinal
        );
    }
    ...
    private sealed class ExplicitRouteRequestHandler
        : IZLinkRouteRequestHandler<TestRequest, TestReply>
    {
        public ValueTask<TestReply> HandleAsync(
            TestRequest request,
            ZLinkRouteMessageContext context,
    ...
    private sealed class AlternateRouteRequestHandler
        : IZLinkRouteRequestHandler<TestRequest, TestReply>
    {
        public ValueTask<TestReply> HandleAsync(
            TestRequest request,
