...
public sealed class WeightContractTests
{
    ...
    public void StartupBuildersAcceptTheSignedWeightRange(int weight)
    {
        ...
        options
            ...
            .Server()
            .SetWeight(weight);
        options.AddClientServerChannel("rpc").Server().SetWeight(weight);

        Assert.Equal(weight, registration.SpotNodes["mesh"].PlacementWeight);
        Assert.Equal(weight, registration.SpotNodes["mesh"].ChannelMemberships.Single().Weight);
        Assert.Equal(weight, registration.Channels["rpc"].Server!.SocketConfig.Weight);
    }
    ...
    public void StartupBuildersRejectWeightsOutsideTheSignedRange(int weight)
    {
        ...
        Assert.Throws<ZLinkConfigurationException>(() => mesh.SetPlacementWeight(weight));
    ...
    public void WeightedSelectionUsesA64BitSumAndExactRelativeWeights()
    {
        ...
        Assert.Equal(
            2_147_490_000L,
            ZLinkWeightedSelector.Sum(overflowInt32, static weight => weight)
        );

        WeightedCandidate[] candidates = [new("one", 100), new("three", 300)];
    ...
    {
        WeightedCandidate[] candidates = [new("existing", 100), new("new", 100)];
        var currents = new Dictionary<string, long>(StringComparer.Ordinal);

        var selected = Enumerable
            .Range(0, 4)
            .Select(_ =>
                ZLinkWeightedSelector
                    .Select(
                        candidates,
                        static candidate => candidate.Weight,
                        static candidate => candidate.Name,
    ...
    {
        WeightedCandidate[] candidates = [new("z", 100), new("a", 100)];
        var currents = new Dictionary<string, long>(StringComparer.Ordinal);
    ...
    {
        WeightedCandidate[] candidates = [new("a", 100), new("b", 300)];
        var plan = new ZLinkWeightedSelectionPlan<WeightedCandidate, string>(
            candidates,
            static candidate => candidate.Weight,
            static candidate => candidate.Name,
        ...
        var selected = Enumerable.Range(0, 4).Select(_ => plan.Select()!.Name).ToArray();
    ...
    {
        WeightedCandidate[] initialCandidates = [new("a", 100), new("b", 100)];
        var initial = new ZLinkWeightedSelectionPlan<WeightedCandidate, string>(
            initialCandidates,
            static candidate => candidate.Weight,
            static candidate => candidate.Name,
            retainedCurrents: null,
        ...
        WeightedCandidate[] replacementCandidates = [new("b", 100), new("c", 100)];
        var replacement = new ZLinkWeightedSelectionPlan<WeightedCandidate, string>(
    ...
    public void MeshChannelSelectionKeepsDeclaredChannelsSeparateFromEligibleTargets()
    {
        ...
        var target = new ZLinkMeshChannelTarget(RoutingId.From("node-a"), weight: 100);

        selection.Rebuild(["events"], _ => [target]);
        ...
        selection.Rebuild(["events"], _ => Array.Empty<ZLinkMeshChannelTarget>());
    ...
    public void ObjectPlacementFiltersCapacityAndZeroWeightBeforeSelection()
    {
        var eligible = Descriptor(
            ...
            spotType: new ZLinkSpotTypeCapacity(ZLinkPlacementObjectKind.UserSpot, "room", 9, 0, 10)
        );
        Assert.True(ZLinkActorManagerService.IsEligibleCandidate(eligible, "player"));
        Assert.True(ZLinkSpotRuntimeManager.IsEligibleCandidate(eligible, "room"));

        var zeroWeight = eligible with { PlacementWeight = 0 };
        Assert.False(ZLinkActorManagerService.IsEligibleCandidate(zeroWeight, "player"));
        Assert.False(ZLinkSpotRuntimeManager.IsEligibleCandidate(zeroWeight, "room"));

        var actorFull = eligible with
        {
            Capacity = eligible.Capacity with { Actors = new ZLinkPopulationCapacity(9, 1, 10) },
        };
        Assert.False(ZLinkActorManagerService.IsEligibleCandidate(actorFull, "player"));
        ...
        var spotTypeFull = eligible with
        {
            Capacity = eligible.Capacity with
            {
                SpotTypes =
                [
                    new ZLinkSpotTypeCapacity(ZLinkPlacementObjectKind.UserSpot, "room", 9, 1, 10),
                ],
            },
        };
        Assert.False(ZLinkSpotRuntimeManager.IsEligibleCandidate(spotTypeFull, "room"));
    }
    ...
    public void Placement_candidates_require_an_admitted_peer_for_the_same_generation()
    {
        var local = Descriptor(
            ...
            spotType: new ZLinkSpotTypeCapacity(ZLinkPlacementObjectKind.UserSpot, "room", 0, 0, 10)
        ) with
        {
        ...
        var stale = local with { Rid = RoutingId.From("stale-target"), LifecycleGeneration = 9 };
        var peers = new[]
        {
            new MeshNodePeer(
                1,
        ...
        var filtered = ZLinkMeshNodeTargetAvailability
            .FilterAdmitted(local.Rid, new[] { local, current, stale }, peers)
            .Select(static candidate => candidate.Rid)
            .ToArray();

        Assert.Equal(new[] { local.Rid, current.Rid }, filtered);
    }
    ...
    public void Placement_candidates_exclude_a_transport_rejected_peer_until_its_epoch_changes()
    {
        ...
        var remote = local with { Rid = RoutingId.From("remote-target") };
        ...
        var filtered = ZLinkMeshNodeTargetAvailability
            .FilterAdmitted(local.Rid, new[] { local, remote }, [admitted], unavailable)
            .Select(static candidate => candidate.Rid)
            .ToArray();
        ...
        var reselected = ZLinkMeshNodeTargetAvailability
            .FilterAdmitted(local.Rid, new[] { local, remote }, [refreshed], unavailable)
            .Select(static candidate => candidate.Rid)
            .ToArray();
