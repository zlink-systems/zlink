...
public sealed class MeshOutboundPayloadOwnershipTests
{
    ...
    public async Task Public_route_calls_release_original_native_payloads(bool channel, bool request)
    {
        ...
        var targetNode = target.Services.GetRequiredService<ZLinkFrameworkRuntime>().GetMeshNodeRuntime(Mesh);
        ...
        try
        {
            ...
            {
                var payload = new BytesValue { Value = ByteString.CopyFrom(new byte[request ? 64 : 4096]) };
                if (request)
                {
                    var call = channel ? client.RequestToChannel(Channel, payload)
                ...
                {
                    var call = channel ? client.SendToChannel(Channel, payload)
    ...
    [InlineData(true, false)]
    [InlineData(true, true)]
    public async Task Cancellation_after_encode_releases_original_native_payload(bool channel, bool request)
    {
    ...
    public async Task Actor_send_without_peer_releases_original_native_payload()
    {
        ...
        try
        {
            var node = source.Services.GetRequiredService<ZLinkFrameworkRuntime>().GetMeshNodeRuntime(Mesh);
            ...
            var result = await node.SendToActorAsync(
                new Zlink.Framework.Runtime.Backend.Contracts.ZLinkBackendActorRef(
                    RoutingId.From("missing-peer"), "actor", 1), [payload], CancellationToken.None);
    ...
    private static IHost BuildHost(TrackingProtobufCodec? tracking = null,
        RoutingId peer = default, string? endpoint = null, string? listenEndpoint = null)
    {
        ...
        builder.Services.AddZLinkFramework(options =>
        {
            ...
            var mesh = options.AddRouteMesh(Mesh).Listen(listenEndpoint ?? ReserveEndpoint())
            ...
            if (endpoint is not null) mesh.PeerConnections.Connect(peer, endpoint);
        });
    ...
    private static async Task WaitForPeerAsync(IHost host, RoutingId peer)
    {
        ...
        {
            if (runtime.GetStatus(Mesh).Peers.Any(candidate =>
                    candidate.NodeRid == peer && candidate.State == ZLinkPeerState.Ready)) return;
            await Task.Delay(10);
        }
        Assert.Fail("The ownership test peer was not admitted.");
    }

    public sealed class ProbeHandler : IZLinkRouteSendHandler<BytesValue>,
        IZLinkRouteRequestHandler<BytesValue, BytesValue>, IZLinkSendHandler<BytesValue>,
        IZLinkRequestHandler<BytesValue, BytesValue>
    {
        ValueTask IZLinkRouteSendHandler<BytesValue>.HandleAsync(BytesValue message,
            ZLinkRouteMessageContext context, CancellationToken cancellationToken) => ValueTask.CompletedTask;

        ValueTask<BytesValue> IZLinkRouteRequestHandler<BytesValue, BytesValue>.HandleAsync(BytesValue request,
            ZLinkRouteMessageContext context, CancellationToken cancellationToken) =>
            ValueTask.FromResult(new BytesValue { Value = ByteString.CopyFrom(new byte[4096]) });

        public ValueTask HandleAsync(BytesValue message, IZLinkMessageContext context,
            CancellationToken cancellationToken) => ValueTask.CompletedTask;

        ValueTask<BytesValue> IZLinkRequestHandler<BytesValue, BytesValue>.HandleAsync(BytesValue request,
            IZLinkMessageContext context, CancellationToken cancellationToken) =>
            ValueTask.FromResult(new BytesValue { Value = ByteString.CopyFrom(new byte[4096]) });
    }

    private sealed class TrackingProtobufCodec(Action? afterEncode = null) : IZLinkCodecExtension,
        IZLinkMessageSerializer, IZLinkMessagePartSerializer, IZLinkMessageSpanDeserializer, IDisposable
    {
        ...
        public void Register(IZLinkCodecRegistrar codecs) =>
            codecs.AddSerializer("application/x-protobuf", this, type => typeof(IMessage).IsAssignableFrom(type));
        ...
        public ZLinkEncodedPayload Serialize(object value, Type type) => _serializer.Serialize(value, type);
        public object? Deserialize(ZLinkEncodedPayload payload, Type type) => _serializer.Deserialize(payload, type);
