...
public sealed class DrainCoordinatorTests : RegistrationValidationSupport
{
    ...
    public async Task Shutdown_Seals_And_Waits_Accepted_Work_Then_Closes_Spots_Without_Relocating_Actors()
    {
        ...
        var reason = await executor.ExecuteAsync(TimeSpan.FromSeconds(1), CancellationToken.None);
    ...
    public async Task Relocation_workload_coordinator_moves_shells_then_actors_then_aggregates()
    {
        ...
        var coordinator = new ZLinkRelocationWorkloadCoordinator(
            ...
            {
                events.Add(
                    phase == ZLinkSpotRelocationPhase.PerActorShells ? "shells" : "aggregates"
                );
                return ValueTask.FromResult(
                    new ZLinkSpotDrainResult(
                        true,
                        phase == ZLinkSpotRelocationPhase.PerActorShells ? 1UL : 3UL,
                        null,
        ...
        var result = await coordinator.DrainAsync(
            new ZLinkRelocationWorkloadDrainControl(static () => false, CancellationToken.None)
        );

        Assert.True(result.Completed);
        Assert.Null(result.TerminalReason);
        Assert.Equal<ulong>(6, result.CommittedUnitCount);
        Assert.Equal(ZLinkRelocationCommitKnowledge.Committed, result.CommitKnowledge);
    ...
    public async Task Shutdown_signal_between_phases_starts_no_new_relocation_unit()
    {
        ...
        var coordinator = new ZLinkRelocationWorkloadCoordinator(
            (phase, _) =>
            {
                ...
                stopRequested = true;
                return ValueTask.FromResult(new ZLinkSpotDrainResult(true, 1));
            },
            (_, _) =>
            {
                events.Add("actors");
                return ValueTask.FromResult(new ZLinkActorDrainResult(true, null, 1));
            }
        );

        var result = await coordinator.DrainAsync(
            new ZLinkRelocationWorkloadDrainControl(() => stopRequested, CancellationToken.None)
        );
    ...
    public async Task GenerationExhaustionStopsSpotDrainWithoutPollingRetry()
    {
        ...
        var operations = probe.Operations with
        {
            DrainRelocationWorkloads = _ =>
            {
                ...
                return ValueTask.FromException<ZLinkRelocationWorkloadDrainResult>(
                    new ZLinkAuthorityGenerationExhaustedException("testing drain classification")
                );
            },
        };
        var executor = new ZLinkFrameworkDrainExecutor(
    ...
    public async Task Executor_does_not_restore_serving_for_a_stale_relocation_fence()
    {
        ...
        var operations = probe.Operations with
        {
            ...
            CaptureRelocationFence = static () => new ZLinkRelocationAdmissionFence(2),
    ...
    public async Task Executor_does_not_restore_serving_after_rollback_lease_is_invalidated()
    {
        ...
        var operations = probe.Operations with
        {
            ...
            CaptureRelocationFence = static () => new ZLinkRelocationAdmissionFence(3),
    ...
    public async Task Full_rollback_shutdown_takeover_returns_shutdown_blocked()
    {
        ...
        Assert.Equal(ZLinkDrainExecutionDisposition.Blocked, result.Disposition);
        Assert.Equal(ZLinkFrameworkRelocationReason.ShutdownRequested, result.BlockedReason);
    }
    ...
    public async Task Partial_commit_without_source_terminalization_fails_closed()
    {
        ...
        Assert.Equal(ZLinkDrainExecutionDisposition.ForceStop, result.Disposition);
    ...
    public async Task Stream_session_drain_deadline_is_reported_as_deadline_exceeded()
    {
        ...
        Assert.Equal(ZLinkDrainExecutionDisposition.ForceStop, result.Disposition);
        Assert.Equal(ZLinkDrainForceReason.DeadlineExceeded, result.ForceReason);
    }
    ...
    public async Task Shutdown_request_seals_and_finishes_only_the_current_relocation_unit()
    {
        ...
        var operations = probe.Operations with
        {
            DrainRelocationWorkloads = async control =>
            {
                ...
                await releaseCurrentUnit
                    .Task.WaitAsync(control.CancellationToken)
                    .ConfigureAwait(false);
                return new ZLinkRelocationWorkloadDrainResult(false, null, 1);
            },
        };
        var executor = new ZLinkFrameworkDrainExecutor(
            operations,
        ...
        var blocked = await Assert.ThrowsAsync<ZLinkDrainBlockedException>(() => relocation);
    ...
    public async Task Relocate_Detaches_Workload_Without_Shutting_Down_Infrastructure()
    {
        ...
        Assert.Null(reason);
        Assert.True(
            probe.Events.IndexOf("drain-per-actor-shells") < probe.Events.IndexOf("seal-admission")
        );
        Assert.True(
            probe.Events.IndexOf("seal-admission") < probe.Events.IndexOf("host-relocated")
        );
    ...
    public async Task Drain_Executor_Waits_Only_For_Publication_Terminals(bool holdServingWeight)
    {
        ...
        operations = holdServingWeight
            ? operations with
            {
                QuiesceServingChannels = _ => new(publication.Task),
            }
            : operations with
            {
    ...
    )
    {
        ...
        var operations = servingWeight
            ? probe.Operations with
            {
                QuiesceServingChannels = Publish,
            }
            : probe.Operations with
            {
        ...
        Assert.True(
            ...
            $"Shutdown took {elapsed}; polling interval is {pollingInterval}."
        );
    ...
    public async Task Shutdown_Success_Consumes_First_Terminals_Without_Polling()
    {
        ...
        Assert.Equal(1, probe.Events.Count(static item => item == "cleanup-owner"));
        Assert.True(
            probe.Events.IndexOf("marker") < probe.Events.IndexOf("quiesce-serving-channels")
        );
        Assert.True(
            probe.Events.IndexOf("quiesce-serving-channels") < probe.Events.IndexOf("wait-accepted")
        );
        Assert.True(probe.Events.IndexOf("wait-accepted") < probe.Events.IndexOf("drain-spots"));
        Assert.True(probe.Events.IndexOf("drain-spots") < probe.Events.IndexOf("cleanup-owner"));
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Shutdown_Owner_Cleanup_Failure_Consumes_First_Terminal(
    ...
    )
    {
        ...
        var forced = Assert.IsType<ForceStopped>(
            await coordinator.DrainAsync(TimeSpan.FromSeconds(5))
        );
    ...
    public async Task Fixed_Drain_Waits_For_Spot_Queue_Close_Before_Row_Release()
    {
        ...
        var operation = ZLinkSpotNodeCatalog
            .CloseBeforeReleaseAsync(
                async () =>
                {
        ...
        Assert.Equal(new[] { "close-started", "queue-drained-and-closed", "row-released" }, events);
    }
    ...
    {
        var gate = new ZLinkDrainAdmissionGate();
        Assert.True(gate.TryBeginRelocationFence(static snapshot => snapshot.ActiveCount == 0));
        gate.Seal();
    ...
    public async Task Executor_Reported_Marker_Publish_Failure_Is_The_Exact_ForceStopped_Reason()
    {
        ...
        var forced = Assert.IsType<ForceStopped>(await drain.WaitAsync(TimeSpan.FromSeconds(1)));
        Assert.Equal(ZLinkDrainForceReason.DrainingStatePublishFailed, forced.Reason);
    ...
    public async Task Host_Stop_Uses_The_Same_Thirty_Second_Default_Deadline()
    {
        ...
        Assert.InRange(executorDeadline, TimeSpan.FromSeconds(29), TimeSpan.FromSeconds(30));
    ...
    public async Task Deadline_Validation_Happens_Before_Drain_Starts()
    {
        var executor = new FakeDrainExecutor();
        var admission = new ZLinkDrainAdmissionGate();
        using var coordinator = new ZLinkDrainCoordinator(admission, executor);
    ...
    {
        Assert.True(
            ZLinkActorDrainCoordinator.IsTargetLocalRetriable(
    ...
    public void Spot_relocation_failure_mapping_preserves_precommit_reason_and_commit_boundary()
    {
        ...
        {
            (new OperationCanceledException(), ZLinkFrameworkRelocationReason.DeadlineExceeded),
            (
        ...
        foreach (var (error, expected) in cases)
        {
            Assert.Equal(
                expected,
                ZLinkSpotRetireScheduler.MapFailureReason(error, committed: false)
            );
            Assert.Equal(
                ZLinkFrameworkRelocationReason.RelocationFailed,
                ZLinkSpotRetireScheduler.MapFailureReason(error, committed: true)
            );
        }
    }
    ...
    public async Task Host_Stop_With_All_Channel_Kinds_And_An_Actor_Without_Relocation_Target_Is_Stopped()
    {
        ...
        builder.Services.AddZLinkFramework(options =>
        {
            ...
            options
                .AddClientServerChannel("client-server-work")
                .Server()
                .Listen(0)
                .AddRequestHandler<
                    TestChannelRequestHandler,
        ...
        Assert.Equal(
            ...
            runtime.Status.TerminationResult
        );
    ...
    public async Task Framework_Registration_Projects_And_Resets_Host_Capacity()
    {
        ...
        Assert.Throws<InvalidOperationException>(runtime.ResetCapacityMetrics);
    }
    ...
    public void Legacy_Drain_Contracts_Are_Not_Public()
    {
        ...
        Assert.Null(
            contracts.GetType("Zlink.Framework.Contracts.Configuration.IZLinkDrainControl")
        );
        Assert.Null(contracts.GetType("Zlink.Framework.Contracts.Configuration.ZLinkDrainResult"));
    }
    ...
    public async Task Framework_Drain_Sends_ServerDrain_Before_Orderly_Stream_Close()
    {
        ...
        await connector.Connect.Async();
        await connector.Send(new DrainProbeMessage("connected")).PacketName("drain.probe").Async();
    ...
    )
    {
        ...
        try
        {
            ...
            for (var offset = 0; offset < connectors.Length; offset += connectionBatchSize)
            {
                ...
                await Task.WhenAll(
                    batch.Select(async connector => await connector.Connect.Async())
                );
                for (
                    var index = 0;
            ...
            await Task.WhenAll(
                lateConnectors.Select(async connector =>
                {
                    try
                    {
                        await connector.Connect.Async();
            ...
            await Task.WhenAll(
                lateConnectors.Select(connector => connector.DisposeAsync().AsTask())
            );
            lateDisposed = true;
        ...
        finally
        {
            ...
            await Task.WhenAll(connectors.Select(connector => connector.DisposeAsync().AsTask()));
            await host.StopAsync();
        }
    }
        ...
        public ZLinkDrainExecutionOperations Operations =>
            new(
                ...
                DrainRelocationWorkloads: _ =>
                {
                    ...
                    return ValueTask.FromResult(
                        new ZLinkRelocationWorkloadDrainResult(true, null, 0)
                    );
                },
                DrainStreamSessions: _ =>
                {
    ...
    private sealed class MutableFrameworkRuntime : IZLinkFrameworkRuntime
    {
        ...
        public ZLinkFrameworkRuntimeStatus Status =>
            new(
                IsReady ? ZLinkFrameworkRuntimeState.Serving : ZLinkFrameworkRuntimeState.Draining,
                IsReady,
                IsReady,
                null,
                null,
        ...
        ) => throw new InvalidOperationException("The readiness check must not start observation.");
        ...
        public ValueTask<ZLinkFrameworkRelocationResult> RelocateAsync(
            ...
            CancellationToken cancellationToken = default
        ) => throw new InvalidOperationException("The readiness check must not start relocation.");
    ...
    private sealed class ShutdownSealSession(
        ...
        ShutdownSealSessionProbe probe
    ) : IZLinkSession
    {
    ...
    private sealed class DrainSession(IZLinkSessionContext context, DrainSessionProbe probe)
        : IZLinkSession
    {
        public IZLinkSessionContext Context { get; } = context;
