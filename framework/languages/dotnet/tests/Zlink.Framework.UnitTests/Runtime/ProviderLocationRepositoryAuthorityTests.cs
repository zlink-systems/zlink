...
public sealed class ProviderLocationRepositoryAuthorityTests
{
    ...
    public async Task CapacityRowsUseTheNodeJavaCanonicalKeyAndJsonShape()
    {
        ...
        )
        {
            ...
            Capacity = new ZLinkPlacementCapacity(
                ...
                new ZLinkPopulationCapacity(0, 0, 0),
                [new ZLinkSpotTypeCapacity(ZLinkPlacementObjectKind.UserSpot, "방&<>", 0, 0, 0)]
            ),
            EntrySpotId = null,
        ...
        const string key = "zlink:v11:capacity:mesh%20%2F%ED%95%9C:node%20%2F%23%3F";
    ...
    )
    {
        ...
        Assert.Equal(expected, Encoding.UTF8.GetString(found.Value.Bytes.Span));
    }
    ...
    public async Task OwnerClaimResponseLossReconcilesTheAppliedLease()
    {
        ...
        var provider = new ApplyThenThrowLocationStore(inner) { ThrowAfterNextWrite = true };
        var repository = new ZLinkProviderLocationRepository(provider);

        var claimed = Assert.IsType<ZLinkOwnerLeaseClaimResult.Claimed>(
            await repository.ClaimOwnerLeaseAsync("ambiguous-owner", TimeSpan.FromMinutes(2))
        );
    ...
    [InlineData("+1")]
    [InlineData(" 1")]
    public async Task OwnerClaimRejectsNonCanonicalGenerationCounter(string encodedGeneration)
    {
        var provider = new ZLinkInMemoryProviderLocationStore();
        var repository = new ZLinkProviderLocationRepository(provider);
        ...
        var failure = await Assert.ThrowsAsync<InvalidDataException>(async () =>
            await repository.ClaimOwnerLeaseAsync("noncanonical-owner", TimeSpan.FromMinutes(2))
        );
    ...
    public async Task OwnerLeaseReadFailsClosedOnAnUnrecognizedRecordVersion()
    {
        ...
        Assert.IsType<ZLinkOwnerLeaseClaimResult.Claimed>(
            await repository.ClaimOwnerLeaseAsync("record-version-owner", TimeSpan.FromMinutes(2))
        );

        var ownerKey = ZLinkProviderLocationRepository.OwnerKey("record-version-owner");
        var found = Assert.IsType<ZLinkStoreReadResult.Found>(await provider.ReadAsync(ownerKey));
        var row = JsonNode.Parse(Encoding.UTF8.GetString(found.Value.Bytes.Span))!;
    ...
    public async Task DescriptorPagingUsesTheBoundedProviderSnapshotCursor()
    {
        ...
        var page = await repository.ListMeshNodesAsync("play", new ZLinkPageRequest(100));
        ...
        await Assert.ThrowsAsync<ZLinkLocationSnapshotExpiredException>(() =>
            repository
                .ListMeshNodesAsync("play", new ZLinkPageRequest(100, page.ContinuationToken))
                .AsTask()
        );
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            repository
                .ListMeshNodesAsync("play", new ZLinkPageRequest(100, new string('x', 5601)))
                .AsTask()
        );
        Assert.Equal(2, provider.ScanCalls);
    }
    ...
    [InlineData("fanout")]
    public async Task DescriptorResponseLossReconcilesTheExactAppliedRow(string descriptorKind)
    {
        var inner = new ZLinkInMemoryProviderLocationStore();
        ...
        switch (descriptorKind)
        {
            case "mesh":
                ...
                Assert.Single((await repository.ListMeshNodesAsync("play", default)).Items);
                break;
            case "client-server":
                ...
                Assert.Single((await repository.ListClientServersAsync("orders", default)).Items);
                break;
            case "fanout":
                ...
                Assert.Single(
                    (await repository.ListFanoutPublishersAsync("events", default)).Items
                );
                break;
            default:
    ...
    public async Task MeshNewClaimRejectsDescriptorWhoseOwnerLeaseIsLive()
    {
        ...
        var contender = await ClaimAsync(repository, "contending-descriptor-owner");
    ...
    public async Task DescriptorRenewRetriesAfterSameOwnerLeaseHeartbeat()
    {
        ...
        Assert.Equal(
            ZLinkLocationWriteStatus.Stored,
            (
                await repository.UpdateMeshNodeAsync(preparing, ZLinkLocationWriteIntent.NewClaim)
            ).Status
        );
        provider.Arm();
    ...
    public async Task DescriptorRenewConflictRejectsChangedOwnerOrPredecessor(string mutationName)
    {
    ...
    public async Task SharedOpaqueProvider_CreatesAndReadsAuthorityAcrossRepositories()
    {
        ...
        Assert.Equal(
            ZLinkLocationWriteStatus.Stored,
            (await target.UpdateMeshNodeAsync(descriptor, ZLinkLocationWriteIntent.NewClaim)).Status
        );
        ...
        Assert.Empty(await ScanKeysAsync(provider, "zlink:v11:creation-reservation:"));
        var committed = Assert.IsType<ZLinkObjectCommitResult.Committed>(
            await target.CommitAsync(reserved.Reservation, new byte[] { 0x22 })
        );
    ...
    public async Task ReservedCreationCompletesAfterPlacementWeightBecomesZero()
    {
        ...
        var current = Assert.Single((await repository.ListMeshNodesAsync("play", default)).Items);
        ...
        Assert.IsType<ZLinkObjectReserveResult.PlacementCapacityExhausted>(
            await repository.ReserveAsync(Reservation("actor:capacity-proof", descriptor, owner))
        );
    }
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task RecreatingDeletedAuthorityAdvancesObjectGeneration(bool userSpot)
    {
        ...
        var committed = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(first.Reservation, new byte[] { 0x24 })
            )
            .Snapshot;
        ...
        Assert.True(second.Reservation.ObjectGeneration > first.Reservation.ObjectGeneration);
    }
    ...
    )
    {
        ...
        var firstRequest = Reservation(key, sourceDescriptor, sourceOwner, objectKind);
        var first = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(firstRequest)
        );
        var committed = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(first.Reservation, new byte[] { 0x25 })
            )
            .Snapshot;
        Assert.Equal(
    ...
    )
    {
        ...
        var first = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await source.ReserveAsync(Reservation(key, sourceDescriptor, sourceOwner, objectKind))
        );
        ...
        Assert.True(replacement.Reservation.ObjectGeneration > first.Reservation.ObjectGeneration);
    ...
    public async Task ConcurrentExpiredOwnerReservationsPreserveActiveAuthority()
    {
        ...
        var committed = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await firstRepository.CommitAsync(first.Reservation, new byte[] { 0x26 })
            )
            .Snapshot;
        Assert.Equal(
            ZLinkOwnerLeaseReleaseResult.Released,
        ...
        var observed = Assert
            .IsType<ZLinkAuthorityReadResult.Found>(
                await firstRepository.ReadAuthorityAsync(firstRequest.Key)
            )
            .Snapshot;
        Assert.Equal(committed.StoreVersion, observed.StoreVersion);
        Assert.Equal(committed.ObjectGeneration, observed.ObjectGeneration);
    ...
    [Fact]
    public async Task SharedOpaqueProvider_RelocationOwnerAndAllocationCommitAreAtomic()
    {
        var provider = new ZLinkInMemoryProviderLocationStore();
        var source = new ZLinkProviderLocationRepository(provider);
        var target = new ZLinkProviderLocationRepository(provider);
        ...
        var request = Reservation("actor:relocation", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await source.ReserveAsync(request)
        );
        var created = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await source.CommitAsync(reserved.Reservation, new byte[] { 0x31 })
            )
            .Snapshot;
        var targetAllocation = created.Allocation with
        {
            Descriptor = new ZLinkMeshNodeDescriptorKey("play", targetDescriptor.Rid),
        ...
        Assert.Equal(
            targetOwner,
            new ZLinkLocationOwnerToken(observed.OwnerId, observed.OwnerLeaseGeneration)
        );
        ...
        Assert.True(targetGeneration > created.AuthorityOwnerGeneration);
        ...
        Assert.IsType<ZLinkAuthorityCompareExchangeResult.Conflict>(
            await target.CompareExchangeAuthorityAsync(
                request.Key,
                created.StoreVersion,
                new ZLinkAuthorityMutation.Put(
                    new byte[] { 0x32 },
    ...
    [Fact]
    public async Task SharedOpaqueProvider_RelocationOwnerCommitRetriesUnchangedAuthorityAfterAllocationConflict()
    {
        var inner = new ZLinkInMemoryProviderLocationStore();
        var provider = new AuthorityAllocationConflictOnceLocationStore(inner);
        var source = new ZLinkProviderLocationRepository(provider);
        var target = new ZLinkProviderLocationRepository(provider);
        var sourceOwner = await ClaimAsync(source, "source-owner");
        var targetOwner = await ClaimAsync(target, "target-owner");
        var sourceDescriptor = Descriptor("source", sourceOwner);
        var targetDescriptor = Descriptor("target", targetOwner);
        _ = await source.UpdateMeshNodeAsync(sourceDescriptor, ZLinkLocationWriteIntent.NewClaim);
        _ = await target.UpdateMeshNodeAsync(targetDescriptor, ZLinkLocationWriteIntent.NewClaim);

        var request = Reservation(
            "actor:relocation-allocation-retry",
            sourceDescriptor,
            sourceOwner
        );
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
        ...
        var created = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await source.CommitAsync(reserved.Reservation, new byte[] { 0x33 })
            )
            .Snapshot;
        ...
        Assert.Equal(targetGeneration, moved.Snapshot.AuthorityOwnerGeneration);
    }
    ...
    public async Task Stale_Authority_Delete_Submits_The_Expected_Version_Fence()
    {
        ...
        var createRequest = Reservation("actor:stale-delete", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
        ...
        Assert.Equal(created.ObjectGeneration, current.Snapshot.ObjectGeneration);
    }
    ...
    public async Task SharedOpaqueProvider_ReconcilesAppliedAggregateAfterReplyLoss()
    {
        ...
        var createRequest = Reservation("actor:ambiguous-aggregate", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(createRequest)
        );
        var created = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(reserved.Reservation, new byte[] { 0x61 })
            )
            .Snapshot;
        var request = new ZLinkAggregatePrepareRequest(
            ...
            new ZLinkMeshNodeDescriptorKey("play", targetDescriptor.Rid),
        ...
        var reconciled = Assert.IsType<ZLinkAggregatePrepareResult.AlreadyPrepared>(
        ...
        var afterCommit = Assert.IsType<ZLinkAggregatePrepareResult.AlreadyPrepared>(
            await repository.PrepareAggregateAsync(request)
        );
        Assert.Equal(
            reconciled.TargetAuthorityOwnerGenerations[createRequest.Key],
    ...
    public async Task SharedOpaqueProvider_CommittedAggregateIsVisibleBeforePhysicalNormalization()
    {
        ...
        var createRequest = Reservation("actor:aggregate-visible", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(createRequest)
        );
        var created = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(reserved.Reservation, new byte[] { 0x65 })
            )
            .Snapshot;
        var request = new ZLinkAggregatePrepareRequest(
            Guid.NewGuid(),
        ...
        var visible = Assert
            .IsType<ZLinkAuthorityReadResult.Found>(
                await repository.ReadAuthorityAsync(createRequest.Key)
            )
            .Snapshot;
        ...
        Assert.Equal(visible.AuthorityOwnerGeneration, normalized.AuthorityOwnerGeneration);
    }
    ...
    public async Task SharedOpaqueProvider_ConcurrentDistinctReservationsHideCounterContention()
    {
        ...
        var results = await Task.WhenAll(
            Enumerable
                ...
                .Select(index =>
                    repository
                        .ReserveAsync(
                            Reservation($"actor:counter-contention:{index}", descriptor, owner)
                        )
                        .AsTask()
                )
        );
        ...
        var reservations = results
            .Select(static result => Assert.IsType<ZLinkObjectReserveResult.Reserved>(result))
            .ToArray();
        Assert.Equal(
            64,
            reservations
                .Select(static result => result.Reservation.AuthorityOwnerGeneration)
                .Distinct()
                .Count()
        );
    }
    ...
    public async Task ReserveUsesGoldenNextToIssueCountersAsBareDecimal()
    {
        ...
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(Reservation("actor:counter-golden", descriptor, owner))
        );
    ...
    public async Task ReserveBootstrapsAbsentGenerationCountersAtOne()
    {
        ...
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(Reservation("actor:counter-bootstrap", descriptor, owner))
        );
    ...
    public async Task ReserveReturnsGenerationExhaustedWithoutMutatingCounter()
    {
        ...
        Assert.Equal(long.MaxValue.ToString(), await ReadCounterAsync(provider, ObjectCounterKey));
        Assert.IsType<ZLinkStoreReadResult.Missing>(
            await provider.ReadAsync(ZLinkProviderLocationRepository.AuthorityMetaKey(request.Key))
        );
    }
    ...
    public async Task AggregateNewOwnerBlockStoresTheNextGenerationAfterItsRange()
    {
        ...
        for (var index = 0; index < 2; index++)
        {
            ...
            var created = Assert
                .IsType<ZLinkObjectCommitResult.Committed>(
                    await repository.CommitAsync(reservation.Reservation, new byte[] { 0xA1 })
                )
                .Snapshot;
            participants.Add(
                new ZLinkAggregateParticipant(
    ...
    public async Task SharedOpaqueProvider_ConcurrentDistinctCreationCompletionsRetryCapacityContention()
    {
        ...
        var completions = await Task.WhenAll(
            reservations.Select(async item =>
            {
                ...
                return (
                    item,
                    operation,
                    Result: await repository.CompleteCreationAsync(item.Reservation, completion)
                );
            })
        );
        ...
        {
            Assert.IsType<ZLinkObjectCreationCompleteResult.Created>(completion.Result);
            Assert.IsType<ZLinkCreationTerminalReadResult.Found>(
                await repository.ReadCreationTerminalAsync(completion.operation)
            );
            Assert.IsType<ZLinkAuthorityReadResult.Found>(
                await repository.ReadAuthorityAsync(completion.item.request.Key)
            );
        }
    }
    ...
    public async Task CreationTerminalUsesCanonicalSingleKeyAndRawValue()
    {
        ...
        var operation = new ZLinkCreationOperationId(RoutingId.FromHex("00ff10"), 7, 1, 0xabcdef);
        ...
        Assert.Equal([key], await ScanKeysAsync(provider, "creation-terminal\0"));
    ...
    public async Task CreationTerminalReadsNodeHandCodecBytesFromCanonicalKey()
    {
        ...
        var dotnetEnvelope = ZLinkActorCreationTerminalCodec.Encode(
            new ActorCreateOperationTerminal(
                ...
                new ActorCreateCompletion(
                    ...
                    new ActorRef("actor-canonical", 1, "mesh", RoutingId.From("node"))
                )
            ),
            codecs
        );
        ...
        var boundaryFailure = Convert.FromHexString("010000000a00000065000000000000");
        Assert.True(
            ZLinkActorCreationTerminalCodec.TryDecode(
                boundaryFailure,
                codecs,
        ...
        Assert.Equal(ServiceWireConstants.FrameworkErrorCode.None, decodedBoundary.FailureCode);
    ...
    public async Task SharedOpaqueProvider_ConcurrentActorDeletesReleaseSharedCapacity()
    {
        ...
        Assert.All(
            results,
            result => Assert.IsType<ZLinkAuthorityCompareExchangeResult.Deleted>(result)
        );
    ...
    public async Task SharedOpaqueProvider_ConcurrentDistinctCommitsRetryCapacityContention()
    {
        ...
        var commits = await Task.WhenAll(
            reservations.Select(item =>
                repository.CommitAsync(item.Reservation, new byte[] { 0x23 }).AsTask()
            )
        );

        Assert.All(
            commits,
            static result => Assert.IsType<ZLinkObjectCommitResult.Committed>(result)
        );
    ...
    public async Task SharedOpaqueProvider_AggregateCommitRetriesCapacityContention()
    {
        ...
        for (var index = 0; index < 2; index++)
        {
            ...
            var created = Assert
                .IsType<ZLinkObjectCommitResult.Committed>(
                    await repository.CommitAsync(reserved.Reservation, new byte[] { 0x69 })
                )
                .Snapshot;
            participants.Add(
                new ZLinkAggregateParticipant(
        ...
        var request = AggregateRequest(participants, targetDescriptor, targetOwner);
        var prepared = Assert.IsType<ZLinkAggregatePrepareResult.Prepared>(
            await repository.PrepareAggregateAsync(request)
        );
        ...
        foreach (var participant in participants)
        {
            var authority = Assert
                .IsType<ZLinkAuthorityReadResult.Found>(
                    await repository.ReadAuthorityAsync(participant.Key)
                )
                .Snapshot;
            Assert.Equal(targetOwner.OwnerId, authority.OwnerId);
            Assert.True(participant.AuthorityPayload.Span.SequenceEqual(authority.Payload.Span));
        }
    }
    ...
    public async Task SharedOpaqueProvider_PreparesMaximumAggregateWithinKeyBound()
    {
        ...
        for (var index = 0; index < 1024; index++)
        {
            ...
            var created = Assert
                .IsType<ZLinkObjectCommitResult.Committed>(
                    await repository.CommitAsync(reserved.Reservation, new byte[] { 0x71 })
                )
                .Snapshot;
            participants.Add(
                new ZLinkAggregateParticipant(
        ...
        Assert.Equal(1024, prepared.TargetAuthorityOwnerGenerations.Values.Distinct().Count());
        Assert.Equal(
            ZLinkAggregateCommitResult.Committed,
            await repository.CommitAggregateAsync(prepared.Fence)
        );
        ...
        foreach (var participant in new[] { participants[0], participants[^1] })
        {
            var authority = Assert
                .IsType<ZLinkAuthorityReadResult.Found>(
                    await repository.ReadAuthorityAsync(participant.Key)
            ...
            Assert.Equal(targetOwner.OwnerId, authority.OwnerId);
            Assert.Equal(targetOwner.LeaseGeneration, authority.OwnerLeaseGeneration);
            Assert.Equal(
    ...
    public async Task SharedOpaqueProvider_MaximumParticipantPayloadsDoNotExpandAggregateRecord()
    {
        ...
        {
            var create = Reservation($"actor:payload-bound:{index}", sourceDescriptor, sourceOwner);
            var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
                await repository.ReserveAsync(create)
            );
            var created = Assert
                .IsType<ZLinkObjectCommitResult.Committed>(
                    await repository.CommitAsync(reserved.Reservation, new byte[] { (byte)index })
                )
                .Snapshot;
        ...
        foreach (var participant in participants)
        {
            ...
            Assert.Equal(participant.AuthorityPayload.Span[0], authority.Payload.Span[0]);
            Assert.Equal(participant.AuthorityPayload.Span[^1], authority.Payload.Span[^1]);
        }
    }
    ...
    public async Task SharedOpaqueProvider_PartialNormalizationRetriesEveryParticipant()
    {
        ...
        Assert.Contains(
            await ScanKeysAsync(inner, AggregatePrefix(prepared.Fence)),
    ...
    public async Task SharedOpaqueProvider_AggregatePreparationUsesBoundedConcurrency()
    {
        ...
        {
            var create = Reservation($"actor:parallel:{index}", sourceDescriptor, sourceOwner);
            var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
                await repository.ReserveAsync(create)
            );
            var created = Assert
        ...
        _ = Assert.IsType<ZLinkAggregatePrepareResult.Prepared>(
            await repository.PrepareAggregateAsync(
                AggregateRequest(participants, targetDescriptor, targetOwner)
            )
        );
    ...
    public async Task SharedOpaqueProvider_StagingFailureIsOwnedAndRetryable()
    {
        ...
        for (var index = 0; index < 2; index++)
        {
            var create = Reservation($"actor:staging-retry:{index}", sourceDescriptor, sourceOwner);
            var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
                await repository.ReserveAsync(create)
            );
            var created = Assert
    ...
    public async Task SharedOpaqueProvider_CancellationAfterFenceInstallAbortsAndReleasesAllFences()
    {
        ...
        for (var index = 0; index < 2; index++)
        {
            var create = Reservation($"actor:fence-cancel:{index}", sourceDescriptor, sourceOwner);
            var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
                await repository.ReserveAsync(create)
            );
            var created = Assert
        ...
        Assert.IsType<ZLinkAggregatePrepareResult.Conflict>(
            await repository.PrepareAggregateAsync(request, callerCancellation.Token)
        );
        ...
        foreach (var participant in participants)
        {
            var authority = Assert
                ...
                .Snapshot;
            refreshed.Add(participant with { ExpectedStoreVersion = authority.StoreVersion });
        }
        var retry = request with { AggregateId = Guid.NewGuid(), Participants = refreshed };
    ...
    public async Task SharedOpaqueProvider_CancellationAfterAggregateClaimCleansUnfencedChildren()
    {
        ...
        var create = Reservation("actor:claim-cancel", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(create)
        );
        var created = Assert
        ...
        var request = AggregateRequest([participant], targetDescriptor, targetOwner);
        var fence = new ZLinkAggregateFence(request.AggregateId, request.AggregateGeneration);
        using var callerCancellation = new CancellationTokenSource();
        provider.FailAggregateId = request.AggregateId;
        provider.CallerCancellation = callerCancellation;
        ...
        Assert.DoesNotContain(
            await ScanKeysAsync(inner, AggregatePrefix(fence)),
            static key =>
                key.Contains(":participant:", StringComparison.Ordinal)
        ...
        var retry = request with
        {
            ...
            Participants = [participant with { ExpectedStoreVersion = authority.StoreVersion }],
        };
        Assert.IsType<ZLinkAggregatePrepareResult.Prepared>(
            await repository.PrepareAggregateAsync(retry)
        );
    ...
    public async Task SharedOpaqueProvider_FirstAggregateUseReclaimsAbandonedStaging()
    {
        ...
        var create = Reservation("actor:staging-recovery", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(create)
        );
        var created = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(reserved.Reservation, new byte[] { 0x91 })
            )
            .Snapshot;
        var participant = new ZLinkAggregateParticipant(
            create.Key,
        ...
        var abandoned = AggregateRequest([participant], targetDescriptor, targetOwner);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task SharedOpaqueProvider_PreserveAggregateCompletesWhileOwnerDrains(
        bool drainBeforePrepare
    )
    {
        var repository = new ZLinkProviderLocationRepository(
            new ZLinkInMemoryProviderLocationStore()
        ...
        var initial = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(reserved.Reservation, new byte[] { 0x31 })
            )
            .Snapshot;
        ...
        {
            Capacity = new ZLinkCapacityVector(0, 0, null),
        };
        ...
        var normalized = Assert
            .IsType<ZLinkAuthorityReadResult.Found>(
                await repository.ReadAuthorityAsync(creation.Key)
            )
            .Snapshot;
    ...
    public async Task SharedOpaqueProvider_StaleParticipantDoesNotClaimStagingOrCapacity()
    {
        ...
        var create = Reservation("actor:stale-aggregate", sourceDescriptor, sourceOwner);
        var reserved = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(create)
        );
        var created = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(reserved.Reservation, new byte[] { 0x94 })
            )
            .Snapshot;
        var request = AggregateRequest(
            [
    ...
    public async Task SharedOpaqueProvider_TenThousandParticipantsUsePagedRowsAndSmallRoot()
    {
        ...
        var provider = new ZLinkInMemoryProviderLocationStore();
        var repository = new ZLinkProviderLocationRepository(provider);
        var sourceOwner = await ClaimAsync(repository, "source-owner", TimeSpan.FromMinutes(15));
        var targetOwner = await ClaimAsync(repository, "target-owner", TimeSpan.FromMinutes(15));
        var sourceDescriptor = Descriptor("source", sourceOwner, participantCount * 2);
        var targetDescriptor = Descriptor("target", targetOwner, participantCount * 2);
        _ = await repository.UpdateMeshNodeAsync(
            sourceDescriptor,
            ZLinkLocationWriteIntent.NewClaim
        );
        ...
        var root = Assert.IsType<ZLinkStoreReadResult.Found>(
            await provider.ReadAsync(new ZLinkStoreKey(AggregatePrefix(prepared.Fence)))
        );

        Assert.True(root.Value.Bytes.Length < 1024 * 1024);
        var preparedKeys = await ScanKeysAsync(provider, AggregatePrefix(prepared.Fence));
        var inventoryLeafKeys = preparedKeys
            .Where(static key => key.Contains(":inventory:0:", StringComparison.Ordinal))
            .ToArray();
        Assert.Equal(10, inventoryLeafKeys.Length);
        var inventoryPageBytes = new Dictionary<string, byte[]>(StringComparer.Ordinal);
        foreach (var inventoryLeafKey in inventoryLeafKeys)
        {
            var page = Assert.IsType<ZLinkStoreReadResult.Found>(
                await provider.ReadAsync(new ZLinkStoreKey(inventoryLeafKey))
            );
            Assert.InRange(page.Value.Bytes.Length, 1, 1024 * 1024);
            Assert.Equal(0x5A4C4956u, BinaryPrimitives.ReadUInt32BigEndian(page.Value.Bytes.Span));
            Assert.Equal(1, page.Value.Bytes.Span[4]);
            Assert.Equal(2, page.Value.Bytes.Span[5]);
            Assert.InRange(
                BinaryPrimitives.ReadInt32BigEndian(page.Value.Bytes.Span.Slice(22, sizeof(int))),
                1,
                1024
            );
            inventoryPageBytes[inventoryLeafKey] = page.Value.Bytes.ToArray();
        }
        Assert.Equal(participantCount, prepared.TargetAuthorityOwnerGenerations.Count);
        ...
        foreach (var pair in inventoryPageBytes)
        {
            ...
            Assert.True(pair.Value.AsSpan().SequenceEqual(repeated.Value.Bytes.Span));
        }
        Assert.Equal(
            ZLinkAggregateCommitResult.Committed,
            await repository.CommitAggregateAsync(prepared.Fence)
    ...
    public async Task SharedOpaqueProvider_CommittedProjectionReadsOnlyInventoryPath()
    {
        ...
        var sourceDescriptor = Descriptor("projection-source", sourceOwner, participantCount * 2);
        var targetDescriptor = Descriptor("projection-target", targetOwner, participantCount * 2);
        _ = await repository.UpdateMeshNodeAsync(
            sourceDescriptor,
            ZLinkLocationWriteIntent.NewClaim
        );
        ...
        for (var index = 0; index < projectedCount; index++)
        {
            var authority = Assert
                .IsType<ZLinkAuthorityReadResult.Found>(
                    await repository.ReadAuthorityAsync(participants[index].Key)
                )
                .Snapshot;
            Assert.Equal(targetOwner.OwnerId, authority.OwnerId);
        }

        Assert.InRange(provider.InventoryReadCount, projectedCount, projectedCount * 3);
        provider.BlockWrites = false;
        Assert.Equal(
            ZLinkAggregateCommitResult.AlreadyCommitted,
            await repository.CommitAggregateAsync(prepared.Fence)
    ...
    public async Task SharedOpaqueProvider_MissingOrCorruptInventoryPageIsDataLost()
    {
        foreach (var mutation in new[] { "missing", "corrupt" })
        {
            ...
            var inventoryKey = (
                await ScanKeysAsync(fixture.Provider, AggregatePrefix(fixture.Prepared.Fence))
            ).Single(static key => key.Contains(":inventory:0:", StringComparison.Ordinal));
            ...
            else
            {
                ...
                corrupted[^1] ^= 0x5A;
                pageMutation = new ZLinkStoreMutation.Put(key, corrupted, null);
            }
            Assert.IsType<ZLinkStoreWriteResult.Applied>(
                await fixture.Provider.WriteAsync(
                    new ZLinkStoreWriteRequest(
                        [new ZLinkStoreCondition.Version(key, page.Value.Version)],
                        [pageMutation]
                    )
                )
            );

            await Assert.ThrowsAsync<ZLinkRelocationDataLostException>(async () =>
                await fixture.Repository.CommitAggregateAsync(fixture.Prepared.Fence)
            );
        }
    }
    ...
    {
        var fixture = await PrepareInventoryFixtureAsync(2);
        var keys = await ScanKeysAsync(fixture.Provider, AggregatePrefix(fixture.Prepared.Fence));
        ...
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await fixture.Provider.WriteAsync(
                new ZLinkStoreWriteRequest(
                    [],
                    [new ZLinkStoreMutation.Delete(new ZLinkStoreKey(missingMeta))]
                )
            )
        );
        ...
        Assert.Equal(
            ...
            await fixture.Repository.AbortAggregateAsync(fixture.Prepared.Fence)
        );
        Assert.DoesNotContain(
            await ScanKeysAsync(fixture.Provider, AggregatePrefix(fixture.Prepared.Fence)),
            static key => key.Contains(":participant:", StringComparison.Ordinal)
    ...
    public async Task SharedOpaqueProvider_AbortPreservesRowsWhenInventoryIsCorrupt()
    {
        var fixture = await PrepareInventoryFixtureAsync(2);
        var keys = await ScanKeysAsync(fixture.Provider, AggregatePrefix(fixture.Prepared.Fence));
        var inventoryKey = keys.Single(static key =>
            key.Contains(":inventory:0:", StringComparison.Ordinal)
        );
        var key = new ZLinkStoreKey(inventoryKey);
        var page = Assert.IsType<ZLinkStoreReadResult.Found>(await fixture.Provider.ReadAsync(key));
        var corrupted = page.Value.Bytes.ToArray();
        corrupted[^1] ^= 0x5A;
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await fixture.Provider.WriteAsync(
                new ZLinkStoreWriteRequest(
                    [new ZLinkStoreCondition.Version(key, page.Value.Version)],
                    [new ZLinkStoreMutation.Put(key, corrupted, null)]
                )
            )
        );
        ...
        Assert.Contains(
            await ScanKeysAsync(fixture.Provider, AggregatePrefix(fixture.Prepared.Fence)),
            static storedKey => storedKey.Contains(":participant:", StringComparison.Ordinal)
        );
    }
    ...
    public async Task SharedOpaqueProvider_DurableCorruptionIsTypedDataLost(string record)
    {
        var fixture = await PrepareInventoryFixtureAsync(2);
        var aggregatePrefix = AggregatePrefix(fixture.Prepared.Fence);
        var keys = await ScanKeysAsync(fixture.Provider, aggregatePrefix);
        var selected = record switch
        {
            ...
            _ => keys.Single(static key =>
                key.EndsWith(":participant:0:payload", StringComparison.Ordinal)
            ),
        };
    ...
    public async Task SharedOpaqueProvider_CorruptAggregateRootBlocksStartupRecoveryWithoutCleanup()
    {
        ...
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await fixture.Provider.WriteAsync(
                new ZLinkStoreWriteRequest(
                    [new ZLinkStoreCondition.Version(rootKey, root.Value.Version)],
                    [new ZLinkStoreMutation.Put(rootKey, "{not-valid-json"u8.ToArray(), null)]
                )
            )
        );
        var keysBefore = await ScanKeysAsync(fixture.Provider, aggregatePrefix);
        var recovering = new ZLinkProviderLocationRepository(fixture.Provider);
        var next = fixture.Request with { AggregateId = Guid.NewGuid() };
        ...
        Assert.Equal(
            ...
            (await ScanKeysAsync(fixture.Provider, aggregatePrefix)).Order(StringComparer.Ordinal)
        );
        Assert.IsType<ZLinkStoreReadResult.Found>(await fixture.Provider.ReadAsync(rootKey));
    }
    ...
    public async Task SharedOpaqueProvider_ReorderedInventoryRootIsDataLost()
    {
        ...
        var rootKey = new ZLinkStoreKey(AggregatePrefix(fixture.Prepared.Fence));
        var root = Assert.IsType<ZLinkStoreReadResult.Found>(
            await fixture.Provider.ReadAsync(rootKey)
        );
        var json = JsonNode.Parse(root.Value.Bytes.Span) ?? throw new InvalidDataException();
        var inventory = Property(json.AsObject(), "inventory").AsObject();
    ...
    public async Task SharedOpaqueProvider_ParticipantPermutationUsesSameFingerprint()
    {
        ...
        {
            Participants = fixture.Request.Participants.Reverse().ToArray(),
        };

        var repeated = Assert.IsType<ZLinkAggregatePrepareResult.AlreadyPrepared>(
            await fixture.Repository.PrepareAggregateAsync(permuted)
        );
        Assert.Equal(fixture.Prepared.Fence, repeated.Fence);
        Assert.Equal(
            fixture.Prepared.TargetAuthorityOwnerGenerations.OrderBy(static pair => pair.Key.Value),
            repeated.TargetAuthorityOwnerGenerations.OrderBy(static pair => pair.Key.Value)
        );
    }
    ...
    public async Task SharedOpaqueProvider_InflatedInventoryTotalIsRejectedBeforeAllocation()
    {
        ...
        var json = JsonNode.Parse(root.Value.Bytes.Span) ?? throw new InvalidDataException();
        SetJsonNumber(json.AsObject(), "participantCount", int.MaxValue);
        SetJsonNumber(
    ...
    private static JsonNode Property(JsonObject json, string propertyName) =>
        json.First(pair =>
            string.Equals(pair.Key, propertyName, StringComparison.OrdinalIgnoreCase)
        ).Value
        ?? throw new InvalidDataException($"JSON property '{propertyName}' is null.");

    private static async ValueTask<ZLinkAggregateParticipant[]> SeedAuthoritiesAsync(
        IZLinkLocationStore provider,
    ...
    )
    {
        ...
        var templateRequest = Reservation(templateActorId, sourceDescriptor, sourceOwner);
        var templateReservation = Assert.IsType<ZLinkObjectReserveResult.Reserved>(
            await repository.ReserveAsync(templateRequest)
        );
        var template = Assert
            .IsType<ZLinkObjectCommitResult.Committed>(
                await repository.CommitAsync(templateReservation.Reservation, new byte[] { 0x69 })
            )
            .Snapshot;
        var templateMeta = Assert.IsType<ZLinkStoreReadResult.Found>(
            await provider.ReadAsync(AuthorityMetaKey(templateActorId))
        );
        var participants = new ZLinkAggregateParticipant[participantCount];
        ...
        for (var offset = 1; offset < participantCount; offset += participantsPerBatch)
        {
            var count = Math.Min(participantsPerBatch, participantCount - offset);
            ...
            for (var batchIndex = 0; batchIndex < count; batchIndex++)
            {
                ...
                mutations.Add(
                    new ZLinkStoreMutation.Put(
                        metaKey,
                        WithAuthorityOwnerGeneration(templateMeta.Value.Bytes, ownerGeneration),
                        null
                    )
                );
            }
            var applied = Assert.IsType<ZLinkStoreWriteResult.Applied>(
                await provider.WriteAsync(new ZLinkStoreWriteRequest([], mutations))
            );
            for (var batchIndex = 0; batchIndex < count; batchIndex++)
            {
                var index = offset + batchIndex;
                participants[index] = AggregateParticipant(
                    ZLinkAuthorityKeyCodec.EncodeActor($"actor:ten-thousand:{index}"),
        ...
        var capacityJson =
            JsonNode.Parse(capacity.Value.Bytes.Span) ?? throw new InvalidDataException();
        // Canonical capacity row shape (capacity-row-canonical-key-json
        // ruling): active.actors, not a flat actorsActive field.
        SetJsonNumber(capacityJson["active"]!.AsObject(), "actors", (long)participantCount);
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await provider.WriteAsync(
                new ZLinkStoreWriteRequest(
    ...
    {
        var json = JsonNode.Parse(encoded.Span) ?? throw new InvalidDataException();
    ...
    private static void SetJsonString(JsonObject json, string propertyName, string value)
    {
        var key = json.First(pair =>
            string.Equals(pair.Key, propertyName, StringComparison.OrdinalIgnoreCase)
        ).Key;
        json[key] = value;
    }

    private static void SetJsonNumber(JsonObject json, string propertyName, long value)
    {
        var key = json.First(pair =>
            string.Equals(pair.Key, propertyName, StringComparison.OrdinalIgnoreCase)
        ).Key;
    ...
    private static void SetJsonNumber(JsonObject json, string propertyName, ulong value)
    {
        var key = json.First(pair =>
            string.Equals(pair.Key, propertyName, StringComparison.OrdinalIgnoreCase)
        ).Key;
    ...
    private static ZLinkStoreKey CapacityKey(ZLinkMeshNodeDescriptor descriptor) =>
        new(
            $"zlink:v11:capacity:{EncodeUriComponent(descriptor.MeshName)}"
    ...
    private static async ValueTask<ZLinkLocationOwnerToken> ClaimAsync(
        ...
        Assert
            .IsType<ZLinkOwnerLeaseClaimResult.Claimed>(
                await repository.ClaimOwnerLeaseAsync(ownerId, leaseTtl ?? TimeSpan.FromMinutes(2))
            )
            .Token;
    ...
    )
    {
        ...
        ZLinkStoreCondition condition = existing switch
        {
            ...
            ZLinkStoreReadResult.Found found => new ZLinkStoreCondition.Version(
                key,
                found.Value.Version
            ),
        ...
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await store.WriteAsync(
                new ZLinkStoreWriteRequest(
                    [condition],
                    [new ZLinkStoreMutation.Put(key, Encoding.UTF8.GetBytes(value), null)]
                )
            )
        );
    }
    ...
    private static ZLinkMeshNodeDescriptor Descriptor(
        ...
        )
        {
            ...
            Capacity = new ZLinkPlacementCapacity(
                ...
                [new ZLinkSpotTypeCapacity(ZLinkPlacementObjectKind.UserSpot, "player", 0, 0, 100)]
            ),
            EntrySpotId =
    ...
    private static ZLinkObjectReservationRequest Reservation(
        ...
        ZLinkLocationOwnerToken owner,
        ZLinkPlacementObjectKind objectKind = ZLinkPlacementObjectKind.Actor
    )
    {
        ...
        return new ZLinkObjectReservationRequest(
            ...
            objectKind == ZLinkPlacementObjectKind.Actor
                ...
                : new ZLinkCapacityVector(
                    0,
                    1,
                    new ZLinkSpotTypeCapacityDelta(objectKind, "player", 1)
                )
        );
    }
    ...
    private static ZLinkAggregatePrepareRequest AggregateRequest(
        ...
        new(
            ...
            new ZLinkMeshNodeDescriptorKey(targetDescriptor.MeshName, targetDescriptor.Rid),
            targetDescriptor.LifecycleGeneration,
    ...
    private static string AggregatePrefix(ZLinkAggregateFence fence) =>
        $"zlink:v11:aggregate:{fence.AggregateId:N}:" + fence.AggregateGeneration;
    ...
    )
    {
        ...
        do
        {
            var result = Assert.IsType<ZLinkStoreScanResult.Page>(
                await store.ScanAsync(new ZLinkStoreScanRequest(prefix, cursor, 1000))
            );
    ...
    )
    {
        ...
        var read = Assert.IsType<ZLinkStoreReadResult.Found>(await store.ReadAsync(rootKey));
        var json = JsonNode.Parse(read.Value.Bytes.Span) ?? throw new InvalidDataException();
        var property = json.AsObject()
            .First(pair => string.Equals(pair.Key, "stagedAt", StringComparison.OrdinalIgnoreCase))
            .Key;
        json[property] = DateTimeOffset.UtcNow.Subtract(TimeSpan.FromDays(2));
        Assert.IsType<ZLinkStoreWriteResult.Applied>(
            await store.WriteAsync(
                new ZLinkStoreWriteRequest(
                    [new ZLinkStoreCondition.Version(rootKey, read.Value.Version)],
                    [
                        new ZLinkStoreMutation.Put(
                            rootKey,
                            Encoding.UTF8.GetBytes(json.ToJsonString()),
    ...
    private sealed class ApplyThenThrowLocationStore(IZLinkLocationStore inner)
        : IZLinkLocationStore
    {
        ...
        public int InventoryReadCount => Volatile.Read(ref _inventoryReadCount);

        public void ResetInventoryReadCount() => Volatile.Write(ref _inventoryReadCount, 0);
        ...
        )
        {
            ...
            if (request.Mutations.Any(static mutation => mutation is ZLinkStoreMutation.Delete))
                DeleteBatchCalls++;
    ...
    ) : IZLinkLocationStore
    {
        ...
        {
            var descriptorPut = request
                .Mutations.OfType<ZLinkStoreMutation.Put>()
                .SingleOrDefault(static put =>
                    put.Key.Value.StartsWith("mesh-node\0", StringComparison.Ordinal)
                );
            ...
            {
                _mutated = true;
                await MutatePredecessorAsync(request, descriptorPut, cancellationToken);
            }
            return await inner.WriteAsync(request, cancellationToken);
        }
        ...
        )
        {
            ...
            switch (mutation)
            {
                case DescriptorConflictMutation.Heartbeat:
                    await ApplyAsync(
                        new ZLinkStoreCondition.Version(ownerCondition.Key, owner.Value.Version),
                        new ZLinkStoreMutation.Put(
                            ownerCondition.Key,
                            owner.Value.Bytes,
                            retention
                ...
                case DescriptorConflictMutation.DeleteRow:
                    var deleted = Assert.IsType<ZLinkStoreReadResult.Found>(
                        await inner.ReadAsync(descriptorPut.Key, cancellationToken)
                    );
                    await ApplyAsync(
                        new ZLinkStoreCondition.Version(descriptorPut.Key, deleted.Value.Version),
                ...
                case DescriptorConflictMutation.ReplaceRow:
                    ...
                    var replacement =
                        JsonNode.Parse(replaced.Value.Bytes.Span)
                        ?? throw new InvalidDataException();
                    replacement["descriptor"]!["endpoint"] = "tcp://127.0.0.1:7999";
                    await ApplyAsync(
                        new ZLinkStoreCondition.Version(descriptorPut.Key, replaced.Value.Version),
                        new ZLinkStoreMutation.Put(
                            descriptorPut.Key,
                ...
                case DescriptorConflictMutation.RegressRevision:
                    ...
                    var row =
                        JsonNode.Parse(current.Value.Bytes.Span)
                        ?? throw new InvalidDataException();
                    row["descriptorRevision"] = "1";
                    row["descriptor"]!["descriptorRevision"] = "1";
                    await ApplyAsync(
                        new ZLinkStoreCondition.Version(descriptorPut.Key, current.Value.Version),
                        new ZLinkStoreMutation.Put(
                            descriptorPut.Key,
                            Encoding.UTF8.GetBytes(row.ToJsonString()),
                            null
    ...
    private sealed class ExpireFirstSnapshotLocationStore(IZLinkLocationStore inner)
        : IZLinkLocationStore
    {
        ...
        )
        {
            ...
            return inner.ScanAsync(request with { Limit = 1 }, cancellationToken);
        }
    }

    private sealed class CancelAfterFirstAggregateFenceStore(IZLinkLocationStore inner)
        : IZLinkLocationStore
    {
        private int _failed;
        ...
        )
        {
            var result = await inner.WriteAsync(request, cancellationToken);
            if (
                FailAggregateId != Guid.Empty
                && Volatile.Read(ref _failed) == 0
                && request.Mutations.OfType<ZLinkStoreMutation.Put>().Any(ContainsAggregateFence)
                && Interlocked.Exchange(ref _failed, 1) == 0
            )
            {
                CallerCancellation?.Cancel();
                throw new OperationCanceledException(
                    CallerCancellation?.Token ?? new CancellationToken(canceled: true)
                );
            }
            return result;
        }
        ...
        {
            return put.Key.Value.StartsWith("authority\0", StringComparison.Ordinal);
        }
    }

    private sealed class CancelAfterAggregateClaimStore(IZLinkLocationStore inner)
        : IZLinkLocationStore
    {
        ...
        {
            var result = await inner.WriteAsync(request, cancellationToken);
            var aggregateRoot = $"zlink:v11:aggregate:{FailAggregateId:N}:1";
            if (
                FailAggregateId != Guid.Empty
                ...
                && request
                    .Mutations.OfType<ZLinkStoreMutation.Put>()
                    .Any(put =>
                        string.Equals(put.Key.Value, aggregateRoot, StringComparison.Ordinal)
                    )
    ...
    private sealed class ConcurrencyTrackingLocationStore(IZLinkLocationStore inner)
        : IZLinkLocationStore
    {
        ...
        )
        {
            if (
                request
                    .Mutations.OfType<ZLinkStoreMutation.Put>()
                    .Any(static put =>
                        put.Key.Value.StartsWith("zlink:v11:aggregate:", StringComparison.Ordinal)
                        && !put.Key.Value.Contains(":participant:", StringComparison.Ordinal)
                        && Encoding
                            .UTF8.GetString(put.Bytes.Span)
                            .Contains("\"status\":2", StringComparison.Ordinal)
                    )
                && request
                    .Mutations.OfType<ZLinkStoreMutation.Put>()
                    .Any(static put =>
                        put.Key.Value.StartsWith("zlink:v11:capacity:", StringComparison.Ordinal)
                    )
            )
            {
    ...
    private sealed class AuthorityAllocationConflictOnceLocationStore(IZLinkLocationStore inner)
        : IZLinkLocationStore
    {
