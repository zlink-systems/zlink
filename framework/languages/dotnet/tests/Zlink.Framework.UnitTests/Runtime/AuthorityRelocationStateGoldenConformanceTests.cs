...
public sealed class AuthorityRelocationStateGoldenConformanceTests
{
    ...
    public void Production_slot_codec_matches_all_authority_relocation_vectors()
    {
        ...
        Assert.Equal("authority-relocation-state-v1", root.GetProperty("format").GetString());
        Assert.Contains(
            "dotnet",
            root.GetProperty("consumers").EnumerateArray().Select(static item => item.GetString())
        );
        ...
        foreach (var vector in valid)
        {
            ...
            Assert.Equal(
                bytes,
                ZLinkCanonicalRelocationAuthorityStateCodec.EncodeSlot(expected, rootGeneration)
            );
            Assert.True(
            ...
            Assert.Equal(expected.AggregateGeneration, decoded.AggregateGeneration);
            Assert.Equal(expected.RelocationReference, decoded.RelocationReference);
            Assert.Equal(expected.RelocationChecksumCrc32c, decoded.RelocationChecksumCrc32c);
            Assert.Equal(
                expected.CoordinatorExpectedAuthorityStoreVersion,
                decoded.CoordinatorExpectedAuthorityStoreVersion
            );
            Assert.Equal(expected.SourceCleanupState, decoded.SourceCleanupState);
        }
    ...
    private static ZLinkCanonicalRelocationAuthorityState State(JsonElement value)
    {
        ...
        )
        {
            ...
            RelocationReference = value.GetProperty("relocationReference").GetString()!,
            RelocationChecksumCrc32c = value.GetProperty("relocationChecksumCrc32c").GetUInt32(),
            CoordinatorExpectedAuthorityStoreVersion = value
                .GetProperty("coordinatorExpectedStoreVersion")
                .GetString()!,
            SourceCleanupState = Cleanup(value.GetProperty("sourceCleanupState").GetString()!),
        };
    }
    ...
    private static ulong? RootGeneration(JsonElement vector)
    {
        ...
        return value.ValueKind == JsonValueKind.Null ? null : ulong.Parse(value.GetString()!);
    }

    private static byte Phase(string value) =>
        value switch
        {
            "preparing" => 1,
            "captured" => 2,
    ...
    private static byte Cleanup(string value) =>
        value switch
        {
            "pending" => 0,
            "completed" => 1,
