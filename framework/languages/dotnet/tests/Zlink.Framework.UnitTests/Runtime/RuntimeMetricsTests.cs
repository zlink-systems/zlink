...
public sealed class RuntimeMetricsTests
{
    ...
    public void Meter_Catalog_Uses_Exact_Names_Kinds_Units_And_Scope()
    {
        ...
        var expected = new Dictionary<string, (Type GenericType, string? Unit)>(
        ...
        )
        {
            ...
            ["zlink.relocation.interruption"] = (typeof(Histogram<>), "s"),
            ["zlink.relocation.target_resume"] = (typeof(Histogram<>), "s"),
            ["zlink.relocation.route_convergence"] = (typeof(Histogram<>), "s"),
            ["zlink.relocation.cutover_timeout"] = (typeof(Counter<>), "{fallback}"),
        ...
        Assert.DoesNotContain(
            instruments,
            instrument => instrument.Name.StartsWith("zlink.fanout.", StringComparison.Ordinal)
        );
        Assert.DoesNotContain(
            instruments,
            instrument =>
                instrument.Name
                    is "zlink.spot.queue.depth"
        ...
        Assert.DoesNotContain(
            instruments,
            instrument =>
                instrument.Name.StartsWith("zlink.actor.transfer", StringComparison.Ordinal)
        );
        Assert.DoesNotContain(
            instruments,
    ...
    public void Listener_Failure_Does_Not_Change_Runtime_Result()
    {
        ...
        var exception = Record.Exception(() => ZLinkRuntimeMetrics.RecordActorCreated("mesh"));
    ...
    public void Host_capacity_metrics_read_one_projection_with_only_exact_state_labels()
    {
        ...
        var longSamples =
            new List<(string Name, long Value, IReadOnlyDictionary<string, string> Tags)>();
        ...
        using var longListener = Listen<long>(
            names,
            (instrument, value, tags) => longSamples.Add((instrument.Name, value, Tags(tags)))
        );
        ...
        Assert.Equal(
            ["current", "peak"],
            longSamples
                .Where(static sample => sample.Name == "zlink.host.core_hwm.accounted")
        ...
        Assert.Equal(
            ...
            longSamples
                .Where(static sample => sample.Name == "zlink.host.application_job_queue.jobs")
                .Select(static sample => sample.Tags["state"])
                .Order(StringComparer.Ordinal)
        );
        Assert.All(
            longSamples,
            static sample => Assert.All(sample.Tags.Keys, static key => Assert.Equal("state", key))
        );
    ...
    public void Application_job_pressure_metrics_project_exact_states_and_durations()
    {
        var longSamples =
            new List<(string Name, long Value, IReadOnlyDictionary<string, string> Tags)>();
        var durationSamples = new List<(double Value, IReadOnlyDictionary<string, string> Tags)>();
        using var longListener = Listen<long>(
            [
                "zlink.host.application_job_queue.pressure_state",
                "zlink.host.application_job_queue.pressure_transitions",
        ...
        using var registration = ZLinkRuntimeMetrics.RegisterApplicationJobQueuePressure(() =>
        ...
        Assert.Contains(
            durationSamples,
            sample => sample.Value == 5 && sample.Tags["state"] == "current"
        );
        Assert.Contains(
            durationSamples,
            sample => sample.Value == 9 && sample.Tags["state"] == "cumulative"
        );
        ...
        )
        {
            foreach (var expected in baseline)
            {
                var index = samples.FindIndex(candidate =>
                    candidate.Value == expected.Value && TagsEqual(candidate.Tags, expected.Tags)
                );
                if (index >= 0)
        ...
        static void RemoveLongBaseline(
            List<(string Name, long Value, IReadOnlyDictionary<string, string> Tags)> samples,
            IEnumerable<(
                string Name,
                long Value,
        ...
        static bool TagsEqual(
            ...
            && left.All(entry =>
                right.TryGetValue(entry.Key, out var value) && value == entry.Value
            );
    }
    ...
    {
        var observer = new ZLinkRelocationInterruptionObserver(loggerFactory: null);

        var operation = observer.Start(ZLinkRelocationUnitKind.Actor, "entry");

        Assert.Equal(ZLinkRuntimeMetrics.RelocationInterruptionEnabled, operation.Enabled);
        operation.Complete();
    }
    ...
    public void Relocation_Interruption_Records_Unit_And_Execution_Mode()
    {
        ...
        using var listener = new MeterListener
        {
            InstrumentPublished = (instrument, owner) =>
            {
                if (
                    instrument.Meter.Name == ZLinkMeters.Framework
                    && instrument.Name == "zlink.relocation.interruption"
                )
                    owner.EnableMeasurementEvents(instrument);
            },
        };
        ...
        var observer = new ZLinkRelocationInterruptionObserver(loggerFactory: null, time);
        ...
        Assert.Contains(
            measurement.Tags,
            tag => tag.Key == "execution_mode" && Equals(tag.Value, "entry")
        );
    }
    ...
    public void Interruption_Target_Warning_Does_Not_Include_Object_Identifiers()
    {
        ...
        var time = new ManualTimeProvider();
        var observer = new ZLinkRelocationInterruptionObserver(loggerFactory, time);

        var operation = observer.Start(ZLinkRelocationUnitKind.UserSpot, "per_actor");
        ...
        Assert.Equal("user_spot", log.State["UnitKind"]);
        Assert.Equal("per_actor", log.State["ExecutionMode"]);
        Assert.Equal(true, log.State["InterruptionTargetExceeded"]);
        Assert.Equal(1.25, Assert.IsType<double>(log.State["DurationSeconds"]), precision: 3);
        Assert.DoesNotContain(
            log.State.Keys,
            key =>
                key.Contains("ActorId", StringComparison.Ordinal)
    ...
    public void Interruption_Warning_Provider_Failure_Does_Not_Escape()
    {
        using var loggerFactory = LoggerFactory.Create(builder =>
            builder
                ...
                .AddProvider(new ThrowingLoggerProvider(throwFromIsEnabled: false))
        );
        var time = new ManualTimeProvider();
        var observer = new ZLinkRelocationInterruptionObserver(loggerFactory, time);
        var operation = observer.Start(ZLinkRelocationUnitKind.Actor, "entry");
    ...
    public void Interruption_Warning_IsEnabled_Failure_Does_Not_Escape()
    {
        ...
        var observer = new ZLinkRelocationInterruptionObserver(loggerFactory);
    ...
    public void Interruption_Logger_Creation_Failure_Does_Not_Escape()
    {
        var exception = Record.Exception(() =>
        {
            var observer = new ZLinkRelocationInterruptionObserver(new ThrowingLoggerFactory());
            observer.Start(ZLinkRelocationUnitKind.Actor, "entry").Complete();
        });
    ...
    public async Task Actor_Transfer_Pending_Count_Excludes_One_Way_Send()
    {
        ...
        var pendingSend = mailbox.EnterAsync(CancellationToken.None).AsTask();
        var pendingRequest = mailbox
            .EnterAsync(CancellationToken.None, countAsPendingRequest: true)
            .AsTask();
    ...
    public void Relocation_Metric_Records_One_Terminal_When_Outcomes_Compete()
    {
        ...
        Parallel.For(0, 256, index => operation.Complete(outcomes[index % outcomes.Length]));
    ...
    public void Relocation_Bytes_Uses_Its_Exact_Label_Set()
    {
        ...
        var bytes = Assert.Single(samples, sample => sample.Name == "zlink.relocation.bytes");
    ...
    public void Spot_Count_Keeps_Mesh_And_Spot_Kind_Separate()
    {
        ...
        using var listener = Listen<long>(
            "zlink.spot.count",
            (instrument, value, tags) =>
                samples.Add((instrument.Name, value, Tag(tags, "spot_kind")))
        );
        ...
        Assert.Equal(
            0,
            samples
                .Where(sample => sample.Name == "zlink.spot.count" && sample.Kind == "entry")
                .Sum(sample => sample.Value)
        );
        Assert.Equal(
            0,
            samples
                .Where(sample => sample.Name == "zlink.spot.count" && sample.Kind == "user")
                .Sum(sample => sample.Value)
        );
    }
    ...
    public void Location_And_Dropped_Metrics_Use_Closed_Labels_And_Ignore_Listener_Failure()
    {
        ...
        using var listener = Listen<long>(
            ["zlink.location.owner_lease.renew.failures", "zlink.mesh_node.messages.dropped"],
            (instrument, _, tags) => samples.Add((instrument.Name, Tags(tags)))
        );

        ZLinkRuntimeMetrics.RecordOwnerLeaseRenewFailure("mesh", "mesh");
        ZLinkRuntimeMetrics.RecordMessageDropped("mesh", "channel", "request", "stale_target");

        Assert.Contains(
            samples,
            sample => sample.Name == "zlink.location.owner_lease.renew.failures"
        );
        var dropped = Assert.Single(
            samples,
            sample => sample.Name == "zlink.mesh_node.messages.dropped"
        );
    ...
    public void Observable_Metrics_Pull_Current_Source_State_After_Listener_Attaches()
    {
        ...
        listener.SetMeasurementEventCallback<long>(
            (instrument, value, tags, _) =>
            {
                if (instrument.Name == "zlink.mesh_node.peers.configured")
                    peerSamples.Add(value);
                if (instrument.Name == "zlink.host.state")
                    hostSamples.Add(Tag(tags, "state"));
            }
        );
        listener.Start();
    ...
    private static MeterListener Listen<T>(string instrumentName, MetricRecorder<T> record)
    ...
    private sealed class ThrowingLoggerProvider(bool throwFromIsEnabled) : ILoggerProvider
    {
        public ILogger CreateLogger(string categoryName) => new ThrowingLogger(throwFromIsEnabled);
    ...
    private sealed class ThrowingLoggerFactory : ILoggerFactory
    {
        ...
        public ILogger CreateLogger(string categoryName) =>
            throw new InvalidOperationException("injected CreateLogger failure");
    ...
    private sealed class ThrowingLogger(bool throwFromIsEnabled) : ILogger
    {
        ...
        public bool IsEnabled(LogLevel logLevel) =>
            throwFromIsEnabled
                ? throw new InvalidOperationException("injected IsEnabled failure")
                : true;
