...
public sealed class GeneratedRelocationCodecConformanceTests
{
    ...
    public void RelocationEnvelope_RuntimeAndGeneratedCodecsMatchGoldenBytes()
    {
        ...
        Assert.Equal(golden, ReadProjection("relocation-envelope-v1", "hex"));
        ...
        Assert.Equal(golden, ServiceWirePilotCodec.EncodeRelocationEnvelopeV1(generated));
    }

    [Fact]
    public void RelocationEnvelope_BothCodecsRejectAdjudicatedCrossSectionMutations()
    {
        var membershipMutation = ReadGolden("relocation-envelope-v1", "logicalHex");
        membershipMutation[132] = 4;
        AssertRejectedByBoth(membershipMutation);

        var timerReferenceMutation = ReadGolden("relocation-envelope-v1", "logicalHex");
    ...
    {
        var chunkGolden = ReadGolden("relocation-data-chunk-v1", "encodedHex");
        var manifestGolden = ReadGolden("relocation-manifest-v1", "encodedHex");
        Assert.Equal(chunkGolden, ReadProjection("relocation-data-chunk-v1", "encodedHex"));
        Assert.Equal(manifestGolden, ReadProjection("relocation-manifest-v1", "encodedHex"));

        var generatedChunk = ServiceWirePilotCodec.DecodeRelocationDataChunkV1(chunkGolden);
        var generatedManifest = ServiceWirePilotCodec.DecodeRelocationManifestV1(manifestGolden);
        Assert.Equal(
            chunkGolden,
    ...
    {
        Assert.ThrowsAny<Exception>(() => ZLinkRelocationEnvelopeCodec.Decode(encoded));
    ...
    private static byte[] ReadGolden(string name, string property) =>
        ReadFixture($"framework/runtime/protocol/golden/{name}.json", property);

    private static byte[] ReadProjection(string name, string property) =>
        ReadFixture($"framework/runtime/protocol/generated/fixtures/{name}-pilot.json", property);

    private static byte[] ReadFixture(string relativePath, string property)
    {
        var path = Path.Combine(Common.FrameworkTestEnvironment.GetRepoRoot(), relativePath);
    ...
    private sealed class GoldenTreeStore : IZLinkRelocationRepository
    {
        ...
        )
        {
            ...
            var reference =
                bytes.AsSpan(0, 4).SequenceEqual("ZLTC"u8) ? "chunk-0"
                : bytes.AsSpan(0, 4).SequenceEqual("ZLTM"u8) ? "manifest"
