...
public sealed partial class StreamConnectorTests
{
    ...
    public async Task TcpReceiveDispatchesMultipleHeaderPacketsInOrder()
    {
        ...
        var endpoint = (IPEndPoint)listener.LocalEndpoint;
        var headerCodec = new ZlinkStreamHeaderCodec();
        var receivedAll = new TaskCompletionSource(
        ...
        connector.On(
            ...
            {
                received.Add(
                    $"{message.Name}:{Encoding.UTF8.GetString(message.Payload.Payload.Span)}"
                );
                first.SetResult();
    ...
    public async Task WebSocketSendUsesBinaryFrames()
    {
        ...
        await connector
            .Send(new ZlinkStreamEncodedPayload(ZlinkStreamCodec.Raw, "wb"u8.ToArray()))
            .PacketName("wh")
            .Async();
    ...
    public async Task TlsSendWorksWithSkippedCertificateValidation()
    {
        ...
        await connector
            .Send(new ZlinkStreamEncodedPayload(ZlinkStreamCodec.Raw, "tb"u8.ToArray()))
            .PacketName("th")
            .Async();
    ...
    public async Task TlsConnectTimeoutDisposesAcceptedSocket()
    {
        ...
        listener.Start();
        var endpoint = (IPEndPoint)listener.LocalEndpoint;
        var observedClientClose = new TaskCompletionSource(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        var server = Task.Run(async () =>
        {
