...
public sealed partial class StreamConnectorTests
{
    ...
    public async Task CustomPayloadCodecEncodesAndDecodesTypedConnectorCalls()
    {
        ...
        await connector.Send(new CustomProbe("hello")).PacketName("custom.send").Async();
        Assert.Equal(ZlinkStreamCodec.Raw, connector.SendCall.Payload.Codec);
        Assert.Equal(
            "CUSTOM:hello",
            Encoding.UTF8.GetString(connector.SendCall.Payload.Payload.Span)
        );
        ...
        var reply = await connector
            .Request(new CustomProbe("ask"))
            .PacketName("custom.request")
            .Async<CustomProbe>();
        Assert.Equal("reply", reply.Text);
        Assert.Equal(
            "CUSTOM:ask",
            Encoding.UTF8.GetString(connector.RequestCall.Payload.Payload.Span)
        );
        ...
        connector.NextCallbackPayloadResult = ZlinkStreamResult<ZlinkStreamEncodedPayload>.Success(
            codec.Encode(new CustomProbe("callback"))
        );
        connector
            .Request(new CustomProbe("ask"))
        ...
        using var subscription = connector.On<CustomProbe>(
            "custom.notify",
            (message, _) =>
            {
        ...
        var filtered = await connector
            .WaitFor<CustomProbe>("custom.wait")
    ...
    public void AutoCodecFallsBackToBuiltInWhenNoCustomCodecIsRegistered()
    {
        ...
        connector.Send(new CustomProbe("plain")).PacketName("auto.json").Async();
