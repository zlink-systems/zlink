...
{
    private static readonly DateTimeOffset StoreNow = new(2026, 7, 2, 0, 0, 0, TimeSpan.Zero);
    ...
    public void Location_contract_excludes_compatibility_lease_and_slot_allocation_surface()
    {
        ...
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkAllocatedRoutingIdProvider")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkRoutingIdSlotAcquireResult")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkOwnerLeaseSnapshot")
        );
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkOwnerLeaseRenewal"));

        var publicMethods =
            typeof(Zlink.Framework.Locations.Redis.ZLinkRedisLocationStore).GetMethods();
        ...
        Assert.DoesNotContain(
            publicMethodNames,
            name =>
                name.Contains("OwnerLease", StringComparison.Ordinal)
    ...
    {
        var assembly = typeof(IZLinkLocationRepository).Assembly;
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkAuthorityStore"));
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkMeshNodeLocationStore")
        );
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkOwnerLeaseStore"));
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkClientServerLocationStore")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkFanoutLocationStore")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkLocationChangeStampStore")
        );
        Assert.Empty(
            assembly
                .GetExportedTypes()
        ...
        Assert.DoesNotContain(
            typeof(IZLinkLocationRepository).GetMethods(),
            static method => method.Name.Contains("Preparing", StringComparison.Ordinal)
        );
        Assert.NotNull(
            typeof(ZLinkAuthorityMutation).GetNestedType(nameof(ZLinkAuthorityMutation.Restore))
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkLocationChangeScopeKind")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkLocationChangeStampScope")
        );
        ...
        var leaseClaim = Assert.IsType<ZLinkOwnerLeaseClaimResult.Claimed>(
            await leases.ClaimOwnerLeaseAsync("owner-b", TimeSpan.FromSeconds(15))
        );
    ...
    public async Task MeshNode_lists_are_paged_and_object_authority_is_opaque()
    {
        ...
        await meshNodes.UpdateMeshNodeAsync(
            MakeDescriptor("owner-a"),
        ...
        var descriptors = await meshNodes.ListMeshNodesAsync("play", new ZLinkPageRequest());
        Assert.Single(descriptors.Items);

        Assert.DoesNotContain(
            typeof(IZLinkLocationRepository).GetMethods(),
            static method =>
                method.Name.Contains("Spot", StringComparison.Ordinal)
        ...
        Assert.Null(
            typeof(IZLinkLocationRepository).Assembly.GetType(
    ...
    public void Object_and_mesh_routes_are_not_public_resolvers()
    {
        var assembly = typeof(IZLinkLocationRepository).Assembly;
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkMeshNodeLocationResolver")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkSpotHandleResolver")
        );
        Assert.Null(
            assembly.GetType("Zlink.Framework.Contracts.Locations.IZLinkActorSpotHandleResolver")
        );
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.SpotHandle"));
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkLocationKey"));
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkLocationKind"));
        Assert.Null(assembly.GetType("Zlink.Framework.Contracts.Locations.ZLinkRouteKind"));
    }

    [Fact]
    public void Runtime_location_projections_are_not_exported()
    {
        var exportedTypeNames = typeof(IZLinkLocationRepository)
    ...
    private static ZLinkMeshNodeDescriptor MakeDescriptor(string ownerId) =>
        new(
            "play",
    ...
    private sealed class ExampleOwnerLeaseStore : LocationStoreContractExample
    {
        ...
        {
            if (_leases.TryGetValue(ownerId, out var current) && current.LeaseExpiresAt > StoreNow)
            ...
            return ValueTask.FromResult<ZLinkOwnerLeaseClaimResult>(
                new ZLinkOwnerLeaseClaimResult.Claimed(token, expiresAt, StoreNow)
            );
        }
        ...
        {
            if (!_leases.TryGetValue(ownerId, out var lease) || lease.LeaseExpiresAt <= StoreNow)
        ...
        )
        {
            ...
            var expiresAt = StoreNow + leaseTtl;
            _leases[token.OwnerId] = lease with { LeaseExpiresAt = expiresAt };
            return ValueTask.FromResult<ZLinkOwnerLeaseRenewResult>(
                new ZLinkOwnerLeaseRenewResult.Renewed(expiresAt, StoreNow)
            );
        }
        ...
        private readonly record struct Lease(long LeaseGeneration, DateTimeOffset LeaseExpiresAt);
    }
    ...
    private sealed class ExampleLocationRuntimeQuery : IZLinkLocationRuntimeQuery
    {
        ...
        public ValueTask<ZLinkLocationPage<ZLinkLocationTopologyEntry>> ListTopologyAsync(
            ...
            ValueTask.FromResult(
                new ZLinkLocationPage<ZLinkLocationTopologyEntry>(
                    [
        ...
        public ValueTask<ZLinkLocationPage<ZLinkLocationServiceSummary>> ListServiceSummariesAsync(
        ...
        )
        {
            ...
            [
                new ZLinkLocationServiceSummary("play", 1, 1, 0, 0, StoreNow),
            ];
            return ValueTask.FromResult(
    ...
    private sealed class ExampleLocationReadiness(IZLinkLocationRuntimeQuery query)
