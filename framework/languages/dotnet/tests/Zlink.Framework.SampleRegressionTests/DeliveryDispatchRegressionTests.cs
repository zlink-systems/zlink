...
public sealed partial class RegressionTests
{
    ...
    public void DeliveryDispatch_Uses_One_Role_Specific_Mesh_Per_Host_And_Scanned_Channel_Handlers()
    {
        ...
        foreach (var host in hosts)
        {
            ...
            var expectedMesh =
                host.Contains("CustomerGateway", StringComparison.Ordinal)
        ...
        var dispatch = ReadSource(
            Path.Combine(sampleRoot, "Server", "Dispatch", "DispatchServerHostFactory.cs")
        );
        var tracking = ReadSource(
            Path.Combine(sampleRoot, "Server", "Tracking", "TrackingServerHostFactory.cs")
        );
        Assert.Contains(
        ...
        Assert.Contains(
            NormalizeWhitespace(
                "AddClientServerChannel(SampleNames.TrackingRouteChannel).Server()"
            ),
    ...
    public void DeliveryDispatch_Client_Gate_Verifies_Status_Arrival_Order()
    {
        ...
        var scenario = ReadSource(
            Path.Combine(sampleRoot, "Client", "DeliveryDispatchClientScenario.cs")
        );
        var worker = ReadSource(
            Path.Combine(sampleRoot, "Server", "Dispatch", "DispatchWorker.cs")
        );
        ...
        Assert.DoesNotContain(
            "var assignedWait = WaitForStatusAsync",
            scenario,
            StringComparison.Ordinal
        );
    ...
    public void DeliveryDispatch_Offer_Deadline_Starts_After_Status_Publish()
    {
        ...
        Assert.True(
            reassignPublish >= 0 && reassignPublish < reassignOffer && reassignOffer < reassignSend
        );
    }
    ...
    public void DeliveryDispatch_Runner_Uses_Isolated_Docker_Redis_And_Location_Store()
    {
        ...
        Assert.DoesNotContain(
            "if [[ -z \"${DELIVERYDISPATCH_REDIS_ENDPOINT:-}\" ]]",
            shellRunner,
            StringComparison.Ordinal
        );
        ...
        Assert.DoesNotContain(
            "DELIVERYDISPATCH_STARTUP_DELAY_SECONDS",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "DELIVERYDISPATCH_STARTUP_SETTLE_SECONDS",
            shellRunner,
            StringComparison.Ordinal
        );
        ...
        {
            Assert.Contains(
                $"\"deliverydispatch-ready kind=route node={node}\" \"${{LOG_DIR}}/{node}.log\" 1",
                shellRunner,
                StringComparison.Ordinal
            );
            Assert.Contains(
                $"(Join-Path $LogDir \"{node}.out.log\") \"deliverydispatch-ready kind=route node={node}\" 1",
                powershellRunner,
                StringComparison.Ordinal
            );
        }
        ...
        {
            Assert.Contains(
                $"\"deliverydispatch-ready kind=actor-route node=dispatch target={node}\" \"${{LOG_DIR}}/dispatch.log\" 1",
                shellRunner,
                StringComparison.Ordinal
            );
            Assert.Contains(
                $"(Join-Path $LogDir \"dispatch.out.log\") \"deliverydispatch-ready kind=actor-route node=dispatch target={node}\" 1",
                powershellRunner,
                StringComparison.Ordinal
            );
        }
        ...
        foreach (
            var hostFactory in Directory.EnumerateFiles(
                Path.Combine(sampleRoot, "Server"),
                "*HostFactory.cs",
                SearchOption.AllDirectories
            )
    ...
    public void DeliveryDispatch_Contracts_Match_Common_Role_Model()
    {
        ...
        var courierBinder = ReadSource(
            Path.Combine(sampleRoot, "Server", "CourierSession", "CourierSessionBinder.cs")
        );
        var courierOffers = ReadSource(
            Path.Combine(sampleRoot, "Server", "Dispatch", "DispatchZLinkAdapters.cs")
        );
        var customerSession = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "CustomerGateway",
        ...
        var customerStatusHandler = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "CustomerGateway",
                ...
                "EntrySpot",
                "Handlers",
                "DeliveryStatusUpdatedHandler.cs"
            )
        );
        var customerAccess = ReadSource(
            Path.Combine(sampleRoot, "Server", "CustomerGateway", "CustomerActorAccess.cs")
        );
        var courierEntrySpot = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "CourierActorNode",
        ...
        var customerEntrySpot = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "CustomerGateway",
        ...
        Assert.DoesNotContain(
            "record DeliveryStatusUpdatedRes",
            messages,
            StringComparison.Ordinal
        );
        ...
        {
            Assert.Contains(
                $"\"deliverydispatch-courier bound courier={courier}\" \"${{LOG_DIR}}/courier-session.log\" 1",
                shellRunner,
                StringComparison.Ordinal
            );
            Assert.Contains(
                $"(Join-Path $LogDir \"courier-session.out.log\") \"deliverydispatch-courier bound courier={courier}\" 1",
                powershellRunner,
                StringComparison.Ordinal
            );
        }
        ...
        Assert.DoesNotContain(
            "DELIVERYDISPATCH_CUSTOMER_ROUTE",
            topology,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "DELIVERYDISPATCH_COURIER_ROUTE",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
    ...
    {
        var sampleRoot = ResolveSampleRoot("DeliveryDispatch");
        var courierSessionBinder = ReadSource(
            Path.Combine(sampleRoot, "Server", "CourierSession", "CourierSessionBinder.cs")
        );
        var courierSessionHandler = ReadSource(
            Path.Combine(sampleRoot, "Server", "CourierSession", "BindCourierSessionHandler.cs")
        );
        var courierSessionHost = ReadSource(
            Path.Combine(sampleRoot, "Server", "CourierSession", "CourierSessionHostFactory.cs")
        );

        Assert.Contains(
            ".GetOrCreate(courierId, SampleNames.CourierActorType)",
            courierSessionBinder,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "await binder.BindAsync(request.CourierId",
    ...
    {
        var sampleRoot = ResolveSampleRoot("DeliveryDispatch");
        var dispatchHost = ReadSource(
            Path.Combine(sampleRoot, "Server", "Dispatch", "DispatchServerHostFactory.cs")
        );
        var dispatchAdapters = ReadSource(
            Path.Combine(sampleRoot, "Server", "Dispatch", "DispatchZLinkAdapters.cs")
        );
        var dispatchWorker = ReadSource(
            Path.Combine(sampleRoot, "Server", "Dispatch", "DispatchWorker.cs")
        );
        ...
        Assert.DoesNotContain(
            "new FindCourierActorReq(\"__health__\")",
            dispatchHost,
            StringComparison.Ordinal
        );
        ...
        foreach (
            var file in Directory.EnumerateFiles(sampleRoot, "*.cs", SearchOption.AllDirectories)
        )
        {
            if (
                file.Contains(
        ...
        var courierActor = ReadSource(
            Path.Combine(sampleRoot, "Server", "CourierActorNode", "CourierActor.cs")
        );
