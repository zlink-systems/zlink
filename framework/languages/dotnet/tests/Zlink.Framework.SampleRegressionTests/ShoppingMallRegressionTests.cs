...
public sealed partial class RegressionTests
{
    ...
    public void ShoppingMall_Runner_Uses_Isolated_Docker_Redis_And_Redis_Stores()
    {
        ...
        var topology = ReadSource(
            Path.Combine(sampleRoot, "Server", "Configuration", "SampleNames.cs")
        );
        var commerceApi = ReadSource(
            Path.Combine(sampleRoot, "Server", "CommerceApi", "Program.cs")
        );
        var commerceWorkflowPorts = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "CommerceApi",
        ...
        var commerceWorkflowRouter = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "CommerceApi",
                "Infrastructure",
                "ZLink",
                "ZLinkOrderWorkflowRouter.cs"
            )
        );
        var workflowHostFactory = ReadSource(
            Path.Combine(sampleRoot, "Server", "OrderWorkflow", "OrderWorkflowServerHostFactory.cs")
        );
        var workflowService = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "OrderWorkflow",
        ...
        var workflowSelfCheck = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "OrderWorkflow",
                "Application",
                "SelfCheck",
                "OrderWorkflowSelfCheckService.cs"
            )
        );
        var workflowSpot = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "OrderWorkflow",
        ...
        var startUseCase = ReadSource(
            Path.Combine(
                ...
                "Application",
                "OrderWorkflow",
                "StartOrderUseCase.cs"
            )
        );
        var messages = ReadSource(Path.Combine(sampleRoot, "Shared", "Contracts", "Messages.cs"));
        var clientScenario = ReadSource(
            Path.Combine(sampleRoot, "Client", "ShoppingMallClientScenario.cs")
        );
        var stores = ReadSource(
            Path.Combine(sampleRoot, "Server", "Shared", "Store", "RedisCommerceStores.cs")
        );
        ...
        Assert.DoesNotContain(
            "${SHOPPINGMALL_API_A_HTTP_URL:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_API_B_HTTP_URL:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_API_A_ROUTE_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_API_B_ROUTE_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_A_HTTP_URL:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_B_HTTP_URL:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_A_ROUTE_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_B_ROUTE_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_A_SPOT_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_A_SPOT_ROUTER_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_B_SPOT_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_WORKFLOW_B_SPOT_ROUTER_ENDPOINT:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${SHOPPINGMALL_REDIS_KEY_PREFIX:-",
            shellRunner,
            StringComparison.Ordinal
        );
        ...
        Assert.DoesNotContain(
            "SHOPPINGMALL_STARTUP_DELAY_SECONDS",
            shellRunner,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "$WorkflowUrls.ContainsKey($sourceInstanceId)",
            powershellRunner,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.DoesNotContain(
            "$url -eq $WorkflowUrls[0]",
            powershellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "/self-check/relocation/$relocationOrderId/arm",
            powershellRunner,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        {
            Assert.Contains(
                $"wait_log_contains {node}-order \"${{LOG_DIR}}/{node}.log\" \"shoppingmall-order started order=\"",
                shellRunner,
                StringComparison.Ordinal
            );
            Assert.Contains(
                $"Wait-ShoppingMallLogContains \"{node}-order\" (Join-Path $LogDir \"{node}.out.log\") \"shoppingmall-order started order=\"",
                powershellRunner,
                StringComparison.Ordinal
            );
        }
        ...
        Assert.Contains(
            "new RedisCommerceStores(topology)",
            workflowHostFactory,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "PrepareInventoryReservedOrderUseCase",
            commerceApi,
            StringComparison.Ordinal
        );
        ...
        Assert.DoesNotContain(
            "var cart = await commerce.GetCartAsync",
            commerceApi,
            StringComparison.Ordinal
        );
        ...
        Assert.DoesNotContain(
            "StartOrderWorkflowToInventoryReq",
            messages,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "StartOrderWorkflowToInventoryRes",
            messages,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "StartOrderWorkflowToInventoryRouteHandler",
            commerceWorkflowRouter,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain("StartToInventoryAsync", workflowService, StringComparison.Ordinal);
        Assert.Contains(
            "OrderWorkflowSelfCheckService",
            workflowHostFactory,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "ContinueUntilInventoryReservedAsync(command.OrderId, cancellationToken)",
            workflowSelfCheck,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "Task.WhenAll(concurrentA, concurrentB)",
            clientScenario,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "/orders/order-resume-001/continue",
            clientScenario,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "/orders/order-repair-001/rebuild",
            clientScenario,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "SaveProjectionFromEventsAsync(stored, cancellationToken)",
            workflowService,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "ContinueWorkflowInBackgroundAsync",
            workflowService,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "state.Reservations.TryGetValue(command.ReservationId",
            stores,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "state.Payments.ContainsKey(command.PaymentId)",
            stores,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "주문 이벤트 스트림, 조회 모델, 장바구니·재고·결제·멱등 상태도 같은 Redis",
            readme,
            StringComparison.Ordinal
        );
        Assert.Contains(
    ...
    public void ShoppingMall_Domain_Does_Not_Depend_On_Transport_Configuration_Or_Storage_Format()
    {
        ...
        var domainFiles = Directory
            .GetFiles(Path.Combine(sampleRoot, "Server"), "*.cs", SearchOption.AllDirectories)
            .Where(static path =>
                path.Contains(
        ...
        var mapper = ReadSource(
            Path.Combine(sampleRoot, "Server", "Shared", "Contracts", "OrderContractMapper.cs")
        );
    ...
    public void ShoppingMall_Runners_Observe_The_Same_Planned_Relocation_Evidence()
    {
        ...
        Assert.Contains(
            "curl -fsS \"${SHOPPINGMALL_API_A_HTTP_URL}/orders/${order_id}\"",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "Invoke-RestMethod -Method Get -Uri \"$ApiUrl/orders/$OrderId\"",
            powershellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
