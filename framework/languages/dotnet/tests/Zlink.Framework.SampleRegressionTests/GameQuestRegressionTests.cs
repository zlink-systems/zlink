...
public sealed partial class RegressionTests
{
    ...
    public void GameQuest_Runner_Uses_Isolated_Docker_Redis_And_Stream_Actions()
    {
        ...
        var clientScenario = ReadSource(
            Path.Combine(sampleRoot, "Client", "GameQuestClientScenario.cs")
        );
        var sessionHandlers = ReadSource(
            Path.Combine(sampleRoot, "Server", "GameApi", "Session", "GameQuestSessionHandlers.cs")
        );
        var session = ReadSource(
            Path.Combine(sampleRoot, "Server", "GameApi", "Session", "GameQuestSession.cs")
        );
        var playerSessionActor = ReadSource(
            Path.Combine(sampleRoot, "Server", "GameApi", "Session", "PlayerSessionActor.cs")
        );
        var topology = ReadSource(
            Path.Combine(sampleRoot, "Server", "Configuration", "SampleConfiguration.cs")
        );
        var gameApiStore = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "GameApi",
        ...
        var redisJsonStore = ReadSource(
            Path.Combine(sampleRoot, "Server", "Configuration", "RedisJsonStore.cs")
        );
        var gameApiProgram = ReadSource(
            Path.Combine(sampleRoot, "Server", "GameApi", "Program.cs")
        );
        var missionProgram = ReadSource(
            Path.Combine(sampleRoot, "Server", "QuestMission", "Program.cs")
        );
        var gameplayIngress = missionProgram;
        var playerQuestSpot = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "QuestMission",
        ...
        var questDomain = ReadSource(
            Path.Combine(sampleRoot, "Server", "QuestMission", "Domain", "QuestDomain.cs")
        );
        var actionService = ReadSource(
            Path.Combine(sampleRoot, "Server", "GameApi", "Application", "GameplayActionService.cs")
        );
        var eventDispatcher = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "GameApi",
                "Infrastructure",
                "ZLink",
                "GameplayEventOwnerDispatcher.cs"
            )
        );
        var progressSynchronizer = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "GameApi",
                "Infrastructure",
                "Http",
                "HttpQuestProgressSynchronizer.cs"
            )
        );
        var progressNotifier = ReadSource(
            Path.Combine(
                sampleRoot,
                "Server",
                "QuestMission",
        ...
        Assert.DoesNotContain(
            "${GAMEQUEST_GAMEAPI_A_HTTP_BASE_URL:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "${GAMEQUEST_REDIS_KEY_PREFIX:-",
            shellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain("GAMEQUEST_STORE_DIR", shellRunner, StringComparison.Ordinal);
        Assert.DoesNotContain(
            "GAMEQUEST_STARTUP_DELAY_SECONDS",
            shellRunner,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "$clientProcess = Start-SampleProcess -Name \"client\"",
            powershellRunner,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
        ...
        Assert.Contains(
            "record JoinSessionRes(string PlayerId, QuestProgress[] ActiveQuests)",
            messages,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "/self-check/owner/player-alice/close",
            clientScenario,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "SyncQuestProgressReq(\"player-alice\")",
            clientScenario,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "new QuestProgressNotify(message.PlayerId, message.Progress)",
            playerSessionActor,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "new QuestProgressMsg(playerId, progress)",
            progressNotifier,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.Contains(
            "new QuestCompletedMsg(playerId, completed, true)",
            progressNotifier,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "new RedisJsonStore(topology.RedisEndpoint)",
            gameApiStore,
            StringComparison.Ordinal
        );
        Assert.Contains(
        ...
        Assert.DoesNotContain(
            "interface IPlayerQuestOwnerProvisioner",
            questProcessor,
            StringComparison.Ordinal
        );
        Assert.DoesNotContain(
            "IPlayerQuestOwnerProvisioner",
            gameplayIngress,
            StringComparison.Ordinal
        );
        ...
        Assert.Contains(
            "rehydrated:{pair.Key}:{pair.Value}",
            gameApiProgram,
            StringComparison.Ordinal
        );
