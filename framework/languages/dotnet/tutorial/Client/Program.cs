...
app.MapPost("/players/{playerId}/logins", async (
    string playerId,
    IZLinkRouteClient route,
    CancellationToken cancellationToken) =>
{
    ...
    await route
        .SendToChannel("profile", new RecordLogin(playerId))
...
app.MapGet("/ops/nodes/{nodeRid}/status", async (
    string nodeRid,
    IZLinkRouteClient route,
    CancellationToken cancellationToken) =>
{
...
app.MapPost("/players/{playerId}/tickets", async (
    string playerId,
    IZLinkRouteClient route,
    CancellationToken cancellationToken) =>
{
...
app.MapPost("/rooms/{roomId}/chat", async (
    string roomId,
    PostChat message,
...
app.MapPost("/match-queues/{mode}", async (
    string mode,
...
app.MapGet("/locations/rooms/{roomId}", async (
    string roomId,
    IZLinkSpotManager rooms,
    CancellationToken cancellationToken) =>
{
...
app.MapGet("/locations/players/{playerId}", async (
    string playerId,
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return player is null
        ? Results.NotFound()
        : Results.Ok(new { actorId = player.Value.ActorId, node = player.Value.NodeRid.ToString() });
});
// --8<-- [end:location-find]

// --8<-- [start:actor-create-call]
app.MapPost("/players/{playerId}", async (
    string playerId,
...
app.MapPost("/players/{playerId}/nickname", async (
    string playerId,
...
app.MapGet("/status", (IZLinkFrameworkRuntime runtime) =>
{
