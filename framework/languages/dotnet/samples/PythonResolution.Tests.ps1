...
. (Join-Path $PSScriptRoot 'sample_runner.ps1')

# Regression for issue #632 (the same defect PR #633 fixed for Java/Kotlin): the
# .NET ZoneWorld runner resolved the ZW-B8 fault proxy interpreter with
...
# Get-ZlinkSamplePythonCommand (framework/languages/dotnet/samples/sample_runner.ps1)
...
$probeDir = Join-Path ([IO.Path]::GetTempPath()) "zlink-python-resolution-dotnet-$PID"
...
try {
    ...
    Write-Host 'Negative control #1 confirmed: with no Python on PATH the shipped lookup resolves nothing, which is the "Python is required for the ZW-B8 fault proxy." stop reported in issue #632 (and reproduced against the .NET ZoneWorld gate on this machine before Python was put on PATH by hand).'
    if ($IsWindows) {
    ...
    Write-Host 'PASS dotnet ZW-B8 python resolution: finds a working Python 3 off PATH and rejects an executable that resolves but cannot run'
} catch {
    $env:PATH = $originalPath
    Write-Error $_
    exit 1
