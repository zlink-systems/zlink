...
def is_command_44(body: bytes) -> bool:
        ...
        while True:
            ...
            for raw, flags, body in parser.feed(data):
                ...
                if flags & ZMP_FLAG_MORE:
                    continue
                if blocked:
                    print(f"blocked-command-44 direction={direction}", flush=True)
                else:
...
class Handler(socketserver.BaseRequestHandler):
    def handle(self) -> None:
        ...
        print(
            f"proxy-connection listen={proxy.server_address} target={proxy.target}",
            flush=True,
        )
