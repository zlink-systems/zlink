// Contract enums for the Unity WebGL adapter.
//
// Values mirror Systems.Zlink.Stream.Connector (NuGet Zlink.Stream.Connector)
// member for member and ordinal for ordinal, so game code compiles unchanged
// against either assembly. Only one of the two is compiled per build target:
// this assembly is WebGL-only (see the asmdef), the native one is excluded from
// WebGL.
