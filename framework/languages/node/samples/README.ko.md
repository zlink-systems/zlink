...
| `npm error gyp ERR! ... ZLINK_CORE_INSTALL_PREFIX must name an absolute installed Core ... package prefix`(Windows) | `@zlink-systems/zlink`에 이 플랫폼용 prebuild가 아직 없는 버전이다. 별도로 추적 중이다(#656) — 최신 버전으로 다시 시도한다 |
