...
import { encodeStatefulReply } from '../../packages/framework/src/runtime/foundation/service-stateful-wire-codec';
...
test('canonical Actor Join uses the current authority fence when a previous-owner private route is stale', async () => {
  ...
  const sourceRaw = {
    ...
    async requestService(_rid: string, parts: readonly Buffer[]) {
      ...
      assert.deepEqual(decoded.payload!.payload, canonicalPayload);
      ...
      assert.equal(
        preparedType,
        'Player',
        'S4b Store admission must prepare the Store-resolved factory'
      );
      assert.equal(activated, false, 'canonical command 28 must not materialize the target Actor');
      assert.equal(
        received?.stateful?.reply?.(RequestResult.Ok, 0, undefined, {
          kind: 'actorJoin',
          joinResult: 0,
          spot: targetSpot.ref,
          membershipEpoch: 1n
...
test('command 28 accepts only canonical flags-zero records', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      'protocolError',
      'the removed private 0x01 flavor must be terminal'
    );
    assert.equal(
      await ingress!({
        command: 28,
      ...
      'protocolError',
      'malformed canonical command 28 must be terminal'
    );
