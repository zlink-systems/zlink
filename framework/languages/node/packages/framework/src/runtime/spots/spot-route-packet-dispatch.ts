...
export class ZLinkSpotRoutePacketDispatch {
  ...
  private async dispatchSpotDirectEnvelope(
    ...
    const replyable =
      envelope.kind === ZLinkChannelMessageKind.Request && hasReplyToken(received.replyToken);
    const messageKind = replyable
    ...
    try {
      ...
      if (replyable) {
        submitRouteReply(received.reply().message(encodeSpotDirectReply(true, response)));
      }
    } catch (error) {
      this.options.dispatchErrors?.report({
        surface: ZLinkDispatchErrorSurface.SpotRoute,
...
function decodeSpotDirectEnvelope(parts: readonly Message[]): ZLinkSpotDirectEnvelope | undefined {
  ...
  try {
    ...
    if (
      ...
      (decoded.kind !== ZLinkChannelMessageKind.Request &&
        decoded.kind !== ZLinkChannelMessageKind.Command) ||
