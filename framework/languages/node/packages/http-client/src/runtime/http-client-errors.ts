...
export function responseBodySizeExceeded(
  message = 'HTTP response exceeded the maximum body size'
): ZLinkFrameworkException {
  return new ZLinkFrameworkException(ZLinkFrameworkErrorKind.Rejected, message);
}

export function redirectLimitExceeded(
  message = 'HTTP request exceeded the redirect limit'
): ZLinkFrameworkException {
