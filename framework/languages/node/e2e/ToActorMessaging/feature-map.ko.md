...
|---------|------|-----------|-------------|
| TA-A1 | implemented | `Client/main.ts`의 `runTaA1()`이 bound·unbound Actor variant에 direct send/request를 각각 전달한다. Bound variant의 기존 Session binding과 push를 보존하고, unbound variant에는 암묵적인 binding을 만들지 않는지 함께 검증한다. | `run_e2e.sh TA-A1` 또는 전체 실행에서 `scenario TA-A1 passed`와 `to-actor-messaging e2e result=passed`를 출력한다. |
