...
import type { HandoffProbeMsg } from '../../Shared/messages.js';
...
export async function runStF3(): Promise<void> {
  ...
  try {
    ...
    await connector.send({ scenario: 'ST-F3', marker: 'S1' } satisfies HandoffProbeMsg)
      .packetName(SpotActorTransferNames.packetHandoff).submit();
    await connector.send({ scenario: 'ST-F3', marker: 'S2' } satisfies HandoffProbeMsg)
      .packetName(SpotActorTransferNames.packetHandoff).submit();
    ...
    await connector.send({ scenario: 'ST-F3', marker: 'S3' } satisfies HandoffProbeMsg)
      .packetName(SpotActorTransferNames.packetHandoff).submit();
    await connector.send({ scenario: 'ST-F3', marker: 'S4' } satisfies HandoffProbeMsg)
      .packetName(SpotActorTransferNames.packetHandoff).submit();
