...
| SM-D2 | `Client/Scenarios/SmD2Scenario.cs` | `Client/Scenarios/sm-d2-scenario.ts`, `Client/Support/sm-d2-local-relay-variant.ts`, `Server/Session/Endpoints/session-endpoints.ts`, `Server/Play/Handlers/control-handlers.ts`, `Server/Play/Spots/scenario-actors.ts` | done | session-a에서 local `play-a`와 remote `play-b` Actor를 각각 bind하고 reply와 bound-session push를 검증한다. Historical variant logs: `logs/20260629-211928-1555127`, `logs/20260629-212246-1563843`. |
...
| SM-E1 | `Client/Scenarios/SmE1Scenario.cs` | `Client/Scenarios/sm-e1-scenario.ts`, `Client/Support/sm-e1-actor-missing-handler-variant.ts`, `Server/Play/Handlers/dispatch-error-observer.ts` | done | Actor와 Spot missing-handler request를 함께 실행하고 surface별 error와 logger evidence를 검증한다. |
