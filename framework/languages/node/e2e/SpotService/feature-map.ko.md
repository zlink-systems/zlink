...
| SM-D9 | 미구현 | 공통 e2e spec의 SM-D9는 **서버 application logger provider**가 correlation ID·packet name·message kind를 handler evidence와 같게 기록하는지 본다. 이전 구현은 client `observeInbound(...)`로 inbound frame을 세는 것이라 시나리오 의미와 달랐고, `#371`에서 observer가 제거되며 함께 삭제했다. logger provider 기반으로 다시 구현해야 한다. |
| SM-D10 | 미구현 | 공통 e2e spec의 SM-D10은 **서버 Session별 job queue 격리**(`MaxQueuedApplicationJobs`, slow consumer A가 B의 request·push를 막지 않음)를 본다. 이전 구현은 client `maxReceivedMessages=1` drop을 확인하는 것이라 시나리오 의미와 달랐고, `#371`에서 수신 한도가 제거되며 함께 삭제했다. 서버 gateway 격리 기준으로 다시 구현해야 한다. |
