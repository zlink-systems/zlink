...
| Scenario | 상태 | 근거 |
|----------|------|------|
| RC-A1 | 구현 | provider discovery와 Node decorator variant의 request/send marker를 함께 검증한다. .NET method attribute와 언어별 표면은 다르다. |
...
| RC-B2 | 구현 | Main role의 전역 codec registry에서 Protobuf와 MessagePack content-type, request/send/reply evidence를 각각 검증한다. |
...
검증 결과: `framework/languages/node/e2e/RegistrationCodec/run_e2e.sh` 실행에서 `RC-A1`,
`RC-A3`, `RC-A4`, `RC-A5`, `RC-A6`, `RC-B1`, `RC-B2`, `RC-B4`, `RC-B5`가 통과하고
