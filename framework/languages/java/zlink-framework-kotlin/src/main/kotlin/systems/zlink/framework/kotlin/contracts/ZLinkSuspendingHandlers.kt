...
interface ZLinkSuspendingSpotRequestHandler<TSpot : Any, TRequest, TReply> {
    ...
    suspend fun handle(spot: TSpot, request: TRequest, context: ZLinkMessageContext): TReply =
        handle(spot, request)
}
...
interface ZLinkSuspendingSpotSubscriptionHandler<TSpot : Any, TEvent> {
    ...
    suspend fun handle(spot: TSpot, event: TEvent, context: ZLinkPublishMessageContext) =
        handle(spot, event)
}
...
interface ZLinkSuspendingSpotActorSendHandler<TSpot : ZLinkSpot<*>, TActor : ZLinkActor, TMessage> {
    suspend fun handle(spot: TSpot, actor: TActor, context: ZLinkMessageContext, message: TMessage)
}
...
abstract class ZLinkSuspendingActor : ZLinkActor {
    ...
    ): CompletionStage<Void> = coroutineVoidStage { onJoinCompletedSuspending(completion) }

    abstract suspend fun onJoinCompletedSuspending(completion: ZLinkActorJoinCompletion)
}
...
abstract class ZLinkSuspendingSpot<TActor : ZLinkActor> : ZLinkSpot<TActor> {
    ...
    final override fun onInitialize(): CompletionStage<Void> = coroutineVoidStage {
        onInitializeSuspending()
    }

    final override fun onClosing(context: ZLinkSpotClosingContext): CompletionStage<Void> =
        coroutineVoidStage {
    ...
    ): CompletionStage<ZLinkSpotActorJoinResult> = coroutineStage {
        onActorJoinSuspending(actorId, request)
    }

    final override fun onJoinedActor(actor: TActor): CompletionStage<Void> = coroutineVoidStage {
        onJoinedActorSuspending(actor)
    }

    final override fun onLeaveActor(actor: TActor): CompletionStage<Void> = coroutineVoidStage {
        onLeaveActorSuspending(actor)
    }

    final override fun onDisconnectActor(actor: TActor): CompletionStage<Void> =
        coroutineVoidStage {
            onDisconnectActorSuspending(actor)
        }
    ...
    protected open suspend fun onClosingSuspending(context: ZLinkSpotClosingContext) {}
...
abstract class ZLinkSuspendingEntrySpot<TActor : ZLinkActor> : ZLinkEntrySpot<TActor> {
    ...
    ): CompletionStage<ZLinkActorCreateResponse> = coroutineStage {
...
abstract class ZLinkSuspendingSession : ZLinkSession {
    ...
    final override fun onConnected(): CompletionStage<Void> = coroutineVoidStage {
        onConnectedSuspending()
    }

    final override fun onDisconnected(): CompletionStage<Void> = coroutineVoidStage {
    ...
    final override fun onActorBindingReplaced(actorId: String): CompletionStage<Void> =
        coroutineVoidStage {
            onActorBindingReplacedSuspending(actorId)
        }
