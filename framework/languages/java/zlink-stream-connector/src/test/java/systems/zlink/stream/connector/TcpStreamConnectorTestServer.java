...
final class TcpStreamConnectorTestServer implements Closeable {
                ...
                () -> {
                    try {
                        ...
                        return new ReceivedFrame(
                                ZLinkStreamWireProtocol.decodeHeader(header), payload);
                    } catch (IOException ex) {
                        throw new RuntimeException(ex);
                    }
                },
    ...
    CompletableFuture<Void> sendRawAsync(byte[] header, byte[] payload) {
        return CompletableFuture.runAsync(
                () -> {
                    try {
                        Socket socket = socket();
                        byte[] frame =
                                ZLinkStreamWireProtocol.encodeFrame(header, payload, 64 * 1024);
    ...
    ZLinkStreamConnectorOptions options(ZLinkStreamDispatchMode dispatchMode) {
        return new ZLinkStreamConnectorOptions(endpoint(), dispatchMode, Duration.ofSeconds(1), 1);
    }
    ...
    static ZLinkStreamWireProtocol.Header responseTo(
            ReceivedFrame request, String packetName, Map<String, String> metadata) {
