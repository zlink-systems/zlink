...
/**
 * Reconnect delay arithmetic, kept as pure functions so a test can drive the
 * randomness instead of sampling it.
 *
 * <p>Common connector spec §6: the base delay starts at the initial delay,
 * is multiplied by the backoff factor on each attempt and stops at the
 * maximum delay. The time actually waited is a value picked between 50% and
 * 100% of that base delay, so clients that were connected to one server do
...
final class ZLinkStreamReconnectDelay {
    ...
    /**
     * The base delay for the attempt after one that waited {@code current}.
     * Deterministic: jitter is applied by {@link #jittered} at wait time and
     * never feeds back into the base, so the growth curve stays predictable.
     */
    ...
    /**
     * The time to wait for one attempt: a value in
     * {@code [0.5 * base, base]}. {@code random} yields values in
     * {@code [0, 1)}.
     */
    static Duration jittered(Duration base, DoubleSupplier random) {
        ...
        double fraction = MIN_JITTER_FRACTION
            + (1.0 - MIN_JITTER_FRACTION) * clampUnit(random.getAsDouble());
