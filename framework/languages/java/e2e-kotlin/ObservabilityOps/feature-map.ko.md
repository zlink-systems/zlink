...
| OBS-A5 | public message-flow mode 전환, typed request handler, stdout evidence | 구현·compile·focused 실행 통과. typed `ProbeRequest`/`ProbeReply`가 기본 JSON codec에서 decode되고 `key_transitions -> off -> errors_only -> key_transitions` 전환과 standard logger evidence를 검증했다. 로그 디렉터리: `logs/20260806-054800-3449065-a5/` |
