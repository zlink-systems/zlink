...
final class ZLinkFrameworkCapabilityDelegates {
    ...
    private static final String SPOT_PUBLISHER_CLIENT_BEAN_NAME = "zlinkSpotPublisherClient";
    ...
    static void register(
        ...
        boolean hasSpotRuntime =
                !options.registration().spotNodes().isEmpty()
        ...
        boolean hasActorFactory =
                options.registration().spotNodes().stream()
                                .anyMatch(node -> !node.actorFactories().isEmpty())
                        || options.registration().meshNodes().stream()
                                .anyMatch(node -> !node.actorFactories().isEmpty());
        boolean hasObjectRole =
                options.registration().meshNodes().stream()
                        .anyMatch(node -> node.objectRoleEnabled());
        boolean hasSpotPublisherClient =
                options.registration().spotNodes().stream().anyMatch(node -> node.pubSubEnabled())
                        || options.registration().meshNodes().stream()
                                .anyMatch(node -> !node.channelNames().isEmpty());
        boolean hasLocationStore = options.registration().locations().enabled();

        if (hasSpotRuntime
                && !ZLinkSpringBeanDefinitions.hasBean(beanFactory, ZLinkSpotManager.class)) {
            ZLinkSpringBeanDefinitions.registerDelegate(
                    registry, SPOT_MANAGER_BEAN_NAME, ZLinkFrameworkSpotManagerBean.class);
        }
        if (hasSpotRuntime
                && !ZLinkSpringBeanDefinitions.hasBean(beanFactory, ZLinkSpotOutbound.class)) {
            ZLinkSpringBeanDefinitions.registerDelegate(
                    registry, SPOT_OUTBOUND_BEAN_NAME, ZLinkFrameworkSpotOutboundBean.class);
        }
        if (hasSpotPublisherClient
                && !ZLinkSpringBeanDefinitions.hasBean(
                        beanFactory, ZLinkSpotPublisherClient.class)) {
            ZLinkSpringBeanDefinitions.registerDelegate(
                    registry,
        ...
        if (hasSpotRuntime
            ...
            ZLinkSpringBeanDefinitions.registerDelegate(
                    registry, ACTOR_MANAGER_BEAN_NAME, ZLinkFrameworkActorManagerBean.class);
        }
        if (hasSpotRuntime
                && (hasActorFactory || hasLocationStore)
                && !ZLinkSpringBeanDefinitions.hasBean(beanFactory, ZLinkActorDirectory.class)) {
            ZLinkSpringBeanDefinitions.registerDelegate(
                    registry, ACTOR_DIRECTORY_BEAN_NAME, ZLinkFrameworkActorDirectoryBean.class);
        }
        if (hasSpotRuntime
                && hasLocationStore
                && !ZLinkSpringBeanDefinitions.hasBean(beanFactory, ZLinkActorClient.class)) {
            ZLinkSpringBeanDefinitions.registerDelegate(
                    registry, ACTOR_CLIENT_BEAN_NAME, ZLinkFrameworkActorClientBean.class);
        }
    }
}
