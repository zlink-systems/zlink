...
final class ZLinkRequestMetricsTest {
    ...
    void inflightSurvivesProviderAttachmentAndTerminalOutcomesAreExact() throws Exception {
        ...
        try (ZLinkRequestMetricProbe probe = ZLinkRequestMetricProbe.install()) {
            ...
            assertEquals(0L, probe.durationCount("late-mesh", "node", "completed"));

            assertOutcome(
                    probe,
                    ZLinkRequestMetrics.channel("outcomes"),
                    "outcomes",
                    "channel",
                    null,
                    "completed",
            ...
            assertOutcome(
                    probe,
                    ZLinkRequestMetrics.spot("outcomes"),
                    "outcomes",
                    "spot",
            ...
            assertOutcome(
                    probe,
                    ZLinkRequestMetrics.instanceSpot("outcomes"),
                    "outcomes",
                    "instance_spot",
                    new ZLinkFrameworkException(
                            ZLinkFrameworkErrorKind.DEADLINE_EXCEEDED, "deadline"),
                    "timed_out",
                    true);
            assertOutcome(
                    probe,
                    ZLinkRequestMetrics.actor("outcomes"),
                    "outcomes",
                    "actor",
            ...
            assertOutcome(
                    probe,
                    ZLinkRequestMetrics.actor("transport-outcome"),
                    "transport-outcome",
                    "actor",
                    new RuntimeException(new ZlinkRequestException(RequestResult.TIMED_OUT)),
                    "timed_out",
                    true);
        }
    }
    ...
    private static void assertOutcome(
        ...
        assertEquals(timedOut ? 1L : 0L, probe.timeoutCount(meshName, surface));
    }
}
