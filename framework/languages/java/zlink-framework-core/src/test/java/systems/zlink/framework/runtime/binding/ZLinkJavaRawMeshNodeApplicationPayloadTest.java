...
final class ZLinkJavaRawMeshNodeApplicationPayloadTest {
    ...
    void typedActorRequestUsesFrameworkMultipartAndRestoresAllParts() {
        byte[] body = "{\"actorId\":\"actor-1\"}".getBytes(StandardCharsets.UTF_8);
        ...
        try (Message header = Message.from(ZLinkStreamHeaderCodec.encode(request));
            ...
            var wirePayload = ZLinkJavaRawMeshNode.applicationPayload(List.of(header, payload));

            assertEquals(
                    ServiceWireConstants.FRAMEWORK_MULTIPART_PACKET_NAME, wirePayload.packetName());
            List<Message> decoded =
                    systems.zlink.framework.runtime.internal.service.ZLinkServiceM6AWireCodec
                            .decodeFrameworkMultipart(wirePayload);
            try {
                assertArrayEquals(header.toByteArray(), decoded.getFirst().toByteArray());
            ...
            List<Message> restored =
                    ZLinkJavaRawMeshNode.applicationMessages(wirePayload, true, 7L);
            try {
                ZLinkStreamHeader restoredHeader =
                        ZLinkStreamHeaderCodec.decodeOrPlain(restored.getFirst().toByteArray());
                assertEquals(ZLinkStreamMessageKind.REQUEST, restoredHeader.kind());
    ...
    void plainTwoPartMessageKeepsItsPacketName() {
        try (Message packetName = Message.from("plain.packet");
            ...
            var wirePayload = ZLinkJavaRawMeshNode.applicationPayload(List.of(packetName, payload));
            ...
            try {
                ...
                assertArrayEquals(new byte[] {1, 2, 3}, decoded.get(1).toByteArray());
            } finally {
                decoded.forEach(Message::close);
            }
        }
        ...
        try (Message header = Message.from(ZLinkStreamHeaderCodec.encode(request));
            ...
            List<Message> restored =
                    ZLinkJavaRawMeshNode.applicationMessages(
                            ZLinkJavaRawMeshNode.applicationPayload(List.of(header, payload)),
                            true,
                            11L);
            try {
                ZLinkStreamHeader restoredHeader =
