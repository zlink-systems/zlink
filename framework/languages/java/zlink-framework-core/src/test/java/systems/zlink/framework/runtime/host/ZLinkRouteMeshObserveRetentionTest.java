...
/**
 * {@code observe(mesh, capacity).subscribe(subscriber)} is the natural call shape: nothing outside
 * the runtime keeps the returned publisher, and the subscriber drops its Subscription. The
 * observation has to keep working after the collector runs.
 */
final class ZLinkRouteMeshObserveRetentionTest {
    private static final String MESH = "gc-observed-mesh";

    @Test
    void meshObservationKeepsDeliveringAfterGarbageCollection() throws Exception {
        DefaultZLinkFrameworkOptions options = new DefaultZLinkFrameworkOptions();
        options.addRouteMesh(MESH).listen("inproc://" + MESH + "-" + UUID.randomUUID());
        ...
        try {
            ...
            runtime.shutdown(Duration.ofSeconds(2)).toCompletableFuture().get(5, TimeUnit.SECONDS);
            awaitAtLeast(
                    delivered,
                    beforeCollection + 1,
    ...
    private static void awaitReady(ZLinkFrameworkRuntime runtime) throws InterruptedException {
    ...
    /**
     * Runs the collector until a sentinel is proven cleared, so the assertion below never depends
     * on a hint that the runtime was free to ignore.
     */
    ...
    private static void awaitAtLeast(AtomicInteger counter, int expected, String what)
        ...
        assertTrue(
                ...
                "timed out waiting for " + what + " (observed " + counter.get() + ")");
    }
}
