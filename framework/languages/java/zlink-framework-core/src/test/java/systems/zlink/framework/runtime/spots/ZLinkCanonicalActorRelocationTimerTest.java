    ...
    void standaloneActorRootPreservesLogicalTimers() {
        var timer =
                new ZLinkSpotTimerRelocationEnvelope.CanonicalTimer(
                        ...
                        9,
                        123456,
                        new ZLinkSpotTimerRelocationEnvelope.CanonicalPending(8, 10, 123456, 2));
        byte[] timerEnvelope = ZLinkSpotTimerRelocationEnvelope.encodeCanonical(List.of(timer));
        ...
        byte[] root =
                ZLinkCanonicalActorRelocationEnvelope.encode(
                        ...
                        timerEnvelope);
        var decoded =
                ZLinkCanonicalActorRelocationEnvelope.decode(root, relocationId, "actor-a", true);

        assertArrayEquals(timerEnvelope, decoded.timerEnvelope());
        assertEquals(
                1, ZLinkSpotTimerRelocationEnvelope.canonicalize(decoded.timerEnvelope()).size());
    }
