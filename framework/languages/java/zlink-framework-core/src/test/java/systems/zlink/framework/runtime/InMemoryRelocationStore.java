...
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationMissing;
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationReadResult;
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationRenewMissing;
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationRenewResult;
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationRenewed;
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationStore;
import systems.zlink.framework.runtime.internal.locations.ZLinkRelocationStored;
...
public final class InMemoryRelocationStore
    ...
    private final AtomicReference<PutGate> nextInternalPut = new AtomicReference<>();
    ...
    public CompletionStage<ZLinkRelocationStored> put(
        ...
        if (gate != null) {
            ...
            return gate.release().thenCompose(ignored -> put(payload, retention, cancellation));
        }
        ...
        return CompletableFuture.completedFuture(
                new ZLinkRelocationStored(reference, checksum.getValue(), expiresAt, now));
    }

    public void gateNextInternalPut(
            CompletableFuture<Void> entered, CompletableFuture<Void> release) {
        if (!nextInternalPut.compareAndSet(null, new PutGate(entered, release))) {
            throw new IllegalStateException("an internal relocation put gate is already installed");
        }
    }
    ...
    public CompletionStage<ZLinkBlobPutResult> put(
        ...
        if (current == null) {
            return CompletableFuture.completedFuture(new ZLinkBlobStored(expiresAt, now));
        }
        return CompletableFuture.completedFuture(
                Arrays.equals(current.payload(), payload)
                        ? new systems.zlink.framework.locationprovider.ZLinkBlobAlreadyStored(
                                current.expiresAt(), now)
                        : new systems.zlink.framework.locationprovider.ZLinkBlobConflict(now));
    }
    ...
    public CompletionStage<ZLinkBlobReadResult> read(
        ...
        if (entry == null || !entry.expiresAt().isAfter(now)) {
            ...
            return CompletableFuture.completedFuture(new ZLinkBlobMissing(now));
        }
        return CompletableFuture.completedFuture(
                new ZLinkBlobFound(entry.payload(), entry.expiresAt(), now));
    }
    ...
    public CompletionStage<ZLinkBlobRenewResult> renew(
        ...
        if (entry == null) {
            return CompletableFuture.completedFuture(
                    new systems.zlink.framework.locationprovider.ZLinkBlobRenewMissing(now));
        }
        Instant expiresAt = now.plus(retention);
        values.put(reference.value(), new Entry(entry.payload(), expiresAt));
        return CompletableFuture.completedFuture(new ZLinkBlobRenewed(expiresAt, now));
    }
    ...
    public CompletionStage<ZLinkRelocationReadResult> get(
        ...
        if (entry == null || !entry.expiresAt().isAfter(now)) {
            ...
            return CompletableFuture.completedFuture(new ZLinkRelocationMissing());
        }
        return CompletableFuture.completedFuture(new ZLinkRelocationFound(entry.payload()));
    }
    ...
    public CompletionStage<ZLinkRelocationRenewResult> renew(
        ...
        Entry entry = values.get(reference);
        if (entry == null) {
            return CompletableFuture.completedFuture(new ZLinkRelocationRenewMissing());
        }
    ...
    private record PutGate(CompletableFuture<Void> entered, CompletableFuture<Void> release) {}
}
