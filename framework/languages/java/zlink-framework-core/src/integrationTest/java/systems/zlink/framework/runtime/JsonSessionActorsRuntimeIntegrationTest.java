...
final class JsonSessionActorsRuntimeIntegrationTest {
    ...
    void sessionGateway_relaysJsonActorSendWithDefaultPacketName() throws Exception {
        ...
        try (ZLinkFrameworkRuntime runtime = startLocalJsonRuntime()) {
            ...
            assertEquals(
                    actorId + ":json-hello",
                    awaitActorRelay(actorId + ":json-hello", 2, TimeUnit.SECONDS));
        }
    }
    ...
    private static ZLinkFrameworkRuntime startLocalJsonRuntime() {
        ...
        options.addLocationStore(new ZLinkInMemoryLocationStore());
        {
            ...
            node.listen("inproc://json-play-" + System.nanoTime())
                    .setRoutingId(RoutingId.from("play-node"));
            node.objects()
                    .server()
                    .addSpotFactory(
                            "SessionActorsRuntimeIntegrationTest.GameSpot",
                            SessionActorsRuntimeIntegrationTest.GameSpot.class,
                            factory -> factory.disableRelocation());
            node.objects()
                    .server()
                    .addEntrySpot(SessionActorsRuntimeIntegrationTest.GameEntrySpot.class);
            node.objects()
                    .server()
                    .addActorFactory(
                            "player",
    ...
    public static final class DefaultJsonActorSendHandler {
        ...
        public CompletionStage<Void> handle(
                SessionActorsRuntimeIntegrationTest.PlayerActor actor, JsonRelaySend request) {
    ...
    private static String awaitActorRelay(String expected, long timeout, TimeUnit unit)
        ...
        while (true) {
            ...
            String received =
                    SessionActorsRuntimeIntegrationTest.actorRelayRequests.poll(
