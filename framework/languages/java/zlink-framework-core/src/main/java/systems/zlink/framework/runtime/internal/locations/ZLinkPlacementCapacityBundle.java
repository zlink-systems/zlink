...
public record ZLinkPlacementCapacityBundle(
        int actorSlots, int spotSlots, Optional<ZLinkSpotTypeCapacityDelta> spotType) {
    public ZLinkPlacementCapacityBundle {
        if (actorSlots < 0 || spotSlots < 0) {
            throw new IllegalArgumentException("capacity bundle slots must not be negative");
        }
        ...
        spotType.ifPresent(
                delta -> {
                    if (delta.slots() != spotSlots) {
                        throw new IllegalArgumentException("spot type slots must equal spotSlots");
                    }
                });
    }

    public static ZLinkPlacementCapacityBundle actor(int slots) {
        return new ZLinkPlacementCapacityBundle(slots, 0, Optional.empty());
    }
    ...
    public static ZLinkPlacementCapacityBundle spot(
        ...
        return new ZLinkPlacementCapacityBundle(
                0,
                slots,
                Optional.of(new ZLinkSpotTypeCapacityDelta(objectKind, stableType, slots)));
    }
}
