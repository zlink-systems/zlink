...
final class ZLinkJavaRouterSocket implements ZLinkBackendRouterSocket, ZLinkJavaSocketBacked {
    ...
    @Override
    public synchronized void setConnectRoutingId(RoutingId routingId) {
        socket.options().setConnectRoutingId(routingId);
    }

    @Override
    public synchronized void setProbe(boolean enabled) {
        socket.options().probe(enabled);
    }

    @Override
    public synchronized long maxMessageSize() {
        return Math.max(0, socket.options().maxMessageSize());
    }

    @Override
    public synchronized void setMaxMessageSize(long value) {
        socket.options().maxMessageSize(value == 0 ? -1 : value);
    }

    @Override
    public synchronized int peerWeight() {
        return socket.options().peerWeight();
    }

    @Override
    public synchronized void setPeerWeight(int weight) {
        socket.options().peerWeight(weight);
    }

    @Override
    public synchronized String lastEndpoint() {
    ...
    @Override
    public synchronized CompletionStage<Void> send(RoutingId routingId, List<Message> parts) {
    ...
    public synchronized CompletionStage<ZLinkBackendReceived> request(
        ...
        return ZLinkJavaSocketSupport.submitRequest(socket.request(routingId), parts, timeout);
    }

    @Override
    public synchronized void reply(RoutingId routingId, long requestSeq, List<Message> parts) {
        throw new UnsupportedOperationException(
                "native ROUTER replies require the opaque ReplyToken retained by the receive"
                        + " record");
    }

    @Override
    public synchronized void disconnectPeer(RoutingId routingId) {
