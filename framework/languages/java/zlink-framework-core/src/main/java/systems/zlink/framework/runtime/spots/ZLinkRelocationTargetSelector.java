...
final class ZLinkRelocationTargetSelector {
    ...
    static ZLinkMeshNodeDescriptor select(
        ...
        List<ZLinkMeshNodeDescriptor> candidates = descriptors.stream().filter(base).toList();
        requireCandidates(
                candidates, unavailableMessage, ZLinkFrameworkRelocationReason.TARGET_UNAVAILABLE);
        candidates = candidates.stream().filter(policy::acceptsVersion).toList();
        requireCandidates(
                candidates, unavailableMessage, ZLinkFrameworkRelocationReason.TARGET_UNAVAILABLE);
        candidates = candidates.stream().filter(capability).toList();
        requireCandidates(
                candidates, unavailableMessage, ZLinkFrameworkRelocationReason.STATE_INCOMPATIBLE);
        candidates = candidates.stream().filter(policy::acceptsWave).toList();
        requireCandidates(
                candidates, unavailableMessage, ZLinkFrameworkRelocationReason.TARGET_UNAVAILABLE);
        ...
        requireCandidates(
                candidates, unavailableMessage, ZLinkFrameworkRelocationReason.TARGET_UNAVAILABLE);
        candidates =
                candidates.stream().filter(candidate -> candidate.placementWeight() > 0).toList();
        requireCandidates(
                candidates, unavailableMessage, ZLinkFrameworkRelocationReason.TARGET_UNAVAILABLE);
        long total =
                candidates.stream()
    ...
    private static void requireCandidates(
        ...
        if (candidates.isEmpty()) {
            throw new ZLinkUserSpotRetireRuntime.RelocationBlockedException(reason, message);
        }
    }
}
