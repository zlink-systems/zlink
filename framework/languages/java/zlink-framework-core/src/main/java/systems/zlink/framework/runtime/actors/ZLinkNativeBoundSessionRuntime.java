...
final class ZLinkNativeBoundSessionRuntime implements ZLinkBoundSession {
    ...
    public CompletionStage<Void> disconnect() {
        return CompletableFuture.runAsync(
                        () -> spotNode.closeActorBoundSession(currentActorRef(), timeout))
    ...
    private static CompletionStage<Void> submitBoundSessionFrame(
        ...
        if (spotNode.hasRemoteActorBoundSessionRoute(actorRef)) {
            ...
            try {
                submission = spotNode.sendRemoteActorBoundSession(actorRef, List.of(frame));
        ...
        if (!spotNode.hasLocalActorBoundSessionRoute(actorRef)) {
            frame.close();
            return ZLinkOneWayCalls.oneWayStatus(ZLinkOneWayCalls.TARGET_NOT_FOUND);
        }
        CompletionStage<Void> submission;
        try {
            submission =
