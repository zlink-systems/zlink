...
final class ZLinkActorTransferRegistry {
    ...
    CompletionStage<TransferState> transferOut(String actorType, ZLinkActor actor) {
        Class<? extends ZLinkActorRelocationAdapter<?>> adapterType = adapters.get(actorType);
        if (adapterType == null) {
            return CompletableFuture.completedFuture(new TransferState(null, ZLinkMessage.empty()));
        }
                        ...
                        state -> {
                            if (state == null) {
                                throw new ZLinkConfigurationException(
                                        "actor transfer adapter returned a null state: "
                                                + adapterType.getName());
                            }
    ...
    CompletionStage<ZLinkActor> transferIn(
        ...
        ZLinkActorFactory factory = (ZLinkActorFactory) handlerFactory.create(factoryType);
                ...
                .thenCompose(
                        actor ->
                                invokeRestore(
                                                ...
                                                actor,
                                                state.decode(byte[].class))
                                        .thenApply(ignored -> actor))
                .thenApply(
                        actor -> {
                            if (actor == null || !actorId.equals(actor.context().actorId())) {
                                throw new ZLinkConfigurationException(
                                        "actor transfer adapter returned an actor with a different"
                                                + " id: "
                                                + adapterType.getName());
                            }
                            return actor;
    ...
    private static CompletionStage<byte[]> invokeCapture(
            ZLinkActorRelocationAdapter adapter, ZLinkActor actor) {
        return adapter.capture(actor, () -> false);
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    private static CompletionStage<Void> invokeRestore(
            ZLinkActorRelocationAdapter adapter, ZLinkActor actor, byte[] state) {
