...
final class DefaultZLinkIoWorkerCall<T> implements ZLinkWorkerCall<T> {
    ...
    public CompletionStage<T> submit() {
                                    ...
                                    if (settled.compareAndSet(false, true)) {
                                        ...
                                        result.completeExceptionally(
                                                new ZLinkWorkerTimeoutException(
                                                        "I/O worker call timed out after "
                                                                + timeout));
                                    }
                                },
                                timeout);
        ...
        try {
            CompletionStage<T> stage =
                    Objects.requireNonNull(work.run(cancellation), "I/O worker result");
            stage.whenComplete(
                    (value, error) -> {
                        if (!settled.compareAndSet(false, true)) {
                            return;
                        ...
                        if (error instanceof CancellationException
                            ...
                            result.completeExceptionally(
                                    new ZLinkWorkerFailedException(
                                            "I/O worker call failed", error));
                        } else {
    ...
    public CompletionStage<T> yield() {
        systems.zlink.framework.runtime.internal.handlers.ZLinkSuspendInvocationContext
                .requireYieldAllowed("I/O worker");
