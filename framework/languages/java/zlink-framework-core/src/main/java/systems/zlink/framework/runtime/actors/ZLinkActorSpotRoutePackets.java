...
public final class ZLinkActorSpotRoutePackets {
        ...
        return Message.from(
                String.join(
                                ...
                                relocationManifest == null
                                        ? ""
                                        : Base64.getEncoder()
                                                .encodeToString(relocationManifest.root()),
                                relocationManifest == null
                                        ? "0"
                                        : Long.toUnsignedString(
                                                relocationManifest.checksumCrc32c()),
                                relocationManifest == null
                                                || relocationManifest.rawReply().length == 0
                                        ? ""
                                        : Base64.getEncoder()
                                                .encodeToString(relocationManifest.rawReply()),
                                relocationManifest == null
                                        ? "0"
                                        : Long.toUnsignedString(
                                                relocationManifest.operationIdHigh()),
                                relocationManifest == null
                                        ? "0"
                                        : Long.toUnsignedString(
                                                relocationManifest.operationIdLow()),
                                relocationManifest == null
                                        ? "0"
                                        : Long.toUnsignedString(
                                                relocationManifest.aggregateIdHigh()),
                                relocationManifest == null
                                        ? "0"
                                        : Long.toUnsignedString(
                                                relocationManifest.aggregateIdLow()),
                                relocationManifest == null
                                        ? "0"
                                        : Long.toUnsignedString(
                                                relocationManifest.aggregateGeneration()),
                                sessionRouteCommand44 == null || sessionRouteCommand44.length == 0
                                        ? ""
                                        : Base64.getEncoder().encodeToString(sessionRouteCommand44),
                                Long.toUnsignedString(actorNodeGeneration),
                                Long.toUnsignedString(authorityOwnerGeneration),
                                Long.toUnsignedString(ownerLeaseGeneration))
        ...
        try {
            return new TransferRequest(
                    ...
                    !fields[21].isBlank()
                            ...
                            : null,
                    !fields[29].isBlank() ? Base64.getDecoder().decode(fields[29]) : new byte[0],
                    Long.parseUnsignedLong(fields[30]),
                    Long.parseUnsignedLong(fields[31]),
                    Long.parseUnsignedLong(fields[32]));
        } catch (IllegalArgumentException error) {
            throw new ZLinkFrameworkException(
                    ZLinkFrameworkErrorKind.PROTOCOL_ERROR,
                    "invalid actor Spot transfer request",
                    error);
        }
    }
            ...
            backlog.add(
                    new WireHandoffPacket(
                            ...
                            ZLinkStreamHeaderCodec.decodeOrPlain(
                                    parts.get(offset + 2).toByteArray()),
    ...
    /**
     * Encodes the actor join admission outcome as the schema's canonical actor-join-reply-tail
     * (service-wire-v1.schema.json:2251-2294) instead of a delimited-text dialect. Returns two
     * parts: the binary tail ({@code trailingBytes: forbidden} on the tail forbids appending the
     * reply payload after it) and the admission reply payload as its own part. {@code
     * spotId}/{@code spotGeneration}/{@code membershipEpoch} are only meaningful (and required
     * nonzero) when {@code accepted}.
     */
    public static List<Message> encodeAdmissionReply(
        ...
        try {
            ...
            throw new ZLinkFrameworkException(
                    ZLinkFrameworkErrorKind.PROTOCOL_ERROR,
                    "invalid actor Spot admission reply",
                    failure);
        }
        return List.of(
    ...
    /**
     * Decodes an admission reply encoded by {@link #encodeAdmissionReply}: a two-part binary tail
     * plus its reply payload. Strict binary decode - the earlier delimited-text dialect (including
     * its two-field mixed- version tolerance) no longer exists on the wire.
     */
    ...
    public static List<Message> createBoundSessionSendParts(
            ZLinkBackendActorRef actorRef, Message frame) {
        return List.of(
    ...
    public static List<Message> createActorPacketParts(
        ...
        return createActorPacketParts(
                actorRef, header, payload, replyRoute, handoffArrivalIndex, new byte[0]);
    }
    ...
    public static List<Message> createActorPacketParts(
        ...
        return List.of(
                ...
                Message.from(
                        (handoffArrivalIndex == null ? "" : Long.toString(handoffArrivalIndex))
                                .getBytes(StandardCharsets.UTF_8)),
                Message.from(acceptedJournalRecord == null ? new byte[0] : acceptedJournalRecord));
    }
    ...
    public static BoundSessionSend decodeBoundSessionSend(List<Message> parts) {
        if (parts.size() < 3
            ...
            throw new ZLinkFrameworkException(
                    ZLinkFrameworkErrorKind.PROTOCOL_ERROR,
                    "invalid routed actor bound session send packet");
        }
    ...
    private static Message encodeReplyRoute(ZLinkActorReplyRoute route) {
        ...
        return Message.from(
                String.join(
                                ...
                                Integer.toUnsignedString(route.flags()))
                        .getBytes(StandardCharsets.UTF_8));
    }
    ...
    private static ZLinkActorReplyRoute decodeReplyRoute(Message message) {
        ...
        return new ZLinkActorReplyRoute(
                new ZLinkBackendActorRef(
                        RoutingId.from(fields[0]), fields[1], Long.parseUnsignedLong(fields[2])),
    ...
    public static Message encodeJoinReply(
            boolean accepted, ZLinkBackendActorRef actorRef, Message reply) {
        String encodedReply =
                reply == null ? "" : Base64.getEncoder().encodeToString(reply.toByteArray());
        return Message.from(
                String.join(
                                "\n",
    ...
    public record TransferRequest(
        ...
        public TransferRequest {
            sessionRouteCommand44 =
                    sessionRouteCommand44 == null ? new byte[0] : sessionRouteCommand44.clone();
        }

        @Override
        public byte[] sessionRouteCommand44() {
            return sessionRouteCommand44.clone();
        }

        public TransferRequest withActorType(String stableType) {
            return new TransferRequest(
                    phase,
                    transferId,
                    timeoutMillis,
    ...
    public record BoundSessionSend(ZLinkBackendActorRef actorRef, Message frame)
            implements AutoCloseable {
        @Override
        public void close() {
            frame.close();
