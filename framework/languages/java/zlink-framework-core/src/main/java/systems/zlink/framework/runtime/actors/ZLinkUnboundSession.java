 ...
 *
 * <p>Spec 04-actor-model §8.1: an operation needing a bound session with no
 * valid binding ends with {@code InvalidOperation}, and the failure surfaces at
 * the call's terminal like every other call failure — not thrown by the
 * accessor that creates the call. Returning this instead of throwing from
 * {@code boundSession()} keeps the JVM surface the same as the other languages,
 * where {@code exceptionally} observes the failure and no {@code try} is
 * needed around the accessor.
 */
