 ...
 *
 * <p>21-location-runtime.md#2.4: the reference has the form
 * {@code inline-v1:{base64url}}, where {@code {base64url}} encodes the creation
 * request bytes over the alphabet {@code A-Z a-z 0-9 - _} with no {@code =}
 * padding. No other form is recognized. The same atomic record's
 * {@code requestEncodedSize} and {@code requestSha256} decide the content's
...
public final class ZLinkInlineCreationContentCodec {
     ...
     * {@code requestEncodedSize} and {@code requestSha256}. A reference outside
     * the specified form, a length mismatch or a digest mismatch all throw, and
     * the caller must record the creation as failed without running the
     * factory.
     */
    public static byte[] decode(
        String reference,
        byte[] expectedSha256,
        int expectedEncodedSize) {
