...
final class ZLinkSpotRelocationReplyRoutes {
    ...
    Registration register(
        ...
        OperationId operation = new OperationId(frozen.operationHigh(), frozen.operationLow());
        ...
        if (reply == null) {
            ...
            throw new IllegalStateException("accepted request has no source reply capability");
        }
        inStateLane(
                () -> {
                    if (routes.get(plan.operation()) == plan.route()) {
    ...
    private RegistrationPlan registerOnLane(
        ...
        if (routes.size() >= MAX_ROUTES) {
            throw new IllegalStateException("Spot relocation reply-route registry is full");
        }
        Route route =
                new Route(
                        spotId,
        ...
        return new RegistrationPlan(
                operation, route, new Registration(this, operation, relocationRelease, true));
    }
    ...
    LazyRegistration registerLazy(
        ...
        return new LazyRegistration(
                acceptedRecord, record -> register(record, received, spotId, objectGeneration));
    }
    ...
    private Registration registerActorOnLane(
        ...
        if (!frozen.actorId().equals(actorId) || frozen.objectGeneration() != objectGeneration) {
            throw new IllegalArgumentException(
                    "accepted Actor reply capability differs from its object fence");
        }
        OperationId operation = new OperationId(frozen.operationHigh(), frozen.operationLow());
        if (frozen.replyRouteId().isEmpty()) {
            return new Registration(this, operation, relocationRelease, false);
        }
        removeExpired(Instant.now());
        if (routes.size() >= MAX_ROUTES) {
            throw new IllegalStateException("Actor relocation reply-route registry is full");
        }
        Route route =
                new Route(
                        actorId,
        ...
        return new Registration(this, operation, relocationRelease, true);
    }
    ...
    private void bindCommittedOnLane(
        ...
        if (targetNodeGeneration == 0 || targetAttemptGeneration <= 0) {
            throw new IllegalArgumentException(
        ...
        for (byte[] acceptedRecord : acceptedRecords) {
            ...
            if (route == null
                    ...
                    || route.sourceOwnerLeaseGeneration != frozen.sourceOwnerLeaseGeneration()
                    || route.committed
                            && (!targetNodeRid.equals(route.targetNodeRid)
    ...
    private void attachCanonicalFence(
            List<byte[]> acceptedRecords, boolean actor, CommittedFence fence) {
        ...
        for (byte[] acceptedRecord : acceptedRecords) {
            ...
            Route route = routes.get(new OperationId(operationHigh, operationLow));
    ...
    CanonicalRoute lookupCanonical(
            ZLinkServiceRelocationWireCodec.ReplyRelay relay, RoutingId transportSource) {
    ...
    private CanonicalRoute lookupCanonicalOnLane(
        ...
        removeExpired(Instant.now());
        Route route =
                routes.get(new OperationId(relay.operation().high(), relay.operation().low()));
        if (route == null
                || !route.committed
                || route.authorityKey == null
                || route.participantId == 0
                || route.replyRouteId != relay.replyRouteId()
                || route.participantId != relay.participantId()
                || route.targetAttemptGeneration != relay.targetAttemptGeneration()
                || !transportSource.equals(route.targetNodeRid)
                || !transportSource.equals(relay.coordinator().nodeRid())
                || route.targetNodeGeneration != relay.coordinator().nodeGeneration()) {
            return null;
        }
    ...
    CompletionStage<Ack> deliverCanonical(CanonicalRoute selected, List<byte[]> parts) {
        ...
        RouteSelection selection =
                inStateLane(
                        () -> {
                            ...
                            if (route == null
                                    || !route.committed
                                    || !Objects.equals(route.authorityKey, selected.authorityKey())
        ...
        try {
            delivery =
                    Objects.requireNonNull(
                            route.delivery.deliver(parts), "relocation reply delivery result");
        } catch (RuntimeException failure) {
            inStateLane(
                    () -> {
    ...
    record OperationId(long high, long low) {
        OperationId {
            if (high == 0 && low == 0) {
                throw new IllegalArgumentException("operation identity must not be zero");
            }
        }
    }

    record CommittedFence(String authorityKey, long participantId, long targetAttemptGeneration) {
        CommittedFence {
            Objects.requireNonNull(authorityKey, "authorityKey");
            if (authorityKey.isBlank() || participantId == 0 || targetAttemptGeneration == 0) {
                throw new IllegalArgumentException("canonical committed reply fence is invalid");
            }
        }
    }
    ...
    record Relay(
        ...
        Relay {
            ...
            if (replyRouteId == 0
                ...
                throw new IllegalArgumentException("reply relay fence contains an invalid value");
            }
        }
    ...
    static final class LazyRegistration {
        ...
        private LazyRegistration(
                Supplier<byte[]> recordSupplier, Function<byte[], Registration> registration) {
            this.recordSupplier = Objects.requireNonNull(recordSupplier, "recordSupplier");
        ...
        byte[] record() {
            ...
            byte[] supplied =
                    Objects.requireNonNull(
                            recordSupplier.get(), "accepted record supplier returned null");
