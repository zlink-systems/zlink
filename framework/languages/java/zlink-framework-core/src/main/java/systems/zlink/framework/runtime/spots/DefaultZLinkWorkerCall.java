...
/**
 * Runtime {@code runWorker(...)} call. The work runs on the shared elastic worker pool; the
 * awaitable terminator completes the returned stage, and the callback terminator posts both
 * callbacks to the owning Spot serial line. A late result after a timeout is dropped without
...
final class DefaultZLinkWorkerCall<T> implements ZLinkWorkerCall<T> {
    ...
    DefaultZLinkWorkerCall(ZLinkWorkerPool pool, ZLinkWorkerTask<T> work) {
    ...
    public CompletionStage<T> yield() {
        systems.zlink.framework.runtime.internal.handlers.ZLinkSuspendInvocationContext
                .requireYieldAllowed("CPU worker");
        return ZLinkSerialExecutionQueue.yieldCurrent(submit());
    }

    private void start(CompletableFuture<T> result) {
        AtomicBoolean settled = new AtomicBoolean();
        DefaultZLinkWorkerCancellation cancellation = new DefaultZLinkWorkerCancellation();
                                ...
                                () -> {
                                    if (settled.compareAndSet(false, true)) {
                                        ...
                                        result.completeExceptionally(
                                                new ZLinkWorkerTimeoutException(
                                                        "worker call timed out after " + timeout));
                                    }
                                },
                                timeout);
        AtomicReference<Runnable> unregisterShutdown = new AtomicReference<>(() -> {});
        result.whenComplete(
                (ignored, error) -> {
        ...
        unregisterShutdown.set(
                pool.registerShutdownListener(
                        () -> {
                            result.cancel(false);
                            cancellation.cancel();
                        }));
