 ...
 * (service-wire-v1.schema.json:2251-2294, tail at :5387-5397). This is a structural codec, not a
 * byte-preserver: decode parses every field into {@link ActorJoinReply} and encode reconstructs the
 * wire bytes from those fields, so a decode + re-encode round trip is a genuine field-for-field
 * verification rather than a pass-through of the original buffer.
 *
 * <p>{@link #encodeTail}/{@link #decodeTail} expose just the {@code join} tail body (joinResult +
 * case fields, service-wire-v1.schema.json:5387- 5397) without the surrounding reply(20) frame
 * (prefix, correlation, terminalResult, failureCode) for transports - such as the routed actor Spot
 * admission reply - that carry the same tail bytes outside a full service-wire reply(20) frame.
 */
public final class ZLinkCanonicalActorJoinReplyCodec {
    ...
    public record Tail(
            int joinResult, SpotRef spot, Long membershipEpoch, Long receiveChunkLimitBytes) {
    ...
    public byte[] encode(ActorJoinReply reply) {
        ...
        if (reply.terminalResult() != 0
                || !ServiceWireConstants.validTerminalFailure(
                        reply.terminalResult(), reply.failureCode())) {
            throw invalid("terminal/failure pair");
        }
        byte[] tail =
                encodeTail(
    ...
    /**
     * Encodes just the {@code join} tail (joinResult + bodyLength + case fields, no reply(20) frame
     * prefix/correlation/terminal/failure).
     */
    public byte[] encodeTail(Tail tail) {
        ...
        if (tail.joinResult() == JOIN_RESULT_ACCEPTED) {
            ...
            long membershipEpoch =
                    Objects.requireNonNull(tail.membershipEpoch(), "membershipEpoch");
            long receiveChunkLimitBytes =
                    Objects.requireNonNull(tail.receiveChunkLimitBytes(), "receiveChunkLimitBytes");
    ...
    private static final class Reader {
        ...
        long u32Unsigned() {
            return Integer.toUnsignedLong(u8() << 24 | u8() << 16 | u8() << 8 | u8());
        }

        long u64() {
            return u32Unsigned() << 32 | u32Unsigned();
        }
