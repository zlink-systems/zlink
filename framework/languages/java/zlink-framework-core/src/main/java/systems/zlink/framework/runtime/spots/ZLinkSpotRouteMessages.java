...
/**
 * SPOT route request/send/publish wire codec on the shared cross-language envelope: {@code [JSON
 * header, body]} with content type, application metadata and the flow pair carried as header fields
 * (no separate content-type/flow frames). A call without a packet name stays a bare single-part
...
final class ZLinkSpotRouteMessages {
    ...
    /**
     * One-way send envelope (kind 3). The flow state is an explicitly passed value (R1
     * value-passing): it is consumed only here at encode time, so callers never install a scope or
     * wrap the stage they return, which keeps the spot dispatch lane's turn stage a bare admission
     * future.
     */
        ...
        try {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                        ex.getMessage() + " (spot route reply parts=" + describe(replyParts) + ")",
                        ex);
            }
        } finally {
            if (emptyReply != null) {
