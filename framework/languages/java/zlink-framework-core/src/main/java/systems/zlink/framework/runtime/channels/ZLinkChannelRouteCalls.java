...
final class RouteSendCall implements ZLinkSendCall {
    ...
    public ZLinkSendCall metadata(String key, String value) {
        return new RouteSendCall(
                ...
                packetName,
                contentType,
                (metadata == null ? ZLinkApplicationMetadata.empty() : metadata).with(key, value),
                submitGate);
    }
    ...
    public ZLinkSendCall metadata(Map<String, String> values) {
        return new RouteSendCall(
                ...
                packetName,
                contentType,
                (metadata == null ? ZLinkApplicationMetadata.empty() : metadata).withAll(values),
                submitGate);
    }
    ...
    private CompletionStage<Void> submitNode(ZLinkInternalSpotNode node) {
        ZLinkApplicationMetadata metadata =
                this.metadata == null ? ZLinkApplicationMetadata.empty() : this.metadata;
        traceSent();
    ...
    private void traceSent() {
        ZLinkMessageFlowTracer.TracePoint sent = runtime.flow().begin(ZLinkMessageFlowOutcome.SENT);
        if (sent != null) {
            sent.trace(
    ...
    private static CompletionStage<Integer> submitLocal(
            ZLinkInternalSpotNode node, RoutingId target, byte[] metadata, List<Message> parts) {
...
final class RouteRequestCall implements ZLinkRequestCall {
    ...
    public <TReply> CompletionStage<TReply> yield(Class<TReply> replyType) {
        return systems.zlink.framework.runtime.internal.handlers.ZLinkSuspendInvocationContext
                .rejectYield("MeshNode request");
    }
}
...
final class ChannelRequestCall implements ZLinkRequestCall {
    ...
    public <TReply> CompletionStage<TReply> submit(Class<TReply> replyType) {
        ...
        ZLinkRequestMetrics.Series metric =
                ZLinkRequestMetrics.channel(sockets.requestMetricMeshName(channelName));
        long started =
                ZLinkRequestMetrics.durationEnabled()
                        ? runtime.nanoTime()
                        : ZLinkRequestMetrics.NO_START;
    ...
    private <TReply> CompletionStage<TReply> submitMesh(
        ...
        ZLinkApplicationMetadata metadata =
                this.metadata == null ? ZLinkApplicationMetadata.empty() : this.metadata;
        CompletableFuture<TReply> result =
                requestResult(ZLinkDispatchErrorSurface.ROUTE_MESH_CHANNEL, metric, started);
        var operationId = ZLinkServiceOperationIds.next();
        List<Message> parts =
                ZLinkChannelCallRuntime.envelopeParts(
                        systems.zlink.framework.runtime.messaging.ZLinkChannelEnvelope.KIND_REQUEST,
    ...
    private <TReply> CompletableFuture<TReply> requestResult(
            ZLinkDispatchErrorSurface surface, ZLinkRequestMetrics.Series metric, long started) {
        CompletableFuture<TReply> result = new CompletableFuture<>();
        result.whenComplete(
                (ignored, error) -> {
                    completeMetric(metric, started, error);
    ...
    public <TReply> CompletionStage<TReply> yield(Class<TReply> replyType) {
        systems.zlink.framework.runtime.internal.handlers.ZLinkSuspendInvocationContext
                .requireYieldAllowed("Channel request");
