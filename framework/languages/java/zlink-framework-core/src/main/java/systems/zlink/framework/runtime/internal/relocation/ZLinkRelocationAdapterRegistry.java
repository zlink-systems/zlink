...
public final class ZLinkRelocationAdapterRegistry {
    ...
    public ZLinkRelocationAdapterRegistry(
            ZLinkFrameworkRegistration registration, ZLinkHandlerActivator activator) {
        ...
        for (MeshNodeRegistration node : registration.meshNodes()) {
            node.relocatableActorFactories()
                    .forEach(
                            (stableType, factory) ->
                                    addPreservedState(
                                            actors, stableType, factory.relocationPolicy()));
            node.relocatableSpotFactories()
                    .forEach(
                            (stableType, factory) ->
                                    addPreservedState(
                                            spots, stableType, factory.relocationPolicy()));
    ...
    public CompletionStage<byte[]> captureActor(
            String stableType, ZLinkActor actor, ZLinkRelocationCancellation cancellation) {
        ZLinkActorRelocationAdapter<ZLinkActor> adapter =
                (ZLinkActorRelocationAdapter<ZLinkActor>)
                        create(actorAdapters, stableType, "Actor");
    ...
    public CompletionStage<byte[]> captureSpot(
            String stableType, Object spot, ZLinkRelocationCancellation cancellation) {
        ZLinkSpotRelocationAdapter<Object> adapter =
                (ZLinkSpotRelocationAdapter<Object>) create(spotAdapters, stableType, "Spot");
    ...
    private Object create(Map<String, Class<?>> adapters, String stableType, String kind) {
    ...
    private static void addPreservedState(
            Map<String, Class<?>> adapters, String stableType, RelocationPolicy policy) {
