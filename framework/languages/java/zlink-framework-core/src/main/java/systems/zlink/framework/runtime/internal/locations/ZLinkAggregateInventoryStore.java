 ...
 *
 * <p>The aggregate authority record contains only the inventory metadata. The participant payload
 * and membership mutation are stored under immutable participant keys, so the provider batch used
 * for the authority CAS remains bounded even when an aggregate contains many participants.
 */
final class ZLinkAggregateInventoryStore {
    ...
    private static final byte[] PAGE_MAGIC = {'Z', 'L', 'A', 'P'};
    private static final byte[] ROOT_MAGIC = {'Z', 'L', 'A', 'R'};
    ...
    CompletionStage<Void> store(
        ...
        for (Page page : tree.pages()) {
            ...
            if (bytes.length > MAX_PAGE_BYTES) {
                return failed(new IllegalStateException("aggregate inventory page exceeds 1 MiB"));
            }
            writes =
                    writes.thenCompose(
                            ignored ->
            ...
            writes =
                    writes.thenCompose(
                            ...
                            .thenCompose(
                                    ignored ->
                                            putImmutable(
                                                    participantMembershipKey(
                                                            fence, participantIndex),
                                                    participant.membershipMutation(),
                                                    cancellation));
        }
        return writes.thenCompose(
                        ignored ->
                                putImmutable(rootKey(fence), encodeRoot(tree.root()), cancellation))
                .thenCompose(
                        ignored ->
                                load(
                                        fence,
    ...
    CompletionStage<List<ZLinkAggregateParticipant>> load(
        ...
        if (expectedCount < 1 || declaredDigest.length != HASH_BYTES) {
            return failed(new IllegalArgumentException("aggregate inventory metadata is invalid"));
        }
        return readValue(rootKey(fence), cancellation)
                .thenCompose(
                        bytes -> {
                            ...
                            for (Reference reference : root.topPages()) {
                                ...
                                expectedStart =
                                        Math.addExact(expectedStart, reference.entryCount());
                                pages =
                                        pages.thenCompose(
                                                ignored ->
                                                        readPage(
                            ...
                            return pages.thenCompose(
                                    ignored -> {
                                        validateObservedPages(
                                                root, entries, visited, observedPageCounts, fence);
                                        return loadParticipantValues(fence, entries, cancellation);
                                    });
                        });
    }
     ...
     *
     * <p>The aggregate marker remains as a terminal cleanup tombstone while this method runs. Every
     * item is also protected by the version observed in the scan page so a concurrent cleanup
     * cannot delete a replacement value.
     */
    CompletionStage<Void> delete(ZLinkAggregateFence fence, ZLinkStoreCancellation cancellation) {
        Objects.requireNonNull(fence, "fence");
        Objects.requireNonNull(cancellation, "cancellation");
    ...
    private CompletionStage<Void> delete(
            ...
            ZLinkStoreCancellation cancellation,
            int restartCount) {
        return provider.scan(new ZLinkStoreScanRequest(prefix, cursor, 1_000), cancellation)
                .thenCompose(
                        result -> {
                                ...
                                if (restartCount >= 8) {
                                    return failed(
                                            new IllegalStateException(
                                                    "aggregate inventory cleanup scan repeatedly"
                                                            + " expired"));
                                }
    ...
    CompletionStage<byte[]> readParticipantPayload(
            ZLinkAggregateFence fence, int index, ZLinkStoreCancellation cancellation) {
        if (index < 0) {
            return failed(
    ...
    static byte[] fingerprint(ZLinkAggregatePrepareRequest request) {
        ...
        try {
            ...
            throw new IllegalStateException(
                    "aggregate request fingerprint could not be encoded", failure);
        }
    }

    static ZLinkStoreKey rootKey(ZLinkAggregateFence fence) {
        return new ZLinkStoreKey(
                PREFIX + fence.aggregateId() + ":" + fence.aggregateGeneration() + ":root");
    }
    ...
    private CompletionStage<Void> readPage(
        ...
        if (!visited.add(identity)) {
            return failed(dataLost(fence, "aggregate inventory page is referenced more than once"));
        }
        return readValue(pageKey(fence, reference.level(), reference.index()), cancellation)
                .thenCompose(
                        bytes -> {
                            if (bytes.length > MAX_PAGE_BYTES
                                ...
                                for (Entry entry : page.entries()) {
                                    if (entry.index() != entries.size()) {
                                        return failed(
                                                dataLost(
                                                        fence,
                                                        "aggregate inventory entries are"
                                                                + " reordered"));
                                    }
                                    entries.add(entry);
                                }
                            ...
                            for (Reference child : page.children()) {
                                if (child.level() != page.level() - 1
                                    ...
                                    return failed(
                                            dataLost(
                                                    fence,
                                                    "aggregate inventory index children are"
                                                            + " reordered"));
                                }
                                childStart = Math.addExact(childStart, child.entryCount());
                                children =
                            ...
                            if (childStart != Math.addExact(page.startIndex(), page.entryCount())) {
                                return failed(
                                        dataLost(
                                                fence,
    ...
    private CompletionStage<List<ZLinkAggregateParticipant>> loadParticipantValues(
            ZLinkAggregateFence fence, List<Entry> entries, ZLinkStoreCancellation cancellation) {
        List<ZLinkAggregateParticipant> participants = new ArrayList<>(entries.size());
                ...
                .thenCompose(
                        result ->
                                result instanceof ZLinkStoreReadFound found
                                        ...
                                        : failed(
                                                new IllegalStateException(
                                                        "aggregate inventory value is missing: "
                                                                + key.value())));
    }

    private CompletionStage<Void> putImmutable(
            ZLinkStoreKey key, byte[] bytes, ZLinkStoreCancellation cancellation) {
        return provider.write(
                ...
                .thenCompose(
                        result -> {
                            ...
                            return readValue(key, cancellation)
                                    .thenCompose(
                                            existing ->
    ...
    private static Tree buildTree(ZLinkAggregatePrepareRequest request) {
        ...
        while (references.size() > MAX_PAGE_ENTRIES
            ...
            if (level >= MAX_TREE_LEVELS) {
                throw new IllegalArgumentException("aggregate inventory tree exceeds 32 levels");
            }
        ...
        if (encodeRoot(root).length > MAX_PAGE_BYTES) {
            throw new IllegalArgumentException("aggregate inventory root exceeds 1 MiB");
        }
        return new Tree(root, List.copyOf(pages));
    }

    private static List<Reference> packLeafPages(List<Entry> entries, List<Page> pages) {
        ...
        for (int offset = 0; offset < entries.size(); ) {
            ...
            if (count < 1 || encoded.length > MAX_PAGE_BYTES) {
                throw new IllegalArgumentException("aggregate inventory entry exceeds 1 MiB");
            }
            pages.add(page);
    ...
    private static List<Reference> packIndexPages(
            int level, List<Reference> children, List<Page> pages) {
        List<Reference> references = new ArrayList<>();
        for (int offset = 0; offset < children.size(); ) {
            ...
            byte[] encoded;
            do {
                List<Reference> selected = List.copyOf(children.subList(offset, offset + count));
                int entryCount = selected.stream().mapToInt(Reference::entryCount).sum();
                page =
                        new Page(
                                level,
                                references.size(),
    ...
    private static Root rootCandidate(
        ...
        return new Root(
                totalCount, digest, declaredDigest, topLevel, List.copyOf(topPages), pageCounts);
    }

    private static Reference referenceFor(Page page, byte[] encoded) {
        return new Reference(
                page.level(), page.index(), page.startIndex(), page.entryCount(), sha256(encoded));
    }

    private static void validateRoot(
            Root root, ZLinkAggregateFence fence, int expectedCount, byte[] declaredDigest) {
        ...
        if (leafPages < 1
                ...
                || (long) root.totalCount() > (long) leafPages * MAX_PAGE_ENTRIES
                || root.topPages().size() != root.pageCounts()[root.topLevel()]) {
            throw dataLost(fence, "aggregate inventory root page counts are invalid");
        }
        for (int level = 0; level < root.pageCounts().length; level++) {
            if (root.pageCounts()[level] < 1 || root.pageCounts()[level] > root.totalCount()) {
            ...
            if (level > 0) {
                ...
                if (current < (int) (((long) lower + MAX_PAGE_ENTRIES - 1) / MAX_PAGE_ENTRIES)
                        || current > lower) {
    ...
    private static void validateReference(
            Reference reference, Root root, ZLinkAggregateFence fence) {
        if (reference.level() < 0
                ...
                || (long) reference.startIndex() + reference.entryCount() > root.totalCount()
    ...
    private static void validateEntries(List<Entry> entries, ZLinkAggregateFence fence) {
        byte[] previousKey = null;
        for (int index = 0; index < entries.size(); index++) {
            Entry entry = entries.get(index);
            byte[] currentKey = entry.authorityKey().getBytes(StandardCharsets.UTF_8);
            if (entry.index() != index
                ...
                throw dataLost(fence, "aggregate inventory entries are invalid or reordered");
            }
    ...
    private static Page decodePage(byte[] bytes) {
        try {
            ...
            if (level < 0
                    ...
                    || entryCount < 1
                    || entrySize < 0
                    || entrySize > MAX_PAGE_ENTRIES
                    || childSize < 0
                    || childSize > MAX_PAGE_ENTRIES
            ...
            return new Page(level, index, start, entryCount, entries, children);
        } catch (IOException | RuntimeException failure) {
            throw new IllegalStateException("aggregate inventory page is invalid", failure);
        }
    }
    ...
    private static Root decodeRoot(byte[] bytes) {
        try {
            ...
            return new Root(totalCount, digest, declaredDigest, topLevel, topPages, pageCounts);
        } catch (IOException | RuntimeException failure) {
            throw new IllegalStateException("aggregate inventory root is invalid", failure);
        }
    }
    ...
    private static byte[] encodePage(Page page) {
        try {
            ...
            throw new IllegalStateException(
                    "aggregate inventory page could not be encoded", failure);
        }
    }
    ...
    private static byte[] encodeRoot(Root root) {
        try {
            ...
            throw new IllegalStateException(
                    "aggregate inventory root could not be encoded", failure);
        }
    }
    ...
    private static void writeReference(DataOutputStream out, Reference reference)
            throws IOException {
    ...
    private static Reference readReference(DataInputStream in) throws IOException {
        int level = in.readInt();
        int index = in.readInt();
    ...
    private static DataInputStream input(byte[] bytes, byte[] magic) throws IOException {
    ...
    private static byte[] encodeEntry(Entry entry) {
        try {
            ...
            throw new IllegalStateException(
                    "aggregate inventory entry could not be encoded", failure);
        }
    }
    ...
    private static ZLinkAggregateFence fence(ZLinkAggregatePrepareRequest request) {
        return new ZLinkAggregateFence(request.aggregateId(), request.aggregateGeneration());
    }

    private static ZLinkStoreKey pageKey(ZLinkAggregateFence fence, int level, int index) {
        return new ZLinkStoreKey(
                PREFIX
                        + fence.aggregateId()
                        + ":"
    ...
    private static String inventoryPrefix(ZLinkAggregateFence fence) {
        return PREFIX + fence.aggregateId() + ":" + fence.aggregateGeneration() + ":";
    }

    private static ZLinkStoreKey participantPayloadKey(ZLinkAggregateFence fence, int index) {
        return new ZLinkStoreKey(
                PREFIX
                        + fence.aggregateId()
                        + ":"
    ...
    private static ZLinkStoreKey participantMembershipKey(ZLinkAggregateFence fence, int index) {
        return new ZLinkStoreKey(
                PREFIX
                        + fence.aggregateId()
                        + ":"
    ...
    private static IllegalStateException dataLost(ZLinkAggregateFence fence, String message) {
    ...
    private record Root(
        ...
        @Override
        public byte[] declaredDigest() {
            return declaredDigest.clone();
        }

        @Override
        public int[] pageCounts() {
            return pageCounts.clone();
        }
    }
    ...
    private record Reference(int level, int index, int startIndex, int entryCount, byte[] sha256) {
        Reference {
            sha256 = sha256.clone();
        }
