...
public record ActorRef(String actorId, long objectGeneration, String meshName, RoutingId nodeRid) {
    public ActorRef {
        Objects.requireNonNull(actorId, "actorId");
        int byteLength = actorId.getBytes(StandardCharsets.UTF_8).length;
        if (byteLength < 1 || byteLength > 255) {
            throw new IllegalArgumentException("actorId must contain 1..255 UTF-8 bytes");
        }
