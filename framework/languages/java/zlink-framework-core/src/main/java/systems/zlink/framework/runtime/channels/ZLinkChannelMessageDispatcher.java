...
final class ZLinkChannelMessageDispatcher {
    ...
    void dispatchRequest(
        ...
        try {
            ...
            dispatchRequestHandler(
                    channelName, router, received, requestSeq, packet, registration, contentType);
        } finally {
            flowScope.close();
            if (received.isRequest()) {
                received.closeParts();
    ...
    private void traceReceivedRequest(String channelName, String packetName, long requestSeq) {
        traceFlow(
                ZLinkMessageFlowOutcome.RECEIVED,
                ZLinkDispatchMessageKind.REQUEST,
                packetName,
