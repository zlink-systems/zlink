...
public final class ZLinkActorEntrySpotRoutePackets {
    ...
    public static Message encodeJoinRequest(
            String actorId, String actorType, RoutingId sourceNodeRid, long sourceGeneration) {
        return Message.from(
                String.join(
                                "\n",
                                actorId,
                                ...
                                Long.toUnsignedString(sourceGeneration))
    ...
    public static JoinRequest decodeJoinRequest(Message message) {
        ...
        return new JoinRequest(
                fields[0], fields[1], RoutingId.from(fields[2]), Long.parseUnsignedLong(fields[3]));
    }

    public static Message encodeJoinReply(
            String actorId, String actorType, RoutingId targetNodeRid, long actorGeneration) {
        return Message.from(
                String.join(
                                "\n",
                                actorId,
                                ...
                                Long.toUnsignedString(actorGeneration))
                        .getBytes(StandardCharsets.UTF_8));
    }
