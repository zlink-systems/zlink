...
/**
 * Dispatches MeshNode application records through the framework's typed serializer and handler
 * activation path.
 */
public final class ZLinkMeshApplicationDispatcher implements ZLinkMeshApplicationReceiver {
    @FunctionalInterface
    ...
    ZLinkMeshApplicationDispatcher(
        ...
        namespaces.put(
                NODE_NAMESPACE, routeNamespace(mesh.nodeHandlers(), framework.handlerExecutor()));
        mesh.channelHandlers()
                .forEach(
                        (name, handlers) ->
                                namespaces.put(
                                        name,
    ...
    public CompletionStage<Integer> submitLocalNodeSend(
            RoutingId sourceNodeRid, byte[] metadataBytes, List<Message> parts) {
        ...
        ZLinkMeshDrainCoordinator.Claim claim = drains == null ? null : drains.tryClaim(meshName);
        if (drains != null && claim == null) {
            return CompletableFuture.completedFuture(ZLinkOneWayCalls.SHUTDOWN);
        }
    ...
    private static void closeLocalSubmission(
            Message payload, ZLinkMeshDrainCoordinator.Claim claim) {
        try {
            payload.close();
        } finally {
            if (claim != null) {
    ...
    private void dispatchSend(
        ...
        traceFlow(ZLinkMessageFlowOutcome.RECEIVED, record, packetName);
        try {
            CompletionStage<Void> queued =
                    namespace.sendQueue.enqueue(
                            () -> {
                                Message payload = record.parts().get(1);
                                traceFlow(ZLinkMessageFlowOutcome.ADMITTED, record, packetName);
                                traceFlow(ZLinkMessageFlowOutcome.DISPATCHED, record, packetName);
    ...
    private void dispatchRequest(
        ...
        if ((token == null && !record.canReply()) || (route == null && channel == null)) {
                            ...
                            .enqueue(
                                    () -> {
                                        ...
                                        traceFlow(
                                                ...
                                                packetName);
                                        try {
                                            CompletionStage<Message> invocation =
                                                    route != null
                                                            ? invoker.executeHandler(
                                                                    () ->
                                                                            invoker
    ...
    private void reject(
            ZLinkMeshDispatchRecord record, String message, ZLinkMeshDrainCoordinator.Claim claim) {
        ReplyToken token = record.receive().replyToken();
        try {
        ...
        try {
            if (token != null || record.canReply()) {
                replyAndClose(
                        record,
                        token,
                        ZLinkFrameworkErrorReply.create(envelope, kind, message, Map.of()));
            }
        } finally {
            closeRecord(record, claim);
        }
    ...
    private static void closeRecord(
            ZLinkMeshDispatchRecord record, ZLinkMeshDrainCoordinator.Claim claim) {
        try {
            record.close();
        } finally {
            if (claim != null) {
    ...
    private void replyError(ZLinkMeshDispatchRecord record, ReplyToken token, String message) {
    ...
    private void replyError(
        ...
        ZLinkFrameworkErrorKind kind = ZLinkChannelDispatchReporter.frameworkErrorKind(cause);
        replyAndClose(
                record,
                token,
                ZLinkFrameworkErrorReply.create(envelope, kind, cause.getMessage(), Map.of()));
    }

    /**
     * Successful request reply parts: a kind-2 envelope echoing the request identifiers when the
     * request arrived as a shared envelope, else the legacy raw single-part reply.
     */
    ...
    private void replyAndClose(
            ZLinkMeshDispatchRecord record, ReplyToken token, List<Message> parts) {
        try {
            if (record.canReply()) {
                record.reply(parts);
            } else {
    ...
    private void traceFlow(
            ZLinkMessageFlowOutcome outcome, ZLinkMeshDispatchRecord record, String packetName) {
    ...
    private void traceLocalNodeSend(
            ZLinkMessageFlowOutcome phase, String packetName, RoutingId sourceNodeRid) {
    ...
    private static Namespace routeNamespace(
            List<MeshNodeRegistration.DispatchHandler> handlers, Executor executor) {
        ...
        for (MeshNodeRegistration.DispatchHandler handler : handlers) {
            ...
            if (handler.request()) {
                ...
                putUnique(
                        namespace.routeSends,
                        packetName,
                        new ChannelRouteSendHandlerRegistration(
                                handler.handlerType(), handler.messageType(), packetName));
            }
        }
        return namespace;
    }
    ...
    private static Namespace channelNamespace(
        ...
        Namespace namespace = new Namespace(executor);
        Set<String> groups =
                handlerGroups.isEmpty() ? Set.of(channelName) : Set.copyOf(handlerGroups);
        for (ZLinkScannedHandler handler :
                scannedHandlers.matching(
                        groups,
                        ZLinkScannedHandlerSurface.CHANNEL,
                        ZLinkScannedHandlerKind.REQUEST)) {
            putUnique(
                    namespace.channelRequests,
                    handler.packetName(),
        ...
        for (ZLinkScannedHandler handler :
                scannedHandlers.matching(
                        groups, ZLinkScannedHandlerSurface.CHANNEL, ZLinkScannedHandlerKind.SEND)) {
            putUnique(
                    namespace.channelSends,
                    handler.packetName(),
        ...
        for (ZLinkScannedHandler handler :
                scannedHandlers.matching(
                        groups, ZLinkScannedHandlerSurface.ROUTE, ZLinkScannedHandlerKind.SEND)) {
            putUnique(
                    namespace.routeSends,
                    handler.packetName(),
                    new ChannelRouteSendHandlerRegistration(
    ...
    private static final class Namespace {
        private final Map<String, ChannelRouteSendHandlerRegistration> routeSends = new HashMap<>();
        private final Map<String, ChannelRouteRequestHandlerRegistration> routeRequests =
                new HashMap<>();
        private final Map<String, ChannelSendHandlerRegistration> channelSends = new HashMap<>();
        ...
        Namespace(Executor executor) {
            sendQueue = new ZLinkSerialExecutionQueue(executor, ZLinkExecutionLanePolicy.generic());
            requestQueue =
