...
/**
 * Framework-generated error replies on the shared cross-language envelope wire: a kind-5 envelope
 * whose header carries the snake_case {@code errorCode}, the {@code errorMessage} and the reply
 * metadata object (including the {@code zlink.origin=framework} marker for framework-origin
 * errors). The legacy raw-parts representation {@code ["ZLinkFrameworkError", message, KIND,
 * metadata?]} is gone; encode and decode operate on the envelope only.
 */
public final class ZLinkFrameworkErrorReply {
    ...
    public static List<Message> create(ZLinkFrameworkErrorKind kind, String message) {
    ...
    public static List<Message> create(
            ZLinkFrameworkErrorKind kind, String message, Map<String, String> metadata) {
        return create(null, kind, message, metadata);
    }

    /**
     * Error envelope echoing the request header's channel/message names and correlation id, per the
     * C++ reply writer ({@code channel_reply_writer.cpp} {@code create_error_header}).
     */
    public static List<Message> create(
    ...
    public static boolean isReply(List<Message> parts) {
        ZLinkChannelEnvelope.Header header = ZLinkChannelEnvelope.tryDecodeHeader(parts, false);
        return header != null && header.isError();
    }

    /**
     * Legacy stray-error marker name. Envelope error replies are detected via the header kind; this
     * remains only for callers that inspect a raw first-frame packet name.
     */
    ...
    /**
     * Reply metadata from the envelope header. A reply without metadata — or parts that are not an
     * error envelope at all — yields an empty map, so a missing or malformed marker never turns
     * into a framework-origin claim.
     */
