...
final class ZLinkActorEntrySpotJoinCall implements ZLinkActorJoinCall {
    ...
    public ZLinkActorJoinCall timeout(Duration timeout) {
        ...
        ZLinkActorEntrySpotJoinCall configured =
                new ZLinkActorEntrySpotJoinCall(
                        context, targetResolver, request, timeout, services);
    ...
    public void defer() {
        ...
        ZLinkActorJoinOperationId operationId = ZLinkActorSpotJoinCall.newOperationId();
    ...
    private CompletionStage<Void> executeDeferred(
        ...
        Duration remaining = ZLinkActorSpotJoinCall.remainingTimeout(deadlineNanos);
        if (remaining == null) {
            return notifyCompletion(
                    new ZLinkActorJoinCompletion.Failed(
                            operationId, ZLinkFrameworkErrorKind.DEADLINE_EXCEEDED));
        }
        ZLinkActorEntrySpotJoinCall bounded = (ZLinkActorEntrySpotJoinCall) timeout(remaining);
    ...
    interface TargetResolver {
        CompletionStage<ZLinkActorRuntime.EntrySpotTarget> resolve(Duration timeout);
    }
