...
final class ZLinkFilterPipeline {
    ...
    public static <T> CompletionStage<Result<T>> invoke(
        ...
        AtomicReference<CompletionStage<T>> terminalStage = new AtomicReference<>();
            ...
            next =
                    () -> {
                        ...
                        return Objects.requireNonNull(
                                filter.invoke(context, currentNext), "filter stage");
                    };
        }
        return next.invoke()
                .thenCompose(
                        ignored -> {
                            ...
                            if (invoked == null) {
                                return CompletableFuture.completedFuture(new Result<>(false, null));
                            }
                            return invoked.thenApply(value -> new Result<>(true, value));
                        });
    }

    private static <T> ZLinkHandlerFilterNext<T> invokeAtMostOnce(ZLinkHandlerFilterNext<T> next) {
