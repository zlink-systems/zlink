...
final class ZLinkActorAcceptedJournal {
    ...
    static byte[] encode(
            String actorId, ZLinkStreamHeader header, Message payload, byte[] canonical) {
        ...
        if (systems.zlink.framework.runtime.internal.service.ZLinkServiceFrozenRecordCodec
            ...
            var decoded =
                    systems.zlink.framework.runtime.internal.service.ZLinkServiceFrozenRecordCodec
                            .decodeActor(encoded);
            DecodedApplication application =
                    decodeApplication(
                            decoded.packetName(),
                            decoded.contentType(),
                            decoded.payload(),
    ...
    private static DecodedApplication decodeApplication(
        ...
        if (!ServiceWireConstants.FRAMEWORK_MULTIPART_PACKET_NAME.equals(packetName)) {
            return new DecodedApplication(
                    new ZLinkStreamHeader(
                            ...
                            EnumSet.noneOf(
                                    systems.zlink.framework.runtime.streams.ZLinkStreamHeaderFlag
                                            .class),
                            replyRouteId,
                            packetName,
                            metadata),
        ...
        try {
            ...
            ZLinkStreamHeader header =
                    ZLinkStreamHeaderCodec.decodeOrPlain(parts.getFirst().toByteArray());
            ...
            return new DecodedApplication(header, parts.get(1).toByteArray());
        } finally {
    ...
    record Record(
        ...
        Record {
            if (sourceNodeRid == null
                    ...
                    || sourceSessionRid == null != (sourceBindingGeneration == 0)
                    || sourceSessionRid == null != (sourceSessionSequence == 0)
                    || objectGeneration <= 0
                    || operationHigh == 0 && operationLow == 0) {
                throw new IllegalArgumentException("accepted Actor source fence is invalid");
            }
