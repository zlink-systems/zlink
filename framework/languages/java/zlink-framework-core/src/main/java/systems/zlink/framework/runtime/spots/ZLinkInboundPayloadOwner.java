...
/**
 * Owns the single typed decode outcome for one admitted inbound payload. Transport storage remains
 * owned by the surrounding dispatch until the first decode completes; typed handler values never
 * own that storage.
 */
final class ZLinkInboundPayloadOwner {
    ...
    ZLinkInboundPayloadOwner(Message payload, ZLinkMessageSerializer serializer) {
    ...
    Object deserialize(Class<?> declaredType) {
        ...
        if (selected == null) {
            ...
            if (outcome.compareAndSet(null, candidate)) {
                try {
                    Object value =
                            ZLinkMessagePayloads.deserialize(serializer, payload, declaredType);
        ...
        if (decoded.declaredType() != declaredType) {
            throw new ZLinkFrameworkException(
                    ...
                    "Inbound payload was decoded as "
                            + decoded.declaredType().getName()
    ...
    private static RuntimeException propagate(Throwable failure) {
        ...
        return new ZLinkFrameworkException(
                ZLinkFrameworkErrorKind.PROTOCOL_ERROR, "Inbound payload decode failed", failure);
    }

    private sealed interface DecodeOutcome permits Decoded, DecodeFailed {}

    private record Decoded(Class<?> declaredType, Object value) implements DecodeOutcome {}
