...
final class ZLinkProviderOwnerLeaseRepository {
    ...
    private static final ZLinkStoreKey OWNER_COUNTER = new ZLinkStoreKey(PREFIX + "owner-counter");
    private final ZLinkLocationStore provider;

    ZLinkProviderOwnerLeaseRepository(ZLinkLocationStore provider) {
        this.provider = Objects.requireNonNull(provider, "provider");
    }

    CompletionStage<ZLinkOwnerLeaseClaimResult> claim(String ownerId, Duration leaseTtl) {
                ...
                .thenCompose(
                        current -> {
                            ...
                            return provider.write(
                                    ...
                                    .thenApply(
                                            result ->
                                                    result instanceof ZLinkStoreWriteApplied applied
    ...
    CompletionStage<ZLinkOwnerLeaseReleaseResult> release(ZLinkLocationOwnerToken token) {
        Objects.requireNonNull(token, "token");
        ZLinkStoreKey key = ownerKey(token.ownerId());
        return provider.read(key, active())
                .thenCompose(
    ...
    private CompletionStage<ZLinkOwnerLeaseClaimResult> claimLoop(
        ...
        return provider.read(ownerKey, active())
                .thenCompose(
                        owner -> {
                                    ...
                                    .thenCompose(
                                            counter -> {
                                                ...
                                                try {
                                                    ...
                                                    return reconcileClaim(ownerKey, token, failure);
                                                }
                                                CompletionStage<
                                                                CompletionStage<
    ...
    private CompletionStage<ZLinkOwnerLeaseClaimResult> reconcileClaim(
            ZLinkStoreKey key, ZLinkLocationOwnerToken expected, Throwable originalFailure) {
        return provider.read(key, active())
                .toCompletableFuture()
                .orTimeout(5, TimeUnit.SECONDS)
                .handle(
                        (result, failure) -> {
                            if (failure == null && result instanceof ZLinkStoreReadFound found) {
                                ZLinkOwnerLeaseRecordCodec.Record record =
                                        ZLinkOwnerLeaseRecordCodec.decode(found.value().bytes());
                                if (record.ownerId().equals(expected.ownerId())
                                        && record.leaseGeneration() == expected.leaseGeneration()
    ...
    private static long decodeCounter(byte[] bytes) {
        ...
        try {
            ...
            throw new IllegalStateException("Location Store owner counter is invalid", failure);
        }
        if (value <= 0 || !Long.toString(value).equals(text)) {
            ...
            throw new IllegalStateException("Location Store owner counter is invalid");
        }
        return value;
    }
    ...
    private static void requireRetention(Duration retention) {
        ...
        if (retention.isZero() || retention.isNegative()) {
            throw new IllegalArgumentException("lease retention must be positive");
        }
    }
