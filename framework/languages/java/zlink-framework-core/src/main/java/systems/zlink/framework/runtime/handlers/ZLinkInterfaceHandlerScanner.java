...
final class ZLinkInterfaceHandlerScanner {
    ...
    static void addHandlers(
            List<ZLinkScannedHandler> handlers, Class<?> candidate, Set<String> groups) {
    ...
    private static void addChannelInterfaceHandlers(
        ...
        addInterfaceHandler(
                ...
                candidate,
                groups,
                handlerInterface(ZLinkSendHandler.class),
        ...
        addInterfaceHandler(
                ...
                candidate,
                groups,
                handlerInterface(KOTLIN_SEND_HANDLER),
                ZLinkScannedHandlerSurface.CHANNEL,
                ZLinkScannedHandlerKind.SEND);
        addInterfaceHandler(
                handlers,
                candidate,
                groups,
                handlerInterface(ZLinkRequestHandler.class),
                ZLinkScannedHandlerSurface.CHANNEL,
        ...
        addInterfaceHandler(
                ...
                candidate,
                groups,
                handlerInterface(KOTLIN_REQUEST_HANDLER),
                ZLinkScannedHandlerSurface.CHANNEL,
                ZLinkScannedHandlerKind.REQUEST);
        addInterfaceHandler(
                handlers,
                candidate,
                groups,
                handlerInterface(ZLinkFanoutHandler.class),
                ZLinkScannedHandlerSurface.CHANNEL,
        ...
        addInterfaceHandler(
                ...
                candidate,
                groups,
                handlerInterface(KOTLIN_PUBLISH_HANDLER),
                ZLinkScannedHandlerSurface.CHANNEL,
                ZLinkScannedHandlerKind.PUBLISH);
        addInterfaceHandler(
                handlers,
                candidate,
                groups,
                handlerInterface(ZLinkRouteSendHandler.class),
        ...
        addInterfaceHandler(
                ...
                candidate,
                groups,
                handlerInterface(KOTLIN_ROUTE_SEND_HANDLER),
                ZLinkScannedHandlerSurface.ROUTE,
                ZLinkScannedHandlerKind.SEND);
        addInterfaceHandler(
                handlers,
                candidate,
                groups,
                handlerInterface(ZLinkRouteRequestHandler.class),
                ZLinkScannedHandlerSurface.ROUTE,
                ZLinkScannedHandlerKind.REQUEST);
        addInterfaceHandler(
                handlers,
                candidate,
                groups,
                handlerInterface(KOTLIN_ROUTE_REQUEST_HANDLER),
                ZLinkScannedHandlerSurface.ROUTE,
                ZLinkScannedHandlerKind.REQUEST);
    }
    ...
    private static void addInterfaceHandler(
        ...
        Class<?> replyType =
                kind == ZLinkScannedHandlerKind.REQUEST
                        ? requireClassArgument(candidate, arguments[1])
                        : Void.class;
        handlers.add(
                new ZLinkScannedHandler(
                        surface,
                        kind,
                        candidate,
                        messageType,
    ...
    private static void addSpotInterfaceHandlers(
        ...
        addSpotSubscriptionInterfaceHandler(
                handlers, candidate, groups, handlerInterface(KOTLIN_SPOT_SUBSCRIPTION_HANDLER));
        addSpotTimerInterfaceHandler(handlers, candidate, groups);
        addSpotTimerInterfaceHandler(
                handlers, candidate, groups, handlerInterface(KOTLIN_SPOT_TIMER_HANDLER));
    }
    ...
    private static void addSpotSubscriptionInterfaceHandler(
            List<ZLinkScannedHandler> handlers, Class<?> candidate, Set<String> groups) {
        addSpotSubscriptionInterfaceHandler(
                handlers, candidate, groups, handlerInterface(ZLinkSpotSubscriptionHandler.class));
    }
    ...
    private static void addSpotTimerInterfaceHandler(
            List<ZLinkScannedHandler> handlers, Class<?> candidate, Set<String> groups) {
        addSpotTimerInterfaceHandler(
                handlers, candidate, groups, handlerInterface(ZLinkSpotTimerHandler.class));
    }
