...
final class ZLinkRelocationAdapterTypeMatcher {
    ...
    static boolean matches(Class<?> adapterType, Class<?> adapterContract, Class<?> objectType) {
    ...
    /**
     * Walks past an intermediate interface between the adapter and the contract (e.g. a base/delta
     * capability interface that itself extends the plain capture/restore contract), substituting
     * the intermediate interface's own type variable for the contract's type parameter so an
     * adapter can implement the contract indirectly (spec 15 §5).
     */
    private static boolean matches(
            Type implemented, Class<?> adapterContract, Class<?> objectType) {
    ...
    /**
     * Replaces {@code declaringClass}'s own type variables in {@code type} with the concrete
     * arguments the subtype supplied for them in {@code actual} (e.g. resolving {@code TActor} in
     * {@code ZLinkActorRelocationAdapter<TActor>} to {@code TestActor} using the concrete adapter
     * subtype's argument).
     */
    private static Type substitute(Type type, Class<?> declaringClass, ParameterizedType actual) {
