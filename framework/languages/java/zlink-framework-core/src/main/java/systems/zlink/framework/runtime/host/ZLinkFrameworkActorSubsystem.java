...
final class ZLinkFrameworkActorSubsystem {
    ...
    static ZLinkFrameworkActorSubsystem create(
                        ...
                        .orElseGet(
                                () ->
                                        registration.meshNodes().stream()
                                                ...
                                                .findFirst()
                                                .orElse(null));
        String actorNodeName =
                legacyActorNode != null
        ...
        var actorFactories =
                legacyActorNode != null
                        ...
                        : meshActorNode == null
                                ? Map.<String, Class<? extends ZLinkActorFactory>>of()
    ...
    private static void wireActorRuntime(
        ...
        if (meshActorNode != null) {
            ...
            if (meshNode != null) {
                ...
                try {
                    meshNode.spotNode().setMessageFollowRelayHandler(actors::relayMessageFollow);
        ...
        if (storeLocationResolvers != null) {
            meshNodes
                    ...
                    .forEach(
                            node ->
                                    node.setMessageFollowHandler(
                                            (sourceNodeRid, notice) -> {
                                                if (!acceptsMessageFollowNotice(
                                                        sourceNodeRid, notice)) {
                                                    return;
                                                }
                                                storeLocationResolvers.invalidateRouteIfMatches(
                                                        notice);
                                            }));
        }
        actors.setMessageFlowTracer(
        ...
        meshNodes
                ...
                .forEach(
                        node ->
                                node.setCanonicalActorJoinHandler(
                                        (sourceNodeRid, join) ->
                                                spots.admitCanonicalActorJoinForServiceWire(
                                                        join, sourceNodeRid)));
    ...
    static boolean acceptsMessageFollowNotice(
            ...
            systems.zlink.framework.runtime.internal.service.ZLinkServiceMessageFollowWireCodec
                            .Notice
                    notice) {
