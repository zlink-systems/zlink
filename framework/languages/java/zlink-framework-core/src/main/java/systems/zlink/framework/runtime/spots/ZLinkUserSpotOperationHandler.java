...
final class ZLinkUserSpotOperationHandler
                ...
                .thenCompose(
                        read -> {
                                        ...
                                        .thenCompose(
                                                ignored ->
                                                        failed(
                                                                ...
                                                                "User Spot create payload failed"
                            ...
                            return lifecycle
                                    ...
                                    .thenCompose(
                                            prepared ->
                                                    finish(request, snapshot, admission, prepared))
        ...
        return authorityStore
                ...
                .thenCompose(
                        read -> {
                            ...
                            if (readiness == ZLinkSpotLifecycle.CloseReadiness.GENERATION_STALE) {
                                return failed(
                                        TERMINAL_INVALID_STATE,
                                        FAILURE_SPOT_GENERATION_STALE,
    ...
    private Admission validate(
        ...
        var pending =
                snapshot.pendingCreation()
                        .orElseThrow(() -> stale("User Spot creation projection is missing"));
        var authority =
                authorities
                        .decode(snapshot.payload())
    ...
    private CompletionStage<ZLinkObjectAbortResult> abort(ZLinkObjectReservation reservation) {
    ...
    private ZLinkInternalMeshNode.UserSpotCreateResponse response(
        ...
        List<Message> parts =
                reply == null
                        ? List.of()
                        : List.of(Message.from(reply.toEncodedPayload(serializer).bytes()));
    ...
    private static ZLinkUserSpotOperationException stale(String message) {
        return new ZLinkUserSpotOperationException(
                TERMINAL_INVALID_STATE, FAILURE_SPOT_GENERATION_STALE, message);
    }

    private static <T> CompletionStage<T> failed(
            int terminalResult, int failureCode, String message) {
        return CompletableFuture.failedFuture(
                new ZLinkUserSpotOperationException(terminalResult, failureCode, message));
    }
    ...
    private static String message(Throwable failure) {
        Throwable current = unwrap(failure);
        return current.getMessage() == null ? "User Spot operation failed" : current.getMessage();
    }
