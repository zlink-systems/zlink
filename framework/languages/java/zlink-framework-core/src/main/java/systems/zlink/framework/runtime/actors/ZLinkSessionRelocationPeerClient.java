...
public final class ZLinkSessionRelocationPeerClient {
    private static final Duration[] RETRANSMIT_INTERVALS = {
        Duration.ofSeconds(1), Duration.ofSeconds(1), Duration.ofSeconds(2), Duration.ofSeconds(4),
    };
    private static final Duration STEADY_RETRANSMIT_INTERVAL = Duration.ofSeconds(5);
    ...
    ZLinkSessionRelocationPeerClient(ZLinkInternalMeshNode node, ZLinkServiceM6BWireCodec codec) {
    ...
    public CompletionStage<Void> sendRoute(
        ...
        return node.sendSessionRelocationRoute(
                command.session().nodeRid(), codec.encodeSessionRelocationRoute(command));
    }

    public CompletionStage<ZLinkServiceM6BWireCodec.SessionRelocationSealed> sealRouteUntilAck(
            ZLinkServiceM6BWireCodec.SessionRelocationSeal command, Duration deadline) {
        Objects.requireNonNull(command, "command");
        ...
        CompletableFuture<ZLinkServiceM6BWireCodec.SessionRelocationSealed> settled =
                new CompletableFuture<>();
        attemptSeal(command, command42, System.nanoTime() + deadline.toNanos(), settled, 0);
        return settled;
    }
    ...
    private void attemptSeal(
        ...
        Duration window = remaining.compareTo(interval) < 0 ? remaining : interval;
        long sentAt = System.nanoTime();
        node.requestSessionRelocationSeal(command.session().nodeRid(), command42, window)
                .thenApply(codec::decodeSessionRelocationSealed)
                .whenComplete(
                        (sealedResult, failure) -> {
                            if (failure == null) {
                                if (sealedResult.echoes(command)) {
                                    settled.complete(sealedResult);
                                } else {
                                    settled.completeExceptionally(
                            ...
                            Duration delay = interval.minusNanos(System.nanoTime() - sentAt);
