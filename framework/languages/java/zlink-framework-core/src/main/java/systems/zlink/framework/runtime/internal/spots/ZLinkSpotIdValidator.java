...
/**
 * Validates the Framework SpotId text contract without changing the supplied identity. In
 * particular, validation does not normalize Unicode or fold case.
 */
public final class ZLinkSpotIdValidator {
    private static final Pattern FRAMEWORK_ENTRY_SPOT_ID =
            Pattern.compile(
    ...
    public static String requireValid(String spotId) {
        if (spotId == null || spotId.indexOf('\0') >= 0) {
            throw new IllegalArgumentException("SpotId must contain 1..255 UTF-8 bytes");
        }
