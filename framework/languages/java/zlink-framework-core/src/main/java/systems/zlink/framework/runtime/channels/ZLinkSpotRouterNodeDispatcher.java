...
final class ZLinkSpotRouterNodeDispatcher {
    ...
    static CompletionStage<Void> send(
        ...
        entrySpot
                .sendToSpot(targetNodeRid, targetSpotId, targetSpotGeneration, spotParts)
                .whenComplete(
                        (ignored, failure) -> {
                            if (failure == null) {
    ...
    private static void rememberAuthority(
            ...
            long authorityOwnerGeneration,
            long ownerLeaseGeneration) {
        if (targetSpotGeneration > 0 && authorityOwnerGeneration > 0 && ownerLeaseGeneration > 0) {
    ...
    /**
     * Spec 32:58,87 — a Framework failure discovered while waiting on route resolution or the
     * remote reply is delivered as a typed Framework exception; a route or connection that is not
     * available is {@code Unavailable}. The binding's local preflight throws the raw {@link
     * ZlinkRequestException}; keep it as the cause.
     */
        ...
        while (current != null) {
            ...
            if (current instanceof ZlinkRequestException request) {
                return switch (request.getResult()) {
                    case NOT_CONNECTED, CONFLICT ->
                            ZLinkFrameworkErrorOrigin.framework(
                                    ZLinkFrameworkErrorKind.UNAVAILABLE,
                                    "SPOT route request failed because the target route is not"
                                            + " available.",
                                    request);
                    case NOT_FOUND ->
                            ZLinkFrameworkErrorOrigin.framework(
                                    ZLinkFrameworkErrorKind.NOT_FOUND,
                                    "SPOT route request failed because the target route was not"
                                            + " found.",
                                    request);
                    default -> failure;
                };
    ...
    private static void completeReply(
            ZLinkBackendReceived reply, CompletableFuture<List<Message>> result) {
        try {
