...
public interface ZLinkInternalMeshNode extends ZLinkBackendObject {
    ...
    default void setDispatchErrorReporter(Consumer<ZLinkDispatchFailure> reporter) {
    ...
    default void setObjectRole(ZLinkMeshNodeObjectRole role) {
    ...
    /**
     * Defers the local descriptor's serving transition until the host calls {@link
     * #markServiceReady()} after Framework startup preparation.
     */
    ...
    /**
     * Publishes the local service descriptor as available for application selection after the
     * Framework host has finished preparing its handlers, objects, and peer connections.
     */
    ...
    default long replacePeerConnection(
            ...
            String expectedSecurityIdentity) {
        return connectPeer(
                endpoint, expectedRoutingId, expectedLifecycleGeneration, expectedSecurityIdentity);
    }
    ...
    /**
     * Returns the current descriptor-backed peer identities for metric collection. This is internal
     * topology state, not a public snapshot.
     */
                ...
                .filter(
                        peer -> {
                            PeerChannels channels =
                                    peerChannels(peer.routingId(), peer.lifecycleGeneration());
    ...
    /**
     * Whether the exact admitted transport for a relocation target is available. Canonical
     * relocation's Prepare is the target-readiness handshake; it must not wait for an independent
     * liveness round-trip before that handshake can be sent.
     */
    ...
    /**
     * Installs the immutable content-type to stream-codec mapping used by typed Actor ingress.
     * Unknown incoming content types must return an empty result so the service boundary can report
     * a protocol failure without attempting a JSON fallback.
     */
    ...
    default void setUserSpotOperationHandler(UserSpotOperationHandler handler) {
        // Alternate backends may not yet own Framework service operations.
    }

    default void setActorCreateOperationHandler(ActorCreateOperationHandler handler) {
    ...
    default void setCanonicalActorJoinHandler(CanonicalActorJoinHandler handler) {
    ...
    default CompletionStage<CanonicalActorJoinReply> requestCanonicalActorJoin(
            CanonicalActorJoinRequest request, Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Canonical actorJoin transport is unavailable"));
    }

    /**
     * Installs the durable source-owner resolver used before a remote Spot or Actor operation
     * enters an application queue.
     */
    default void setPeerAuthorityResolver(PeerAuthorityResolver resolver) {
    ...
    /**
     * Installs the infrastructure-only relocation command endpoint. Commands use the admitted
     * RouteMesh peer and never enter an application mailbox.
     */
    default void setRelocationControlHandler(RelocationControlHandler handler) {
        // Alternate backends may not yet support remote relocation control.
    }

    /**
     * Sends one relocation command to the exact target node. The transport performs one submission
     * only; a stale or failed route is not resolved and retried inside the same operation.
     */
    default CompletionStage<byte[]> requestRelocationControl(
            RoutingId targetNodeRid, byte[] command, Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Remote relocation control is unavailable"));
    }

    /** Installs the canonical service-wire command 30-35/40-41 endpoint. */
    default void setCanonicalRelocationControlHandler(CanonicalRelocationControlHandler handler) {
        // Alternate backends may not yet support canonical relocation control.
    }

    /**
     * Accepts one canonical relocation control record for infrastructure delivery to the exact
     * admitted peer.
     */
    default CompletionStage<Void> sendCanonicalRelocationControl(
            RoutingId targetNodeRid, byte[] command) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Canonical relocation control is unavailable"));
    }

    /**
     * Sends command 40 as a request and completes with its command 30 or 53 reply. Other canonical
     * relocation controls remain one-way.
     */
    ...
    default CompletionStage<Void> sendActorLeft(
        ...
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Actor Left notification is unavailable"));
    }

    /**
     * Installs the infrastructure-only Message Follow notification endpoint. The notice carries
     * only route fences and queue accounting; it never enters a user Spot or Actor mailbox.
     */
    ...
    /**
     * Sends one exact command 50 notice to the admitted target node. Route resolution and retry are
     * owned by the caller; this method submits the notice to the supplied node only.
     */
    default CompletionStage<Void> sendMessageFollow(
            RoutingId targetNodeRid, ZLinkServiceMessageFollowWireCodec.Notice notice) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Message Follow notification is unavailable"));
    }

    default void setRelocationReplyRelayHandler(RelocationReplyRelayHandler handler) {
        // Alternate backends may not support command 33/46 dispatch.
    }

    /**
     * Sends one canonical command 33 record plus its application payload and returns the exact
     * command 46 closed acknowledgement. The landing node is the relocation source that owns the
     * reply capability; it may differ from the request-source fence carried in {@code
     * expectedSource}.
     */
    default CompletionStage<byte[]> requestRelocationReplyRelay(
            ...
            Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Relocation reply relay is unavailable"));
    }

    default void setSessionRelocationRouteHandler(SessionRelocationRouteHandler handler) {
        // Alternate backends may not support command 44 dispatch.
    }

    default void setSessionRelocationSealHandler(SessionRelocationSealHandler handler) {
        // Alternate backends may not support command 42/43 dispatch.
    }

    default void setBoundSessionSendHandler(BoundSessionSendHandler handler) {
    ...
    default CompletionStage<byte[]> requestSessionRelocationSeal(
        ...
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Session relocation seal is unavailable"));
    }
    ...
    default CompletionStage<Void> sendSessionRelocationRoute(
        ...
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Session relocation routing is unavailable"));
    }

    /**
     * Installs the one-way command 51 endpoint. The record is infrastructure state and must not
     * enter the application mailbox.
     */
    default void setBoundSessionReplacedHandler(BoundSessionReplacedHandler handler) {
        // Alternate backends may not support bound-session replacement.
    }

    /**
     * Sends command 51 to the exact retired Session owner. Admission failure is owned by the
     * sender's bounded asynchronous retry queue; no ACK is expected for this one-way record.
     */
    ...
    default CompletionStage<ActorCreateResponse> requestActorCreate(
            RoutingId targetNodeRid, ActorCreateIntent intent, Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Remote Actor create is unavailable"));
    }

    default CompletionStage<UserSpotCreateResponse> requestUserSpotCreate(
            RoutingId targetNodeRid, UserSpotCreateIntent intent, Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Remote User Spot create is unavailable"));
    }

    default CompletionStage<UserSpotCloseResponse> requestUserSpotClose(
            RoutingId targetNodeRid, UserSpotCloseIntent intent, Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Remote User Spot close is unavailable"));
    }

    default void rememberSpotAuthority(SpotAuthorityRoute route) {
    ...
    default void forgetSpotAuthority(SpotAuthorityRoute route) {
        // Alternate backends may resolve the durable route on each call.
    }
    ...
    default void forgetInstanceIntent(ZLinkServiceM6BWireCodec.InstanceRouteFence route) {
        // Alternate backends may materialize Instance Spot elsewhere.
    }
    ...
    default CompletionStage<Void> recoverInstanceActivation(
        ...
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Instance activation recovery is unavailable"));
    }
    ...
    default CompletionStage<List<Message>> requestInstanceSpot(
            ...
            Duration timeout) {
        return CompletableFuture.failedFuture(
                new UnsupportedOperationException("Remote Instance Spot request is unavailable"));
    }

    interface UserSpotOperationHandler {
        CompletionStage<UserSpotCreateResponse> create(UserSpotCreateRequest request);

        CompletionStage<UserSpotCloseResponse> close(UserSpotCloseRequest request);
    }

    interface ActorCreateOperationHandler {
        CompletionStage<ActorCreateResponse> create(ActorCreateRequest request);
    }
    ...
    interface CanonicalActorJoinHandler {
        CompletionStage<CanonicalActorJoinResponse> admit(
                ...
                systems.zlink.framework.runtime.protocol.ServiceWirePilotCodec.ActorJoin28 join);
    }
    ...
    record CanonicalActorJoinRequest(
        ...
        public CanonicalActorJoinRequest {
            ...
            applicationPayload =
                    Objects.requireNonNull(applicationPayload, "applicationPayload").clone();
        }
    }
    ...
    record CanonicalActorJoinReply(
        ...
        public CanonicalActorJoinReply(
                boolean accepted, long receiveChunkLimitBytes, List<Message> applicationReply) {
            this(accepted, receiveChunkLimitBytes, applicationReply, null, "application/json");
        }

        public CanonicalActorJoinReply {
            applicationReply =
                    List.copyOf(Objects.requireNonNull(applicationReply, "applicationReply"));
    ...
    interface PeerAuthorityResolver {
        CompletionStage<Optional<PeerAuthorityFence>> resolve(
                String meshName, RoutingId sourceNodeRid, long sourceNodeGeneration);
    }
    ...
    record PeerAuthorityFence(
        ...
        public PeerAuthorityFence {
            ...
            if (sourceNodeGeneration == 0 || ownerLeaseGeneration <= 0) {
                throw new IllegalArgumentException(
                        "source node and owner lease generations must be positive");
            }
            if (ownerId == null || ownerId.isBlank()) {
                throw new IllegalArgumentException("ownerId must be non-blank");
            }
        }
    }
    ...
    interface RelocationControlHandler {
        CompletionStage<byte[]> handle(RoutingId sourceNodeRid, byte[] command);
    }

    @FunctionalInterface
    interface CanonicalRelocationControlHandler {
        CompletionStage<byte[]> handle(
                RoutingId sourceNodeRid, Long requestSequence, byte[] command);
    }
    ...
    interface ActorLeftHandler {
        CompletionStage<Void> handle(
                RoutingId targetNodeRid, ZLinkServiceM6BWireCodec.ActorLeft left);
    }

    @FunctionalInterface
    interface MessageFollowHandler {
        void handle(RoutingId sourceNodeRid, ZLinkServiceMessageFollowWireCodec.Notice notice);
    }

    @FunctionalInterface
    interface RelocationReplyRelayHandler {
        CompletionStage<byte[]> handle(
                RoutingId targetNodeRid, byte[] command33, List<byte[]> payload);
    }

    @FunctionalInterface
    interface SessionRelocationRouteHandler {
        CompletionStage<Void> handle(RoutingId sourceNodeRid, byte[] command44);
    }

    @FunctionalInterface
    interface SessionRelocationSealHandler {
        CompletionStage<byte[]> handle(RoutingId sourceNodeRid, byte[] command42);
    }
    ...
    interface BoundSessionReplacedHandler {
        void handle(
                RoutingId sourceNodeRid, ZLinkServiceM6BWireCodec.BoundSessionReplaced replacement);
    }
    ...
    record UserSpotCloseIntent(
            ZLinkServiceM6BWireCodec.UserSpotCloseFence target, long deadlineUnixMs) {
    ...
    record CanonicalActorJoinResponse(
            boolean accepted, long membershipEpoch, List<Message> applicationReply) {
