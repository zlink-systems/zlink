...
final class ZLinkActorSpotJoinCall implements ZLinkActorJoinCall {
    private static final String CANONICAL_ACTOR_JOIN_PACKET_NAME = "ZLinkFrameworkActorJoinRequest";
    private static final String CANONICAL_ACTOR_JOIN_CONTENT_TYPE = "application/json";
    ...
    private final AtomicBoolean acceptedCallbackDeliveredOnTarget = new AtomicBoolean();
    ...
    public ZLinkActorJoinCall timeout(Duration timeout) {
        ...
        ZLinkActorSpotJoinCall configured =
    ...
    CompletionStage<ZLinkActorJoinOutcome> execute() {
        return execute(newOperationId(), saturatedDeadline(System.nanoTime(), timeout.toNanos()));
    }

    CompletionStage<ZLinkActorJoinOutcome> execute(ZLinkActorJoinOperationId operationId) {
        return execute(operationId, saturatedDeadline(System.nanoTime(), timeout.toNanos()));
    }

    CompletionStage<ZLinkActorJoinOutcome> execute(
            ZLinkActorJoinOperationId operationId, long deadlineNanos) {
    ...
    private CompletionStage<Void> executeDeferred(
        ...
        if (remaining == null) {
            return notifyCompletion(
                    new ZLinkActorJoinCompletion.Failed(
                            operationId, ZLinkFrameworkErrorKind.DEADLINE_EXCEEDED));
        }
        ZLinkActorSpotJoinCall bounded = (ZLinkActorSpotJoinCall) timeout(remaining);
        ...
        try (var ignored =
                systems.zlink.framework.runtime.internal.handlers.ZLinkSuspendInvocationContext
                        .enterApplicationExecution(null)) {
            return bounded.execute(operationId, deadlineNanos)
                    .handle(
                            (result, error) -> {
                                if (error != null) {
    ...
    static Duration remainingTimeout(long deadlineNanos) {
        ...
        return remainingNanos <= 0 ? null : Duration.ofNanos(remainingNanos);
    }

    static long saturatedDeadline(long now, long timeoutNanos) {
        return timeoutNanos > 0 && now > Long.MAX_VALUE - timeoutNanos
                ? Long.MAX_VALUE
                : now + timeoutNanos;
    }

    private CompletionStage<Void> notifyCompletion(ZLinkActorJoinCompletion completion) {
        try {
            CompletionStage<Void> stage = context.actor().onJoinCompleted(completion);
            CompletionStage<Void> normalized =
                    stage == null ? CompletableFuture.completedFuture(null) : stage;
            return normalized;
        } catch (RuntimeException error) {
    ...
    static ZLinkActorJoinOperationId newOperationId() {
        ...
        do {
            id = UUID.randomUUID();
        } while (id.getMostSignificantBits() == 0L && id.getLeastSignificantBits() == 0L);
    ...
    private void traceJoinSent() {
        ZLinkMessageFlowTracer.TracePoint tracePoint =
                services.flow() == null
                        ? null
                        : services.flow().begin(ZLinkMessageFlowOutcome.SENT);
        if (tracePoint != null) {
            tracePoint.trace(
    ...
    private void traceJoinFailed(ZLinkFrameworkErrorKind kind, Throwable cause) {
        ...
        ZLinkMessageFlowTracer.TracePoint tracePoint = services.flow().beginDispatchError();
        if (tracePoint != null) {
            tracePoint.trace(
    ...
    private void traceJoinReplyReceived(Throwable error) {
        ZLinkMessageFlowTracer.TracePoint tracePoint =
                error != null || services.flow() == null
                        ? null
                        : services.flow().begin(ZLinkMessageFlowOutcome.REPLY_RECEIVED);
        if (tracePoint != null) {
            tracePoint.trace(
                    new ZLinkMessageFlowEvent(
                            ...
                            if (entryTarget) {
                                context.markMovedToEntrySpot(
                                        ...
                                        new ZLinkActorRuntime.EntrySpotTarget(
                                                result.actor().nodeRid(), joinedSpotId));
                            } else {
                                context.markJoined(
                                        result.actor(),
                                        joinedSpotId,
    ...
    private String effectiveJoinedSpotId(ZLinkBackendActorJoinResult result) {
        ...
        return joinedSpotId == null || joinedSpotId.toString().isBlank() ? spotId : joinedSpotId;
    }

    private CompletionStage<ZLinkBackendActorJoinResult> joinRemoteRoutedSpot(
            Message requestPart, ZLinkActorJoinOperationId operationId, long deadlineNanos) {
        CompletionStage<SpotTransportAddress> resolved =
                internalRouteChannel == null
                        ...
                        : resolveRemoteAddress(spotId)
                                .thenApply(
                                        address -> {
                                            if (!internalTargetNode.equals(
                                                    address.targetNodeRid())) {
                                                throw new ZLinkConfigurationException(
                                                        "resolved drain target does not match"
                                                                + " selected node: "
                                                                + internalTargetNode);
                                            }
        ...
        return resolved.thenCompose(
                target -> {
                    ...
                    return services.actors()
                            .readActorJoinAuthority(currentActorRef.actorId())
                            .thenCompose(
                                    authority -> {
                                        if (authority.objectGeneration()
                                                ...
                                                || !authority
                                                        .ownerNodeRid()
                                                        .equals(currentActorRef.nodeRid())) {
                                            return CompletableFuture.failedFuture(
                                                    new ZLinkFrameworkException(
                                                            ZLinkFrameworkErrorKind.PROTOCOL_ERROR,
                                                            "Actor Join Authority row does not"
                                                                    + " match the source Actor fence"));
                                        }
                                        ...
                                        if (meshNode == null) {
                                            return CompletableFuture.failedFuture(
                                                    new ZLinkFrameworkException(
                                                            ZLinkFrameworkErrorKind.UNAVAILABLE,
                                                            "canonical actor Join mesh transport is"
                                                                    + " unavailable"));
                                        }
                ...
                .thenCompose(
                        reply -> {
                            Message applicationReply =
                                    reply.applicationReply().isEmpty()
    ...
    private static ZLinkInternalMeshNode.CanonicalActorJoinRequest canonicalActorJoinRequest(
    ...
    private CompletionStage<ZLinkBackendActorJoinResult> submitCanonicalRelocation(
        ...
        ZLinkActorJoinRelocationPort.Goal goal =
                new ZLinkActorJoinRelocationPort.Goal(
                        ...
                        replyContentType);
        return services.actors()
                .relocateActorJoin(goal, remaining)
                .thenApply(
                        submission -> {
    ...
    private static UUID newRelocationId(ZLinkActorJoinOperationId operationId) {
        UUID id;
        do {
            id = UUID.randomUUID();
    ...
    private String entryRouterChannelId(SpotTransportAddress target) {
        ...
        return existing == null || existing.isBlank() ? target.routerChannelId() : existing;
    }

    private ZLinkBackendActorJoinResult rejectedRemoteJoin(
            ZLinkBackendActorRef currentActorRef, Message reply) {
        try (reply) {
            return new ZLinkBackendActorJoinResult(
                    ZLinkBackendRequestResult.OK,
                    1,
                ...
                .thenApply(
                        address -> {
                            if (address.isEmpty()
                                ...
                                throw new ZLinkConfigurationException(
                                        "SPOT remote address resolver returned an incomplete owner"
                                                + " snapshot: "
                                                + spotId);
                            }
                            return address.get();
    ...
    private static CompletionStage<SpotHandle> resolveHandle(
            SpotTransportAddressResolver resolver, String spotId) {
