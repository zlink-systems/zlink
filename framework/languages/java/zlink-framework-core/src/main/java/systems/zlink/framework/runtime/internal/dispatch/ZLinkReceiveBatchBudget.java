 ...
 *
 * <p>The first record is always eligible. This preserves progress for one record whose payload is
 * larger than the byte limit; subsequent records are admitted only while the count, byte, and
 * elapsed-time limits remain open. The caller owns the received records and must close or dispatch
 * them even when the budget stops the next receive.
 */
public final class ZLinkReceiveBatchBudget {
    ...
    public ZLinkReceiveBatchBudget() {
        this(DEFAULT_MAX_MESSAGES, DEFAULT_MAX_BYTES, DEFAULT_MAX_ELAPSED_NANOS);
    }

    public ZLinkReceiveBatchBudget(int maxMessages, long maxBytes, long maxElapsedNanos) {
        if (maxMessages <= 0) {
    ...
    public boolean canReceiveNext() {
        ...
        return byteCount < maxBytes && System.nanoTime() - startedNanos < maxElapsedNanos;
    }
    ...
    public static long bytesOf(
            List<Message> parts, int applicationMetadataBytes, int acceptedJournalRecordBytes) {
