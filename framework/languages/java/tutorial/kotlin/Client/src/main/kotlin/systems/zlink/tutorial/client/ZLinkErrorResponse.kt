...
class ZLinkErrorResponse {
    ...
    fun onFrameworkError(error: ZLinkFrameworkException): ResponseEntity<Map<String, String>> =
        ResponseEntity.status(statusFor(error.kind())).body(
            mapOf(
                "error" to nameFor(error.kind()),
                "message" to (error.message ?: ""),
            ),
        )

    // The same table the framework's own HTTP host uses.
    private fun statusFor(kind: ZLinkFrameworkErrorKind): HttpStatus = when (kind) {
    ...
    private fun nameFor(kind: ZLinkFrameworkErrorKind): String = when (kind) {
