...
public final class HttpClientProgram {
    ...
    public static void main(String[] args) {
        ...
        ZLinkHttpClient client = ZLinkHttpClient.create("http://127.0.0.1:5280")
        ...
        try {
            ...
            HttpResponse<NodeStatus> status = client
            ...
            try (ZLinkHttpClient admin = ZLinkHttpClient.create("http://127.0.0.1:5281")
                .basicAuth("ops", "tutorial-admin")
                .build()) {
                HttpResponse<WeightChanged> weight = admin
                    .post("/admin/channels/profile/weight")
                    .query("value", "2")
                    .submit(WeightChanged.class)
                    .toCompletableFuture()
                    .join();
                System.out.println("request shaping: status " + status.status()
                    + " weight " + weight.body().weight());
            }
            // --8<-- [end:http-request-shaping]

            // --8<-- [start:http-json-body]
            RawHttpResponse player = client
                .post("/players/p2")
                .body(new CreatePlayer("rookie"))
                .submitRaw()
                .toCompletableFuture()
                .join();
            RawHttpResponse room = client
                .post("/rooms")
            ...
            RawHttpResponse chat = client
                .post("/rooms/" + roomId + "/chat")
                .body(new PostChat("p2", "hello"))
                .submitRaw()
                .toCompletableFuture()
                .join();
            System.out.println("json body: player " + player.status() + " room " + roomId
                + " chat " + chat.status());
            ...
            HttpResponse<PlayerInfo> typed = client
                .get("/players/p2")
                .submit(PlayerInfo.class)
                .toCompletableFuture()
                .join();
            RawHttpResponse raw = client
                .get("/players/p2")
                .submitRaw()
                .toCompletableFuture()
                .join();
            PlayerInfo fetched = client
                .get("/players/p2")
                .fetch(PlayerInfo.class)
                .toCompletableFuture()
                .join();
            System.out.println("response kinds: typed " + typed.status() + " raw "
                + raw.headers().get("content-type") + " fetch " + fetched.nickname());
            ...
            try (ZLinkHttpClient compressed = ZLinkHttpClient.create("http://127.0.0.1:5280")
                .timeout(Duration.ofSeconds(3))
                .compression()
                .build()) {
                HttpResponse<RoomState> response = compressed
                    .get("/rooms/" + roomId)
                    .submit(RoomState.class)
                    .toCompletableFuture()
                    .join();
                boolean encodingRemoved = !response.headers().containsKey("content-encoding")
                    && response.body() != null;
                System.out.println("compressed response: " + response.status()
                    + " encoding-removed " + encodingRemoved);
            }
            // --8<-- [end:http-compressed-response]

            // --8<-- [start:http-redirect]
            try (ZLinkHttpClient redirects = ZLinkHttpClient.create("http://127.0.0.1:5280")
                .followRedirects()
                .build()) {
                HttpResponse<PlayerInfo> redirected = redirects
                    .get("/player/p1")
                    .submit(PlayerInfo.class)
                    .toCompletableFuture()
                    .join();
                System.out.println("redirect: " + redirected.status() + " " + redirected.body().playerId());
            }
            // --8<-- [end:http-redirect]

            // --8<-- [start:http-basic-auth]
            try (ZLinkHttpClient unauthenticated = ZLinkHttpClient.create("http://127.0.0.1:5281")
                .build();
                ZLinkHttpClient authenticated = ZLinkHttpClient.create("http://127.0.0.1:5281")
                    .basicAuth("ops", "tutorial-admin")
                    .build()) {
                int without = unauthenticated
                    .post("/admin/channels/profile/weight")
                    .query("value", "2")
                    .submitRaw()
                    .toCompletableFuture()
            ...
            HttpResponse<Imported> imported = client
            ...
            try (ZLinkHttpClient closedPort = ZLinkHttpClient.create("http://127.0.0.1:6180").build()) {
            ...
            System.out.println("error kinds: bad request " + badRequestKind
                + " connection refused " + connectionRefusedKind);
