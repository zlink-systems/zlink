...
- **Windows에서 별도 설정이 필요 없다.** `zlink-framework-core`의 POM이 binding
  ...
  native/windows-x86_64/zlink.dll
  ```
...
- **Redis가 필요하다.** `127.0.0.1:6379`, 키 prefix는 `zlink-tutorial-java:`다. Spot
  단계가 쓴다 — 방은 host가 아니라 id로 불리므로 지금 어느 node에 있는지를 Location Store에서
  읽는다. Channel 메시징은 channel 이름과 routing id로만 대상을 정하므로 Store가 필요 없다.
...
```bash
cd framework/languages/java/tutorial
./gradlew :java:Server:installDist :java:Client:installDist

export JAVA_HOME=/c/Users/<user>/.cache/zlink/jdk25/extracted/jdk-25.0.4.1+1
