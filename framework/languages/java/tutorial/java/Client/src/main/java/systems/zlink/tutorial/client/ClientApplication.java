...
class TutorialController {
    ...
    CompletionStage<ResponseEntity<Void>> recordLogin(@PathVariable String playerId) {
        ...
        return route
            .sendToChannel("profile", new Contracts.RecordLogin(playerId))
            .submit()
    ...
    CompletionStage<Contracts.NodeStatus> nodeStatus(@PathVariable String nodeRid) {
        ...
        return route
            .requestToNode("game", RoutingId.from(nodeRid), new Contracts.GetNodeStatus())
    ...
    CompletionStage<String> issueTicket(@PathVariable String playerId) {
        ...
        return route
            .requestToChannel("ticketing", new Contracts.IssueSessionTicket(playerId))
    ...
    CompletionStage<ResponseEntity<Void>> postChat(
        ...
        @RequestBody Contracts.PostChat message) {
    ...
    CompletionStage<ResponseEntity<Map<String, String>>> findRoom(@PathVariable String roomId) {
        return rooms.find(roomId).thenApply(found -> found
            .map(room -> ResponseEntity.ok(Map.of(
                "spotId", room.spotId(),
                "node", room.nodeRid().toString())))
    ...
    CompletionStage<ResponseEntity<Map<String, String>>> findPlayer(@PathVariable String playerId) {
        return playerManager.find(playerId).thenApply(found -> found
            .map(player -> ResponseEntity.ok(Map.of(
                "actorId", player.actorId(),
                "node", player.nodeRid().toString())))
            .orElseGet(() -> ResponseEntity.notFound().build()));
    }
    // --8<-- [end:location-find]
    ...
    CompletionStage<String> createPlayer(
        ...
        @RequestBody Contracts.CreatePlayer request) {
    ...
    CompletionStage<ResponseEntity<Void>> changeNickname(
        @PathVariable String playerId,
        @RequestBody Contracts.ChangeNickname message) {
    ...
    CompletionStage<Contracts.PlayerInfo> getPlayer(@PathVariable String playerId) {
        return players
            .requestToActor(playerId, new Contracts.GetPlayer())
            .timeout(Duration.ofSeconds(3))
