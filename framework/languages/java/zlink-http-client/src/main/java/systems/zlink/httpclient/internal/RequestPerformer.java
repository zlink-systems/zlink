 ...
 * integration, and same-origin {@code Authorization} scrubbing. Redirect/URL rules live in {@link
 * RedirectPolicy} and response decoding in {@link ResponseBodyReader}. Mirrors the C++ {@code
 * request_performer.cpp}. The redirect loop is an async {@link CompletionStage} chain so no thread
 * is parked between hops; the blocking body read is offloaded to an executor.
 */
public final class RequestPerformer {
    ...
    public CompletionStage<RawResult> performAsync(HttpRequestSpec spec) {
        ...
        URI current =
                URI.create(origin + RedirectPolicy.makeTarget(base.getRawPath(), spec.target()));
    ...
    private CompletionStage<RawResult> handle(
        ...
        return readWithDeadline(
                ...
                () -> {
                    ...
                    ResponseBodyReader.DecodedBody decoded =
                            options.compression()
                                    ? bodyReader.decompress(bytes, headers)
                                    : new ResponseBodyReader.DecodedBody(bytes, headers);
                    return new RawResult(
                            status, decoded.headers(), bodyReader.decodeText(decoded.body()));
                });
    }

    /**
     * Runs the offloaded body read with a deadline. {@code ofInputStream} completes {@code
     * sendAsync} after headers, so the HttpRequest timeout no longer covers body reading. Closing
     * the stream at the deadline unblocks a stalled read — freeing the worker thread and the
