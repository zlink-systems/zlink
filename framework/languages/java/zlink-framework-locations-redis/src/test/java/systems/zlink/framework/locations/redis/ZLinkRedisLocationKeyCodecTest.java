...
class ZLinkRedisLocationKeyCodecTest {
    ...
    void descriptorKeysRoundTripUnicodeWithoutNormalization() {
        var meshNode =
                new ZLinkMeshNodeDescriptorKey(
                        "mésh", RoutingId.from(new byte[] {(byte) 0xff, 0x00}));
        var clientServer =
                new ZLinkClientServerServerDescriptorKey("注文", RoutingId.from(new byte[] {0x01}));
        var publisher =
                new ZLinkFanoutPublisherDescriptorKey("이벤트", RoutingId.from(new byte[] {0x02}));
        ...
        assertEquals(
                ...
                keys.creationTerminalKey(
                        new ZLinkCreationOperationIdentity(
                                RoutingId.from(new byte[] {0x0a, 0x0b}), 7, 1, 2)));
    }
}
