...
public final class ZLinkRedisRelocationStore implements ZLinkRelocationStore, AutoCloseable {
                ...
                .thenApply(
                        raw -> {
                            ...
                            return new ZLinkBlobFound(
                                    rawBytes(raw.get(3)), now.plusMillis(number(raw.get(2))), now);
                        });
    }
    ...
    private static boolean cancelled(ZLinkStoreCancellation cancellation) {
        return Objects.requireNonNull(cancellation, "cancellation").isCancellationRequested();
    }

    private static <T> CompletionStage<T> cancelledStage() {
        return CompletableFuture.failedFuture(
                new CancellationException("store operation was cancelled before I/O"));
    }
