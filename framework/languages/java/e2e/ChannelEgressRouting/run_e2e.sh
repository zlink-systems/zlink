...
declare -A ROLE_FANOUT_BIND_HOST ROLE_FANOUT_ADVERTISE_HOST ROLE_FANOUT_PUBLISHER ROLE_FANOUT_SUBSCRIBER
declare -A ROLE_STREAM_BIND_HOST ROLE_STREAM_ADVERTISE_HOST ROLE_STREAM_SERVER
GAME_PROXY_PID=""
...
write_role_config() {
  ...
  local instance_spot="${14:-false}"
  local object_client="${15:-false}"
...
base_url, expected_text = sys.argv[1:]
...
while time.monotonic() < deadline:
    try:
        ...
        if (last.get("isReady") is True
                and int(last.get("readyTargetCount", 0)) == expected):
            raise SystemExit(0)
    except (OSError, ValueError, urllib.error.URLError):
        pass
    time.sleep(0.1)
...
base, expected = sys.argv[1], int(sys.argv[2])
deadline = time.monotonic() + 20
last = None
while time.monotonic() < deadline:
    try:
        with urllib.request.urlopen(base + "/status/fanout", timeout=1) as response:
            last = json.load(response)
        if last.get("isReady") is True and int(last.get("readyPublisherCount", 0)) >= expected:
            raise SystemExit(0)
    except OSError:
        pass
    time.sleep(0.1)
print(f"fanout did not become ready: {json.dumps(last)}", file=sys.stderr)
raise SystemExit(1)
PY
}
...
base_url, channel = sys.argv[1:]
deadline = time.monotonic() + 20
last = None
while time.monotonic() < deadline:
    try:
        with urllib.request.urlopen(base_url + "/status/route", timeout=1) as response:
            last = json.load(response)
        target = next((item for item in last.get("channels", [])
                       if item.get("channelName") == channel), None)
        if target is not None and target.get("isReady") is False \
                and int(target.get("readyTargetCount", -1)) == 0:
            raise SystemExit(0)
    except OSError:
        pass
    time.sleep(0.1)
print(f"route channel did not become unavailable: {json.dumps(last)}", file=sys.stderr)
raise SystemExit(1)
PY
}
...
start_game_proxy() {
  ...
  local port="${endpoint##*:}"
  setsid python3 "${SCRIPT_DIR}/tcp_proxy.py" \
...
assert_invalid_startup() {
  ...
  allocate_role "${role}"
  write_role_config "${role}" "${role}" "" "" \
    workflow.command "" "" "" "" "" true false 0
  start_role "${role}"
  local pid="${ROLE_PID[${role}]}"
  for _ in $(seq 1 50); do
    if ! kill -0 "${pid}" >/dev/null 2>&1; then
      wait "${pid}" >/dev/null 2>&1 && {
        echo "duplicate ChannelName host exited successfully" >&2
        return 1
      }
      if ! rg -qi 'channelname|already registered|duplicate' \
          "${LOG_DIR}/${CURRENT_SCENARIO}/${role}/stderr.log" \
          "${LOG_DIR}/${CURRENT_SCENARIO}/${role}/stdout.log"; then
        echo "duplicate ChannelName host did not report a configuration error" >&2
        return 1
      fi
      return 0
    fi
    if curl --max-time 1 -fsS "${ROLE_HTTP[${role}]}/health" >/dev/null 2>&1; then
...
run_one() {
  ...
  ROLE_CONFIG=()
  ROLE_GAME_BIND_HOST=()
  ROLE_GAME_ADVERTISE_HOST=()
  ROLE_WORKFLOW_BIND_HOST=()
  ROLE_WORKFLOW_ADVERTISE_HOST=()
  ROLE_FANOUT_BIND_HOST=()
  ROLE_FANOUT_ADVERTISE_HOST=()
  ROLE_FANOUT_PUBLISHER=()
  ROLE_FANOUT_SUBSCRIBER=()
  ROLE_STREAM_BIND_HOST=()
  ROLE_STREAM_ADVERTISE_HOST=()
  ROLE_STREAM_SERVER=()
  ...
  case "${CURRENT_SCENARIO}" in
    CH-E2E-01)
      ...
      ;;
    CH-E2E-02)
      allocate_role session; allocate_role play; allocate_role audit
    ...
    CH-E2E-03)
      ...
      write_role_config play play "" "" game.play "" "" "" "" "" \
    ...
    CH-E2E-06)
      assert_invalid_startup
      unset 'ROLE_PID[invalid-duplicate]'
      unset 'ROLE_CONFIG[invalid-duplicate]'
      allocate_role session; allocate_role play; allocate_role caller; allocate_role workflow-a
      write_role_config session session play "${ROLE_GAME[play]}" \
        game.session game.play "" "" "" "" false false 0
      write_role_config play play session "${ROLE_GAME[session]}" \
        game.play "" "" "" "" "" false false 0
      write_role_config caller caller "" "" "" "" "" "" "" "" true false 0
      write_role_config workflow-a workflow-a "" "" "" "" "" "" "" "" false true 100
      ;;
    CH-E2E-07A)
    ...
    CH-E2E-07B)
      ...
      write_role_config api-a api-a api-b "${ROLE_GAME[api-b]}" \
        game.api "" "" "" "" "" false false 0
      write_role_config api-b api-b api-a "${ROLE_GAME[api-a]}" \
        game.api "" "" "" "" "" false false 0
      ;;
    CH-E2E-07C)
      ...
      write_role_config session session "" "" \
        game.session game.api "" "" "" "" false false 0
      write_role_config api-a "${api_rid}" "" "" \
        game.api "" "" "" "" "" false false 0
      ;;
    CH-E2E-08)
      allocate_role play; allocate_role workflow-a; allocate_role caller
      write_role_config play play workflow-a "${ROLE_GAME[workflow-a]}" \
        game.play "" "" "" "" "" false false 0 true true
      write_role_config workflow-a workflow-a play "${ROLE_GAME[play]}" \
        "" game.play "" "" "" "" false true 100 false true
      write_role_config caller caller "" "" "" "" "" "" "" "" true false 0
      ;;
    CH-E2E-09)
  ...
  if [[ "${CURRENT_SCENARIO}" == "CH-E2E-07C" ]]; then
    ...
    for role in "${!ROLE_CONFIG[@]}"; do
      [[ -n "${ROLE_PID[${role}]:-}" ]] || start_role "${role}"
    done
  ...
  case "${CURRENT_SCENARIO}" in
    CH-E2E-01)
      wait_route session game.play; wait_route play game.session
      ;;
    CH-E2E-02)
      wait_route session game.play; wait_route play game.session
      wait_route play audit.record /status/audit; wait_workflow play 2
      ;;
    CH-E2E-03)
      wait_workflow play 1
      ;;
    CH-E2E-04A|CH-E2E-04B|CH-E2E-10)
      wait_workflow caller 2
      ;;
    CH-E2E-04C)
      wait_workflow caller 1
      stop_role workflow-a
      local workflow_port="${ROLE_WORKFLOW[workflow-a]}"
      allocate_role workflow-a
      ROLE_WORKFLOW[workflow-a]="${workflow_port}"
    ...
    CH-E2E-05)
      ...
      ;;
    CH-E2E-06)
      wait_route session game.play; wait_workflow caller 1
      ;;
    CH-E2E-07B)
      wait_route api-a game.api
      ;;
    CH-E2E-07C)
      wait_route session game.api
      stop_game_proxy
      wait_route_unavailable session game.api
      ;;
    CH-E2E-08)
      wait_route workflow-a game.play; wait_workflow caller 1
      ;;
      CH-E2E-09)
        wait_route session game.api; wait_workflow caller 1
        wait_fanout caller 1
        ;;
    CH-E2E-11)
      wait_route session game.api /status/route 2 2
      ;;
    CH-E2E-12)
      wait_workflow workflow-a 2
      ;;
  esac
...
case "${SELECTOR^^}" in
  ALL)
    for scenario in "${SCENARIOS[@]}"; do run_one "${scenario}"; done
    ;;
  CH01) run_one CH-E2E-01 ;;
  CH02) run_one CH-E2E-02 ;;
  CH03) run_one CH-E2E-03 ;;
  CH04A) run_one CH-E2E-04A ;;
  CH04B) run_one CH-E2E-04B ;;
  CH04C) run_one CH-E2E-04C ;;
  CH05) run_one CH-E2E-05 ;;
  CH06) run_one CH-E2E-06 ;;
  CH07A) run_one CH-E2E-07A ;;
  CH07B) run_one CH-E2E-07B ;;
  CH07C) run_one CH-E2E-07C ;;
  CH08) run_one CH-E2E-08 ;;
  CH09) run_one CH-E2E-09 ;;
  CH10) run_one CH-E2E-10 ;;
  CH11) run_one CH-E2E-11 ;;
  CH12) run_one CH-E2E-12 ;;
