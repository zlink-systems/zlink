...
| `Client/Scenarios/RlC2TopologyRecoveryScenario.cs` | `Client/src/main/java/systems/zlink/e2e/resiliencelifecycle/client/Scenarios/RlC2TopologyRecoveryScenario.java` | scenario | done | provider-b 강제 종료 뒤 owner lease TTL stale row가 제거되고, 남은 provider로 request가 수렴하며, provider-b 복구 후 topology와 traffic이 회복되는지 검증한다. |
