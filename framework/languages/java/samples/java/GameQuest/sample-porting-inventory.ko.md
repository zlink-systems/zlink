...
| Runner | 완료 | 실제 process runner가 topology, remote Instance Spot 활성화, scale-out, server evidence, reconnect, projection rebuild, sync/reconcile, owner rehydrate를 모두 확인한다. |
...
- runner가 `topology=ready`, `gamequest-scale-out=completed`, `gamequest-server-evidence=completed`,
  `gamequest-rehydrate=completed`를 출력했다.
- 같은 runner가 mission-a를 종료·재기동한 뒤 owner replay와 새 client의 channel 조회를 확인했다.
