...
# Regression for issue #632 (Java and Kotlin share this file): both ZoneWorld
# runners resolved the ZW-B8 fault proxy interpreter with
#     Get-Command python.exe, python, py.exe, py | Select-Object -First 1
...
# resolve and launch and then refuse to run anything (exit 9009).
# Get-ZlinkSamplePythonCommand (framework/languages/java/samples/redis-common.ps1)
...
function Get-PythonTheOldWay {
    $command = Get-Command python.exe, python, py.exe, py -ErrorAction SilentlyContinue |
        Select-Object -First 1
    if ($null -eq $command) { return $null }
    $arguments = @()
    if ($command.Name -in @('py.exe', 'py')) { $arguments += '-3' }
...
$probeDir = Join-Path ([IO.Path]::GetTempPath()) "zlink-python-resolution-$PID"
...
try {
    ...
    # while the new resolver still finds the install.
    ...
    Write-Host 'Negative control #1 confirmed: with no Python on PATH the shipped lookup resolves nothing, which is the "Python is required for the ZW-B8 fault proxy." stop reported in issue #632.'
    if ($IsWindows) {
    ...
    # to run anything, so "found" and "works" are not the same predicate.
    ...
    Write-Host 'PASS java/kotlin ZW-B8 python resolution: finds a working Python 3 off PATH and rejects an executable that resolves but cannot run'
} catch {
    $env:PATH = $originalPath
    Write-Error $_
    exit 1
