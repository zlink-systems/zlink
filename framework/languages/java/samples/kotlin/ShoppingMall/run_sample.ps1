...
try {
    ...
    $writeRole = { param($path, $id, $channel, $http) Set-ZlinkSampleProperties -Path $path -Value @("sample.instanceId=$id", "sample.httpEndpoint=$http", "sample.logDirectory=$LogDir", "sample.channelEndpoint=$channel", "sample.redisEndpoint=$redisEndpoint", "sample.redisKeyPrefix=$prefix", "sample.storeDirectory=$StoreDir") }
    ...
    Set-ZlinkSampleProperties -Path $clientConfig -Value @("sample.apiAHttpUrl=$apiAHttpUrl", "sample.apiBHttpUrl=$apiBHttpUrl")
    ...
    Set-ZlinkSampleProperties -Path $clientConfig -Value @("sample.apiAHttpUrl=$apiAHttpUrl", "sample.apiBHttpUrl=$apiBHttpUrl", "sample.pendingIdempotencyKey=$pendingKey", "sample.pendingOrderId=$($pending.orderId)", "sample.resumeOrderId=$checkpointOrder", "sample.rebuildOrderId=$($rebuild.orderId)")
