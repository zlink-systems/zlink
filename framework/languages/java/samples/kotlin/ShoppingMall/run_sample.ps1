...
function Start-Role([string]$Project, [string]$Name, [string]$Config, [string]$LogName) { $script = if ($IsWindows) { "$Name.bat" } else { $Name }; $bin = Join-Path $SampleDir "$Project/build/install/$Name/bin/$script"; $process = Start-ZlinkSampleProcess -FilePath $bin -ArgumentList @("--config", $Config) -WorkingDirectory $SampleDir -NoNewWindow -RedirectStandardOutput (Join-Path $LogDir $LogName) -RedirectStandardError (Join-Path $LogDir "$LogName.err.log") -PassThru; $Processes.Add($process); Register-ZlinkSampleProcessTree -Process $process; return $process }
