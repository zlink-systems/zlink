...
| `.NET: Client` | `Client/src/main/kotlin/.../supportchat/client/*` | client | done | agent/customer/reconnect/waiting-customer stream client self-check를 추가했다. agent는 방마다 `JoinConversationRes.actorId`의 Actor handle로 보내고, assignment/join/message/typing/idle/close/no-agent 검증을 public connector wait path로 수행한다. |
...
| `.NET: Server/Session` | `Server/Session/src/main/kotlin/.../server/session/*` | server-role | done | stream node, session spot mesh, authentication packet, identity actor binding, agent per-conversation actor binding, Actor slot 기반 relay를 추가했다. |
