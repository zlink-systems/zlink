...
class session_termination_test_access_t
{
  public:
    ...
    static size_t attached_pipe_count (socket_base_t *socket_)
    {
...
void test_terminating_lane_cannot_complete_delayed_pair_admission ()
{
    ...
    TEST_ASSERT_EQUAL_UINT64 (0, zlink::session_termination_test_access_t::
                                  attached_pipe_count (socket));
...
void test_monitor_owner_start_waits_for_idle_detach_linearization ()
{
    ...
    if (monitor && source && peer) {
        contract_socket_pair_t pair (source, peer);
        source_handle.socket->emit_inproc_connection_ready (pair.application[0]);
        const bool connected = pair.attached;
...
#define RUN_CTX_DESTROY_TEST(test_) \
    ...
    RUN_CTX_DESTROY_TEST (test_ctx_term_rearms_reaper_when_last_socket_closes_during_restart);
    RUN_CTX_DESTROY_TEST (test_pending_inproc_disconnect_releases_socket_before_context_term);
    RUN_CTX_DESTROY_TEST (test_engine_less_session_releases_socket_term_ack_with_pending_message);
    RUN_CTX_DESTROY_TEST (test_terminating_lane_cannot_complete_delayed_pair_admission);
