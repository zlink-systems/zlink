...
PAIR's public receive API is recv/poller-only: receive one complete record with
`zlink_recv()`, typically inside a poller loop. Both peers can send and
...
Place multipart data in array order and make one `zlink_send()` call. Core
submits the complete array atomically as one record.
