...
- XPUB exposes SUB subscription events via `zlink_xpub_recv()`,
...
|--------|-----------|-------------|-------|
| PUB | Send only | `zlink_publish()` | Cannot receive |
| SUB | Receive only | `zlink_subscribe()` | Topic + payload array returned separately |
| XPUB | Bidirectional | `zlink_publish()` / `zlink_xpub_recv()` | Receives subscription events |
...
> **Note:** Passing `NULL` as `topic_id_` (`zlink_publish(pub, NULL, &part, ...)`)
...
| 3 | SUB | `zlink_subscribe()` | Final consumption |

> **Key point:** The proxy data relay in `zlink_proxy(xsub, xpub, NULL)` uses
> internal recv/send paths, not the public `zlink_send()`/`zlink_recv()`
