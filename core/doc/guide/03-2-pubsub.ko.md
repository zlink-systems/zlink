...
- XPUB은 SUB의 구독 이벤트를 `zlink_xpub_recv()`로 노출해
...
| 소켓 | 방향 | 송수신 API | 비고 |
|------|------|----------|------|
| PUB | 송신 전용 | `zlink_publish()` | 수신 불가 |
| SUB | 수신 전용 | `zlink_subscribe()` | 토픽 + payload 배열 분리 반환 |
| XPUB | 양방향 | `zlink_publish()` / `zlink_xpub_recv()` | 구독 이벤트 수신 |
...
`zlink_publish()`는 **토픽**과 payload part 배열을 별도 파라미터로 받는다.
...
zlink_publish(pub, "quotes.KRW-BTC", &quote, 1, ZLINK_SEND_FLAGS_DONTWAIT);
...
`zlink_publish()`가 `ZLINK_SUBMIT_BACKPRESSURED`를 반환해 호출자가 대응할
수 있다.
...
memcpy(zlink_msg_data(&quote), &tick, sizeof(tick));
zlink_submit_result_t rc = zlink_publish(
    pub, "quotes.KRW-BTC", &quote, 1, ZLINK_SEND_FLAGS_DONTWAIT);
...
| 단계 | 주체 | 동작 | 비고 |
|------|------|------|------|
| 1 | PUB | `zlink_publish(pub, topic, ...)` | 데이터 발행 |
