...
> **이 장이 답하는 것** — ROUTER·DEALER·PAIR와 PUB 계열 소켓을 조합해
> 메시지를 중계하는 proxy를 구성하는 방법을 설명한다. 개별 socket의 계약은
> 각 socket 스펙이 소유한다.
...
- **소켓 타입 제한 없음** — 내부적으로 raw socket API와 같은 `socket_base_t`
  recv/send 경로를 그대로 쓰므로, 공개 send/recv 표면이
  `ZLINK_SUBMIT_NOT_SUPPORTED` / `ZLINK_RECV_NOT_SUPPORTED`를 반환하는 소켓
  (예: XSUB, XPUB)을 짝지어도 proxy 안에서는 정상 동작한다
