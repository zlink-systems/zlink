...
An invalid event bit returns `ZLINK_CONFIG_INVALID_ARGUMENT` with `EINVAL`; an event unsupported by the source returns `ZLINK_CONFIG_NOT_SUPPORTED` with `ENOTSUP`. Destroy while wait is active returns `ZLINK_CLOSE_BUSY` with `EBUSY`. The [errno map](03-errors.en.md#result-and-errno-mapping) defines all result and errno mappings.
