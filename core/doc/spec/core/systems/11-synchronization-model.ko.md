...
> 어떤 장치로 잇는지, 그 장치를 잡고 놓고 깨우는 실제 규칙, 그리고 message마다 실행되는
> 경로(hot path)에 lock을 두어도 되는 조건. 이 장의 규칙은 Core의 모든 변경이 따르며, 코드가
> 다르면 코드를 고친다. 현재 코드가 규칙과 어디서 다르고 어떤 순서로 맞추는지는
> [Core 리팩토링 계획 §7.7](../../../../doc/plan/core-refactor-stream-perf-0.17.0-plan.ko.md#77-동기화-모델-인벤토리와-목표)이
> 소유한다.

## 1. Synchronization 개요

Core의 hot path는 application thread가 queue에 message를 넣고 I/O thread가 그 queue에서 꺼내
보내거나, 그 반대로 넣어 주는 일이다. 이 장은 그 queue와 queue 주변 상태를 여러 thread가 함께
만질 때 어느 thread가 무엇을 소유하는지, 소유가 갈리는 지점마다 어떤 장치 — lock,
single-producer queue, atomic 값 — 를 쓰는지, 그리고 그 장치를 어떻게 다루는지 정한다.
...
| Core 유지보수자 | hot path에 lock을 더하거나 뺄 때 답해야 하는 질문과 제출해야 하는 검증 |
...
**잡는 방법.** turn은 공개 API 진입 상태어의 `public_api_sync` bit이다. 잡는 동작은 그 상태어에
대한 compare-and-swap 한 번이고, 놓는 동작은 release store 한 번이다. 경합이 없으면 비용은
atomic 연산 한 쌍이며, 경합하면 [Thread safety §3](04-thread-safety.ko.md#3-내부-규칙)의 backoff
규칙을 따른다 — 짧은 yield 뒤 재시도이고, 이 대기는 socket 상태를 읽지 않는다.
...
두려면 그 command가 C2 상태를 만지지 않는다는 것을 [§6](#6-검증-요구)의 표로 보여야 하며,
"자주 실행되니 lock-free로 둔다"는 이유는 예외의 근거가 아니다 — 그 가정은 측정 대상이다.

**turn 안에서 하지 않는 것.** turn을 쥔 코드는 같은 socket의 공개 API를 다시 부르지 않고(비재진입),
다른 socket의 turn을 기다리지 않으며(inproc peer의 상태는 command로 바꾼다), 블로킹 대기를
하지 않는다.

**블로킹 대기.** `NONE` send가 admission을, blocking recv가 message를 기다릴 때 그 thread는
turn을 **놓고** 기다린다. 대기는 signaler 또는 condition variable 위에서 하고, 깨어나면 turn을
다시 잡은 뒤 조건을 다시 검사한다. 대기 중에 turn을 쥐고 있으면 command owner가 그 socket에
message나 credit을 전달할 수 없어 대기가 끝나지 않는다.

### 3.2 pipe의 두 끝

socket과 session 사이에서 message를 나르는 pipe([Architecture](01-architecture.ko.md))는 양 끝이
각각 thread 하나에 고정된다. socket 쪽 끝은 socket turn이, session 쪽 끝은 그 connection의 I/O
thread가 소유한다([Threading model §3](02-threading-model.ko.md#3-thread-간-통신)).

**queue.** 두 끝 사이의 queue(`ypipe_t`)는 넣는 쪽 하나와 꺼내는 쪽 하나만 있는
single-producer single-consumer 구조라 lock이 없다. 넣는 쪽은 write 뒤 flush에서 CAS 한 번으로
"꺼내는 쪽이 잠들어 있는가"를 판정하고, 잠들어 있을 때만 `activate_read` command를 보낸다.
꺼내는 쪽은 queue가 비면 같은 CAS로 자신을 잠든 상태로 표시한다. 이 전이가 깨어남의 유일한
근거이며, 깨어남을 따로 세거나 중복해서 보내지 않는다.

**pipe 끝의 상태.** 그 끝을 소유한 thread만 쓴다. 반대쪽 끝이 읽어야 하는 값 — peer가 소비한
message·byte 수, 출력 활성 여부 — 만 C3 atomic으로 발행한다. 발행은 release store, 읽기는
acquire load다. 두 끝이 같은 값을 쓰는 일은 없다. 활성 여부처럼 한 방향으로만 바뀌는 플래그의
전이는 CAS로 한 쪽이 단독 확정한다.

**credit.** byte credit은 두 끝이 각자 세는 두 단조 counter — 쓴 양과 읽은 양 — 로 이뤄지고,
여유 credit은 두 값의 차다. 어느 쪽도 상대의 counter를 잠그지 않는다. credit이 없어 보내지
못한 쪽은 turn을 놓고 기다리며, 읽는 쪽이 counter를 올린 뒤 `activate_write` command로 깨운다.

**cold 경로.** pipe 분리, peer 식별, 종료처럼 message당 0회인 경로는 pipe 상태 전체를 바꾼다.
이 경로는 command로 소유 thread에서 실행하거나, 그 경로 쪽만 lock을 잡는다. hot path의
write·read·flush는 그 lock을 잡지 않는다.
...
꺼내는 multi-producer single-consumer 구조다.

**삽입.** 넣는 쪽이 여럿이므로 삽입점에 lock 하나가 있다. 이것이 hot path에서 허용되는 유일한
"여러 producer" lock이다. lock 안에서 하는 일은 queue 삽입과 "소비 쪽이 잠들어 있었는가"의
판정뿐이다.

**깨어남.** 소비 쪽이 잠들어 있었을 때만 signaler(eventfd 또는 fd pair)에 한 번 쓴다. 이미 깨어
있는 소비 쪽에는 쓰지 않는다 — 깨어남은 "잠듦→깸" 전이당 한 번이다. 소비 쪽은 mailbox를 다
비운 뒤에야 잠들고, 잠들기 직전에 queue를 한 번 더 확인해 그 사이에 들어온 command를 놓치지
않는다. signaler를 읽어 소비하는 주체는 하나여야 한다 — poller와 command owner가 같은
signaler를 각각 읽으면 한쪽의 깨어남이 사라지므로, 읽은 쪽이 다시 무장(re-arm)하는 규칙을
[Polling](../05-polling.ko.md)이 정한다.

**꺼냄.** 꺼내는 쪽은 소유 thread 하나라 lock이 없다. 꺼낸 command는 [§3.1](#31-socket-turn)대로
socket turn 아래에서 적용한다.
...
사건마다 한 번이다. plan처럼 두 값(계획·적용)을 가진 상태는 두 값이 같으면 lock을 잡지 않고
돌아온다.

### 3.5 lock의 종류와 순서

- **lock 종류.** 기본은 재진입하지 않는 `mutex_t`다. 같은 thread가 같은 lock을 다시 잡아야
  하는 자리만 `recursive_mutex_t`를 쓰고, 그 재진입 경로를 선언부 주석에 적는다. 재귀 mutex를
  condition variable과 함께 쓰지 않는다. 디버그·sanitizer 빌드의 `mutex_t`는 재진입을 즉시
  잡는(`ERRORCHECK`) 모드로 만든다.
- **잡는 순서.** socket turn → 그 socket이 소유한 pipe 끝 → context 레지스트리 순서로만 잡는다.
  역순으로 잡지 않고, 한 socket의 turn을 쥔 채 다른 socket의 turn이나 pipe 끝을 잡지 않는다.
  inproc처럼 두 socket이 pipe 하나로 마주 보는 경우, 상대 socket에 영향을 주는 변경은 command로
  보낸다.
- **memory ordering.** 다른 thread에 값을 발행할 때는 release store, 읽을 때는 acquire load를
  쓴다. `seq_cst`는 "잠듦→깸" 전이처럼 두 thread가 서로의 결정을 동시에 봐야 하는 자리에만
  쓴다. relaxed는 통계 counter처럼 순서가 의미 없는 값에만 쓴다.
...
hot path에 새 lock을 들이는 변경은 이 질문의 답과 [§6](#6-검증-요구)의 검증을 함께 낸다.

## 5. 금지되는 형태

| 형태 | 왜 안 되는가 |
|---|---|
| 지키는 조건이 없는 lock | 경합이 없어도 lock 한 쌍마다 명령 수와 cache line 왕복이 든다 |
| 함께 지켜야 할 조건을 여러 lock으로 나눔 | 나눈 경계에서 조건이 깨진다(C2 규칙) |
| socket type이나 command 종류별 turn 예외 | "자주 실행된다"는 가정은 측정 대상이지 규칙의 근거가 아니다 |
| 반대쪽 pipe 끝이 쓰는 값을 lock으로 읽음 | single-producer 구조를 lock으로 다시 감싼다 |
| turn이나 lock을 쥔 채 블로킹 대기 | 깨워 줄 thread가 그 lock을 기다린다 |
| 이름과 성질이 다른 장치 | 재귀 mutex를 `pthread_cond_wait`와 함께 쓰면 정의되지 않은 동작이다 |
| cold 경로 때문에 hot path가 lock을 짊어짐 | message당 0회인 경로가 message당 lock을 강제한다 |
...
1. **lock별 표** — 획득 지점, message당 획득 횟수(callgrind), 지키는 조건, 두 번째 thread,
   ...
   다섯 셀이다.
2. **깨어남 조건 불변** — [§3.2](#32-pipe의-두-끝)와 [§3.3](#33-mailbox와-깨어남)의 내부 확인
   조건. 변경 전후 코드가 같거나, 다르면 왜 같은 조건인지.
