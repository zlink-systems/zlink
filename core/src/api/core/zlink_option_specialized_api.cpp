...
//  Same shape as set/get_socket_only_option above, but resolved through
//  option_target_t: pub/sub options accept the underlying xpub/xsub type
//  as well as the public pub/sub type, so the target's actual socket type
//  is passed through to set/get_socket_option_checked rather than assumed.
zlink_config_result_t set_socket_option_by_target (void *handle_,
                                                   int socket_option_,
                                                   int expected_a_,
                                                   int expected_b_,
                                                   const void *optval_,
    ...
    if (target.kind == zlink::option_target_socket) {
        return zlink::config_result_internal::from_rc (
          set_socket_option_checked (target.socket, socket_type_of (target.socket),
                                     expected_a_, expected_b_, socket_option_, optval_,
                                     optvallen_));
    }
    return zlink::config_result_internal::from_errno (errno);
}

zlink_config_result_t get_socket_option_by_target (void *handle_,
                                                   int socket_option_,
                                                   int expected_a_,
                                                   int expected_b_,
                                                   void *optval_,
                                            ...
                                            size_t optvallen_)
{
    ...
    return set_socket_option_by_target (handle_, socket_option, ZLINK_CORE_SOCKET_PUB,
                                        ZLINK_CORE_SOCKET_XPUB, optval_, optvallen_);
}
zlink_config_result_t
zlink_get_pub_option (void *handle_, zlink_pub_option_t option_, void *optval_, size_t *optvallen_)
{
    ...
    return get_socket_option_by_target (handle_, socket_option, ZLINK_CORE_SOCKET_PUB,
                                        ZLINK_CORE_SOCKET_XPUB, optval_, optvallen_);
}
                                            ...
                                            size_t optvallen_)
{
    ...
    return set_socket_option_by_target (handle_, socket_option, ZLINK_CORE_SOCKET_SUB,
                                        ZLINK_CORE_SOCKET_XSUB, optval_, optvallen_);
}
...
zlink_get_sub_option (void *handle_, zlink_sub_option_t option_, void *optval_, size_t *optvallen_)
{
    ...
    return get_socket_option_by_target (handle_, socket_option, ZLINK_CORE_SOCKET_SUB,
                                        ZLINK_CORE_SOCKET_XSUB, optval_, optvallen_);
}
