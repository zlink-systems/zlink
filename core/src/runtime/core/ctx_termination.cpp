...
void zlink::ctx_t::wait_for_reaper_done ()
{
    command_t cmd;
    ...
    //  objects, so a stale or coalesced edge can outlive its command. A signal
    //  reports EINTR. Neither is a termination result - keep waiting for the
    //  one reaper acknowledgement this mailbox exists to deliver.
    while (true) {
