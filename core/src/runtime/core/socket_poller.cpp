...
void zlink::socket_poller_t::reserve (size_t items_)
{
    if (items_ > _items.capacity ())
        _items.reserve (items_);
}
...
zlink::signaler_t *zlink::socket_poller_t::ensure_socket_signaler ()
{
    if (!_socket_signaler) {
#if defined ZLINK_HAVE_WINDOWS
        _socket_signaler = new (std::nothrow) signaler_t (false, true);
#else
