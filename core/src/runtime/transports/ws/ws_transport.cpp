...
void ws_transport_t::async_writev (const boost::asio::const_buffer *buffers,
                                   std::size_t buffer_count,
                                   completion_handler_t handler)
{
    const std::shared_ptr<connection_generation_t> connection = _connection;
    ws_transport_common_internal::async_writev (
      connection, connection && connection->handshake_complete, buffers,
      buffer_count, std::move (handler), "WS");
}
