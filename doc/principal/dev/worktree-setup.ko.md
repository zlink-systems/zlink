...
WSL `/home/hep7/worktree/zlink-<N>-<slug>` 또는 Windows `D:\worktree\zlink-<N>-<slug>` worktree를 만들고 로컬 패키지를 준비한다. 이때 binding 패키지는
**빌드하지 않고 링크한다**.
...
`work.sh done --verified <sha>`(Windows `work.ps1`)는 PR HEAD SHA를 확인하고, 열린 PR을 merge하거나
