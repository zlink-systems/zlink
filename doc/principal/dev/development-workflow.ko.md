...
1. **간단한 작업**: 새 검증이 필요 없는 작은 수정 — 기록 문서·안내 문서의 문장과 오탈자, 에이전트 규칙
   문서(`AGENTS.md` 등), 설정 값 한두 줄. 별도 worktree를 만들지 않고 main worktree에서 작업해
   `git commit -- <경로>`로 파일을 지정해 커밋한다. 동작을 바꾸는 코드, 스펙, 여러 모듈에 걸친 수정은 PR이다.
