...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `ROUTER_ROUTER / inproc` before throughput ratio는 50.385%/54.026%/50.999%/23.156%/57.889%/67.866%, 산술평균 50.720%이며 before latency ratio는 9.400x/1.904x/1.836x/4.500x/1.769x/1.500x, 산술평균 3.485x다. Sol `Unsafe.SkipInit` after throughput ratio는 54.791%/65.056%/50.935%/23.565%/60.250%/64.205%, 산술평균 53.134%, 평균 latency ratio 3.043x다. local routed one-way aggregate 목표 55%와 latency 상한 3.0x에 미달해 보류하고 다음 대상은 아직 미측정인 `Single ROUTER_ROUTER / ipc`다. |
| paired C | 완료 | C throughput은 2688652/1822075/2001808/483366/165454/85244 Kmsg/s, 평균 latency는 0.015/0.166/0.177/0.006/0.013/0.020 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/single/report/perf_c_single_linux_20260810_173203_dotnet-router-router-inproc-paired-c1.txt`다. |
| binding paired 결과 | 완료·미달 | .NET before throughput은 1354680/984394/1020907/111930/95780/57852 Kmsg/s다. Sol after throughput은 1473140/1185362/1019612/113906/99686/54731 Kmsg/s이며 평균 latency는 0.103/0.252/0.339/0.028/0.022/0.032 ms다. before 대비 Sol after throughput은 50.720%→53.134%, latency는 3.485x→3.043x로 개선됐다. private native routing-id cache A/B는 throughput 52.251%, latency 2.332x였지만 public `RoutingId` struct layout·생성 비용 변경으로 제거했다. |
| 개선 결과 | 보류·Sol `Unsafe.SkipInit` 유지 | `ReceiveRouterParts`의 `zlink_msg_init` 전 중복 zeroing을 제거한 Sol 후보를 유지한다. before보다 throughput과 latency가 모두 개선됐지만 aggregate throughput 55%와 latency 3.0x에 미달하고 추가 contract-safe 후보가 없어 보류한다. public contract·ownership·error semantics는 변경하지 않았다. |
...
| 2 | .NET | 완료·통과 23 / 보류 13 | 미측정 | 다음 대상은 아직 미측정인 `Single ROUTER_ROUTER / ipc`; C → .NET 순서로 한 대상씩 측정 |
