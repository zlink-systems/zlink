...
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  ...
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_PUBSUB / tcp`를 보류로 확정했으므로, 개선 검토가 빠진 `MULTI_ROUTER_ROUTER_SENDSEND / tcp`와 `MULTI_ROUTER_ROUTER_REQREP / tcp`를 C → .NET 순서로 한 대상씩 재검토한다.
...
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 59.456% | 41.294% | 84.284% | 70.016% | 131.140% | 122.806% | 재검토 대기·C semantic `MULTI_ROUTER_ROUTER_SENDSEND`와 .NET runner `MULTI_ROUTER_ROUTER`를 같은 routed echo로 paired 측정했다. 기존 parity correction 결과 throughput 산술평균 84.832%, 평균 latency ratio 1.388x다. 통과했지만 hot path/POSDDD 후보 재검토가 남아 있다. C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_193520_dotnet-multi-router-router-sendsend-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_193856_dotnet-multi-router-router-sendsend-tcp-paired-before-connect-rid.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 60.123% | 56.588% | 64.781% | 61.228% | 125.799% | 102.361% | 재검토 대기·C semantic과 .NET runner `MULTI_ROUTER_ROUTER_REQREP`를 같은 routed request/reply로 paired 측정했다. 기존 parity correction 결과 throughput 산술평균 78.480%, 평균 latency ratio 1.409x다. 통과했지만 hot path/POSDDD 후보 재검토가 남아 있다. C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_194652_dotnet-multi-router-router-reqrep-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_194708_dotnet-multi-router-router-reqrep-tcp-paired-before-connect-rid.txt` |
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `MULTI_PUBSUB / tcp` C와 .NET을 같은 one-way publish/subscribe 의미로 paired 측정했다. 최종 throughput ratio는 60.588%/60.647%/77.217%/85.441%/106.666%/107.023%, 산술평균 82.930%이며 평균 latency ratio 산술평균은 1.101x다. .NET simple one-way aggregate 목표 85%에 미달해 보류한다. 다음 대상은 개선 검토가 빠진 `MULTI_ROUTER_ROUTER_SENDSEND / tcp`다. |
| paired C | 완료 | C throughput은 1578456/1516658/1115962/538284/102952/56610 Kmsg/s, 평균 latency는 346.451/385.566/309.695/255.565/106.523/95.833 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_195045_dotnet-multi-pubsub-tcp-paired-c1.txt`다. |
| binding paired 결과 | 완료·측정값 기록 | .NET lifecycle rebaseline throughput은 956353/919808/861707/459917/109815/60586 Kmsg/s, 평균 latency는 421.442/417.280/390.531/312.956/99.327/85.491 ms다. 초기 baseline은 72.264%, 자체 PollOut after는 73.963%, lifecycle rebaseline은 82.930%다. |
| 개선 결과 | 보류·lifecycle/POSDDD 개선 채택 | 자체 publish EAGAIN 후 PollOut 대기를 적용해 72.264%→73.963%로 개선했다. C와 .NET의 active deadline, stop token 전송 책임을 맞춘 lifecycle rebaseline을 최종 결과로 채택했으며, 이는 public contract·ownership·error semantics와 측정 의미를 유지한다. Sol `Unsafe.SkipInit` 후보 A/B는 throughput 78.895%·latency 1.121x로 lifecycle rebaseline보다 악화되어 제거했다. 추가 contract-safe 효과 후보는 없어 보류한다. build `0 warning / 0 error`, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 4 / 보류 2, 나머지 미측정 | 개선 검토가 빠진 `MULTI_ROUTER_ROUTER_SENDSEND / tcp`를 C → .NET 순서로 재검토 |
...
| 2026-08-10 | .NET | Multi / MULTI_ROUTER_ROUTER_SENDSEND (runner `MULTI_ROUTER_ROUTER`) / tcp / 64·256·1024·4096·65536·131072B | `dotnet-multi-router-router-sendsend-tcp-paired-before-connect-rid` | 59.456%, 41.294%, 84.284%, 70.016%, 131.140%, 122.806% | 84.832% (산술평균) | 통과·C semantic과 .NET runner pattern을 정렬하고 `CONNECT_ROUTING_ID=SERVER` parity correction 후 complete·latency 1.388x·추가 hotpath pass 없음 | C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_193520_dotnet-multi-router-router-sendsend-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_193856_dotnet-multi-router-router-sendsend-tcp-paired-before-connect-rid.txt` |
| 2026-08-10 | .NET | Multi / MULTI_ROUTER_ROUTER_REQREP / tcp / 64·256·1024·4096·65536·131072B | `dotnet-multi-router-router-reqrep-tcp-paired-before-connect-rid` | 60.123%, 56.588%, 64.781%, 61.228%, 125.799%, 102.361% | 78.480% (산술평균) | 통과·C/.NET Router identity parity correction 후 complete·평균 latency ratio 1.409x·추가 hotpath pass는 재검토 예정 | C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_194652_dotnet-multi-router-router-reqrep-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_194708_dotnet-multi-router-router-reqrep-tcp-paired-before-connect-rid.txt` |
