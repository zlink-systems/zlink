...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, `tcp / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_DEALER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_PUBSUB` 완료·보류, `tcp / MULTI_STREAM` 완료·보류, `ws / MULTI_DEALER_DEALER` 완료·보류, `ws / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, `ws / MULTI_DEALER_ROUTER_REQREP` 완료·통과, `ws / MULTI_ROUTER_ROUTER_SENDSEND` 완료·통과, `ws / MULTI_ROUTER_ROUTER_REQREP` 완료·통과, `ws / MULTI_PUBSUB` 완료·통과, `ws / MULTI_STREAM` 완료·보류, `wss / MULTI_STREAM` 완료·통과, `tls / MULTI_STREAM` 완료·통과, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_STREAM / tls`의 C → .NET paired 측정과 자체·Sol 개선 검토를 완료했으므로, 다음 미측정 대상 `MULTI_DEALER_DEALER / wss`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `MULTI_STREAM / tls` C와 .NET runner `MULTI_STREAM`을 같은 stream echo 의미로 paired 측정했다. C와 .NET은 clients 7000, duration 1s, runs 1, 64/256/1024/65536B, connect concurrency 1024, monitor HWM 4096000, auto-HWM balanced 조건이다. baseline throughput 산술평균 93.025%/latency 1.142x로 simple one-way 기준을 통과했다. 생성 참조가 없는 dead `StreamSendOperation`을 제거한 own 결과는 85.590%/1.205x이며, Sol review는 public contract·ownership·error semantics 영향이 없는 POSDDD 구조 정리로 GO하고 TLS 고유 비용 외 추가 hotpath 후보를 no-go 판정했다. 다음 대상은 아직 미측정인 `MULTI_DEALER_DEALER / wss`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 13 / 보류 5, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_DEALER_DEALER / wss`; C → .NET 순서로 한 대상씩 측정 |
