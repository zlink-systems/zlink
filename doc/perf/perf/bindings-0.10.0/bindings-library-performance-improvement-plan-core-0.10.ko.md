...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `DEALER_ROUTER / ipc` baseline throughput ratio는 59.439%/74.358%/76.068%/86.025%/86.635%/94.496%, 산술평균 79.503%이며 latency ratio 산술평균은 1.180x다. 자체 routed inlining after throughput ratio는 60.863%/74.941%/76.114%/89.097%/88.914%/92.107%, 산술평균 80.339%, latency 1.288x로 개선됐지만 throughput 목표 85%에 미달한다. Sol `ReceiveRouterParts` `Unsafe.SkipInit` off A/B는 throughput 79.839%, latency 1.367x로 더 악화되어 기존 후보를 유지한다. 추가 후보가 없어 보류하며 다음 대상은 아직 미측정인 `Single DEALER_ROUTER_REQREP / ipc`다. |
| paired C | 완료 | C throughput은 2571763/1353246/883394/41831/27369/16153 Kops/s, 평균 latency는 0.200/0.204/0.245/4.891/7.545/12.923 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/single/report/perf_c_single_linux_20260810_181357_dotnet-dealer-router-ipc-paired-c1.txt`다. |
| binding paired 결과 | 완료·미달 | .NET baseline throughput은 1528621/1006250/671981/35985/23711/15264 Kops/s, 평균 latency는 0.246/0.257/0.300/5.670/8.664/13.662 ms다. baseline throughput 산술평균은 79.503%, latency는 1.180x다. 자체 after throughput은 1565258/1014132/672384/37270/24335/14878 Kops/s, 평균 latency는 0.418/0.216/0.307/5.479/8.514/13.929 ms다. after throughput 산술평균은 80.339%, latency는 1.288x다. Sol A/B off throughput은 1483380/961427/673312/39577/24348/14621 Kops/s, ratio 산술평균 79.839%, latency 1.367x다. |
| 개선 결과 | 보류·Sol 후보 유지 | `SendRoutedMessageUnchecked` inlining은 throughput aggregate를 개선했고 latency aggregate는 기준 내라 최종 코드에 유지한다. Sol `ReceiveRouterParts` `Unsafe.SkipInit`도 off A/B보다 throughput·latency가 개선되어 유지한다. 추가 contract-safe 후보는 no-go이며 throughput aggregate 85%에는 미달한다. public contract·ownership·error semantics는 변경하지 않았고 build 성공, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 25 / 보류 16 | 미측정 | 다음 대상은 아직 미측정인 `Single DEALER_ROUTER_REQREP / ipc`; C → .NET 순서로 한 대상씩 측정 |
