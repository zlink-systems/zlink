...
- Multi 상태: `미측정`
- 다음 작업: C++ Single 표의 미측정 transport/pattern을 측정한다. 각 항목은 C를 먼저 단독 측정하고 같은 조건의 C++을 이어서 측정하며, 완료된 결과만 9.1.1과 11절에 기록한다. C++ Single이 완료된 뒤에만 Multi suite와 다음 binding으로 이동한다.
...
|------|------|-------------|------------|-----------|
| 1 | C++ | 진행 중·안정성 blocker | 미측정 | `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER`의 선택 transport 측정을 완료했고 `DEALER_ROUTER_REQREP / ipc`는 64B 진단 ratio 94.38%와 C++ variation 11.56%로 안정성 blocker로 분류했다. `ROUTER_ROUTER / tcp`는 timeout parity 후 안정성 blocker로 분류했다. `DEALER_ROUTER_REQREP / tcp`는 공식 중앙값 94.87%로 미달했고, `ROUTER_ROUTER / tls`는 262144B 안정 boundary ratio 약 83.92%, `/ inproc`은 262144B 안정 boundary ratio 약 63.97%로 valid performance fail, `/ ws`, `/ wss`, `/ ipc`, `ROUTER_ROUTER / tcp`는 일부 size의 안정성 blocker로 남았다. `/ ipc`의 256B 공식 ratio는 94.75%다. routed/request-reply transport에서 binding-only source optimization은 no-go다. |
