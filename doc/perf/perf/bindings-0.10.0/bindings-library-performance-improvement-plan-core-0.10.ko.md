...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `DEALER_DEALER / ipc` baseline throughput ratio는 43.892%/75.722%/82.661%/89.780%/91.173%/95.439%, 산술평균 79.778%이며 latency ratio 산술평균은 2.125x다. 자체 `SendSingleCore` AggressiveInlining after throughput ratio는 52.902%/78.561%/79.599%/87.427%/93.348%/93.863%, 산술평균 80.950%, latency 1.979x로 개선됐지만 throughput 목표 85%에 미달한다. Sol 검토 후 후보를 유지하고 보류하며 다음 대상은 아직 미측정인 `Single DEALER_ROUTER / ipc`다. |
| paired C | 완료 | C throughput은 2411528/1319972/902662/41271/26294/15545 Kops/s, 평균 latency는 0.107/0.204/0.234/4.919/7.809/13.180 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/single/report/perf_c_single_linux_20260810_180750_dotnet-dealer-dealer-ipc-paired-c1.txt`다. |
| binding paired 결과 | 완료·미달 | .NET baseline throughput은 1058458/999507/746153/37053/23973/14836 Kops/s, 평균 latency는 0.783/0.212/0.271/5.449/8.486/13.716 ms다. baseline throughput 산술평균은 79.778%, latency는 2.125x다. 자체 after throughput은 1275735/1036979/718509/36082/24545/14591 Kops/s, 평균 latency는 0.676/0.222/0.282/5.603/8.322/13.970 ms다. after throughput 산술평균은 80.950%, latency는 1.979x다. |
| 개선 결과 | 보류·Sol no-go | `SendSingleCore` inlining은 throughput·latency aggregate를 모두 개선해 최종 코드에 유지한다. Sol은 기존 Message pool·`Unsafe.SkipInit`·struct submitter가 이미 적용되어 추가 inlining·pool·builder reuse 후보를 no-go 판정했다. throughput aggregate 85%에는 미달하며 public contract·ownership·error semantics는 변경하지 않았다. build 성공, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 25 / 보류 15 | 미측정 | 다음 대상은 아직 미측정인 `Single DEALER_ROUTER / ipc`; C → .NET 순서로 한 대상씩 측정 |
