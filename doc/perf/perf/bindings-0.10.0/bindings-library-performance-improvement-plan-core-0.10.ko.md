...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `MULTI_DEALER_DEALER / wss` C와 .NET runner를 같은 one-way 의미로 paired 측정했다. C와 .NET은 clients 100, duration 1s, runs 1, 64/256/1024/4096/65536/131072B, connect concurrency 128, monitor HWM 4096000, auto-HWM balanced 조건이다. baseline throughput 산술평균 73.699%/latency 1.028x로 simple one-way 85%에 미달했다. 자체 `Message.Allocate` send-side pool 제거 후 83.876%/0.901x로 개선했고, Sol review에서 이 변경을 유지했지만 builder allocation 제거는 public builder lifetime·ownership 경계 때문에 no-go로 판정했다. 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_SENDSEND / wss`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 13 / 보류 6, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_SENDSEND / wss`; C → .NET 순서로 한 대상씩 측정 |
...
Multi의 throughput ratio 산술평균은 대부분 선택된 행에서 해당 pattern 목표를 충족한다.
`MULTI_PUBSUB / tcp`는 lifecycle parity와 PollOut 개선 후에도 throughput 평균이 82.930%로
.NET simple one-way 목표 85%에 미달해 `보류`한다. 또한 `MULTI_PUBSUB`의 평균 latency ratio는
`ws 6.576x`, `wss 2.076x`, `tls 2.017x`로 2.0x를 넘어 해당 세 행은 `보류`한다. 그 외
Multi 행은 throughput과 평균 latency aggregate를 통과한다. `MULTI_DEALER_ROUTER_REQREP / wss`의
throughput 평균은 85.02%로 중앙값 85% 목표를 충족하므로 이전의 중앙값 82.02% 기반 보류 표기를
`통과`로 정정한다.
