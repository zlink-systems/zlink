...
|------|------|------------------|
| 현재 언어 | C++ |  |
| 현재 pattern | 진행 중 | 정책 parity 후 `DEALER_ROUTER_REQREP / tcp`를 C → C++ 순서로 측정했다. full sweep ratio는 95.10%, 93.51%, 96.72%, 94.99%, 95.76%, 93.89%이고 boundary 재검증 반영 중앙값은 94.87%다. 목표 95%에 미달해 binding-only optimization은 no-go이며 다음 선택 대상은 `DEALER_ROUTER_REQREP / ws`다. |
