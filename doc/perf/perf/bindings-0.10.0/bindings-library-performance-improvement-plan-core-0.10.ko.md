...
- Multi 상태: `부분 완료` — `MULTI_DEALER_ROUTER_SENDSEND / ws`, `MULTI_ROUTER_ROUTER_SENDSEND / ws`, `MULTI_DEALER_ROUTER_REQREP / ws`, `MULTI_ROUTER_ROUTER_REQREP / ws`, `MULTI_PUBSUB / ws`, `MULTI_DEALER_DEALER / wss`, `MULTI_DEALER_ROUTER_SENDSEND / wss`, `MULTI_DEALER_ROUTER_REQREP / wss`의 complete report를 기록했다. `MULTI_ROUTER_ROUTER_REQREP / ws`는 pool bypass 후 모든 size의 throughput ratio가 97.49% 이상이고 평균 latency ratio도 2.0배 이내다. `MULTI_PUBSUB / ws`는 throughput 중앙값 103.79%지만 1024B 이상 평균 latency가 기준을 넘는다. `MULTI_DEALER_DEALER / wss`는 throughput 중앙값 90.04%와 65536B ratio 79.67%로 미달이다. `MULTI_DEALER_ROUTER_SENDSEND / wss`는 중앙값 86.70%로 통과했다. `MULTI_DEALER_ROUTER_REQREP / wss`는 ratio 94.06%/80.49%/80.49%/83.56%/78.79%/92.71%, 중앙값 82.02%, latency ratio 1.042x~1.270x로 중앙값 목표에 미달이다.
- 다음 작업: `MULTI_DEALER_ROUTER_REQREP / wss` 결과를 반영했으며, 다음 선택 대상
  `MULTI_ROUTER_ROUTER_SENDSEND / wss`를 C→C++ 순서로 짧게 측정한다.
...
| request/reply ws parity boundary | 측정값 기준 결과 | C++ measured-loop parity 수정 후 256B ratio 92.11%, 131072B ratio 98.97%, 262144B ratio 94.82%, 1024B ratio 86.03%를 모두 측정값으로 평가한다. 1024B도 throughput ratio와 평균 latency 기준으로 판정하며 반복값은 기록만 한다. |
...
| request/reply ipc 결과 | 측정값 기준 결과 | `ROUTER_ROUTER_REQREP / ipc` full sweep ratio는 87.77%, 89.40%, 81.98%, 90.53%, 90.03%, 99.70%, 중앙값은 89.72%다. 1024B boundary ratio 90.23%, latency ratio 1.017배와 64B boundary ratio 73.62%, latency ratio 약 1.270배를 모두 측정값으로 판정한다. six-size 중앙값과 transport pass/fail은 측정값으로 평가하며 binding-only source optimization은 중단한다. |
...
| Multi one-way 개선 결과 (DEALER_DEALER/wss) | no-go·측정값 기준 결과 | complete report의 throughput과 평균 latency로 판정했다. 65536B 개별 ratio와 six-size 중앙값이 목표에 미달하지만 latency는 기준 이내다. 현재 `dealer_socket_t`의 direct `socket_t::send(message, flags)`는 protected라서 public API를 확장하지 않고 기존 builder 경로를 유지한다. 다음 대상은 `MULTI_DEALER_ROUTER_SENDSEND / wss`다. |
...
| Multi routed echo binding 결과 (DEALER_ROUTER_SENDSEND/wss) | 완료·측정값 기준 통과 | C++ throughput은 136.922 / 122.341 / 103.849 / 77.203 / 10.498 / 5.770 Kops/s이고 평균 latency는 0.350 / 0.392 / 0.461 / 0.624 / 4.732 / 8.616 ms다. C++/C throughput ratio는 96.35% / 89.93% / 81.42% / 81.36% / 83.48% / 93.15%, 중앙값은 86.70%다. 평균 latency ratio는 1.039x / 1.114x / 1.229x / 1.226x / 1.197x / 1.073x다. 모든 개별 throughput 기준, 중앙값 목표와 latency 기준을 충족한다. |
...
| Multi request/reply binding 결과 (DEALER_ROUTER_REQREP/wss) | 완료·측정값 기준 미달 | C++ throughput은 84.340 / 68.629 / 64.040 / 54.221 / 9.282 / 5.305 Kops/s이고 평균 latency는 0.469 / 0.564 / 0.597 / 0.751 / 5.136 / 9.215 ms다. C++/C throughput ratio는 94.06% / 80.49% / 80.49% / 83.56% / 78.79% / 92.71%, 중앙값은 82.02%다. 평균 latency ratio는 1.042x / 1.205x / 1.211x / 1.166x / 1.270x / 1.078x다. 개별 throughput 최소 75%와 latency 기준은 충족하지만 중앙값 목표 85%는 미달이다. |
| Multi request/reply 개선 결과 (DEALER_ROUTER_REQREP/wss) | no-go·측정값 기준 결과 | Sol review에서 callback state 재사용은 completion thread와 callback lifetime 위험이 있고, request builder와 `received.reply()`는 이미 single-part native 경로를 사용하며 large-message pool도 bypass 상태임을 확인했다. public contract 변경 없이 채택할 binding-only 후보가 없어 source 변경 없이 다음 대상으로 진행한다. Sol review: `SOL-MULTI-DR-REQREP-WSS-20260810`. 다음 대상은 `MULTI_ROUTER_ROUTER_SENDSEND / wss`다. |

### 10.3 언어 진행 상태

| 순서 | 언어 | Single 상태 | Multi 상태 | 다음 작업 |
|------|------|-------------|------------|-----------|
| 현재 | C++ | 진행 중·측정값 기준 결과 | 부분 완료·측정값 기준 결과 | `MULTI_ROUTER_ROUTER_REQREP / ws`는 pool bypass 후 throughput·평균 latency 기준을 모두 통과했다. `MULTI_PUBSUB / ws`는 throughput은 통과했지만 1024B 이상 평균 latency가 미달이다. `MULTI_DEALER_DEALER / wss`는 throughput 중앙값 90.04%와 65536B ratio 79.67%로 미달이고 평균 latency는 기준 이내다. `MULTI_DEALER_ROUTER_SENDSEND / wss`는 중앙값 86.70%, 개별 최소 81.36%, 최대 latency ratio 1.229x로 통과했다. `MULTI_DEALER_ROUTER_REQREP / wss`는 중앙값 82.02%로 미달이며 callback state 재사용 등 구조 후보는 no-go다. 다음 대상은 `MULTI_ROUTER_ROUTER_SENDSEND / wss`다. |
...
| `MULTI_DEALER_ROUTER_REQREP / wss` | 94.06%, 80.49%, 80.49%, 83.56%, 78.79%, 92.71% | 82.02% | 미달·개별 최소 75%와 latency 기준은 충족하지만 중앙값 목표 85% 미달, public interface 변경 없이 no-go |
