...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, `tcp / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_DEALER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_PUBSUB` 완료·보류, `tcp / MULTI_STREAM` 완료·보류, `ws / MULTI_DEALER_DEALER` 완료·보류, `ws / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_DEALER_ROUTER_SENDSEND / ws`의 C → .NET paired 측정과 자체·Sol 개선 검토를 완료했으므로, 다음 미측정 대상 `MULTI_DEALER_ROUTER_REQREP / ws`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `MULTI_DEALER_ROUTER_SENDSEND / ws` C와 .NET runner `MULTI_DEALER_ROUTER`를 같은 routed echo 의미로 paired 측정했다. C와 .NET은 clients 100, duration 1s, runs 1, 64/256/1024/4096/65536/131072B, connect concurrency 128, monitor HWM 4096000, auto-HWM balanced 조건이다. parity baseline throughput 산술평균 89.960%/latency 1.104x에서 retry timestamp·sequence parity correction 후 91.125%/1.085x로 개선되어 통과했다. 자체·Sol 개선 검토를 완료했고 추가 binding hotpath 후보는 no-go다. 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_REQREP / ws`다. |
...
| Multi routed echo 개선 결과 (`DEALER_ROUTER_SENDSEND/ws`) | 통과·두 차례 개선 검토 완료 | 자체 Received send-operation inlining A/B는 89.995%/1.104x로 baseline 89.960%/1.104x와 차이가 없어 제거했다. Sol review는 binding hotpath 추가 후보를 no-go로 판정했고, C와 .NET의 blocked retry timestamp·sequence 처리 차이를 parity correction으로 수정한 뒤 91.125%/1.085x로 통과했다. public contract·ownership·error semantics는 변경하지 않았으며 build `0 warning / 0 error`, contract test `149 passed / 0 failed / 0 skipped`다. 다음 대상은 `MULTI_DEALER_ROUTER_REQREP / ws`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 7 / 보류 4, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_REQREP / ws`; C → .NET 순서로 한 대상씩 측정 |
...
| 2026-08-10 | .NET | Multi / MULTI_DEALER_ROUTER_SENDSEND / ws / 64·256·1024·4096·65536·131072B | `dotnet-multi-dealer-router-sendsend-ws-sol-parity-timestamp` | 84.557%, 83.867%, 80.775%, 86.224%, 106.329%, 104.998% | 91.125% (산술평균) | 통과·retry timestamp·sequence parity correction 적용·자체 inlining A/B 제거·Sol 추가 binding 후보 no-go·평균 latency ratio 1.085x | C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_204848_dotnet-multi-dealer-router-sendsend-ws-paired-c1.txt`; .NET baseline: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_204921_dotnet-multi-dealer-router-sendsend-ws-paired-baseline.txt`; own A/B: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_205216_dotnet-multi-dealer-router-sendsend-ws-own-received-send-inline.txt`; corrected: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_205820_dotnet-multi-dealer-router-sendsend-ws-sol-parity-timestamp.txt` |
