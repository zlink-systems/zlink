...
|------|------|-------------|------------|-----------|
| 1 | C++ | 진행 중·안정성 blocker | 미측정 | `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER`의 선택 transport 측정을 완료했다. `DEALER_ROUTER_REQREP / tcp`는 공식 중앙값 94.87%로 미달했고, `/ ws`와 `/ wss`는 각각 일부 size의 안정성 blocker로 남았다. request/reply transport에서 binding-only source optimization은 no-go다. |
