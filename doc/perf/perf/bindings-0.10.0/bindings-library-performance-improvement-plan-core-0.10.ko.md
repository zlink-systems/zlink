...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, `tcp / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_DEALER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_REQREP` 완료·통과, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_ROUTER_ROUTER_REQREP / tcp`를 통과로 확정했으므로 아직 미측정인 `MULTI_PUBSUB / tcp`를 C → .NET 순서로 한 대상씩 측정한다.
...
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 59.456% | 41.294% | 84.284% | 70.016% | 131.140% | 122.806% | 통과·C semantic `MULTI_ROUTER_ROUTER_SENDSEND`와 .NET runner `MULTI_ROUTER_ROUTER`를 같은 routed echo로 paired 측정했다. throughput 산술평균 84.832%, 평균 latency ratio 1.388x다. .NET client에 C와 같은 `CONNECT_ROUTING_ID=SERVER`와 `client_<index>` routing id를 설정하는 parity correction 후 complete가 됐으며 aggregate 목표 70%를 충족해 추가 hotpath pass는 수행하지 않았다. C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_193520_dotnet-multi-router-router-sendsend-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_193856_dotnet-multi-router-router-sendsend-tcp-paired-before-connect-rid.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 60.123% | 56.588% | 64.781% | 61.228% | 125.799% | 102.361% | 통과·C semantic과 .NET runner `MULTI_ROUTER_ROUTER_REQREP`를 같은 routed request/reply로 paired 측정했다. throughput 산술평균 78.480%, 평균 latency ratio 1.409x다. .NET Router client에 C와 같은 `client_<index>` routing id와 `ConnectRoutingId=SERVER`를 설정해 routed peer identity 의미를 정렬했고 complete가 됐다. aggregate 목표 70%를 충족해 추가 hotpath pass는 수행하지 않았다. C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_194652_dotnet-multi-router-router-reqrep-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_194708_dotnet-multi-router-router-reqrep-tcp-paired-before-connect-rid.txt` |
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `MULTI_ROUTER_ROUTER_REQREP / tcp` C semantic과 .NET runner `MULTI_ROUTER_ROUTER_REQREP`를 같은 routed request/reply로 paired 측정했다. throughput ratio는 60.123%/56.588%/64.781%/61.228%/125.799%/102.361%, 산술평균 78.480%이며 latency ratio 산술평균은 1.409x다. 다음 대상은 아직 미측정인 `MULTI_PUBSUB / tcp`다. |
...
| binding paired 결과 | 완료·통과 | .NET runner `MULTI_ROUTER_ROUTER_REQREP` throughput은 54380/52205/54758/49850/25790/15520 Kops/s, 평균 latency는 0.518/0.547/0.490/0.559/1.419/2.915 ms다. |
| 개선 결과 | 통과·C identity parity 유지 | C와 같은 `ConnectRoutingId=SERVER` 및 `client_<index>` routing id를 .NET Router client에 설정해 readiness와 routed peer identity 의미를 정렬했다. aggregate throughput 목표 70%와 latency 기준을 충족했으므로 추가 hotpath/POSDDD source pass는 수행하지 않았다. public contract·ownership·error semantics는 변경하지 않았으며 build `0 warning / 0 error`, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 4 / 보류 1, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_PUBSUB / tcp`; C → .NET 순서로 한 대상씩 측정 |
...
| 2026-08-10 | .NET | Multi / MULTI_ROUTER_ROUTER_REQREP / tcp / 64·256·1024·4096·65536·131072B | `dotnet-multi-router-router-reqrep-tcp-paired-before-connect-rid` | 60.123%, 56.588%, 64.781%, 61.228%, 125.799%, 102.361% | 78.480% (산술평균) | 통과·C/.NET Router identity parity correction 후 complete·평균 latency ratio 1.409x·추가 hotpath pass 없음 | C: `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_194652_dotnet-multi-router-router-reqrep-tcp-paired-c1.txt`; .NET: `/home/hep7hep7/project/zlink/bindings/dotnet/perf/results/multi/report/perf_dotnet_multi_linux_20260810_194708_dotnet-multi-router-router-reqrep-tcp-paired-before-connect-rid.txt` |
