...
|------|------|-------------|------------|-----------|
| 1 | C++ | 진행 중 | 미측정 | `PUBSUB / ipc`는 ratio 중앙값 95.93%로 통과했다. `DEALER_DEALER / tcp`, `/ ws`, `/ wss`, `/ tls`, `/ inproc`, `/ ipc`의 중앙값은 90.65%, 92.08%, 88.07%, 93.89%, 78.97%, 92.45%다. `DEALER_ROUTER / tcp`, `/ ws`, `/ tls`, `/ inproc`, `/ ipc`는 policy parity 후 ratio 중앙값 91.46%, 92.88%, 90.34%, 75.49%, 87.13%로 미달했고 `/ wss`는 64B boundary 재검증 후 ratio 중앙값 97.46%로 통과했다. binding-only source optimization은 no-go이며 다음 선택 대상은 `DEALER_ROUTER_REQREP / tcp`다. |
