...
- Multi 상태: `부분 완료` — `MULTI_DEALER_ROUTER_SENDSEND / ws`, `MULTI_ROUTER_ROUTER_SENDSEND / ws`, `MULTI_DEALER_ROUTER_REQREP / ws`, `MULTI_ROUTER_ROUTER_REQREP / ws`, `MULTI_PUBSUB / ws`, `MULTI_DEALER_DEALER / wss`의 complete report를 기록했다. `MULTI_ROUTER_ROUTER_REQREP / ws`는 pool bypass 후 모든 size의 throughput ratio가 97.49% 이상이고 평균 latency ratio도 2.0배 이내다. `MULTI_PUBSUB / ws`는 throughput 중앙값 103.79%지만 1024B 이상 평균 latency가 기준을 넘는다. `MULTI_DEALER_DEALER / wss`는 throughput ratio 79.67%~104.59%, 중앙값 90.04%, 평균 latency ratio 0.245x~1.164x로 측정값을 판정했다.
- 다음 작업: `MULTI_DEALER_DEALER / wss` 결과를 반영했으며, 다음 선택 대상
  `MULTI_DEALER_ROUTER_SENDSEND / wss`를 C→C++ 순서로 짧게 측정한다.
