...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, `tcp / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_DEALER_ROUTER_REQREP` 완료·통과, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_DEALER_ROUTER_REQREP / tcp`를 통과로 확정했으므로 아직 미측정인 `MULTI_ROUTER_ROUTER_SENDSEND / tcp`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `MULTI_DEALER_ROUTER_REQREP / tcp` own after throughput ratio는 62.026%/66.268%/61.640%/65.696%/103.571%/113.714%, 산술평균 78.819%이며 latency ratio 산술평균은 1.418x다. parity baseline은 76.314%/1.466x다. 다음 대상은 아직 미측정인 `MULTI_ROUTER_ROUTER_SENDSEND / tcp`다. |
| paired C | 완료 | C throughput은 99335/100320/101207/92840/24335/16056 Kops/s, 평균 latency는 0.347/0.350/0.343/0.372/1.015/1.553 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_191447_dotnet-multi-dealer-router-reqrep-tcp-paired-c1.txt`다. |
| binding paired 결과 | 완료·통과 | .NET parity baseline throughput은 61775/64372/54351/61490/24763/17635 Kops/s, 평균 latency는 0.482/0.457/0.540/0.492/1.570/2.571 ms다. own after throughput은 61614/66480/62384/60992/25204/18258 Kops/s, 평균 latency는 0.470/0.448/0.477/0.483/1.613/2.481 ms다. |
| 개선 결과 | 통과·parity 및 binding 개선 유지 | C의 activity-driven lifecycle, HWM-before-READY와 explicit EAGAIN/POLLOUT retry를 .NET harness에 맞췄다. retry template과 server 수명 polling 목록으로 ownership·retry 책임을 명확히 했고, request/reply builder inlining은 throughput 76.314%→78.819%, latency 1.466x→1.418x로 개선되어 채택했다. Sol 2차 review는 추가 binding/harness 구조 변경을 no-go 판정했다. public contract·ownership·error semantics는 변경하지 않았으며 build `0 warning / 0 error`, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 2 / 보류 1, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_ROUTER_ROUTER_SENDSEND / tcp`; C → .NET 순서로 한 대상씩 측정 |
