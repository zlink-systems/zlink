...
|------|------|-------------|------------|-----------|
| 1 | C++ | 진행 중·안정성 blocker | 미측정 | `PUBSUB`, `DEALER_DEALER`, `DEALER_ROUTER`의 선택 transport 측정을 완료했고 `ROUTER_ROUTER / tcp`는 timeout parity 후 안정성 blocker로 분류했다. `DEALER_ROUTER_REQREP / tcp`는 공식 중앙값 94.87%로 미달했고, `ROUTER_ROUTER / tls`는 262144B 안정 boundary ratio 약 83.92%, `/ inproc`은 262144B 안정 boundary ratio 약 63.97%로 valid performance fail, `/ ws`, `/ wss`, `ROUTER_ROUTER / tcp`는 일부 size의 안정성 blocker로 남았다. routed/request-reply transport에서 binding-only source optimization은 no-go다. |
