...
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  ...
  측정 완료로 간주하지 않는다.
- 다음 작업: `Single ROUTER_ROUTER_REQREP / ipc`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `ROUTER_ROUTER_REQREP / inproc` before throughput ratio는 48.346%/45.404%/47.076%/59.498%/102.851%/100.205%, 산술평균 67.230%이며 before 평균 latency ratio는 2.036x/2.112x/2.067x/1.615x/0.889x/1.022x, 산술평균 1.624x다. 자체 single-part specialization after throughput ratio는 50.408%/51.727%/47.226%/58.765%/98.026%/97.485%, 산술평균 67.273%, 평균 latency ratio 1.559x다. throughput aggregate 70%에 미달해 보류하고 다음 대상은 `Single ROUTER_ROUTER_REQREP / ipc`다. |
| paired C | 완료 | C throughput은 295094/306826/308693/128330/81310/46879 Kops/s, 평균 latency는 0.112/0.107/0.105/0.052/0.045/0.045 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/single/report/perf_c_single_linux_20260810_171326_dotnet-router-router-reqrep-inproc-paired-c1.txt`다. |
| binding paired 결과 | 완료·미달 | .NET before throughput은 142666/139310/145320/76354/83628/46975 Kops/s, 평균 latency는 0.228/0.226/0.217/0.084/0.040/0.046 ms다. 자체 single-part after throughput은 148750/158713/145784/75413/79705/45700 Kops/s, 평균 latency는 0.211/0.198/0.215/0.084/0.042/0.046 ms다. before 대비 after throughput 산술평균은 67.230%→67.273%, latency 산술평균은 1.624x→1.559x다. Sol submitter after throughput은 66.994%, latency 1.632x로 악화되어 제거했다. |
| 개선 결과 | 보류·자체 single-part specialization 유지 | `RouterRequestOperation`의 single-part 경로가 임시 `IReadOnlyList`를 만들지 않고 기존 ownership·timeout·callback·error 처리를 private 공통 경로에서 유지하도록 변경했다. throughput과 latency가 자체 A/B에서 개선됐으므로 POSDDD 기준의 책임 경계 개선과 함께 최종 코드에 유지한다. Sol이 제안한 private readonly submitter로 lambda/closure allocation을 제거하는 후보는 throughput 66.994%, latency 1.632x로 악화되어 제거했다. aggregate throughput 70%에 미달하고 추가 contract-safe 후보가 없어 보류한다. |
...
| 2 | .NET | 완료·통과 22 / 보류 12 | 미측정 | 다음 대상은 `Single ROUTER_ROUTER_REQREP / ipc`; C → .NET 순서로 한 대상씩 측정 |
