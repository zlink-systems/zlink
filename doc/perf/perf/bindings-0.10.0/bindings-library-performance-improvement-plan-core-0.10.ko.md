...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, `tcp / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_DEALER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_SENDSEND` 완료·통과, `tcp / MULTI_ROUTER_ROUTER_REQREP` 완료·통과, `tcp / MULTI_PUBSUB` 완료·보류, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_ROUTER_ROUTER_SENDSEND / tcp`와 `MULTI_ROUTER_ROUTER_REQREP / tcp`의 재검토를 완료했으므로, 아직 미측정인 `MULTI_STREAM / tcp`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `MULTI_ROUTER_ROUTER_REQREP / tcp` C와 .NET runner를 같은 routed request/reply 의미로 paired 측정했다. 재검토 after throughput ratio는 67.756%/62.832%/67.995%/67.436%/122.877%/112.419%, 산술평균 83.553%이며 평균 latency ratio 산술평균은 1.323x다. 다음 대상은 아직 미측정인 `MULTI_STREAM / tcp`다. |
| paired C | 완료 | C throughput은 90448/92255/84528/81417/20501/15162 Kops/s, 평균 latency는 0.365/0.371/0.386/0.408/1.255/1.631 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_194652_dotnet-multi-router-router-reqrep-tcp-paired-c1.txt`다. |
| binding paired 결과 | 완료·통과 | .NET re-review after throughput은 61284/57966/57475/54904/25191/17045 Kops/s, 평균 latency는 0.458/0.499/0.486/0.529/1.502/2.587 ms다. 기존 baseline은 throughput 78.480%/latency 1.409x였고 after는 83.553%/1.323x다. |
| 개선 결과 | 통과·request override inlining 채택 | sealed `RouterPeerRequestOperation`의 내부 override에 AggressiveInlining을 적용해 throughput·latency aggregate가 함께 개선됐다. 기존 C/.NET identity parity와 POSDDD 책임 경계를 유지하며 public contract·ownership·error semantics를 변경하지 않았다. build `0 warning / 0 error`, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 6 / 보류 2, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_STREAM / tcp`; C → .NET 순서로 한 대상씩 측정 |
