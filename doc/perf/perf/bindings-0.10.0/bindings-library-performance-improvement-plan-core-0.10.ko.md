...
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  ...
  측정 완료로 간주하지 않는다.
- 다음 작업: Single 전체가 완료되었으므로 .NET Multi inventory gate를 확인하고 아직 미측정인 `MULTI_DEALER_DEALER / tcp`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `DEALER_ROUTER_REQREP / ipc` throughput ratio는 52.670%/63.262%/59.000%/97.351%/98.193%/100.336%, 산술평균 78.469%이며 latency ratio 산술평균은 1.239x다. .NET request/reply aggregate 기준을 충족해 추가 hotpath·POSDDD 구조 변경 없이 통과한다. 다음은 .NET Multi inventory gate 이후 `MULTI_DEALER_DEALER / tcp`다. |
| paired C | 완료 | C throughput은 257368/220382/231910/21106/14277/8640 Kops/s, 평균 latency는 0.149/0.182/0.194/0.563/0.416/0.343 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/single/report/perf_c_single_linux_20260810_182021_dotnet-dealer-router-reqrep-ipc-paired-c1.txt`다. |
| binding paired 결과 | 완료·통과 | .NET throughput은 135557/139417/136828/20547/14019/8669 Kops/s, 평균 latency는 0.273/0.249/0.239/0.571/0.418/0.337 ms다. throughput 산술평균은 78.469%, latency ratio 산술평균은 1.239x다. |
| 개선 결과 | 통과·변경 없음 | request/reply aggregate 기준을 충족해 hotpath 개선과 POSDDD 구조 변경을 추가하지 않는다. public contract·ownership·error semantics는 변경하지 않았다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 미측정 | Single 전체 완료. .NET Multi inventory gate 후 `MULTI_DEALER_DEALER / tcp`를 C → .NET 순서로 한 대상씩 측정 |
