...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, `tcp / MULTI_DEALER_ROUTER_SENDSEND` 완료·통과, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_DEALER_ROUTER_SENDSEND / tcp`를 통과로 확정했으므로 아직 미측정인 `MULTI_DEALER_ROUTER_REQREP / tcp`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·통과 | `MULTI_DEALER_ROUTER_SENDSEND / tcp` baseline throughput ratio는 81.625%/83.768%/84.022%/85.710%/128.561%/129.268%, 산술평균 98.826%이며 latency ratio 산술평균은 1.105x다. 자체 `Received.Send` 단일-part 경계 inlining A/B는 throughput 96.478%·latency 1.126x로 악화되어 제거했다. Sol 2차 review는 추가 hotpath 후보를 no-go 판정했다. 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_REQREP / tcp`다. |
| paired C | 완료 | C throughput은 182915/183683/171372/165034/37001/22304 Kops/s, 평균 latency는 0.251/0.251/0.267/0.279/0.979/1.600 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_185640_dotnet-multi-dealer-router-sendsend-tcp-paired-c1.txt`다. |
| binding paired 결과 | 완료·통과 | .NET baseline throughput은 149304/153868/143990/141450/47569/28832 Kops/s, 평균 latency는 0.294/0.286/0.302/0.313/0.990/1.685 ms다. 자체 A/B throughput은 150215/152566/142212/139483/42520/29278 Kops/s, 평균 latency는 0.293/0.287/0.307/0.316/1.101/1.661 ms다. |
| 개선 결과 | 통과·parity correction 유지 | baseline aggregate가 .NET simple one-way throughput 기준 85%를 충족한다. 자체 A/B는 두 aggregate를 악화시켜 제거했고 Sol 2차 no-go 후 추가 hotpath 변경 없이 통과한다. C relay의 connection-ready count gate 제거와 HWM-before-READY 정렬은 C와 동일한 lifecycle 책임을 회복하는 POSDDD 개선으로 유지한다. public contract·ownership·error semantics는 변경하지 않았으며 build `0 warning / 0 error`, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 1 / 보류 1, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_REQREP / tcp`; C → .NET 순서로 한 대상씩 측정 |
