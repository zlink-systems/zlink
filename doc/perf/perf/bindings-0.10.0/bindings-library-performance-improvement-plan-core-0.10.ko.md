...
- Multi 상태: `tcp / MULTI_DEALER_DEALER` 완료·보류, 나머지 `미측정`
- `미측정` 행은 완료나 다음 언어 전환을 의미하지 않는다. 특히 9.2.2의 `tls` `MULTI_*`
  행은 .NET에서 아직 측정하지 않은 대상이며, C++ 9.1의 같은 이름 행이 완료되어도 .NET
  측정 완료로 간주하지 않는다.
- 다음 작업: `MULTI_DEALER_DEALER / tcp`를 보류로 확정했으므로 아직 미측정인 `MULTI_DEALER_ROUTER_SENDSEND / tcp`를 C → .NET 순서로 한 대상씩 측정한다.
...
|------|------|------------------|
| 현재 언어 | .NET |  |
| 현재 pattern | 완료·보류 | `MULTI_DEALER_DEALER / tcp` baseline throughput ratio는 41.415%/67.082%/123.920%/76.729%/82.112%/78.791%, 산술평균 78.342%이며 latency ratio 산술평균은 4.268x다. 자체 inlining after throughput ratio는 41.457%/70.323%/119.959%/75.815%/92.068%/86.133%, 산술평균 80.959%, latency 2.222x로 개선됐다. Sol guard inlining after는 81.033%·latency 2.460x로 throughput +0.074%p에 그치고 latency가 악화되어 제거했다. 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_SENDSEND / tcp`다. |
| paired C | 완료 | C throughput은 2759596/1510617/726799/371826/95081/52851 Kmsg/s, 평균 latency는 0.181/0.531/404.765/297.811/204.628/290.186 ms다. report는 `/home/hep7hep7/project/zlink/bindings/c/perf/results/multi/report/perf_c_multi_linux_20260810_182810_dotnet-multi-dealer-dealer-tcp-paired-c1.txt`다. |
| binding paired 결과 | 완료·미달 | .NET baseline throughput은 1142885/1013355/900652/285299/78073/41642 Kmsg/s, 평균 latency는 3.919/0.337/3.524/378.780/190.741/321.269 ms다. 자체 after throughput은 1144046/1062310/871859/281899/87539/45522 Kmsg/s, 평균 latency는 1.590/0.714/3.893/369.523/195.066/289.593 ms다. Sol after throughput은 1163151/1081574/876890/271899/89598/44627 Kmsg/s, 평균 latency는 1.944/0.386/3.680/391.555/182.160/312.482 ms다. |
| 개선 결과 | 보류·자체 후보 유지 | 자체 `Message` allocate/dispose와 단일 part buffer 경계 inlining은 throughput·latency aggregate를 개선해 유지한다. Sol guard inlining은 추가 개선으로 채택하지 않는다. throughput aggregate 85%에 미달하고 추가 contract-safe 후보가 없어 보류한다. C/.NET cleanup parity 수정은 Sol review 후 적용했으며 public contract·ownership·error semantics는 변경하지 않았다. build 성공, contract test `149 passed / 0 failed / 0 skipped`다. |
...
| 2 | .NET | 완료·통과 26 / 보류 16 | 완료·통과 0 / 보류 1, 나머지 미측정 | 다음 대상은 아직 미측정인 `MULTI_DEALER_ROUTER_SENDSEND / tcp`; C → .NET 순서로 한 대상씩 측정 |
