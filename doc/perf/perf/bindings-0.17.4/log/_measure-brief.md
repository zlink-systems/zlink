# bindings 0.17.4 성능 측정 job (codex 서브에이전트 브리프)
...
2. **Core source를 다시 build하지 않는다.** 측정 runtime은 GitHub release `core/v0.17.4`이며
   러너에 `--core-version 0.17.4`를 전달해 선택한다. 러너가 report META의 `core_version`을
   `0.17.4`로, `core_source`를 release로 보고하는지 확인한다. 0.17.4가 아니면 즉시 중단하고
   실제 값과 함께 보고한다.
3. **문서(계획서·스펙·정책)를 수정하지 않는다.** 너의 출력은 `log/` 아래의 결과 TSV와
   최종 보고뿐이다. `doc/perf/perf/bindings-0.17.4/*.md` 계획서를 편집하지 않는다.
...
- `doc/perf/perf/bindings-0.17.4/bindings-library-performance-improvement-plan-core-0.17.4.ko.md`
...
3. **본 측정**: 이번 transport에 대해, 해당 suite의 각 pattern을 (또는 러너가 허용하면
   ...
   - `--duration 5 --runs 1`
   - `--results-tag c0174-<lang>-<suite>-<transport>` (C·binding 동일 tag = pair tag)
...
`doc/perf/perf/bindings-0.17.4/log/results-<lang>-<suite>.tsv`에 **append**한다(없으면 헤더와
