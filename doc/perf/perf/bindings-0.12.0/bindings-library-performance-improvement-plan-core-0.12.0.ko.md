<!--
이 계획서는 Core `0.12.0`을 기준으로 작성했다. 측정값과 실행 과정은 이 폴더의 표와 `log/`에 기록한다.
-->
# core 0.12.0 bindings 라이브러리 성능 개선 계획

> 시작일: 2026-08-19
>
> 작업 브랜치: `codex/bindings-0.12.0-performance`
>
> 성능 측정과 구현은 `codex/bindings-0.12.0-performance`에서 진행하며, 측정과 문서 변경은 고정한 WSL/Linux 작업영역에서 수행한다.
>
> 이 문서는 core 0.12.0을 기준으로 bindings 라이브러리 성능 개선을 처음부터
...
이번 작업의 core 기준 버전은 0.12.0이다. 측정 전에 다음 세 파일의 버전이 모두 같은지
확인한다.

- `VERSION`: `LIBZLINK_VERSION=0.12.0`
- `core/CMakeLists.txt`: `project(zlink VERSION 0.12.0 ...)`
- `core/include/zlink.h`: major, minor, patch values matching 0.12.0

`bindings/tools/local_core_runtime.sh`는 `VERSION`의 값을 이용해 GitHub의
`core/v0.12.0` release asset을 기존 release 절차로 가져오고 versioned
...
core 0.12.0의 통과 비율이나 완료 근거로 사용하지 않는다.
...
비교 기준은 같은 core 0.12.0 runtime으로 실행한 `bindings/c/perf` 결과다. 같은 suite,
...
96.3%였고 routed one-way와 multi routed echo도 비슷했다. 다만 현재 core 0.12.0의
...
근거로 쓰지 않으며 Node와 같은 60% 중앙값 목표에서 시작한다. 이후 현재 core 0.12.0의
...
core 0.12.0의 현재 multi runner 기본값을 따른다. 이전 표의 256 KiB는 제거하고
4 KiB를 추가한다.
...
2. GitHub `core/v0.12.0` release asset과 package provenance를 준비하고 재현
...
- `미측정`: 같은 조건의 core 0.12.0 C 결과와 binding 결과를 아직 비교하지 않았다.
...
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `PAIR` | 보류(80.93%) | 보류(93.52%) | 보류(92.29%) | 보류(70.90%) | 보류(83.22%) | 보류(88.64%) | aggregate throughput mean 84.92%(목표 95%/완화 90% 미달), aggregate latency mean 1.201배(상한 2.0배 이내). 자체 pass(C1/C3/C4/C5)와 Sol 리뷰 pass 완료, 잔여 contract-보존 후보 없음 → 보류 확정. 65536B 셀은 glibc heap trim page fault가 지배하는 환경 요인으로 판명(binding 결함 아님, C6-후속 재확인 88.22%). `log/2026-08-23-cpp-pair-tcp-official.md`, `log/2026-08-23-sol-review-pair-tcp.md`, `log/2026-08-23-c6-fix-and-64k-diagnosis.md` |
| `tcp` | `PUBSUB` | 보류(91.37%) | 보류(94.00%) | 보류(92.25%) | 보류(76.21%; 환경 지배) | 보류(84.40%) | 보류(88.34%) | §7.2 `최종·경계 판정` 조건의 5-run paired 측정값(session tag `bindings-0.12.0-official-pubsub-final5-20260823`, default duration 5s, runs 5, CPU pin 없음, C→C++ 같은 session). aggregate throughput mean **87.76%**(목표 95%/완화 90% 미달), aggregate latency mean **1.095배**(상한 2.0배 이내, 통과). 자체 개선 pass(P1 측정 의미 정렬, P2 중복 제거 + Sol 지적 contract-order 복원 채택 / P3~P7 no-go)와 Sol 리뷰 pass(LTO/IPO A/B 유일 후보 → **no-go**: LTO-ON 89.16% vs LTO-OFF 89.50%, latency 1.079배 vs 1.060배로 개선 없음)가 모두 완료돼 잔여 contract-보존 후보가 없으므로 §8에 따라 `보류` 확정. 65536B(76.21%)는 `PAIR`과 같은 glibc heap-trim page fault **환경 지배** 셀(C++ minor fault가 C의 약 540배, trim만 끄면 94.31%, 같은 조건을 C에 강제하면 C가 C++보다 낮아짐)이지만 aggregate에서 제외하지 않는다 — 65536B 제외 5셀 평균 90.07%와 `MALLOC_*` 조건의 94.31%는 진단용일 뿐 판정 입력이 아니다. 이력: 첫 공식 측정 89.40%(87.94/93.75/93.19/78.29/90.04/93.16), pass2 86.15%(91.72/92.17/89.86/70.18/84.37/88.60). `log/2026-08-23-cpp-pubsub-final-judgment.md`, `log/2026-08-23-sol-review-pubsub-tcp.md`, `log/2026-08-23-cpp-pubsub-improvement-pass.md`, `log/2026-08-23-cpp-pubsub-tcp-official.md` |
| `tcp` | `DEALER_DEALER` | 미달(31.79%) | 미달(38.23%) | 미달(66.66%) | 미달(75.98%) | 미달(83.46%) | 미달(84.05%) | **자체 개선 pass 후 공식 재측정(session tag `bindings-0.12.0-official-dd3-20260823`)**. aggregate throughput mean **63.36%**(직전 dd2 53.59% 대비 +9.77%p, 목표 95%/완화 90%/개별 최소 85% 모두 미달 — 최댓값 262144B 84.05%), aggregate latency mean **1.210배**(상한 2.0배 이내, 통과). 자체 pass 내용: D2 호출 스레드 물리 admission fast path(수락되면 pending record·큐·deadline·ticket을 만들지 않음), D3 target별 레코드 통합(map/deque/set churn 제거), D4 콜백 3종 `std::function` → `routed_admission_request_t` 인터페이스, D5 대기할 때만 `sndtimeo` 조회 + terminal이면 ticket 생략. 64B `async()` 메시지당 1,882 ns → 1,079 ns(그 중 Core submit 452 ns). no-go: 단일 파트 직접 submit(+3.5%였으나 `test_cpp_contract_exact_target_retry` 위반 → 되돌림). byte-HWM 백프레셔 가설은 **기각**: HWM 64배 완화 A/B에서 C는 64B +29.4%인데 C++는 개선 전·후 모두 ±2% 무반응이고, park 계수기에서 400만 건 중 park 0~19건·deadline 만료 0건 → **bookkeeping-dominated, lost wake 없음**. 비율이 크기에 따라 단조 증가해 65536B 국소 dip 없음. Sol 리뷰 pass 미수행이므로 `보류`가 아닌 `미달`로 기록. `log/2026-08-23-cpp-routed-send-improvement.md`, `log/2026-08-23-cpp-dealer-dealer-tcp-official.md`, `log/2026-08-23-cpp-dealer-dealer-ceiling.md`|
...
|------|------|---------------|----------|----------------|------|---------------|
| 2026-08-19 | 전체 | 계획 초기화 | - | Core 0.12.0 release, C 기준과 binding paired 비교, 단일 perf process 조건을 사용한다. | 계획 작성 | 이 문서 |
...
- 각 pattern의 최종 판정에 사용한 core 0.12.0 C와 binding paired report가 모두
  `status: complete`다.
