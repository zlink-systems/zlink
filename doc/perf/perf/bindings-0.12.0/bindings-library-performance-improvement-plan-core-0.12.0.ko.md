...
| `tcp` | `DEALER_DEALER` | 미달(0.37%) | 미달(0.49%) | 미달(0.85%) | 미달(21.61%) | 미달(35.05%) | 미달(55.77%) | aggregate throughput mean **19.02%**(목표 95%/완화 90%/개별 최소 85% 모두 크게 미달), aggregate latency mean **0.031배**(상한 2.0배 이내, C보다 낮음 — 처리량이 초당 약 1만 건 상한에 걸려 in-flight가 거의 없는 부작용으로 추정). throughput이 크기와 무관하게 약 9,000~10,000 msg/s로 평탄 — 국소적 65536B dip(환경 지배)이 아니라 pattern 전역 상한으로 판단, `/usr/bin/time -v` fault 재확인 절차는 적용하지 않음. `DEALER_DEALER` 전용 자체 pass·Sol 리뷰 pass 미수행이므로 `보류`가 아닌 `미달`로 기록. `log/2026-08-23-cpp-dealer-dealer-tcp-official.md` |
