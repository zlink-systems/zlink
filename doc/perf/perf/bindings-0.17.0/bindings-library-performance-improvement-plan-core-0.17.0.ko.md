...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | **측정 차단 — Go client가 duration 5에서 결정적으로 `exit_nonzero`**(0.17.2, `r5go` 전 크기 실패, `goddA` 64 B·runs 1·duration 5 재현 13초 실패, 결과 0줄). duration 1 smoke(`go172smoke`, Go REQREP job의 DD 회귀 smoke)는 통과했으므로 **duration 의존 결함**이며 이번 캠페인에서 처음 드러났다 — Go DD는 지금까지 smoke만 돌았다. REQREP 러너 변경(`add837942b`)은 DD 파일을 건드리지 않았다. **원인 문자열 확보**(`goddCAP2`, client_err): `multi dealer/dealer send drain timed out` — client가 active 구간 뒤 미완료 send를 drain하다 deadline을 넘긴다. duration 1이면 backlog가 작아 들고 5면 못 든다. **PUBSUB·REQREP은 완주하고 DD·DR SENDSEND(단방향/echo client)만 실패**하므로 Go sender 경로 고유다. §5상 timeout을 늘려 넘기지 않는다 — 질문은 왜 backlog가 그렇게 깊은가이며, dealer goroutine이 admission을 기다리지 않고 제출해 누적하는 D-BP24 계열이 유력 후보다. 수정 job 대기(codex 상한). |
