...
- Multi 상태: **0.17.2 기준 24 cell 기록 완료** — `통과` 10(tcp DD 97.56·PUBSUB 95.32·DR SS 91.86·RR SS 91.67; tls DD 95.69·PUBSUB 94.61·DR SS 94.95; ws DD 95.24·PUBSUB 95.44; wss PUBSUB 95.15), `보류` 14(REQREP 8 = tcp·tls 4는 D-BP26 구조적 근거, ws·wss 4는 `C 기준 이상`; SENDSEND latency 간헐 이상 5 = tls RR·ws DR/RR·wss DR/RR; STREAM 4 = Core 결함), `차단` 1(wss DD — C 러너 간헐 실패 조사 중). Core 보고 2건: STREAM 수신 정체(D-BP23, 0.17.2에서도 재현), ws/wss 왕복 크기 비례 붕괴(D-BP28). 진행 중: C++ SENDSEND latency 조사(4 cell, C는 정상이라 C++ 경로), wss DD C 실패 재현.
