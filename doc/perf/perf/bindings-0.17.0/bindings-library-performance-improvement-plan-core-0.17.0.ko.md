...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미달(7.7%) | 미달(9.7%) | 미달(11.1%) | 미달(21.7%) | 미달(29.1%) | 미측정 | **`미달(15.89%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r8py`, C `complete`·python `complete`); latency 0.15x. relay 정합(fb3f37191d) 전 3-run 1.6~22.6% → 7.7~29.1%로 올랐으나 목표 60%에 멀다. latency 비율 0.15x는 단위 문제가 아니라(양쪽 ms) C sender가 100 client 큐를 깊게 채워 C one-way latency가 770 ms까지 커지는 반면 Python sender는 느려 큐가 얕은 결과 — one-way latency 비율은 처리량이 낮을수록 작아지므로 판정 근거로 쓰지 않는다. 처리량 python·C 133.9/1,736.5, 132.8/1,367.0, 130.6/1,172.0, 129.1/593.8, 42.4/145.5 Kops/s; C `perf_c_multi_linux_20260908_105754_r8py.txt`, python `perf_python_multi_linux_20260908_105852_r8py.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 미달(16.1%) | 미달(17.5%) | 미달(18.1%) | 미달(17.2%) | 통과(52.0%) | 미측정 | **`미달(24.19%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r8py`, C `complete`·python `complete`); latency 1.43x. relay 정합(fb3f37191d) 뒤 첫 유효 측정. 처리량 python·C 77.4/482.3, 76.5/436.4, 76.1/420.3, 67.4/391.0, 41.6/80.1 Kops/s; C `perf_c_multi_linux_20260908_111540_r8py.txt`, python `perf_python_multi_linux_20260908_111742_r8py.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(9.2%) | 미달(9.7%) | 미달(8.8%) | 미달(9.8%) | 통과(33.4%) | 미측정 | **`미달(14.20%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r8py`, C `complete`·python `complete`); latency 9.15x **latency 9.15x > 5.0x**. relay 정합(fb3f37191d) 뒤 첫 유효 측정(이전 3-run 7.6~37.9%). 처리량 python·C 35.5/385.2, 34.5/354.1, 31.8/360.0, 31.9/325.9, 21.3/63.8 Kops/s; C `perf_c_multi_linux_20260908_113023_r8py.txt`, python `perf_python_multi_linux_20260908_113220_r8py.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미달(15.5%) | 미달(16.8%) | 미달(16.1%) | 미달(17.9%) | 통과(50.3%) | 미측정 | **`미달(23.33%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r8py`, C `complete`·python `complete`); latency 1.18x. relay 정합(fb3f37191d) 뒤 첫 유효 측정. 처리량 python·C 73.9/475.5, 73.7/437.7, 71.2/442.1, 69.9/391.4, 36.9/73.3 Kops/s; C `perf_c_multi_linux_20260908_112652_r8py.txt`, python `perf_python_multi_linux_20260908_112947_r8py.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(9.9%) | 미달(10.6%) | 미달(10.9%) | 미달(11.3%) | 통과(35.6%) | 미측정 | **`미달(15.66%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r8py`, C `complete`·python `complete`); latency 9.23x **latency 9.23x > 5.0x**. relay 정합 뒤 첫 유효 측정(이전 3-run 8.7~40.7%). 처리량 python·C 35.6/360.6, 34.0/319.2, 33.4/308.0, 32.0/282.7, 20.6/57.9 Kops/s; C `perf_c_multi_linux_20260908_113314_r8py.txt`, python `perf_python_multi_linux_20260908_113501_r8py.txt` |
| `tcp` | `MULTI_PUBSUB` | 미달(18.0%) | 미달(18.7%) | 미달(16.5%) | 미달(24.6%) | 통과(92.4%) | 미측정 | **`미달(34.02%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r8py`, C `complete`·python `complete`); latency 0.80x. relay 정합 뒤 첫 유효 측정(이전 3-run 13.7~60.5%). 처리량 python·C 413.8/2,305.1, 437.5/2,343.3, 414.7/2,507.0, 390.7/1,588.9, 103.4/112.0 Kops/s; C `perf_c_multi_linux_20260908_113622_r8py.txt`, python `perf_python_multi_linux_20260908_111241_r8py.txt` |
