...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | **측정 차단 — Go client가 duration 5에서 결정적으로 `exit_nonzero`**(0.17.2, `r5go` 전 크기 실패, `goddA` 64 B·runs 1·duration 5 재현 13초 실패, 결과 0줄). duration 1 smoke(`go172smoke`, Go REQREP job의 DD 회귀 smoke)는 통과했으므로 **duration 의존 결함**이며 이번 캠페인에서 처음 드러났다 — Go DD는 지금까지 smoke만 돌았다. REQREP 러너 변경(`add837942b`)은 DD 파일을 건드리지 않았다. 원문 stderr 재현(`goddRAW`) 진행 중. Go PUBSUB·SENDSEND도 같은 조건이면 같이 실패할 가능성이 크다. |
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
