...
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 통과(81.3%) | 통과(87.5%) | 통과(100.8%) | 통과(98.8%) | 통과(108.6%) | 미측정 | `p5cmodel` aggregate throughput **95.42%**(기본 목표 95%), latency 0.83x `통과`; 처리량 C++·C 1,410.6/1,690.6 등 — pass 1(`p3pin`, 83.4/85.1/99.8/100.6/109.1% → 95.59%) 뒤 러너 C 모델 정합 후 재측정값이다. **§7.2 경계 판정 미충족 — 5-run 재측정 필요.** `p5cmodel`은 `runs 1`(§7.2 '탐색' 조건)인데 이 값은 목표 95% 바로 위다. pass 2가 library source diff 0줄로 같은 셀을 다시 재 DD 1024 B 처리량 −5.52%·latency +25.00%가 나왔으므로 1-run 변동폭이 목표와의 여유보다 크다. §7.2의 '최종·경계 판정 5회'로 다시 재기 전까지 이 `통과`는 확정이 아니다. C·C++ `*_p5cmodel.txt`; [pass 1 log](log/2026-09-07-cpp-multi-tcp-dd-pass1.ko.md) |
...
| `tcp` | `MULTI_PUBSUB` | 통과(89.8%) | 통과(89.6%) | 통과(94.9%) | 통과(97.9%) | 통과(105.4%) | 미측정 | `p5cmodel` aggregate throughput **95.52%**(기본 목표 95%), latency 1.01x `통과`; 2026-09-05의 `보류(93.2%)`는 러너 정합 이전 값이라 참고값이다(D-BP19). **§7.2 경계 판정 미충족 — 5-run 재측정 필요.** `p5cmodel`은 `runs 1`(§7.2 '탐색' 조건)인데 이 값은 목표 95% 바로 위다. pass 2가 library source diff 0줄로 같은 셀을 다시 재 DD 1024 B 처리량 −5.52%·latency +25.00%가 나왔으므로 1-run 변동폭이 목표와의 여유보다 크다. §7.2의 '최종·경계 판정 5회'로 다시 재기 전까지 이 `통과`는 확정이 아니다. C·C++ `*_p5cmodel.txt` |
