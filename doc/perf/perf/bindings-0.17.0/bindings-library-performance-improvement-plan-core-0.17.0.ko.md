...
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 보류(75.1%) | 보류(70.2%) | 보류(72.3%) | 보류(73.9%) | 보류(73.0%) | 미측정 | before 재측정 aggregate throughput **72.87%**(목표 85%), latency **2.69x**(목표 2.0x); 64KiB latency 9.62x가 aggregate를 끌어올린다; 처리량 C++·C 297.8/396.8, 271.6/387.1, 273.0/377.7, 254.8/345.0, 45.6/62.4 Kops/s; C `perf_c_multi_linux_20260907_220942_p5cmodel.txt`, C++ before `perf_cpp_multi_linux_20260907_221009_p5cmodel.txt`; 이번 캠페인 재측정(`p5cmodel`, C·C++ 같은 조건, 5-run, clients 100, 2-part, 전부 complete)으로 before 값을 갱신했다(2026-09-05 판정 57.4/68.4%는 러너 C 모델 복원 D-BP15 이전 값이라 비교 대상이 아니다). **격차는 전부 C++ client다** — 교차 pairing에서 `C++ server + C client`는 64B에서 C 기준 DR 100.8%/RR 97.6%, 64KiB latency 0.73~0.80 ms로 정상이고, `C server + C++ client`는 server 언어와 무관하게 64KiB latency가 3.9~9.5 ms로 오른다. server recv/reply 병목 가설은 기각. 원인은 두 갈래로 분리됐다 — **64B는 coroutine·completion 변환을 포함한 application 명령 수**(native submit 비용은 C와 동일), **64KiB는 admission 전 대기와 반복된 WRITABLE 재제출**. pass 1(astra/xhigh)은 후보 전부 기각·원복(구현한 후보는 DR 72.87→71.58%, RR 76.39→76.01%로 하락). pass 2 진행 중; 계약 유지 후보가 없으면 §7.4 16단계로 `보류` 확정. D-BP17(blocking REQUEST publication wake 누락)은 async 경로를 밟지 않으므로 이 격차에 합산하지 않는다. [pass 1](log/2026-09-07-cpp-multi-reqrep-pass1.ko.md) |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 보류(75.9%) | 보류(74.8%) | 보류(74.9%) | 보류(76.5%) | 보류(79.9%) | 미측정 | before 재측정 aggregate throughput **76.39%**(목표 85%), latency **2.15x**(목표 2.0x); 64KiB latency 5.98x가 aggregate를 끌어올린다; 처리량 C++·C 271.6/357.7, 259.0/346.2, 249.7/333.5, 236.0/308.3, 46.3/58.0 Kops/s; C `perf_c_multi_linux_20260907_221036_p5cmodel.txt`, C++ before `perf_cpp_multi_linux_20260907_221103_p5cmodel.txt`; 이번 캠페인 재측정(`p5cmodel`, C·C++ 같은 조건, 5-run, clients 100, 2-part, 전부 complete)으로 before 값을 갱신했다(2026-09-05 판정 57.4/68.4%는 러너 C 모델 복원 D-BP15 이전 값이라 비교 대상이 아니다). **격차는 전부 C++ client다** — 교차 pairing에서 `C++ server + C client`는 64B에서 C 기준 DR 100.8%/RR 97.6%, 64KiB latency 0.73~0.80 ms로 정상이고, `C server + C++ client`는 server 언어와 무관하게 64KiB latency가 3.9~9.5 ms로 오른다. server recv/reply 병목 가설은 기각. 원인은 두 갈래로 분리됐다 — **64B는 coroutine·completion 변환을 포함한 application 명령 수**(native submit 비용은 C와 동일), **64KiB는 admission 전 대기와 반복된 WRITABLE 재제출**. pass 1(astra/xhigh)은 후보 전부 기각·원복(구현한 후보는 DR 72.87→71.58%, RR 76.39→76.01%로 하락). pass 2 진행 중; 계약 유지 후보가 없으면 §7.4 16단계로 `보류` 확정. D-BP17(blocking REQUEST publication wake 누락)은 async 경로를 밟지 않으므로 이 격차에 합산하지 않는다. 원인·후보 목록은 `MULTI_DEALER_ROUTER_REQREP` 행과 같다. [pass 1](log/2026-09-07-cpp-multi-reqrep-pass1.ko.md) |
