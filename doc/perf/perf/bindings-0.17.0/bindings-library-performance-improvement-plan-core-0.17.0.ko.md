...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미달(65.7%) | 미달(60.8%) | 통과(75.9%) | 미달(49.8%) | 미달(46.4%) | 미측정 | **`미달(59.69%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`m173`, C `complete`·java `complete`); latency 0.82x. 0.17.3(로컬 고정 prefix) 미달 재측정 — 오늘 러너 수정(relay·teardown·계측) 반영. **주의: 이 run은 codex/agent 빌드(cargo·gradle)와 겹쳐 절대값이 13:40 대비 REQREP −40~60%·DD −8% 낮다(부하 의심). 한가한 구간 재측정 `m173b` 대기 — 그 값으로 덮는다.** 처리량 java·C 1,104.9/1,682.2, 838.4/1,380.0, 874.5/1,152.7, 306.2/615.3, 70.9/152.7 Kops/s; C `perf_c_multi_linux_20260908_143751_m173.txt`, java `perf_java_multi_linux_20260908_144109_m173.txt` |
...
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(32.3%) | 미달(37.1%) | 미달(41.5%) | 미달(47.0%) | 미달(9.6%) | 미측정 | **`미달(33.47%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`m173`, C `complete`·java `complete`); latency 4.73x **latency 4.73x > 3.0x**. 0.17.3(로컬 고정 prefix) 미달 재측정 — 오늘 러너 수정(relay·teardown·계측) 반영. **주의: 이 run은 codex/agent 빌드(cargo·gradle)와 겹쳐 절대값이 13:40 대비 REQREP −40~60%·DD −8% 낮다(부하 의심). 한가한 구간 재측정 `m173b` 대기 — 그 값으로 덮는다.** 처리량 java·C 116.0/359.6, 117.4/316.8, 131.1/316.1, 125.1/266.4, 4.4/46.2 Kops/s; C `perf_c_multi_linux_20260908_143751_m173.txt`, java `perf_java_multi_linux_20260908_144109_m173.txt` |
...
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(26.6%) | 미달(34.0%) | 통과(55.7%) | 통과(59.9%) | 미달(12.7%) | 미측정 | **`미달(37.79%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`m173`, C `complete`·java `complete`); latency 6.68x **latency 6.68x > 3.0x**. 0.17.3(로컬 고정 prefix) 미달 재측정 — 오늘 러너 수정(relay·teardown·계측) 반영. **주의: 이 run은 codex/agent 빌드(cargo·gradle)와 겹쳐 절대값이 13:40 대비 REQREP −40~60%·DD −8% 낮다(부하 의심). 한가한 구간 재측정 `m173b` 대기 — 그 값으로 덮는다.** 처리량 java·C 74.4/279.3, 91.6/269.4, 125.8/225.8, 85.4/142.7, 3.4/26.5 Kops/s; C `perf_c_multi_linux_20260908_143751_m173.txt`, java `perf_java_multi_linux_20260908_144109_m173.txt` |
...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미달(33.1%) | 미달(38.0%) | 미달(42.5%) | 미달(51.5%) | 미달(50.1%) | 미측정 | **`미달(43.05%)`** — §7.2 5-run, 고정 Core **0.17.2**(`r4node`, C·Node 모두 `status: complete`); latency 3789.991x. 0.17.1 참고값 35.9%. **latency 비율 3,790x는 C artifact가 아니다** — C 64 B는 0.082 ms로 다른 run과 일치하고 Node가 **1,267 ms(p99 2,444 ms)** 를 보고한다. 578k msg/s 처리량에 1.27 s 지연이면 약 73만 건이 파이프라인에 있어야 하므로 HWM상 불가능하고, 65536 B(320 ms)보다 64 B가 더 느린 것도 다른 언어와 정반대다. **Node 전체의 clock·단위 결함은 아니다** — 같은 날 PUBSUB(`r4node`)의 latency 비율은 ~1x(Node 1,218 ms vs C 1,345 ms, 둘 다 fan-out 큐 깊이)라 timestamp가 정상이다. 따라서 **DD client의 수신·drain 경로 고유 문제**(event loop가 수신을 몰아서 처리해 큐가 깊어짐)로 좁혀진다. Node 쪽 조사 항목(codex 상한 해제 후 개설). throughput 판정과 별개로 latency는 이 원인이 갈릴 때까지 판정 입력이 아니다. SENDSEND 2종은 relay 정합(D-BP24 '있음') 뒤 측정. 처리량 Node·C 578.0/1,744.5, 548.3/1,444.8, 502.5/1,181.3, 334.1/649.2, 81.8/163.1 Kmsg/s; C `perf_c_multi_linux_20260908_090843_r4node.txt`, Node `perf_node_multi_linux_20260908_092340_r4node.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 통과(36.2%) | 통과(35.3%) | 통과(30.4%) | 미달(24.3%) | 통과(42.2%) | 미측정 | **`미달(33.66%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`nodedrain`/C `r10node`, C `complete`·node `complete`); latency 1679.61x **latency 1679.61x > 5.0x**. client echo drain + teardown 창 C 정합(D-BP34) 적용 뒤 complete(drain 적용 전 r10node 32.6%와 동일 수준). 초 단위 latency는 그대로. 처리량 node·C 169.1/467.6, 156.9/444.7, 132.7/437.2, 98.9/406.4, 33.6/79.6 Kops/s; C `perf_c_multi_linux_20260908_113220_r10node.txt`, node `perf_node_multi_linux_20260908_122655_nodedrain.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(35.7%) | 미달(35.6%) | 미달(32.7%) | 미달(27.6%) | 미달(31.0%) | 미측정 | **`미달(32.51%)`** — §7.2 5-run, 고정 Core **0.17.2**(`r4node`, C·Node 모두 `status: complete`); latency 3.458x(64 B Node 0.55 ms vs C 0.229 ms). 0.17.1 참고값 24.3%. Node는 REQREP도 DD·PUBSUB과 같은 20~40% 띠에 있다. **latency는 DD의 초 단위 이상과 다르다** — 0.55~0.79 ms로 C의 2.4~3.2x이며 Go REQREP(3.0~3.4x)과 같은 수준이라 **async terminal 왕복의 고정 비용**(Go·Node 공통, 조사 항목)으로 보인다. DD의 cadence 이상은 단방향 수신 경로에만 있다. 처리량 Node·C 137.2/384.2, 131.4/369.4, 119.6/365.7, 92.8/335.8, 18.6/60.1 Kops/s; C `perf_c_multi_linux_20260908_100508_r4node.txt`, Node `perf_node_multi_linux_20260908_102811_r4node.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 통과(36.2%) | 통과(37.2%) | 통과(31.5%) | 미달(28.9%) | 통과(43.8%) | 미측정 | **`미달(35.51%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`nodedrain`/C `r10node`, C `complete`·node `complete`); latency 566.73x **latency 566.73x > 5.0x**. client echo drain + teardown 창 C 정합(D-BP34) 적용 뒤 complete(적용 전 r10node 35.4%). 초 단위 latency는 그대로. 처리량 node·C 171.8/474.0, 163.1/438.7, 140.0/445.1, 107.6/372.7, 29.9/68.2 Kops/s; C `perf_c_multi_linux_20260908_111241_r10node.txt`, node `perf_node_multi_linux_20260908_122655_nodedrain.txt` |
...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미달(14.3%) | 미달(19.4%) | 미달(24.8%) | 미달(44.8%) | 미달(53.9%) | 미측정 | **`미달(31.46%)`** — 5-run(D-BP32), 고정 Core **0.17.2**(`codex-oneway-verdict-5x-final`/C `r5go`, C `complete`·go `partial`); latency 2.46x. 단방향 drain 수정(C turn 모델) 뒤 첫 complete 측정(이전 '차단': send drain timed out). 64 B 14%·latency 7x는 제출당 goroutine+channel 왕복(turn 구조 비용) 후보 — Go pass 1 지도 대상. 처리량 go·C 248.9/1,737.8, 281.2/1,449.6, 291.5/1,174.3, 288.1/642.9, 87.7/162.6 Kops/s; C `perf_c_multi_linux_20260908_091350_r5go.txt`, go `perf_go_multi_linux_20260908_114359_codex-oneway-verdict-5x-final.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 통과(53.6%) | 통과(56.9%) | 통과(45.8%) | 미달(38.7%) | 통과(55.4%) | 미측정 | **`미달(50.05%)`** — 5-run(D-BP32), 고정 Core **0.17.2**(`codex-oneway-verdict-5x-final`/C `r5go`, C `complete`·go `partial`); latency 2169.48x **latency 2169.48x > 3.0x**. 5-run median이므로 경계 확정. latency 2,169x는 Node·Rust와 같은 초 단위 교차 현상. 처리량 go·C 261.5/488.3, 250.2/439.5, 202.8/443.1, 151.2/391.0, 40.7/73.5 Kops/s; C `perf_c_multi_linux_20260908_101855_r5go.txt`, go `perf_go_multi_linux_20260908_114359_codex-oneway-verdict-5x-final.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(70.1%) | 미달(75.7%) | 미달(74.2%) | 미달(62.9%) | 미달(55.8%) | 미측정 | **`미달(67.72%)`** — §7.2 5-run, 고정 Core **0.17.2**(`r2go`, C·Go 모두 `status: complete`); latency **6.427x**(64~1024 B 3.03~3.05x, 4096 B 2.22x, 65536 B 20.81x). **캠페인 첫 유효 Go REQREP 수치** — 0.17.1에서는 동시 multipart 결함(D-BP12)으로 `EINVAL`/`EAGAIN` 실패라 측정 불가였고 0.17.1 참고값 19.0%는 그 깨진 러너의 값이다. 0.17.2가 동시 multipart를 지원하면서 러너 상한을 제거(`add837942b`, D-BP16 완료)하고 처음 잰 결과다. **latency 3x가 작은 크기에서 일정하게 나타나는 것은 Go 고유 서명**이다 — C++ REQREP은 작은 크기에서 1.0x 안팎이었다. throughput 격차(32%p)와 별개로 조사 가치가 있다. 처리량 Go·C 274.5/391.5, 272.1/359.4, 265.0/357.3, 203.3/323.4, 33.1/59.4 Kops/s; C `perf_c_multi_linux_20260908_085358_r2go.txt`, Go `perf_go_multi_linux_20260908_085815_r2go.txt`; [재개 기록](log/2026-09-08-go-reqrep-resume.ko.md) |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 통과(54.3%) | 통과(57.6%) | 통과(47.0%) | 미달(39.2%) | 통과(56.5%) | 미측정 | **`미달(50.93%)`** — 5-run median; 64~4096 B는 `codex-oneway-verdict-5x-final`(0.17.2 prefix), 65536 B는 relay shutdown 수정 뒤 `gorrfix`(0.17.3-alpha prefix, 5/5 complete; 이전 `차단` server_shutdown_failed 해소, `log/2026-09-08-go-rr-sendsend-65536-shutdown.ko.md`); C 기준 `r5go`(0.17.2). prefix가 섞인 행이므로 다음 C 기준 재측정 때 alpha로 다시 짝짓는다. |
