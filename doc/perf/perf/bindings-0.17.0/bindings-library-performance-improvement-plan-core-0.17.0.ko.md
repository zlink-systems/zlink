...
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 측정됨 | 측정됨 | 측정됨 | 측정됨 | 차단 | 미측정 | **`차단`** — 고정 Core 0.17.2, C turn 모델 client(위 커밋) 뒤에도 65536 B만 5-run 중 2~3회 `server_shutdown_failed`(client는 RESULT까지 정상 완료, relay server가 STOP 뒤 5 s 안에 종료 못 함; `gorr`/`gorr2`, 러너가 사유를 FAIL 줄에 붙이도록 수정). 64~4096 B는 complete(`codex-oneway-verdict-5x-final`). DR SS는 같은 echo server 함수로 5/5 통과하므로 ROUTER client 쪽 teardown 차이 후보 — 0.18.0 이월. 처리량 참고: 65536 B 32.9~42.9 Kops/s(C 73.5K) |
