...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미달(57.8%) | 미달(47.2%) | 미달(52.1%) | 미달(84.4%) | 미달(60.9%) | 미측정 | **`미달(60.48%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r7rust`, C `complete`·rust `complete`); latency 0.75x. relay 정합 뒤 첫 유효 측정(이전 3-run 43.6~100.1%). 처리량 rust·C 1,005.7/1,740.3, 668.6/1,415.2, 606.2/1,164.3, 541.7/642.2, 98.1/161.0 Kops/s; C `perf_c_multi_linux_20260908_113340_r7rust.txt`, rust `perf_rust_multi_linux_20260908_113803_r7rust.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 통과(82.8%) | 통과(85.5%) | 통과(101.0%) | 미달(60.3%) | 미달(37.6%) | 미측정 | **`미달(73.45%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r7rust`, C `complete`·rust `complete`); latency 446.22x **latency 446.22x > 2.0x**. relay 정합(fb3f37191d) 뒤 첫 유효 측정. 처리량 rust·C 403.7/487.3, 359.3/420.2, 369.7/366.0, 237.0/393.0, 29.3/77.7 Kops/s; C `perf_c_multi_linux_20260908_111649_r7rust.txt`, rust `perf_rust_multi_linux_20260908_112622_r7rust.txt` |
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(56.8%) | 미달(59.6%) | 미달(60.6%) | 미달(60.4%) | 미달(33.9%) | 미측정 | **`미달(54.26%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r7rust`, C `complete`·rust `complete`); latency 2.45x **latency 2.45x > 2.0x**. relay 정합 뒤 첫 유효 측정(이전 3-run 62.4~70.0%). 처리량 rust·C 230.9/406.4, 214.7/360.3, 212.3/350.5, 202.6/335.7, 20.1/59.3 Kops/s; C `perf_c_multi_linux_20260908_113127_r7rust.txt`, rust `perf_rust_multi_linux_20260908_113246_r7rust.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 통과(80.3%) | 통과(79.6%) | 통과(79.2%) | 미달(58.7%) | 미달(21.5%) | 미측정 | **`미달(63.84%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r7rust`, C `complete`·rust `complete`); latency 232.83x **latency 232.83x > 2.0x**. relay 정합(fb3f37191d) 뒤 첫 유효 측정. 처리량 rust·C 381.8/475.2, 339.1/426.2, 344.5/435.1, 233.4/397.9, 16.9/78.6 Kops/s; C `perf_c_multi_linux_20260908_112854_r7rust.txt`, rust `perf_rust_multi_linux_20260908_112950_r7rust.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(58.8%) | 미달(58.0%) | 미달(59.5%) | 미달(62.9%) | 미달(36.2%) | 미측정 | **`미달(55.08%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r7rust`, C `complete`·rust `complete`); latency 2.83x **latency 2.83x > 2.0x**. relay 정합 뒤 첫 유효 측정(이전 3-run 66.0~72.3%). 처리량 rust·C 206.4/350.9, 193.8/333.9, 192.5/323.5, 187.9/298.9, 20.7/57.3 Kops/s; C `perf_c_multi_linux_20260908_113407_r7rust.txt`, rust `perf_rust_multi_linux_20260908_113528_r7rust.txt` |
