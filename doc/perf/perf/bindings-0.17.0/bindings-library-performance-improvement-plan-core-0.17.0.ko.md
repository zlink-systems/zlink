...
| `tcp` | `ROUTER_ROUTER_REQREP` | 61.1% | 57.9% | 65.6% | 100.5% | 102.1% | 110.1% | **참고 82.9%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.2(`sgfix-cpp`, C `complete`·cpp `complete`); latency 2.72x. D-BP40 admission 창(applied SNDHWM bytes ÷ wire size) 연속 제출 적용 뒤(이전 sg1: 56.3/47.4%, 깊이 1.00). 65536 B+ latency 4~8x는 창 모델 한계(C는 그 크기에서 BACKPRESSURED를 만나지 않아 깊이 ~2). 처리량 cpp·C 536.8/878.6, 475.6/820.9, 458.6/698.6, 11.0/10.9, 9.4/9.2, 7.8/7.0 Kops/s; C `perf_c_single_linux_20260908_135850_sgfix-cpp.txt`, cpp `perf_cpp_single_linux_20260908_135952_sgfix-cpp.txt` |
...
| `tcp` | `ROUTER_ROUTER_REQREP` | 3.6% | 3.9% | 4.5% | 27.5% | 29.6% | 30.4% | **참고 16.6%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.2(`sg1`, C `partial`·go `complete`); latency 40.58x. Core 0.17.3-alpha prefix, C와 같은 시각 짝지음(D-BP39). Go Single 러너 정책 정합(§10.2 6번)은 부분 완료 상태. 처리량 go·C 32.1/881.8, 32.9/837.2, 32.4/724.5, 3.1/11.3, 2.7/9.2, 2.1/7.0 Kops/s; C `perf_c_single_linux_20260908_130119_sg1.txt`, go `perf_go_single_linux_20260908_135357_sg1.txt` |
| `ws` | `PAIR` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `PUBSUB` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `DEALER_DEALER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `DEALER_ROUTER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
