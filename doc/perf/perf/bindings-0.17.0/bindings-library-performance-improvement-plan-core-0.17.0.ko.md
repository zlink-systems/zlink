...
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 통과(82.1%) | 통과(81.1%) | 통과(101.8%) | 통과(99.5%) | 통과(109.1%) | 미측정 | **`통과(94.73%)` 확정 — §7.2 '최종·경계 판정' 5-run**(`p6b5run`, C·C++ 모두 `status: complete`, `success: 5`); latency 0.745x `통과`. **§2.1 완화 목표 90%를 선택했다**: DD는 2026-09-05 자체 pass 1·Sol 리뷰 pass 2에 이번 캠페인 pass 1(`p3pin`)까지 세 차례를 거쳤고 후보 10개 중 1개만 채택, 9개는 계약·ABA·공개 ABI 사유로 기각돼 §2.1의 '개선 작업이 과도하게 길어지는 경우'에 해당한다. 94.73%는 완화 목표 90%를 넘는다. 개별 최소 85% 미달 outlier는 64 B(82.12%)·256 B(81.09%)이며 남은 원인은 **메시지당 고정 비용**으로 REQREP의 요청당 고정 비용(D-BP20)과 같은 뿌리다. 1-run 값 95.42%는 §7.2 '탐색' 조건이라 판정으로 쓰지 않는다(D-BP21). 처리량 C++·C 1,391.2/1,694.0, 1,250.0/1,541.5, 1,252.1/1,230.5, 648.1/651.1, 181.7/166.5 Kmsg/s; C `perf_c_multi_linux_20260907_231628_p6b5run.txt`, C++ `perf_cpp_multi_linux_20260907_231855_p6b5run.txt`; [pass 1 log](log/2026-09-07-cpp-multi-tcp-dd-pass1.ko.md) |
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 통과(91.1%) | 통과(94.7%) | 통과(90.4%) | 통과(90.4%) | 통과(95.3%) | 미측정 | **`통과(92.37%, 완화 목표 90%)` 확정** — §7.2 5-run(`p8ss5r`, C·C++ `status: complete`·`success: 5`); latency 1.134x `통과`; 개별 최소 85% 미달 없음(최저 90.39%); 처리량 C++·C 451.5/495.8, 428.1/452.1, 407.1/450.3, 366.7/405.7, 70.7/74.2 Kmsg/s; **§2.1 완화 목표 90%를 선택했다.** pass 1(astra/high, `2026-09-08-cpp-multi-sendsend-pass1`)이 **routed send 고유 비용에는 격차가 없음**을 측정으로 보였다 — native routed send inclusive는 C 5,278 / C++ 5,278 Ir로 같고(RR client native는 C 5,627 / C++ 5,362로 C++가 오히려 낮다), C++ builder 추가분은 132 Ir뿐이다. 64 B에서 C++와 C의 application 구간 차이 약 1,732 Ir 중 pass가 찾은 축소 가능 항목 3개(RID ring 복사, `commit_receive_metadata` 103 Ir, 성공 경로 errno 약 14 Ir)를 **모두 합쳐도 약 250 Ir = 격차의 14%**로, 전부 구현해도 약 93.5%에 그쳐 95%에 닿지 않는다(감독자가 수치로 확인해 pass 2를 열지 않았다). 즉 남은 격차는 routed 고유 비용이 아니라 **DD와 같은 메시지당 고정 비용**이며, 그 후보 공간은 DD가 3회 pass·9개 기각으로 이미 소진했다. 완화해도 90%는 넘는다. 기각 근거: RID snapshot을 빌린 pointer로 대체(builder·거절된 async operation이 caller RID보다 오래 산다 — exact-target 재제출 보존), native route lookup·pipe cache(route 교체·credit·admission의 소유자는 Core), 기존 no-go(singleton result·pool·공개 ABI·native part move·러너 변경·상한). pass는 library를 수정하지 않았고 공개 헤더 diff 0줄, 기능 27/27, 회귀 gate 5-run 통과(처리량 최대 −2.30%, latency 최대 +5.08%). **변경 없는 source 재측정으로 5-run 재현성 확인**: DR 92.37→92.25%(−0.13%p), RR 92.45→92.25%(−0.21%p) — 1-run의 ±5%p와 대비된다. C `perf_c_multi_linux_20260907_232939_p8ss5r.txt`, C++ `perf_cpp_multi_linux_20260907_233213_p8ss5r.txt`; [pass 1](log/2026-09-08-cpp-multi-sendsend-pass1.ko.md) |
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 보류(75.1%) | 보류(70.2%) | 보류(72.3%) | 보류(73.9%) | 보류(73.0%) | 미측정 | **`보류(72.87%)` 확정**(목표 85%), latency 2.69x(목표 2.0x, 65536B 9.62x가 aggregate를 끌어올린다); 처리량 C++·C 297.8/396.8, 271.6/387.1, 273.0/377.7, 254.8/345.0, 45.6/62.4 Kops/s; 격차는 전부 C++ client다 — 교차 pairing에서 `C++ server + C client`는 64 B에서 C 기준 100.8%, 64 KiB latency 0.80 ms로 정상(D-BP18). pass 1(astra/xhigh, 후보 전부 기각·원복)과 pass 2(astra/xhigh, `2026-09-07-cpp-multi-reqrep-pass2`) 완료 → **§7.4 16단계로 `보류` 확정**. pass 2 후보(64 B continuation 중복 참조 제거)는 기능 검증 27/27 통과했으나 DR을 72.87→69.35%로 떨어뜨렸고 RR은 이득이 없었으며(76.39→76.39%) 대표 회귀 gate도 넘겨(PUBSUB 64 B −5.33%) 원복했다. 최종 `bindings/cpp/{src,include,tests}` diff 0줄. **보류 근거는 '후보를 못 찾았다'가 아니다(D-BP20)**: 재제출 경로 자체는 C보다 느리지 않고(WRITABLE→재제출 DR 10.2 µs·RR 1.5 µs, 추가 poll 없음; retry vector 할당은 client 전체 Ir의 0.65%, `drain→entry->retry()`가 69.37%), `NO_DATA`까지 비운 뒤 재제출하는 것은 Core 계약이다(`core/doc/spec/core/socket/README.ko.md:1080-1081`). 남은 격차는 **요청당 고정 비용 하나**이며 64 KiB latency는 계약이 정한 drain-then-resubmit pacing이 그 비용을 turn 길이로 바꿔 증폭한 결과다. binding이 계약을 지키는 한 구조적으로 줄일 수 없다. C `perf_c_multi_linux_20260907_220942_p5cmodel.txt`, C++ `perf_cpp_multi_linux_20260907_221009_p5cmodel.txt`; [pass 1](log/2026-09-07-cpp-multi-reqrep-pass1.ko.md), [pass 2](log/2026-09-07-cpp-multi-reqrep-pass2.ko.md) |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 통과(88.5%) | 통과(90.7%) | 통과(86.6%) | 통과(93.2%) | 통과(103.3%) | 미측정 | **`통과(92.45%, 완화 목표 90%)` 확정** — §7.2 5-run(`p8ss5r`); latency 1.291x `통과`; 개별 최소 85% 미달 없음(최저 86.60%); 처리량 C++·C 432.9/489.4, 402.2/443.2, 391.6/452.2, 365.7/392.6, 81.2/78.6 Kmsg/s; 1-run 탐색(`p7explore`)의 C 4096 B 붕괴(282.9 Kmsg/s·337.24 ms, p99 997 ms)는 5-run에서 재현되지 않아(392.6 Kmsg/s·0.284 ms) 1회성 outlier로 판정에서 제외했다(D-BP21). **§2.1 완화 목표 90%를 선택했다.** pass 1(astra/high, `2026-09-08-cpp-multi-sendsend-pass1`)이 **routed send 고유 비용에는 격차가 없음**을 측정으로 보였다 — native routed send inclusive는 C 5,278 / C++ 5,278 Ir로 같고(RR client native는 C 5,627 / C++ 5,362로 C++가 오히려 낮다), C++ builder 추가분은 132 Ir뿐이다. 64 B에서 C++와 C의 application 구간 차이 약 1,732 Ir 중 pass가 찾은 축소 가능 항목 3개(RID ring 복사, `commit_receive_metadata` 103 Ir, 성공 경로 errno 약 14 Ir)를 **모두 합쳐도 약 250 Ir = 격차의 14%**로, 전부 구현해도 약 93.5%에 그쳐 95%에 닿지 않는다(감독자가 수치로 확인해 pass 2를 열지 않았다). 즉 남은 격차는 routed 고유 비용이 아니라 **DD와 같은 메시지당 고정 비용**이며, 그 후보 공간은 DD가 3회 pass·9개 기각으로 이미 소진했다. 완화해도 90%는 넘는다. 기각 근거: RID snapshot을 빌린 pointer로 대체(builder·거절된 async operation이 caller RID보다 오래 산다 — exact-target 재제출 보존), native route lookup·pipe cache(route 교체·credit·admission의 소유자는 Core), 기존 no-go(singleton result·pool·공개 ABI·native part move·러너 변경·상한). pass는 library를 수정하지 않았고 공개 헤더 diff 0줄, 기능 27/27, 회귀 gate 5-run 통과(처리량 최대 −2.30%, latency 최대 +5.08%). **변경 없는 source 재측정으로 5-run 재현성 확인**: DR 92.37→92.25%(−0.13%p), RR 92.45→92.25%(−0.21%p) — 1-run의 ±5%p와 대비된다. 근거·후보 목록은 `MULTI_DEALER_ROUTER_SENDSEND` 행과 같다. C `perf_c_multi_linux_20260907_233439_p8ss5r.txt`, C++ `perf_cpp_multi_linux_20260907_233704_p8ss5r.txt`; [pass 1](log/2026-09-08-cpp-multi-sendsend-pass1.ko.md) |
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 보류(75.9%) | 보류(74.8%) | 보류(74.9%) | 보류(76.5%) | 보류(79.9%) | 미측정 | **`보류(76.39%)` 확정**(목표 85%), latency 2.15x(65536B 5.98x); 처리량 C++·C 271.6/357.7, 259.0/346.2, 249.7/333.5, 236.0/308.3, 46.3/58.0 Kops/s; 격차는 전부 C++ client다 — `C++ server + C client`는 64 B에서 C 기준 97.6%, 64 KiB latency 0.73 ms(D-BP18). pass 1(astra/xhigh, 후보 전부 기각·원복)과 pass 2(astra/xhigh, `2026-09-07-cpp-multi-reqrep-pass2`) 완료 → **§7.4 16단계로 `보류` 확정**. pass 2 후보(64 B continuation 중복 참조 제거)는 기능 검증 27/27 통과했으나 DR을 72.87→69.35%로 떨어뜨렸고 RR은 이득이 없었으며(76.39→76.39%) 대표 회귀 gate도 넘겨(PUBSUB 64 B −5.33%) 원복했다. 최종 `bindings/cpp/{src,include,tests}` diff 0줄. **보류 근거는 '후보를 못 찾았다'가 아니다(D-BP20)**: 재제출 경로 자체는 C보다 느리지 않고(WRITABLE→재제출 DR 10.2 µs·RR 1.5 µs, 추가 poll 없음; retry vector 할당은 client 전체 Ir의 0.65%, `drain→entry->retry()`가 69.37%), `NO_DATA`까지 비운 뒤 재제출하는 것은 Core 계약이다(`core/doc/spec/core/socket/README.ko.md:1080-1081`). 남은 격차는 **요청당 고정 비용 하나**이며 64 KiB latency는 계약이 정한 drain-then-resubmit pacing이 그 비용을 turn 길이로 바꿔 증폭한 결과다. binding이 계약을 지키는 한 구조적으로 줄일 수 없다. 원인·후보 목록은 `MULTI_DEALER_ROUTER_REQREP` 행과 같다. C `perf_c_multi_linux_20260907_221036_p5cmodel.txt`, C++ `perf_cpp_multi_linux_20260907_221103_p5cmodel.txt`; [pass 1](log/2026-09-07-cpp-multi-reqrep-pass1.ko.md), [pass 2](log/2026-09-07-cpp-multi-reqrep-pass2.ko.md) |
| `tcp` | `MULTI_PUBSUB` | 통과(88.9%) | 통과(94.2%) | 통과(90.9%) | 통과(111.5%) | 통과(90.4%) | 미측정 | **`통과(95.16%)` 확정 — §7.2 '최종·경계 판정' 5-run**(`p6b5run`, C·C++ 모두 `status: complete`, `success: 5`); **기본 목표 95% 충족**, latency 1.007x `통과`. 개별 최소 85% 미달 없음(64 B 88.9%가 최저). 1-run 값 95.52%와 차이가 작아 판정이 뒤집히지 않았다. 2026-09-05의 `보류(93.2%)`는 러너 정합 이전 값이라 참고값이다(D-BP19). 처리량 C++·C 2,126.8/2,393.5, 2,238.7/2,376.2, 2,302.4/2,533.5, 1,840.2/1,650.4, 127.6/141.2 Kmsg/s; C `perf_c_multi_linux_20260907_232122_p6b5run.txt`, C++ `perf_cpp_multi_linux_20260907_232356_p6b5run.txt` |
...
| `tls` | `MULTI_DEALER_DEALER` | 통과(81.4%) | 통과(91.4%) | 통과(97.0%) | 통과(103.0%) | 통과(105.0%) | 미측정 | **`통과(95.55%)` 확정 — §7.2 5-run**(`p9tls5`, C·C++ 모두 `status: complete`); **기본 목표 95% 충족**, latency 0.745x `통과`; 개별 최소 85% 미달 outlier는 64 B(81.35%) 하나 — `tcp`와 같은 메시지당 고정 비용이다. 탐색 1-run(`p9tls`)은 95.23%로 5-run 95.55%와 0.32%p 차이다. **2026-09-05의 `보류(79.3%)`는 러너 정합 이전 값이라 참고값이며 16%p 차이가 난다**(D-BP19). 처리량 C++·C 1,452.3/1,785.2, 1,377.7/1,507.8, 1,205.3/1,242.3, 814.9/791.4, 104.9/99.9 Kmsg/s; C `perf_c_multi_linux_20260908_003014_p9tls5.txt`, C++ `perf_cpp_multi_linux_20260908_003243_p9tls5.txt` |
| `tls` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | **측정 차단 — C++ client가 `tls` 4096 B에서 exit 1**(`DEALER_ROUTER_CLIENT_FAIL`, `perf_dealer_router_client.cpp:395`의 `bench.run()` false). 5-run 시도(`pBtls5`)는 `status: partial`(success 3 / fail 2)이라 판정에 쓰지 않는다. 감독자가 좁힌 결과 **트리거는 duration**이다 — 4096 B 단독에서 duration 2는 성공, 5는 실패하며 선행 크기와 무관하다. **`tls`와 routed의 조합에서만 실패**한다: `tls`+`MULTI_DEALER_DEALER` 통과(102.98%), `tcp`+SENDSEND 통과(90.39%), `tls`+SENDSEND 2종 실패. **C 러너는 같은 조건에서 `status: complete`로 완주**하므로 Core가 tls 4096 B를 못 하는 것이 아니다. 진단 job 진행 중([기록](log/2026-09-08-cpp-tls-4096-sendsend.ko.md)). 해소 뒤 5-run으로 다시 재 판정한다. 부분 측정값(64/256/1024 B만)은 참고: DR 91.07/93.84/91.37%, RR 89.40/92.41/88.82%. |
...
| `tls` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | **측정 차단 — C++ client가 `tls` 4096 B에서 exit 1**(`DEALER_ROUTER_CLIENT_FAIL`, `perf_dealer_router_client.cpp:395`의 `bench.run()` false). 5-run 시도(`pBtls5`)는 `status: partial`(success 3 / fail 2)이라 판정에 쓰지 않는다. 감독자가 좁힌 결과 **트리거는 duration**이다 — 4096 B 단독에서 duration 2는 성공, 5는 실패하며 선행 크기와 무관하다. **`tls`와 routed의 조합에서만 실패**한다: `tls`+`MULTI_DEALER_DEALER` 통과(102.98%), `tcp`+SENDSEND 통과(90.39%), `tls`+SENDSEND 2종 실패. **C 러너는 같은 조건에서 `status: complete`로 완주**하므로 Core가 tls 4096 B를 못 하는 것이 아니다. 진단 job 진행 중([기록](log/2026-09-08-cpp-tls-4096-sendsend.ko.md)). 해소 뒤 5-run으로 다시 재 판정한다. 부분 측정값(64/256/1024 B만)은 참고: DR 91.07/93.84/91.37%, RR 89.40/92.41/88.82%. 원인·대조는 `MULTI_DEALER_ROUTER_SENDSEND` 행과 같다. |
...
| `tls` | `MULTI_PUBSUB` | 통과(93.7%) | 통과(119.4%) | 통과(88.9%) | 통과(94.4%) | 통과(90.7%) | 미측정 | **`통과(97.40%)` 확정 — §7.2 5-run**(`pAtls5`, C·C++ 모두 `status: complete`); **기본 목표 95% 충족**, latency 1.001x `통과`; 개별 최소 85% 미달 없음(최저 1024 B 88.87%). 2026-09-05의 `통과(100.2%)`는 러너 정합 이전 참고값이다(D-BP19). 처리량 C++·C 763.2/815.0, 1,140.4/955.4, 1,539.9/1,732.7, 838.3/888.1, 89.5/98.7 Kmsg/s; C `perf_c_multi_linux_20260908_003555_pAtls5.txt`, C++ `perf_cpp_multi_linux_20260908_003822_pAtls5.txt` |
...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 보류(41.6%) | 보류(41.2%) | 보류(46.3%) | 보류(70.4%) | 보류(106.1%) | 미측정 | 판정 확정 `보류(61.1%)`(목표 85%; before 44.7 → pass 1 `b4fc201f7e` → pass 2 `5dc05bfb3e`, 조용한 머신 3-run `p1dotnet-r3q` 15:0x); latency 큐 깊이 제외; 처리량 .NET/C 413.1/992.3, 392.1/950.9, 401.2/866.6, 248.7/353.1, 69.4/65.5 Kmsg/s; [log](log/2026-09-05-dotnet-multi-tcp-before.ko.md) |
...
| `tcp` | `MULTI_PUBSUB` | 보류(43.7%) | 보류(49.4%) | 보류(53.6%) | 보류(64.8%) | 보류(73.1%) | 미측정 | 판정 확정 `보류(56.9%)`(목표 85%; before 44.8), latency 1.25x; 처리량 .NET/C 340.3/778.3, 349.1/707.1, 432.2/806.8, 381.1/588.5, 46.2/63.1 Kmsg/s; [log](log/2026-09-05-dotnet-multi-tcp-before.ko.md) |
