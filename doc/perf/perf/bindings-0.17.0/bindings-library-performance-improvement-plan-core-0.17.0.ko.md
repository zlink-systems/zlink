...
- Single 상태: `미측정`
- Multi 상태: `미달` — `tcp` 3 pattern은 자체 pass 1과 Sol 리뷰 pass 2를 마치고 판정을 닫았다: `MULTI_DEALER_DEALER` `통과(90.8%)`(완화 목표 90% 선택), `MULTI_DEALER_ROUTER_REQREP` `보류(57.4%)`, `MULTI_ROUTER_ROUTER_REQREP` `보류(68.4%)`. `MULTI_PUBSUB`은 before 93.3%로 `미달`(기본 목표 95%, 개선 pass 전)
- 다음 작업: pass 2 변경 커밋·푸시(§7.4 15단계), `MULTI_PUBSUB` `tcp`의 자체 hot-path pass 1과 Sol 리뷰 pass 2, after 측정(§7.4 10~12단계). `131072`와 나머지 transport·pattern은 미측정.
...
|------|------|------------------|
| 현재 언어 | C++ | 순서 1 |
| 현재 pattern | 미달 | Multi `tcp` `MULTI_DEALER_DEALER` 75.0%→90.8%→90.8% `통과`(완화 목표 90%), `MULTI_DEALER_ROUTER_REQREP` 51.8%→55.8%→57.4% `보류`, `MULTI_ROUTER_ROUTER_REQREP` 60.7%→68.4%→68.4% `보류`(pass 2 after, 판정 확정), `MULTI_PUBSUB` 93.3% `미달`(before, 기본 목표 95%, 개선 pass 전); latency aggregate 0.62x / 0.72x / 0.68x / 1.06x로 모두 2.0x 이내(pass 1의 `DEALER_DEALER` 1024B outlier 재현 안 됨); `131072` 미측정 |
...
|------|------|-------------|------------|-----------|
| 1 | C++ | 미측정 | 미달 | Multi `tcp` 3 pattern 판정 확정(`DEALER_DEALER` 통과 90.8% 완화 목표, REQREP 2개 보류 57.4/68.4%); pass 2 변경을 커밋·푸시하고 `MULTI_PUBSUB` `tcp`(93.3%, 목표 95%)의 자체 pass 1과 Sol 리뷰 pass 2를 진행한다. |
