...
- Single 상태: `미측정`
- Multi 상태: `미달` — `tcp` 3 pattern은 자체 pass 1과 Sol 리뷰 pass 2를 마치고 판정을 닫았다: `MULTI_DEALER_DEALER` `통과(90.8%)`(완화 목표 90% 선택), `MULTI_DEALER_ROUTER_REQREP` `보류(57.4%)`, `MULTI_ROUTER_ROUTER_REQREP` `보류(68.4%)`. `MULTI_PUBSUB`은 before 93.3%로 `미달`(기본 목표 95%, 개선 pass 전). `tls`·`ws`·`wss` 4 pattern은 pass 2 코드로 before만 측정했다(04:41~04:55 KST, 판정 미확정): `MULTI_DEALER_DEALER` 77.6% / 84.9% / 93.9% `미달`(`tls` latency 6.23x로 상한 초과), `MULTI_DEALER_ROUTER_REQREP` `tls` 54.5% `미달`, `MULTI_ROUTER_ROUTER_REQREP` `tls` 60.5%·`ws` 58.0% `미달`, `MULTI_PUBSUB` 100.2% / 104.1% / 104.8% `통과 후보`(§7.4 14단계 검토 전). C runner의 `ws`·`wss` `DEALER_ROUTER_REQREP` 4096B와 `wss` `ROUTER_ROUTER_REQREP` 4096B는 C 기준 이상(처리량 붕괴)으로 해당 셀 `보류(C 기준 이상)`, 세 transport aggregate 미확정 — Core/runner 조사 job(brief `core-c-ws-reqrep-4k`) 대기
- 다음 작업: pass 2 변경 커밋·푸시(§7.4 15단계), `MULTI_PUBSUB` `tcp`의 자체 hot-path pass 1과 Sol 리뷰 pass 2, after 측정(§7.4 10~12단계); `tls`·`ws`·`wss`는 `DEALER_DEALER`·REQREP 개선 pass와 `PUBSUB` §7.4 14단계 검토, C 기준 이상 3셀은 조사 결과 후 C·C++ 재측정. `131072`와 SENDSEND·STREAM pattern은 미측정.
...
| `ws` | `MULTI_DEALER_DEALER` | 미달(73.5%) | 미달(67.0%) | 미달(73.6%) | 미달(97.1%) | 미달(113.1%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); aggregate throughput 84.9%(목표 95%), latency 0.63x; 개별 최소 85% 미달 64/256/1024; 처리량 C++/C 616.7/839.2, 592.9/884.4, 581.0/789.2, 276.2/284.3, 57.0/50.4 Kmsg/s; `p1cpp-ws`; C `perf_c_multi_linux_20260905_044714_p1cpp-ws.txt`, C++ `perf_cpp_multi_linux_20260905_044743_p1cpp-ws.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `ws` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `MULTI_DEALER_ROUTER_REQREP` | 미달(37.3%) | 미달(39.4%) | 미달(40.7%) | 보류(C 기준 이상) | 미달(102.9%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); 4096B는 C runner 처리량 붕괴(C 7,867.2 ops/s / 58.623 ms, 1024B는 127,386.8 / 1.978 ms; C++ 4096B 51,107.0 ops/s / 0.954 ms 정상)로 §7.1·§7.3에 따라 비교 제외, Core/runner 조사 job(brief `core-c-ws-reqrep-4k`) 대기; 5 size aggregate 미확정, 4096B 제외 4 size 참고 평균 throughput 55.1%(목표 85%), latency 0.60x; 처리량 C++/C 55.0/147.2, 53.1/134.7, 51.9/127.4, 51.1/7.9(C 이상), 16.6/16.2 Kops/s; `p1cpp-ws`; C `perf_c_multi_linux_20260905_044810_p1cpp-ws.txt`, C++ `perf_cpp_multi_linux_20260905_044839_p1cpp-ws.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `ws` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(47.2%) | 미달(45.5%) | 미달(49.7%) | 미달(47.7%) | 미달(99.9%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); aggregate throughput 58.0%(목표 85%), latency 0.59x; 개별 최소 75% 미달 64/256/1024/4096; 처리량 C++/C 61.7/130.7, 57.9/127.2, 57.1/114.9, 49.7/104.1, 17.6/17.7 Kops/s; `p1cpp-ws`; C `perf_c_multi_linux_20260905_044906_p1cpp-ws.txt`, C++ `perf_cpp_multi_linux_20260905_044934_p1cpp-ws.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `ws` | `MULTI_PUBSUB` | 통과 후보(80.9%) | 통과 후보(115.0%) | 통과 후보(107.6%) | 통과 후보(97.2%) | 통과 후보(119.6%) | 미측정 | before(pass 2 코드); aggregate throughput 104.1%(목표 95% 충족), latency 1.09x; 개별 최소 85% 미달 outlier 64; §7.4 14단계 hot path·POSDDD 검토와 `tcp` `MULTI_PUBSUB` pass 뒤 after 확인 전이라 `통과`로 닫지 않은 임시 표기; 처리량 C++/C 245.4/303.2, 339.7/295.5, 508.6/472.6, 411.2/423.2, 45.0/37.6 Kmsg/s; `p1cpp-ws`; C `perf_c_multi_linux_20260905_045001_p1cpp-ws.txt`, C++ `perf_cpp_multi_linux_20260905_045029_p1cpp-ws.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `ws` | `MULTI_STREAM` | 미측정 | 미측정 | 미측정 | 해당 없음 | 미측정 | 해당 없음 |  |
| `wss` | `MULTI_DEALER_DEALER` | 미달(71.0%) | 미달(73.2%) | 미달(123.0%) | 미달(97.6%) | 미달(105.0%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); aggregate throughput 93.9%(기본 목표 95% 미달, 완화 목표 90%는 개선 pass 전이라 미선택), latency 1.91x(상한 2.0x 이내; 1024B 5.60x, 4096B 2.19x outlier); 개별 최소 85% 미달 64/256; 1024B 123.0%는 C 441.4 Kmsg/s가 `tls`·`ws`보다 낮아 나온 값; 처리량 C++/C 665.1/936.9, 601.2/821.6, 542.9/441.4, 215.7/220.9, 21.5/20.5 Kmsg/s; `p1cpp-wss`; C `perf_c_multi_linux_20260905_045104_p1cpp-wss.txt`, C++ `perf_cpp_multi_linux_20260905_045133_p1cpp-wss.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `wss` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `wss` | `MULTI_DEALER_ROUTER_REQREP` | 미달(39.8%) | 미달(36.9%) | 미달(40.2%) | 보류(C 기준 이상) | 미달(113.4%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); 4096B는 C runner 처리량 붕괴(C 3,413.4 ops/s / 22.360 ms, 1024B는 102,773.2 / 3.954 ms; C++ 4096B 32,961.4 ops/s / 1.499 ms 정상)로 §7.1·§7.3에 따라 비교 제외, Core/runner 조사 job(brief `core-c-ws-reqrep-4k`) 대기; 5 size aggregate 미확정, 4096B 제외 4 size 참고 평균 throughput 57.6%(목표 85%), latency 0.41x; 처리량 C++/C 47.0/118.2, 42.1/114.2, 41.3/102.8, 33.0/3.4(C 이상), 7.7/6.8 Kops/s; `p1cpp-wss`; C `perf_c_multi_linux_20260905_045202_p1cpp-wss.txt`, C++ `perf_cpp_multi_linux_20260905_045231_p1cpp-wss.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `wss` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `wss` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(42.6%) | 미달(39.6%) | 미달(39.7%) | 보류(C 기준 이상) | 미달(99.8%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); 4096B는 C runner 처리량 붕괴(C 16,836.6 ops/s / 20.395 ms, 1024B는 102,266.6 / 4.956 ms; C++ 4096B 35,799.0 ops/s / 1.379 ms 정상)로 §7.1·§7.3에 따라 비교 제외, Core/runner 조사 job(brief `core-c-ws-reqrep-4k`) 대기; 5 size aggregate 미확정, 4096B 제외 4 size 참고 평균 throughput 55.4%(목표 85%), latency 0.39x; 처리량 C++/C 49.5/116.2, 42.6/107.4, 40.6/102.3, 35.8/16.8(C 이상), 7.2/7.2 Kops/s; `p1cpp-wss`; C `perf_c_multi_linux_20260905_045258_p1cpp-wss.txt`, C++ `perf_cpp_multi_linux_20260905_045327_p1cpp-wss.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `wss` | `MULTI_PUBSUB` | 통과 후보(83.6%) | 통과 후보(89.6%) | 통과 후보(107.4%) | 통과 후보(116.7%) | 통과 후보(126.9%) | 미측정 | before(pass 2 코드); aggregate throughput 104.8%(목표 95% 충족), latency 0.92x; 개별 최소 85% 미달 outlier 64; §7.4 14단계 검토와 `tcp` `MULTI_PUBSUB` pass 뒤 after 확인 전이라 `통과`로 닫지 않은 임시 표기; 처리량 C++/C 355.9/425.9, 438.1/488.9, 524.7/488.4, 254.3/217.9, 25.4/20.0 Kmsg/s; `p1cpp-wss`; C `perf_c_multi_linux_20260905_045355_p1cpp-wss.txt`, C++ `perf_cpp_multi_linux_20260905_045424_p1cpp-wss.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `wss` | `MULTI_STREAM` | 미측정 | 미측정 | 미측정 | 해당 없음 | 미측정 | 해당 없음 |  |
| `tls` | `MULTI_DEALER_DEALER` | 미달(64.4%) | 미달(71.7%) | 미달(84.4%) | 미달(66.4%) | 미달(101.3%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); aggregate throughput 77.6%(목표 95%), latency 6.23x(상한 2.0x **초과**; 1024B 25.96x = C 27.7 ms / C++ 719.3 ms, 4096B 3.97x outlier, 중앙값 0.88x); 개별 최소 85% 미달 64/256/1024/4096; 처리량 C++/C 680.1/1056.1, 668.4/932.1, 580.9/688.6, 229.6/345.9, 26.7/26.4 Kmsg/s; `p1cpp-tls`; C `perf_c_multi_linux_20260905_044211_p1cpp-tls.txt`, C++ `perf_cpp_multi_linux_20260905_044349_p1cpp-tls.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `tls` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `tls` | `MULTI_DEALER_ROUTER_REQREP` | 미달(45.4%) | 미달(42.2%) | 미달(42.3%) | 미달(46.0%) | 미달(96.4%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); aggregate throughput 54.5%(목표 85%), latency 0.43x; 개별 최소 75% 미달 64/256/1024/4096; C 4096B latency 33.7 ms(1024B 1.96 ms)는 처리량이 유지돼 비교 셀로 두되 Core/runner 조사 참고 신호로 기록; 처리량 C++/C 56.2/123.9, 52.9/125.3, 47.5/112.4, 40.3/87.5, 8.5/8.8 Kops/s; `p1cpp-tls`; C `perf_c_multi_linux_20260905_044417_p1cpp-tls.txt`, C++ `perf_cpp_multi_linux_20260905_044446_p1cpp-tls.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `tls` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `tls` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(54.1%) | 미달(50.0%) | 미달(46.5%) | 미달(51.1%) | 미달(100.6%) | 미측정 | before(pass 2 코드, 이 transport 개선 pass 전); aggregate throughput 60.5%(목표 85%), latency 0.38x; 개별 최소 75% 미달 64/256/1024/4096; 처리량 C++/C 55.3/102.3, 51.3/102.6, 47.7/102.4, 41.6/81.5, 9.2/9.2 Kops/s; `p1cpp-tls`; C `perf_c_multi_linux_20260905_044513_p1cpp-tls.txt`, C++ `perf_cpp_multi_linux_20260905_044542_p1cpp-tls.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `tls` | `MULTI_PUBSUB` | 통과 후보(85.7%) | 통과 후보(85.7%) | 통과 후보(89.4%) | 통과 후보(104.3%) | 통과 후보(135.7%) | 미측정 | before(pass 2 코드); aggregate throughput 100.2%(목표 95% 충족), latency 0.97x; 개별 최소 85% 미달 없음(64/256B 85.7% 경계); §7.4 14단계 hot path·POSDDD 검토와 `tcp` `MULTI_PUBSUB` pass 뒤 after 확인 전이라 `통과`로 닫지 않은 임시 표기; 처리량 C++/C 318.3/371.5, 386.4/450.8, 510.3/570.7, 281.4/269.8, 33.8/24.9 Kmsg/s; `p1cpp-tls`; C `perf_c_multi_linux_20260905_044610_p1cpp-tls.txt`, C++ `perf_cpp_multi_linux_20260905_044638_p1cpp-tls.txt`; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
...
|------|------|------------------|
| 현재 언어 | C++ | 순서 1 |
| 현재 pattern | 미달 | Multi `tcp` `MULTI_DEALER_DEALER` 75.0%→90.8%→90.8% `통과`(완화 목표 90%), `MULTI_DEALER_ROUTER_REQREP` 51.8%→55.8%→57.4% `보류`, `MULTI_ROUTER_ROUTER_REQREP` 60.7%→68.4%→68.4% `보류`(pass 2 after, 판정 확정), `MULTI_PUBSUB` 93.3% `미달`(before, 기본 목표 95%, 개선 pass 전); latency aggregate 0.62x / 0.72x / 0.68x / 1.06x로 모두 2.0x 이내(pass 1의 `DEALER_DEALER` 1024B outlier 재현 안 됨); `131072` 미측정. Multi `tls`·`ws`·`wss` before(pass 2 코드, 판정 미확정): `DEALER_DEALER` 77.6% / 84.9% / 93.9% `미달`(latency 6.23x / 0.63x / 1.91x, `tls` 상한 초과), `DEALER_ROUTER_REQREP` `tls` 54.5% `미달`(0.43x), `ROUTER_ROUTER_REQREP` `tls` 60.5%·`ws` 58.0% `미달`(0.38x / 0.59x), `PUBSUB` 100.2% / 104.1% / 104.8% `통과 후보`(0.97x / 1.09x / 0.92x, §7.4 14단계 전); `DEALER_ROUTER_REQREP` `ws`·`wss`와 `ROUTER_ROUTER_REQREP` `wss`는 C 4096B 기준 이상으로 aggregate 미확정(4 size 참고 55.1% / 57.6% / 55.4%) |
| paired C | 완료 | `p1cpp`, 2026-09-05 03:49~03:54 KST, pattern마다 C 직후 C++ 순차 실행; [log](log/2026-09-05-cpp-multi-tcp-before.ko.md). `p1cpp-tls`·`p1cpp-ws`·`p1cpp-wss`, 04:41~04:55 KST, transport마다 4 pattern을 C 직후 C++ 순차 실행, 24 report 모두 complete; C runner `ws`·`wss` `DEALER_ROUTER_REQREP` 4096B(7,867 / 3,413 ops/s)와 `wss` `ROUTER_ROUTER_REQREP` 4096B(16,837 ops/s) 처리량 붕괴는 C 기준 blocker로 기록하고 Core/runner 조사 job(brief `core-c-ws-reqrep-4k`) 개설; [log](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| 개선 반복 | 완료(3 pattern) | 자체 hot-path pass 1(04:13 KST, `perf_cpp_multi_linux_20260905_041338.txt`)과 Sol read-only 리뷰 기반 pass 2(04:35 KST, `perf_cpp_multi_linux_20260905_043517.txt`, 후보 9개 중 3개 채택·6개 no-go) 각 after 1회 측정; `MULTI_PUBSUB`은 두 pass 모두 전; [pass 1](log/2026-09-05-cpp-multi-tcp-pass1.ko.md), [pass 2](log/2026-09-05-cpp-multi-tcp-pass2.ko.md) |
| 커밋과 푸시 | 진행 중 | pass 1 코드 변경은 `86b897abf7`로 커밋; pass 2 코드 변경(`bindings/cpp/src/Runtime/Messaging/**` 5개 파일)은 작업 worktree `/home/hep7hep7/project/zlink-wt-cpp-perf2`에 미커밋(§7.4 15단계 커밋·푸시 대기) |

### 10.3 언어 진행 상태

| 순서 | 언어 | Single 상태 | Multi 상태 | 다음 작업 |
|------|------|-------------|------------|-----------|
| 1 | C++ | 미측정 | 미달 | Multi `tcp` 3 pattern 판정 확정(`DEALER_DEALER` 통과 90.8% 완화 목표, REQREP 2개 보류 57.4/68.4%); pass 2 변경을 커밋·푸시하고 `MULTI_PUBSUB` `tcp`(93.3%, 목표 95%)의 자체 pass 1과 Sol 리뷰 pass 2를 진행한다. `tls`·`ws`·`wss` before 측정 완료(`DEALER_DEALER` 77.6/84.9/93.9% 미달, REQREP `tls`·`ws` 54.5~60.5% 미달, `PUBSUB` 100.2~104.8% 통과 후보): 이 세 transport의 개선 pass와 §7.4 14단계 검토를 진행하고, C 기준 이상 3셀(`ws`·`wss` `DEALER_ROUTER_REQREP` 4096B, `wss` `ROUTER_ROUTER_REQREP` 4096B)은 Core/runner 조사(brief `core-c-ws-reqrep-4k`) 결과 뒤 C·C++를 재측정한다. |
