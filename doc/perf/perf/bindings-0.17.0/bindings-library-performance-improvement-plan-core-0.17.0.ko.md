...
- 다음 작업: pass 2 변경 커밋·푸시(§7.4 15단계), `MULTI_PUBSUB` `tcp`의 자체 hot-path pass 1과 Sol 리뷰 pass 2, after 측정(§7.4 10~12단계); `tls`·`ws`·`wss`는 `DEALER_DEALER`·REQREP 개선 pass와 `PUBSUB` §7.4 14단계 검토(C 기준 이상 3셀은 D-B89 뒤 재측정 완료). `131072`와 SENDSEND·STREAM pattern은 미측정.
...
| `wss` | `MULTI_DEALER_ROUTER_REQREP` | 미달(35.5%) | 미달(38.5%) | 미달(46.8%) | 미달(63.5%) | 미달(92.7%) | 미측정 | before 재측정(pass 2 코드, 이 transport 개선 pass 전; D-B89 뒤 재짝지음, `보류(C 기준 이상)` 해소); aggregate throughput 43.7%(목표 85%), latency 1.02x; 개별 최소 75% 미달 64/256/1024/4096; 처리량 C++/C 48.7/137.4, 47.1/122.3, 47.6/101.7, 34.9/54.9, 7.2/7.8 Kops/s; `p1cpp-wss-fix`; C `perf_c_multi_linux_20260905_052801_p1cpp-wss-fix.txt`, C++ `perf_cpp_multi_linux_20260905_053247_p1cpp-wss-fix.txt`; [재측정](log/2026-09-05-cpp-multi-ws-wss-reqrep-remeasure.ko.md), [before](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
| `wss` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `wss` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(35.7%) | 미달(36.7%) | 미달(45.9%) | 미달(73.7%) | 미달(110.5%) | 미측정 | before 재측정(pass 2 코드, 이 transport 개선 pass 전; D-B89 뒤 재짝지음, `보류(C 기준 이상)` 해소); aggregate throughput 44.7%(목표 85%), latency 0.75x; 개별 최소 75% 미달 64/256/1024/4096; 처리량 C++/C 41.1/115.1, 39.9/108.6, 40.0/87.2, 35.0/47.6, 7.2/6.5 Kops/s; `p1cpp-wss-fix`; C `perf_c_multi_linux_20260905_052828_p1cpp-wss-fix.txt`, C++ `perf_cpp_multi_linux_20260905_053315_p1cpp-wss-fix.txt`; [재측정](log/2026-09-05-cpp-multi-ws-wss-reqrep-remeasure.ko.md), [before](log/2026-09-05-cpp-multi-tls-ws-wss-before.ko.md) |
