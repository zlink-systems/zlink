...
- Single 상태: `미달` — before 측정 완료(paired C·C++ 7 pattern × 6 transport × 6 size, 06:08~07:16 KST, [log](log/2026-09-05-cpp-single-before.ko.md)). throughput aggregate: one-way(PAIR/PUBSUB/DD) 82~109%(`inproc` DD 57.0%·PUBSUB 87.0%·PAIR 82.0%만 크게 낮음), routed one-way(DR/RR) 84~96%(`inproc` 64.4/75.1%), REQREP 40.6~46.4%(REQUEST async 경로, multi tcp `보류` 57/68%와 같은 형태). **one-way·routed one-way의 평균 latency는 C 대비 수십~수백 배**: 두 러너 모두 blocking send + header timestamp one-way latency로 정의가 같으나 C는 송신이 병목이라 큐가 비어 있고(0.002~0.03 ms) C++는 수신 경로가 송신보다 느려 큐가 HWM까지 찬다(4~118 ms) → single one-way의 실제 과제는 수신 경로(`socket_t::recv(received_t&)`·`received_t`·poller wrapper)이며 pass 1 job 진행 중(07:17 KST~). REQREP latency는 0.29~0.39x.
- Multi 상태: `보류/미달` — `tcp` 3 pattern은 자체 pass 1과 Sol 리뷰 pass 2를 마치고 판정을 닫았다: `MULTI_DEALER_DEALER` `통과(90.8%)`(완화 목표 90% 선택), `MULTI_DEALER_ROUTER_REQREP` `보류(57.4%)`, `MULTI_ROUTER_ROUTER_REQREP` `보류(68.4%)`. `MULTI_PUBSUB`은 자체 pass 1·Sol pass 2 모두 no-go(코드 변경 없음) 뒤 3-run 81.5%로 `보류` 확정(목표 95%; runner parity 차이 항목은 별도 트랙). `tls`·`ws`·`wss` 4 pattern은 pass 2 코드로 before만 측정했다(04:41~04:55 KST, 판정 미확정): `MULTI_DEALER_DEALER` 77.6% / 84.9% / 93.9% `미달`(`tls` latency 6.23x로 상한 초과), `MULTI_DEALER_ROUTER_REQREP` `tls` 54.5% `미달`, `MULTI_ROUTER_ROUTER_REQREP` `tls` 60.5%·`ws` 58.0% `미달`, `MULTI_PUBSUB` 100.2% / 104.1% / 104.8% `통과 후보`(§7.4 14단계 검토 전). C runner의 `ws`·`wss` REQREP 4096B 붕괴(C 기준 이상 3셀)는 C runner 제출 턴 문제로 확정해 runner만 수정(D-B89)하고 05:27~05:33 KST에 C·C++를 재짝지어 쟀다: `MULTI_DEALER_ROUTER_REQREP` `ws` 53.1% / `wss` 43.7%, `MULTI_ROUTER_ROUTER_REQREP` `ws` 72.3% / `wss` 44.7% 모두 `미달`(`ws` 두 pattern은 latency 평균 2.43x/2.54x도 상한 초과 — C runner 턴 구조로 C latency가 0.3 ms까지 낮아진 결과), [재측정 log](log/2026-09-05-cpp-multi-ws-wss-reqrep-remeasure.ko.md).
...
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `PAIR` | 미달(65.6%) | 미달(73.0%) | 미달(95.6%) | 미달(90.3%) | 미달(95.9%) | 미달(106.0%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 87.7%(목표 95%), latency 19.18x; 처리량 C++/C 867.9/1323.3, 814.9/1116.9, 796.7/833.3, 57.3/63.4, 33.5/35.0, 17.0/16.0 Kmsg/s; latency ms C++/C 1.703/0.028, 0.298/0.030, 1.018/0.030, 0.232/0.049, 0.201/0.069, 0.203/0.105; `p1cpp-single`; C `perf_c_single_linux_20260905_060852_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_061312_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `tcp` | `PUBSUB` | 미달(87.1%) | 미달(83.3%) | 미달(112.3%) | 미달(99.2%) | 미달(99.0%) | 미달(138.8%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 103.3%(목표 95%), latency 4.03x; 처리량 C++/C 764.1/877.5, 703.3/843.9, 707.0/629.4, 12.8/12.9, 5.9/5.9, 3.5/2.5 Kmsg/s; latency ms C++/C 0.087/0.051, 0.132/0.050, 0.569/0.051, 0.248/0.068, 0.241/0.090, 0.265/0.112; `p1cpp-single`; C `perf_c_single_linux_20260905_061703_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_062200_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `tcp` | `DEALER_DEALER` | 미달(71.5%) | 미달(75.7%) | 미달(95.6%) | 미달(95.9%) | 미달(93.0%) | 미달(97.0%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 88.1%(목표 95%), latency 453.76x; 처리량 C++/C 781.6/1092.7, 761.7/1006.3, 747.0/781.3, 39.5/41.2, 24.2/26.0, 14.9/15.3 Kmsg/s; latency ms C++/C 95.615/0.047, 30.820/0.048, 2.548/0.049, 0.342/0.068, 0.288/0.114, 0.250/0.119; `p1cpp-single`; C `perf_c_single_linux_20260905_062626_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063047_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `tcp` | `DEALER_ROUTER` | 미달(74.6%) | 미달(79.6%) | 미달(91.4%) | 미달(79.1%) | 미달(82.8%) | 미달(96.9%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 84.1%(목표 85%), latency 382.67x; 처리량 C++/C 784.9/1052.5, 749.6/941.2, 674.8/738.7, 31.5/39.9, 21.0/25.4, 14.8/15.3 Kmsg/s; latency ms C++/C 86.416/0.050, 22.657/0.051, 6.130/0.051, 0.434/0.069, 0.335/0.083, 0.252/0.123; `p1cpp-single`; C `perf_c_single_linux_20260905_063438_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063859_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `tcp` | `ROUTER_ROUTER` | 미달(77.8%) | 미달(78.8%) | 미달(93.0%) | 미달(96.6%) | 미달(94.4%) | 미달(98.6%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 89.9%(목표 85%), latency 37.83x; 처리량 C++/C 795.9/1023.0, 742.6/942.7, 700.3/753.3, 39.1/40.5, 23.4/24.8, 14.7/14.9 Kmsg/s; latency ms C++/C 0.470/0.049, 0.531/0.046, 9.626/0.049, 0.345/0.070, 0.293/0.081, 0.255/0.122; `p1cpp-single`; C `perf_c_single_linux_20260905_065502_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_065923_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `ws` | `PAIR` | 미달(77.3%) | 미달(85.2%) | 미달(91.6%) | 미달(96.7%) | 미달(101.2%) | 미달(99.6%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 91.9%(목표 95%), latency 1195.83x; 처리량 C++/C 871.1/1126.4, 787.4/924.1, 601.3/656.7, 32.1/33.2, 19.6/19.4, 10.5/10.5 Kmsg/s; latency ms C++/C 117.633/0.024, 40.104/0.024, 16.513/0.033, 4.823/0.075, 0.336/0.096, 0.340/0.145; `p1cpp-single`; C `perf_c_single_linux_20260905_060852_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_061312_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `ws` | `PUBSUB` | 미달(79.2%) | 미달(84.6%) | 미달(96.8%) | 미달(97.7%) | 미달(116.9%) | 미달(129.0%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 100.7%(목표 95%), latency 489.34x; 처리량 C++/C 681.0/860.3, 668.1/790.1, 578.7/597.8, 12.6/12.9, 7.0/6.0, 3.3/2.5 Kmsg/s; latency ms C++/C 87.976/0.052, 48.283/0.051, 16.059/0.053, 0.411/0.082, 0.369/0.099, 0.285/0.133; `p1cpp-single`; C `perf_c_single_linux_20260905_061703_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_062200_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `ws` | `DEALER_DEALER` | 미달(75.7%) | 미달(82.4%) | 미달(88.9%) | 미달(95.1%) | 미달(97.5%) | 미달(99.4%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 89.8%(목표 95%), latency 658.90x; 처리량 C++/C 725.2/957.9, 733.6/890.0, 529.6/595.5, 23.5/24.7, 15.7/16.1, 9.7/9.7 Kmsg/s; latency ms C++/C 135.005/0.049, 41.973/0.050, 16.113/0.054, 5.222/0.087, 0.431/0.116, 0.383/0.166; `p1cpp-single`; C `perf_c_single_linux_20260905_062626_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063047_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `ws` | `DEALER_ROUTER` | 미달(76.0%) | 미달(83.3%) | 미달(97.6%) | 미달(97.8%) | 미달(100.1%) | 미달(99.5%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 92.4%(목표 85%), latency 696.03x; 처리량 C++/C 767.7/1010.5, 743.9/893.0, 553.8/567.6, 23.9/24.5, 15.9/15.9, 9.6/9.7 Kmsg/s; latency ms C++/C 140.139/0.052, 55.094/0.051, 18.081/0.054, 5.134/0.097, 0.425/0.112, 0.387/0.178; `p1cpp-single`; C `perf_c_single_linux_20260905_063438_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063859_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `ws` | `ROUTER_ROUTER` | 미달(79.1%) | 미달(82.4%) | 미달(95.1%) | 미달(97.5%) | 미달(95.7%) | 미달(97.4%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 91.2%(목표 85%), latency 615.44x; 처리량 C++/C 764.7/966.9, 714.8/867.5, 546.4/574.3, 23.8/24.4, 15.6/16.3, 9.5/9.8 Kmsg/s; latency ms C++/C 118.411/0.051, 49.742/0.050, 16.689/0.054, 5.657/0.085, 0.431/0.114, 0.389/0.161; `p1cpp-single`; C `perf_c_single_linux_20260905_065502_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_065923_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `wss` | `PAIR` | 미달(74.8%) | 미달(89.3%) | 미달(99.2%) | 미달(98.5%) | 미달(107.8%) | 미달(109.2%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 96.5%(목표 95%), latency 1089.54x; 처리량 C++/C 913.3/1220.8, 722.9/809.7, 406.4/409.8, 11.2/11.4, 6.0/5.5, 3.2/2.9 Kmsg/s; latency ms C++/C 92.875/0.024, 42.218/0.025, 21.515/0.026, 12.403/0.148, 10.816/0.294, 10.568/0.407; `p1cpp-single`; C `perf_c_single_linux_20260905_060852_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_061312_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `wss` | `PUBSUB` | 미달(84.6%) | 미달(98.6%) | 미달(103.8%) | 미달(102.6%) | 미달(96.4%) | 미달(85.4%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 95.2%(목표 95%), latency 487.55x; 처리량 C++/C 757.8/895.5, 672.8/682.4, 311.1/299.8, 10.7/10.5, 5.7/5.9, 2.2/2.6 Kmsg/s; latency ms C++/C 92.298/0.059, 51.048/0.068, 30.365/0.060, 10.276/0.167, 9.539/0.244, 0.818/0.410; `p1cpp-single`; C `perf_c_single_linux_20260905_061703_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_062200_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `wss` | `DEALER_DEALER` | 미달(73.8%) | 미달(87.1%) | 미달(102.7%) | 미달(97.4%) | 미달(99.7%) | 미달(98.4%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 93.2%(목표 95%), latency 577.28x; 처리량 C++/C 776.1/1052.3, 705.5/810.5, 324.8/316.4, 9.7/10.0, 5.6/5.6, 3.1/3.2 Kmsg/s; latency ms C++/C 112.292/0.057, 53.230/0.059, 28.930/0.062, 14.372/0.182, 9.711/0.264, 9.075/0.453; `p1cpp-single`; C `perf_c_single_linux_20260905_062626_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063047_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `wss` | `DEALER_ROUTER` | 미달(71.7%) | 미달(94.3%) | 미달(102.5%) | 미달(99.7%) | 미달(100.9%) | 미달(99.3%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 94.7%(목표 85%), latency 636.59x; 처리량 C++/C 773.1/1078.9, 744.9/789.6, 318.4/310.6, 10.0/10.0, 5.8/5.7, 3.2/3.2 Kmsg/s; latency ms C++/C 136.721/0.058, 47.216/0.057, 30.748/0.060, 11.957/0.167, 9.622/0.264, 8.941/0.444; `p1cpp-single`; C `perf_c_single_linux_20260905_063438_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063859_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `wss` | `ROUTER_ROUTER` | 미달(81.3%) | 미달(88.7%) | 미달(99.7%) | 미달(96.7%) | 미달(96.7%) | 미달(98.8%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 93.7%(목표 85%), latency 470.36x; 처리량 C++/C 825.5/1015.2, 679.1/766.0, 309.0/309.8, 9.8/10.1, 5.6/5.8, 3.1/3.2 Kmsg/s; latency ms C++/C 78.367/0.056, 46.806/0.059, 27.544/0.059, 14.269/0.164, 12.021/0.252, 11.191/0.449; `p1cpp-single`; C `perf_c_single_linux_20260905_065502_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_065923_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `tls` | `PAIR` | 미달(68.3%) | 미달(89.4%) | 미달(98.3%) | 미달(96.3%) | 미달(96.6%) | 미달(99.9%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 91.5%(목표 95%), latency 665.60x; 처리량 C++/C 869.5/1272.4, 842.4/941.9, 486.8/495.5, 14.0/14.5, 7.3/7.6, 3.8/3.8 Kmsg/s; latency ms C++/C 121.218/0.031, 2.711/0.033, 1.596/0.034, 0.882/0.121, 0.867/0.192, 0.891/0.332; `p1cpp-single`; C `perf_c_single_linux_20260905_060852_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_061312_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `tls` | `PUBSUB` | 미달(84.7%) | 미달(100.7%) | 미달(112.7%) | 미달(99.8%) | 미달(92.3%) | 미달(85.8%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 96.0%(목표 95%), latency 14.76x; 처리량 C++/C 807.2/953.4, 730.1/725.3, 347.4/308.4, 12.1/12.2, 5.6/6.1, 2.2/2.5 Kmsg/s; latency ms C++/C 0.156/0.054, 2.333/0.057, 1.909/0.055, 0.714/0.138, 0.667/0.200, 0.708/0.356; `p1cpp-single`; C `perf_c_single_linux_20260905_061703_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_062200_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `tls` | `DEALER_DEALER` | 미달(67.7%) | 미달(84.5%) | 미달(119.3%) | 미달(97.0%) | 미달(97.0%) | 미달(94.1%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 93.3%(목표 95%), latency 613.00x; 처리량 C++/C 728.0/1076.0, 728.6/861.8, 430.5/361.0, 12.0/12.4, 7.0/7.2, 3.7/3.9 Kmsg/s; latency ms C++/C 146.298/0.057, 58.405/0.056, 1.800/0.054, 1.049/0.136, 0.934/0.208, 0.953/0.332; `p1cpp-single`; C `perf_c_single_linux_20260905_062626_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063047_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `tls` | `DEALER_ROUTER` | 미달(72.5%) | 미달(87.2%) | 미달(118.3%) | 미달(101.7%) | 미달(98.4%) | 미달(100.4%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 96.4%(목표 85%), latency 657.77x; 처리량 C++/C 785.0/1083.1, 774.5/888.2, 422.6/357.1, 12.2/12.0, 7.1/7.2, 4.0/3.9 Kmsg/s; latency ms C++/C 160.756/0.053, 51.211/0.059, 1.829/0.059, 1.031/0.144, 0.914/0.216, 0.901/0.381; `p1cpp-single`; C `perf_c_single_linux_20260905_063438_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063859_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `tls` | `ROUTER_ROUTER` | 미달(77.3%) | 미달(90.1%) | 미달(95.9%) | 미달(98.5%) | 미달(98.4%) | 미달(98.1%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 93.0%(목표 85%), latency 176.03x; 처리량 C++/C 821.1/1061.8, 760.4/844.3, 450.8/469.8, 12.3/12.5, 7.1/7.2, 3.9/4.0 Kmsg/s; latency ms C++/C 0.834/0.055, 28.480/0.054, 20.821/0.056, 10.278/0.141, 8.679/0.211, 8.627/0.347; `p1cpp-single`; C `perf_c_single_linux_20260905_065502_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_065923_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `inproc` | `PAIR` | 미달(71.3%) | 미달(74.0%) | 미달(72.3%) | 미달(90.9%) | 미달(87.4%) | 미달(96.3%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 82.0%(목표 95%), latency 597.54x; 처리량 C++/C 999.4/1401.6, 916.1/1238.0, 857.9/1186.6, 304.7/335.2, 199.0/227.6, 127.7/132.6 Kmsg/s; latency ms C++/C 4.504/0.002, 2.399/0.002, 0.819/0.002, 0.005/0.004, 0.007/0.005, 0.010/0.008; `p1cpp-single`; C `perf_c_single_linux_20260905_060852_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_061312_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `inproc` | `PUBSUB` | 미달(91.8%) | 미달(96.8%) | 미달(98.4%) | 미달(48.0%) | 미달(115.1%) | 미달(71.9%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 87.0%(목표 95%), latency 6.56x; 처리량 C++/C 849.4/925.2, 755.9/780.6, 741.1/753.3, 79.1/164.9, 62.7/54.4, 14.2/19.7 Kmsg/s; latency ms C++/C 0.035/0.007, 0.076/0.008, 0.158/0.007, 0.025/0.022, 0.026/0.024, 0.026/0.028; `p1cpp-single`; C `perf_c_single_linux_20260905_061703_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_062200_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `inproc` | `DEALER_DEALER` | 미달(80.6%) | 미달(84.6%) | 미달(87.5%) | 미달(33.8%) | 미달(25.9%) | 미달(29.5%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 57.0%(목표 95%), latency 194.91x; 처리량 C++/C 860.0/1067.3, 768.1/908.3, 763.6/873.0, 117.1/346.2, 56.4/218.0, 23.1/78.3 Kmsg/s; latency ms C++/C 5.203/0.008, 2.876/0.008, 0.932/0.008, 0.024/0.024, 0.032/0.027, 0.062/0.035; `p1cpp-single`; C `perf_c_single_linux_20260905_062626_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063047_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `inproc` | `DEALER_ROUTER` | 미달(73.7%) | 미달(76.5%) | 미달(77.2%) | 미달(34.7%) | 미달(28.9%) | 미달(95.2%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 64.4%(목표 85%), latency 188.45x; 처리량 C++/C 882.0/1197.1, 761.5/995.7, 754.3/977.5, 131.6/379.1, 60.7/210.0, 77.4/81.3 Kmsg/s; latency ms C++/C 5.061/0.008, 2.913/0.008, 0.946/0.008, 0.024/0.026, 0.031/0.028, 0.025/0.035; `p1cpp-single`; C `perf_c_single_linux_20260905_063438_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063859_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `inproc` | `DEALER_ROUTER_REQREP` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `inproc` | `ROUTER_ROUTER` | 미달(78.2%) | 미달(80.8%) | 미달(86.2%) | 미달(80.1%) | 미달(41.1%) | 미달(84.2%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 75.1%(목표 85%), latency 472.14x; 처리량 C++/C 930.0/1189.1, 830.5/1028.3, 801.9/930.4, 288.3/360.0, 78.2/190.1, 66.7/79.2 Kmsg/s; latency ms C++/C 12.992/0.008, 7.326/0.008, 2.477/0.008, 0.018/0.024, 0.023/0.028, 0.025/0.034; `p1cpp-single`; C `perf_c_single_linux_20260905_065502_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_065923_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `inproc` | `ROUTER_ROUTER_REQREP` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ipc` | `PAIR` | 미달(79.0%) | 미달(77.6%) | 미달(98.1%) | 미달(96.7%) | 미달(96.5%) | 미달(101.7%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 91.6%(목표 95%), latency 8.99x; 처리량 C++/C 927.7/1174.9, 796.3/1026.7, 832.6/849.1, 62.2/64.3, 34.3/35.5, 18.1/17.7 Kmsg/s; latency ms C++/C 0.655/0.027, 0.163/0.028, 0.425/0.030, 0.212/0.048, 0.195/0.061, 0.187/0.090; `p1cpp-single`; C `perf_c_single_linux_20260905_060852_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_061312_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `ipc` | `PUBSUB` | 미달(80.2%) | 미달(84.6%) | 미달(101.5%) | 미달(143.2%) | 미달(143.8%) | 미달(98.7%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 108.7%(목표 95%), latency 2.64x; 처리량 C++/C 760.8/948.3, 704.5/832.8, 718.7/708.4, 18.5/12.9, 8.6/6.0, 2.5/2.5 Kmsg/s; latency ms C++/C 0.089/0.046, 0.096/0.046, 0.151/0.048, 0.239/0.065, 0.240/0.078, 0.204/0.107; `p1cpp-single`; C `perf_c_single_linux_20260905_061703_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_062200_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `ipc` | `DEALER_DEALER` | 미달(76.4%) | 미달(79.0%) | 미달(90.1%) | 미달(99.1%) | 미달(96.6%) | 미달(98.8%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 90.0%(목표 95%), latency 178.08x; 처리량 C++/C 844.7/1105.6, 786.1/994.6, 764.5/848.9, 42.1/42.5, 25.6/26.5, 16.0/16.1 Kmsg/s; latency ms C++/C 31.081/0.044, 11.367/0.047, 5.168/0.048, 0.324/0.069, 0.273/0.082, 0.234/0.114; `p1cpp-single`; C `perf_c_single_linux_20260905_062626_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063047_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
| `ipc` | `DEALER_ROUTER` | 미달(79.6%) | 미달(81.3%) | 미달(92.0%) | 미달(97.8%) | 미달(97.5%) | 미달(98.9%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 91.2%(목표 85%), latency 151.54x; 처리량 C++/C 869.1/1091.7, 792.3/975.0, 764.4/830.9, 41.6/42.5, 26.0/26.7, 16.1/16.3 Kmsg/s; latency ms C++/C 29.242/0.046, 8.363/0.051, 5.319/0.051, 0.328/0.069, 0.268/0.079, 0.232/0.115; `p1cpp-single`; C `perf_c_single_linux_20260905_063438_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_063859_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `ipc` | `ROUTER_ROUTER` | 미달(84.0%) | 미달(83.5%) | 미달(93.3%) | 미달(94.2%) | 미달(96.1%) | 미달(98.3%) | before(pass 2 코드, 개선 pass 전); aggregate throughput 91.6%(목표 85%), latency 6.89x; 처리량 C++/C 817.0/972.6, 731.3/876.0, 739.3/792.4, 39.1/41.5, 25.4/26.5, 15.8/16.1 Kmsg/s; latency ms C++/C 0.510/0.046, 0.264/0.053, 0.666/0.046, 0.348/0.066, 0.272/0.077, 0.236/0.115; `p1cpp-single`; C `perf_c_single_linux_20260905_065502_p1cpp-single.txt`, C++ `perf_cpp_single_linux_20260905_065923_p1cpp-single.txt`; [log](log/2026-09-05-cpp-single-before.ko.md) |
...
| `tcp` | `MULTI_PUBSUB` | 보류(80.3%) | 보류(61.9%) | 보류(80.4%) | 보류(91.4%) | 보류(93.4%) | 미측정 | 자체 hot-path pass 1 no-go(subscriber 공개 wrapper 약 5%만 남고 계약 유지 후보 없음; 코드 변경 없음) 뒤 3-run 재짝지음으로 판정 확정 `보류(81.5%)`(목표 95%); aggregate throughput 81.5%, latency 1.08x; before 1-run 93.3%·job after 1-run 120.7%와 같은 코드 — lossy PUBSUB run-to-run 편차 큼; 처리량 C++/C 659.3/821.3, 610.6/986.3, 769.4/956.9, 620.2/678.4, 63.9/68.4 Kmsg/s; `p1cpp-pubsub-r3`; C `perf_c_multi_linux_20260905_055350_p1cpp-pubsub-r3.txt`, C++ `perf_cpp_multi_linux_20260905_055517_p1cpp-pubsub-r3.txt`; [pass 1·3-run](log/2026-09-05-cpp-multi-tcp-pubsub-pass1.ko.md), [before](log/2026-09-05-cpp-multi-tcp-before.ko.md) |
