...
| Transport | Pattern | 64 | 256 | 1024 | 65536 | 131072 | 262144 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|-------|--------|--------|------------------|
| `tcp` | `PAIR` | 35.9% | 42.0% | 56.4% | 48.6% | 40.7% | 40.4% | **참고 44.0%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3-alpha(`sg1`, C `partial`·go `complete`); latency 1.28x. Core 0.17.3-alpha prefix, C와 같은 시각 짝지음(D-BP39). Go Single 러너 정책 정합(§10.2 6번)은 부분 완료 상태. 처리량 go·C 637.4/1,777.8, 602.1/1,435.0, 597.5/1,059.8, 52.3/107.6, 26.1/64.2, 13.1/32.5 Kops/s; C `perf_c_single_linux_20260908_130119_sg1.txt`, go `perf_go_single_linux_20260908_135357_sg1.txt` |
| `tcp` | `PUBSUB` | 45.2% | 52.6% | 76.2% | 380.5% | 392.8% | 462.7% | **참고 235.0%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3-alpha(`sg1`, C `partial`·go `complete`); latency 1.27x. Core 0.17.3-alpha prefix, C와 같은 시각 짝지음(D-BP39). Go Single 러너 정책 정합(§10.2 6번)은 부분 완료 상태. 처리량 go·C 657.0/1,452.0, 615.5/1,169.6, 599.3/786.3, 51.9/13.6, 24.9/6.3, 12.6/2.7 Kops/s; C `perf_c_single_linux_20260908_130119_sg1.txt`, go `perf_go_single_linux_20260908_135357_sg1.txt` |
| `tcp` | `DEALER_DEALER` | 32.5% | 40.4% | 52.3% | 45.9% | 40.2% | 38.4% | **참고 41.6%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3-alpha(`sg1`, C `partial`·go `complete`); latency 1.16x. Core 0.17.3-alpha prefix, C와 같은 시각 짝지음(D-BP39). Go Single 러너 정책 정합(§10.2 6번)은 부분 완료 상태. 처리량 go·C 589.8/1,813.6, 561.3/1,388.5, 550.3/1,052.2, 50.7/110.5, 25.8/64.1, 13.1/34.1 Kops/s; C `perf_c_single_linux_20260908_130119_sg1.txt`, go `perf_go_single_linux_20260908_135357_sg1.txt` |
| `tcp` | `DEALER_ROUTER` | 32.8% | 38.5% | 52.5% | 47.1% | 40.4% | 42.1% | **참고 42.2%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3-alpha(`sg1`, C `partial`·go `complete`); latency 1.25x. Core 0.17.3-alpha prefix, C와 같은 시각 짝지음(D-BP39). Go Single 러너 정책 정합(§10.2 6번)은 부분 완료 상태. 처리량 go·C 593.5/1,806.9, 554.4/1,440.7, 552.5/1,051.9, 51.5/109.2, 26.0/64.3, 14.3/33.8 Kops/s; C `perf_c_single_linux_20260908_130119_sg1.txt`, go `perf_go_single_linux_20260908_135357_sg1.txt` |
| `tcp` | `DEALER_ROUTER_REQREP` | 39.5% | 37.2% | 44.6% | 68.9% | 73.5% | 74.5% | **참고 56.4%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3(`sgfix-go`, C `complete`·go `complete`); latency 3.70x. D-BP40 창 정합 뒤(log 2026-09-08-single-reqrep-parity-rust-go): 18.8%→56.4%. latency /2 정의 오류 제거. 큰 크기는 창 16/8/4 vs C 실제 깊이 1.9 → latency 2.7~11.8x(blocking Submit이 admission 결과를 돌려주지 않는 binding 한계, C++·.NET과 동일). 요청 goroutine의 LockOSThread 제거(역할 goroutine만 고정, §1.1.4). 처리량 go·C 306.9/776.7, 311.3/837.2, 317.4/710.9, 7.2/10.5, 6.3/8.5, 5.2/7.0 Kops/s; C `perf_c_single_linux_20260908_153924_sgfix-go.txt`, go `perf_go_single_linux_20260908_154025_sgfix-go.txt` |
| `tcp` | `ROUTER_ROUTER` | 32.5% | 41.8% | 49.9% | 43.5% | 43.4% | 42.2% | **참고 42.2%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3-alpha(`sg1`, C `partial`·go `complete`); latency 1.25x. Core 0.17.3-alpha prefix, C와 같은 시각 짝지음(D-BP39). Go Single 러너 정책 정합(§10.2 6번)은 부분 완료 상태. 처리량 go·C 539.2/1,659.6, 510.5/1,221.0, 491.6/984.7, 46.6/107.2, 25.8/59.4, 14.0/33.1 Kops/s; C `perf_c_single_linux_20260908_130119_sg1.txt`, go `perf_go_single_linux_20260908_135357_sg1.txt` |
| `tcp` | `ROUTER_ROUTER_REQREP` | 36.8% | 40.7% | 47.2% | 61.8% | 73.7% | 71.1% | **참고 55.2%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3(`sgfix-go`, C `complete`·go `complete`); latency 4.81x. D-BP40 창 정합 뒤: 16.6%→55.2%(같은 log). latency /2 정의 오류 제거; 큰 크기 latency는 창 모델 한계. 처리량 go·C 326.3/886.7, 329.2/808.9, 325.6/689.4, 6.7/10.9, 6.4/8.7, 5.0/7.1 Kops/s; C `perf_c_single_linux_20260908_153924_sgfix-go.txt`, go `perf_go_single_linux_20260908_154025_sgfix-go.txt` |
| `ws` | `PAIR` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `PUBSUB` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `DEALER_DEALER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `DEALER_ROUTER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
...
| `tcp` | `DEALER_ROUTER_REQREP` | 62.8% | 미측정 | 58.3% | 44.3% | 53.2% | 52.6% | **참고 54.2%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3(`sgfix-rust`, C `partial`·rust `complete`); latency 1.25x. D-BP40 창 정합 뒤(log 2026-09-08-single-reqrep-parity-rust-go): 6.4%→54.2%. 256 B는 C DR REQREP 러너 간헐 결함(replier_fatal, 짝 없음)으로 무효. latency는 이전 러너의 /2 정의 오류를 고쳤으므로 이전 값과 비교 불가. 큰 크기 깊이는 C와 같은 1.9인데 처리량 44~53%는 Rust binding 요청 경로 per-message 비용. 처리량 rust·C 507.1/807.7, 420.4/721.8, 4.8/10.9, 4.6/8.6, 3.5/6.7 Kops/s; C `perf_c_single_linux_20260908_153813_sgfix-rust.txt`, rust `perf_rust_single_linux_20260908_161653_sgfix-rust.txt` |
| `tcp` | `ROUTER_ROUTER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `tcp` | `ROUTER_ROUTER_REQREP` | 57.9% | 56.9% | 62.4% | 50.0% | 50.4% | 53.2% | **참고 55.1%**(D-BP2: Single은 판정 대상 아님) — 1-run, 고정 Core 0.17.3(`sgfix-rust`, C `partial`·rust `complete`); latency 1.32x. D-BP40 창 정합 뒤: 6.1%→55.1%(같은 log). latency /2 정의 오류 제거. 큰 크기 깊이 C와 동일, 처리량 차이는 Rust binding 요청 경로 비용. 처리량 rust·C 511.7/883.6, 469.6/824.7, 433.0/693.8, 5.3/10.7, 4.6/9.0, 3.4/6.4 Kops/s; C `perf_c_single_linux_20260908_153813_sgfix-rust.txt`, rust `perf_rust_single_linux_20260908_161653_sgfix-rust.txt` |
| `ws` | `PAIR` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `PUBSUB` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `DEALER_DEALER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `ws` | `DEALER_ROUTER` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
