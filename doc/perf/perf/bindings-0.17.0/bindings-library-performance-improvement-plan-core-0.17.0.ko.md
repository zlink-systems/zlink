...
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 미달(91.1%) | 미달(94.7%) | 미달(90.4%) | 미달(90.4%) | 미달(95.3%) | 미측정 | before aggregate throughput **92.37%**(목표 95%), latency 1.134x `통과`; 개별 최소 85% 미달 없음(최저 90.39%); 처리량 C++·C 451.5/495.8, 428.1/452.1, 407.1/450.3, 366.7/405.7, 70.7/74.2 Kmsg/s; §7.2 '최종·경계 판정' **5-run** before(`p8ss5r`, C·C++ 모두 `status: complete`·`success: 5`, 23:29~23:39 KST). 기본 목표 95% 미달, 완화 목표 90% 초과. **완화 목표를 아직 적용하지 않는다** — §2.1의 완화 조항은 '개선 작업이 과도하게 길어지는 경우'인데 SENDSEND는 이번 캠페인에서 개선 pass를 거친 적이 없다(DD는 3회·후보 9개 기각 뒤 완화를 택했다). 이 경로는 **target RID를 지정하는 routed one-way send**로 DD·REQREP과 다른 미프로파일 경로여서 기각 목록을 물려받는다고 단정하지 않는다. 개선 pass 1 진행 중(`2026-09-08-cpp-multi-sendsend-pass1`). C `perf_c_multi_linux_20260907_232939_p8ss5r.txt`, C++ `perf_cpp_multi_linux_20260907_233213_p8ss5r.txt` |
...
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미달(88.5%) | 미달(90.7%) | 미달(86.6%) | 미달(93.2%) | 미달(103.3%) | 미측정 | before aggregate throughput **92.45%**(목표 95%), latency 1.291x `통과`; 개별 최소 85% 미달 없음(최저 86.60%); 처리량 C++·C 432.9/489.4, 402.2/443.2, 391.6/452.2, 365.7/392.6, 81.2/78.6 Kmsg/s; 1-run 탐색(`p7explore`)에서 C 4096 B가 throughput 282.9 Kmsg/s·latency 337.24 ms(p99 997 ms)로 무너져 aggregate가 98.11%로 부풀었으나 **5-run에서 재현되지 않았다**(C 4096 B 392.6 Kmsg/s·0.284 ms). 1회성 outlier이므로 `보류(C 기준 이상)`로 기록하지 않는다 — D-BP21의 '1-run은 탐색, 판정은 5-run' 규칙이 걸러낸 사례다. §7.2 '최종·경계 판정' **5-run** before(`p8ss5r`, C·C++ 모두 `status: complete`·`success: 5`, 23:29~23:39 KST). 기본 목표 95% 미달, 완화 목표 90% 초과. **완화 목표를 아직 적용하지 않는다** — §2.1의 완화 조항은 '개선 작업이 과도하게 길어지는 경우'인데 SENDSEND는 이번 캠페인에서 개선 pass를 거친 적이 없다(DD는 3회·후보 9개 기각 뒤 완화를 택했다). 이 경로는 **target RID를 지정하는 routed one-way send**로 DD·REQREP과 다른 미프로파일 경로여서 기각 목록을 물려받는다고 단정하지 않는다. 개선 pass 1 진행 중(`2026-09-08-cpp-multi-sendsend-pass1`). 원인·후보 목록은 `MULTI_DEALER_ROUTER_SENDSEND` 행과 같다. C `perf_c_multi_linux_20260907_233439_p8ss5r.txt`, C++ `perf_cpp_multi_linux_20260907_233704_p8ss5r.txt` |
