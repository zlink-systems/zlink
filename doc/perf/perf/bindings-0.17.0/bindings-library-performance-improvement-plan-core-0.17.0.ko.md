...
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 통과(91.5%) | 통과(103.3%) | 통과(111.1%) | 미달(39.1%) | 통과(59.3%) | 미측정 | **`통과(80.87%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`javarr`/C `r9java`, C `complete`·java `complete`); latency 468.72x **latency 468.72x > 3.0x**. Java relay를 async submit + 단일 pending reply FIFO(C 모델)로 고친 뒤 첫 complete(이전: 4096 B relay BACKPRESSURED 실패로 차단). 4096 B latency 2,326x는 Node·Rust와 같은 초 단위 교차 현상, 64~1024 B latency 3.1~7.5x도 한도(3.0x) 초과 — throughput은 통과, latency 미달. 처리량 java·C 433.4/473.8, 440.7/426.8, 456.9/411.1, 145.4/371.5, 44.5/75.0 Kops/s; C `perf_c_multi_linux_20260908_113501_r9java.txt`, java `perf_java_multi_linux_20260908_121935_javarr.txt` |
