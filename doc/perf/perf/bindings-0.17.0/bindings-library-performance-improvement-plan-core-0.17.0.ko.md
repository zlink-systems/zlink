...
| `ws` | `MULTI_STREAM` | 통과(98.1%) | 통과(96.1%) | 통과(95.2%) | 해당 없음 | 통과(101.0%) | 해당 없음 | **`통과(97.61%)`** — 1-run(D-BP32), 고정 Core **0.17.3-alpha**(`s173x`, C `complete`·cpp `complete`); latency 1.03x. Core 0.17.3-alpha(STREAM 수신 소유권 수정)에서 첫 complete. ws/wss 64 KB 크기 페널티(D-BP28)는 C에도 남아 있음. 처리량 cpp·C 445.0/453.5, 450.1/468.4, 441.9/464.3, 35.3/34.9 Kops/s; C `perf_c_multi_linux_20260908_135739_s173x.txt`, cpp `perf_cpp_multi_linux_20260908_140055_s173x.txt` |
...
| `wss` | `MULTI_STREAM` | 통과(91.2%) | 통과(89.7%) | 통과(92.6%) | 해당 없음 | 통과(91.9%) | 해당 없음 | **`미달(91.33%)`** — 1-run(D-BP32), 고정 Core **0.17.3-alpha**(`s173x`, C `complete`·cpp `complete`); latency 1.09x. Core 0.17.3-alpha(STREAM 수신 소유권 수정)에서 첫 complete. ws/wss 64 KB 크기 페널티(D-BP28)는 C에도 남아 있음. 처리량 cpp·C 370.0/405.7, 362.4/404.0, 348.7/376.6, 19.7/21.4 Kops/s; C `perf_c_multi_linux_20260908_135739_s173x.txt`, cpp `perf_cpp_multi_linux_20260908_140055_s173x.txt` |
...
| `tls` | `MULTI_STREAM` | 통과(95.1%) | 통과(98.5%) | 통과(99.3%) | 해당 없음 | 통과(96.0%) | 해당 없음 | **`통과(97.21%)`** — 1-run(D-BP32), 고정 Core **0.17.3-alpha**(`s173x`, C `complete`·cpp `complete`); latency 1.03x. Core 0.17.3-alpha(STREAM 수신 소유권 수정)에서 첫 complete. ws/wss 64 KB 크기 페널티(D-BP28)는 C에도 남아 있음. 처리량 cpp·C 398.5/419.2, 405.6/411.9, 394.1/397.0, 42.9/44.6 Kops/s; C `perf_c_multi_linux_20260908_135739_s173x.txt`, cpp `perf_cpp_multi_linux_20260908_140055_s173x.txt` |
