...
| `tls` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | **측정 차단 — 러너 결함 3건을 순차로 해소 중**. (1) relay가 수신마다 async reply를 던져 socket당 completion reservation 65,536 초과 → C 기준 수신-송신 결합으로 수정(`e0862e1e5c`, D-BP24). (2) client가 admission만 보고 socket을 닫아 relay의 잔여 echo가 ETIMEDOUT → C처럼 admitted echo를 다 받고 닫도록 수정(`33f63ae89d`); 이 둘로 **C++ report는 `status: complete`가 됐다**(검증 5건 전 크기 runs 5). (3) **그런데 C 기준선이 `tls` SENDSEND runs 5에서 비결정적으로 죽는다** — SIGABRT `malloc_consolidate(): unaligned fastbin chunk detected`와 SIGSEGV, 5회 중 3회(runs 2는 0회). 메모리 손상이며 진단 job 진행 중(D-BP25, [기록](log/2026-09-08-c-tls-sendsend-corruption.ko.md)). C는 7개 언어 전부의 기준이라 이 셀은 C가 안정될 때까지 판정하지 않는다. 크래시 run의 partial C report로 계산한 값은 무효다. |
...
| `tls` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | **측정 차단 — 러너 결함 3건을 순차로 해소 중**. (1) relay가 수신마다 async reply를 던져 socket당 completion reservation 65,536 초과 → C 기준 수신-송신 결합으로 수정(`e0862e1e5c`, D-BP24). (2) client가 admission만 보고 socket을 닫아 relay의 잔여 echo가 ETIMEDOUT → C처럼 admitted echo를 다 받고 닫도록 수정(`33f63ae89d`); 이 둘로 **C++ report는 `status: complete`가 됐다**(검증 5건 전 크기 runs 5). (3) **그런데 C 기준선이 `tls` SENDSEND runs 5에서 비결정적으로 죽는다** — SIGABRT `malloc_consolidate(): unaligned fastbin chunk detected`와 SIGSEGV, 5회 중 3회(runs 2는 0회). 메모리 손상이며 진단 job 진행 중(D-BP25, [기록](log/2026-09-08-c-tls-sendsend-corruption.ko.md)). C는 7개 언어 전부의 기준이라 이 셀은 C가 안정될 때까지 판정하지 않는다. 크래시 run의 partial C report로 계산한 값은 무효다. 원인·대조는 `MULTI_DEALER_ROUTER_SENDSEND` 행과 같다. |
