...
| Transport | Pattern | 64 | 256 | 1024 | 4096 | 65536 | 131072 | 결과 파일 / 메모 |
|-----------|---------|----|-----|------|------|-------|--------|------------------|
| `tcp` | `MULTI_DEALER_DEALER` | 미달(65.7%) | 미달(60.8%) | 통과(75.9%) | 미달(49.8%) | 미달(46.4%) | 미측정 | **`미달(59.69%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`m173`, C `complete`·java `complete`); latency 0.82x. 0.17.3(로컬 고정 prefix) 미달 재측정 — 오늘 러너 수정(relay·teardown·계측) 반영. 처리량 java·C 1,104.9/1,682.2, 838.4/1,380.0, 874.5/1,152.7, 306.2/615.3, 70.9/152.7 Kops/s; C `perf_c_multi_linux_20260908_143751_m173.txt`, java `perf_java_multi_linux_20260908_144109_m173.txt` |
...
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(32.3%) | 미달(37.1%) | 미달(41.5%) | 미달(47.0%) | 미달(9.6%) | 미측정 | **`미달(33.47%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`m173`, C `complete`·java `complete`); latency 4.73x **latency 4.73x > 3.0x**. 0.17.3(로컬 고정 prefix) 미달 재측정 — 오늘 러너 수정(relay·teardown·계측) 반영. 처리량 java·C 116.0/359.6, 117.4/316.8, 131.1/316.1, 125.1/266.4, 4.4/46.2 Kops/s; C `perf_c_multi_linux_20260908_143751_m173.txt`, java `perf_java_multi_linux_20260908_144109_m173.txt` |
...
| `tcp` | `MULTI_ROUTER_ROUTER_REQREP` | 미달(26.6%) | 미달(34.0%) | 통과(55.7%) | 통과(59.9%) | 미달(12.7%) | 미측정 | **`미달(37.79%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`m173`, C `complete`·java `complete`); latency 6.68x **latency 6.68x > 3.0x**. 0.17.3(로컬 고정 prefix) 미달 재측정 — 오늘 러너 수정(relay·teardown·계측) 반영. 처리량 java·C 74.4/279.3, 91.6/269.4, 125.8/225.8, 85.4/142.7, 3.4/26.5 Kops/s; C `perf_c_multi_linux_20260908_143751_m173.txt`, java `perf_java_multi_linux_20260908_144109_m173.txt` |
