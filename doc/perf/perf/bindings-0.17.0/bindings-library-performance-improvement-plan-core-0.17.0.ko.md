...
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
| `tcp` | `MULTI_DEALER_ROUTER_REQREP` | 미달(35.7%) | 미달(35.6%) | 미달(32.7%) | 미달(27.6%) | 미달(31.0%) | 미측정 | **`미달(32.51%)`** — §7.2 5-run, 고정 Core **0.17.2**(`r4node`, C·Node 모두 `status: complete`); latency 3.458x(64 B Node 0.55 ms vs C 0.229 ms). 0.17.1 참고값 24.3%. Node는 REQREP도 DD·PUBSUB과 같은 20~40% 띠에 있고, latency 이상은 DD 행과 같은 binding client 수신·drain cadence 계열이다. 처리량 Node·C 137.2/384.2, 131.4/369.4, 119.6/365.7, 92.8/335.8, 18.6/60.1 Kops/s; C `perf_c_multi_linux_20260908_100508_r4node.txt`, Node `perf_node_multi_linux_20260908_102811_r4node.txt` |
| `tcp` | `MULTI_ROUTER_ROUTER_SENDSEND` | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 | 미측정 |  |
