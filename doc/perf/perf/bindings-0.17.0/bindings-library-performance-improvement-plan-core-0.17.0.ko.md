...
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 미달(85.8%) | 미달(88.1%) | 미달(83.3%) | 미달(70.3%) | 미달(66.1%) | 미측정 | **`미달(78.72%)`** — §7.2 5-run, 고정 Core **0.17.2**(`r3java`, C·Java 모두 `status: complete`); latency 432.256x. **latency 432x는 C artifact가 아니다** — C는 정상(0.2 ms대)이고 Java client가 4096 B에서 초 단위를 보고한다. Java DD(0.69x)·PUBSUB(1.03x)은 정상이므로 **echo 수신(SENDSEND client) 경로 고유**다. Node DD(1,267 ms vs C 0.082 ms)와 같은 계열의 **binding client 수신·drain cadence 이상**이며 `tcp`에서 나므로 D-BP29의 Core tls/ws 문제와는 별개다. 원인이 갈릴 때까지 latency는 판정 입력이 아니다(조사 항목, codex 상한 해제 후). Java relay(server)는 `submit_sync()`로 이미 C 모델(D-BP24 대상 아님). 0.17.1 참고값 없음. 처리량 Java·C 412.0/480.2, 385.7/437.6, 362.5/435.4, 274.4/390.5, 49.2/74.4 Kmsg/s; C `perf_c_multi_linux_20260908_100243_r3java.txt`, Java `perf_java_multi_linux_20260908_102147_r3java.txt` |
...
| `tcp` | `MULTI_DEALER_ROUTER_SENDSEND` | 통과(36.3%) | 통과(34.1%) | 미달(29.9%) | 미달(24.7%) | 통과(37.9%) | 미측정 | **`미달(32.59%)`** — 1-run(D-BP32), 고정 Core **0.17.2**(`r10node`, C `complete`·node `complete`); latency 1654.71x **latency 1654.71x > 5.0x**. relay 정합(d744799803) 뒤 첫 유효 측정. 처리량 node·C 170.0/467.6, 151.7/444.7, 130.6/437.2, 100.5/406.4, 30.2/79.6 Kops/s; C `perf_c_multi_linux_20260908_113220_r10node.txt`, node `perf_node_multi_linux_20260908_113734_r10node.txt` |
