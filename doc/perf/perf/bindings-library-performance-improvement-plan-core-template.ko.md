<!--
템플릿 사용법: 문서 전체의 `{{CORE_VERSION}}`를 대상 Core 버전으로, `{{START_DATE}}`를
시작일로 치환한다. 이 문서와 같은 폴더에 `log/`를 만들고 측정 기록을 저장한다.
-->
# core {{CORE_VERSION}} bindings 성능 측정 및 개선 시트

> 시작일: {{START_DATE}}
>
> 작업 브랜치: `core-{{CORE_VERSION}}-bindings-performance`

이 시트는 Core {{CORE_VERSION}} release runtime을 기준으로 C와 각 bindings를 비교하는
측정 결과와 판정만 기록한다. 이전 버전의 수치와 완료 판정은 가져오지 않는다.
측정 원본, 재측정 결과와 개선 전후 비교는 같은 폴더의 `log/`에 남긴다.

## 측정 기준

- Core library는 Git release로 배포한 {{CORE_VERSION}} runtime을 사용한다. 측정 목적으로 Core를 다시 build하지 않는다.
...
아래 행은 형식만 보여 주는 예시이며, 실제 측정 결과가 아니다. 같은 transport·pattern의
모든 지원 size 비율을 적고 그 산술평균으로 판정한다.

| 언어 | Suite / pattern | Transport | C 대비 throughput 비율 | 산술평균 | 목표 | 판정 | 결과 기록 |
|---|---|---|---|---:|---:|---|---|
| 예시 C++ | Single `PAIR` | `tcp` | 64B 90.0% / 256B 92.0% / 1KiB 88.0% / 64KiB 92.0% / 128KiB 90.0% / 256KiB 88.0% | 90.0% | 90.0% | 통과 | `log/YYYY-MM-DD-cpp-single-pair-tcp.ko.md` |
...
언어별 평균도 같은 가중치로 계산한다. 예를 들어 C++의 완료 또는 보류 transport·pattern
두 개가 90.0%, 86.0%이면 이 표에는 `2 / 88.0%`를 기록한다.
