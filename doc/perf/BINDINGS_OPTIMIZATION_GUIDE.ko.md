...
| .NET | (pass 2, 0.17.1) reply submit closure→struct −352 B/op, pump closure 지연 | relay 결합(`fb3f37191d`), client echo drain(`537c6eec93`, D-BP34) | **지배 항목 둘** — send builder 177.7 ns·80.5 B/msg(격차 35%), message helper P/Invoke 전환 137.5 ns(27%); GC 0.3%로 무관(D-BP31). 둘 다 **공개 API 계약**: builder는 공개 operation 객체, close는 terminal 시점. 전환 10→9 후보는 효과 없음(D-BP32) | 보류 6(DD 62.5·PUBSUB 61.0·DR SS 63.9·RR SS 69.5(3-run)·DR REQREP 60.5·RR REQREP 63.1; 목표 70) — 지도 완료, 후보 3건 측정 기각(pass 1 close 병합 효과 없음; pass 2 P/Invoke 융합 13→9회는 −9% **역효과**, operation 객체 재사용은 one-shot 계약 위반, D-BP41). 남은 후보: Message wrapper 생성·소멸 76 ns |
| Java | **지도(alpha, `log/2026-09-08-java-reqrep-cost-map.ko.md`)**: REQREP 64~4096 B 격차에 지배 항목 없음 — completion handoff(poller→worker) 299.5 ns/op = 격차 13%, GC 0.6%; 관리 할당 1,542 B/op(REQREP)·377 B/msg(DD); 잔여 86%는 할당 실행·FFM 전이·materialization이 섞여 미분리 → pass 1 미실행. 다음은 할당 1,542 B/op의 출처 분해 | relay: `submit_sync()`가 SNDTIMEO 1 s 만료 시 BACKPRESSURED를 throw해 4096 B에서 결정적 실패 → async submit + 단일 pending reply FIFO(`611b022b37`); TIMEOUTS 계측(`3cc840d82e`); teardown 창 C 정합(`ad89fb599a`) | 미작성. 65536 B만 무너지는 **크기 의존형**: DD 53.7%, DR REQREP 10.9%·RR REQREP 11.2%(latency 14.5~15.6x; 64~4096 B는 52~59%로 평탄). PUBSUB 65536 B는 138%이므로 수신 경로가 아니다. 정적 분석(`log/2026-09-08-java-reqrep-64k-analysis.ko.md`): 크기 분기는 코드가 아니라 **byte HWM**(1 MiB → 65536 B는 소켓당 16개)이고, REQREP client 루프에 **admission backpressure gate가 없어**(C `blocked_out` 없음, binding이 retained 제출을 알리지 않음) retained 대기가 latency에 계상된다. SENDSEND는 CAS gate가 있어 정상. JNI는 없다(FFM). POLLOUT gate는 **기각**(POLLOUT은 admission credit이 아닌 backpressure 회복 edge — `core/doc/spec/core/05-polling.ko.md:54-68`; 4096 B에서 0건, 65536 B에서 99%). 진단: 소켓당 미완료 최대 2 → 쌓임이 아니라 **turn당 17.5 ms 왕복 비용**(4096 B 0.61 ms). `snd_pending_bytes` 65,664(1 MiB 창의 6%)인데 POLLOUT 99% → 65536 B의 실효 admission 창을 byte HWM이 정하지 않는다 — Java request terminal(retained+WRITABLE 회복)과 Core admission의 상호작용, 다음 캠페인(0.17.4 뒤) 첫 조사 항목(`log/2026-09-08-java-reqrep-64k-gate.ko.md`) | PUBSUB 통과 93.2; DR SS 통과 73.2(3-run); RR SS 통과 84.9(4096 B teardown 간헐 실패는 `9fd9c08f24`로 해소); DD 72.0·DR REQREP 46.3·RR REQREP 47.6 미달(REQREP 둘 다 65536 B 10.9~11.2%) — tcp 6/6 완료. **Single에서 확인된 추가 단서**: 같은 창·같은 깊이(≈16)에서 65536 B+ Java REQREP이 42~45%인데 C++은 99% → Java 공통 송수신 경로의 대형 메시지 건당 비용(one-way 5 pattern도 65536 B+에서 C의 34~41%)이 Multi 64 KB 붕괴와 별개로 존재 |
...
| Go | — | REQREP 상한 제거·C turn 구조(`add837942b`, 0.17.2 동시 multipart로 재개); relay는 이미 동기 제출; 단방향 drain·echo drain·probe 단일 제출(C turn 모델, 커밋) | **지도(alpha, DD 64 B, `log/2026-09-08-go-dd-cost-map.ko.md`)**: Go 2,593 ns/msg vs C 590 — cgo 경계 23회/op 847 ns(격차 42%, native message ownership·retry snapshot·received wrapper는 계약), goroutine/channel handoff 2회 315 ns(16%), heap 880 B/17회(9.5%), 나머지 32%. 내부 경계 23→19 병합은 microbench -9%였지만 e2e DD가 386→149 K로 회귀(원인 미확인)해 원복; goroutine 대체(장기 worker·worker 재사용)도 latency 1.5 s·DR SS 종료 실패로 기각. REQREP 작은 크기 latency ~3x(Node도 동일) → async terminal 왕복 후보 | PUBSUB 39.8·DD 31.5·DR SS 50.1(5-run 경계, 목표 53)·DR REQREP 67.7·RR REQREP 67.1 미달; RR SS 65536 B `차단`(server shutdown). DD 64 B 14%·latency 7x = turn 구조의 제출당 goroutine 비용 후보 |
...
| Python | — | relay 결합(`fb3f37191d`) | **지도(부분, Single DD, `log/2026-09-08-python-cost-map.ko.md`)**: C 571 ns/msg vs Python 5,325(10.7%) — 메시지당 Python 함수 호출 19.3회, Python→C 확장 경계 5.0회; ns 귀속(CPU·alloc)은 0.17.3 bump로 중단. Single DR REQREP 러너는 측정 구간에 `asyncio.sleep(0)`+event loop → §1.1 위반(러너 정합 항목, 진행 중). 기존: 전 pattern 14~24%로 **평탄**(크기·pattern 무관한 고정 per-message 비용 = 인터프리터·GIL·asyncio 경계) — 세부 지도 전에 "메시지당 Python 함수 호출 수"를 먼저 셀 것(pass 2에서 7→? 이미 줄였음). 65536 B만 36~52%로 C 대비 상대 개선 | DD 15.9·PUBSUB 34.0·DR SS 24.2·RR SS 23.3·DR REQREP 14.2·RR REQREP 15.7 미달(목표 60) — tcp 6/6 완료 |
...
- **STREAM(Core 0.17.3, D-BP37)** 7언어 × tcp/ws/wss/tls 28 cell 완료: C++ 96.7/97.6/91.3/97.2, .NET 95.8/93.2/96.2/93.1, Java 103.4/97.6/99.5/98.3, Go 69.5/68.6/77.0/70.7 통과(경계는 3-run 확인 중); **Rust는 20.4%→pass 1(1 ms park_timeout→Poller 대기, `044079fc68`) 뒤 97.6/104.9/106.7/98.2 통과**; Node 35.1/39.6/50.6/41.8, Python 19.1/22.4/33.3/22.6 미달 — 둘 다 tcp 다른 pattern과 같은 고정 비용 성격.
- **Single REQREP 러너는 전 언어가 깊이 1의 RTT 루프였다**(D-BP40): 공개 awaitable terminal이 admission을 안 돌려줘 러너가 1건 제출 뒤 completion을 기다렸다. Core admission 창(applied SNDHWM bytes ÷ wire size)까지 연속 제출로 바꾼 결과 — C++ 47~56→83~87%, .NET 31~34→53~59%, Java 22~26→44~46%(Node·Rust·Go·Python 검증 중). 남은 격차는 각 언어 Multi REQREP과 같은 request 경로 고정 비용. 64 KB 이상 latency 4~19x는 창 모델 한계(C는 그 크기에서 backpressure 없이 깊이 ~2).
