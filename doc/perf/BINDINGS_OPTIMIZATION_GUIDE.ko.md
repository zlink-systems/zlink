...
| Java | — | relay는 이미 `submit_sync()`(C 모델) | 미작성. 65536 B만 무너지는 **크기 의존형**: DD 53.7%, RR REQREP 11.2%(latency 15.6x; 64~4096 B는 53~59%로 평탄). PUBSUB 65536 B는 138%이므로 수신 경로가 아니라 **client 송신 쪽 64 KB 경로**(JNI 복사·direct buffer 재할당 후보) — Java pass 1의 첫 지도 대상 | PUBSUB 통과 93.2; DD 72.0·DR SS 78.7·RR REQREP 47.6 미달; DR REQREP·RR SS 측정 중 |
...
| Rust | — | relay 결합(`fb3f37191d`) | 미작성. **4096 B만** latency가 초 단위(DR SS 2,226x·RR SS 1,154x; 64~1024 B는 0.6~1.8x, 65536 B 1.6~5x) → 크기 하나에 걸린 client 대기 구조 후보(위 교차 현상). 65536 B 처리량 21~38%도 별도 | PUBSUB 89.0(목표 95, 경계 아님)·DR SS 73.5·RR SS 63.8·DR REQREP 54.3·RR REQREP 55.1 미달(REQREP은 0.17.0 3-run 62~72%보다 낮음 — C 기준 상승분인지 확인); DD 측정 중 |
| Python | — | relay 결합(`fb3f37191d`) | 미작성. 전 pattern 14~24%로 **평탄**(크기·pattern 무관한 고정 per-message 비용 = 인터프리터·GIL·asyncio 경계) — 세부 지도 전에 "메시지당 Python 함수 호출 수"를 먼저 셀 것(pass 2에서 7→? 이미 줄였음). 65536 B만 36~52%로 C 대비 상대 개선 | DD 15.9·DR SS 24.2·RR SS 23.3·DR REQREP 14.2·RR REQREP 15.7 미달(목표 60); PUBSUB C 기준 대기 |
