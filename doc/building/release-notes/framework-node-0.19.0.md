[English](./framework-node-0.19.0.md) | [한국어](./framework-node-0.19.0.ko.md)

# ZLink Node.js Framework 0.19.0 Release Notes

Framework 0.19.0 uses binding 1.2.1 and Core 1.2.0. Each Framework language release is versioned independently.

## Contract Changes

- HTTP client (Node): the typed-response and callback terminator changes from `async<T>()` to **`submit<T>()`**, the same name the Node framework messaging builders use. `submitRaw()`, `fetch<T>()` and `download()` are unchanged. (#707)
...
```bash
npm install @zlink-systems/framework@0.19.0 @zlink-systems/http-client@0.19.0
```

The release tag is [`framework-node/v0.19.0`](https://github.com/zlink-systems/zlink/releases/tag/framework-node%2Fv0.19.0).
