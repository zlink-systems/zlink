...
# ZLink C++ binding 1.10.0 릴리스 노트

이 릴리스는 C++ binding 패키지 버전을 1.10.0으로 올리고 Core 1.10.0을 요구합니다.
...
- ROUTER의 선택 route를 `routes_snapshot()`, `pollroute`, `route_generation()`로 관찰할 수 있습니다. 수신 메시지의 route generation은 snapshot의 값과 동등성만 비교하는 token입니다.
- Core 1.8 이후 공개 C 헤더의 상수와 선언을 C++ binding의 raw header에 반영했습니다.

## 검증

- 릴리스 사전 검사는 C++ binding 버전, 릴리스 노트, Conan·vcpkg 메타데이터를 확인합니다.

릴리스 태그는 `cpp/v1.10.0`입니다.
