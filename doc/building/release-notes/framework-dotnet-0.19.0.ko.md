...
# ZLink .NET Framework 0.19.0 릴리스 노트

Framework 0.19.0은 binding 1.2.2과 Core 1.2.0을 사용합니다. Framework 언어별 릴리스는 독립적으로 버전이 지정됩니다.

## 계약 변경

- HTTP client(.NET): server builder의 one-way `ValueTask Async(CancellationToken)`(non-generic)이 없어집니다. `Async<T>`·`AsyncRaw`·`Fetch<T>`·`DownloadAsync`·`Yield<T>`와 callback overload는 그대로입니다. (#707)
...
릴리스 태그는 [`framework-dotnet/v0.19.0`](https://github.com/zlink-systems/zlink/releases/tag/framework-dotnet%2Fv0.19.0)입니다.
