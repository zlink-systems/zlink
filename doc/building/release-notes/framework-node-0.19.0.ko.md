...
# ZLink Node.js Framework 0.19.0 릴리스 노트

Framework 0.19.0은 binding 1.2.1과 Core 1.2.0을 사용합니다. Framework 언어별 릴리스는 독립적으로 버전이 지정됩니다.

## 계약 변경

- HTTP client(Node): typed 응답·callback 종결자가 `async<T>()`에서 **`submit<T>()`**로 바뀝니다(Node framework의 messaging 관용과 같은 이름). `submitRaw()`·`fetch<T>()`·`download()`는 그대로입니다. (#707)
...
릴리스 태그는 [`framework-node/v0.19.0`](https://github.com/zlink-systems/zlink/releases/tag/framework-node%2Fv0.19.0)입니다.
