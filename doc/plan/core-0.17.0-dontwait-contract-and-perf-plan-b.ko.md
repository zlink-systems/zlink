...
- [x] bindings 성능 측정·개선 — §6 실행 계획의 tcp Multi 4 pattern 판정을 7개 언어 모두 확정(09-06 00:22, Core `a40cb46335` quiet 3-run; 계획서 §9 표·log·§11): C++ DD 통과(완화 90)·나머지 보류, .NET 보류(59.6/58.3/67.0/61.3), Java 미달(80.9/59.4/58.7/80.9; 최소 충족), Node 보류(35.9/24.3/24.8/30.2), Go 미달(53.2/19.0/21.8/54.6), Rust 보류(59.3/67.3/69.8/87.7), Python 보류(15.4/15.5/19.6/31.9). pass 15건 push(C++ 2, .NET 3, Java 3, Node 4, Go 2, Rust 2, Python 2) + Core 공통 원인 수정(64 KiB REQUEST 토큰 busy loop `a40cb46335`). **미완(사용자 결정 의존)**: D-B127 — binding 러너의 포화 제출 경계(공개 `POLLOUT` level 권고)와 C single 러너 latency 단계 정합 → 결정 뒤 Single suite·Go/Java Multi REQREP 러너 재구성·재판정; tls/ws/wss·Single 표는 C++만 측정. 다음 개선 후보(공개 API 추가 필요, spec 결정): Node 서버 native 배치 recv+reply, Python GIL 밖 배치 API, .NET submit 무락 구조.
