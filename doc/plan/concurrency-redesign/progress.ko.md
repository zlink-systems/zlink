...
| **마일스톤 게이트** (CP3) | **대부분 완료** — unit 1900/0·6샘플·cross-language e2e 그린, 스냅샷 실질 0, sol 감사·스펙 06 개정(`078d1e22b6`) 완료. §8 판정: Z2 부분 하위증상 입증(0/3→3/5) | **수정·게이트 통과** — 결함 6건 전건 [실증] 수정 + focused 테스트 6건. cleanTest 강제 실측: core 1149/0 · kotlin 67/0 · connector 123/0 · redis 27/0, **실패 0**. 되돌림 검증(테스트가 결함을 실제로 잡는가)은 확인 대기 | 감사 보고 후 판정 (스위트 재실행 T5와 합산) | 감사 보고 후 판정 (스위트는 그린 — T2) |
...
| T11 | **cpp 전환 재개** (2026-08-27 세션) | 진행 중 | 소형 배치 A·B·C·D + channel_runtime = **상태 보호 138취득 → 0**. 커밋 `7acc3dcb6c`(배치 101). **별도 worktree 이분으로 배치분 무결 증명**: base→HEAD 6단계 누적 × 각 3런 = 18런 전부 `test_cpp_framework_execution` 통과. 대형 3건(`public_host_runtime` 99·`spot_runtime` 152)은 1차에서 "C2 aggregate라 통째로 옮겨야 하는데 경계 재설계 필요"로 STOP했으나 **오답으로 판정** — dotnet #9 `ZLinkManagedMeshNode`(130취득)가 **한 클래스 안 lane 5개 분리 + 객체별 lane + 발견7 gate 유지**로 완료한 전례가 있고(`b3497532a6`), cpp `channel_runtime`(37, 같은 C2 aggregate 모양)도 STOP 없이 전환됐다. design §3의 "경계를 새로 긋지 않는다"는 클래스 분할 금지이지 다중 lane 금지가 아니다. 전례 명시해 재발행 |
