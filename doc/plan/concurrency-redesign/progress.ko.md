...
| **Z0** cross-language e2e (샘플 작업 선행) | **완료 (2026-08-27 재검증)** — dotnet 무관 **10스테이지 전부 rc=0**: `spot-route`·`message-follow`·`relocation`·`java-cross` + `user-spot-join` 6쌍(cpp↔java·cpp↔node·java↔node 양방향). **java mavenLocal 발행을 선행**해 이번 세션 jvm 수정 6건이 반영된 jar로 검증했다(미선행 시 2026-08-25자 낡은 jar로 통과 판정될 뻔했다). `MSBUILDDISABLENODEREUSE` 미설정으로 MSBuild 노드가 언어 락 3개를 9분 붙든 사고 1회 — rules §4 등재 | ← | ← | ← |
...
| T5 | 언어별 샘플 스위트 (ZoneWorld 제외) | **완료 — 신규 실패 0** | **node 6/6 통과** · **cpp 5/6**(Bingo 1건) · **java placement 6/6**. 실패 2건 모두 이번 세션 무관으로 확정: ①cpp Bingo — 전환 전(11:41, 소형 배치 커밋 전)과 후(18:28, 250여 취득 완료)가 **같은 step `wait observer Entry Spot return`에서 정지**. 오늘 4/4 실패로 종전 "~1/5 flake" 기록보다 나빠 **결정적 실패로 재분류**(rules §4). ②java — 전 샘플이 `placement=completed` 뒤 러너의 **금지 패턴 정적 검사**에서 rc=1. 위반 파일 최근 커밋이 브랜치 base `3cbfbde4f9` 자체이고 이번 세션은 java samples를 **diff 0**으로 건드리지 않았다. cpp `layout_contract`와 같은 계열의 기존 지문 드리프트. rules §3 게이트 기준은 "placement marker 유지"이므로 java는 통과 판정 |
| T6 | Z4 — ZoneWorld 측정·판정 | **이번 캠페인 제외(2026-08-27 사용자 지시)** | 사용자 지시: "zoneworld는 제외하고 작업 완료되면 알려줘". dotnet Z4는 애초에 다른 세션 소관이고, cpp ZoneWorld ×8 측정도 이번 세션 범위에서 뺀다. 앞선 지시 "ZoneWorld는 맨 마지막"이 "제외"로 갱신됐다. 기존 기록 보존: dotnet 조용한 ×8 = pass 1/fail 7, 모드 매런 상이(ZW-B8×2·D1×2·B4·G4·C2), "부하 오염" 가설 기각, 기준선 worktree `/home/hep7/project/zlink-z4base` 준비됨(다른 세션이 사용 중 — **건드리지 말 것**) |
