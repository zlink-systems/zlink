...
§1 매트릭스 셀과 §4 추적표를 항목 단위로 직접 집계한 값이다. 부분 완료는 0.5로 계산했다.

| 영역 | 완료/전체 | 진행률 | 남은 것 |
|---|---:|---:|---|
| 본체 전환 (L0·L1·L2조사·L2전환 — 4언어 16셀) | 16/16 | **100%** | 없음 |
| 마무리·CP3 판정 (호환 경계 3 + posddd 4 + CP3 4 = 11셀) | 4/11 | **36%** | cpp·jvm 감사 재기동, node 의심 5건 검증, 언어별 CP3 판정 |
| Z0 + 샘플 검증 (1 + 6샘플×4언어 + ZW 4 = 29셀) | 13/29 | **45%** | cpp·java 스위트 재실행(T5), ZoneWorld Z4 이분 판정(T6) |
| 꼬리 추적 T1~T10 (10건) | 6/10 | **60%** | T4·T5·T6·T9·T10 |
| **항목 수 기준 합계** | **39/66** | **약 59%** | |
