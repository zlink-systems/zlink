...
| **L2** 클래스 전환 (§2 상세) | **진행 중** — 대형 9 · 소형 36/99 (+제외 2) | **진행 중** — 순서 1~6 완료 (8클래스) | **진행 중** — 순서 1~6 완료 (6클래스, kotlin 승계) | **진행 중** — 순서 1~8 완료 (발견8 다수) |
...
| — | 소형 배치 10~22 (나머지) | 대기 | | 배치 21·22 개방됨(#9 완료) |
...
| 지표 | 시작 | 현재 | 목표 |
|---|---:|---:|---|
| dotnet `lock` 문 | 999 | **442** (−56%) | ~60 (lane·큐 내부만) |
| `_gate` 보유 클래스 | 61 | **29** | 0 |
| state lane 사용 파일 | 0 | **35** | — |
| **async 경계 스냅샷** *(진짜 지표)* | ~465 | 미측정 | **0** |
| 호환 경계 `AwaitStateLane` *(부채)* | 0 | **301** | 0 (마무리에서 회수) |
