...
| **L2** 클래스 전환 (§2 상세) | **진행 중** — 대형 9 · 소형 19/99 (+제외 2) | **진행 중** — 순서 1~4 완료 (6클래스) | **진행 중** — 순서 1~4 완료 (4클래스, kotlin 승계) | **진행 중** — 순서 1~6 완료 (발견8 판정 포함) |
...
| 지표 | 시작 | 현재 | 목표 |
|---|---:|---:|---|
| dotnet `lock` 문 | 999 | **517** (−48%) | ~60 (lane·큐 내부만) |
| `_gate` 보유 클래스 | 61 | **42** | 0 |
| state lane 사용 파일 | 0 | **21** | — |
| **async 경계 스냅샷** *(진짜 지표)* | ~465 | 미측정 | **0** |
| 호환 경계 `AwaitStateLane` *(부채)* | 0 | **215** | 0 (마무리에서 회수) |
