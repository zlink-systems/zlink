...
언어별 현황은 **§4 언어별 진행표가 단일 진실 원천**이다 (여기 중복 요약표는 2026-08-26
사용자 지적으로 제거 — 이중 갱신이 표를 거짓말하게 만든다).
...
| 8 | ZLinkActorRuntimeState | 35 | **요청됨** | | codex terra 실행 중 (2026-08-26). semaphore + 재진입 ~20 |
...
- **샘플은 7개 일괄이 아니라 하나씩**: 감독관 agent(Claude)가 샘플 1개 단위로 작업을
  요청하고, 완료되면 **감독관이 리뷰**한 뒤 다음 샘플로 넘어간다. (codex에 7개를 한 번에
  맡기지 않는다.)
