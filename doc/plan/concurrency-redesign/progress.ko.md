...
| T15 | **`public_host_runtime` 잔여 36 = 누락 그룹** | g4 예약 | g1~g3 종료 후 잔여 36의 실제 소유 멤버를 중앙 실측: `_relocation_target_attempts` 28 · `_session_seal_terminals` 15 · `_user_spot_terminals` 12 · `_relocation_assemblies` 9 · `_routes` 7. **원래 3그룹 분석이 relocation·session terminal 계열을 빠뜨렸다.** (`_spots`·`_actors`는 lock_guard 아래 0회 — g1은 정상 수행 확인). g3 보고서가 잔여를 "local dispatch/completion, Spot/Actor index"라고 적었으나 부정확 — **에이전트 보고의 그룹 이름을 믿지 말고 중앙에서 멤버를 직접 세라** |
