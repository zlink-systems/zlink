...
### 9.0 사용자 목표 — "lock 최대한 제거 + 동일 메커니즘 스펙 고정" (2026-08-27)

목표는 **고성능 실시간 메시징이므로 lock을 최대한 없애고, 같은 프레임워크인 만큼 동일한
동시성 메커니즘을 스펙으로 고정한다**이다. 실측에 근거한 판단은 다음과 같다.

**(1) lock 제거는 이미 한계에 닿았다.** 4언어의 잔여는 성능이 아니라 정합성 때문에 남았다.

| 잔여 유형 | 예 | 제거 불가 사유 |
|---|---|---|
| lane primitive 자체 | cpp `serial_execution_queue` 23 | lane이 lock으로 구현된다 |
| socket 수명 gate | cpp `stream_host_service` 68 · `raw_mesh_node_owner` 49 | 외부 await를 품는다. lane turn에 넣으면 그 lane이 막힌다 |
| dispose/close 프로토콜 | dotnet `_socketGate`·`_disposeGate` | 발견 7 — 상태 보호가 아니라 작업 직렬화 |

여기서 더 줄이면 **성능이 나빠진다.** lane은 큐잉이라 turn 전환 비용이 있고, socket I/O 같은
긴 작업을 lane에 넣으면 그동안 다른 상태 변경이 전부 막힌다.

**(2) 진짜 성능 부채는 lock이 아니라 호환 경계다.** 전환이 만든 블로킹 브리지가
**dotnet `AwaitStateLane` 664곳 · cpp `.get()` 456곳**이고 **전환으로 오히려 늘었다**
(cpp `location_lifecycle` 한 파일에서만 13개 신설). 무경합 `lock`은 수십 나노초인데
브리지는 **turn 큐잉 + 스레드 대기**다. 고성능 실시간이라면 이쪽이 더 아프다.
**따라서 다음 성능 작업은 "lock 더 줄이기"가 아니라 "핫패스 브리지 회수"다.**
handoff §6의 "lock 개수는 성공 지표가 아니다"가 여기에도 적용된다.

**(3) 스펙 고정은 찬성하되 대상은 "메커니즘"이 아니라 "소유 규칙"이어야 한다.**
4언어 메커니즘은 이미 다르다. 다만 **통일 불가인 것은 node 하나뿐**이다(단일 스레드 — JS turn이
이미 원자적이라 넣을 메커니즘이 없다). **dotnet·cpp·java 셋은 통일 가능하며, 셋 다 상위 직렬
소유를 구현할 재료(`ZLinkSerialExecutionQueue`·`serial_execution_queue`·`ZLinkAsyncSerialQueue`)를
이미 갖고 있다.** 차이는 언어 제약이 아니라 설계 선택이다(§9.1 정정 참조).
통일하면 브리지 664+456이 사라지므로 (2)의 성능 부채가 근본에서 해소된다 — 대신 진입 경로
재설계 비용을 치른다. 그와 별개로 다음은 4언어 공통으로 고정 가능하고 이미 작동했다:

1. **상태 분류 규칙**(C1/C2/C3) — 스펙 06에 있고 4언어 전환에서 유효했다
2. **금지 형태** — await 경계를 넘는 스냅샷, C2를 원자 자료구조로 쪼개기
3. **소유 단위** — 메커니즘은 달라도 **소유 경계는 같아야** 한다
4. **진입 소유 형태** — 지금 갈려 있다. §9.2의 판정 대상이다

**권고 순서**: ①스펙 06을 소유 규칙·금지 형태 중심으로 확장하고 언어별 구현은 매핑표로 붙인다
②진입 소유 형태(§9.2)를 판정한다 — 안 정하면 "동일 메커니즘"은 선언만 남는다
③성능은 lock 수가 아니라 **브리지 핫패스 실측**으로 다룬다.

### 9.1 실제 쟁점은 동기화 메커니즘이 아니라 **진입 아키텍처**다

cpp `spot_runtime`을 java 방식(클래스 내부 동기화 0)으로 갈 수 있는지 실측했다.
**현재 설계로는 안 되지만 언어 제약은 아니다** (2026-08-27 정정 — 처음에 "불가능"으로 적었던 것은
근거가 약했다).
...
**★ 정정 (2026-08-27): 이 차이는 언어 제약이 아니라 설계 선택이다.**
세 언어 모두 상위 직렬 소유를 구현할 재료를 **이미 갖고 있다**:

| 언어 | 직렬 실행기 |
|---|---|
| java | `ZLinkAsyncSerialQueue` |
| dotnet | `ZLinkSerialExecutionQueue`(1,118줄) + `ZLinkSpotSerialExecutor` |
| cpp | `runtime/execution/serial_execution_queue`(23취득) |

java만 그 재료로 Spot 진입을 하나로 모았고 dotnet·cpp는 안 모았을 뿐이다. cpp의 진입 217곳도
"cpp라서"가 아니라 그렇게 설계했기 때문이며, 재설계하면 줄일 수 있다.
**따라서 dotnet·cpp·java 셋은 통일 가능하다 — 비용이 클 뿐이다.**

**정말 언어 제약인 것은 node 하나뿐이다.** 단일 스레드라 lock 개념 자체가 없고 JS turn이 이미
원자적이다. "동일 메커니즘"에 넣을 것이 없다.

**통일의 실제 손익** (이게 판단의 핵심이다):

| 항목 | 영향 |
|---|---|
| 진입 경로 재설계 | cpp Spot 217곳 · dotnet 유사 규모 — **비용** |
| 호환 경계 | **dotnet 664 · cpp 456이 사라진다** — 상위가 직렬 소유하면 브리지가 불필요 |
| 성능 | turn 큐잉·스레드 대기 제거 → **고성능 실시간 목표에 직접 부합** |
| 안전성 | lane 자기 강제가 사라져 **소유권 단언이 필수**(없으면 조용히 깨진다) |

즉 통일은 단순한 일관성 문제가 아니라 **§9.0이 지목한 성능 부채 664+456을 근본에서 없애는
방법**이다. 통일하면 "핫패스 브리지 회수" 작업이 통째로 사라진다.
**판단해야 할 것은 가능 여부가 아니라 재설계 비용을 감수할지다.**

### 9.2 통일 안건이 실제로 결정해야 하는 것

1. **핵심 컴포넌트(Spot·Actor·Session)의 진입 소유를 어느 형태로 통일할 것인가**
   - (A) 상위 단일 직렬 소유(java 형) — 내부 동기화 0, **브리지 0**(dotnet 664·cpp 456 소멸).
     대신 진입 경로를 하나로 재설계해야 하고, 자기 강제가 없어 **소유권 단언**이 필수다.
     **세 언어 모두 직렬 실행기를 이미 갖고 있어 재료는 있다.**
...
**(B)를 유지한다.** 단 이유는 "(A)가 불가능해서"가 아니라 **이번 캠페인이 동작 보존 전환이고
