...
4언어 메커니즘은 이미 다르고, 그중 일부는 **구조적으로 통일 불가**다 —
cpp는 진입 파일 8개·콜백/타이머 진입 217곳이라 java 방식(내부 동기화 0)이 성립하지 않고,
node는 애초에 lock 개념이 없다(JS turn 원자성). 대신 다음은 4언어 공통으로 고정 가능하고
이미 작동했다:
