...
- [ ] 진행 중(17:52) job 7: Core D-094(admission 특례 제거, worktree b), Core STREAM disconnect_rid DISCONNECTED(worktree a), node DD out-of-sequence, node monotonic readiness cap 구현(D-095), java DD courier-session teardown(+종료 원인 로그), java mesh transport identity, dotnet monotonic durations(D-095). 이전 job 5 중 완료: java M6A → D-094 결정
