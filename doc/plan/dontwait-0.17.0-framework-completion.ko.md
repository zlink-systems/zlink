...
- [ ] 진행 중: Core 수정(worktree b, Claude agent) — attach/terminate 대칭 단일 불변식(dist-attached pipe만 dist-terminated), 다른 socket type 동일 점검, pub_churn 회귀 + ASan 검증 → 패키지 재빌드 → cpp unit ×N + 4언어 pub/sub 경로 재확인
