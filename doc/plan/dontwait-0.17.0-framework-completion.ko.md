...
- [ ] 진행 중(15:05): **Core 회귀** — java `:zlink-framework-core:test`가 새 Core에서 `Assertion failed: written (pipe.cpp:210)`(`send_routing_id`가 종료된 pipe에 write; inproc connect 경로 `socket_base_endpoint.cpp:406` 의심, D-090/D-B112 조합) → Claude agent(worktree b; codex는 content filter로 중단); dotnet ShoppingMall B4(relocation root 대조) + dotnet 최종 gate job(astra); node gate6(aggregate + npm test, 새 Core)
- [ ] 이후: rebuild7(Core `ccb418b6ee` + send_routing_id 수정 + B node binding D-B113) → dotnet gate v10 재확인 → cpp 7샘플·aggregate → java core test·7샘플 → cross-language E2E → §F 마감·push
