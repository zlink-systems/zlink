...
| ZoneWorld (cpp·jvm·node·dotnet, codex sol ultra) | **전 언어 종결** — jvm `ca305b4236`+`3b03935b72`(ZW 4/5+×2 rc=0) · cpp `5f4a1ec76d`+`892eef4b4d`(multicast fail-fast·Join fence + source OnLeaveActor 유실 3중 원인: 조기 leave no-op 소실→hold/replay, transferred reservation 이중 capacity 판정 제거, canonical seal의 membership Spot 덮어쓰기 제거 — **감독관 ×6=6/6**, 게이트 45/45) · dotnet `526f6ae08a`(A3·B7 binding lane 단일화 + G3 병렬 relocation READY 유실 _socketGate 직렬화·직접전송 표식 오인 — ZW 3/3, 게이트 1918/1919+flake 단독통과) · node `a8c5b115ee`(E3 Deferred Join 자기 교착 해소 + 두 선형화점 구현; 작업 중 회귀 B8·B2는 r1의 스펙 외 과잉 검증(합성 lease·ownsLocation)과 target publish의 raw registry 미복원이 근원 — 제거·복원 선행으로 수렴, **ZW 6/6**, 게이트 그린) · 스펙 §5 상세화 `f51ed3757a`(bind 승인=3종 값만). 교훈: 6라운드 중 4라운드가 "unit 그린·샘플 레드" — 샘플 직접 실행 전략 전환 후 즉시 수렴 |
| ZoneWorld 완료 후 마감 (2026-08-29 사용자 지정) | **완료** — 커밋·push·tag `seven-samples-green-v1`(=4bad5ac979) 완료, vite 좀비 정리, 메모리 기준선 갱신. 이월: **cpp bind lease 비교 제거 수렴**(스펙 §5 정합화 — 별도 라운드, ZW 재실증 동반) · cpp channel_messaging flake(관찰 지속) · **dotnet ZW 간헐**(D1·G3-restart·F4 산발 ~1/3 — 0.14.1/0.14.2 교차서 버전 무관 확인, 증거 zw-evidence/v0142-regression) |
| **core/bindings 0.14.2 감사** (2026-08-29 사용자 지시) | **완료 — 이슈 없음** — 로컬 패키지 빌드(cpp·dotnet·java·node) 후 4언어 게이트 전부 그린(cpp는 0.14.2 재구성 포함), ZW: cpp·node 스모크 그린, java 3/3(초기 0/N은 감독관의 상대경로 호출 실수 — child lane `$0` 재해석 실패로 판명·무효), dotnet 4/6은 0.14.1 교차(2/3)와 동률로 버전 무관 간헐 판정 |
| **남은 lock/동기 경계 정리** (2026-08-29 사용자 지정, ZoneWorld 마감 후) | **조사 완료** — 전수 보고서 `doc/plan/lock-boundary-survey.ko.md` (재계수: dotnet 제품 550콜사이트·핫패스 19~20회/원격msg, cpp effective 1,060·핫패스 17/19 — 종전 664·456은 스코프 착오로 판정, 회귀 아님; java wrapper 30곳 직접 제거 0·[의심]2; dotnet C2 `_barrierGate` 16곳은 이미 이관 완료로 계획 :102가 코드와 불일치 → H급 재설계 대상 4건 재목록화). 회수 우선순위 12건 표 작성. **1차 라운드 완료(사용자 승인)**: cpp 우선순위1 `3e7284f2a2`(phase projection 통합, send 17→10/request 19→12, 게이트 45/45·ZW 6/6) · dotnet 우선순위12 `2cd884287e`(rollback 중첩 bridge 2곳 회수, 1919/1919·ZW 5/5). **우선순위3(dotnet registry 3→1)은 미수행 판정** — 두 번째 lookup이 replacement 감지의 실제 의미, 제거엔 새 fence 필요(새 제어점 금지 충돌) → 후속 안건. **캠페인 완주(2026-08-30)** — 2차 java `c67b1d8fe2`(우선순위 11·5·6: owner 계약 테스트 선행 후 hot wrapper 7→3 회수, fanout close race 등 실결손 2건 보수, ZW java 5/5·kotlin 2/2) · 3차 dotnet `a2b0334b31`(8·2·7: **19/20→14/15**, P2는 교체감지 보존 위해 -3, P7 [의심-H]는 단일 admission owner 입증으로 해소, 1920/1920·ZW 5/5) · 4차 cpp `903c09bdc6`(unified admission token: **send 10→7 / request 12→7**, Yield lease 승계·capacity/token rollback 회귀, 기존 잠재결함 2건 수정 — 게이트 45/45·ZW 6/6. send 5~6 대비 +1은 TOCTOU correctness 보수치). **핫패스 총계: cpp 17/19→7/7, dotnet 19/20→14/15, java hot wrapper 7→3.** |
| **과잉 검증·TOCTOU 후속** (2026-08-30) | ① cpp bind lease 수렴 완료 `a0f48c9941`(4언어 §5 정합 종결) ② **TOCTOU 판정 완결** `doc/plan/relocation-toctou-verdict.ko.md` — B-2 실존: **cpp 잠재 결함**(barrier 뒤 FIFO 진입 메시지가 capture 3종 어디에도 안 잡혀 옛 owner에서 조용히 실행 — membership :903 위반, 수정=coordinator admission↔FIFO admission 원자 fence) + **java 잠재 결함**(post-cut queue 거부를 호출자가 묵살) · dotnet(재투영)·node(동기 선형화)는 안전 ③ dotnet registry 재조회는 08-routing §2.6 :316 위반 확정(제거 대상, -2) ④ GameQuest 판정 확정(사용자): **RecreateOnRelocation 정본** — 문서+cpp+node 수정 대기 ⑤ 이후 전 항목 실행 완료 — 실행 단위 백로그와 종결 기록은 `work-backlog.ko.md`가 소유(핫패스 최종: cpp 6/6·dotnet 12/13·java wrapper 3, 과잉 검증 ~49곳 제거, B-2 결함 2건 수정, java MF 수렴, GameQuest 통일, dual 표면 8개). 작업자: codex sol medium + sonnet/opus 병용 |
