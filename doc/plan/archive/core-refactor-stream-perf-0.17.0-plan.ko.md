...
| **0.17.4 착지(`eb6abfb995`) pull 4 스택 1 run**(D-B298, MERGE-3, CCU 1000, idle; asio·cppserver = pull 변형, D-B275) | zlink/asio **0.931/1.298/2.486**, zlink/cppserver **0.843/1.071/1.005**, zlink/zmq 0.724/0.964/–(zmq 64 KiB run_failed) | 212.8 / 228.5 | 251.1 / 193.4 | **42.7** / 17.2 | SD-2 유효 창(3-run, base 0.17.3): zlink 64 KiB 32.3 → 40.3(B2, +25 %), p99 −24 %; RSS +141 MiB | 0.10 시작 |
...
| D-S1 | S-D(sol, 09-08) · SD-2 측정 | `rcvbuf=-1`일 때 STREAM decoder read target max(현재 4 KiB)를 OS default socket buffer 크기까지 올릴지 — CCU별 user-space buffer 메모리 상한 정책이 먼저 필요 | 64 KiB 처리량/latency vs connection당 RSS | **대기** — D-B267·D-B291: B2 채택으로 64 KiB CCU 1000 서버 RSS 279 → 423 MiB(+141 MiB, 연결당 1 MiB 상한의 14 %); max 상향 여부는 이 자료로 결정 |
...
- **STREAM 판정 스택(D-B274·275, 09-08)**: pull 모델만 — **zlink · asio · cppserver · zmq**(asio·cppserver는 pull 변형, 표기에서 `_pull` 생략). 목표 cppserver 동률. callback 본체 스택은 표에서 제외(저장소 유지, 기본 실행 목록 제외).
