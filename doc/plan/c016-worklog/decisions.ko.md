...
## D-B231 (2026-09-08 12:30, 머신 B) ALL-1 결과 — A~E 완료(B-4 부분), TSan 전체 경고 0, lock/msg 8.39; ALL-2(rebase + D-ALL-1) → 리뷰·게이트
...
## D-B233 (2026-09-08 13:20, 머신 B) 사용자 결정 — 0.17.4 작업은 Windows 세션에서 진행; WSL 세션은 0.17.3 릴리스까지
...
묶여 있다")는 **비용 지도가 있는 C++(D-BP26)·.NET(D-BP31)에만** 성립한다. Node(25~43%)·Go(31~68%)·
Rust(54~89%)·Python(14~34%)·Java REQREP(46~48%)는 지도 자체가 없다(가이드 §3.1 "미작성"). 따라서
캠페인은 계속한다: 우선순위 언어 Java·Node·Go 비용 지도 job 3건(codex sol/high, "지배 항목 ≥20%일 때만
pass 1 구현, 아니면 지도만 보고")을 12:55에 띄웠고, Rust·Python은 그 뒤에 잇는다.

**공개 API 변경은 제안하지 않는다.** 사용자 확인("예전에 바꿔봐도 별 차이 없던데"): .NET 지도의 두 항목
(메시지당 send 빌더 177.7 ns·P/Invoke 전환 137.5 ns)은 사용 형태에서 나오지만 무엇을 어떻게 바꾸면 얼마나
좋아지는지 설계·측정된 적이 없고, D-BP32의 전환 10→9 후보는 효과가 없었다. C++는 지배 항목이 없다. 두
언어는 "지도 완료, 현재 수준 유지"로 두고, 새 측정 근거가 생길 때만 다시 연다.
...
## D-B239 (2026-09-08 14:20, 머신 B) Actions run 34185949926 — Windows x64 통과(WIN-1 유효), Linux x64 `unittest_flow_state_monitor` timeout → LIN-1
...
## D-B245 (2026-09-08 14:15, 머신 B) review-all2 — 차단 1건(B-ALL2-1) + W 6건 → ALL-2b(sol, worktree all); ALL-3와 합쳐 0.17.4 최종 게이트
...
## D-B250 (2026-09-08 15:05, 머신 B) macOS Intel 릴리스 제거(사용자 결정); macOS ctest 실패가 job을 막지 않는 gap + macOS 실제 실패 → 0.17.4 MAC-1
...
**S-D 착수(16:15)**: cppserver의 write/read 경로가 asio 기본 서버·zlink engine과 어떻게 다른지(read buffer·write 병합·복사·할당·socket 옵션) 정적+동적(strace -c, callgrind) 대조, zlink가 계약 안에서 채택 가능한 항목과 예상 이득·job 크기. codex sol/high, 분석 전용, 2 h.

## D-B258 (2026-09-08 16:30, 머신 B) MAC-1 결과 — macOS `ipc://` bind 전면 실패(mkdtemp 탐지 결함) 수정, APPLE timeout 3배; 잔여 5건(wake 계열) → MAC-2

**MAC-1**(`core-rf-MAC-1-report.md`, Claude, Actions loop 브랜치 `wip/mac-1`): (1) `core/CMakeLists.txt:528` `check_cxx_symbol_exists(mkdtemp stdlib.h …)` — Darwin은 `unistd.h`에 선언 → "not found" → `ip.cpp:443` `mkstemp` 폴백이 일반 파일 생성 → `asio_ipc_listener.cpp:86`이 socket 아님으로 거부 → **macOS에서 모든 `ipc://` bind가 EADDRINUSE**(0.17.3 macOS 산출물 포함). Unity abort 뒤 socket 잔류로 `wait_for_reaper_done` 영구 대기가 timeout의 정체. 수정: 탐지 헤더 `"stdlib.h;unistd.h"`(Linux 불변). (2) macos-15 runner가 Linux 대비 ~3× 느림(단독 실행은 rc=0 PASS 확인) → `core/tests/scale_test_timeouts.cmake`로 APPLE에서만 TIMEOUT 3배. 결과 실패 9→5, 병렬 그룹 100 %, serial 833→716 s(run 34196588768). patch 31행(빌드 시스템만).
**잔여 5건**(단독 통과, 순차 실행에서 실패): `test_ctx_options:657` blocking send EAGAIN(peer drain 후 1 s 내 미기상), `test_xpub_nodrop:243` sent==recv 정지, `test_single_lane_wire_{mandatory_count,old_peer_rejected}` 잘못된 READY 뒤 3 s 내 close 안 됨, `test_wake_invariants:569` completion 1개 잔류 → Darwin wake/readiness(kqueue·signaler·SO_SNDBUF·close 순서) 계열 의심. **MAC-2**(Claude opus, 3.5 h) 착수. 0.17.4는 macOS gating이 켜져 있으므로 MAC-2 미해결분은 최후 수단으로 `ZLINK_CTEST_EXCLUDE_REGEX` 최소 목록을 사용자 결정으로.

## D-B259 (2026-09-08 16:45, 머신 B) ALL-2b/2c 완료 — review-all2 차단·비차단 전부 반영, TSan 전체 경고 0, hotpath 5/5, ws 게이트 PASS

**결과**(`core-rf-ALL-2b-report.md`, codex→Claude 인계): B-ALL2-1 max snapshot을 `pipe_write.cpp` record admission→part 쓰기→part별 HWM 재검사까지 하나로 전달; W-ALL2-1 command batch를 기존 `inbound_poll_rate`로 상한 + batch 내 `_ctx_terminated`; W-ALL2-2 inproc 종료를 turn 아래 `begin_inproc_pipe_termination`과 turn 밖 `finish_inproc_endpoint_termination`으로 분리; W-ALL2-3 WS batch/scratch 16/64 KiB 기본으로 되돌리고 기존 STREAM write-target 정책으로 128 KiB까지 성장·회수; W-ALL2-4 ws 게이트가 META 9필드(timestamp·load 등) 일치 검사; S-ALL2-2 주석 5건. Claude가 추가로 잡은 것: W-ALL2-2 리팩토링이 peer `set_nodelay()`를 `terminate()` 뒤·turn 밖으로 옮겨 `pipe_t::_delay` race(TSan, `test_inproc_unbind_disconnected_and_not_found`)를 만들었던 것 → 원래 순서로 복구. 검증: dev 210/210, 관련 70/70, 신규 5×20, TSan 전체 209/210 경고 0(잔여 1 = 기존 fixture), ASan 6/6, hotpath 5/5(3104.9/15635.5/2456.7/3042.0/13982.5, −3.9~+4.6 %), ws 게이트 Q64/Q1 ws 0.90·wss 1.46 PASS. patch `all-artifacts/ALL-2b.patch`(23 파일 +603/−88).
**잔여 위험**(보고서): `process_term_endpoint`의 pipe 벡터 empty assertion(네트워크 transport에만 session 존재 전제), 게이트가 같은 run의 report만 결합 가능, WS 연결당 메모리 128 KiB 상한(1,000쌍 RSS 미측정), ws Q 절대값 하락은 16 KiB 롤백+TCP 분모 차이(귀속 안 함).
**병합 순서(0.17.4)**: ALL-2(브랜치) + ALL-2b(patch) → ALL-3 patch → RR-1 → MAC-1/2 → 최종 게이트.

## D-B260 (2026-09-08 17:05, 머신 B) S-C 결과 — STREAM 9,041 vs asio 4,999 vs zmq 7,274 Ir/msg; asio 격차는 pull 핸드오프 층, zmq 격차 1,767은 receive 인계·pipe 회계·Asio handler 구성; 후속 job SC-1~4

**분해**(`core-rf-S-C-stream-ir-breakdown.md`, astra, CCU 20 축소 셀, 빈 서버 차감): 단계별 zlink/asio/zmq — engine read+reactor 2,468/2,678/792, 앱 recv 경계·pipe·command·wake 3,770/0/2,380, echo+send 제출 1,426/1,038/2,766, engine output+I/O 1,377/1,282/1,336. hotpath RAW 13,804. mutex 8.5/msg(asio 12.0, zmq 2.0). 잘못된 가설(packet당 msg_t 힙, 이중 버퍼, RID envelope, per-byte 파서)은 이 경로에서 실행 0. **A(설계)**: pull 핸드오프·activate_read·turn·byte HWM. **B(계약 유지 후보)**: B1 단일-part receive 중간 출력 표현 통합(250~450), B2 routed submit 전달 계층 통합, B3 Asio completion 표현·allocator 전달 통합(new/delete ~161), B4 output drain 판정 통합(prepare 2/msg·pipe read 3/msg) — 합산 CCU20 600~1,000, hotpath 700~1,200 Ir/msg 추정 → zlink/zmq 0.94~1.02(모델). **C(D 항목)**: thread-safety 해제, wake 지연 batching, borrowed payload — 제외. zmq 대조: epoll→raw read 직행(reactor 103 vs 805), 얇은 pipe write/flush, mailbox 삽입만.
**결정**: SC-1(receive 인계 통합)·SC-2(send admission 계층)·SC-3(completion 표현 진단)·SC-4(output drain 통합) 각 3 h를 **0.17.4 마감 뒤 "메시지당 명령 수" 캠페인**(0.17.5)의 job 목록으로 계획 §8에 등록. 사용자가 0.17.4에 넣기를 원하면 SC-1·SC-2부터.
## D-B261 (2026-09-08 17:20, 머신 B) pull 변형 6 스택 확정(idle) — zlink ≈ asio_pull(64 B), zlink/cppserver_pull 0.84/0.81/0.78; 64 KiB는 hop이 아니라 zlink 큰 payload 경로; zlink CCU 4000 run_failed

**결과**(`stream-pull-variants-summary.md`, results/20260908_164225, load 0.58, CCU 1000 runs 3 중앙값 kops): zlink 273.0/245.3/32.8, asio 335.5/319.7/38.6, asio_pull 271.7/260.4/17.2, cppserver 344.8/322.0/39.3, cppserver_pull 324.2/304.2/42.2, zmq 310.0/290.1/26.0. 비율 asio_pull/asio 0.81/0.81/0.45, cppserver_pull/cppserver 0.94/0.95/1.08, zlink/asio_pull 1.005/0.94/1.91, zlink/cppserver_pull 0.84/0.81/0.78, zlink/zmq 0.88/0.85/1.26. 검증: PULL_STATS off_worker=0(모든 echo가 worker 경유), CCU 100/1000/4000 스윕에서 스택별 독립 이동(공통 병목 아님; 148 triple은 오염 창), eventfd vs condvar −1.5/−5.6 %(hop 자체가 비용). pull 변형은 서버 CPU +30~70 pp.
**해석**: (1) asio 기본 서버 대비 격차는 hop이 전부(64 B) ~ 대부분(1 KiB). (2) cppserver_pull은 같은 hop에 −6 %만 잃음 — asio 기본 서버가 세션당 read→write 직렬화(half-duplex)라 hop 비용이 과대. **zlink 계층 고유 비용 = cppserver_pull 대비 16~22 %** → SC-1~4(D-B260)의 목표치. (3) 64 KiB: hop 무관(cppserver_pull > cppserver), zlink 큰 payload 경로 문제(ALL-3 B-4·S-D). (4) **zlink만 CCU 4000/64 B run_failed**(5 스택 완료) → 조사 항목 CCU-1.
**§7.1**: 6 스택 행 추가. 벤치 스택 커밋.

## D-B262 (2026-09-08 17:45, 머신 B) MAC-2 결과 — macOS 잔여 5건은 전부 테스트의 Linux 속도 가정(Core 런타임 불변), 4/5 해소, 1건 검증 run 진행 중

**정정**: MAC-1의 "단독 실행 시 PASS"는 틀림 — 5개만 좁혀 돌린 run 34198394530에서 5/5 실패 → 순서·TIME_WAIT·포트 가설 기각, 재현 700 s→12 s. 원인·수정(`core-rf-MAC-2-report.md`): `single_lane_wire_*` 2건 `wait_for_raw_close()`가 1 B recv+10 ms sleep(100 B/s)라 69 B를 3 s에 못 뺌 → 큐 전체 drain; `test_ctx_options:657` drain 루프 레코드당 20 ms 대기 × queued > SNDTIMEO 1 s → 대기 제거; `test_xpub_nodrop:243` 구독자 1 ms polling vs NODROP SNDTIMEO 200 ms → RCVTIMEO 50 ms 블로킹 수신; `test_wake_invariants` fill이 첫 EAGAIN을 포화로 간주(TCP·peer 큐 여유로 WRITABLE 조기 발행) → saturation settle 단계 + BUSY 프로브 yield→msleep(1). Linux 5개 until-fail:3 통과. macOS run 34201309177 serial 148/149(잔여 `test_wake_invariants` waiters_blocked=0, 관찰자 스핀 기아 → `77a2f81db6`), 확인 run 34202650122 진행 중. 병렬 그룹에 `unittest_flow_state_monitor` Timeout 1건 신규(−j3 부하 플레이크 추정). patch `all-artifacts/MAC-2.patch`(MAC-1+2, core 8 파일).
**최후 수단(미적용)**: `ZLINK_CTEST_EXCLUDE_REGEX='^(test_wake_invariants)$'`(+`unittest_flow_state_monitor`). 확인 run 결과에 따라 사용자 결정.

## D-B263 (2026-09-08 17:55, 머신 B) ALL-3 결과 — 정확성 전부 통과, 큰 payload latency 목표 미달(RSS −70 %); MERGE-1로 0.17.4 브랜치 병합 + 최종 게이트

**ALL-3**(`core-rf-ALL-3-report.md`, astra 3 h 56 m): A 큰 payload — `decoder_allocators.cpp`가 현재 read buffer를 유지한 채 같은 recycle state의 block으로 message 생성(수명 규칙 준수), 신규 unit 6개; C relay(perf 러너)의 무제한 receive drain 제거(계약상 application 책임); WS shutdown fixture를 ping/pong fence로 결정화(`test_stream_packet_progress` 기존 debt 해소); Windows 이식성 수정(MSVC 선언·식별자, WinSock client, Unity hook)으로 Windows 전체 ctest 201/210(잔여 5 suite). 검증: dev 210/210, TSan 전체 210/210 suppression 0, ASan 12/12, Python gate 29/29, 공개 API diff 0.
**성능**: WS/WSS/TLS 64 KiB DR_SENDSEND latency tcp 대비 5.0~5.9×(RR 14.6~15.1×) — 3× 목표 미달; 서버 RSS ws 1418→431 MiB(−70 %), 4.2× — 3× 목표 미달. ws Q64/Q1 0.455 FAIL(WSS 1.01 PASS). hotpath 3/5 PASS + DD·reqrep 하한 FAIL(개선 방향). B(1) single 1024 B 교대 PAIR 92.6/99.3 %, DD 96.6/96.5 % → 회귀 미확정(첫 PAIR −7.4 % 기록); RR 3런 251k vs Phase 2G 242.5k(회귀 없음). D-BP29 tls RR 1024 B 5/5 complete·median 3.6 ms(스파이크 재현 안 됨), D-BP34 tls 64 KB 5/5 complete(echo 누락 0) — 수정된 relay 기준.
**결정**: 0.17.4에는 ALL-3의 정확성·메모리·이식성 변경을 포함하고, 큰 payload latency(ws/tls 64 KiB 5×)는 S-D 결과와 합쳐 다음 캠페인 항목으로 이월(계약 안에서 allocator만으로는 불충분). MERGE-1(Claude opus, 4 h): 브랜치 `wip/0.17.4` = `wip/0.17.3-all2` + ALL-2b + ALL-3 delta + RR-1 + MAC-2 → origin/main rebase → Linux 전체(ctest·TSan·ASan) + hotpath + with_stream 6 스택 + perf/c multi 3런 + ws/latency 게이트 + Windows 전체 ctest. 통과 시 main 착지 → changelog 0.17.4 → bump 예고·bump·태그 → Actions 릴리스.
...
## D-B267 (2026-09-08 18:40, 머신 B) S-D 결과 — cppserver 우위는 zero-copy가 아니라 성장형 receive buffer·read/write 독립 진행(64 KiB recvfrom 1.0 vs asio 5.0/msg); zlink 64 KiB는 frame 2회 read 분할이 RAW echo fast path를 무력화(copy 197.8k Ir/msg); 후속 B1·B2

**보고서** `core-rf-S-D-cppserver-contrast.md`(sol, 정적 대조 + strace/Callgrind, idle). **원인 판정**: 주원인 = cppserver receive buffer가 full-read 때 2× 성장(asio 16 KiB 고정) → 64 KiB 성공 recv/msg asio 5.003 vs cppserver 1.004, sendto 2.0(composed `async_write`) vs 1.0(`write_some`). zero-copy 가설 반박(1 KiB copy 3 vs 2). 보조 = strand 미사용(1 KiB), handler inline alloc. zlink: 1 KiB syscall 9.24/msg, self Ir 8,686 vs asio 4,986/cppserver 3,642(고정 API·pipe 비용, copy는 asio보다 적음); **64 KiB는 decoder target 고착(“연속 full hit 2회” 규칙)으로 frame이 2 read로 갈라져 fragment slow path → memcpy 132k+memset 66k Ir/msg**가 실병목. 계측 app 결함 1건(`test_scenario_stream_zlink.cpp:243-250` 정상 TCP suffix를 malformed 처리)은 B2 전에 per-RID frame buffer로 고칠 것.
**채택 후보(계약 내, Core Asio engine 소유)**: **B1** `restart_input()` speculative read를 기존 full-read evidence gate와 통합 → 실패 `recvfrom` 1/msg 제거(2~3 h). **B2** decoder read target 성장 규칙을 “full hit 1회면 2×, 기존 max clamp” 하나로 단순화 → 64 KiB 1 read/msg, fragment copy 소멸(2~3 h; 64 B/1 KiB 처리량·connection당 RSS 재측정 후 채택). B1+B2 결합 목표 socket recv 3.025→≈1/msg. 이미 채택된 것(async_read_some, read/write 독립, inline storage, pointer write, write_some, bounded batch)은 job 아님. **불채택**: completion callback에서 app echo 직접 호출(08-stream §5·11-sync owner 분리 위반), 준비 안 된 message 대기 batch(C 우회), 고정 128 KiB buffer(메모리 상시 지불). **D 항목 D-S1**: `rcvbuf=-1`일 때 decoder max 4 KiB를 OS default 크기로 올릴지 — CCU별 user-space buffer 메모리 상한 정책 필요, 사용자 결정.
**결정**: B1·B2를 다음 캠페인(0.17.5) job 목록에 SC-1~4 앞순위로 등록(64 KiB ws/tcp latency 축과 직결). 0.17.4에는 넣지 않음(MERGE-1 진행 중). 계획 §8 갱신.
## D-B268 (2026-09-08 18:10, 머신 B) CCU-2 결과 — attach 증분 plan 확장(O(log n))으로 CCU 4000 실패 → 202.8 kops PASS; 편차 1건(수위 하강 구간의 기존 큐 목표 인하가 debounce까지 지연) = **D-H1 사용자 결정**; review-CCU-2(sol) 착수

**결과**(`core-rf-CCU-2-report.md`, 패치 `all-artifacts/CCU-2.patch` 9파일 785줄): attach 시 붙는 파이프의 두 방향만 마지막 plan에 O(log n)으로 더한다(자동 방향이 같은 role이면 water-filling level은 하나라 새 level O(1), 나머지는 stable ID 순서라 ID 최대인 새 방향은 정확히 level). 폴백(기존 전체 재계산): role 혼합·manual·budget 부족·재계산 예약·ID wrap·plan 입력 변경. 새 옵션/플래그/타이머 0, `resolve_direction()` 하나로 방향 해석 규칙 통합, public 헤더 무변경. with_stream 64 B: CCU 4000 rc=2(0 kops) → **202.8 kops**(asio 327.9), CCU 1000 313.6(asio 363.4); 메인 스레드 100 % 프로파일 소멸. ctest 210/210. 미실행: TSan·hotpath 5셀·`--size all` before/after(시간 상한) → review-CCU-2에서 실행.
**계약 확인**: 스펙 §3 `budget_generation`(attach 직후 증가, `test_ctx_options.cpp:800`)·§4(attach가 planner 실행)·§5 결정성은 유지. CCU-1의 B안(순수 debounce)은 generation 계약을 깨므로 기각.
**D-H1(편차)**: `06-auto-hwm.ko.md` §2 표 "연결 증가로 queue별 목표가 감소 → 새 목표를 즉시 기록". 수위가 내려가는 구간(N·role최대 > budget; Balanced/STREAM·11.9 GB 호스트에서 연결 2048개 초과)에서 **새 방향은 즉시 새 level, 기존 방향의 인하는 debounce(3000 ms) 재계산까지 지연**. 영향: 그 동안 기존 큐 admission 한도가 최대(이전 level − 새 level)만큼 느슨; `total_planned`는 정확, `total_applied > planned`는 스펙이 이미 인정하는 deferred shrink 상태로 보고. 편차 없이 지키면 attach마다 N개 방향에 써야 해 O(N²) 원복(saturated 구간만 확장하는 변형은 CCU 4000 여전히 실패, 17:47 측정). 근본 해소는 `planned_hwm`을 role별 level+generation 파생값으로 바꾸는 리팩터(0.17.5 후보).
**감독관 권고**: 채택. 근거 — 옵션 변경도 이미 같은 debounce 경로(§103-106)를 쓰고, 편차는 상한이 있는 일시적 느슨함이지 한도 초과가 아니며, 단일 규칙("attach는 새 방향의 목표를 즉시 기록하고 기존 방향의 인하는 debounce 재계산에 맡긴다")으로 스펙 §2 표 한 행만 고치면 된다. 대안 = CCU-2를 0.17.5로 미루고 0.17.4는 CCU 4000 실패(기존 동작) 유지. **사용자 답 전까지 CCU-2는 wip/0.17.4에 올리지 않고 review-CCU-2 결과만 준비**한다. §7.5 D-H1 등록.
## D-B269 (2026-09-08 18:20, 사용자) D-H1 **채택** — CCU-2를 0.17.4에 포함; 06-auto-hwm §2 표 한 행 갱신(ko/en)

사용자: "채택으로 진행하고 0.17.4에 포함해". 규칙 단일화: attach는 attach하는 방향의 목표만 즉시 기록하고, 기존 방향의 인하는 option 변경과 같은 debounce 재계산 경로가 기록한다(동기 전체 재계산 없음). §7.5 D-H1 → 채택. 순서: review-CCU-2(TSan·hotpath·정확성) 결과 → 차단 항목 수정 → MERGE-1 완료 후 `wip/0.17.4`에 MAC-3·CCU-2 순으로 적용 → 변경 suite 재검증 → 착지·changelog·bump.
...
## D-B272 (2026-09-08 19:05, 머신 B) MAC-3 결과 — macOS ARM64 ctest **149/149 serial·57/57 parallel 통과(run 34210988899)**, 제외 정규식 없음, Core 런타임·Darwin 가드 무변경

**원인**(`core-rf-MAC-3-report.md`): `test_wake_invariants` 두 겹 — (1) MAC-2 settle의 1회 DONTWAIT 샘플링이 조기 크레딧을 놓침 → `zlink_poll(POLLOUT, 1000 ms)` 대기로 교체(MAC-2의 "BUSY 프로브 기아" 해석은 오판); (2) 서버 RCVHWM 1 MiB×100 저수지가 차면 drain이 메모리에서 끝나고 51 MB 전송 뒤에야 크레딧 → 서버 RCVHWM 2레코드로 낮춰 drain이 wire에서 끌게 함. 단언·기대값·토큰 규칙 불변. 부수 3건: `unittest_flow_state_monitor`(LIN-1 `550f0e3f6e` 동일), `unittest_asio_transport_writev_lifetime`(fill_send_buffer가 첫 EAGAIN에서 멈춤 → POLLOUT timeout까지), `test_stream_packet_progress`(`yield()` 스핀→`msleep(1)`, 진단용 TCP 샘플러가 3코어 러너 부하). Linux `-R wake until-fail:5` ×2 통과, 계측값 불변.
**결정**: 패치 `all-artifacts/MAC-3.patch`(MAC-1+2+3 누적 11파일 +267/−45)를 MERGE-1 뒤 `wip/0.17.4`에 적용(rel174에 MAC-1+2 커밋 `84d25131a6`이 있으므로 MAC-3 증분만 3-way 적용). 진단 workflow는 병합 대상 아님; 상시 macOS 게이트는 `build.yml` macOS job이 `build.sh`(serial/parallel 분리, 실패 시 exit 1)를 이미 실행하므로 추가 후속 없음.
## D-B273 (2026-09-08 19:25, 머신 B) MERGE-1 결과 — `wip/0.17.4`(`84d25131a6` = ALL-2·2b·3 + RR-1 + MAC-2, main rebase 충돌 0) Linux 게이트 전부 통과; Windows 신규 실패 1건(WIN-2), ws 비율 게이트 0.534 FAIL은 SD-1에 연동, hotpath reqrep 0.949(개선)

**통과**(`merge-1-summary.md`): ctest 211/211, 변경 suite 147 until-fail:3, 신규 11 target until-fail:10, wake 5 target until-fail:20, TSan 211/211 경고 0(억제 없음), ASan 41/41, `core/include`·`libzlink.vers` diff 0, 4언어 헤더 mirror 동일. perf/c multi tcp 1024 3-run: DD 991.5k, DR_SENDSEND 327.5k, RR_SENDSEND 266.9k, DR_REQREP 217.0k. with_stream 3 스택(rel174에 vendored 부재로 cppserver·zmq 미측정): zlink 279.1/140.0/27.9, asio 350.1/177.6/34.8.
**미해결**: (1) Windows 5개 잔존 중 **신규** `test_writable_resubmit_from_other_thread_while_sequence_open`(+`_send`) exe 기동 불가(ALL-3 clone 동일 소스는 통과 → 환경/링크 차이 의심) → **WIN-2**(MERGE-1 에이전트 연장, 2 h): 원인 확정·수정. 나머지 3개(wake large-HWM, flow_state_socket timeout, unittest_mutex CRITICAL_SECTION)는 0.17.3 Actions Windows green이었으므로 host MSVC 한정 여부만 확인. (2) ws Q64/Q1 0.534(ALL-2b 0.90 → ALL-3 0.455 → 병합 0.534; wss 1.26 PASS): 원인이 64 KiB frame 2-read 분할(S-D)이므로 **SD-1(B2) 결과로 판정** — SD-1 후 ws 게이트 재실행이 0.80 미만이면 ALL-3의 decoder allocator 변경을 되돌릴지 결정. (3) latency·RSS 게이트 미실행 → SD-1 A/B와 함께. (4) `dealer_router_reqrep_inproc` 0.9494 개선 → reference 갱신(WIN-2 job에 포함). (5) `bindings/rust/include/zlink/common.h` PATCH 2 잔존(기존 결함) → 0.17.4 bump 체크리스트에 4언어 mirror **전체 파일** 확인 추가.
**남은 착지 순서**: WIN-2·MAC-3 적용(rel174) → review-CCU-2 통과 후 CCU-2 적용 → SD-1 게이트 통과 시 적용 → 변경 suite·TSan·hotpath·ws 게이트 재검증 → main 착지 → CHANGELOG [0.17.4] → bump 공지·bump → tag → Actions → Release.
...
## D-B276 (2026-09-08 19:50, 머신 B) review-CCU-2 결과 — 차단 3건(채택 불가) → CCU-3 수정 라운드(2 h); 해소 실패 시 CCU-2는 0.17.5 이월

**리뷰**(`review-ccu2.md`, sol): 증분식 숫자(level·remainder·total)는 전체 water-filling과 비트 단위 동일, 폴백·직렬화 주요 경계 확인, TSan 8 suite 0건. 차단: **B-1** attach fast path의 `std::vector` 할당 예외가 기존 bad_alloc→ENOMEM 경계 밖(`socket_base.cpp:320-328`); **B-2** plan·`budget_generation`을 붙는 pipe의 writer `_hwm` 적용 전에 공표(full path는 적용 후 기록) → 동일 generation snapshot(§3)·admission 목표 적용(§4) 위반, 동시 attach 순서 역전 가능; **B-3** "새 상태 0" 주장이 거짓 — registry 8개·queue record 2개의 새 지속 상태가 `applied_plan()`이 소유한 같은 plan 사실을 복제(이중 소유, 일치 guard·invalidation flag 추가) → POSDDD 위반. 경고: 집중 테스트 없음, 주석과 구현 반대(비포화도 성공), hotpath reqrep 0.9448(개선 방향, ALL-3 기인 → reference 갱신 진행 중). S-1: 편차 없는 O(1) 설계는 registry 소유 immutable plan descriptor{generation, role별 level, remainder cutoff} 하나 + generation-aware admission, 2~4 engineer-day(0.17.5).
**결정**: CCU-3(CCU-2 에이전트 연장, 2 h) — applied plan을 단일 소유자로 하여 복제 상태 제거(B-3), 예외 경계 안으로/힙 할당 제거(B-1), pipe hwm 적용 후 기록·mutex 범위 정렬(B-2), 주석 정정, 집중 테스트(전체 재계산과 비트 동일 비교 포함), TSan·전체 ctest·CCU 4000 재확인. B-3이 해소되지 않으면 CCU-2 전체를 0.17.5(S-1 설계)로 이월하고 0.17.4는 CCU 4000 실패(기존 동작) 유지. 통과 시 review-CCU-3(짧은 재검증) 후 `wip/0.17.4` 적용.
...
## D-B280 (2026-09-08 20:30, 머신 B) CCU-3 완료 — 차단 3건 해소(단일 소유 3필드, 힙 할당 0, apply→record 순서·mutex 전체), unittest 7 케이스, ctest 211/211, TSan 9 suite 0건, CCU 4000 212.7 kops; review-CCU-3(sol, 40분) 착수

`core-rf-CCU-3-report.md`, 패치 `all-artifacts/CCU-3.patch`(core/** 13파일). 새 상태 10개 → `auto_hwm_context_plan_t` 3필드(`auto_hwm_policy.hpp:66-78`); "확장 가능" 판정은 plan 필드에서만(`registry.cpp:971-981`), role 혼합은 role=none; detach 무효화는 pending generation guard(`socket_base_api.cpp:2022` schedule이 endpoint 은퇴보다 먼저); 이미 plan에 든 방향 판별 `queue_id > max_planned_queue_id` 한 규칙(완성된 inproc pair 두 번째 attach는 전체 재계산 = CCU-2 이전 동작). B-1 고정 배열 `policies[2]`·`extensions[2]` + ctx 경계 backstop(`ctx_auto_hwm_recalc.cpp:163-180`). B-2 mutex 전체·registry 확장→pipe `_hwm`→plan/generation 기록. ID wrap 미커버(공개 API로 도달 불가, wrap 시 전체 재계산 폴백). CCU 4000은 load1 4.71 창(PASS 유지 확인용). 통과 시 `wip/0.17.4`에 적용.
## D-B281 (2026-09-08 20:45, 머신 B) WIN-2 결과 — Windows 신규 실패는 **호스트 Smart App Control 차단**(Core 결함 아님); MAC-3 적용(`bdedf4300d`)·hotpath reqrep reference 15609.787(`b27f25dea1`); Windows 판정은 Actions로 — `wip/0.17.4` 사전 run 34219639442

`win-2-summary.md`: `test_writable_resubmit_*` exe·의존 DLL·소스가 ALL-3 clone과 바이트 동일, cmd.exe 실행 원문 "Device Guard 정책에 의해 차단", CodeIntegrity 3033/3077/3118, SAC 강제(`VerifiedAndReputablePolicyState=1`). MERGE-1의 BAD_COMMAND 6건도 같은 원인(각 1회 차단 후 통과). 코드 수정 없음. 나머지 3개(wake large-HWM drain 60 s 만료 2/3, flow_state_socket 10 s aggregate timeout, unittest_mutex CRITICAL_SECTION 재귀 = spec11:224-228 백엔드 차이)는 host 한정 여부를 Actions Windows x64로 확인. **결정**: Windows 게이트는 GitHub Actions(SAC 없음)에서 판정; host MSVC는 빌드 확인용. `wip/0.17.4`(`b27f25dea1` = MAC-3 포함)로 `build.yml` 사전 run 34219639442 시작(릴리즈 job은 tag에서만 실행). 남은 미실행: latency·RSS 게이트(SD-1 A/B와 함께), ws 게이트(SD-1).
...
## D-B284 (2026-09-08 21:50, 머신 B) SD-1 결과 — 64 KiB A/B는 대조군 4 stack 동반 −40 % **오염 창이라 판정 불가**; B2 효과 확인(성공 read 2.017→1.010/msg, ws Q64/Q1 0.431→0.919, DR ws 64 KiB 5.5→18.7 kops), B1 미달(EAGAIN 1.0/msg 그대로), RSS 64 KiB +51 %(귀속 불가); job의 "B1·B2 기각" 권고는 **채택하지 않음** → SD-2(3 h)

`core-rf-SD-1-report.md`, patch `SD-1.patch`, 실험 `SD-1-DS1-experiment.patch`. 통과: ctest 211/211, 27-suite ×3, TSan 0건, Release+LTO. 변경: 계측 app per-RID frame buffer(`test_scenario_stream_zlink.cpp:208-290`), B2 1-hit 2× 성장(`asio_stream_fastpath_policy.hpp:332-399`, decoder·encoder hit 상태 제거), B1 단일 predicate(`asio_engine.cpp:804-815,985-1025,1673-1681`), D-f STREAM gather env 3개 제거(`:129-144`, ZMP `ZLINK_ASIO_GATHER_WRITE` 유지). 64 KiB per-run: zlink base 32.4/32.4/20.3 vs patch 41.3/19.9/17.9, asio 41.3/39.9/20.6 vs 38.7/23.4/21.4 — 시간축 하락(CCU-3 빌드·ctest와 겹친 창). 1 KiB 이하 kops 비 0.95(대조군도 0.95~0.98, 창 영향). D-S1 실험: CCU 1000 RSS 차이 없음, CCU 4000은 양쪽 readiness timeout으로 무효.
**결정**: D-B277(이월 없음)대로 SD-2 — B1 완성(full read 직후 EAGAIN 1회의 호출 지점 확정, async rearm vs 1회 speculative 후보 strace/latency 비교), ablation 3빌드(base / B2 / B1+B2) 같은 창 interleave, **유효 창 규칙**(다른 worktree의 ninja/ctest/codex 없음·load1<1.0, 대조군 15 % 이상 변동 시 묶음 폐기), 비교 스택은 pull 4종(D-B275), ws 게이트 base/patch 각 3회(ws·wss ≥ 0.80, 미달 시 수정), RSS 상한 계산 대조. 측정 창은 CCU-4·MAC-4 빌드가 끝난 뒤 확보. **ETA**: 0.17.4 릴리즈 09-09 02:00~03:00 → **09-09 03:30~04:30**.
...
## D-B290 (2026-09-09 00:50, 머신 B) SD-2 중간 — **유효 창 A/B**(인접 대조군 변동 ≤ 7.5 %): 64 KiB zlink 32.3 → 40.3(B2) / 39.6(B1+B2) kops(+25 %/+23 %), RSS 279 → 423/425 MiB(+52 %, B2 귀속); B1 완성(총 recvfrom 2.02 → 1.02/msg, EAGAIN 0, p50 동률, 축소셀 p99 +14 %); ws 게이트 3-run 중앙값 base ws 0.547 / final 0.527 FAIL(wss PASS) → ZMP-over-WS 64 KiB 경로 별도 조사 중; **ETA 09-09 05:00~06:00**

B1 EAGAIN의 실제 출처는 Boost.Asio `async_read_some` 시작 시 선행 nonblocking recv — full TCP read 뒤에는 `async_wait(wait_read)` 후 readiness callback에서 즉시 nonblocking read(spurious만 기존 async로 fallback), 새 상태 0. SD-1의 ws 0.919는 1-run 노이즈였음(3-run으로 반증). B1/B2는 RAW STREAM 경로라 ZMP-over-WS 게이트를 바꾸지 않으며 base도 같은 병목 재현 → SD-2가 D-B277대로 WS 64 KiB DR의 decoder/encoder·frame 경계 2단계 read/copy를 좁혀 수정. MERGE-2(rel174 dev 빌드·until-fail·ctest·TSan) 재개(SD-2 조사 창 활용, PERF_LOCK 인지). 릴리즈 ETA 03:30~04:30 → **05:00~06:00**(ws 수정 1.5~2 h + 최종 게이트 1 h + 착지·Actions 1.5 h).
## D-B291 (2026-09-09 01:30, 머신 B) SD-2 결과 판정 — **채택: 계측 app 수정·B2·D-f·WS gate 수정 / 기각: B1**(실측 열세); MERGE-2 검증 통과; SD-3(B1 제외 patch)·review-SD-2 병렬

`core-rf-SD-2-report.md`(sol), 유효 창(대조군 인접 변동 ≤ 7.5 %, 3 cycle interleave). **B2**: 64 KiB zlink 32.29 → 40.32 kops(+24.9 %), p50 −19.9 %, p99 −23.8 %; 64 B·1 KiB 동률. **B1+B2 vs B2-only**: 64 B −1.6 %, 1 KiB −3.9 %, 64 KiB −1.8 %·p99 +15.9 % — recvfrom 2.02→1.02/msg·EAGAIN 0은 달성했으나 실측이 나쁨 → §4 규칙(실측 기준, syscall/Ir은 참고)대로 **B1 기각**; patch·측정은 0.17.5 재평가 자료로 보존(job 권고 "B1 채택"은 syscall 목표 근거라 채택하지 않음). **RSS** 64 KiB 279 → 423 MiB(+141 MiB, 연결당 1 MiB 상한의 14 %) — B2 채택 조건으로 §7.5 D-S1 측정 자료. **WS gate 수정**(Core 결함): ZMP header/body가 message-boundary transport(WS/WSS)에서 WebSocket frame 2개로 나가던 것을 "header 생성 가능 && gather 지원 && message boundary"이면 connection 생성 시 gather 선택(새 env 없음, RAW STREAM·TCP ZMP 불변) → ws Q64/Q1 3-run 0.547 → 1.845, wss 1.157 → 1.416, 3 run 모두 PASS. **D-f** 채택(죽은 STREAM gather env 3개 제거; 스펙 08-stream :405-407 삭제 — 감독관이 착지 시 반영). hotpath 5/5(reqrep reference 15608.4, 브랜치 15609.8과 동일 대역). 후속: SD-3(45분, B2-only+WS 수정 patch, 검증) + review-SD-2(40분, B2·WS gather 계약 대조: WS frame↔ZMP frame 문장, 하위 호환, bounded batch). **비교표(§7.1, pull 4 스택, base 창)**: zlink 270.6/250.1/32.3, asio 277.5/255.7/18.7, cppserver 327.1/298.4/42.3, zmq 313.6/284.8/25.4 → B2 후 zlink 64 KiB 40.3(zlink/cppserver 0.83/0.86/0.95).
## D-B292 (2026-09-09 01:55, 머신 B) review-SD-2 — app 수정·B2·D-f 채택 가능, B1 분리 가능; **WS gate 수정 차단(B-SD2-1)**: 단일-message gather 기본화가 ZMP §9 bounded multi-frame batch owner·128 KiB max를 우회 → SD-4(1.5 h): bounded batch 경로 안에서 [batch, 큰 body]를 한 writev로

`review-sd2.md`(sol): W-SD2-1 — header/body 별도 WebSocket message는 wire 위반이 아님(ZMP §8/§9 WebSocket = byte carrier, `01-zmp.ko.md:125-127,381-387,481-492`) → D-B291의 "Core 결함(wire)" 분류를 **batching 성능 결함**으로 정정. 반례: target 128 KiB에 64 KiB+작은 frame이 준비돼도 첫 message만 제출, body > 128 KiB도 max 검사 없이 한 operation. W-SD2-2: "연결당 1 MiB 상한"은 벤치 `rcvbuf` 유도값이고 런타임 기본 `rcvbuf=-1`은 4 KiB cap(D-S1 자료 정정); RSS 산술은 hard cap 증명 아님. W-SD2-3: header+body 결합/분리 양쪽 wire는 0.17.3과 호환(decoder는 header 길이로 복원). hotpath reference hunk는 채택 patch에서 제외(브랜치 값 유지). **결정**: SD-3(B1 제외 구성) 완료 후 같은 worktree에서 SD-4 — SD-2 WS hunk 되돌리고 `prepare_output_buffer()` bounded batch가 닫히는 지점의 pointer 전달 큰 body를 batch와 같은 `async_writev()`로 제출(target·128 KiB max 검사 유지, 새 상태 0), 반례 2건 테스트, ws gate 3-run ≥ 0.80. **ETA**: 0.17.4 릴리즈 05:00~06:00 → **09-09 07:00~08:00**.
## D-B293 (2026-09-09 01:20, 머신 B) SD-3·SD-4 완료 — 채택 구성 = 계측 app 수정 + B2 + D-f + **WS bounded batch 수정**(SD-2 단일-message gather 되돌림; `prepare_output_buffer()` batch 뒤 pointer 전달 큰 body가 오면 합계 ≤ 128 KiB일 때만 `[batch, body]` 한 `async_writev()`, 초과는 기존 분리), ws 게이트 3-run 전부 PASS; review-SD-4(30분) 착수

SD-3(`core-rf-SD-3-report.md`): B1 제외 구성, ctest 211/211, TSan 27/27 0건, strace 성공 read 1.011/msg·EAGAIN 1.001/msg(B2-only 특성), ws gate 1회 2.10/1.30. SD-4(`core-rf-SD-4-report.md`, patch `SD-4.patch` SHA e7f99f0b…): TCP ZMP env opt-in gather 유지·기본 off, RAW STREAM 불변; 성장 입력은 실제 batch buffer byte만(zero-copy body를 성장량에 더한 중간안은 wss 회귀로 폐기); 반례 테스트 2건(128 KiB target 내 64 KiB+small 한 copy batch / max 초과 분리); dev 211/211, TSan 27/27 0건; **ws Q64/Q1 1.172/1.257/1.173, wss 0.931/1.068/1.083 — 3 run 모두 ≥ 0.80**; hotpath 4/5 PASS(reqrep 셀은 sd1 worktree의 구 reference 16455 대비 −5 %, 브랜치 reference 15609.8과 정합). 이후 MERGE-3: `wip/0.17.4`에 SD-4.patch 적용 → 최종 게이트(전체 ctest·TSan·hotpath·ws gate·with_stream 4 pull stack 1회) → main rebase → 착지.
## D-B294 (2026-09-09 01:40, 머신 B) review-SD-4 — 차단 1건(B-SD4-1): pointer gather가 body가 target을 넘는지 검사하지 않아 target 128 KiB에서 64 KiB body를 즉시 제출, 뒤의 small frame 미포함(§9 "준비된 byte를 상한까지" 위반); unit 2건이 원 반례 순서·write 수를 관찰하지 않음 → SD-5(1 h): **[batch 앞, pointer body, batch 뒤] 3-buffer writev**

`review-sd4.md`(sol): SD-2 gather 기본화 되돌림·TCP env opt-in 유지는 확인. SD-4 iteration 1(64 KiB body를 batch에 copy)은 ws 0.51 FAIL이었으므로 "copy해서 모은다"는 성능상 불가, "즉시 제출"은 스펙 위반 → 두 요구를 모두 만족하는 규칙은 batch buffer 안에 body 자리를 표시(split offset 하나)하고 뒤의 frame을 계속 모아 한 writev로 내는 것(한 batch에 pointer body 최대 1개, 합계 max 검사). 테스트는 원 반례 순서(64 KiB→small)·small→64 KiB→small·max 초과 분리·pointer body 2개를 write 수로 고정. 스펙 문장 변경 없음. **ETA**: 05:00~06:00 → **09-09 06:30~07:30**(SD-5 1 h + review 30분).
## D-B295 (2026-09-09 02:50, 머신 B) SD-5 완료 — WS/WSS bounded gather: 첫 pointer body 위치만 `gather_split_offset`에 기록하고 후속 frame을 같은 batch 뒤에 계속 모아 `[앞, body, 뒤]` 한 `async_writev`·Beast write 1회; 반례 4건 실제 encoder 경로 테스트; ws gate r1/r2 PASS(ws 0.97/0.83, wss 0.83/1.36), r3 partial(ROUTER_ROUTER ws 64 KiB endpoint 오류 1건, 시작 load 3.99); hotpath 5/5; review-SD-5(25분) + MERGE-3 적용·Linux 게이트 병렬

`core-rf-SD-5-report.md`, `SD-5.patch`(SHA 07a9bbc9…). r3의 endpoint 오류는 gate 대상 외 셀이지만 새 3-buffer 경로 결함 가능성을 리뷰에서 코드로 검토하고, MERGE-3 최종 ws gate(idle, 18셀 단일 run)에서 재현 여부 확인 — 재현되면 착지 전 수정. ws 값은 SD-4(1.17~1.26)보다 낮으나 시작 load 1.3~3.99의 비idle 창; 최종 게이트에서 idle 재측정. MERGE-3: `wip/0.17.4`(e215322ea5)에 SD-5.patch 적용 → 로컬 커밋 → dev 빌드·until-fail·전체 ctest·TSan 전체(리뷰와 병렬), 측정·push는 리뷰 통과 후.
## D-B296 (2026-09-09 03:05, 머신 B) review-SD-5 — **차단 0, 채택 가능**(W 4건: r3 RR ws64 endpoint 오류 원인 미확정, 원로그 부재, 3-buffer pending cancel 전용 테스트 없음, Windows/macOS compile 로그 없음) → MERGE-3 측정·rebase·push 승인

`review-sd5.md`(sol): split은 engine별 `_pipeline` field, transport는 connection별 객체라 connection 간 공유 없음; pointer body는 heap `msg_t` `shared_ptr`를 completion lambda가 잡아 수명 유지; 정상 completion이 split 정규화, 취소/종료는 pipeline 파괴로 정리; Windows TCP/IPC는 3-buffer를 Asio sequence로, WS/WSS는 Beast 공통 API, Unix 2-buffer native 경로 유지. **후속**: W-SD5-1은 MERGE-3에서 RR ws64 셀 단독 3회로 종결(재현 시 착지 전 수정); W-SD5-4는 Actions Windows/macOS run으로; W-SD5-3(3-buffer pending 중 cancel/disconnect 테스트)은 테스트 보강 항목으로 0.17.5 목록(이슈 아님). SD 리뷰 4회(SD-2→5) 누적 차단 3건 해소. MERGE-3 진행: SD-5 적용 커밋 `ec230249f6`(23파일, public iface diff 0).
## D-B297 (2026-09-09 03:15, 머신 B) **예고: VERSION 0.17.3 → 0.17.4 bump 예정**(MERGE-3 최종 게이트 통과·main 착지 직후, 예상 04:30~05:30) — 머신 A 러너는 그 시점부터 release prefix 0.17.4로 재고정 필요(D-B247 규칙)

bump 범위: `0761c1d4d0` + `4cdafee9b7`과 같은 파일(VERSION, BINDINGS_VERSION, 4언어 header mirror 8파일 — rust `common.h` PATCH 2 잔존 수정 포함, c/cpp 계약 테스트 상수, dotnet/java/node/python/rust 패키지 버전, packaging, vcpkg), `framework/**` 제외. 태그 `core/v0.17.4` → `gh workflow run build.yml --ref core/v0.17.4` → Release(CHANGELOG [0.17.4] 섹션 본문). brief `briefs/bump-0.17.4.prompt`.
## D-B298 (2026-09-09 03:35, 머신 B) **0.17.4 Core 착지 — main `eb6abfb995`**(= `wip/0.17.4` a8cb7b3c9e를 A의 dotnet/node 릴리즈 커밋 2개 위로 rebase, core·bindings/c 내용 동일); MERGE-3 최종 게이트 전부 통과

커밋 10개: ALL-2 → ALL-2b → ALL-3 → RR-1 → MAC-1+2 → MAC-3 → hotpath ref → MAC-4 → CCU-2..5 → SD-1..5. 게이트: dev 빌드, 변경 suite 40 until-fail:5, ctest 212/212, **TSan 전체 212/212 경고 0**, Release+LTO, hotpath 5/5(DD 0.962, reqrep 1.001, PAIR 1.047, RR_tcp 1.021, STREAM 0.994), **ws gate 18셀 3/3 PASS**(ws Q64/Q1 0.848/1.112/1.295, wss 1.005/0.977/1.161; MERGE-1의 0.534 FAIL 해소), W-SD5-1 RR ws64 단독 3회 + 18셀 3회 = 0/6 재현(종결), with_stream pull 4 stack 1회(CCU 1000): zlink 212.8/251.1/**42.7**, asio 228.5/193.4/17.2, cppserver 252.5/234.5/42.5, zmq 293.9/260.3/run_failed(64 KiB, 외부) — zlink 64 KiB 27.9 → 42.7 kops(+53 % vs MERGE-1 창), cppserver와 동률. public 헤더·`libzlink.vers` diff 0. 다음: bump-0.17.4(terra) → tag → Actions → Release. 사소한 문서 불일치 1건: perf 러너 `--help`의 `--io-threads` legacy alias가 실제로는 거부됨(0.17.5 정리).
## D-B299 (2026-09-09 03:55, 머신 B) **0.17.4 bump 착지·태그** — bump `59b0af72bf`(43파일, 4언어 header mirror 8파일 Core 헤더와 byte 동일, framework 제외) + VERSION `LIBZLINK_VERSION_PATCH` 누락 수정 `5d2d215367`; tag `core/v0.17.4` = `5d2d215367`; Actions run 34265135721 실행 중

첫 run 34265021472는 `read-versions`에서 "VERSION components 0.17.3 do not match LIBZLINK_VERSION 0.17.4"로 실패(bump job이 `LIBZLINK_VERSION=` 한 줄만 올리고 `LIBZLINK_VERSION_PATCH=`를 놓침; Windows lib suffix도 이 구성요소에서 나옴) → 수정 후 태그 이동(소비된 산출물 없음). bump 검증: dev ctest 버전/계약 테스트 2/2, `libzlink.so.0.17.4`, C smoke 10/10·6/6, C++ 19/19·7/7; Rust cargo는 로컬 Cargo 1.75(2024 edition 미지원)라 미실행 — Actions/A 러너가 확인. **체크리스트 추가**: 다음 bump부터 `VERSION`의 4줄(MAJOR/MINOR/PATCH/LIBZLINK_VERSION) 전부와 `BINDINGS_VERSION`을 첫 항목으로 확인. 머신 A: release prefix 0.17.4로 재고정 가능.
...
## D-B302 (2026-09-09 08:35, 사용자) D-B301 선택지 **(a) 채택** — build.yml Linux x64/ARM64 runner를 ubuntu-22.04(glibc 2.35)로 내리고 `core/v0.17.4` 자산 재빌드·교체(태그 이동, 소스 core/** 불변); provenance에 glibc 최소 버전 기록

사용자: "1번으로 진행해". 변경: `build-linux` ubuntu-24.04 → **ubuntu-22.04**, `build-linux-arm64` ubuntu-24.04-arm → **ubuntu-22.04-arm**; release job의 provenance에 `linux_x64_glibc_min`·`linux_arm64_glibc_min`(readelf로 산출, 아키텍처 무관) 추가; CHANGELOG [0.17.4] Changed 항목 추가. 빌드 요건 확인: `cmake_minimum_required 3.10`·C++17 기본이라 22.04의 CMake 3.22/GCC 11로 충족. `softprops/action-gh-release@v3`는 동명 자산을 교체하므로 재릴리즈가 아니라 자산 갱신이다. 태그 `core/v0.17.4`를 workflow 수정 커밋으로 이동(core/** 내용 동일, provenance의 `source_sha`만 갱신). 검증: run 성공 후 linux-x64/arm64 자산을 내려받아 `readelf`로 요구 glibc ≤ 2.35 확인.
## D-B303 (2026-09-09 08:40, 머신 B) **D-BP49(3) 해소 — 0.17.4 Linux 자산 재빌드 완료**: 요구 glibc **2.38 → 2.34**(x64·ARM64 모두, `__isoc23_*` 참조 0), provenance에 `linux_*_glibc_min=2.34` 기록

Actions run 34289994087(태그 `core/v0.17.4` = `445e93ca4f`) 전 job 성공, `softprops@v3`가 동명 자산 교체(재릴리즈 아님). 검증: 릴리즈에서 linux-x64/arm64 tar.gz를 내려받아 `readelf --dyn-syms` — 최고 심볼 버전 GLIBC_2.34, isoc23 0건. Ubuntu 22.04(2.35)·Debian 12(2.36)·RHEL 9(2.34)에서 로드 가능. **머신 A**: 0.17.4 artifact를 다시 받아 sha256을 갱신할 것(checksums_sha256 `f20fc086…`, source_sha `445e93ca4f`). 이로써 0.17.4의 열린 이슈는 없다.
