...
gate: worktree identity 5/5, integration 121/121, 전체 171/171; main dev 전체 171/171, boundary 3회. Release+LTO hotpath_gate는 측정 체인 뒤 1회. 커밋: 아래 해시.
**A용 한 줄: Core 수정 커밋(아래 해시) + §4 REJECT 규칙 단순화 — framework ClientServer가 REJECT ROUTER에서 NOT_CONNECTED를 기대하는 코드가 있으면 timeout/HANDOVER 기준으로 정리(D-088 해소).**
