...
러너 버그 별도 수정: DD active deadline·프로세스 실패 판정, freshness guard. gate: `run_tests.sh`(go test/vet, guards, samples 7/7), 관련 5회, race, 공개 API 불변, spec gap 없음(러너 blocker 제외). 다음: pass 1b — Go 공개 async request 표면(admission 결과 + completion poller) 추가(spec 확인 뒤) + 러너 정책 정합. 커밋: 아래 해시.
