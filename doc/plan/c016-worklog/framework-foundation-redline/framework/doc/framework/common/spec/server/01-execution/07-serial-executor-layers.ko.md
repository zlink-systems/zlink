...
State lane의 기다리는 진입에 적용되는 재진입 검사는
[상태 소유와 state lane §7](06-state-ownership-and-lanes.ko.md#state-lane-reentrancy)이 소유한다.
위 표의 일반 `isOnLane` 진단 assertion은 디버그 빌드에서 필수이고 릴리스 빌드에서는
선택이며, 실제 재진입 이력이 있는 컴포넌트에서는 릴리스에서도 유지한다.
