...
> Spot 종류와 종료 사유는 [Spot 모델](01-spot-model.ko.md), gate·turn·Yield는
> [실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate),
> generation 사용은 [Spot·Actor routing](08-routing.ko.md),
> owner 장애 뒤 결과는 [Failure와 failover policy](../05-location-relocation/06-failure-failover-policy.ko.md)가 소유한다.
> 이 장은 그 계약을 만족시키는 구현 구조와 lifecycle 경계 위반을 설명하며,
> Application이 관찰하는 동작을 추가하거나 변경하지 않는다.
...
| 종류 | 만드는 시점 | 이동 |
|---|---|---|
| Entry Spot | Object Server 시작 시 그 node에 하나 | **하지 않는다** |
| User Spot | 만들기를 명시적으로 요청할 때 | 한다 |
| Instance Spot | 만들겠다는 의사를 명시한 호출이 처음 도착할 때 | 한다 |

`Yield` 제공 여부는 [실행 계약 §16](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-call-eligibility)을 따른다.
