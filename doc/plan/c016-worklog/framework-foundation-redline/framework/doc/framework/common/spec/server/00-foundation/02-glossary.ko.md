...
선언과 member 이름은 언어별 interface를 그대로 사용하고, 각 member가 하는 일은
해당 줄의 한국어 주석으로 설명한다.

실제 .NET 선언의 단일 기준은
[.NET Server 언어별 interface](../languages/dotnet/interfaces/README.ko.md)와
[.NET Stream Connector 언어별 interface](../../stream-connector/languages/dotnet/03-stream-connector.ko.md)다.
...
- 값이나 record를 나타내는 용어는 먼저 요약 표(형태·.NET 표기·공개 구성·생성·관리·수명)를
  두고, 필요하면 그 아래에 실제 `.NET` 선언을 보여준다.
- `.NET 표기`에 "public type 없음"이라고 적은 항목의 코드 블록은 구조를 읽기 위한
  contract pseudocode이며 실제 API 이름이 아니다.
- 실제 `.NET` 선언의 단일 기준은 `.NET` 언어별 interface 문서다. 이 용어집의 `.NET` 표기는
  공통 계약을 구체적으로 읽기 위한 보조 표기일 뿐이다.
...
- 새 용어는 다음 네 조건을 모두 만족할 때만 추가한다 — 기존 public identifier나 업계
  용어로 정확히 설명할 수 없다, 여러 문서나 API에서 같은 개념을 반복해서 구분해야 한다,
  이름을 붙이는 편이 동작을 반복해서 설명하는 것보다 이해하기 쉽다, 다른 용어와 의미가
  겹치지 않는다([스펙 문서 작성 가이드](../../../../../../../doc/principal/documentation/spec-writing-guide.ko.md#34-새-제품-용어) §3.4).
...
| .NET 표기 | `IZLinkSpot`, `IZLinkInstanceSpot`, `IZLinkEntrySpot`; 특정 incarnation을 고정한 location snapshot은 `SpotRef` |
...
| 형태 | UTF-8 string |
| .NET 표기 | `string` |
| 공개 구성 | UTF-8 encoded 크기 1..255 bytes이며 대소문자를 구분해 그대로 비교하는 문자열이다. MeshName, Spot kind와 stable type은 ID에 포함되지 않는다. |
...
public readonly record struct SpotRef(
    ...
    string MeshName,         // 이 특정 incarnation이 속한 Mesh
...
Actor ID로 식별되고 자신의 상태와 mailbox를 갖는 실행 객체다. Spot 소속과 lifecycle은 [Actor 모델](../03-spot-actor/04-actor-model.ko.md)이 소유한다.

| 항목 | 내용 |
|---|---|
| 형태 | 논리 실행 객체 |
| .NET 표기 | `IZLinkActor`; 선언은 [Actor interface](../languages/dotnet/interfaces/06-actors.ko.md)를 따른다. |
| 공개 구성 | Actor ID와 incarnation을 식별하는 ref·context. 전체 member 목록은 Actor interface가 소유한다. |
| 생성·관리 | 등록한 factory를 통해 runtime이 생성·관리한다. Identity 범위는 Actor 모델을 따른다. |
| 전달 | Actor message는 global Actor ID로 주소를 지정한다. 객체 자체를 wire로 보내지 않는다. |
| 수명 | 생성부터 해당 logical incarnation의 종료까지다. 이동 중 연속성은 Actor lifecycle 계약을 따른다. |
| Application 권한 | Application은 Actor 구현·handler를 등록하고 public manager와 client로 사용한다. |
...
| Mode | 실행 계약 |
|---|---|
| `SpotWide` | Gate·turn·Yield는 [실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)을 따른다. |
| `PerActor` | 같은 실행 계약의 mode별 범위를 따른다. |
...
Mode별 relocation 절차와 `PerActor` target에서 Spot instance를 다시 만드는 방법은
[Spot 모델 「5. User Spot」](../03-spot-actor/01-spot-model.ko.md#5-user-spot)이 소유한다.
...
`ApplicationSignaled`에서 `Defer()`는 relocation을 요청하지 않는다 — 현재 host에 준비된
relocation이 있으면 그 경계를 사용하고, 없으면 같은 owner에서 계속한다.
`FrameworkManaged`, `PerActor`, Entry Spot과 Instance Spot에서 `Defer()`를 호출하면
queue를 바꾸기 전에 `InvalidOperation`으로 실패한다.

`Defer()` 뒤 `OnRelocationReadyCompleted` callback 호출 시점과 기본 no-op 구현은
[Spot 모델 「5.1 SpotWide relocation 경계」](../03-spot-actor/01-spot-model.ko.md#51-spotwide-relocation-경계)가
소유한다.
...
Instance Spot cold activation에 필요한 activation envelope와 relocation 뒤 완료되는
request의 reply payload·완료 결과를 opaque byte payload로 보관하는 저장소다. Location
Store가 관리하는 authority는 다루지 않으며, Location Store에 게시하기 전의 payload와
이미 게시된 payload만 Framework가 발급한 reference로 저장한다.

| 항목 | 내용 |
|---|---|
| 형태 | Distributed provider capability |
| .NET 표기 | `IZLinkRelocationStore`; `Put`·`Read`·`Renew`·`Delete` operation을 제공하는 provider interface다. |
| 공개 구성 | Framework가 발급한 opaque reference로 식별하는 immutable payload, retention과 provider 기준 만료 시각(`StoreNow`)을 관리한다. |
| 생성·관리 | Provider package가 SPI를 구현하고 Framework가 등록된 provider instance를 호출한다. Application은 이 SPI를 직접 호출하지 않는다. |
| 수명 | Framework가 지정한 retention 동안 유지되며 `Renew`로 연장하거나 `Delete`로 명시적으로 제거한다. |
...
| 생성·관리 | Application이 factory 등록 시 지정한다. 언어 class 이름(namespace를 포함한 전체 이름)을 자동으로 사용하지 않는다. |
...
| 수명 | 같은 logical incarnation 동안 유지된다. Same-node Join, cross-node relocation과 `RecreateOnRelocation`에서도 바꾸지 않는다. Logical incarnation을 종료한 뒤 새 object를 만들 때 새 값을 발급한다. 최대값 뒤에는 값을 처음으로 되돌리지 않고 `GenerationExhausted`로 실패한다. |
...
`ObjectGeneration`, `AuthorityOwnerGeneration`, `StoreVersion`과 현재 owner lease를
...
정해진 범위의 신규 application 작업 수락을 닫는 lifecycle 동작이다.
[Host relocation §14](../05-location-relocation/05-host-relocation-flow.ko.md#14-shutdown과-relocate의-경쟁)가
Shutdown의 host seal, Relocate의 unit seal과 이미 수락한 작업의 처리를 소유한다.
실행 순서를 정하는 [execution gate](#execution-gate)나 용량을 제한하는 permit과 다른 판정이다.
...
| 공개 구성 | 해당 operation의 `ObjectGeneration`, `AuthorityOwnerGeneration`, `OwnerId`와 `OwnerLeaseGeneration`을 current authority의 값과 모두 일치하는지 비교한다. Operation에 따라 expected `StoreVersion`도 함께 검사한다. |
...
| `Failed` | Node 종료, timeout 또는 callback exception 때문에 정상적인 승인·거절 결과를 만들지 못했다. Typed creation failure를 포함한다. | Ready authority와 active capacity를 만들지 않고 reserved capacity를 반환한다. |
...
각 단계의 순서, 경쟁하는 target 사이의 조정과 sequence diagram은
[Framework API 「16. Missing object 생성 — cold activation 순서」](06-framework-api.ko.md#16-missing-object-생성--cold-activation-순서)가
소유한다.
...
| 수명 | Registration부터 Join terminal과 completion ordering이 끝날 때까지 유지한다. Location commit 전 process가 종료되면 이 barrier 자체를 다시 만들지 않는다. |
...
| 수명 | Object Server registration 동안 유지된다. Creation attempt에서 at-least-once 실행될 수 있으므로 다시 시도해도 안전해야 한다. |
...
| 생성·관리 | Location Store provider가 successful `Reserve`에서 발급한다. Commit과 Abort는 같은 fence가 일치하는지 비교한다. |
| 수명 | 해당 Creating authority가 Ready로 commit되거나 그 abort로 정리될 때 닫힌다. |
...
같은 실행 범위의 callback이 동시에 상태에 접근하지 않도록 실행 권한을 정하는 runtime 기구다. 적용 범위는 [Handler turn과 execution gate §2](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)가 소유한다.

| 항목 | 내용 |
|---|---|
| 형태 | Runtime 실행 권한 |
| .NET 표기 | public type 없음 |
| 공개 구성 | 공개 field 없음. Callback 실행 순서로 관찰한다. |
| 생성·관리 | Runtime이 실행 mode에 맞게 관리한다. ID로 전달하는 값이 아니다. |
| 전달 | Wire로 전달하지 않는 process-local 실행 문맥이다. |
| 수명 | 실행 객체의 수명 안에서 사용한다. Turn과 Yield의 권한 이전은 실행 계약 §2·§3을 따른다. |
| Application 권한 | Application은 gate를 직접 만들거나 획득하지 않고 등록 옵션과 public call을 사용한다. |

<a id="handler-turn"></a>
### Handler turn

Handler가 execution gate를 얻어 실행하고 그 권한을 반납하기까지의 구간이다. Spot뿐 아니라 Node·Channel·Actor·Session의 실행 문맥을 설명할 때 사용한다.

| 항목 | 내용 |
|---|---|
| 형태 | Callback 실행 구간 |
| .NET 표기 | public type 없음 |
| 공개 구성 | 공개 field 없음. Handler와 continuation의 실행으로 관찰한다. |
| 생성·관리 | Runtime이 해당 handler의 실행 권한에 연결한다. |
| 전달 | Message field가 아니라 process-local 실행 문맥이다. |
| 수명 | 시작·대기·반납·재개의 경계는 [실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)을 따른다. |
| Application 권한 | Application은 handler와 허용된 terminal을 사용하며 turn 식별자를 만들거나 변경하지 않는다. |

<a id="spot-turn"></a>
### Spot turn

Spot callback을 실행하는 [handler turn](#handler-turn)이다. Gate 공유 범위와 Yield에 따른
turn 경계는 [실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)이 소유한다.
...
| 공개 구성 | 둘 다 같은 request 결과를 반환한다. Call별 제공 범위는 [실행 계약 §16](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-call-eligibility)을 따른다. |
| 수명 | 한 fluent request call에서 terminal method 하나만 실행할 수 있다. |

Gate와 [Actor queue claim](#actor-queue-claim)의 수명은
[실행 계약 §3](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-gate-and-claim)을 따른다.

<a id="source-local-admission"></a>
### Source-local admission

송신 runtime에서 해당 operation family의 queue가 작업을 수락하는 경계다. Remote handler의 처리 결과와 one-way 완료를 구별하기 위한 용어다.
...
확정된 Framework operation의 완료 callback을 새 실행 구간에 전달하는 주체다. 완료 경쟁을 판정하는 pending registry와 구별한다.

| 항목 | 내용 |
|---|---|
| 형태 | Process 공유 runtime 실행 주체 |
| .NET 표기 | public type 없음 |
| 공개 구성 | 공개 field 없음. Callback의 실행·완료로 관찰한다. |
| 생성·관리 | Runtime이 process 단위로 관리한다. Native completion owner와는 다른 Framework service operation 책임이다. |
| 전달 | Wire로 전달하지 않는다. 확정된 callback을 runtime 내부에서 받는다. |
| 수명 | 자리 예약과 callback 반환의 경계는 [Submit과 completion §11](../01-execution/01-submit-and-completion.ko.md#11-완료-callback의-execution-turn-구현)이 소유한다. |
| Application 권한 | Application은 dispatcher를 직접 구성하거나 게시하지 않고 public operation의 결과를 받는다. |

<a id="backpressure"></a>
### Backpressure

Core가 local byte HWM과 remote receive-flow 상태를 반영해 새 송신 admission을 제한하는
흐름 제어다. Local HWM 값과 remote의 PAUSED/RUNNING 상태는 서로 다른 입력이다.
그 합성은 [Core socket receive-flow 계약](../../../../../../../core/doc/spec/core/socket/README.ko.md#zlink_socket_set_receive_flow_state)을,
Framework permit pressure의 전달은 [Application job queue §6](../01-execution/04-application-job-queue-and-backpressure.ko.md#6-pressure-상태와-socket-제어)을 따른다.

One-way call의 대기·완료는 [Submit과 completion §4·§5](../01-execution/01-submit-and-completion.ko.md#4-one-way-submit--admission-경계)가 소유한다.
대기 중의 내부 상태를 [Backpressured](#backpressured)라고 하며 public terminal result는 아니다.
...
| 수명 | Spot handler registry lifecycle 동안 유지되며 같은 Spot에서 같은 key를 중복 등록할 수 없다. |

<a id="publish-target-snapshot"></a>
### Publish target snapshot

Publish 대상 집합을 나타내는 [Snapshot](#snapshot)이다. 생성 시점과 target 선택은
[Spot 메시징 §4](../03-spot-actor/02-spot-messaging.ko.md#4-channel-범위-logical-multicast)가 소유한다.
...
컴포넌트 하나의 mutable 상태를 읽고 쓰는 코드를 직렬화하는 실행 단위다. Handler 업무를 분류하는 application·lifecycle lane과 다르다.

| 항목 | 내용 |
|---|---|
| 형태 | 컴포넌트 내부 상태 소유 primitive |
| .NET 표기 | public type 없음 |
| 공개 구성 | 공개 field 없음. 공통 내부 진입점은 [상태 소유와 state lane §7](../01-execution/06-state-ownership-and-lanes.ko.md#7-언어별-매핑)을 따른다. |
| 생성·관리 | 상태를 소유하는 컴포넌트가 관리한다. 여러 collection이 같은 불변식에 속하면 같은 소유 경계를 따른다. |
| 전달 | Wire로 전달하지 않는 process-local 실행 단위다. |
| 수명 | 컴포넌트가 상태 접근을 받는 동안 유효하며 종료·재진입 처리는 state lane 계약을 따른다. |
| Application 권한 | Application이 직접 lane을 선택하는 public API가 아니다. |
...
| Application 권한 | Application은 메시지·timer API를 사용하며 내부 lane을 직접 변경하지 않는다. |
...
| Application 권한 | Application은 lifecycle API와 callback을 사용하며 내부 lane을 직접 변경하지 않는다. |

<a id="yield-debt"></a>
### 양보 부채

Lifecycle 작업이 연속으로 선택된 뒤 application turn을 우선할 필요를 나타내는 실행 객체별 상태다. Public `Yield` call과는 다른 scheduler 개념이다.

| 항목 | 내용 |
|---|---|
| 형태 | Runtime 공정성 상태 |
| .NET 표기 | public type 없음 |
| 공개 구성 | 공개 field 없음. 상한과 해제 조건은 [실행 계약 §7](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-lanes)이 소유한다. |
| 생성·관리 | Scheduler가 각 실행 객체의 lane 선택에서 관리한다. |
| 전달 | Wire·public snapshot으로 전달하지 않는다. |
| 수명 | Lifecycle 연속 실행 상한에 도달한 때부터 application turn을 실행해 해제할 때까지의 상태다. |
| Application 권한 | Application이 읽거나 설정하는 값이 아니다. |
...
Spot 업무 payload와 timer callback을 보관하는 application queue다.
작업 분류와 control claim의 별도 lifecycle lane은
[Handler turn과 execution gate §7](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-lanes)이 소유한다.

| 항목 | 내용 |
|---|---|
| 형태 | Framework-owned serialized application queue |
| .NET 표기 | Public queue type 없음 |
| 공개 구성 | Spot direct, matching publish와 timer work item을 보관한다. Actor 업무 payload와 lifecycle control의 분류는 위 실행 계약을 따른다. |
| 수명 | Spot incarnation 동안 유지되며 close·relocation에서는 해당 lifecycle 계약을 따른다. |
...
| 공개 구성 | `PlannedMaintenance`는 source와 같은 effective target version을 사용한다. `RollingUpdate`는 source보다 큰, 지정한 `TargetApplicationVersion`을 함께 지정한다. |
| 수명 | 한 host `Relocate` operation을 시작할 때 고정하며 terminal result에도 같은 mode와 effective target version을 기록한다. |

Mode가 정한 version을 먼저 적용하고 그 뒤 capability, policy, adapter, capacity와
...
connection과 chunk가 실은 identity가 같은지로만 판정한다.
...
| 공개 구성 | [Runtime monitoring](../06-observability/01-runtime-monitoring.ko.md)의 host status 조회·변화 관찰로 노출한다. |
...
| 수명 | 게시 뒤 deployment orchestrator가 `Shutdown` 호출 판단에 사용한다. 게시 전에 `Shutdown`을 호출하는 것도 허용되며, 그 경우 남은 Message Follow route가 transport와 함께 사라져 이전 route를 캐시에 둔 sender의 request는 `Unavailable`로 끝날 수 있다. |
...
| 공개 구성 | Mode와 지정한 target application version을 받는 `RelocateAsync`, 별도 `ShutdownAsync`, host runtime state와 deadline을 사용한다. |
...
[RouteMesh](#routemesh)의 ChannelName별 등록 역할이다.
...
| .NET 표기 | RouteMesh builder의 Client·Server 등록 method로 표현한다. ClientServer monitoring의 `ZLinkClientServerRole`은 ClientServer Channel 문서를 따른다. |
...
| 수명 | MeshNode join·leave와 readiness에 따라 다시 맞추며 각 connection은 독립 lifecycle을 가진다. |
...
| 공개 구성 | Identity, lifecycle, advertised endpoint, state와 현재 owner lease를 공통으로 가지며 topology별 field는 각 descriptor 항목이 정한다. |
...
- Object Server이면 같은 lifecycle에 발급한 Entry Spot ID
...
public sealed record ZLinkMeshNodeDescriptor(
    ...
    string? EntrySpotId,                        // Object Server lifecycle의 Entry Spot ID
...
`0`은 별도 Framework 상한을 사용하지 않고 Core의
`ZLINK_OPT_MAXMSGSIZE = -1`로 변환한다. 양수는 유한한 상한이고 음수는 startup
configuration error다.

이 상한은 server에서 client로 보내는 outbound message에는
적용하지 않는다.

상한을 넘은 message는 일부도 session handler에 전달하지 않으며,
...
| 수명 | Host handler registry lifecycle 동안 유지되며 같은 namespace의 같은 key를 중복 등록할 수 없다. |
...
| 수명 | 해당 message와 handler registration 동안 고정되어야 한다. |
...
| 항목 | 내용 |
|---|---|
| 형태 | Two-frame internal multipart record |
| .NET 표기 | Public type 없음 |
...
| 수명 | Subscriber는 정의된 형식과 일치하는 record만 liveness 신호로 처리한다. Topic이 같지만 payload나 frame 수가 다르면 protocol error이며 application handler로 전달하지 않는다. |
...
현재 request에 한 번 reply할 수 있도록 Framework가 제공하는 권한이다.
Typed handler의 reply 반환과 STREAM의 명시적 Reply call은 이 권한을 서로 다른 표면으로 사용한다.
...
| 수명 | `LeaseExpiresAt`까지 유효하다. 저장된 owner token과 다르거나 store 기준 expiry를 지나면 stale이다. |
...
public abstract record ZLinkOwnerLeaseReadResult
{
    ...
    public sealed record Found(
        ZLinkLocationOwnerToken Token, // 현재 owner identity와 generation
...
| 공개 구성 | 단일 opaque token이다. 내부 encoding은 공개 계약이 아니다. Binding 관계는 바인딩한 그 `ActorRef`, current authority·lease generation, session identity와 binding generation을 함께 검증한다. |
...
Location Store와 Relocation Store는 binding route를 저장하거나 갱신하지 않는다. Actor
relocation 중 target이 route 갱신을 요청하는 절차, 어떤 Actor의 route가 바뀌는지와 다른
Actor의 route·connection이 유지되는 조건은
...
Target runtime이 Location Store CAS와 target queue 개방 뒤 Session owner에게 `[send]`로
전달하는 command 44 route update다. Session owner는 이 update로 binding route를 바꾸고
seal 중 보관한 message를 제출한다. Reply는 없으며 reserved command 45
`sessionRelocationRouted`를 보내거나 수락하지 않는다.

Command 44의 정확한 field, timeout, abort 처리와 Message Follow 조건은
[Session과 Actor binding 「8.2 Control message 42·43·44」](../04-session/02-session-actor-binding.ko.md#82-control-message-424344)가
소유한다.
...
| 수명 | 같은 ObjectGeneration 안에서 owner가 바뀔 때 새 값을 사용한다. 최대값에서 값을 처음으로 되돌리지 않고 `GenerationExhausted`로 실패한다. |
...
| 공개 구성 | Positive generation 값 하나이며 `OwnerId`와 함께 owner token을 구성한다. |
...
| 수명 | 한 STREAM session 안에서만 비교한다. Relocation owner 전환 증거로 사용하지 않는다. |
