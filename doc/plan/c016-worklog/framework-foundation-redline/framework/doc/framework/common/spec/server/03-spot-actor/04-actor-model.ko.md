...
ID별 상태와 mailbox를 갖는 [Actor](../00-foundation/02-glossary.ko.md#actor)가 주소와 상태를 가진 논리 instance인 [Spot](../00-foundation/02-glossary.ko.md#spot) —
...
Entry Spot과 User Spot의 membership 처리 순서는
[Spot과 Actor membership](05-spot-actor-membership.ko.md#3-entry-spot과-user-spot의-actor-membership)을 따른다.
...
Framework는 이 lifecycle 작업을 target Spot의 전용 queue에 넣는다.
Callback의 gate와 turn은 [실행 계약 §2](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)를 따른다.
...
Gate·Yield·Actor claim과 self-request의 관찰은
