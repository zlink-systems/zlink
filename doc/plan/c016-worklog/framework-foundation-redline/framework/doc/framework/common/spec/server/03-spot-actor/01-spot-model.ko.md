...
### 6.2 쓰지 않고 남아 있는 Instance Spot 정리

Framework는 Instance Spot을 쓰지 않고 남아 있는 시간 기준으로 정리할 수 있다.
...
| 조건 | 내용 |
|---|---|
| 쓰지 않고 남아 있는 시간 | 마지막 application 작업 완료 이후 `InstanceSpotIdleTimeout`을 넘겼다. 기본값은 `0`이며 `0`은 정리하지 않음을 뜻한다. |
