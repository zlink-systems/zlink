    ...
    : IZLinkMetadataCall<IZLinkSpotRequestCall>
{
    ...
    // Turn and Yield contexts follow execution contract §2, §3, and §16.
...
public interface IZLinkRequestCall : IZLinkMetadataCall<IZLinkRequestCall>
{
    ...
    // Yield availability follows execution contract §16.
    ValueTask<TReply> Async<TReply>(
        CancellationToken cancellationToken = default);
    ValueTask<TReply> Yield<TReply>(
        CancellationToken cancellationToken = default);
...
Request turn retention, release, and resumption follow
[execution contract §2–§3](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate).
[§16](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-call-eligibility)
owns `Yield` availability by call and outcomes in invalid execution contexts.
...
node. This initially fixed target list is called a [publish target snapshot](../00-foundation/02-glossary.en.md#publish-target-snapshot). Even if
...
A Spot control claim is lifecycle work for the target Spot, separate from Actor business messages.
Gates and turns follow [execution contract §2](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate);
lane classification follows [§7](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-lanes).
...
|---|---|---|
| [Spot application queue](../00-foundation/02-glossary.en.md#spot-application-queue)(the application queue retaining Spot business payloads and timer callbacks) | Spot direct payload, matching Logical Multicast payload, timer callback | Actor business payload, Actor join/leave and lifecycle control callback |
...
[Execution contract §2–§3](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate)
owns callback gates, turns, Yield, and Actor claims. This section describes only message destinations
and state-ownership boundaries.
