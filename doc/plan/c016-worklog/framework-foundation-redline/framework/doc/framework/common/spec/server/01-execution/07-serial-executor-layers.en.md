...
[State ownership and state lanes §7](06-state-ownership-and-lanes.en.md#state-lane-reentrancy)
owns the reentrancy check on waiting state-lane entry.
The table's general `isOnLane` diagnostic assertion is required in debug builds and optional
in release builds; components with actual reentrancy history retain that assertion in release builds.
