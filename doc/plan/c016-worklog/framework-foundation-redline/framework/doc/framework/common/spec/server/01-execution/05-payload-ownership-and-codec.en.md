...
Copies fall into two distinct categories.
...
**The target is zero additional copies introduced by framework.** A copy the binding forces
is accepted as a per-language fact, and the implementation is evaluated by how many more
copies framework adds on top of it.

## 2. Copies That Can Be Eliminated

Framework internals do not create the following copies. Their internal confirmation conditions
measure full-buffer copies and simultaneously retained representations. Views, slices, and object
construction are accounted for separately under §5.

| Type | What it does | Why it can be eliminated |
|---|---|---|
| **Boundary round trip** | Converts `message → byte array → message` | Coming back to the same representation makes the middle step a pure waste |
...
intermediate representations can keep four copies of the same payload resident at once.
...
The internal confirmation condition checks that the normal message path creates no move record before sealing.
...
`binding-forced copy + one deserialization`, and everything in between is the target for
reduction.
...
"One deserialization" is a value on the buffer-copy axis. Counting the objects produced by
deserialization on the same scale as buffer copies makes it impossible to
...
twice adds two copies. Keep public-API immutability while providing a separate no-copy
...
Reading the header first isn't a choice — it's needed to decide which owner to send it to. The
...
Holding the message during relocation preserves it while postponing deserialization until
...
**The entire payload is not parsed twice to determine its format.** Test-parsing the payload
once and then parsing it again for actual processing duplicates both cost and failure points.
The header identifies the format; it is not discovered by test-parsing the body.
...
Codec selection assumes that multiple serializers can be registered at once, so which one to
use must be decided per message. The precise contract for this selection is owned by
[Framework API 「9. Codec」](../00-foundation/06-framework-api.en.md#12-codec). This document covers only
the implementation-side choices that contract leaves open.

The API shape expressing this selection differs per language. One language's API shape does
not define the common structure. However, every language runtime treats send and receive as
different boundaries.

| Direction | Chosen by | If not found |
|---|---|---|
| Send | The message type declared at the call site | Uses the JSON codec |
| Receive | The normalized content-type carried in the envelope | Ends in `ProtocolError` without re-parsing as JSON |
...
receive, the wire's normalized content-type is compared exactly with the registry key. An
...
selection happens is the contract, and its cost is decided by the implementation — the
implementation must follow the rules below.

- **Codec lookup does not construct new strings, arrays, or call objects per message.**
  Allocations made to prepare a lookup also scale with throughput. The internal confirmation
  condition measures per-message allocations for the repeated work below.
...
|---|---|---|
| Receive | The wire's normalized content-type | Since content-type kinds are finite, all results can be precomputed at startup |
...
for frequently used types already in the cache while bounding cache size. The internal confirmation
condition checks the 1,024-type bound, retention of existing entries, and re-evaluation of uncached
types after the bound against this paragraph.
...
Precomputing removes the runtime write and the race that comes with it. The internal confirmation
condition is startup fixation and lock-free reads of the receive codec table, distinct from the
send cache populated at runtime.

**Content-type comparison does not create a new string.** If trimming/normalizing is needed, do
...
only one registered codec, building one every time attaches that allocation to every message.

**When the receive content-type isn't found, it ends in `ProtocolError` rather than falling
back to JSON.** The contract already specifies this behavior. Falling back would try to parse a
...
## 8. Payload Storage and 1:N Child Ownership

- **A record retained by the Framework after receipt has one shared owner for payload storage.**
  This ownership keeps payload available to child and reply processing.
  The [Core socket HWM contract](../../../../../../../core/doc/spec/core/socket/README.en.md#transportbuffer)
  owns the Core byte-credit return boundary, separate from Framework payload-storage and child-permit lifetime.

The application shared permit is returned at each individual target's
...
- **The shared payload owner releases storage exactly once after all children and any required
  record-level reply attempt are terminal.** This boundary prevents releasing storage before its
  last user finishes; it is not a boundary for holding Core credit.
...
For a one-way record requiring no reply attempt, the record becomes terminal when all
...
item leads to one test. Internal allocation, copying, and cache structure are checked in their
own rule paragraphs in §2, §3, and §7.

**Ownership and Release**

- The payload's `close`/`Dispose` or release callback is observed after handler completion.
