...
This document's **[state lane](../00-foundation/02-glossary.en.md#state-lane)** is the execution unit a component uses to own its
mutable state. Every piece of code that reads or writes that component's state runs only on
this lane.

A state lane serializes state access, while the
[application lane](../00-foundation/02-glossary.en.md#application-lane) and
[lifecycle lane](../00-foundation/02-glossary.en.md#lifecycle-lane) classify handler work.
[Handler turn and execution gate §7](02-handler-turn-and-execution-gate.en.md#execution-lanes)
owns the per-execution-object scope of handler FIFOs. That execution object is distinct from
the MeshNode [Owner](../00-foundation/02-glossary.en.md#owner) identified by Location Store authority.
...
- **Reentry that waits for the current turn of the same state lane is rejected at entry.**
  The `throwIfReentrant` check is required regardless of build mode or previous lock implementation.
  Without it, the current turn waits for work queued behind itself and deadlocks.
