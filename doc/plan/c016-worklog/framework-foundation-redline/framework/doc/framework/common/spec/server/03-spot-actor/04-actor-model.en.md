...
An [Actor](../00-foundation/02-glossary.en.md#actor) has per-ID state and a mailbox. Regardless of whether it is in a
...
[Spot and Actor membership](05-spot-actor-membership.en.md#3-actor-membership-for-entry-spot-and-user-spot).
...
The Framework puts lifecycle work on the target Spot's dedicated queue.
Callback gates and turns follow [execution contract §2](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate).
...
- The same Actor's payload runs in Actor-queue acceptance order, regardless
  of ingress kind.
Gate, Yield, Actor-claim, and self-request observations reference
