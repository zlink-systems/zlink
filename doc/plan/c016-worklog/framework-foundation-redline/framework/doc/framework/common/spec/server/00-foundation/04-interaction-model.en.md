...
- **The transaction start is the commit point of the
  [snapshot](02-glossary.en.md#publish-target-snapshot) operation.** Cancellation or shutdown
...
An [execution gate](02-glossary.en.md#execution-gate) grants a callback authority to run now.
The [execution contract §2](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate) owns sharing scope and turns by Spot kind;
[§3](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-gate-and-claim) owns gate release and Actor claims during Yield.

A Node handler does not read a Spot’s mailbox on its behalf. A Spot’s work runs on the Spot’s turn.
...
2. That node confirms the first record in the [durable activation inbox](02-glossary.en.md#durable-activation-inbox).
...
New-work admission is a lifecycle decision separate from handler execution order.
[Host relocation §14](../05-location-relocation/05-host-relocation-flow.en.md#14-the-race-between-shutdown-and-relocate)
owns the Shutdown host [admission seal](02-glossary.en.md#admission-seal) and the Relocate unit seal.
Feature-specific selection, placement, and existing-owner routing follow
[§15 of that document](../05-location-relocation/05-host-relocation-flow.en.md#15-admission-per-state).
This document therefore does not combine `Relocating` and `Draining` into one admission condition.
...
Public shutdown and relocation observations reference
