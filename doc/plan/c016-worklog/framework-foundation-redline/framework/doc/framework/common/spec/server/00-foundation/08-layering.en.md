...
- **The host invokes the lifecycle procedure in [Host relocation §14](../05-location-relocation/05-host-relocation-flow.en.md#14-the-race-between-shutdown-and-relocate),
  and each topology closes its own resource on that call. Calling it
...
owns cleanup stages and closing-callback timing. This section assigns responsibility for closing
resources without redefining the host's sequence of stages.
...
| Node [lifecycle generation](02-glossary.en.md#lifecycle-generation) | An opaque equality token distinguishing node executions — see the glossary for form, lifetime, and comparison |
