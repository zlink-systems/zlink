    ...
    : IZLinkMetadataCall<IZLinkSpotRequestCall>
{
    ...
    // Turn과 Yield 문맥은 실행 계약 §2·§3·§16을 따른다.
...
Request의 turn 유지·반납·재개는
[실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)을 따른다.
`Yield`의 call별 제공 범위와 잘못된 실행 문맥의 결과는
[§16](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-call-eligibility)이 소유한다.
...
고정한 대상 목록을 [publish target snapshot](../00-foundation/02-glossary.ko.md#publish-target-snapshot)이라 한다. Publish 도중 참여 node가 바뀌어도 이
...
Spot control claim은 target Spot의 lifecycle 작업이다. Actor 업무 message와 섞지 않는다.
Gate와 turn은 [실행 계약 §2](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate),
lane 분류는 [§7](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-lanes)을 따른다.
...
|---|---|---|
| [Spot application queue](../00-foundation/02-glossary.ko.md#spot-application-queue)(Spot 업무 payload와 timer callback을 보관하는 application queue) | Spot direct payload, 일치한 Logical Multicast payload, timer callback | Actor 업무 payload, Actor join·leave와 lifecycle control callback |
