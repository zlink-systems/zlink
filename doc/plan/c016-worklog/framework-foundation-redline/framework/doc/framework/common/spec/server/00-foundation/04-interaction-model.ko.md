...
Callback이 지금 실행할 권한을 [execution gate](02-glossary.ko.md#execution-gate)라고 한다.
Spot 종류별 공유 범위와 turn은 [실행 계약 §2](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate),
Yield의 gate 반납과 Actor claim은 [§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-gate-and-claim)을 따른다.

Node handler가 Spot의 mailbox를 대신 읽어 처리하지 않는다. Spot의 일은 Spot의 차례에서 실행된다.
...
2. 그 node가 최초 record를 [durable activation inbox](02-glossary.ko.md#durable-activation-inbox)에 확정한다.
...
신규 작업 수락 여부는 handler 실행 순서와 별개의 lifecycle 판정이다.
[Host relocation §14](../05-location-relocation/05-host-relocation-flow.ko.md#14-shutdown과-relocate의-경쟁)가
Shutdown의 host [admission seal](02-glossary.ko.md#admission-seal)과 Relocate의 unit seal을 소유한다.
기능별 selection·placement·기존 owner route의 처리는
[같은 문서 §15](../05-location-relocation/05-host-relocation-flow.ko.md#15-state별-admission)를 따른다.
따라서 이 문서는 `Relocating`과 `Draining`을 하나의 수락 조건으로 합치지 않는다.
