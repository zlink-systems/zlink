...
[Application job queue §3](../01-execution/04-application-job-queue-and-backpressure.ko.md#3-ordinary-ingress-permit-순서)를 따른다.
이 infrastructure 작업은 application handler가 점유할 수
...
4. 여러 target이나 중복 message가 경쟁해도 이 reservation을 먼저 확보한 runtime만 factory와 initialize를
   실행하고, activation envelope의 message를 [durable activation inbox](02-glossary.ko.md#durable-activation-inbox)의 첫 record로 확정한다.
...
`Yield`의 call별 제공 범위와 실행 문맥은
[Handler turn과 execution gate §16](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-call-eligibility),
gate·Actor claim의 수명은 [같은 문서 §3](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-gate-and-claim)이 소유한다.
이 API 문서는 별도 eligibility나 실행 규칙을 정의하지 않는다.
