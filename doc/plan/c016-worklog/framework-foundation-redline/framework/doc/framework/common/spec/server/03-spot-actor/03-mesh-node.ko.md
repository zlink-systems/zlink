...
[Spot·Actor 주제 목차](README.ko.md) · [스펙 목차](../README.ko.md) · [이전: 02. Spot 메시징](02-spot-messaging.ko.md) · [다음: 04. Actor 모델](04-actor-model.ko.md)

> RouteMesh에 참여하는 MeshNode의 identity, object role, object를 배치할 수 있는 조건과
> startup 순서를 정의한다.

## 1. MeshNode 개요

[MeshNode](../00-foundation/02-glossary.ko.md#meshnode)는 다른 node와 물리적으로 연결되고 Channel membership을 제공한다. Actor와
Spot은 이 연결을 사용하지만 MeshNode RID를 자신의 logical identity로 사용하지 않는다.
Framework는 Actor와 [Spot](../00-foundation/02-glossary.ko.md#spot)의 전역 logical identity를 현재 owner가 존재하는 MeshNode의
route로 연결한다.
...
| 항목 | 의미와 변경 가능 여부 |
|---|---|
| [`MeshName`](../00-foundation/02-glossary.ko.md#meshname)(하나의 RouteMesh 물리 연결 그룹을 식별하는 이름) | 어떤 물리 [RouteMesh](../00-foundation/02-glossary.ko.md#routemesh)와, MeshNode가 RouteMesh의 automatic discovery에서 자신의 identity와 접속 정보를 알리기 위해 Location Store에 게시하는 등록 정보인 [MeshNode descriptor](../00-foundation/02-glossary.ko.md#meshnode-descriptor) namespace에 속하는지를 정한다. MeshNode가 시작된 뒤에는 바꿀 수 없다. |
| [Routing ID](../00-foundation/02-glossary.ko.md#routing-id)(`RID`) | 같은 RouteMesh 안에서 MeshNode를 식별하는 byte 값이며, 현재 MeshNode lifecycle을 식별하는 transport identity다. |
| Endpoint | 다른 peer가 이 MeshNode의 ROUTER에 연결할 주소다. |
| [`ChannelName`](../00-foundation/02-glossary.ko.md#channelname)(message를 보낼 Channel 범위를 식별하는 이름) set | Server role로 참여하는 Channel 목록이다. 0개 이상 등록할 수 있으며 시작된 뒤에는 바꿀 수 없다. |
| Object role | `None`, `Client`, `Server` 중 하나다. Startup 전에 정한다. |
| [Lifecycle generation](../00-foundation/02-glossary.ko.md#lifecycle-generation) | 같은 logical server/listener가 재시작되었을 때 이전 실행과 현재 실행을 구분하는 값이며, 같은 transport identity에서 서로 다른 lifecycle을 구분하는 0이 아닌 식별 값이다. |
| [Descriptor](../00-foundation/02-glossary.ko.md#descriptor) revision | 같은 lifecycle 안에서 바뀔 수 있는 [MeshNode descriptor](../00-foundation/02-glossary.ko.md#meshnode-descriptor) 내용을 구분하는 0이 아닌 값이다. |
...
`MeshName`은 object를 처음 배치할 Mesh와 현재 [owner](../00-foundation/02-glossary.ko.md#owner)에게 도달할 물리 route를 나타내는
속성이다.
...
descriptor를 공개한 뒤에는 다음 설정을 바꿀 수 없다.

- Channel [membership](../00-foundation/02-glossary.ko.md#membership)
- Object role
- 등록된 stable type에 맞는 Spot instance를 생성하는 application 제공 코드인 [Factory](../00-foundation/02-glossary.ko.md#factory)
...
| `Server` | `Client`가 제공하는 기능을 모두 포함한다. | 등록한 type의 object와 Entry Spot을 실행한다. | 등록한 type과 다른 placement 조건이 맞으면 후보가 된다. |
...
Object Client는 Spot·Actor factory와, application이 MeshName과 target RID를 함께
지정해 특정 MeshNode로 message를 보내는 방식인 [Node direct](../00-foundation/02-glossary.ko.md#node-direct) handler를 등록할 수
...
- UTF-8 `1..255` bytes의 [stable type](../00-foundation/02-glossary.ko.md#stable-type)
...
Stable type은 대소문자를 구분해 그대로 비교하는 값이다. Framework는 normalization을
적용하지 않으며 언어의 class 이름(namespace를 포함한 전체 이름)을 wire나 Store identity로 사용하지 않는다. 같은
...
| 항목 | 계약 |
|---|---|
| Placement [weight](../00-foundation/02-glossary.ko.md#weight) | 범위는 `0..10000`, 기본값은 `100`이다. Channel weight와는 별개다. 범위 밖 값은 startup 설정과 runtime 변경에서 configuration error다. |
...
2. Location Store가 필요한 role이면 host [owner lease](../00-foundation/02-glossary.ko.md#owner-lease)를 확보하고 automatic RID의
   MeshNode descriptor owner CAS를 완료한다. Object Server role이면 이 단계에서 Entry Spot
   ID를 발급하고 instance를 초기화한다. Initialization이 끝나기 전에는 descriptor와
   resolver에 Entry Spot을 공개하지 않는다.
3. ROUTER를 bind한 뒤 다른 peer에 알릴 실제 endpoint를 확정한다.
4. 필요한 정보를 모두 포함한 MeshNode descriptor를 공개하고 어떤 peer와 연결해야
   하는지 계산한다.
5. 연결한 remote node의 MeshName, RID, lifecycle, descriptor, object role과
   security identity를 검사해 ready peer로 받아들일지 결정하는 과정인
   [Peer admission](../00-foundation/02-glossary.ko.md#peer-admission), local handler와
   object runtime 준비를 마친 뒤 `Serving` 상태와 신규 target selection을 공개한다.
...
- `MeshName`
- RID
- [Lifecycle generation](../00-foundation/02-glossary.ko.md#lifecycle-generation)
- [Descriptor revision](../00-foundation/02-glossary.ko.md#descriptor-revision)
...
방법은 [§7.3](#73-메시징-방식별-target-선택)이 정의한다 — 이 그림은 그 아래에 있는
물리 층만 그린다.

```mermaid
sequenceDiagram
    participant A as MeshNode A (작은 RID)
    participant B as MeshNode B (큰 RID)
...
Peer는 현재 generation에서 더 큰 revision이 붙은 전체 weight [snapshot](../00-foundation/02-glossary.ko.md#snapshot)만 적용한다.
Weight 변경은 connection을 다시 만들거나 application message를 다시 적용하지 않으며,
...
[Node direct](../00-foundation/02-glossary.ko.md#node-direct)는 지정한 `MeshName`과 RID가 operation 의미에 포함되는 infrastructure,
진단 또는 manual topology에 사용한다. 여러 node가 같은 기능을 제공하는 application
request에는 [ChannelName](../00-foundation/02-glossary.ko.md#channelname)을 사용한다.
...
제공한다. Missing [Instance Spot](../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot)에서만 [Spot direct](../00-foundation/02-glossary.ko.md#spot-direct) fluent call의 Instance intent에
...
Application payload는 실행 객체의 application turn에서 처리한다.
Service-wire control과 Core completion의 수신 경계는
[Application job queue §3](../01-execution/04-application-job-queue-and-backpressure.ko.md#3-ordinary-ingress-permit-순서),
binding completion과의 합성은 [같은 문서 §7](../01-execution/04-application-job-queue-and-backpressure.ko.md#7-send-completion과의-합성)을 따른다.
...
Spot [Logical Multicast](../00-foundation/02-glossary.ko.md#logical-multicast)는 `(ChannelName, topic filter)` [subscription](../00-foundation/02-glossary.ko.md#subscription)을 node-local로
...
신규 작업의 target 선택과 admission은 [Host relocation §14](../05-location-relocation/05-host-relocation-flow.ko.md#14-shutdown과-relocate의-경쟁)·[§15](../05-location-relocation/05-host-relocation-flow.ko.md#15-state별-admission)를 따른다.

아직 relocation permit을 얻지 못한 unit의 existing owner message와 timer는 계속
처리한다. 모든 unit이 source dispatch에서 분리되면 host는 `Relocated`가 된다.
`Draining`은 `Shutdown`이 admission을 닫을 때만 진입하며, 두 상태의 전이는
[Host relocation §3](../05-location-relocation/05-host-relocation-flow.ko.md#3-host-state와-완료-결과)이 소유한다.
...
순서는 [Host relocation와 shutdown](../05-location-relocation/05-host-relocation-flow.ko.md)이 정의한다.

Runtime이 종료를 진행 중이어서 새 operation admission을 받을 수 없는 상태인
[`Shutdown`](../00-foundation/02-glossary.ko.md#shutdown)은 새 relocation을 시작하지 않는다. `Relocate`는 등록한 relocation policy에
...
[Runtime 상태와 운영 진단](../06-observability/01-runtime-monitoring.ko.md)이 정의한다.

## 10. 검증 요구

구현과 contract test는 공개 표면(MeshNode builder, startup 결과, MeshNode descriptor,
runtime snapshot과 event)만으로 다음을 확인한다.
