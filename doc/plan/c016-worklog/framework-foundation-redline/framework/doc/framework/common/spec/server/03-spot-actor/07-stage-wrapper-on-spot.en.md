...
Execution-mode registration follows the [Spot model](01-spot-model.en.md).
[Execution contract §2](../01-execution/02-handler-turn-and-execution-gate.en.md#execution-gate)
owns gates, turns, and continuations;
[§3](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-gate-and-claim) owns Yield and Actor claims.

- **A wrapper does not change this execution contract through a separate scheduler or lock rule.**
  Wrapping a Framework callback must preserve the same execution order.
- **The application owns synchronization of shared Stage state in `PerActor`.** Callbacks on
  different gates permitted by the execution contract can access the same domain state.
...
Stage timer registration and tick processing follow the public [Spot timer](10-spot-timer.en.md) contract.
...
[execution contract §16](../01-execution/02-handler-turn-and-execution-gate.en.md#yield-call-eligibility).
...
Execution order, continuation, Yield, and reentrancy observations for wrapper callbacks reference
[execution contract §16](../01-execution/02-handler-turn-and-execution-gate.en.md#16-verification-requirements).
