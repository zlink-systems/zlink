...
[Channel·Transport topic table of contents](README.en.md) · [Spec table of contents](../README.en.md) · [Previous: 03. ClientServer Channel](03-client-server-channel.en.md) · [Next: 05. Transport Liveness](05-transport-liveness.en.md)

> Defines the contract for when a network listener has two different addresses
> for different purposes, and the policy for issuing transport RID and Spot ID
> system-wide.

## 1. Listener Address Overview

A [network listener](../00-foundation/02-glossary.en.md#network-listener) (a transport
component in the current process that binds a network endpoint and accepts
remote connections) may need two addresses for different purposes.
...
other languages. The precise .NET signature is defined by the
...
remote process connects to, specify
[`BindHost`](../00-foundation/02-glossary.en.md#bindhost) (the address that says which
local network interface on the current host the listener accepts connections
on) and
[`AdvertiseHost`](../00-foundation/02-glossary.en.md#advertisehost) (the host/address a
remote process actually uses to connect to this listener)
...
An override on one [RouteMesh](../00-foundation/02-glossary.en.md#routemesh) listener
...
### 2.2 Wildcard Address

`0.0.0.0` and `::` are
[wildcard addresses](../00-foundation/02-glossary.en.md#wildcard-address) for accepting
...
```mermaid
flowchart LR
    A["BindHost and configured port<br/>(the value the local socket binds to)"] -->|bind the listener| B["confirm the actual bound port"]
    B -->|combine with AdvertiseHost| C["advertised endpoint remote will use<br/>(the value the remote peer sees)"]
...
If port is omitted, an
[automatic discovery](../00-foundation/02-glossary.en.md#automatic-discovery) (finding a
remote endpoint and identity by querying a descriptor published to the
Location Store) listener uses port `0`. The
...
The confirmed
[advertised endpoint](../00-foundation/02-glossary.en.md#advertised-endpoint) is only
recorded in the [descriptor](../00-foundation/02-glossary.en.md#descriptor) or
configuration that matches the listener kind, such as a RouteMesh
...
| [Classic fanout](../00-foundation/02-glossary.en.md#classic-fanout) publisher | Records the endpoint in the [fanout publisher descriptor](../00-foundation/02-glossary.en.md#fanout-publisher-descriptor) identified by [ChannelName](../00-foundation/02-glossary.en.md#channelname) and Publisher RID. | Not recorded in the MeshNode or ClientServer Server descriptor. |
...
[automatic discovery](../00-foundation/02-glossary.en.md#automatic-discovery) publishes a
...
MeshNode RID, the Framework [diagnostic prefix](../00-foundation/02-glossary.en.md#routing-id-prefix), and Entry
[Spot ID](../00-foundation/02-glossary.en.md#spot-id) (the global logical address that
identifies a Spot)
issuance — not an individual listener's configuration.

Core has no separate public type called `NID`. A
[Routing ID](../00-foundation/02-glossary.en.md#routing-id) is a 1..255-byte binary-safe
...
- **A UUID conflict isn't treated as a normal operating situation.** If
  [an active conflict is confirmed](../00-foundation/02-glossary.en.md#routing-id-conflict), the Framework doesn't change the
  existing descriptor, doesn't attempt a new UUID claim, and ends
  immediately with a startup configuration error, because if a stale
...
Confirm the following using only the network configuration API, the
listener state query, the endpoint recorded in the
...
- The listener endpoint isn't duplicated into a
  [Spot](../00-foundation/02-glossary.en.md#spot)/Actor location row.
