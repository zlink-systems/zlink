...
> 이 문서는 [Spot](../00-foundation/02-glossary.ko.md#spot)이 등록하는 반복·지연 timer의
> tick 정보, cancel과 overrun 계약을 정의한다. Callback의 실행 권한인
> [execution gate](../00-foundation/02-glossary.ko.md#execution-gate)와 turn은
> [실행 계약 §2](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate),
> Yield는 [§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#yield-gate-and-claim)이 소유한다.

## 1. Timer generation과 cancel

Service runtime은 platform timer의 만료를 callback work로 변환한다. Callback의 mode별
실행은 위 실행 계약을 따르며, backend와 관계없이 아래 timer 고유 의미를 유지한다.
...
Callback의 gate 선택·획득·반납은
[실행 계약 §2·§3](../01-execution/02-handler-turn-and-execution-gate.ko.md#execution-gate)을 따른다.

- **Timer가 실행 권한을 얻지 못하면 해당 tick을 보관하고 다음 실행 기회에 다시 시도한다.**
  아직 실행하지 않은 tick을 잃지 않기 위한 timer 고유 보관 규칙이다.
