...
This section owns the cold-activation sequence and first-message preservation for
calls with Instance intent. Per-language interfaces project marker overloads, stable
types, and completion types and refer here for the common activation procedure.
...
10. If the Store has no owner and the target also has no currently usable
    ...
    same transaction it records the [recovery receipt](../00-foundation/02-glossary.en.md#recovery-receipt) confirming the link between the recovery root and creation reservation, provider-issued
...
11. Only the target that succeeds this reservation runs factory and
    ...
    message is confirmed as the first record of the [durable activation inbox](../00-foundation/02-glossary.en.md#durable-activation-inbox) (an internal confirmation condition — no handler runs before the
