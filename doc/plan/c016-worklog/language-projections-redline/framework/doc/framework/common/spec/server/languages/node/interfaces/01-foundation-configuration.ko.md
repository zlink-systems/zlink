...
중복은 startup 오류다. RouteMesh 역할 단일 선택과 [ChannelName](../../../00-foundation/02-glossary.ko.md#channelname) 충돌 규칙은 바꾸지 않는다.

Automatic [RouteMesh](../../../00-foundation/02-glossary.ko.md#routemesh)는 RID를 canonical byte order로 비교하고 더 작은 RID의 [MeshNode](../../../00-foundation/02-glossary.ko.md#meshnode)만 상대 endpoint로
...
Client는 manual endpoint와 location store [automatic discovery](../../../00-foundation/02-glossary.ko.md#automatic-discovery)를 함께 사용할 수 있다. 두 source가 같은
...
Server와 같은 candidate 집합에 포함한다. [Ready](../../../00-foundation/02-glossary.ko.md#ready), positive weight, non-draining 조건을 동일하게 적용하며
...
Actor adapter를 사용한다. Whole User Spot의 Spot root와 cross-node User·Instance Spot materialization에는 [Spot](../../../00-foundation/02-glossary.ko.md#spot)
...
target을 `"ready"`로 연다. Source cleanup, `"completed"` 기록은 target
...
application payload다. Framework는 payload를 Relocation Store에 기록하지 않고, source memory에서
`relocationPayloadChunkLimitBytes` 이하의 chunk로 나눠 source–target ordered mesh 연결로 직접
전송한다. JavaScript runtime에서
...
폐기한다. 모든 target이 실패하기 전에는 [deadline](../../../00-foundation/02-glossary.ko.md#deadline) 안에서 replacement를 시도할 수 있다. Relocation Store, authority
...
commit한 뒤 target message 처리를 시작한다. Bound Session의 relocation route 갱신은 [Session–Actor binding §8.2](../../../04-session/02-session-actor-binding.ko.md#82-control-message-424344)가 소유한다. `PerActor` Spot policy는 `RecreateOnRelocation`만 허용하고 Spot
adapter를 등록하지 않는다.
...
[Location Store](../../../00-foundation/02-glossary.ko.md#location-store)가 필요하다.
...
Placement [weight](../../../00-foundation/02-glossary.ko.md#weight)는 정수 `0..10000`이고 기본값은 `100`이다. RouteMesh
