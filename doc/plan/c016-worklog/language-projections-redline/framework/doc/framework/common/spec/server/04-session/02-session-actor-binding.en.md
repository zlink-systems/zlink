...
| [Route and location after an Actor moves to another node](../00-foundation/02-glossary.en.md#binding-route-ack) | After a relocation commit, the framework updates the Actor route and bound-session current Actor location snapshot kept in the session to the target. `ActorId`/`ObjectGeneration` are kept. |
...
**Command 44 is submitted once without a response and is not retransmitted at the
application level.** The Session owner's seal timeout owns missing-update handling,
so no separate response wait or retransmission interval is added.
...
and is **left to each language**. The route update applies only to the same
ObjectGeneration and is not a rebind, so no disconnect callback runs (§3). The routes
and physical connections of other Actors on the same Session that are not part of the
relocation are left unchanged. Per-language interfaces project the public ActorRef
query type and refer to this section for the route-update procedure.
