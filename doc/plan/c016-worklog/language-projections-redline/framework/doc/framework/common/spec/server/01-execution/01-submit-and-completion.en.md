...
A call object offers only the terminator that fits its operation kind.
[Actor Model §6.2](../03-spot-actor/04-actor-model.en.md#62-create-and-getorcreate-input) owns Actor Create/GetOrCreate single-use rules,
duplicate-option handling, and terminal re-invocation errors. Per-language interfaces
project the names and types of this contract. For other operations, each per-language
interface defines single-use rules, repeated-option handling, and terminal re-invocation errors.
...
but do not roll back work the remote handler already started. A late reply for a Framework
service operation is not redelivered to a closed correlation. Binding-operation lifetime and native
completion cleanup follow the ownership boundary in
[Cancellation and shutdown §3](03-cancellation-and-shutdown.en.md#3-handling-the-cancellation-race).
...
transport after registering the pending completion entry, a slot in the [completion dispatcher](../00-foundation/02-glossary.en.md#completion-dispatcher) that delivers completion callbacks, and reply route.**
...
Binding cancellation observations reference
