...
[Kotlin 계약 목차](../README.ko.md) | [언어별 interface 목차](../../README.ko.md)
...
Actor·Spot state 보존 adapter는 Java `ZLinkActorRelocationAdapter`와 `ZLinkSpotRelocationAdapter`가 이 규칙을 소유한다. Kotlin은
...
Channel extension은 process-local ChannelName만 받으며 MeshName과 [ChannelName](../../../00-foundation/02-glossary.ko.md#channelname)을 함께 받는 선택 overload를
...
Framework-issued reference 기반 immutable blob 계약도 Java public interface가 이 규칙을 소유한다.
...
한다. Node를 직접 지정하는 extension의 첫 번째 `String` 인자는 Java 계약과 같이 [MeshName](../../../00-foundation/02-glossary.ko.md#meshname)이다.
...
Kotlin 언어별 interface는 Java와 같은 global ActorId·SpotId, immutable `ActorRef`·`SpotRef`, ID-only 일반
messaging과 -ref mutation·session bind를 사용한다. Actor와 User Spot의 create/get-or-create는 single-user
fluent operation이다. [Spot](../../../00-foundation/02-glossary.ko.md#spot) manager는 User Spot 전용이며 Instance Spot creation member를 제공하지 않는다.
Missing [Instance Spot](../../../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot)의 cold activation은 Spot 전용 send/request call에서 `instanceSpot()` 또는
...
[authority](../../../00-foundation/02-glossary.ko.md#authority)는 등록 type 수와 관계없이 저장된 type을 사용한다. Mesh object role은 None, Client, Server로
...
0 또는 `Long.MAX_VALUE` 초과 값은 거부한다. String identity는 value를 유지하며 normalization하지 않는다.
