...
[인터페이스 목차](README.ko.md) · [STREAM session](../../../04-session/01-stream-session.ko.md)
...
Session bind는 `ActorRef`를 한 번 제출한다. Active Message Follow route가 없으면 `NOT_FOUND`,
...
binding generation과 session sequence를 application callback 전에 검사한다. Rebind와 close는 binding
identity transition이다. Identity는 session owner Node RID·lifecycle generation·owner-local [binding generation](../../../00-foundation/02-glossary.ko.md#binding-generation)을
함께 사용하므로 다른 [MeshNode](../../../00-foundation/02-glossary.ko.md#meshnode)나 재시작한 [owner](../../../00-foundation/02-glossary.ko.md#owner)의 작은 local counter도 새 binding으로 등록할 수 있다. 이전 owner
...
binding을 복원하지 않는다. `ZLinkSessionActor.ref()`는 `ActorRef`를 반환한다.
