...
`ZLinkObservedStatus<T>.loss` delivered by `observe(...)` has type
`ZLinkObservationLoss`. `coalescedCount` and `discardedTerminalCount` have type
`long`. [Runtime monitoring §7.2](../../../06-observability/01-runtime-monitoring.en.md#72-coalescing) owns the delivery unit, loss-counter semantics, range and saturation,
