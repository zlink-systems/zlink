...
네 `observe(...)`는 `ZLinkObservedStatus<TStatus>`를 전달하며, `loss`는
`ZLinkObservationLoss`다. `coalescedCount`와 `discardedTerminalCount`의 타입은
`bigint`다. 전달 단위와 유실 counter의 의미·범위·포화 및 구독 유지 계약은
[Runtime monitoring §7.2](../../../06-observability/01-runtime-monitoring.ko.md#72-합치기)가 소유한다. 구독 취소 입력은 `signal`이다.
