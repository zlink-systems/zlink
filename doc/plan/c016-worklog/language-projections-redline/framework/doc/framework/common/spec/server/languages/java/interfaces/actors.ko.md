...
[인터페이스 목차](README.ko.md) · [Actor 공통 계약](../../../03-spot-actor/04-actor-model.ko.md)
...
[Factory](../../../00-foundation/02-glossary.ko.md#factory) registration의 정확한 builder member는
...
remote User·[Entry Spot](../../../00-foundation/02-glossary.ko.md#entry-spot-user-spot과-instance-spot) join과 whole User Spot relocation의 각 Actor participant에 같은 Actor adapter를 사용한다.
...
등록을 제거하고 dispatch를 atomic하게 전환한 뒤 target을 `READY`로 연다. Source cleanup, `COMPLETED` 기록은 target message 처리를 막지 않는다. `READY` 뒤 target process가 종료되면
ordinary [owner](../../../00-foundation/02-glossary.ko.md#owner) loss로 처리하며 이전 relocation을 자동 replay하지 않는다. 이 barrier를 조작하는 public phase API는 제공하지 않는다.
...
확정되지 않은 precommit adapter exception과 contract 위반은 `Blocked/StateIncompatible`로 분류한다. [Deadline](../../../00-foundation/02-glossary.ko.md#deadline)이
...
도달한 뒤에만 `CAPTURED`를 commit한다. Durable accepted journal은 일치하는 owner lease가 있는 source에서만
...
owner, authority owner generation과 current [Spot](../../../00-foundation/02-glossary.ko.md#spot)을
