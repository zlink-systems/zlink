...
`spot_relocation_adapter_t<TSpot>`를 구현해야 한다. Actor adapter를 전달하거나 [Spot](../../../00-foundation/02-glossary.ko.md#spot) factory에 맞지 않는 adapter를
...
[MeshNode](../../../00-foundation/02-glossary.ko.md#meshnode) ROUTER를 통해 remote MeshNode마다 한 번 제출하고, 수신 node는 node-local
subscription만 검사한다. 각 remote ROUTER와 local mailbox는 대상별로 수락하며, 한 대상의
실패가 앞에서 수락된 전송을 취소하지 않는다. Spot·Actor 등록은 [owner](../../../00-foundation/02-glossary.ko.md#owner)
...
그 동작 의미는 [공통 스펙](../../../03-spot-actor/05-spot-actor-membership.ko.md)을 따른다. 구현이나 contract test가
...
먼저 `leave_actor(...)` 또는 Entry Spot join을 완료해야 한다. Destroy는 [membership](../../../00-foundation/02-glossary.ko.md#membership) 이동이 아니므로
`on_leave_actor`를 다시 호출하지 않으며, 같은 Actor instance의 중복 destroy는 lifecycle callback을
추가로 실행하지 않고 성공으로 끝난다. 전체 순서는 [Actor model §6](../../../03-spot-actor/04-actor-model.ko.md#6-actor-lifecycle)을
따른다.
...
[deadline](../../../00-foundation/02-glossary.ko.md#deadline) 때문에 callback을 취소하면 `deadline_exceeded`로 분류한다. Recovery 때문에 두 method가 at-least-once
...
Multicast [subscription](../../../00-foundation/02-glossary.ko.md#subscription) 등록 member가 존재하지 않는다. 같은 MeshNode에서 stable
`instance_spot_type`이나 같은 Spot class를 User Spot factory와 Instance factory에 중복 등록해도 socket bind
전에 설정 오류로 실패한다.

`spot_send_call_t`와 `spot_request_call_t`의 `instance_spot()`은 type을 생략한 marker이고,
`instance_spot(std::string stable_type)`은 type을 지정한 marker다.
`in_mesh(std::string mesh_name)`은 Mesh 입력을 표현한다. Send terminal은
`async()` → `task_t<void>`, request terminal은 `async<TReply>()`·`yield<TReply>()` →
`task_t<TReply>`다. 정확한 call member는 [Channel messaging](03-channel-messaging.ko.md)가 소유한다.
...
Close에서는 `on_closing(context, cleanup_cancellation)`을 한 번 호출하고
...
`spot_context_t::close()`와 `instance_spot_context_t::close()`는 context가 보유한 current
SpotRef를 사용한다.
...
SpotId와 stable type은 UTF-8 `1..255` byte 값이며 trim, case folding과 Unicode normalization을 적용하지
않는다. `spot_id_t`는 UTF-8 encoded 크기 1..255 bytes의 case-sensitive `std::string`이다.
...
authority·Entry membership을 commit한 뒤 Actor message 처리를 시작한다. Bound Session의 relocation route 갱신은 [Session–Actor binding §8.2](../../../04-session/02-session-actor-binding.ko.md#82-control-message-424344)가 소유한다.
...
function을 호출한다. Application은 invoker, service provider, serializer registry와 [descriptor](../../../00-foundation/02-glossary.ko.md#descriptor) 조회 표면을
...
Timer backend 선택은 [비동기 실행 정책](../../../03-spot-actor/10-spot-timer.ko.md)을 따른다.
...
timer 등록 검증은 [stage-wrapper §4.1](../../../03-spot-actor/07-stage-wrapper-on-spot.ko.md)이 소유한다.
...
포함하지 않는다. `Close`는 User Spot의 SpotRef만 변경한다. Instance Spot은
`instance_spot_context_t::close()`가 context에 보관한 current SpotRef로 local close를 수행한다. 같은 User
...
**lifecycle callback의 호출 순서는 [MeshNode §7](../../../03-spot-actor/03-mesh-node.ko.md)가 소유한다** —
