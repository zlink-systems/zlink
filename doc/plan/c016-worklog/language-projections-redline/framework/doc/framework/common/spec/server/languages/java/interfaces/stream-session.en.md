...
[Interface table of contents](README.en.md) · [STREAM Session](../../../04-session/01-stream-session.en.md)
...
Session bind submits an `ActorRef` once. If there's no active
...
the application callback. Rebind and close are specified binding identity
transitions. Since identity uses the session owner Node RID/lifecycle
generation/owner-local
[binding generation](../../../00-foundation/02-glossary.en.md#binding-generation)
together, even a small local counter of a different
[MeshNode](../../../00-foundation/02-glossary.en.md#meshnode) or a restarted
[owner](../../../00-foundation/02-glossary.en.md#owner) can be registered as a new
...
once per specified binding identity. `notifyDisconnected()` is a logical
notification while the connection is kept, and waits for the callback
terminal. The binding callback runs at most once, then the binding is
...
not wait for the previous session. The previous session may notify the
...
doesn't remove the new binding or restore the old one. `ZLinkSessionActor.ref()` returns `ActorRef`.
