#!/usr/bin/env bash
# C four-size baseline/candidate sweep driver with aggregate gating.
#
# Usage:
#   ${ZLINK_WORK}/c016/sweep2.sh [--only single|multi] \
#       [--cells PATTERN:TRANSPORT,PATTERN:TRANSPORT] \
#       [--msg-sizes 64,256,1024,65536] [--retry-failed]
#
...
RESULTS_FILE="${WORK_DIR}/sweep2-results.md"
LOG_FILE="${WORK_DIR}/sweep2.log"
...
while (($#)); do
  case "$1" in
    ...
    --cells=*)
      ...
      ;;
    --msg-sizes)
      (($# >= 2)) || die "--msg-sizes requires a comma-separated size list"
      MSG_SIZES="$2"
      shift 2
      ;;
    --msg-sizes=*)
      MSG_SIZES="${1#--msg-sizes=}"
      shift
      ;;
    --retry-failed)
      RETRY_FAILED=1
...
[[ "$MSG_SIZES" =~ ^[1-9][0-9]*(,[1-9][0-9]*)*$ ]] || die "--msg-sizes must be a comma-separated list of positive integers"

declare -A seen_msg_sizes=()
IFS=',' read -r -a parsed_msg_sizes <<< "$MSG_SIZES"
for msg_size in "${parsed_msg_sizes[@]}"; do
  [[ -z "${seen_msg_sizes[$msg_size]+set}" ]] || die "--msg-sizes contains duplicate size: $msg_size"
  seen_msg_sizes[$msg_size]=1
done
...
append_result() {
  local suite="$1" pattern="$2" transport="$3" verdict="$4" worst="$5" aggregate="$6" baseline_report="$7" candidate_report="$8" summary="$9"
  summary="${summary//$'\n'/ }"
  summary="${summary//|/\/}"
  printf '| %s | %s | %s | %s | %s | %s | %s | %s | %s |\n' \
    "$suite" "$pattern" "$transport" "$verdict" "$worst" "$aggregate" "$baseline_report" "$candidate_report" "$summary" >> "$RESULTS_FILE"
}
...
run_runner() {
  ...
  mkdir -p "$report_dir"
  marker="$(mktemp "$WORK_DIR/.sweep2-report-marker.XXXXXX")"
  output="$(mktemp "$WORK_DIR/.sweep2-runner-output.XXXXXX")"
...
worst_ratio() {
  ...
  awk -F'|' '
    function clean(v) { gsub(/^[ \t]+|[ \t]+$/, "", v); return v }
    /^\|/ && clean($2) ~ /^(single|multi)$/ && clean($5) ~ /^[0-9]+$/ {
      metric=clean($6); ratio=clean($9)
      if (ratio !~ /^[-+]?([0-9]*\.)?[0-9]+$/) next
      if (metric == "throughput" || metric == "bandwidth") score = ratio + 0
      else score = (ratio + 0 == 0) ? 1e308 : 1 / (ratio + 0)
...
aggregate_geomean() {
  ...
  awk -F'|' '
    function clean(v) { gsub(/^[ \t]+|[ \t]+$/, "", v); return v }
    /^\|/ && clean($2) ~ /^(single|multi)$/ && clean($5) ~ /^(throughput|bandwidth|latency|latency_p95|latency_p99)$/ {
      metric=clean($5); geomean=clean($10)
      if (geomean !~ /^[-+]?([0-9]*\.)?[0-9]+$/) next
      if (metric == "throughput" || metric == "bandwidth") score = geomean + 0
      else score = (geomean + 0 == 0) ? 1e308 : 1 / (geomean + 0)
      if (!seen || score < worst) { seen=1; worst=score; result="agg=" metric ":" geomean }
    }
    END { if (seen) print result; else print "agg=-" }
  ' "$gate_output"
}

run_gate() {
  local suite="$1" baseline_report="$2" candidate_report="$3" output
  output="$(mktemp "$WORK_DIR/.sweep2-gate-output.XXXXXX")"
  if [[ "$suite" == "single" ]]; then
  ...
  GATE_AGG="$(aggregate_geomean "$output")"
...
run_cell() {
  local suite="$1" pattern="$2" transport="$3"
  local baseline_tag="sweep2-b-${pattern}-${transport}"
  local candidate_tag="sweep2-c-${pattern}-${transport}"
  ...
  if [[ "$RUN_RC" -ne 0 || -z "$baseline_report" || ! -s "$baseline_report" ]]; then
    append_result "$suite" "$pattern" "$transport" ERROR - agg=- "${baseline_report:--}" - \
  ...
  if [[ "$RUN_RC" -ne 0 || -z "$candidate_report" || ! -s "$candidate_report" ]]; then
    append_result "$suite" "$pattern" "$transport" ERROR - agg=- "$baseline_report" "${candidate_report:--}" \
  ...
  if [[ "$GATE_RC" -eq 0 ]]; then
    append_result "$suite" "$pattern" "$transport" PASS "$GATE_WORST" "$GATE_AGG" "$baseline_report" "$candidate_report" "$summary"
  else
    append_result "$suite" "$pattern" "$transport" FAIL "$GATE_WORST" "$GATE_AGG" "$baseline_report" "$candidate_report" "$summary"
  fi
  sleep 2
}
...
record_unsupported() {
  ...
  append_result "$suite" "$pattern" "$transport" UNSUPPORTED - agg=- - - "expected unsupported: absent from $missing_side --pattern ALL"
