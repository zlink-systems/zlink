...
## 17. 세 언어 send 결과 — C++만 통했다 (2026-09-13)

같은 수정(즉시 수락 send의 completion graph 제거)을 세 언어에 적용했다.

| send 1 KiB 비율 | 기준선 | 수정 후 | 판정 |
|---|---:|---:|---|
| **C++** | 0.064 | **(0.149)** | **2.3배** |
| .NET | 0.104 | 0.101 | 변화 없음 |
| Java | 0.125 | 0.128 | 변화 없음 |

### 17.1 절대값은 셋 다 올랐는데 비율은 C++만 움직였다

| send 1 KiB | 기준선 fw | 수정 후 fw | 기준선 raw | 수정 후 raw |
|---|---:|---:|---:|---:|
| C++ | 52.8 | **122.8** | 822 | **819** |
| .NET | 53.5 | 105.4 | 515 | **1047** |
| Java | ~72 | 90.5 | ~578 | **708** |

.NET framework가 2배(53.5 → 105.4)지만 **raw도 똑같이 2배**(515 → 1047)다. 계층 비용은
그대로다. Java도 같다.

**그 공통 상승은 `#5`/`#6`이 아니다.** 두 행 모두 같은 Core를 쓰고, 그 사이
**`#311`(Core byte HWM 회계 수정)이 머지**됐다. C++만 raw가 안 움직인 것은 C++ 측정이
`#311` 머지 **전**이었기 때문이다.

**즉 `#311`이 성능에도 영향을 준 것으로 보인다.** 결함 수정의 부수 효과이고, 확인하려면
같은 빌드에서 `#311` 전후를 재야 한다.

### 17.2 왜 .NET은 효과가 없었나 — 조사의 "공통"이 틀렸다

`findings-binding-gap.md` §3은 네 언어가 모두 "정상 admission에서도 request state를
만든다"고 적었다. **.NET은 아니었다.**

`bindings/dotnet/src/Zlink/Runtime/Messaging/CompletionOwner.cs:43-100`이 이미 `Ok`에서
registry entry도 completion source도 만들지 않고 `Task.CompletedTask`를 반환한다.
그래서 .NET 2차의 할당 감소는 **0개/message**였고(job이 그렇게 적었다), 측정도 안 움직였다.

| 언어 | 즉시 수락 send의 completion graph | 제거 |
|---|---|---:|
| C++ | 있었음 | 6개 → **2.3배** |
| Java | 있었음 | 6개 → 변화 없음(아래) |
| .NET | **binding이 이미 안 만듦** | 0개 |

**교훈: "네 언어 공통"이라는 조사 결론도 언어마다 코드로 확인해야 한다.**

### 17.3 Java도 효과 없음 — A/B로 확정했다

Java도 dependent `CompletableFuture` 3개 + observer node 3개를 없앴다. 같은 빌드
(`#311` 포함)에서 main과 `#6` 워크트리를 3 run씩 번갈아 쟀다.

| Java send 1 KiB | A (main, `#6` 없음) | B (`#6` 있음) |
|---|---:|---:|
| `zlink-framework-java` | 89.45 KMSG/s | **90.23** |
| `zlink-java` (raw) | 714.13 | 714.62 |

**0.9% 차이 — 측정 오차다.** raw가 거의 동일하므로 조건이 같았고 비교가 성립한다.

§17.1의 추정도 이로써 확인됐다 — 앞서 Java framework가 72 → 90으로 보인 것은 `#6`이
아니라 **`#311`(Core 회계 수정)** 덕이었다. raw도 578 → 714로 같이 올랐다.

### 17.5 결론 — 같은 수정이 언어마다 다르게 작동했다

| 언어 | 즉시 수락 send의 completion graph | 제거 | 결과 |
|---|---|---:|---|
| **C++** | 있었음 | 6개 | **2.3배** (PR #313 머지) |
| Java | 있었음 | 6개 | **효과 없음** (A/B 0.9%) |
| .NET | binding이 이미 안 만듦 | 0개 | 효과 없음 |

C++과 Java가 **같은 수의 할당을 없앴는데 결과가 갈렸다.** 할당 수가 비용의 척도가 아니라는
뜻이다 — C++의 6개는 heap 할당 + coroutine frame + registry 조회였고, Java의 6개는 이미
JIT이 상당 부분 처리하는 `CompletableFuture` 체인이었을 수 있다.

**다음은 Java·.NET에 대해 프로파일링이 필요하다.** §15.6에서 성급히 내렸던 그 결론이
이 두 언어에는 맞았다. C++에는 틀렸다 — 조사가 지목한 2순위가 정확했다.

읽기 기반 조사는 **후보를 좁히는 데는 유효하되, 언어마다 그 후보가 실제 비용인지는
측정으로만 가려진다.** 이번 라운드가 그것을 세 번(C++ 성공, Java·.NET 실패) 보여줬다.
...
## 20. 2라운드 — 프로파일 근거로 다시 시도했다 (2026-09-13)

§18·§19의 표본이 지목한 곳을 고쳤다. 검증은 §17.4의 규율대로 **같은 빌드에서 main(A)과
수정본(B)을 3 run씩 번갈아** 쟀다.

| 언어 | 고친 것 | 표본 근거 | A | B | 결과 |
|---|---|---|---:|---:|---|
| **Java** | 매 send의 peer 분류 stream | `ReferencePipeline$2` **1.287 GB**, 105 표본 | 85.44 | **100.04** | **+17%** |
| .NET | `RunStateAsync`의 closure | lookup delegate **0.204 GB** | 103.80 | 104.21 | 0.4% |

두 경우 모두 raw가 0.3~1.1% 차이로 동일해 조건이 같았다.

### 20.1 Java — 프로파일 근거가 읽기 근거를 이겼다

| 근거 | 변경 | 결과 |
|---|---|---|
| 코드 읽기(§7) | completion graph 6개 할당 제거 | 0.9% |
| **JFR 표본(§18)** | **peer 분류 stream 제거** | **+17%** |

PR **#314**. 캐시를 추가하지 않고 같은 상태를 더 싸게 읽는 방식이라 규칙이 늘지 않았고,
분류 결과도 동일하다.

### 20.2 .NET — 세 번 빗나갔다. 내가 작은 것을 골랐다

.NET은 1차(`_socketGate`), 2차(completion graph), 3차(route lookup closure)가 모두
효과 없다.

3차는 **프로파일이 지목한 것 중 가장 작은 것**(0.204 GB)이었다. 프로파일은
`ExecutionContext`/`AsyncLocalValueMap`과 **메시지당 9.5 KiB 할당**(raw 1.6 KiB)을 훨씬
크게 짚었는데, 내가 "실행 문맥 전파를 끄면 diagnostics·flow id가 깨질 수 있다"는 위험을
이유로 그쪽을 미뤘다.

Java에서 통한 것(1.287 GB)과 .NET에서 안 통한 것(0.204 GB)의 **크기 차이가 6배**다.
그것이 갈림의 이유로 보인다.

**교훈: 프로파일이 크기 순서를 주면 그 순서대로 시도해야 한다.** 위험을 이유로 작은 것부터
가면 실패를 한 번 더 산다. 위험한 후보는 피하는 것이 아니라 **어떻게 안전하게 할지**를
먼저 설계해야 한다.

### 20.3 세 언어가 각각 다른 곳에서 통했다

| 언어 | 지배적 비용 | 결과 |
|---|---|---|
| **C++** | 즉시 수락 send의 completion graph | **2.3배** (PR #313 머지) |
| **Java** | 매 send의 peer 분류 stream | **+17%** (PR #314) |
| .NET | **아직 미확정** — 실행 문맥 전파와 9.5 KiB/message가 유력 | 세 번 실패 |

`findings-binding-gap.md` §3이 "네 언어 공통"이라고 한 구조는 **언어마다 다르게 작동했다.**
C++에서 2.3배를 낸 completion graph가 Java에서는 0.9%였고 .NET에는 아예 없었다.
