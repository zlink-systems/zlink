...
### 17.3 Java는 6개를 없앴는데 왜 안 움직였나 — 미확인

Java도 dependent `CompletableFuture` 3개 + observer node 3개를 없앴는데 비율이 그대로다.
`#311`의 상승에 묻혔을 수도, 실제로 효과가 없을 수도 있다. **지금 값으로는 가릴 수 없다.**

가리려면 같은 빌드(= `#311` 포함)에서 `#6` 전후를 재야 한다. 기준선이 `#311` 이전이라
조건이 다르다.
