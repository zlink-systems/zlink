...
| 릴리스 #87 바인딩 0.18.0 태그 | **사용자 승인 대기** — `<lang>/v0.18.0` 4언어 태그 → Actions 배포(외부·비가역). A의 Core 릴리스·framework G5와 순서 조율 필요. |
...
**캠페인 완료 상태 (2026-09-11):** 머신 B 몫 G1·G2·G3·G4·G6·G7(릴리스 노트)는 전부 완료·머지됐다. 완료 기준 충족 — 7언어 contract 통과, 스펙·guide 옛 시그니처 0건, 릴리스 노트 breaking 기록, decisions FB-071. 남은 것은 ① **바인딩 0.18.0 태그 발행(#87, 사용자 승인+A 순서 조율 대기)**, ② criterion 3 gRPC perf·G5 framework·RawKotlinStack 적응(전부 **A**). B가 독립적으로 끝낼 작업은 없다.
