...
| Store snapshots | Redis Location Store before/source/target authority states; Maintenance Store row before/after restart; NO positive Relocation Store fixture from ZoneWorld (final ruling 2026-08-24: per spec 21 §7.5 + §5 table row "Host maintenance의 Actor·User Spot 이동", completion records are written only on the spec-30 host-maintenance move path, and Instance Spot cold-activation envelopes only for Instance Spots — ZoneWorld is adjudicated app-admission-only/spec-30-exempt (scenario ruling ⑦) and has no Instance Spot, so zero `relocation:`-prefix access is the CONFORMANT reference behavior; a dedicated 8-request pending-through-cutover probe confirmed replay+reply after cutover with 0 store writes via the spec 28 §4.4 memory relay window). The golden encodes this as NEGATIVE assertions: zero relocation-prefix keys in clean B2 AND in the pending-request-through-cutover probe. Positive Relocation Store fixtures belong to spec-30 host-relocation and Instance Spot cold-activation e2e (stage 9), not this package. Mask endpoint/RID/lease token but retain relationships/generation transitions. | same key namespaces/separation and state transition invariants, including both relocation-prefix absence assertions. |
