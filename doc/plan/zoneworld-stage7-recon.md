...
| Store snapshots | Redis Location Store before/source/target authority states, separate Relocation Store terminal records, Maintenance Store row before/after restart. Mask endpoint/RID/lease token but retain relationships/generation transitions. | same key namespaces/separation and state transition invariants. |
