...
- [ ] **단계 3 — attempt-lifecycle · 매트릭스 · dialect 제거**
  ...
  - [x] 3b [A6] 크로스랭 canonical 매트릭스 `4언어` — **✅ 완료(2026-08-22 19:47): 12/12 전 그린,
    ...
    명문화 `5df53feae2`)**. **단계-마감 sol 리뷰 완료**: 커밋 정합·스펙 근거·V0/live 비모순
    승인. 실질 지적 1건 = **capacity row key/shape 무규정 발산 잔존** — .NET 독자 key(lifecycle
    포함, Authority.cs:5015), cpp는 .NET형 우선+Node형 fallback(:3311), Java는 Node형 전환(이번
    clean-break) → .NET↔Java 회계가 서로 다른 row로 갈라질 수 있음(셀은 그린이나 회계 정합성
    미보증). **후속 ruling 카드: capacity row 정본 = Node/Java형 확정 + .NET 정렬 + cpp .NET형
    fallback 제거**(3c .NET 다음 배치). 부수: 동일 relocation 내 재준비 시 attempt=1 재사용
    금지(재시도 카운터 카드에 병합). **3c 병렬 착수(2026-08-22
    ...
    분기 제거·tautology/순서비교 정리 — 1835/1838, 실패 3건 기존). .NET 스테이지 검증 완료 — dotnet 관여 6셀 전부
    그린(raw 프레이밍 교차 실증). **신규 카드 2건**: ① legacy JSON recovery 이중 표현 제거는 durable 마이그레이션
    ...
    **[정책] 리뷰·수정 감사(2026-08-22 사용자 지시)**: sol 리뷰는 큰 단계 마감에만, 일상 리뷰는
    ...
    18:5x)** + Java 회귀 3셀(java→node·java→dotnet·node→java) 무회귀 그린. `all` 편입 완료 `fe352b9e87`. 결정성 스윕:
    1차 12/12 그린, 2차에서 **dotnet→java 간헐 실패(~1/3, join-not-observed) 발견** — 나머지
    11셀은 2연속 그린. 실패 run /tmp/tmp.NOaTf8yl2H vs 성공 2건 대조 sol 진단 중. 이 flake
    해소가 3b 마지막 항목. 이월
...
- [x] **단계 5 — [B0] 하니스 안정화** `혼합` — **완료(2026-08-23)**: H-10 ✅ · TicTacToe flake
  ...
  - **[~] [갱신] Node TTT 모드 C — 정적 가설 실증 기각(2026-08-22)**: 실패 run서 보류 notify·
    ...
    **기존 잠복 결함**(0d7d26b1d2 직접 인과 배제·노출 효과): successor Session binding에 old
    ...
    거부(:569 — queue 소멸 vs seal 불일치 분기 미상). 2차 계측 진단 수렴 — **ruling ⓐ 확정**:
    resolveRemoteBoundSessionTarget(:564)가 받는 sourceSessionRid 미사용이 identity 유실점 —
    그 경계서 sessionNodeRid/sessionRid 보존 → 기존 successor 가드 유효화(스펙 48:116·20:153/
  ...
  - **[ ] [신규 등재 2026-08-23] dotnet HEAD 기존 red 3건(stash-baseline 확정, capacity 정렬 diff
    ...
    **field-12+ slot 레이아웃 발산 통합 판정 카드**(§3b 이월 ①)로 흡수. **판정 완료(2026-08-23,
    Claude ruling — sol 설계 분석 report-slot-design.md 기반): 옵션 A** = aggregateGeneration을
    slot 정규 필드로 승격, 순서 `relocation, aggregateGeneration, targetAttemptGeneration, …`,
    primitive ordinal-or-zero + 의미 불변식(phase1만 0, root 후 1..2^63-2, sentinel 2^63-1 발급
    금지, root와 일치). B(root 해석) 기각 — 발견 인덱스+TOCTOU 의미론 신설 = 복잡화 금지 위반.
