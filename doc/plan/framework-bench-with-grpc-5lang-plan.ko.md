...
### Phase 6 — 통합 검증과 보고서 (2026-09-07 개정)

초안은 다섯 언어를 3회씩 다시 재도록 했다. 그 규정이 막으려던 위험(개발 중 값 채택)은
이미 없다. 다섯 언어 모두 구현을 마친 뒤 전용 gated 구간에서 ROUTER 3회 + DEALER 1회로
측정됐다. 남는 진짜 위험은 다섯 구간의 commit이 다르다는 점이며, 언어를 가로지르는 결론이
그 비교 위에 서 있다.

**통합 검증 구간 (sonnet 1 job, ~1 h)**

- 한 commit에서 언어당 1회씩, 다섯 언어를 순차로 실행한다.
- 각 언어의 검증 run을 그 언어의 기존 중앙값과 대조한다. G5 허용 범위(±10%) 안이면
  기존 데이터를 채택한다. 벗어나는 언어만 3회 재측정한다.
- 측정 규율은 동일하다. 빌드는 구간 밖, `flock`, 매 run 전 loadavg 2.0 미만, 게이트 값 기록.

**보고서 (opus 1 job, ~1.5 h)**

§7.2의 열 항목 구성을 따른다. 데이터가 다섯 구간에서 나왔다는 사실과 검증 결과를
측정 조건에 명시한다. 검증이 통과했더라도 한 구간에서 잰 것처럼 쓰지 않는다.
...
### 7.2 통합 보고서의 구성 (2026-09-07 개정)

계획 초안의 주 표는 "언어별 계층 비용 비율"이었다. 그 표는 다섯 언어의 두 판정식을 나란히
놓아 어느 언어의 framework 계층이 가장 비싼지에 답하는 구조였다. **Phase 0~4의 실측이 그
표를 쓸 수 없게 만들었으므로 구성을 바꾼다.**

바꾸는 이유는 결과가 나빠서가 아니라, 그 표가 담을 값이 대부분 존재하지 않기 때문이다.
없는 값을 빈칸으로 둔 표는 "측정하지 못했다"로 읽히지만 실제로는 **각 칸이 비어 있는 이유가
서로 다르고 그 이유들이 이 캠페인의 답**이다.

1. **요약** — 세 문장 이내.
2. **측정 조건** — CPU 모델, OS·커널, 언어별 런타임과 gRPC 라이브러리 version, commit hash,
   payload, warmup과 그 근거, active duration, request window, send concurrency, 측정 시각과
   loadavg 게이트 값 전부.
3. **주 표: 판정 가능 여부와 그 이유** — 이 캠페인의 핵심 표다.

   | 언어 | formula 1 | formula 2 | 게재하지 못한 이유 |
   |---|---|---|---|

   각 칸은 게재된 값이거나 `unsupported`이며, `unsupported`인 칸은 **어느 행이 무엇 때문에
   막았는지**를 적는다. 언어마다 이유가 다르다는 것이 표의 내용이다.
4. **완료 전달 관측** — 언어별 raw 행이 동시 미완료 요청 수에 따라 어떻게 달라지는가.
   C가 depth 90에서 오류 없이 동작한다는 사실을 함께 싣는다. 이 항목이 3번 표의 이유들을
   하나로 묶는다(FB-031).
5. **언어별 3자 표** — 언어마다 규격 §4 형식의 표 하나. 게재 가능한 수치만 싣고, 정지한 셀은
   처리량이 아니라 정지 관측으로 싣는다.
6. **판정 기준 자체의 문제** — `zlink-c` request-window @4096이 독립된 세 구간에서 재현성
   조건을 넘지 못했다(FB-034). 이 행은 formula 1의 공유 분모이므로 **어떤 언어도 그 판정을
   게재할 수 없다.** 언어별 결과와 별개인 캠페인 수준의 결론으로 싣는다.
7. **패턴별 관찰** — 세 패턴을 각각 따로 서술한다. 평균내지 않는다.
8. **계측이 바로잡은 것** — 이 캠페인이 중간에 정정한 값과 정정하지 않았다면 보고서에
   실렸을 값. 최소한 framework send의 겉보기 우위(정정 전 gRPC 대비 2.8배, 정정 후 동등)와
   `.NET` raw의 겉보기 비율(window 8 실험을 window 91 실험으로 나눈 0.087)을 싣는다.
   측정 도구를 신뢰하는 근거가 이 항목이다.
9. **한계** — loopback 단일 머신, TLS·mesh 없음, gRPC 기본 구성, C++ gRPC 1.51.1,
   포화 셀 목록, 언어별 warmup 차이.
10. **후속 후보** — 원인을 기록만 하고 고치지 않은 항목과 그 우선순위.
...
- [–] Phase 7 — 사용자 지시로 중단(2026-09-07). §11.1 참조
...
### 11.1 Phase 7 중단 (2026-09-07, 사용자 지시)

사용자가 "여기서 더 진행 안할께"로 중단을 지시했다. 측정은 수행하지 않았고 재개 계획도 두지 않는다.

**중단 시점의 상태**

| 항목 | 상태 |
|---|---|
| 규격에 패턴 추가와 집계기 지원 | 커밋 `d76efff905` |
| C harness readiness 결함 수정(FB-044) | 커밋 `d76efff905` |
| C++·Node harness 루프 이식 | 미커밋, 작업 중단 |
| 규격 단순화(패턴 하나로) | 미커밋, 작업 중단 |
| 측정·판정·보고서 절 | 수행하지 않음 |

미커밋 변경은 전부 bench 쪽이며 `framework`·`bindings`·`core` 소스는 건드리지 않았다.
되돌리려면 해당 경로를 checkout하면 되고, 두어도 라이브러리에 영향이 없다.

**중단 전까지 정리된 설계 판단**

감독관이 이 단계에서 불필요한 구조를 만들었고 사용자가 두 번 정정했다.

1. **패턴을 나누지 말 것.** request는 request 하나다. 실제 서비스는 응답을 기다렸다 다음
   요청을 보내지 않으므로 비동기 연속 제출이 request의 정상 형태이고, 거기에 이름을 더
   붙일 이유가 없다. 고정 window 100은 패턴이 아니라 우리가 썼던 조건이었다.
2. **모든 언어가 같은 형식이어야 한다.** 감독관이 "C는 admission backpressure 변형, 관리형
   넷은 cooperative yield 변형"이라는 job 보고를 받아들였는데, 그 순간 여섯 행이 같은 것을
   재지 않게 된다. C가 제출 거절을 볼 수 있는 것은 C의 능력이지 측정 형식을 가를 이유가
   아니다. 전부가 할 수 있는 모양 하나로 통일해야 한다.
3. 필요한 것은 새 모델이 아니라 **`bindings/node/perf/perf_multi_socket_reqrep.ts`의 루프를
   그대로 이식하는 것**이었다. 저장소에 이미 정책을 만족하는 구현이 있다.
