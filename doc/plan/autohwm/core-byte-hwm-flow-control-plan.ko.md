...
| Done | 확인 항목 | Evidence |
|---|---|---|
| [ ] | 제거 기준점에서 public HWM option을 유지한 채 기능 test와 성능 회복을 확인했다. | `BLOCKED:` `doc/plan/autohwm/worklog/stage1-removal-baseline.md` — registry 회계·decoder reservation·drain wakeup·recv 공유 lock을 차례로 우회해도 회복 0. Bisect 결과 회귀 소유 commit은 `3ef4d09a37`(부모 164.7 → 62.2 Kops/s)이고 성격은 byte 회계 비용이 아니라 message당 blocking/wakeup 증가(총 CPU 감소, voluntary ctx switch 증가). Sampling profile(worklog §8)로 `zlink_poll`이 호출마다 poller 소유 signaler를 모든 socket mailbox에 등록·해제하던 비용을 특정하고 3건을 수정했다(`d58a179033`, `b818c03f71`, `5ad65a4627`). 재부팅 후 한산한 host(load 0.05~0.08, worklog §8.10)에서 재실행한 최종 gate(`autohwm-stage1-final3-*`, 이전 `final2`는 host 재부팅 직후 저하로 무효화)의 median local/0.10.1 = throughput·bandwidth 88.6%(시작 69.9%, 이전 최종 82.3%), mean/p95/p99 115~119%(시작 142~147%, 이전 최종 125~126%). 5개 metric 모두 아직 미달이라 통과가 아니다. 남은 비용은 poll 경로(`socket_poller_t::wait`/`add_item`, `enter_public_api`)이며 byte-HWM 계약 밖의 poll API 내부 설계 변경 승인이 필요하다. |
