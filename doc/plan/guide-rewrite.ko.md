...
## 16. connector 가이드는 구현 통일을 기다린다 (2026-09-18)

### 16.1 가이드를 쓰려다 기능 차이를 만났다

stream connector 가이드의 분량이 언어마다 네 단계로 갈려 있었다.
...
서버 가이드처럼 공통 소스 한 벌에 언어 탭을 두고 생성기로 다섯 벌을 만들기로 정했다. 그러려면
다섯 구현의 **기능이 같아야 한다.** 한 언어에만 있는 능력은 탭 하나에만 적히고, 그 순간 공통
소스가 성립하지 않는다.

전수 감사 결과 **기능 차이 22건, 스펙 공백 12건**이 나왔다. 언어 관용구 차이(`Flow` 대 `event`
대 콜백, 명명 규칙, 해제 관용구)와 환경 제약(브라우저 샌드박스)은 제외한 수다.

그중 셋은 지금 동작이 깨진다.

- **C++이 scheme에서 transport를 추론하지 않는다.** `ws://` endpoint를 기본 옵션으로 주면
  구성 오류가 된다.
- **C++의 typed 기본 packet 이름이 `typeid(T).name()`이다.** 컴파일러 맹글링 이름이라 서버와
  client가 같은 이름에 합의하지 못한다.
- **C++이 operation별 codec 지정을 노출한다.** 공통 스펙 §5.4가 금지한 표면이다.

### 16.2 순서 — 이슈, 스펙, 구현, 그다음 가이드

문서로 덮을 문제가 아니므로 순서를 정했다.

| 단계 | 상태 |
| --- | --- |
| 이슈 등록 | **끝났다.** #563 상위 + 언어별 #569~#572 |
| 결정 | **끝났다.** #563의 코멘트에 기록 |
| 스펙 통일 | **끝났다.** 공통 1편 + 언어별 4편, 각 한글·영문 |
| 구현 통일 | 이슈 넷이 대기한다 |
| 가이드 재작성 | 구현이 같아진 뒤 |

### 16.3 확정한 결정

**오류는 코드로 통일하고 전달만 플랫폼을 따른다.** 13개 닫힌 집합이 전부이고, 받는 쪽이 그중
무엇인지 읽어낼 수 있어야 한다. 무엇으로 전달하는지는 플랫폼이 정한다.

| 대상 | 전달 |
| --- | --- |
| 예외를 끈 빌드(Unreal 등) | 값으로 돌려준다. 던지지 않는다 |
| C++ e2e·도구 | 예외를 던지는 어댑터를 사용한다 |
| 그 밖의 언어 | 코드를 담은 전용 예외 타입을 던진다 |

**언어 표준 예외를 그대로 쓰지 않는다.** 코드를 담는 자리가 없어 호출자가 `ValidationFailed`와
`ConfigurationError`를 구분하지 못한다. Java가 91곳에서 그렇게 하고 있었다.

그 밖에 —

- **`receivedCount`는 받은 개수다.** 소비해도 줄지 않고 dispatch mode와 무관하다. 기준점은
  연결이 성립한 시점이다.
- **옵션은 전 항목을 검증한다.** 지점은 그 언어가 실패를 알릴 수 있는 가장 이른 곳이다. C++
  factory 시그니처를 바꾸지 않기 위해 "생성 시점"으로 못박지 않았다.
- **재연결 지연에 무작위를 넣는다.** 기준 지연의 50~100% 사이에서 고른다. 결정적 지연이면
  서버가 한 번 끊겼을 때 client가 모두 같은 시점에 다시 연결한다. 시도를 다 쓰면 끊김
  handler가 실행된다.
- **계기는 client connector에서 통째로 걷어냈다.** §6.2를 지우고 뒤 절 번호를 당겼다.
- **등록 네 종이 모두 해제할 수 있는 값을 돌려준다.** `.NET`의 C# `event` 세 종을 `IDisposable`
  반환 메서드로 바꾼다. 다만 돌려준 값의 수명이 등록의 수명인지는 언어가 정한다 — C++은
  소멸자가 해제하고, 나머지는 명시적으로 해제할 때까지 유지된다.
- **대기 표면 셋**은 이름 명시와 타입 기반 결정을 모두 제공하고, 술어와 반환이 message이며,
  위반은 `ValidationFailed`다.

### 16.4 리뷰가 잡은 것

스펙을 Opus로 한 번 더 읽혔다. **결정과 어긋난 변경이 하나 들어가 있었다** — 시그니처를 바꾸지
않기로 정했는데 C++ 스펙이 `result_t<connector_t>`로 바뀌어 있었다. 되돌렸다.

그 밖에 `.NET` 예제 셋이 컴파일되지 않는 모양이었고(초기화식 누락으로 CS9113·CS8618), 영문판이
등록 네 종 규정을 한 종으로 줄였으며, C++ 템플릿 선언 12개가 C++ 문법이 아니었다. 새로 쓴
문단에 원칙 7.7·7.9·7.10 위반 서른 몇 곳이 들어가 있었다.

**§12 회귀 테스트 표에 이번 결정이 만든 검증 항목이 하나도 없었다.** 작성 가이드 §9.3이
"관찰 가능한 행동은 이 절이 소유한다"고 정하므로 아홉 항목을 채웠다.

### 16.5 가이드 쪽에서 미리 해 둔 것

- **생성기를 절별로 일반화했다.** `guide/server`가 여덟 군데에 못박혀 있던 것을
  `SECTIONS = ("server", "stream-connector")`로 바꿨다. 서버 가이드는 회귀 없이 돈다.
- **다섯 언어의 공개 표면을 소스에서 확인해 대조표로 정리했다.** 구현이 같아지면 그대로 공통
  소스의 재료가 된다.

### 16.6 http-client는 사실 오류만 고친다

구조는 다섯 언어 모두 14편으로 이미 고르다. 검증이 확인한 사실 오류(컴파일되지 않는 예제
셋 포함)만 고치고, "최신이 아님" 배너는 고친 뒤 뗀다. connector처럼 구조를 바꾸지 않는다.

## 17. connector 가이드를 끝냈다 (2026-09-18)

### 17.1 기다리던 것이 끝났다

§16이 "구현 통일을 기다린다"로 멈춰 있었다. 스펙 한 벌과 네 구현이 main에 들어가면서
그 조건이 충족됐다. 공통 소스 7편을 쓰고 생성기가 다섯 벌을 만든다.

| 언어 | 전 | 후 |
| --- | ---: | ---: |
| C++ | 11편 1,726줄 | 11편 1,695줄 |
| `.NET` | 3편 362줄 | 9편 1,278줄 |
| **Java** | **1편 130줄** | **7편 1,015줄** |
| **Kotlin** | **1편 133줄** | **7편 990줄** |
| Node | 3편 254줄 | 9편 1,207줄 |

공통 7편은 개요·설치와 첫 연결·옵션·송신·수신·생명주기·오류 처리다. 그 뒤 장은 **그 언어가
실제로 도는 환경**을 따른다 — C++은 e2e client·엔진 어댑터·배포·성능, `.NET`은 Unity·Godot C#,
Node는 브라우저·Unity WebGL, Java와 Kotlin은 없다. 기능 차이가 아니라 환경 차이다.

**다이어그램은 넣지 않기로 했다.** 작성 가이드 §5가 기능마다 그림을 요구하지만, connector와
http client는 내용이 표면 사용법이라 그림이 설명을 더하지 않는다.

### 17.2 집필이 구현 결함을 드러냈다

소스와 한 줄씩 대조하며 쓰자 문서가 아니라 **구현이 틀린 자리**가 나왔다.

| 이슈 | 내용 |
| --- | --- |
| #599 | C++이 송신 한도 초과에 `frame_too_large`를 돌려준다. 스펙 §9.1의 닫힌 13개 밖이다 |
| #599 | C++ `dispatch()` 주석은 "한 개를 실행한다"인데 구현은 큐를 다 비운다 |
| #600 | Kotlin 래퍼에 종료 사유 읽기 표면이 없다(§6.2) |
| #600 | Kotlin 대기 표면이 타입만으로 이름을 정하는 길을 제공하지 않는다(§10.1) |

**옛 C++ 가이드가 소스와 다른 곳도 다섯 군데 나왔다** — heartbeat 기본값(문서 10초/30초,
실제 1초/5초), 없는 옵션(`max_metadata_size`), `send().submit()`이 결과를 돌려준다는 서술(실제
`void`), 사라진 오류 코드 세 개를 담은 목록. 새 가이드는 소스를 따랐다.

**손으로 쓴 가이드는 소스와 조용히 갈라진다.** 공통 소스에서 생성하면 한 곳만 고치면 되고,
검사기가 다섯 벌을 함께 본다.

### 17.3 다이어그램이 276곳에서 잘리고 있었다

iframe에 높이가 없어 브라우저 기본값 150px이 적용됐다. 그림은 대부분 잘리고 archify가 내는
`1.5rem` 제목만 보였다. **문서 276곳 전부 그 상태였다.**

고정 높이도 맞지 않았다 — 그림마다 비율이 달라 짧은 그림은 아래가 비고 긴 그림은 잘린다.
다이어그램이 문서와 같은 출처에서 오므로 부모가 안쪽 문서의 높이를 재서 넣는다. 그림 안
제목은 캡션 크기로 줄였다. 그림 위 산문이 같은 내용을 이미 적는다.

산출물에 세 세대가 섞여 있던 것도 정리했다 — 정리 규칙이 없는 것 6개, 옛 규칙 180개, 새
규칙 54개. 옛 규칙은 뷰어가 화면 높이만큼 늘어나는 것을 풀지 않아 그림 아래에 빈 공간이
남았다.

### 17.4 가이드 홈이 언어마다 달랐다

C++만 프레임워크를 설명했고 나머지 넷은 목차 표 하나였다. 랜딩 페이지를 거치지 않고 가이드
홈으로 바로 들어온 독자는 그 언어의 가이드가 무엇을 다루는지 알 수 없었다.

네 언어에 C++과 같은 차례의 도입부를 넣었다. **코드는 손으로 쓰지 않고 튜토리얼 소스의
snippet을 가리킨다** — 여섯 marker를 네 언어가 모두 갖고 있어 문서와 컴파일되는 코드가
갈라지지 않는다.

**언어 선택이 드러나지 않던 것도 함께 고쳤다.** 상단 선택기가 아이콘 하나뿐이라 무엇을 고르는
자리인지 알 수 없었고, 가이드 홈에는 장 문서와 달리 전환 줄이 없었다. 선택기에 지금 언어의
이름을 적고, 가이드 홈 제목 아래에 같은 전환 줄을 두었다. 언어 순서도 C++ · `.NET` · Java ·
Kotlin · Node로 통일했다.

**client 라이브러리 가이드가 nav에 한 줄도 없었다.** connector와 http client 모두 랜딩 페이지
표로만 닿을 수 있었다. 사이드바를 따라 읽는 독자에게는 없는 문서였다.

### 17.5 브랜치가 72 커밋 뒤처진 채로 반나절을 썼다

작업을 시작할 때 `origin/main`과의 거리를 확인하지 않았다. 워크트리가 이미 어떤 브랜치에
있었고 그 상태를 그대로 받았다.

**단서는 있었다.** 이 워크트리가 framework 0.16.0인데 정본이 0.17.0이라는 것을 세션 초반에
확인하고도 "워크트리의 성질"로 적어 두고 넘어갔다. 버전 차이는 브랜치가 뒤처졌다는 증거다.

merge에서 38개 파일이 겹쳤고, 그중 21개는 main이 **언어별 e2e를 통째로 삭제**한 것이었다.
이 브랜치가 그 파일들을 고친 것은 connector 공개 표면 변경에 맞춰 호출부를 맞춘 것이라,
파일이 사라지면 의미가 없다. 삭제를 받았다.

나머지 충돌은 **양쪽이 같은 병을 다르게 고친 것**이었다. 옵션 record에 구성 요소가 늘어 자리
인자가 깨진 것을 한쪽은 `createDefault`에서 파생시켜, 다른 쪽은 자리 인자를 다시 세어 고쳤다.
파생시키는 쪽을 택했다 — 구성 요소가 또 늘어도 깨지지 않는다.

### 17.6 검증이 실행마다 달라 한 시간을 썼다

merge 후 C++ 빌드가 깨졌다. 실패한 TU를 단독으로 컴파일하니 통과했고, 그 사이 C++ 소스는
하나도 바뀌지 않았다. 병렬도를 8에서 4로 낮추니 오류 0으로 끝났다.

**같은 커밋이 실행할 때마다 다르게 실패하면 진짜 회귀와 구분할 수 없다.** 그날 장비에서
C++·Java·`.NET`·Node 빌드가 동시에 돌고 있었다. 검증할 때는 병렬도를 낮추고, 실패를 보면
단독 실행으로 먼저 갈라야 한다.

**로그를 자르지 않는다.** `tail -6`으로 잘라 `cc1plus:`로 시작하는 실제 사유 줄을 잃었다.
그 한 줄이 있었으면 한 시간이 들지 않았다.

### 17.7 남은 것

| 항목 | 상태 |
| --- | --- |
| `01`·`03`의 손으로 적은 코드 | **남았다.** `01`에 코드 블록 40개. snippet으로 바꾸려면 tutorial에 구간 마커를 먼저 단다. 릴리즈 뒤로 미뤘다 |
| `16`의 공통 정본 승격 | **끝났다.** 소스와 대조해 기본값이 틀린 자리 열 곳 넘게 함께 고쳤다. Kotlin에 없던 장도 생겼다 |
| `13`의 공통 정본 승격 | **올리지 않는다.** 타입 이름 목록이라 탭으로 감쌀 공통 산문이 거의 없다 |
| 볼드 범위(원칙 10.1 #9) | **끝났다.** 실제 위반은 종속절을 감싼 여섯 곳이었다. "45곳·62곳"은 대부분 저장소 전체가 쓰는 문장 머리 볼드다 |

### 17.8 그 뒤에 이어 한 것

- **샘플 설명이 계약 정본뿐이었다.** nav의 `Picking a sample`이 `exclude_docs`가 제외한 경로를
