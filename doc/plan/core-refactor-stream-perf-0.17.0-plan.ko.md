...
- [ ] 0.17.3 이월: **ST-1 STREAM packet pump 정체 수정(D-B218, 최우선)**, single tcp 1024 B PAIR/DD −4~5 % 관찰 재확인(D-B219), G-11 2a/2b/2d(socket 쪽 `_out_sync`·receive partition·route shard, 목표 lock/msg 8.3), backlog(`_slot_sync`, mailbox 예외 경로, `receive_once_guarded`(D-e), pipe.cpp 분할, ws/wss 병합, lb::sendpipe, R7 #4/#7, R11-B), TSan 기존 debt 5건(monitor/ctx lock-order, lb peer-weight).
