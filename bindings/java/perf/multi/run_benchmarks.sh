...
PATTERN="${PERF_PATTERNS:-${PATTERNS:-ALL}}"
if [[ -n "${PERF_TRANSPORTS:-}" ]]; then
  TRANSPORTS="${PERF_TRANSPORTS}"
else
  TRANSPORTS="tcp,tls,ws,wss"
fi
MSG_SIZES="${PERF_MSG_SIZES:-${SIZES:-64,256,1024,4096,65536,131072}}"
CLIENTS="${PERF_MULTI_CLIENTS:-${PERF_CLIENTS:-${CCU:-100}}}"
RUNS=1
DURATION="${PERF_MULTI_DURATION_SECONDS:-${PERF_DURATION_SECONDS:-${DUR:-5}}}"
...
[[ -n "${PERF_MULTI_CLIENTS+x}" || -n "${PERF_CLIENTS+x}" \
  || -n "${CCU+x}" ]] && explicit_clients=1
[[ -n "${PERF_MSG_SIZES+x}" || -n "${SIZES+x}" ]] && explicit_msg_sizes=1
...
TIMEOUT_SECONDS="${PERF_MULTI_TIMEOUT_SECONDS:-${PERF_TIMEOUT_SECONDS:-${TIMEOUT:-auto}}}"
...
if combo:
    ...
    for key in sorted(result_map.keys()):
        ...
        emit(f"RESULT,current,{p},{tr},{sz},{metric},{result_map[key]:.{precision}f}")
