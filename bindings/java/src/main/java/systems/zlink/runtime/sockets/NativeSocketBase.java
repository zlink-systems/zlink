...
abstract class NativeSocketBase implements Socket {
    ...
    static {
        InternalAccess.register(new InternalAccess.SocketAccess() {
            ...
            @Override
            public boolean completionTransferToPublic(Socket socket,
                                                      Object claimant) {
                return nativeSocket(socket).runtime.completionOwner()
                    .transferToPublic(claimant);
            }
