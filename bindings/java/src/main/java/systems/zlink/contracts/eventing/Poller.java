 ...
 * reporting which become ready. The caller owns this resource and must close
 * it.
 */
public interface Poller extends AutoCloseable {
    ...
    void modify(SocketMonitor monitor, PollEventFlags... events);

    void modifyFd(int fd, PollEventFlags... events);

    boolean remove(Socket socket);

    boolean remove(SocketMonitor monitor);
