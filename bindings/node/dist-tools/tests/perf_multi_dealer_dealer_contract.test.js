...
const { runDealerDealerSendRounds } = require('../perf/multi/perf_multi_dealer_dealer_client');
const { STOP_TOKEN_BYTES } = require('../perf/perf_stop_token');
function nextTurn() {
    return new Promise((resolve) => setImmediate(resolve));
}
function within(promise, timeoutMs = 15_000) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`dealer/dealer fair-round contract timed out after ${timeoutMs}ms`)), timeoutMs);
        promise.then((value) => { clearTimeout(timer); resolve(value); }, (error) => { clearTimeout(timer); reject(error); });
    });
}
async function runDealerDealerFairRoundContract(transport) {
    ...
    try {
        const endpoint = await benchmarkEndpoint(transport, `dealer-dealer-fair-round-contract-${process.pid}`, { suite: 'multi' });
        ...
        const sender = runDealerDealerSendRounds({
            ...
            payloads,
            msgSize: payloadSize,
            activeStopNs: currentEpochNs() + 10000000000n,
            maxTurns: rounds,
            submit,
            yieldTurn: async () => {
                yields += 1;
