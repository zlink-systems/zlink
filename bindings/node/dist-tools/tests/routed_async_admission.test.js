...
async function flushImmediateTurns(count = 64) {
...
test('send submit is Promise-based while submit_sync is flag-free', async () => {
    ...
    try {
        ...
        assert.deepEqual([first.parts[0].getString(), second.parts[0].getString()], ['managed', 'blocking']);
        first.close();
        second.close();
        managed.close();
    }
...
test('PollOut-only observer cannot drain another Poller completion owner', async () => {
    ...
    try {
        ...
        assert.equal(settled, false, 'the public completion owner must retain the queued completion');
        assert.equal(observer.wait(observerEvents, 0), 0, 'a REQUEST completion must not masquerade as writable credit');
        await Promise.resolve();
        assert.equal(settled, false, 'PollOut-only wait must not drain a completion owned by another Poller');
...
test('duplicate socket add failure preserves the original completion owner', async () => {
    ...
    try {
        ...
        await flushImmediateTurns();
        assert.equal(settled, false, 'duplicate-add rollback must not transfer ownership to the runtime pump');
        assert.equal(poller.wait(events, 1_000), 1);
...
test('second PollCompletion owner is rejected without displacing the first', async () => {
    ...
    try {
        ...
        await flushImmediateTurns();
        assert.equal(settled, false, 'failed ownership transfer must leave the first public owner active');
...
test('duplicate public slots drain only the socket named by the native event', async () => {
    ...
    try {
        ...
        await Promise.resolve();
        assert.equal(Number(settledA) + Number(settledB), 1, 'one capacity-limited event must drain exactly one registered socket');
        assert.equal(poller.wait(events, 1_000), 1);
        assert.equal(events.slot(0), 85);
