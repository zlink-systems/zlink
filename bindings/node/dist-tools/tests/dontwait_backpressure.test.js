...
function drainAvailable(socket, values) {
    for (;;) {
    ...
    try {
        ...
        for (let index = 0; index < 512; index += 1) {
            ...
            try {
                ...
                assert.equal(error.result, zlink.SubmitResult.InvalidState);
                assert.equal(error.nativeErrno, 0);
...
test('managed send snapshots a backpressured Buffer and retries it after peer drain', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            sends.push(sender.send().message(payload).submit().admitted.then(() => { settledByIndex[index] = true; settled += 1; }, (error) => {
        ...
        for (let turn = 0; turn < sends.length && !sawWritablePollOut; turn += 1) {
            ...
            if (!sawWritablePollOut)
                await yieldToEventLoop();
        }
        assert.equal(sawWritablePollOut, true, 'a matching WRITABLE completion must wake the public poller as POLLOUT');
        for (let turn = 0; turn < 10_000; turn += 1) {
            drainAvailable(receiver, received);
            readiness.wait(events, 0);
            if (settled === sends.length && received.length === sends.length)
                break;
            await yieldToEventLoop();
        }
        assert.equal(settled, sends.length, 'every managed send must settle after the peer returns write credit');
        assert.equal(received.length, sends.length, 'every logical packet must be delivered after managed retry');
        await Promise.all(sends);
        if (failure !== undefined)
            throw failure;
        assert.deepEqual(received, expected, 'managed retry must send the packet snapshot captured at submit time');
    }
    finally {
        events.close();
        readiness.close();
...
test('retry completion does not consume a Message wrapper reused by the caller', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            expected.push(text);
            sends.push(sender.send().message(Buffer.from(text)).submit().admitted.then(() => { settled += 1; }, (error) => {
                settled += 1;
                failure ??= error;
            }));
        }
        await yieldToEventLoop();
        assert.ok(settled > 0 && settled < sends.length, 'Buffer sends must fill HWM before the wrapper regression attempt');
        const released = zlink.Message.from('wrapper-packet');
        messages.push(released);
        expected.push('wrapper-packet');
        sends.push(sender.send().message(released).submit().admitted.then(() => { wrapperSettled = true; settled += 1; }, (error) => {
            wrapperSettled = true;
            settled += 1;
            failure ??= error;
        }));
        assert.equal(released.size(), 0, 'a backpressured submit must consume Message after taking its snapshot');
        assert.equal(wrapperSettled, false, 'the full HWM must leave the wrapper packet awaiting WRITABLE');
        released.close();
        replacement = zlink.Message.from('replacement-wrapper');
        assert.strictEqual(replacement, released, 'the caller should be able to reuse the consumed wrapper while retry waits');
        for (let turn = 0; turn < 10_000; turn += 1) {
            drainAvailable(receiver, received);
            completions.wait(0);
            if (settled === sends.length && received.length === sends.length)
        ...
        assert.equal(replacement.getString(), 'replacement-wrapper', 'retry success must not consume the caller-reused wrapper');
    }
    finally {
        completions.close();
...
test('context shutdown rejects a backpressured managed send as Terminated', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            state.done = sender.send().message(Buffer.alloc(64, index)).submit().admitted.then(() => { state.settled = true; }, (error) => {
                state.error = error;
                state.settled = true;
            });
            attempts.push(state);
        ...
        assert.equal(pending.settled, true, 'shutdown must terminate the pending WRITABLE wait');
        assert.ok(pending.error instanceof zlink.SubmitError);
        assert.equal(pending.error.result, zlink.SubmitResult.Terminated);
    }
    finally {
        completions.close();
        receiver.close();
...
test('managed routed send retries the same target and packet after HWM drain', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            payloads.push(payload);
            settledByIndex.push(false);
            sends.push(router.send(target).message(payload).submit().admitted.then(() => { settledByIndex[index] = true; settled += 1; }, (error) => {
                settledByIndex[index] = true;
                settled += 1;
                failure ??= error;
            }));
        ...
        await yieldToEventLoop();
        assert.ok(settled > 0 && settled < sends.length, 'routed sends must reach HWM after at least one admission');
        const pendingIndex = settledByIndex.findIndex((value) => !value);
        assert.notEqual(pendingIndex, -1);
        payloads[pendingIndex].fill(0x7a);
        for (let turn = 0; turn < 10_000; turn += 1) {
        ...
        if (failure !== undefined)
            throw failure;
        assert.deepEqual(received, expected, 'managed routed retry must preserve both submit-time payload and target');
    }
    finally {
        completions.close();
        dealer.close();
