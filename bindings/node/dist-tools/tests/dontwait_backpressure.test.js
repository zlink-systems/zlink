...
test('managed send snapshots a backpressured Buffer and retries it after peer drain', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            sends.push(sender.send().message(payload).submit().then(() => { settledByIndex[index] = true; settled += 1; }, (error) => {
...
test('retry completion does not consume a Message wrapper reused by the caller', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            expected.push(text);
            sends.push(sender.send().message(Buffer.from(text)).submit().then(() => { settled += 1; }, (error) => {
                settled += 1;
                failure ??= error;
            }));
        }
        ...
        sends.push(sender.send().message(released).submit().then(() => { wrapperSettled = true; settled += 1; }, (error) => {
            wrapperSettled = true;
            settled += 1;
            failure ??= error;
        }));
...
test('context shutdown rejects a backpressured managed send as Terminated', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            state.done = sender.send().message(Buffer.alloc(64, index)).submit().then(() => { state.settled = true; }, (error) => {
                state.error = error;
                state.settled = true;
            });
            attempts.push(state);
...
test('managed routed send retries the same target and packet after HWM drain', async () => {
    ...
    try {
        ...
        for (let index = 0; index < 64; index += 1) {
            ...
            payloads.push(payload);
            settledByIndex.push(false);
            sends.push(router.send(target).message(payload).submit().then(() => { settledByIndex[index] = true; settled += 1; }, (error) => {
                settledByIndex[index] = true;
                settled += 1;
                failure ??= error;
            }));
