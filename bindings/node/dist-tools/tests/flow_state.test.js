...
test('setReceiveFlowState reports NotSupported on PAIR, PUB/SUB family, and STREAM', () => {
    ...
    for (const socket of [pair, pub, sub, xpub, xsub, stream]) {
        assert.throws(() => socket.setReceiveFlowState(zlink.ReceiveFlowState.PAUSED), (error) => isConfigError(error)
...
test('setReceiveFlowState maps invalid handle and invalid argument per the config error policy', () => {
    ...
    const router = zlink.createRouterSocket(ctx);
    router.close();
    assert.throws(() => router.setReceiveFlowState(zlink.ReceiveFlowState.RUNNING), (error) => isConfigError(error)
        && error.result === zlink.ConfigResult.InvalidHandle);
    const dealer = zlink.createDealerSocket(ctx);
    for (const outOfRange of [2, -1, 999]) {
        assert.throws(() => dealer.setReceiveFlowState(outOfRange), (error) => isConfigError(error)
...
test('setReceiveFlowState after close observes only a bounded close-related outcome', () => {
    ...
    for (let i = 0; i < 20; i += 1) {
        ...
        try {
            ...
            assert.ok(result === zlink.ConfigResult.InvalidHandle || result === zlink.ConfigResult.InvalidState, `unexpected result ${result}`);
...
test('public surface has no flow-frame receive/encode API or PAUSE-bypass send variant', () => {
    ...
    for (const socket of sockets) {
        ...
        for (const name of methodNames) {
            ...
            for (const forbidden of forbiddenSubstrings) {
                assert.ok(!lower.includes(forbidden), `socket exposes forbidden flow API member: ${name}`);
            }
        }
    ...
    for (const exportName of Object.keys(zlink)) {
        ...
        for (const forbidden of forbiddenSubstrings) {
            assert.ok(!lower.includes(forbidden), `zlink package exposes forbidden flow API export: ${exportName}`);
        }
    }
    ctx.close();
});
