...
const { RoutedReplySender, relayShutdownDrainMs, runRoutedSendSendRounds, trackPendingReplyTask } = require('../perf/multi/perf_multi_routed_sendsend');
...
async function waitForMonitorEvent(monitor, expectedEvent, timeoutMs = 5_000) {
    ...
    try {
        ...
        const senders = dealers.map(async (dealer, clientIndex) => {
            for (let sequence = 0; sequence < sendsPerClient; sequence += 1) {
                ...
                maxInFlightByClient[clientIndex] = Math.max(maxInFlightByClient[clientIndex], inFlightByClient[clientIndex]);
                try {
                    await dealer.send()
                        .message(payload)
...
test('routed scheduler advances available sockets while one admission remains pending', async () => {
    ...
    let replyDrains = 0;
    let releaseSlow;
    const slowAdmission = new Promise((resolve) => { releaseSlow = resolve; });
...
test('routed relay drain stays inside the server shutdown budget', () => {
    ...
    try {
        ...
        if (savedSendDrain === undefined)
            delete process.env.PERF_MULTI_SEND_DRAIN_TIMEOUT_MS;
        else
            process.env.PERF_MULTI_SEND_DRAIN_TIMEOUT_MS = savedSendDrain;
        if (savedShutdown === undefined)
            delete process.env.PERF_MULTI_SERVER_SHUTDOWN_TIMEOUT_MS;
        else
            process.env.PERF_MULTI_SERVER_SHUTDOWN_TIMEOUT_MS = savedShutdown;
    }
});
...
test('routed async multipart boundary preserves inline and overflow part storage', async () => {
    ...
    try {
        const endpoint = await benchmarkEndpoint('tcp', `routed-sendsend-small-storage-${process.pid}`, { suite: 'multi' });
        router.bind(endpoint);
        ...
        let stableRoutingId = null;
        for (const partCount of [2, 8, 9]) {
            const expected = Array.from({ length: partCount }, (_, index) => Buffer.from(`part-${partCount}-${index}`));
...
test('received routed send preserves stale-route ownership across immediate or waiting failure', async () => {
    ...
    try {
        const endpoint = await benchmarkEndpoint('tcp', `routed-sendsend-stale-${process.pid}`, { suite: 'multi' });
