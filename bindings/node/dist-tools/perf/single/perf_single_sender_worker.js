...
function signalStatus(port, status, type, code, extra = {}) {
    ...
    try {
        switch (kind) {
            ...
            case 'router_router': {
                ...
                configureTlsClient(socket, transport);
                socket.connect(endpoint);
                activeReceiverRoutingId = handshakeRouterSender(port, control, status, socket, activeReceiverRoutingId);
                break;
            }
