...
async function spawnMultiPair(serverScript, clientScript, args) {
    ...
    if (isSocketReqRepPattern(args.pattern)) {
        ...
        try {
            await Promise.race([
                waitForLine(client, `CLIENT_DONE,${args.msgSize}`, clientScript || 'client', clientTimeoutMs),
                rssGuard.promise
            ]);
        }
        catch (error) {
        ...
        try {
            ...
            const reqRepClientExit = await Promise.race([
                ...
                new Promise((_, reject) => {
                    setTimeout(() => reject(new Error(`client timeout after ${shutdownTimeoutMs}ms`)), shutdownTimeoutMs);
                })
            ]);
