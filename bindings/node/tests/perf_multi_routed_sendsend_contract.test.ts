...
test('REQREP perf server submits once and surfaces reply admission failures', () => {
  ...
  try {
    assert.throws(
      () => submitReply({ reply: () => operation }, payload),
      (error: unknown) => error instanceof zlink.SubmitError
        && (error as { result: number }).result
          === zlink.SubmitResult.Backpressured);
    assert.strictEqual(submittedPart, payload,
      'the reply builder must receive the original received Message');
...
test('routed scheduler advances available sockets while one admission remains pending', async () => {
  ...
  const result = await runRoutedSendSendRounds({
    ...
    submit: (socket, _routerClient, record) => {
      ...
      now += 1n;
      if (!backpressured) inFlight[index] -= 1;
      return {
  ...
  assert.ok(beforeSlowRelease[1] + beforeSlowRelease[2] > 2,
    'an available peer must advance while socket 0 waits for admission');
...
test('routed scheduler keeps polling while every admission is blocked', async () => {
  ...
  const result = await within<any>(runRoutedSendSendRounds({
    sockets,
    payloads,
    measurementRecords: records,
    routerClient: false,
...
test('routed relay submits replies concurrently without an application FIFO', async () => {
  ...
  const received = () => {
    const operation = {
      ...
      submit() {
        ...
        return {
          result: zlink.SubmitResult.Backpressured,
          admitted: new Promise<void>((resolve) => releases.push(resolve)),
        };
      }
    };
    const value = {
  ...
  try {
    ...
    assert.deepEqual(submittedParts, receivedValues.flatMap((value) => value.parts),
      'the send builder must receive the original received Messages');
    releases.forEach((release) => release());
...
test('received routed send preserves stale-route ownership across immediate or waiting failure', async () => {
  ...
  try {
    ...
    if (submission !== null) {
      ...
      assert.equal(sourceParts[1].size(), 0);
      router.close();
      await assert.rejects(submission.admitted, (error: unknown) =>
      ...
      assert.ok((immediateError as { result: number }).result === zlink.SubmitResult.NotConnected
        || (immediateError as { result: number }).result === zlink.SubmitResult.NotFound);
