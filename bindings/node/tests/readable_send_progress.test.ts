...
for (const routed of [false, true]) {
  for (const asynchronous of [false, true]) {
    test(`readable handler survives same-socket send (routed=${routed}, async=${asynchronous})`, async () => {
      ...
      function nextReceipt(): Promise<void> {
        return new Promise<void>((resolve, reject) => {
          const deadline = setTimeout(() => {
            try {
              ...
              received.close();
              assert.equal(queued, false, `readiness lost: recv(DontWait) still returned ${payload}`);
              assert.fail('readable handler deadline: no message remained queued (delivery delayed)');
      ...
      async function send(socket: any, target: any, value: string): Promise<void> {
        const operation = (routed ? socket.send(target) : socket.send()).message(value);
        if (asynchronous) await operation.submit().admitted;
        else operation.submit_sync();
      }
      ...
      try {
        ...
        await receipt;
        assert.deepEqual(values, ['first', 'next']);
        assert.ok(notifications > firstNotifications);
      } finally {
...
test('continuous readable replies yield to the event loop', async () => {
  const ctx = zlink.createContext();
  ...
  try {
        ...
        socket.setReadableHandler(() => {
          try {
            while (socket.recv(received, zlink.RecvFlags.DontWait)) {
              ...
              turns += 1;
              if (turns === 128) {
                assert.equal(loopAdvanced, true, 'readable work starved setImmediate');
                resolve();
              } else {
    ...
    received.close(); right.close(); left.close(); ctx.close();
  }
});
