...
function payloads(count: number): Buffer[] {
  return Array.from({ length: count }, (_, index) =>
    index === 1 ? Buffer.alloc(0) : Buffer.alloc(1024, index));
}

function withParts(operation: any, parts: Buffer[]): any {
  for (const part of parts) operation = operation.message(part);
  return operation;
}
...
test('whole-message PAIR receive grows and reuses capacity without consuming the next record', () => {
  ...
  try {
    ...
    received.close(); receiver.close(); sender.close(); context.close();
  }
});
...
test('whole-message REQUEST growth preserves the route, reply token, and independent part owners', async () => {
  ...
  try {
    ...
    for (const count of [33, 2]) {
      ...
      for (const part of sent) reply = reply.message(part);
      reply.submit();
      sent.forEach(part => assert.equal(part.size(), 0));
      received.close();
      const response = await completions.settle<any[]>(pending);
...
test('whole-message SUB grows topic and part buffers while preserving queued record boundaries', () => {
  ...
  try {
    ...
    received.close(); subscriber.close(); publisher.close(); context.close();
  }
});
