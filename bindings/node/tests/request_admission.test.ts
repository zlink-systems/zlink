...
function configureSmallHwm(context: any, dealer: any, router: any): void {
...
function recvAndReplyAvailable(router: any, received: string[]): number {
  let count = 0;
  for (;;) {
    const request = new zlink.Received();
    try {
      if (!router.recv(request, zlink.RecvFlags.DontWait)) return count;
...
async function hwmRequestRound(round: number): Promise<void> {
  ...
  try {
    ...
    assert.ok(initiallyAdmitted < requestCount,
      'HWM setup must leave requests waiting on WRITABLE tokens');
    ...
    assert.equal(new Set(received).size, requestCount,
      'each submit-time request snapshot must be admitted exactly once');
...
test('REQUEST backpressure waits for its WRITABLE token and resubmits exactly once (5 rounds)',
  async () => {
    for (let round = 0; round < 5; round += 1) await hwmRequestRound(round);
  });
...
test('socket close releases a connect-before-bind REQUEST token', async () => {
  ...
  try {
    ...
    assert.ok(replyError instanceof zlink.SubmitError);
...
test('context shutdown terminates a REQUEST wait token as typed Terminated', async () => {
  ...
  try {
    ...
    assert.ok(outcomes.some((error: any) => error instanceof zlink.SubmitError
      && error.result === zlink.SubmitResult.Terminated),
    'at least one pre-admission wait token must terminate as SubmitError.Terminated');
    dealer.close();
  ...
  try {
    ...
    while (dataCount + requestCount < operationCount) {
      for (;;) {
        ...
        try {
          if (!router.recv(received, zlink.RecvFlags.DontWait)) break;
      ...
      if (dataCount + requestCount < operationCount) {
        assert.ok(poller.wait(events, 1_000) > 0,
          'mixed WRITABLE records must make progress through one drain owner');
      }
    }
