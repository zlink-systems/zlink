...
async function isFulfilledNow(stage: Promise<unknown>): Promise<boolean> {
  let fulfilled = false;
...
test('BACKPRESSURED admission completes after public WRITABLE progress', async () => {
  ...
  try {
    ...
    for (;;) {
      ...
      try {
        if (!router.recv(request, zlink.RecvFlags.DontWait)) break;
    ...
    assert.equal(await isFulfilledNow(blocked.reply), false,
      'reply cannot complete at admission');
    ...
    try {
      ...
      for (const part of blockedReply) part.close();
    }
    const prefixReplies = await Promise.all(
      submissions.slice(0, -1).map((submission) => submission.reply)
    );
