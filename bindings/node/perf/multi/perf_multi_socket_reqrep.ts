...
async function runSocketReqRepClient({ options, pattern, routerClient, serverRoutingId }) {
  ...
  try {
    ...
    while (currentEpochNs() < activeStopNs && !fatal) {
          ...
          try {
            appendMeasurement(operation, payload).timeout(requestTimeoutMs)
              .submit_sync(zlink.SendFlags.DontWait,
                (error, parts) => observe(i, error, parts));
            outstanding[i] += 1;
            submitted = true;
            seq += 1n;
          } catch (error) {
            if (!transientRequest(error)) throw error;
            blocked[i] = true;
            break;
          }
        }
      ...
      if (!submitted) blocked.fill(false);
    }

    const drainStopNs = currentEpochNs()
      + BigInt(Math.max(1000, requestTimeoutMs * 4)) * 1_000_000n;
    while (outstanding.some((count) => count > 0) && currentEpochNs() < drainStopNs && !fatal) {
      poller.wait(pollBuffer, 50);
      await sleepImmediate();
    }
    if (fatal) throw fatal;
    if (outstanding.some((count) => count > 0)) throw new Error('request drain timed out');
