...
if (!configured || !path.isAbsolute(configured)) {
  fail('ZLINK_CORE_INSTALL_PREFIX must name an absolute installed Core 0.12.0 package prefix');
}
...
if (manifest.package !== 'zlink-core' || !/^\d+\.\d+\.\d+$/.test(manifest.version || '')
  ...
  fail(`Installed Core package must have 0.12.0 provenance: ${manifestPath}`);
}
