...
if (coreSource === 'local') {
  ...
  if (!configured || !path.isAbsolute(configured)) {
    fail('ZLINK_CORE_INSTALL_PREFIX must name an absolute installed Core 1.7.0 package prefix');
  }
  ...
  if (fs.existsSync(manifestPath)) {
    ...
    if (manifest.package !== 'zlink-core' || !/^\d+\.\d+\.\d+$/.test(manifest.version || '')
      ...
      fail(`Installed Core package must have 1.7.0 provenance: ${manifestPath}`);
    }
