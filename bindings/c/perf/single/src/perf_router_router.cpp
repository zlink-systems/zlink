                                      ...
                                      zlink_routing_id_t *target_rid_out_)
{
    ...
    while (!connected && std::chrono::steady_clock::now () < deadline) {
        ...
        if (perf_zlink_send_rid_parts (sender_, &target_rid, &ping, 1, ZLINK_DONTWAIT) != 0) {
            ...
            zlink_msg_t part;
            size_t part_count = 0;
            const int recv_rc = zlink_router_recv (
              receiver_, &source_rid, &request_seq, &part, 1, &part_count,
    ...
    zlink_msg_t part;
    size_t part_count = 0;
    if (zlink_router_recv (sender_, &source_rid, &request_seq, &part, 1,
...
void drain_router_socket (void *socket_)
{
    ...
    for (;;) {
        ...
        zlink_msg_t part;
        size_t part_count = 0;
        if (zlink_router_recv (socket_, &source_rid, &request_seq, &part, 1,
                                     ...
                                     bool *header_ok_out_)
{
    ...
    const int rc = zlink_router_recv (
      receiver_, &source_rid, &request_seq, parts, 2u, &part_count,
    ...
    if (!size_ok && bench_debug_enabled ()) {
        std::cerr << "[perf-router-router] excluded payload size=" << actual_size
                       ...
                       latency_stats_t *latency_out_)
{
    ...
    return perf_single_one_way::run_active_phase (
      sender_, receiver_, payload_, state_, duration_s_, recv_timeout_ms_, "perf-router-router",
      [] (void *sender, std::vector<char> *active_payload,
          const router_router_recv_state_t &active_state, uint64_t seq) {
          zlink_msg_t part;
          ...
          if (perf_zlink_send_rid_measurement_parts (
                sender, &active_state.target_rid, &part, ZLINK_SEND_FLAGS_NONE)
              == 0) {
      ...
      recv_router_router_header_flags,
      [state_] (void *sender) {
          return send_router_stop_token (sender, &state_->target_rid) ? 0 : -1;
      },
      received_out_, latency_out_);
}
