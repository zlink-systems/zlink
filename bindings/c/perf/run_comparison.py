...
MULTI_ENV_ALIAS_MAP = {
    ...
    "PERF_MONITOR_HWM_BYTES": "PERF_MULTI_MONITOR_HWM_BYTES",
...
def normalize_multi_pattern_name(pattern_name):
    ...
    if ALLOW_MULTI and pattern == "DEALER_ROUTER":
        return "DEALER_ROUTER_SENDSEND"
    if ALLOW_MULTI and pattern == "ROUTER_ROUTER":
        return "ROUTER_ROUTER_SENDSEND"
    return pattern


def display_pattern_name(pattern_name):
    return normalize_multi_pattern_name(pattern_name)
...
def stop_stream_client_if_server_exited(
    server_proc, client_proc, pending_ready_sizes, state_lock=None
):
    lock = state_lock or _STREAM_START_STATE_LOCK
    with lock:
        if (
            not pending_ready_sizes
            or not server_proc
            or server_proc.poll() is None
            or not client_proc
            or not client_proc.stdin
        ):
            return False
        try:
            client_proc.stdin.write("STOP\n")
            client_proc.stdin.flush()
            return True
        except Exception:
            return False
    ...
    try:
        ...
        def on_client_stdout_line(line):
            ...
            if ready_size is not None:
                ...
                stop_stream_client_if_server_exited(
                    server_proc,
                    client_proc[0],
                    pending_ready_sizes,
                    stream_start_state_lock,
        ...
        def on_client_sample(_sample_cpu, _sample_mem):
            pump_server_output_nonblocking()
            if stop_stream_client_if_server_exited(
                server_proc,
                client_proc[0],
                pending_ready_sizes,
                stream_start_state_lock,
...
def run_sizes_test(
    ...
    # Split every multi pattern, including STREAM, by size so each paired
    # case has the same isolated server/client lifecycle.
    ...
    if pattern_name in STREAM_VARIANT_PATTERNS:
        ...
        # binding STREAM cases use identical lifecycle boundaries.
        ...
        items.extend(
            [
                ...
                (
                    ...
                    str(parse_env_int("PERF_CONNECT_READY_TIMEOUT_MS", 10000)),
                ),
                ("monitor_hwm_bytes", str(parse_env_int("PERF_MONITOR_HWM_BYTES", 4096000))),
