                                     ...
                                     bool *header_ok_out)
{
    ...
    size_t part_count = 0;
    const int rc = ::zlink_std_compat_recv (router, &source_rid, &parts, &part_count,
                                         ...
                                         const std::string &pair_id)
{
    ...
    set_sockopt_int (router, ZLINK_OPT_RCVTIMEO, timeout_ms, "ZLINK_OPT_RCVTIMEO");
    set_sockopt_int (dealer, ZLINK_OPT_RCVTIMEO, timeout_ms, "ZLINK_OPT_RCVTIMEO");
    return true;
}
                              ...
                              latency_stats_t *out_latency)
{
    ...
    if (active_phase) {
        while (std::chrono::steady_clock::now () < deadline) {
            ...
            if (bench_send_single_part (dealer, &part, static_cast<zlink_send_flags_t> (0)) < 0) {
