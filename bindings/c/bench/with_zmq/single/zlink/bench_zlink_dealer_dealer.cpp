...
void run_dealer_dealer (const std::string &transport, size_t msg_size, const std::string &lib_name)
{
    ...
    if (!setup_connected_pair (s1.get (), s2.get (), transport, lib_name + "_dealer_dealer")) {
        print_fail_no_queue ();
        return;
    }

    const int recv_timeout_ms = resolve_single_recv_timeout_ms ();
    set_sockopt_int (s1.get (), ZLINK_OPT_RCVTIMEO, recv_timeout_ms, "ZLINK_OPT_RCVTIMEO");
    set_sockopt_int (s2.get (), ZLINK_OPT_RCVTIMEO, recv_timeout_ms, "ZLINK_OPT_RCVTIMEO");
    ...
    auto print_fail_with_queue = [&] () {
        print_fail_result (lib_name, "DEALER_DEALER", transport, msg_size, &queue_probe);
    };
    ...
    if (!run_oneway_phase (s2.get (), s1.get (), &payload, payload_size, msg_size, run_id, &seq,
    ...
    print_result (lib_name, "DEALER_DEALER", transport, msg_size, throughput, latency_stats.mean_us,
