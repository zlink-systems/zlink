                                          ...
                                          bool *header_ok_out)
{
    ...
    char topic_id[256];
    size_t topic_len = 0;
    const int rc = bench_recv_pubsub_single_part (socket, &msg, topic_id, &topic_len, flags);
    if (rc < 0) {
    ...
    const bool topic_ok = topic_len == 5 && std::memcmp (topic_id, "bench", topic_len) == 0;
                              ...
                              latency_stats_t *out_latency)
{
    ...
    if (active_phase) {
        while (std::chrono::steady_clock::now () < deadline) {
            ...
            if (bench_send_pubsub_single_part (pub_socket, "bench", &part, 0) < 0) {
...
void run_pubsub (const std::string &transport, size_t msg_size, const std::string &lib_name)
{
    ...
    set_sockopt_int (pub.get (), ZLINK_XPUB_NODROP, xpub_nodrop_opt, "ZLINK_XPUB_NODROP");

    zlink_set_subscription (sub.get (), "");
    if (!setup_connected_pair (pub.get (), sub.get (), transport, lib_name + "_pubsub")) {
        print_fail_no_queue ();
        return;
    }

    const int recv_timeout_ms = resolve_single_pubsub_recv_timeout_ms ();
    set_sockopt_int (sub.get (), ZLINK_RCVTIMEO, recv_timeout_ms, "ZLINK_RCVTIMEO");
