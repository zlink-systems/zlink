...
inline void apply_io_threads (void *ctx_)
{
    ...
    if (rc != 0 && bench_debug_enabled ()) {
        std::cerr << "zlink_ctx_set(ZLINK_IO_THREADS) failed: " << zlink_strerror (zlink_errno ())
                  << std::endl;
    }
}
...
{
    const int rc =
      zlink_set_option (socket_, static_cast<zlink_option_t> (option_), &value_, sizeof (value_));
    if (rc != 0 && bench_debug_enabled ()) {
    ...
    if (bench_debug_enabled ()) {
        ...
        const int grc =
          zlink_get_option (socket_, static_cast<zlink_option_t> (option_), &out, &out_size);
        if (grc == 0) {
