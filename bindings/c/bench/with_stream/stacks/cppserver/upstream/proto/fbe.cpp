...
uuid_t uuid_t::sequential()
{
    ...
    uint64_t state = utc() ^ ((counter.fetch_add(1) + 1) * 0x9E3779B97F4A7C15ull);
    for (size_t i = 0; i < result._data.size(); ++i)
    {
        state ^= state << 13;
        state ^= state >> 7;
        state ^= state << 17;
        result._data[i] = (uint8_t)((state >> ((i % 8) * 8)) & 0xFF);
    }
    result._data[6] = (result._data[6] & 0x0F) | 0x10;
    result._data[8] = (result._data[8] & 0x3F) | 0x80;
#endif
#elif defined(_WIN32) || defined(_WIN64)
    ::UUID uuid;
...
uuid_t uuid_t::random()
{
...
#else
    static std::atomic<uint64_t> counter(0);
    uint64_t state = utc() ^ ((counter.fetch_add(1) + 1) * 0xD6E8FEB86659FD93ull);
    for (size_t i = 0; i < result._data.size(); ++i)
    {
        state ^= state << 13;
        state ^= state >> 7;
    ...
    result._data[6] = (result._data[6] & 0x0F) | 0x40;
    result._data[8] = (result._data[8] & 0x3F) | 0x80;
#endif
#elif defined(_WIN32) || defined(_WIN64)
    ::UUID uuid;
