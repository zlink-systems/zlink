...
GUIDE_PATH="${SCRIPT_DIR}/dotnet-socket-surface-execution-guide.ko.md"
...
POLL_SECONDS=30
STRESS_COUNT=1
GATE_LABEL="dotnet_socket_surface_gate"
MODEL_ARG=()
...
Run the .NET socket surface Ralph loop.
...
Options:
  --guide PATH          Override execution guide path
                        (default: ${GUIDE_PATH})
  --master-plan PATH    Legacy compatibility path. Keep this equal to --guide.
  ...
  --stress-count N      Default repeat count for gate loop
                        (default: ${STRESS_COUNT})
  --gate-label NAME     Gate status label
                        (default: ${GATE_LABEL})
  --model MODEL         Pass --model MODEL to codex exec
  -h, --help            Show this help text

Examples:
  ./run_dotnet_socket_surface_execution.sh
  ./run_dotnet_socket_surface_execution.sh --max-iterations 0
  ./run_dotnet_socket_surface_execution.sh --model gpt-5.4
EOF
}
...
require_non_negative_integer "${STRESS_COUNT}" "--stress-count"
...
echo "=== .NET socket surface execution start ==="
echo "Root: ${ROOT_DIR}"
...
if [[ "${MAX_ITERATIONS}" == "0" ]]; then
  "${SUPERVISOR_PATH}" \
    ...
    "${MODEL_ARG[@]}" \
    2> >(grep -v '^Reached max iterations without completion: 0$' >&2)
  rc=$?
else
  "${SUPERVISOR_PATH}" \
    --guide "${GUIDE_PATH}" \
