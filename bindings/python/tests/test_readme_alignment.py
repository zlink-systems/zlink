...
class ReadmeAlignmentTests(unittest.TestCase):
    ...
    def test_perf_readme_references_perf_policy_and_actual_transport_matrix(self):
        ...
        self.assertIn("`STREAM` client uses", readme)
        self.assertNotIn("`MULTI_STREAM`", readme)
    ...
    def test_multi_perf_readme_documents_result_names_and_recv_only_surface(self):
        ...
        self.assertIn("`PUBSUB`", readme)
        self.assertIn("`DEALER_ROUTER_SENDSEND`", readme)
        self.assertIn("`ROUTER_ROUTER_SENDSEND`", readme)
        self.assertNotIn("`MULTI_PUBSUB`", readme)
        self.assertNotIn("`MULTI_DEALER_ROUTER", readme)
        self.assertNotIn("`MULTI_ROUTER_ROUTER", readme)
