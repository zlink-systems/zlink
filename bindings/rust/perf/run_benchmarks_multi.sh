...
CLIENTS="${PERF_MULTI_CLIENTS:-${PERF_CLIENTS:-${EFFECTIVE_DEFAULT_CLIENTS}}}"
...
Options:
  ...
  --transports LIST
  --runs N
  --clients N            Client sockets per pattern (default: 100, including STREAM).
  --smoke
  --build-dir PATH
  --reuse-build
  --clean-build
  ...
  --connect-ready-timeout-ms N
                         Connection-ready timeout in ms (default: 10000).
  --monitor-hwm N
...
export PERF_MULTI_DEFAULT_CLIENTS="${EFFECTIVE_DEFAULT_CLIENTS}"
export PERF_MULTI_DEFAULT_STREAM_CLIENTS="${EFFECTIVE_DEFAULT_STREAM_CLIENTS}"
prepare_core_runtime
...
if [[ "${EXPLICIT_CLIENTS}" != "1" ]]; then
    ...
    if [[ "${all_stream}" == "1" && "${has_stream}" == "1" ]]; then
        REPORT_CLIENTS="${EFFECTIVE_DEFAULT_STREAM_CLIENTS}"
    elif [[ "${has_stream}" == "1" ]]; then
        REPORT_CLIENTS="${EFFECTIVE_DEFAULT_CLIENTS} (stream=${EFFECTIVE_DEFAULT_STREAM_CLIENTS})"
    else
        REPORT_CLIENTS="${EFFECTIVE_DEFAULT_CLIENTS}"
    fi
fi
