...
// async send is the coroutine-native blocking form: on backpressure the
// binding retains the packet and the awaiter stays suspended until WRITABLE.
// It deliberately has no per-operation or drain deadline. The external runner
// remains the process-level failure fallback.
