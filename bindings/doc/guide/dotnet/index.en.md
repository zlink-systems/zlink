...
> The former deep-copy `CopyTo` aligns to `Clone`; `CopyTo` stays an `[Obsolete]` alias
> for one release cycle. `Copy()` is a ref-share and does not guarantee mutation
> isolation — use `Clone()` when you need an independently mutable payload.
