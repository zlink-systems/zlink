...
> day, 2026-08-23. This revision normalizes the terminal surface: (1) it
> codifies that every HWM-managed operation (PAIR/PUB send/publish,
> DEALER/ROUTER routed send, request) is classified ASYNC purely because an
> HWM wait can occur; (2) it re-expands the C++ routed send surface the 1st
> revision had narrowed to "synchronous only" back into a two-terminal
...
> C++ send/publish's async completion depends on the Core send-completion
...
> surface of HWM-managed **send/publish** (PAIR/PUB send/publish,
> DEALER/ROUTER routed send), (2) the Core-driven asynchronous completion of
> **request** (C++ is the exception, with three terminals: `submit()` /
> `submit(callback)` / `async()`), and (3) the synchronous one-shot
> completion of raw reply, which stays HWM-free.

This document defines the name and return type each language binding, except
C, must use for the asynchronous completion of HWM-managed PAIR/PUB
**send/publish** and DEALER/ROUTER **routed send**, the asynchronous
completion of **request**, and the synchronous completion of raw
ROUTER/`Received` reply. The bindings library provides a per-language
...
**The bindings library owns no threads at all.** HWM-managed send/publish and
routed send are classified ASYNC because an HWM wait can occur, but Core
still drives completion. C++'s blocking `submit()` waits inside Core and
resumes on Core's signal, `SNDTIMEO` bounds the wait, and `DONTWAIT` returns
`EAGAIN` immediately. Every other language's single `submit()` (Go's
`Submit(ctx)`) wraps the Core send but returns a language-idiomatic
awaitable, and that awaitable's completion is driven by the upcoming Core
send-completion notification (`doc/plan/core-send-completion-design.ko.md`,
in progress). Backpressure policy belongs to the application. Request
...
| Section | Covers |
|---|---|
| [Classification principle](#classification-principle) | What separates ASYNC HWM-managed operations from SYNC raw reply |
...
| [HWM-managed send/publish completion contract](#hwm-managed-sendpublish-completion-contract) | The asynchronous completion surface for HWM-managed send/publish (including routed send) |
...
| [Per-language normative table for send/request/raw reply](#per-language-normative-table-for-sendrequestraw-reply) | Each of the three operation types' per-language terminal and return type |
...
- **Every HWM-managed operation is classified as an ASYNC function.**
  PAIR/PUB send/publish, DEALER/ROUTER routed send, and request all pass
  through a point where an HWM wait *can* occur, so all three are classified
  ASYNC. The classification criterion is not "does an HWM wait happen
  often" — it is "can an HWM wait happen at all."
- **Raw reply is HWM-free and genuinely synchronous.** A raw
  ROUTER/`Received` reply never passes through the HWM path. It is the sole
  exception to this classification, and the only genuinely synchronous
  completion lane.
- This classification applies identically to bindings and framework
  ...
  HWM-managed send/publish split ASYNC/SYNC by the same criterion — the
  framework does not get a relaxed rule of its own.
- This principle is the basis for every section below: C++ exposing
  `submit()`/`async()` together (or request's three terminals), other
  languages returning an awaitable even for send/publish, and raw reply
  remaining the sole genuinely synchronous lane all follow from this
  classification principle.
...
- **Naming-distinction rule.** C++ must distinguish, by name, functions
  ...
  .NET follows its own Async-suffix convention and keeps a single `Async()`
  terminal. Every other language makes no such linguistic distinction and
  keeps a single `submit()` (Go: `Submit(ctx)`) terminal, whose returned
  awaitable each language consumes idiomatically (await / join / block_on /
  channel recv).
...
## HWM-managed send/publish completion contract

PAIR/PUB **send/publish** and DEALER/ROUTER **routed send** are classified
ASYNC per the [classification principle](#classification-principle) — an HWM
wait can occur. Each invokes the language's canonical terminal on the
builder returned by the same operation entrypoint. It adds no separate name
such as `send_async` or `sendCoroutine`.
...
## Per-language normative table for send/request/raw reply

The table below covers the three operation types — HWM-managed
**send/publish** (PAIR/PUB send/publish and DEALER/ROUTER routed send),
**request**, and **raw reply** — with each language's terminal and
return/completion representation. Send/publish and request are classified
ASYNC per the [classification principle](#classification-principle); raw
reply is the sole SYNC operation. A framework may wrap this surface to
provide another execution model, but it must not grow the bindings public
API's names.

| Binding | HWM-managed send/publish | Request | Raw reply |
|---|---|---|---|
| C | Not applicable — the C ABI does not apply builder policy and follows the functional contract in `core/include/zlink.h` | Not applicable | Not applicable |
| C++ | `submit()` (blocking) → `void`; throws `submit_error_t` on failure. `async()` → move-only `async_result_t<T>` | `submit()` (blocking) → the reply, throws on failure. `submit(callback)` → returns immediately, completion delivered via callback. `async()` → `async_result_t<T>` (framework canonical) | `submit()` → `void`; throws `submit_error_t` on failure |
| .NET | `Async()` → `Task`/`ValueTask`/`Task<T>`/`ValueTask<T>` | `Async(...)` → same as above | `Submit()` → synchronous `void`; throws `ZlinkSubmitException` on failure |
| Java, Kotlin | `submit()` → `CompletionStage<T>` | `submit()` → `CompletionStage<T>` (same as above) | `submit()` → synchronous `void`; throws `ZlinkSubmitException` on failure |
| Node | `submit()` → `Promise<T>` (or `Promise<void>`) | `submit()` → `Promise<T>` (same as above) | `submit()` → synchronous `void`; throws `SubmitError` on failure |
| Python | `submit()` → awaitable coroutine object | `submit()` → same as above | `submit()` → synchronous `None`; raises `SubmitError` on failure |
| Go | `Submit(ctx)` → `error` (`nil` on success) | `Submit(ctx)` → completion channel | `Submit(ctx)` → synchronous `error` (`nil` on success) |
| Rust | `submit()` → `Future<Output = Result<(), SubmitError>>` | `submit()` → runtime-independent `Future<Output = Result<Vec<message_t>, ZlinkError>>` | `submit()` → synchronous `Result<(), SubmitError>` |
...
A raw ROUTER/`Received` reply builder is neither HWM-managed send/publish nor
request. As defined in the [classification principle](#classification-principle),
raw reply is the sole completion lane that never passes through the HWM path,
which is why it is genuinely synchronous. Its terminal submits a terminal
reply or error reply to the HWM-free completion lane with one native call and
finishes synchronously. HWM backpressure is not a reply result.
...
for HWM-managed send/publish either** — the send/publish awaitable (C++'s
`async()`, or the awaitable every other language's `submit()` returns) is
already completed by the Core send-completion notification, so the framework
simply `await`s / `co_await`s that awaitable directly from its own coroutine.
Request already returns a language-native awaitable (or C++'s `async()`
result), so it needs no such wrapping either.
...
- The Java framework wires the `CompletionStage` it gets from a managed
  ...
  timeout policy.
- The `CompletionStage` returned by HWM-managed send/publish's `submit()` is
  wired the same way, exactly like managed request — the Core
  send-completion notification drives completion, so no separate executor
  wrapping is needed.
...
- HWM-managed send/publish is connected to coroutine suspension the same
  way, via the `CompletionStage` from Java `submit()`'s `await()` — it does
  not need to be wrapped in a separate dispatcher such as `Dispatchers.IO`,
  since the Core send-completion notification already drives asynchronous
  completion; it is not a blocking call.
...
- C++ bindings provides move-only `async_result_t<T>` through a managed
  ...
  directly.
- HWM-managed send/publish works the same way: the `async_result_t<T>`
  returned by `async()` is `co_await`ed directly by the framework coroutine
  — the Core send-completion notification drives completion, so the
  framework does not need to wrap blocking `submit()` in a separate
  executor. Because frameworks are coroutine-mandatory, they use `async()`
  only, not blocking `submit()`.
...
- The .NET framework `await`s the `Task`/`ValueTask` returned by a managed
  request as-is. The `Task`/`ValueTask` returned by HWM-managed send/publish's
  `Async()` is `await`ed the same way, directly — it does not need to be
  wrapped in an executor such as `Task.Run(...)`.
- The Node framework `await`s the `Promise` returned by a managed request
  ...
  send/publish's `submit()` is `await`ed the same way, directly — it does
  not need to be wrapped with a worker-offload policy.
- The Python framework awaits a managed request's `submit()` coroutine object
  directly. HWM-managed send/publish's `submit()` coroutine object is
  awaited the same way, directly — it does not need to be wrapped in an
  execution path such as `loop.run_in_executor(...)`.
- The Rust framework polls a managed request's runtime-independent `submit()`
  Future on its own executor. HWM-managed send/publish's `submit()` Future
  is polled the same way — it does not need to be wrapped with something
  like `spawn_blocking`.
