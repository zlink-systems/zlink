...
- **Binding context close calls `zlink_ctx_shutdown` and then `zlink_ctx_term`.** The effects of the
  two calls are defined by [Core Context §3](../../../core/doc/spec/core/01-context.en.md#3-lifecycle-and-shutdown).
- **If native destroy returns `ZLINK_CLOSE_BUSY`/`EBUSY` during an explicit Poller close, the binding
  reports public typed `Busy` and keeps the poller valid.** This result is the
  [Core Polling §7](../../../core/doc/spec/core/05-polling.en.md#7-functions) decision for a destroy while `wait()`
  runs. The caller may close it after `wait()` ends.
