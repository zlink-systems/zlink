...
- **Poller close/destroy/Dispose/Drop preserves completion ownership through teardown.** A fallible close returns `InvalidState` while an operation is outstanding. An infallible destructor retains the registration and operation state until affected socket close and poller destroy actually succeed. If Core returns `EBUSY` because a socket API call or poller `wait()` is active, the owning context retains the handles, registration, and operation state for lifecycle cleanup under [§6](#6-caller-wait-cancellation). Context close calls `zlink_ctx_shutdown`, waits for active socket calls and poller waits to exit, then closes every socket and monitor of that context that is not yet closed and destroys every poller it owns, retained handles included. If native close or destroy still returns `EBUSY`, it retains the handles and completes those closes after the active call exits. It calls `zlink_ctx_term` only after all socket closes have succeeded, then releases registration and operation state exactly once after the native handles are actually closed. If the context stays live, the retained handles remain until context close; no other cleanup trigger or background drain thread is created.
...
- A poller rejects a socket from a context other than its creation context with `InvalidState`, without changing its registrations or owner. Its creation context retains an `EBUSY` poller during an active wait even if no socket has ever been registered or all sockets have been removed.
