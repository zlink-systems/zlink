...
- Running context close while several threads repeatedly make socket, monitor, and poller calls that succeed without context close ends each call with success or the `ETERM` projection (for the configuration family, `INTERNAL_ERROR` with internal errno `ETERM`), and an AddressSanitizer build reports no access to a freed handle. A call made after context close returns ends with the `ETERM` projection.
