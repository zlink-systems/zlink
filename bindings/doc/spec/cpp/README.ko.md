...
- 메시지 값은 효율적인 move를 지원하고, 명시적 payload 공유·이전은 공통 계약의
  `Copy`/`Move`(각각 C `zlink_msg_copy`/`zlink_msg_move`)를 따른다. C++ 시그니처는
  `message_t::copy()`(공유, 새 값 반환) · `message_t::move(message_t&)`(소유권 이전,
  호출자 empty)이며, `move`는 C API를 직접 감싼다(C++ move 시맨틱과 별개). C++엔 Message
  반환 깊은복사(`Clone`) 메서드는 없다 — 독립 버퍼가 필요하면 바이트를 materialize해 새
  `message_t`를 만든다. 정의는 [Message ownership 공통 계약](../draft/message-ownership.ko.md)
  §"명시적 Copy / Move / Clone".
