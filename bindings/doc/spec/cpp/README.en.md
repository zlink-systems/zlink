...
- Namespace: every raw binding public type lives under `zlink`.
...
`IContext`. A public resource object stays a concrete
...
Public Spot and Actor facade shapes are specified by [Framework API](../../../../framework/doc/framework/common/spec/server/00-foundation/06-framework-api.en.md).
...
Runtime detail stays behind the raw socket facade; public headers do not expose native handles, callback trampolines, whole-message arrays, request pumps, or marshalling helpers.
...
3. Decide the C++ domain owner: one of context, message, socket, monitor, timer, error, option.
...
owns the public Spot `GetOrCreate` contract.
...
- A builder is required for Core raw multipart send, publish, request, and reply operations — this hides native request state and makes ownership clear.
...
- Raw send, routed send, publish, request, and reply return a move-only fluent builder.
- A builder's start method takes only a target identity, topic, routing ID, or `reply_token_t`. Payload, flag, timeout, and async submit choices happen at the builder stage.
- No single-payload shortcut overload is added under the same name as an operation's start method. `send(message)`, `send(routing_id, message)`, `publish(topic, message)` are not public contract members. A caller uses `send(...).message(message).submit()`.
...
- Socket family: pair, dealer, router, pub, sub, xpub, xsub, stream, common options, typed socket options, bind/connect/disconnect, TLS, callbacks, request/reply surfaces.
...
- The ROUTER/PUB defaults, raw HWM defaults follow the core header.
