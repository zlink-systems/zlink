...
- Message ownership, multipart payload, routing id, received metadata, topic message, subscription event. Payload share/transfer follow the common contract's `Copy`/`Move`: `Message Copy()` (ref-count share, internal `zlink_msg_copy`) and `void Move(Message dest)` (ownership transfer, caller left empty, `zlink_msg_move`). .NET's existing `CopyTo(Span<byte>)`/`CopyTo(IBufferWriter<byte>)` are span-fill methods that write the payload into a caller buffer — not a Message deep copy — so they stay unchanged (no `Clone`, no rename). See the [common Message ownership contract](../draft/message-ownership.ko.md) §"명시적 Copy / Move / Clone".
