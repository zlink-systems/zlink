...
`build-wsl.sh`는 `bindings/go`를 현재 Core `0.14.1` runtime과 함께 source
archive로 만든다. Go module path는 `zlink.systems/zlink`이고 release version은
`0.14.1`이다.

```bash
scripts/local-package/go/build-wsl.sh \
  --core-prefix /absolute/path/.artifacts/wsl/install/zlink-core/0.14.1
```

출력은 다음 파일이다.

```text
.artifacts/wsl/go/zlink-go-0.14.1.tar.gz
```
