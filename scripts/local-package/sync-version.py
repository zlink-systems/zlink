...
def package_version(relative: str, key: str) -> str:
    """Read one package-owned semantic version file."""
    path = REPO_ROOT / relative
    lines = path.read_text(encoding="utf-8").splitlines()
    if len(lines) != 1 or not lines[0].startswith(f"{key}="):
        raise SyncError(f"{relative} must contain exactly {key}=X.Y.Z")
    actual_key, version = lines[0].split("=", 1)
    if actual_key != key or not re.fullmatch(SEMVER, version):
        raise SyncError(f"{relative} must contain exactly {key}=X.Y.Z")
    return version


BINDING_LANGUAGES = ("cpp", "dotnet", "go", "java", "node", "python", "rust")
FRAMEWORK_LANGUAGES = ("cpp", "dotnet", "java", "node")


def binding_versions() -> dict[str, str]:
    return {
        language: package_version(
            f"bindings/{language}/VERSION", "ZLINK_BINDING_VERSION"
        )
        for language in BINDING_LANGUAGES
    }
...
def framework_versions() -> dict[str, str]:
    return {
        language: package_version(
            f"framework/languages/{language}/VERSION", "ZLINK_FRAMEWORK_VERSION"
        )
        for language in FRAMEWORK_LANGUAGES
    }
...
# This registry is the complete ownership list for language-owned Framework versions. The
...
FRAMEWORK_SCALAR_FIELDS = (
    ...
    FrameworkField(
        ...
        "dotnet",
        "tutorial PackageVersion Zlink.Framework",
        rf'(<PackageVersion Include="Zlink\.Framework" Version=")(?P<version>{SEMVER})(" />)',
        required=False,
    ),
    FrameworkField(
        "framework/languages/dotnet/tutorial/Directory.Packages.props",
        "dotnet",
        "tutorial PackageVersion Zlink.Framework.AspNetCore",
        rf'(<PackageVersion Include="Zlink\.Framework\.AspNetCore" Version=")(?P<version>{SEMVER})(" />)',
        required=False,
    ),
    FrameworkField(
        "framework/languages/dotnet/tutorial/Directory.Packages.props",
        "dotnet",
        "tutorial PackageVersion Zlink.Framework.Locations.Redis",
        rf'(<PackageVersion Include="Zlink\.Framework\.Locations\.Redis" Version=")(?P<version>{SEMVER})(" />)',
        required=False,
    ),
    FrameworkField(
        "framework/languages/dotnet/tutorial/Directory.Packages.props",
        "dotnet",
        "tutorial PackageVersion Zlink.Stream.Connector",
        rf'(<PackageVersion Include="Zlink\.Stream\.Connector" Version=")(?P<version>{SEMVER})(" />)',
        required=False,
    ),
    FrameworkField(
        "framework/languages/dotnet/tutorial/Directory.Packages.props",
    ...
    FrameworkField(
        ...
        rf'("@zlink-systems/nestjs": "\^?)(?P<version>{SEMVER})(")',
        required=False,
    ),
    FrameworkField(
        "framework/languages/node/tutorial/package.json",
        "node",
        "tutorial @zlink-systems/framework",
        rf'("@zlink-systems/framework": "\^?)(?P<version>{SEMVER})(")',
        required=False,
    ),
    FrameworkField(
        "framework/languages/node/tutorial/package.json",
        "node",
        "tutorial @zlink-systems/framework-locations-redis",
        rf'("@zlink-systems/framework-locations-redis": "\^?)(?P<version>{SEMVER})(")',
        required=False,
    ),
    FrameworkField(
...
# fields are owned by the corresponding Framework language VERSION file.
...
def synchronize(
    ...
    sync.regex(
        "bindings/cpp/packaging/conan/conanfile.py",
        rf'(    version = "){SEMVER}(")',
    ...
    binding_version = bindings["python"]
    sync.regex(
    ...
    sync.regex(
        "vcpkg/ports/zlink-framework/portfile.cmake",
        rf"(-DZLINK_FRAMEWORK_CPP_ZLINK_CPP_VERSION=){SEMVER}",
    ...
    sync.regex(
        "vcpkg/ports/zlink-framework/portfile.cmake",
        rf"(-DZLINK_FRAMEWORK_CPP_ZLINK_CORE_VERSION=){SEMVER}",
        rf"\g<1>{core_version}",
        1,
    )
    binding_version = bindings["dotnet"]
    ...
    synchronize_framework(framework_sync, frameworks)
    return (
        core_version,
        bindings,
        frameworks,
...
def main() -> int:
    ...
    if args.check and (changed or framework_changed):
        if changed:
            print(
                ...
                f"(Core={core_version}, bindings={bindings}):",
                file=sys.stderr,
            )
            for path in changed:
    ...
    print(
        f"Core {core_version}; binding packages {bindings} {action} "
        f"({len(changed)} changed file(s))"
    )
    print(
        f"Framework packages {frameworks} {action} "
