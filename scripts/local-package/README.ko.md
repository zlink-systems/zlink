...
binding을 같은 `0.10.1` 기준으로 package하는 경로다. 기본 동작은 GitHub의
`core/v0.10.1` release asset을 다운로드하고 checksum과 provenance를 확인한
...
release Core prefix는 기본적으로 `~/.cache/zlink/core/0.10.1/linux-x64/`에
...
기본 prefix는 `%LOCALAPPDATA%\zlink\core\0.10.1\windows-x64\`이다. 진행 중인
...
검증된 Core prefix의 `0.10.1` runtime과 public header를 binding 작업
