...
binding을 같은 `0.13.1` 기준으로 package하는 경로다. 기본 동작은 GitHub의
`core/v0.13.1` release asset을 다운로드하고 checksum과 provenance를 확인한
...
```text
.artifacts/wsl/install/zlink-core/0.13.1/
  include/
  lib/libzlink.so
  lib/libzlink.so.0
  lib/libzlink.so.0.13.1
  share/zlink/core-package-provenance.json
```

release Core prefix는 기본적으로 `~/.cache/zlink/core/0.13.1/linux-x64/`에
...
`0.13.1`은 release/package version이다. native runtime의 SONAME도 같은
release line에 맞춰 `libzlink.so.0`으로 생성한다. 외부 dependency의 버전은
이 정책의 대상이 아니다.

## binding별 출력

- C: `.artifacts/wsl/c/zlink-c-0.13.1.tar.gz`
- C++: `.artifacts/wsl/install/zlink-cpp/0.13.1/`
- .NET: `.artifacts/wsl/nuget/Systems.Zlink.0.13.1.nupkg`
- Go: `.artifacts/wsl/go/zlink-go-0.13.1.tar.gz`
- Java: `.artifacts/wsl/maven/systems/zlink/zlink/0.13.1/`
- Node.js: `.artifacts/wsl/npm/zlink-systems-zlink-0.13.1.tgz`
- Python: `.artifacts/wsl/python/zlink-0.13.1-*.whl` 및 source archive
- Rust: `.artifacts/wsl/rust/zlink-0.13.1.crate`
...
`0.13.1` runtime과 public header를 사용한다.
...
기본 prefix는 `%LOCALAPPDATA%\zlink\core\0.13.1\windows-x64\`이다. 진행 중인
...
검증된 Core prefix의 `0.13.1` runtime과 public header를 binding 작업
