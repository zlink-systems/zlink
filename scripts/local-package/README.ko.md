...
같은 `0.10.0` source 기준으로 빌드하는 경로다. 기본 출력은
...
```text
.artifacts/wsl/install/zlink-core/0.10.0/
  include/
  lib/libzlink.so
  lib/libzlink.so.0
  lib/libzlink.so.0.10.0
  share/zlink/core-package-provenance.json
```

`0.10.0`은 release/package version이다. native runtime의 SONAME도 같은
release line에 맞춰 `libzlink.so.0`으로 생성한다. 외부 dependency의 버전은
이 정책의 대상이 아니다.

## binding별 출력

- C: `.artifacts/wsl/c/zlink-c-0.10.0.tar.gz`
- C++: `.artifacts/wsl/install/zlink-cpp/0.10.0/`
- .NET: `.artifacts/wsl/nuget/Systems.Zlink.0.10.0.nupkg`
- Go: `.artifacts/wsl/go/zlink-go-0.10.0.tar.gz`
- Java: `.artifacts/wsl/maven/systems/zlink/zlink/0.10.0/`
- Node.js: `.artifacts/wsl/npm/zlink-systems-zlink-0.10.0.tgz`
- Python: `.artifacts/wsl/python/zlink-0.10.0-*.whl` 및 source archive
- Rust: `.artifacts/wsl/rust/zlink-0.10.0.crate`
