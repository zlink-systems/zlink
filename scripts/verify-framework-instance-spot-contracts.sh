...
const projections = {
  dotnet: [
    ...
    '`IZLinkSpotManager`는 User Spot의 명시적 create·get-or-create, resolve와 close만 제공한다.',
  ...
  cpp: [
    ...
    '`instance_spot_context_t::close()`가 context에 보관한 current SpotRef로 local close를 수행한다.',
  ...
  kotlin: [
    ...
    '`SpotRef(spotId, objectGeneration, meshName, nodeRid)`는 지정한 incarnation을 close할 때만',
