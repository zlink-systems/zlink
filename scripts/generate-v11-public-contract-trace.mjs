...
const validateBlockRoleOverrides = config => {
  for (const rule of config.blockRoleOverrides || []) {
    ...
    const matches = currentBlocks.filter(block => block.heading === rule.heading
      && block.tag === rule.tag
      && block.headingOrdinal === rule.headingOrdinal);
    if (matches.length !== 1) {
      throw new Error(`block role override is duplicate or stale: ${rule.document}`
        + ` heading=${rule.heading} tag=${rule.tag} headingOrdinal=${rule.headingOrdinal}`);
    }
  }
};
...
const buildTrace = (config, {refreshReview = false} = {}) => {
  validateBlockRoleOverrides(config);
  ...
  const blockOverrideKey = value => `${value.document}\0${value.heading}\0${value.tag}\0${value.headingOrdinal}`;
  const overrideKeys = new Set(config.blockRoleOverrides?.map(blockOverrideKey));
  const observedOverrideKeys = new Set(codeBlocks
    .filter(block => overrideKeys.has(blockOverrideKey(block)))
    .map(blockOverrideKey));
