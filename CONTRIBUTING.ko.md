...
- 소스 줄바꿈과 포매터: [`doc/principal/dev/source-formatting.ko.md`](doc/principal/dev/source-formatting.ko.md).
  ...
  규칙이다. `scripts/format/format.sh [--check]`로 실행하며, 현재 범위는 framework quickstart·tutorial·samples다.
...
- framework의 기본 빌드·솔루션·CI는 `cross-language` e2e만 포함한다. 7개 샘플(`samples/*`)은
  샘플별 `run_sample.sh`·Node `npm run test:samples`로만 돌리며 CI 솔루션 필터
  (`Zlink.Framework.ci.slnf`)·CMake 기본 타깃·Gradle 루트 빌드·CI·배포에 넣지 않는다. 언어별 구성,
  샘플 두 모드, bindings 참조 스위치, C++ preset은
...
- **작업 진행 방식(2026-09-10부터)**: 모든 작업은 GitHub Issue로 등록하고, Issue마다 브랜치
  ...
  (문서만 바꾸는 수정도 같다. 예외는 릴리스 태그뿐이다). Milestone은 릴리스, Project 보드는
