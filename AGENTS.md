...
- **`luna`가 기본이다.** 변경 지점이 정해진 구현, 검증 실행, 문서 대조, 정해진 패턴 적용은
  전부 `luna`로 시작한다. 더 높은 수준이 필요한 일 — 여러 파일에 걸친 설계 판단, 전수 조사 —
  에만 `terra`. **`sol`은 고차원 작업에만** — 모호한 진단, 계약·수명·에러 경로가 걸린 구현.
  올리는 것은 측정된 실패가 있을 때만이고, 지레 올리지 않는다(2026-09-19: 한도 소진으로 작업자
  여섯이 동시에 멈춘 뒤 보수적으로 조정).
- **추론 레벨의 시작점은 등급마다 다르다.** `luna`·`terra`는 `high`에서 시작한다 — 이 등급은
  레벨을 올려야 판단이 선다. `sol`은 `medium`에서 시작한다 — 모델이 크므로 medium으로 충분한
  경우가 많고, `high`·`xhigh`는 측정된 실패가 있을 때만 올린다.
- **`astra`는 구현에 쓰지 않는다.** 입력이 `sol`의 2배, 출력 1.67배이고 272K 토큰을 넘으면 요금제가
  또 바뀐다(입력 $20, 출력 $75). `sol`이 막힌 작업은 브리프를 좁혀 다시 `sol`로 보내거나
  감독자가 직접 한다. 예외는 **큰 변경의 최종 읽기 전용 리뷰 한 번**이다(2026-09-20 #736 실측:
...
- **수정 뒤 codex `sol` 리뷰를 거쳐 확정한다.** 스펙 diff만 대상으로 읽기 전용 리뷰 job을 `gpt-5.6-sol`로
