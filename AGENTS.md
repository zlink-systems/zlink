...
모델 id는 `gpt-6-<등급>`이다(ChatGPT 계정 실측 2026-09-23, codex-cli 0.155.1: `gpt-6-sol`·`gpt-6-luna`는
동작하고 `gpt-6-terra`는 "not supported when using Codex with a ChatGPT account"로 400이 난다).

| 모델 | 모델 id | 쓰는 일 |
|------|---------|---------|
| `astra` | `gpt-6-astra` | 큰 변경의 최종 읽기 전용 리뷰 한 번(아래) |
| `sol` | `gpt-6-sol` | 고차원 작업. 모호한 진단, 여러 파일에 걸친 설계 판단, 계약·수명·에러 경로가 걸린 구현, 리뷰 |
| `luna` | `gpt-6-luna` | 기본. 변경 지점이 정해진 구현, 검증 실행, 문서 대조, 정해진 패턴 적용, 전수 조사 |

- **`luna`가 기본이다.** 판단이 필요한 일에만 `sol`로 올리고, 올리는 것은 측정된 실패가 있을 때만이다.
  지레 올리지 않는다(2026-09-19: 한도 소진으로 작업자 여섯이 동시에 멈춘 뒤 보수적으로 조정).
- **추론 레벨의 시작점은 등급마다 다르다.** `luna`는 `high`에서 시작한다 — 이 등급은 레벨을 올려야
  판단이 선다. `sol`은 `medium`에서 시작하고, `high`·`xhigh`는 측정된 실패가 있을 때만 올린다.
- **`astra`는 구현에 쓰지 않는다.** 가장 비싼 등급이고 272K 토큰을 넘으면 요금제가 또 바뀐다.
  `sol`이 막힌 작업은 브리프를 좁혀 다시 `sol`로 보내거나 감독자가 직접 한다. 예외는 **큰 변경의 최종 읽기 전용 리뷰 한 번**이다(2026-09-20 #736 실측:
  sol 리뷰 4회가 승인한 diff에서 astra가 산출물을 메모리에서 변환해 경계 입력을 재현하는 방식으로
  실행 가능한 결함 9건을 찾았다 — 입력 145K 토큰). sol 리뷰가 먼저 승인한 뒤에만 넣고,
  브리프에 "sol이 놓친 것"만 찾으라고 적는다.
- **캐시된 입력은 훨씬 싸다.** 연관된 job은 브리프 앞부분(원칙·필독 문서 목록)을 동일하게
