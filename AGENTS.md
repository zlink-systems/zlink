...
- **교차언어 대조(필수).** Framework runtime 동작을 바꾸기 전에 같은 상황을 다른 언어 구현이 어떻게
  ...
  증상인지 완료 보고에 적는다.
- **가장 단순한 근본 수정(필수).** 원인이 확정되면 그 소유 계층에서 가장 단순한 변경만 한다. 새 상태·타이머·인덱스·헬퍼
  계층·옵션·설정값을 추가하는 수정은 기존 코드 삭제로 같은 결과를 낼 수 없을 때만 허용하며, 그 근거를 완료 보고에 적는다.
  두 대안이 있으면 코드가 줄어드는 쪽을 고른다. 같은 규칙·상태·헬퍼·매핑표를 두 곳에 두지 않는다(중복 금지):
  이미 있는 소유자를 재사용하고, 새로 만들어야 하면 기존 것을 그 자리로 옮겨 하나만 남긴다.
